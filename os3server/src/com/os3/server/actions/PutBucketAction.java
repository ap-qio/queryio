...
public class PutBucketAction extends BaseAction {
	...
	public void execute(String operation, HttpServletRequest request, HttpServletResponse response, Map<String, Object> helperMap, int apiType) throws ServletException, IOException {
		...
		else
		{
			...
			if(permission!=null) {
				try {
					status = DataManager.createBucket(dfs, bucketName, bucketOwner, bucketGroup, Short.parseShort(permission));
				} catch(NumberFormatException e) {
					ErrorResponseWriter.invalidArgument(helperMap, response, bucketName, requestId, apiType);
					return;
				} catch(AccessControlException e) {
					ErrorResponseWriter.permissionDenied(helperMap, response, bucketName, e.getLocalizedMessage(), requestId, apiType);
					return;
				}
			} else {
				try {
					...
					ErrorResponseWriter.permissionDenied(helperMap, response, bucketName, e.getMessage(), requestId, apiType);
					return;
				}
			}
			...
			if(status)
			{
				ResponseWriter.handleResponseStatus(response, HttpServletResponse.SC_OK, apiType, requestId, OS3Constants.TEXT_HTML);
				//Add location header
