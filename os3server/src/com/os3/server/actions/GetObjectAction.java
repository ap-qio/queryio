		...
		if (!DataManager.doesBucketExists(dfs, bucketName)) {
			...
			if (ifModifiedSince != null) {
				...
				try {
					...
					logger.error("RequestId" + requestId + ". Error Parsing " + OS3Constants.IF_MODIFIED_SINCE
							+ " request header with value " + ifModifiedSince, pe);
			...
			if (ifUnModifiedSince != null) {
				...
				try {
					...
					logger.error("RequestId" + requestId + ". Error Parsing " + OS3Constants.IF_UNMODIFIED_SINCE
							+ " request header with value " + ifUnModifiedSince, pe);
					ErrorResponseWriter.invalidArgument(helperMap, response, objectName, requestId, apiType);
					return;
				}
			}
			...
			try {
				inputStream = DataManager.getObjectDataInputStream(dfs, bucketName, objectName, compressionType,
						encryptionType);
			...
			if (range != null) {
				...
				if (index == -1 || indexHyphen == -1) {
					logger.error("RequestId" + requestId + ". Error Parsing " + OS3Constants.RANGE
							+ " request header with value " + range);
					ErrorResponseWriter.invalidArgument(helperMap, response, objectName, requestId, apiType);
					return;
				}
				...
				try {
					...
					logger.error("RequestId" + requestId + ". Error Parsing lowerbound for bytes range in "
							+ OS3Constants.RANGE + " request header with value " + range);
					ErrorResponseWriter.invalidArgument(helperMap, response, objectName, requestId, apiType);
					return;
				}
				...
				try {
					...
					logger.error("RequestId" + requestId + ". Error Parsing higherBound for bytes range in "
							+ OS3Constants.RANGE + " request header with value " + range);
					ErrorResponseWriter.invalidArgument(helperMap, response, objectName, requestId, apiType);
					return;
				}
			...
			try {
				...
				ResponseWriter
						.writeGetObjectResponse(request, response, apiType, requestId, inputStream, fs.getLen(),
								fs.getModificationTime(), checkSum, lowerBound, higherBound, range,
								fetchMD, fetchMD
										? DataManager.getObjectMetadata(
												new Path(DFSManager.ROOT_PATH + bucketName, objectName).toString())
										: null);
			} finally {
				if (inputStream != null) {
					try {
