		...
		if(!DataManager.doesBucketExists(dfs, bucketName)){
			...
			if(deflate){
				...
				try{
					...
					while (entry != null) {
						...
						if ( ! entry.isDirectory()) {
							if(permission==null) {
								try {
									status = DataManager.createObject(bucketOwner, bucketGroup, dfs, bucketName, objectName.substring(0,objectName.lastIndexOf(File.separator)+1)+fileName, contentLen, stream, getAllTags(request), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_COMPRESSION), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_ENCRYPTION));
								...
								try {
									status = DataManager.createObject(bucketOwner, bucketGroup, permissionValue, dfs, bucketName, objectName.substring(0,objectName.lastIndexOf(File.separator)+1)+fileName, contentLen, stream, getAllTags(request), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_COMPRESSION), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_ENCRYPTION));
								} catch(AccessControlException e) {
									ErrorResponseWriter.permissionDenied(helperMap, response, objectName, e.getMessage(), requestId, apiType);
									return;
								}
				...
				try{
					...
					if(permission==null) {
						try {
							status = DataManager.createObject(bucketOwner, bucketGroup, dfs, bucketName, objectName, contentLen, new BufferedInputStream(stream), getAllTags(request), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_COMPRESSION), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_ENCRYPTION));
						} catch(AccessControlException e) {
							ErrorResponseWriter.permissionDenied(helperMap, response, objectName, e.getLocalizedMessage(), requestId, apiType);
							return;
						}
						...
						try {
							status = DataManager.createObject(bucketOwner, bucketGroup, permissionValue, dfs, bucketName, objectName, contentLen, new BufferedInputStream(stream), getAllTags(request), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_COMPRESSION), request.getHeader(OS3Constants.X_OS3_AMZ_SERVER_SIDE_ENCRYPTION));
						} catch(AccessControlException e) {
							ErrorResponseWriter.permissionDenied(helperMap, response, objectName, e.getMessage(), requestId, apiType);
							return;
						}
			...
			if( ! deflate){
				if(contentLen != status.getSize()){
					//Delete whatever we wrote and send incomplete body response to caller
