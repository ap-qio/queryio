...
public class ExecuteBigQueryAction extends BaseAction {
	...
	public void execute(String operation, HttpServletRequest request, HttpServletResponse response, Map<String, Object> helperMap, int apiType) throws Exception {
		...
		if(format.equals("PLAIN")){
			...
			ResponseWriter.writeBigQueryResponse(dfs, apiType, response, requestId, sqlQuery, bigQueryRequest.getBigQueryResult(), bigQueryRequest.isSuccess());
		} else {
			if (queryId == null) {
			...
			try{
				...
				ErrorResponseWriter.writeErrorResponse(helperMap, response.getWriter(), IErrorConstants.ERR_REPORT_CREATION,e.getMessage(), null, requestId);
			}
		}
	}
}
