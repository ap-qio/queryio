...
public class HeadObjectAction extends BaseAction {
	...
	public void execute(String operation, HttpServletRequest request, HttpServletResponse response,
		...
		if (!DataManager.doesBucketExists(dfs, bucketName)) {
			...
			ResponseWriter.writeHeadObjectResponse(request, response, apiType, requestId, fs.getLen(),
					fs.getModificationTime(), checkSum, fetchMD, fetchMD ? DataManager.getObjectMetadata(
							new Path(DFSManager.ROOT_PATH + bucketName, objectName).toString()) : null);
		}
	}
}
