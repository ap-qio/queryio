...
public class StartupListener implements ServletContextListener {
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		try {
			...
			if (logger.isDebugEnabled())
				logger.debug(event.getServletContext().getServletContextName() + " context initialized.");
			...
			OS3Constants.poolName = HadoopConstants.getHadoopConf().get(QueryIOConstants.CUSTOM_TAG_DB_DBSOURCEID, "");
			...
			System.setProperty(OS3Constants.SUN_SECURITY_KRB5_DEBUG,
					Settings.get(OS3Constants.SUN_SECURITY_KRB5_DEBUG));
			System.setProperty(OS3Constants.JAVA_SECURITY_KRB5_REALM,
					Settings.get(OS3Constants.JAVA_SECURITY_KRB5_REALM));
			System.setProperty(OS3Constants.JAVA_SECURITY_KRB5_KDC, Settings.get(OS3Constants.JAVA_SECURITY_KRB5_KDC));
			System.setProperty(OS3Constants.JAVA_SECURITY_AUTH_LOGIN_CONFIG,
					event.getServletContext().getRealPath(OS3Constants.JAAS_CONF));
			System.setProperty(OS3Constants.JAVAX_SECURITY_AUTH_USESUBJECTCREDSONLY,
					Settings.get(OS3Constants.JAVAX_SECURITY_AUTH_USESUBJECTCREDSONLY));

			logger.info(
					OS3Constants.SUN_SECURITY_KRB5_DEBUG + " : " + Settings.get(OS3Constants.SUN_SECURITY_KRB5_DEBUG));
			logger.info(OS3Constants.JAVA_SECURITY_KRB5_REALM + " : "
					+ Settings.get(OS3Constants.JAVA_SECURITY_KRB5_REALM));
			logger.info(
					OS3Constants.JAVA_SECURITY_KRB5_KDC + " : " + Settings.get(OS3Constants.JAVA_SECURITY_KRB5_KDC));
			logger.info(OS3Constants.JAVA_SECURITY_AUTH_LOGIN_CONFIG + " : "
					+ event.getServletContext().getRealPath(OS3Constants.JAAS_CONF));
			logger.info(OS3Constants.JAVAX_SECURITY_AUTH_USESUBJECTCREDSONLY + " : "
					+ Settings.get(OS3Constants.JAVAX_SECURITY_AUTH_USESUBJECTCREDSONLY));
			...
			if (Settings.getBoolean(OS3Constants.USE_THREAD_POOL)) {
				// Creation of a global async Executor with CallerRunsPolicy. If
				// Queue is full then calling thread will execute the task.
				final ThreadPoolExecutor executor = new ThreadPoolExecutor(
						Settings.getIntProperty(OS3Constants.CORE_POOL_SIZE, OS3Constants.DEFAULT_CORE_POOL_SIZE),
						Settings.getIntProperty(OS3Constants.MAXIMUM_POOL_SIZE, OS3Constants.DEFAULT_MAXIMUM_POOL_SIZE),
						Settings.getIntProperty(OS3Constants.KEEP_ALIVE_TIME, OS3Constants.DEFAULT_KEEP_ALIVE_TIME),
						TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(Settings
								.getIntProperty(OS3Constants.QUEUE_CAPACITY, OS3Constants.DEFAULT_QUEUE_CAPACITY)),
	...
	public void contextDestroyed(ServletContextEvent event) {
		if (Settings.getBoolean(OS3Constants.USE_THREAD_POOL)
				&& event.getServletContext().getAttribute(OS3Constants.ASYNC_REQUEST_EXECUTOR) != null) {
			((ThreadPoolExecutor) event.getServletContext().getAttribute(OS3Constants.ASYNC_REQUEST_EXECUTOR))
					.shutdown();
			event.getServletContext().removeAttribute(OS3Constants.ASYNC_REQUEST_EXECUTOR);
		}
		if (logger.isDebugEnabled())
			logger.debug(event.getServletContext().getServletContextName() + " context destroyed.");
	}
}