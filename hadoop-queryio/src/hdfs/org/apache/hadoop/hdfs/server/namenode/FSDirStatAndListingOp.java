     ...
     if (node.isFile()) {
       ...
       if(fileNode.getBlocks() != null){
    	   ...
    	   for(int i = 0; i < fileNode.getBlocks().length; i ++){
    		   blocks[i] = new Block(fileNode.getBlocks()[i].getBlockId(), fileNode.getBlocks()[i].getNumBytes(), fileNode.getBlocks()[i].getGenerationStamp());
    	   }
       } else {
  ...
  private static HdfsLocatedFileStatus createLocatedFileStatus(
    ...
    HdfsLocatedFileStatus status =
        new HdfsLocatedFileStatus(size, node.isDirectory(), replication,
          ...
          node.getId(), loc, childrenNum, feInfo, storagePolicy, /*@QUERYIO@*/null);
