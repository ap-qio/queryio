...
public class ProcManager {
	...
	public static String startUpCommand = COMMAND_START + APP_HOME + SCRIPT_FILE_NAME + " > " + APP_HOME
			+ LOG_FILE_NAME;
	...
	public static String execSetCrontabScript() throws IOException {
		String cmd = "sh " + getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/"
				+ SET_CRONTAB_SCRIPT_FILE_NAME;

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("execSetCrontabScript cmd: " + cmd);
	...
	public static String getCrontabFileContentWithoutAgentCommand() throws IOException {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("getCrontabFileContent cmd: " + cmd);
		...
		try {
			...
			String s = null;
			while ((s = stdInput.readLine()) != null) {
				if (s.equals(ProcManager.getStartUpCommandWithoutInterval()))
					continue;
	...
	public static void clearCrontabFile() throws IOException {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("clearCrontabFile cmd: " + cmd);
	...
	public static void updateCrontabFile(String startUpCommand) throws IOException {
		...
		File configFile = new File(
				getInstallDir() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/namenode_conf/topologyConfig.sh");
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Crontab file content: " + content);

		file = new File(
				getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/" + SET_CRONTAB_SCRIPT_FILE_NAME);
		file.delete();
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Response from set crontab command: " + response);
	}

	public static void createAgentStatusScriptFile() throws IOException {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("createAgentStatusScriptFile() called");

		BufferedWriter out = null;

		try {
			FileWriter fstream = new FileWriter(
					getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/" + SCRIPT_FILE_NAME);
			...
			sBuf.append("cd $1");
			sBuf.append("\n");
			sBuf.append("sh start_queryIOAgent.sh " + getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + " "
					+ System.getProperty(QueryIOConstants.QUERYIOAGENT_PORT));
			sBuf.append("\n");
			sBuf.append("fi");
	...
	public static void createSetCrontabScriptFile(String startUpCommand) throws IOException {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("createSetCrontabScriptFile() called");
	...
	public static void addAgentStartUpMonitor() throws IOException {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Will fetch startUpCommand now.");
		String startUpCommand = ProcManager.getStartupCommand();

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("startUpCommand: " + startUpCommand);
	...
	public static int getMonitorInterval(String key) {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("getMonitorIntervalMethod, with key :" + key);

		Configuration conf = new Configuration(false);

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Mark : 1");
		String fileName = getMonitorConfigFile();
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Mark : 2");
		FileInputStream fis = null;
		// String value = null;
		try {
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Mark : 3");
			fis = new FileInputStream(new File(fileName));
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Mark : 4");
			conf.addResource(fis);
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Mark : 5");
			if (conf.get(key) == null)
				return -1;
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Mark : 6");
	...
	public static int getAgentMonitorIntervalInMinutes() {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("fetching agent monitoring intervalImpl");
		int monitorInterval = getMonitorInterval(QueryIOConstants.CONTROLLER_AGENT_MONITOR_INTERVAL_KEY);
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("fetched agent monitoring intervalImpl");
	...
	public static boolean setAgentMonitorInterval(int intervalInMinutes) throws IOException {
		boolean success = updateMonitorIntervalInFile(QueryIOConstants.CONTROLLER_AGENT_MONITOR_INTERVAL_KEY,
				intervalInMinutes);
	...
	public static String getStartupCommand() {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("fetching agent monitoring interval");
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Agent Monitor Interval: " + agentMonitorInterval);
		...
		if (agentMonitorInterval != 0) {
			...
			if (cmdStart.lastIndexOf(",") == (cmdStart.length() - 1))
				cmdStart = cmdStart.substring(0, cmdStart.length() - 1);
		...
		String startUpCommand = cmdStart + getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/"
				+ SCRIPT_FILE_NAME + " " + getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/ "
				+ agentProt + " > " + APP_HOME + LOG_FILE_NAME;

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("StartUp Command: " + startUpCommand);

		return startUpCommand;
	}

	public static String getStartUpCommandWithoutInterval() {
		return getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/" + SCRIPT_FILE_NAME + " "
				+ getInstallDir() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/bin/ > " + APP_HOME + LOG_FILE_NAME;
	}

	public static String getPID(String procName) {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Fetching PID for process : " + procName);
		String procPID = QueryIOConstants.EMPTY_STRING;
