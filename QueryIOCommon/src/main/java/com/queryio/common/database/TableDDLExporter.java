...
public class TableDDLExporter {
	...
	// return (rowsToMigrate == 0) ? 100.0 : (rowsMigrated * 100 /
	// rowsToMigrate);
	// }
	...
	private void initWriter() throws Exception {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("EnvironmentalConstants.getAppHome()" + EnvironmentalConstants.getAppHome());
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("sourceDBName" + sourceDBName);
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("schemaName" + schemaName);
	...
	private void getDDLStatementsForDatabase() throws Exception {
		...
		closeWriter();
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("File path of the generated DDL statements" + filePath);
	}
	...
	private void getDDLForTable(String tableName) throws Exception {
		...
		psQuery.append(')');
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("DDL statements Tables: " + query.toString());
		...
		if (temp != null) {
			...
			for (int k = 0; k < temp.size(); k++) {
				...
				query.append(")");
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("DDL statements Indexes: " + query.toString());
				writeToFile(query.toString());
			}
