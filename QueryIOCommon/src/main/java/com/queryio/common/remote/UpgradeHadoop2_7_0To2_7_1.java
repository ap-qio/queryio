...
public class UpgradeHadoop2_7_0To2_7_1 {
	...
	public static void updateHadoopPath(String path) throws Exception {
		...
		for (File file : f.listFiles()) {
			if (file.getPath().contains("core-site.xml") || file.getPath().contains("hdfs-site.xml")
					...
					|| file.getPath().contains("capacity-scheduler.xml")) {
		...
		try {
			if (in.exists()) {
				...
				while ((line = reader.readLine()) != null) {
					...
					if (isHive && line.contains(QueryIOConstants.HIVE_1_2_0_DIR_NAME))
						line = line.replace(QueryIOConstants.HIVE_1_2_0_DIR_NAME, QueryIOConstants.HIVE_DIR_NAME);
					if (line.contains(QueryIOConstants.RM_RESOURCE_CALC))
						line = line.replace(QueryIOConstants.RM_RESOURCE_CALC,
								QueryIOConstants.RM_RESOURCE_CALC_UPDATE);

					if (line.trim().startsWith("export YARN_HOME=")) {
						line = line.replace("export YARN_HOME=", "export HADOOP_YARN_HOME=");
					}

					// With hadoop 2.7.1 we have started using hive 1.2.1. Hive
					// JDBC Driver class and hive JDBC url has been updated in
					// this version
					// 1. JDBC Driver org.apache.hadoop.hive.jdbc.HiveDriver =>
					// org.apache.hive.jdbc.HiveDriver
					// 2. JDBC URL jdbc:hive://localhost:10000/default =>
					// jdbc:hive2://localhost:10000/default

					if (line.trim().contains("org.apache.hadoop.hive.jdbc.HiveDriver")) {
						line = line.replace("org.apache.hadoop.hive.jdbc.HiveDriver",
								"org.apache.hive.jdbc.HiveDriver");
					}
				...
				for (int i = 0; i < lines.size(); i++) {
					...
					AppLogger.getLogger().debug("Writing : " + (String) lines.get(i));
					writer.write("\n");
				}
			}
		} finally {
