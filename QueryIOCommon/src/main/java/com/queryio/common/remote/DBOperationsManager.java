...
public class DBOperationsManager {
	...
	public static void startDBToFileMigration(final String migrationID, final String dbName, final String backupFolder)
			throws Exception {
		Thread thread = new Thread(migrationID) {
			public void run() {
				...
				try {
					DBBackupTools.databaseToFile(dbName, new Path(backupFolder, "ns-metadata.xml").toString(),
							new Path(backupFolder, "hdfs-metadata.xml").toString(), null);

					updateDBMigrationStatus(migrationID, new Timestamp(System.currentTimeMillis()),
							QueryIOConstants.PROCESS_STATUS_COMPLETED, null);
				} catch (Exception e) {
					AppLogger.getLogger().fatal(e.getMessage(), e);
					updateDBMigrationStatus(migrationID, new Timestamp(System.currentTimeMillis()),
							QueryIOConstants.PROCESS_STATUS_FAILED, e.getMessage());
				}
