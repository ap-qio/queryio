...
{
	public static void addRestoreInfo(final Connection connection, String restoreId, String migrationId, String nameNodeId) throws Exception {
	...
	public static void updateRestoreInfo(final Connection connection, String restoreId,
			Timestamp endTime, String status) throws Exception {
		PreparedStatement ps = null;
		try
		{
	...
	public static void deleteRestoreInfoFromList(final Connection connection, String restoreIds) throws Exception {
		...
		try
		{
			...
			deleteQuery = "DELETE FROM " + TableConstants.TABLE_NN_RESTORE_STATUS + " WHERE " + ColumnConstants.COL_NN_RESTORE_STATUS_RESTOREID+ " IN (" + restoreIds + ")";
	...
	public static ArrayList<NNRestoreInfo> getRestoreInfoList(final Connection connection) throws Exception {
		...
		try {
			...
			rs = DatabaseFunctions.getQueryResultsForStatement(stmt, "SELECT * FROM " + TableConstants.TABLE_NN_RESTORE_STATUS);
	...
	public static ArrayList<NNRestoreInfo> getRestoreInfoSelectedList(final Connection connection, String restoreIds) throws Exception {
		ArrayList<NNRestoreInfo> list = new ArrayList<NNRestoreInfo>();
		...
		try {
			stmt = DatabaseFunctions.getStatement(connection);
			selectQuery = "SELECT * FROM " + TableConstants.TABLE_NN_RESTORE_STATUS+ " WHERE " + ColumnConstants.COL_NN_RESTORE_STATUS_RESTOREID + " IN (" + restoreIds + ")";
