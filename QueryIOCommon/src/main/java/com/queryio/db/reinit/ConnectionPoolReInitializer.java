...
public class ConnectionPoolReInitializer {
	...
	public static void reInitializeDB(String driverName, String jarFile, String userName, String password,
			String connectionURL, String connectionName, String currentConnectionName, boolean isCustomTagDB)
			throws Exception {
		try {
			if (isCustomTagDB) {
				for (int i = 0; i < CustomTagDBConfigManager.getAllCustomTagDBConfigList().size(); i++) {
					...
					if (config != null) {
						...
						config.setCustomTagDriverJarPath(
								EnvironmentalConstants.getJdbcDriverPath() + File.separator + jarFile);
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Re Initializing DB");
		} catch (Exception e) {
			throw e;
		}
