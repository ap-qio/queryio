...
public class DiskManager {
	...
	public static HashMap getPhysicalDiskMapMacOS() throws IOException {
		...
		for (int index = 0; index < size; index++) {
			...
			for (int j = 0; j < list.size(); j++) {
				if ((((String) ((Map.Entry) list.get(j)).getValue()).split(File.separatorChar == '\\' ? "\\\\"
						: File.separator).length) > (((String) ((Map.Entry) list.get(maxIndex)).getValue())
								.split(File.separatorChar == '\\' ? "\\\\" : File.separator).length)) {
					maxIndex = j;
				}
			}

			linkedMap.put(((String) ((Map.Entry) list.get(maxIndex)).getKey()),
					((String) ((Map.Entry) list.get(maxIndex)).getValue()));
	...
	public static boolean validateVolumeDiskMapping(HashMap physicalDiskMap, String disk, String volume)
			throws IOException {
	...
	public static ArrayList validateVolumeDiskMapping(ArrayList disks, ArrayList volumes) {
		...
		try {
			...
			for (int i = 0; i < disks.size(); i++) {
				validationResult.add(DiskManager.validateVolumeDiskMapping(physicalDiskMap, (String) disks.get(i),
						(String) volumes.get(i)));
			}
		} catch (Exception e) {
			AppLogger.getLogger().fatal(e.getMessage(), e);
		}
