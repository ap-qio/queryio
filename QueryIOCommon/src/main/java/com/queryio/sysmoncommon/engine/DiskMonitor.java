...
public class DiskMonitor {
	...
	public static String getDiskHealthStatistics() {
		...
		/*
		 * Arraylist failureHardDiskDetails will contain each hard disk's
		 * information which is imminent to failure. String array will have the
		 * values related to the particular hard disk String [DEVICE_ID] =
		 * Device ID String [DISK_SERIAL_NUMBER] = Disk serial number String
		 * [MODEL_NUMBER] = Model number String [DISK_CAPACITY] = Disk capacity
		 * String [BAD_SECTOR_COUNT] = Bad Sector count String [MOUNT_INFO] =
		 * Mounting information
		 */

		// ArrayList<String []> failureHardDiskDetails = new
		// ArrayList<String[]>();
			...
			if (hardDiskNames != null && hardDiskNames.size() > 0) {
				...
				for (int i = 0; i < hardDiskNames.size(); i++) {
					...
					try {
						...
						inputStreamStatus = new BufferedReader(
								new InputStreamReader(healthStatusProcess.getInputStream()));
						...
						while ((line = inputStreamStatus.readLine()) != null) {
							...
							if (line.startsWith("SMART overall-health self-assessment")) {
								/*
								 * getFailureDiskDetail(hardDiskNames.get(i),
								 * failureHardDiskDetails);
								 */
