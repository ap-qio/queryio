...
public class WindowsMonitor {
	...
	public static void main(final String[] args) {
		...
		try {
			final MonitorWindowsServiceWrapper mwRequest = new MonitorWindowsServiceWrapper(
					IOSProtocolConstants.LOOPBACKADDRESS, UniqueIdGenerator.generateUniqueId("controllerId"));
			...
			// if (flag)
			{
					...
					if (nodes != null) {
						for (int i = 0; i < nodes.length; i++) {
							...
							case 1: {
								// Total virtual memory
								System.out.println(
										"Total virtual memory: " + (int) (nodes[i].getValue() / (1000 * 1000)));
								break;
							}
							case 2: {
								// Used virtual memory
								System.out
										.println("Used virtual memory: " + (int) (nodes[i].getValue() / (1000 * 1000)));
								break;
							}
							case 3: {
								// Page faults per sec
				...
				// AttributeNode[] node = mwRequest.fetchAttributes(new String[]
				// {"PhysicalDisk"});
