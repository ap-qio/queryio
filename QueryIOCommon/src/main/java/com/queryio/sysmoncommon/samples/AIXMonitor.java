...
public class AIXMonitor {
	...
	public static void main(final String[] args) {
		...
		final String hostName = getValue(args, 1, "192.168.1.29");
		final String userName = getValue(args, 2, "manoj");
		final String pwd = getValue(args, 3, "admin");
		...
		try {
			...
			System.out.println("Creating monitor for target AIX machine.");
			final UnixMonitor monitor = new UnixMonitor(hostName, IOSProtocolConstants.AIX);
			...
			System.out.println("AIX monitor has been created, will ping the host now");
			...
			for (int i = 0; i < 2; i++) {
				...
				System.out.println("Used Virtual Memory: " + monitor.getVirtualMemoryInfo().getUsed());
				...
				for (int i1 = 0; i1 < drvInfo.length; i1++) {
					System.out.println("Drive: " + drvInfo[i1].getName() + ", Used space: " + drvInfo[i1].getUsedSpace()
							+ ", Total space: " + drvInfo[i1].getTotalSpace());
				}
				...
				for (int i1 = 0; i1 < processes.length; i1++) {
					System.out.println(processes[i1].getName() + " Memory:" + processes[i1].getMemoryUsage() + " CPU:"
							+ processes[i1].getProcessorTime());
				}
			}
			monitor.disconnect();
