...
abstract class AbstractUnixNativeAPI implements IUnixNativeAPI
{
	 ...
	 * @param authMethod 
	 */
	protected AbstractUnixNativeAPI(final String hostName, final String user, final String pwd, final String key, final int authMethod, final int port, final int iProtocol, 
		final ProcessFilter processFilter, final String fUserName, final String fPassword, final String superUserName, 
		final String superUserPassword, final boolean monitorProcess, final int interval)
	{
					...
					for (int j = 0; j < iSize; j++)
					{
							...
							&& (netInfo.getName()).equals(networkInfoArr[i].getName()))
						{
							if (networkInfoArr[i] instanceof UnixNetworkInfo && netInfo instanceof UnixNetworkInfo)
							{
								...
								((UnixNetworkInfo)netInfo).setTotalRecdPackets(((UnixNetworkInfo)networkInfoArr[i]).getTotalRecdPackets());
								((UnixNetworkInfo)netInfo).setTotalSentPackets(((UnixNetworkInfo)networkInfoArr[i]).getTotalSentPackets());
							}
							else
							{
