...
{
	//private final String PROCESSCOMMAND = "ps -eo pid,etime,time,vsz,thcount,user,command";
...
//	private final String DISKINFOCOMMAND = "iostat -k -d 1"; // "iostat -k -d 1 "; //
...
//	public LinuxNativeAPI(final String hostName, final String user, final String pwd, final int port, final int iProtocol)
//	{
//		this(hostName, user, pwd, port, iProtocol, null, null, null, null, null);
//	}
	
	public LinuxNativeAPI(final String hostName, final String user, final String pwd, final String key, final int authMethod, final int port, final int iProtocol, 
		final ProcessFilter processFilter, final String fUserName, final String fPassword,  final String superUserName, 
		final String superUserPassword, final boolean monitorProcess, final String ioDisks, final int refreshInterval)
	{
		...
		try
		{
			...
			{
				this.wrapperForTop = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.LINUX);
				if (processFilter == null)
				{
					this.wrapperForDisk = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.LINUX);
					this.wrapperForNetwork = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.LINUX);
					this.wrapperForDiskSpace = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.LINUX);
				}
			}
			setFirewallDetails(new AbstractProtocolWrapper [] {
				this.wrapperForTop, this.wrapperForDisk, this.wrapperForNetwork, this.wrapperForDiskSpace});
		}
	...
	public DiskInfo[] getPhysicalDiskInfo() throws Exception
	{
		...
		if (this.bFirstTimeDiskInfo)
		{
			if (this.wrapperForDisk != null)
			{
				command = this.iostatDisks == null ? "iostat -k -d " + this.refreshInterval : "iostat -k -d " + this.refreshInterval + " " + this.iostatDisks;
...
//					this.wrapperForTop.execute(this.TOPWITHPROCESSCOMMAND + " -p " + processFilter.pid, false);
				}
				else if(monitorProcesses)
				{
					command = "top -b -d "+this.refreshInterval;
					this.wrapperForTop.execute(command, true);
//					this.wrapperForTop.execute(this.TOPWITHPROCESSCOMMAND, false);
				}
				else
				{
