...
public class GenericAIXDataParser extends AbstractDataParser {
	...
	public void parseTopCommand(String output) throws ResultParsingException {
		/*
		 * COMMAND PID ELAPSED TIME VSZ THCNT USER init 1 29-05:53:32 00:00:49
		 * 588 1 root java 29318 3-05:44:22 00:06:37 60648 65 root
		 */
	...
	public void parseNetStatCommand(String output) throws ResultParsingException {
		 ...
		 * en0 1500 link#2 0.9.6b.2e.be.3c 251284 0 128748 0 0 en0 1500
		 * 192.168.1 ap-aix 251284 0 128748 0 0 lo0 16896 link#1 10095 0 11270 0
		 * 0 lo0 16896 127 localhost 10095 0 11270 0 0 lo0 16896 ::1 10095 0
		 * 11270 0 0 $
		 */
				...
				try {
					for (int i = 0; i < n; i++) {
						if (headers[i].equals("name")) {
							// do not read local loopback details OR network
							// card with same name has been read
							if (values[i].startsWith("lo") || readNetworks.contains(values[i])
									|| values[i].equals("$")) {
								break;
							}
							name = values[i];
	...
	public void parseIOStatCommand(String output) throws ResultParsingException {
		 ...
		 * 
		 * Disks: % tm_act Kbps tps Kb_read Kb_wrtn hdisk0 5.0 160.0 11.0 0 160
		 * cd0 0.0 0.0 0.0 0 0 $
		 */
	...
	public void parseDFCommand(String output) throws ResultParsingException {
		/*
		 * $ df -k Filesystem 1024-blocks Free %Used Iused %Iused Mounted on
		 * /dev/hd4 65536 38940 41% 1658 6% / /dev/hd2 1572864 412172 74% 24540
		 * 7% /usr /dev/hd9var 65536 57084 13% 381 3% /var /dev/hd3 131072
		 * 125688 5% 53 1% /tmp /dev/hd1 2097152 1161064 45% 243 1% /home /proc
		 * - - - - - /proc /dev/hd10opt 524288 249408 53% 3147 3% /opt $
		 */
	...
	public void parseVmstatCommand(String output) throws ResultParsingException {
		 ...
		 * ------------------------ ------------ ----------- r b avm fre re pi
		 * po fr sr cy in sy cs us sy id wa 1 1 154408 231439 0 0 0 0 0 0 32
		 * 1693 445 18 2 80 1
		 */
		this.fCpuUsageTime = 0;
		if (output != null && output.trim().length() > 0) // output will be null
															// if this method
															// gets called after
															// disconnect
		{
	...
	// parser.hashCode();
	// /*
	// com.queryio.sysmoncommon.sysmon.ProcessFilter processFilter = new
	// com.queryio.sysmoncommon.sysmon.ProcessFilter(false);
	...
	// parser.getVirtualMemoryInfo().getAvailable() + " total: " +
	// parser.getVirtualMemoryInfo().getTotal());
	// System.out.println("Physical Memory: total " +
	// parser.getPhysicalMemoryInfo().getTotal() + " free: " +
	// parser.getPhysicalMemoryInfo().getAvailable());
	// */
	// /*
	// // Processes
	...
	// System.out.println(processes[i].getName() + " " +
	...
	// {
	// System.out.println(nInfo[i].getName() + " " +
	// nInfo[i].getTotalRecdPackets() + " " + nInfo[i].getTotalSentPackets());
	// }
	// */
	// /*
	// // Drive Info
	...
	// //System.out.println(dInfo[i].getTotalSpace() + "\t" +
	// dInfo[i].getUsedSpace() + "\t" + dInfo[i].getName());
	// }
	// System.out.println("TotalSpace: " + totalSpace + " Used space: " +
	// totalUsedSpace);
	// */
	// /*
	// // Disk Info
	...
	// + " " +
	// dkInfo[i].getWritesPerSec() + " " +
	// ((AIXDiskInfo)dkInfo[i]).getTotalReads() + " " +
 ...
 * if (output != null) //output will be null if this method gets called after
 * disconnect { if(networkInfoList == null) { networkInfoList = new
 * LinkedList(); } else { networkInfoList.clear(); }
 * 
 * if(diskInfoList == null) { diskInfoList = new LinkedList(); } else {
 * diskInfoList.clear(); }
 * 
 * if (processInfoList == null) { processInfoList = new LinkedList(); } else {
 * processInfoList.clear(); } try { String sLine = null; StringTokenizer st =
 * new StringTokenizer(output, NEW_LINE_TOKENS); while (st.hasMoreTokens()) {
 * sLine = st.nextToken().trim(); //System.err.println(sLine);
 * if(sLine.trim().startsWith("Idle")) // Calculate CPU usage { StringTokenizer
 * values = new StringTokenizer(sLine); if(values.hasMoreTokens()) {
 * values.nextToken(); // skip first token, its for string "Idle"
 * if(values.hasMoreTokens()) { String temp = values.nextToken(); temp =
 * temp.trim(); float idleCpuTime = 0; try { idleCpuTime =
 * nf.parse(temp).floatValue(); } catch(Exception e) { idleCpuTime = 100; }
 * fCpuUsageTime = 100 - idleCpuTime; //System.out.println("fCpuUsageTime : " +
 * fCpuUsageTime); } } } if(sLine.startsWith("Network") ||
 * sLine.startsWith("Interf") || (sLine.indexOf("I-Pack") != -1 &&
 * sLine.indexOf("O-Pack") != -1)) // Calculate Network data {
 * //System.out.println("**********Network Info starts**********"); sLine =
 * st.nextToken(); StringTokenizer values = new StringTokenizer(sLine);
 * while(st.hasMoreTokens() && !sLine.startsWith("Disk") && values.countTokens()
 * >= 6) { String interfaceName = values.nextToken(); values.nextToken(); //
 * skip next token String temp = values.nextToken(); temp = temp.trim(); float
 * recvPackets = 0; try { recvPackets = nf.parse(temp).floatValue(); }
 * catch(Exception e) { recvPackets = 0; }
 * 
 * temp = values.nextToken(); temp = temp.trim(); float sentPackets = 0; try {
 * sentPackets = nf.parse(temp).floatValue(); } catch(Exception e) { sentPackets
 * = 0; } networkInfoList.add(new UnixNetworkInfo(interfaceName, 0,
 * (long)recvPackets, (long)sentPackets)); //System.out.println("Network Name "
 * + interfaceName + " recvPackets " + recvPackets + " sentPackets " +
 * sentPackets);
 * 
 * sLine = st.nextToken(); values = new StringTokenizer(sLine); }
 * //System.out.println("**********Network Info Ends**********"); }
 * if(sLine.trim().startsWith("Disk") || (sLine.indexOf("KB-Read") != -1 &&
 * sLine.indexOf("KB-Writ") != -1) ) // Disk Info {
 * //System.out.println("**********Disk Info starts**********"); sLine =
 * st.nextToken(); // values line StringTokenizer values = new
 * StringTokenizer(sLine); while(st.hasMoreTokens() &&
 * !sLine.trim().startsWith("Name") && values.countTokens() >= 6) { String
 * diskName = values.nextToken(); values.nextToken(); // skip next token
 * values.nextToken(); // skip next token values.nextToken(); // skip next token
 * String temp = values.nextToken(); temp = temp.trim(); float readPerSec = 0;
 * try { readPerSec = nf.parse(temp).floatValue(); } catch(Exception e) {
 * readPerSec = 0; }
 * 
 * temp = values.nextToken(); temp = temp.trim(); float writePerSec = 0; try {
 * writePerSec = nf.parse(temp).floatValue(); } catch(Exception e) { writePerSec
 * = 0; } diskInfoList.add(new DiskInfo(diskName, readPerSec, writePerSec));
 * //System.out.println("Disk Name " + diskName + " readPerSec " + readPerSec +
 * " writePerSec " + writePerSec); sLine = st.nextToken(); values = new
 ...
 * while(st.hasMoreTokens()) { sLine = st.nextToken(); // values line
 * StringTokenizer values = new StringTokenizer(sLine); if(values.countTokens()
 * >= 5) { String processName = values.nextToken(); String temp =
 * values.nextToken(); // PID temp = temp.trim(); int pid = 0; try { pid =
 * nf.parse(temp).intValue(); } catch(Exception e) { pid = 0; }
 * 
 * temp = values.nextToken(); // cpu% temp = temp.trim(); float cpuUsage = 0;
 * try { cpuUsage = nf.parse(temp).floatValue(); } catch(Exception e) { cpuUsage
 * = 0; }
 * 
 * temp = values.nextToken(); // paging space temp = temp.trim(); float memUsage
 * = 0; try { memUsage = nf.parse(temp).floatValue(); } catch(Exception e) {
 * memUsage = 0; } processInfoList.add(new ProcessInfo(processName, pid,
 * (int)memUsage, cpuUsage)); //System.out.println("processName " + processName
 * + " pid " + pid + " memUsage " + memUsage + " cpuUsage " + cpuUsage); } }
 * //System.out.println("**********Process Info Ends**********"); } }
 * 
 * StringTokenizer st1 = new StringTokenizer(output, NEW_LINE_TOKENS); while
 * (st1.hasMoreTokens()) { sLine = st1.nextToken(); if(sLine.indexOf("Faults")
 * != -1 && sLine.indexOf("Real,MB") != -1) { StringTokenizer values = new
 * StringTokenizer(sLine); String tokenValue = values.nextToken();
 * while(!tokenValue.equals("Faults") && values.hasMoreTokens()) { tokenValue =
 * values.nextToken(); } tokenValue = values.hasMoreTokens() ?
 * values.nextToken():"0.0";
 * 
 * float pageFaultRate = 0; try { pageFaultRate =
 * nf.parse(tokenValue).floatValue(); } catch(Exception e) { pageFaultRate = 0;
 * }
 * 
 * while(!tokenValue.equals("Real,MB") && values.hasMoreTokens()) { tokenValue =
 * values.nextToken(); } tokenValue = values.hasMoreTokens() ?
 * values.nextToken():"0"; int totalPhysicalMem = 0; try { totalPhysicalMem =
 * nf.parse(tokenValue).intValue(); } catch(Exception e) { e.printStackTrace();
 * totalPhysicalMem = 0; } physicalMemInfo = new MemoryInfo(totalPhysicalMem, 0,
 * 0); physicalMemInfo.setPageFaultRate(pageFaultRate);
 * //System.out.println("Total Physical MB " + totalPhysicalMem + " Faults " +
 * pageFaultRate); } else if(sLine.indexOf("Size,MB") != -1) { StringTokenizer
 * values = new StringTokenizer(sLine); String tokenValue = values.nextToken();
 * while(!tokenValue.equals("Size,MB") && values.hasMoreTokens()) { tokenValue =
 * values.nextToken(); } tokenValue = values.hasMoreTokens() ?
 * values.nextToken():"0";
 * 
 * int totalVirtualMem = 0; try { totalVirtualMem =
 * nf.parse(tokenValue).intValue(); } catch(Exception e) { e.printStackTrace();
 * totalVirtualMem = 0; }
 * 
 * sLine = st1.nextToken(); values = new StringTokenizer(sLine); tokenValue =
 * values.nextToken(); while(!tokenValue.trim().equals("Used") &&
 * values.hasMoreTokens()) { tokenValue = values.nextToken(); } tokenValue =
 * values.hasMoreTokens() ? values.nextToken():"0";
 * 
 * float percentageUsed = 0; try { percentageUsed =
 * nf.parse(tokenValue).floatValue(); } catch(Exception e) { percentageUsed = 0;
 * } int usedMemory = (int)(percentageUsed * totalVirtualMem / 100);
 * virtualMemInfo = new MemoryInfo(totalVirtualMem, usedMemory, totalVirtualMem
 * - usedMemory); //System.out.println("Total virtual MB " + totalVirtualMem +
 * " used Mem " + usedMemory + " free mem " + (totalVirtualMem - usedMemory)); }
 * } } catch (Exception ex) { throw new
 * ResultParsingException("Collect Data Top As command " + ex.getMessage(),
 * output); } } }
 */
