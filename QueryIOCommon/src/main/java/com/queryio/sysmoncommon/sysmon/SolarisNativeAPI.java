...
//	public SolarisNativeAPI(final String hostName, final String user, final String pwd, final int port,
//		final int iProtocol)
//	{
//		this(hostName, user, pwd, port, iProtocol, null, null, null, null, null);
//	}
	public SolarisNativeAPI(final String hostName, final String user, final String pwd, final String key, final int authMethod, final int port,
		...
		try
		{
			...
			else if (iProtocol == IOSProtocolConstants.TELNET)
			{
				...
				if (processFilter == null)
				{
					...
					this.wrapperForDiskSpace = new TelnetWrapper(hostName, user, pwd, port, IOSProtocolConstants.SOLARIS);
				}
			}
			else if (iProtocol == IOSProtocolConstants.SSH)
			{
				this.wrapperForTop = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.SOLARIS);
				if (processFilter == null)
				{
					this.wrapperForDisk = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.SOLARIS);
					this.wrapperForNetwork = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.SOLARIS);
					this.wrapperForDiskSpace = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.SOLARIS);
				}
			}
	...
	public LogInfo searchInLogFile(String logFile, String []search, long lastLine) throws Exception
	{
		...
		/*
		String result = null;
		result = this.wrapperForTop.execute(, false);
