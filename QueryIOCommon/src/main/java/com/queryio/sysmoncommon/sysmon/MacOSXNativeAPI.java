...
//	public MacOSXNativeAPI(final String hostName, final String user, final String pwd, final int port,
//		final int iProtocol)
//	{
//		this(hostName, user, pwd, port, iProtocol, null, null, null, null, null);
//	}
	
	public MacOSXNativeAPI(final String hostName, final String user, final String pwd, final String key, final int authMethod, final int port,
		...
		final String superUserName, final String superUserPassword, final boolean monitorProcess, final int refreshInterval)
	{
		...
		try
		{
			...
			else if (iProtocol == IOSProtocolConstants.TELNET)
			{
				...
				if (processFilter == null)
				{
					this.wrapperForTop_NetworkDisk = new TelnetWrapper(hostName, user, pwd, port, IOSProtocolConstants.MACOSX);
					this.wrapperForDiskSpace = new TelnetWrapper(hostName, user, pwd, port, IOSProtocolConstants.MACOSX);
			...
			{
				this.wrapperForTop = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.MACOSX);
				if (processFilter == null)
				{
					this.wrapperForTop_NetworkDisk = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.MACOSX);
					this.wrapperForDiskSpace = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.MACOSX);
					this.wrapperForNFS = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.MACOSX);
				}
			}
			setFirewallDetails(new AbstractProtocolWrapper [] {
							this.wrapperForTop, this.wrapperForTop_NetworkDisk, this.wrapperForDiskSpace, this.wrapperForNFS});
		}
	...
	public boolean ping() throws Exception
	{
		...
		if (this.wrapperForTop.login())
		{
			...
			{
				this.sVersion = this.wrapperForTop.execute(this.OSVERSIONCOMMAND, true);
				bOS_10_6Version = (this.sVersion.indexOf("10.6") != -1 || this.sVersion.indexOf("10.7") != -1 || this.sVersion.indexOf("10.8") != -1 || this.sVersion.indexOf("10.9") != -1 || this.sVersion.indexOf("10.10") != -1 || this.sVersion.indexOf("10.11") != -1);
				this.dataParser = DataParserFactory.getDataParser(IOSProtocolConstants.MACOSX, this.sVersion);
				this.dataParser.setMonitorProcesses(monitorProcesses);
			}	
			return true;
	...
	public void collectData() throws Exception
	{
		...
		{
			if(bOS_10_6Version) {
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Version: bOS_10_6Version");	
			} else {
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Version: Not bOS_10_6Version");
			}
...
//		Thread.sleep(2000);
		...
		// if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("12");
	...
	public ProcessInfo[] getProcessInfo() throws Exception 
	{
		...
		if (aixProcessInfo != null && aixProcessInfo.length > 0)
		{
			...
			for (int i = 0; i < aixProcessInfo.length; i++) 
			{
				...
				//// if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("ID: " + aixProcessInfo[i].getProcessID() + " found: " + (storedProcessInfo != null));
				if (storedProcessInfo != null)
				{
					// existing process found, update its CPU usage
					set.remove(new Integer(aixProcessInfo[i].getProcessID()));
					
					storedProcessInfo.update(((AIXProcessInfo)aixProcessInfo[i]).getUpTime(), ((AIXProcessInfo)aixProcessInfo[i]).getCPUTime(), 
						this.noOfCPUs, aixProcessInfo[i].getMemoryUsage(), aixProcessInfo[i].getThreadCount());
				}
				else
				{
	...
	public LogInfo searchInLogFile(String logFile, String []search, long lastLine) throws Exception
	{
		...
		if (this.wrapperForTop != null)
		{
			result = this.wrapperForTop.execute(this.LOGFILECOMMAND + lastLine + " " + logFile, false);
		}
	...
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("CPU Usage " + macMachine.getCPUUsage());
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Physical Memory Free " +
	// macMachine.getPhysicalMemoryInfo().getAvailable());
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Virtual Memory Free " +
	...
	// {
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(networkInfo[i1].getName() + " read=" +
	...
	// {
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(info[i1].getName() + " read = " +
	...
	// {
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(drvInfo[i1].getName() + " used=" +
	// drvInfo[i1].getUsedSpace() + ", total:" + drvInfo[i1].getTotalSpace());
	// }
	//
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("collecting Process Info");
	// ProcessInfo[] processes = macMachine.getProcessInfo();
	// for(int i1 = 0; i1 < processes.length; i1++)
	// {
	// // if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(processes[i1].getName() + " Memory " +
