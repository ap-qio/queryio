...
public abstract class AbstractDataParser {
	...
	// All implementor's should have a default constructor as its object is
	// created by reflection using default constructor
	...
	protected long parsePSTime(String value) throws Exception {
		...
		if (value.trim().length() > 0) {
			...
			// time = days * (24 * 60 * 60) + hours * (60 * 60) + mins * 60 +
			// seconds;
			time = days * 86400 + hours * 3600 + mins * 60 + seconds;
			// System.out.println(org + "\tdays: " + days + " hours: " + hours +
			// " mins: " + mins + " seconds: " + seconds + " >> so time: " +
			// time);
		}
		return time;
	}
	 ...
	 ***********************************************************/
	// Note: Following implementation is slightly different compared to
	// implementation in StaticUtilities
