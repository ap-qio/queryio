...
public class WindowsMonitorController {
	...
	public boolean connect() throws UnsatisfiedLinkError, Exception {
		if (!this.connectedToHost) {
			if (this.monitorServiceWrapper == null) {
				this.monitorServiceWrapper = new MonitorWindowsServiceWrapper(IOSProtocolConstants.LOOPBACKADDRESS,
						controllerId);
			}
	...
	private void getDiskInfo() {
		...
		for (int i = 0; i < this.diskInstances.size(); i++) {
			// diskCtr contains counter value from where individual disk
			// read/sec and write/sec are stored in nodes array.
			// For example if diskCtr = 6....then first disk read/sec will be at
			// position 6 and write/sec will be at position 7. For next disk,
			// reads/pec will be at position 8 and write/sec will be at position
			// 9 and so on.
			...
			DiskInfo info = new DiskInfo(this.diskInstances.get(i), (float) this.nodes[diskCtr + i].getValue(),
					(float) this.nodes[diskCtr + i + 1].getValue());
	...
	public ArrayList<String> getDisksInstances(boolean stripDiskName) {
		...
		for (int i = 0; i < node.length; i++) {
			if (node[i].getCounterName().contains("Current Disk Queue Length")
					&& !node[i].getInstanceName().contains("Total")) {
				if (stripDiskName) {
					this.diskInstances.add(
							"Disk " + node[i].getInstanceName().substring(0, node[i].getInstanceName().indexOf(" ")));
				} else {
