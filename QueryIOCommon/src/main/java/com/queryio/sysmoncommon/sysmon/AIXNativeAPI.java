...
public class AIXNativeAPI extends AbstractUnixNativeAPI {
	...
	// public AIXNativeAPI(String hostName, String user, String pwd, int port,
	// int iProtocol,
	// final boolean monitorProcess)
	// {
	// this(hostName, user, pwd, port, iProtocol, monitorProcess, null, null,
	// null, null, null);
	// }
	public AIXNativeAPI(final String hostName, final String user, final String pwd, final String key,
		...
		try {
			...
			if (hostName.equals("127.0.0.1") || hostName.equals("localhost")) {
				...
				this.wrapperForVMStat = new SSHWrapper(hostName, user, pwd, key, authMethod, port,
						IOSProtocolConstants.AIX);
				if (this.monitorProcesses) {
					this.wrapperForProcess = new SSHWrapper(hostName, user, pwd, key, authMethod, port,
							IOSProtocolConstants.AIX);
				}
				if (this.processFilter == null) {
					this.wrapperForDiskInfo = new SSHWrapper(hostName, user, pwd, key, authMethod, port,
							IOSProtocolConstants.AIX);
			...
			setFirewallDetails(new AbstractProtocolWrapper[] { this.wrapperForVMStat, this.wrapperForProcess,
					this.wrapperForDiskInfo, this.wrapperForNetwork, this.wrapperForDiskSpace });
		} catch (Exception ex) {
			...
			if (this.wrapperForProcess != null) {
				if (this.processFilter != null) {
					if (this.processFilter.pid != -1) {
						...
						int i = 5;
						while (i > 0) {
							// Check if result contains PID if not then try
							// fetching result again
							if (result.indexOf(processFilter.pid) == -1) {
								result = this.wrapperForProcess.execute(this.PROCESSCOMMANDPID + this.processFilter.pid,
										false);
							}
						...
						while (i > 0) {
							...
							// fetching result again
							if (result.indexOf("PID") == -1 || result.indexOf("pid") == -1) {
								result = this.wrapperForProcess
										.execute(this.PROCESSCOMMANDOWNER + this.processFilter.owner, false);
							}
							if (result.indexOf("PID") != -1 || result.indexOf("pid") != -1) {
								break;
							}
	...
	public ProcessInfo[] getProcessInfo() throws Exception {
		...
		if (aixProcessInfo != null && aixProcessInfo.length > 0) {
			...
			for (int i = 0; i < aixProcessInfo.length; i++) {
				...
				// System.out.println(aixProcessInfo[i].getName() + " Memory: "
				// + aixProcessInfo[i].getMemoryUsage() + " ID: " +
				...
				if (storedProcessInfo != null) {
					...
					storedProcessInfo.update(aixProcessInfo[i].getUpTime(), aixProcessInfo[i].getCPUTime(), noOfCPUs,
	...
	public LogInfo searchInLogFile(String logFile, String[] search, long lastLine) throws Exception {
		...
		return null;
		/*
		 * String result = null; if (this.wrapperForVMStat != null) { result =
		 * this.wrapperForVMStat.execute(this.LOGFILECOMMAND + lastLine + " " +
