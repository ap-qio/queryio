...
public class GenericMacOSXDataParser extends AbstractDataParser {
			...
			try {
				...
				while (st.hasMoreTokens()) {
					...
					if (sLine.indexOf("Networks:") != -1) {
						 ...
						 * StringTokenizer(sLine); temp = line.nextToken(); //
						 * Ignore token being // Networks: temp =
						 * line.nextToken(); temp = temp.trim(); try {
						 * fPacketsRead = this.nf.parse(temp).floatValue(); }
						 * catch (final Exception e) { fPacketsRead = 0; } temp
						 * = line.nextToken(); // Ignore token being //
						 * ipkts/293K
						 * 
						 * temp = line.nextToken(); temp = temp.trim(); try {
						 * fPacketsSend = this.nf.parse(temp).floatValue(); }
						 * catch (final Exception e) { fPacketsSend = 0; } final
						 * UnixNetworkInfo networkInfo = new
						 * UnixNetworkInfo("Network", 0, (int) fPacketsRead,
						 * (int) fPacketsSend);
					...
					if (sLine.startsWith("Disks:")) {
						 ...
						 * final StringTokenizer line = new
						 * StringTokenizer(sLine); //temp = line.nextToken(); //
						 * Ignore token being Disks: temp = line.nextToken();
						 * temp = temp.trim();
						 * 
						 * try { fPacketsRead =
						 * this.nf.parse(temp).floatValue(); } catch (final
						 * Exception e) { e.printStackTrace(); fPacketsRead = 0;
						 * } temp = line.nextToken(); // Ignore token being //
						 * reads/92754K
						 * 
						 * temp = line.nextToken(); temp = temp.trim();
						 * 
						 * try { fPacketsSend =
						 * this.nf.parse(temp).floatValue(); } catch (final
						 * Exception e) { fPacketsSend = 0; }
						 */
	...
	public void parseDFCommand(final String output) throws ResultParsingException {
		 ...
		 * the next token begins with "[userid" Filesystem 1024-blocks Used
		 * Available Capacity iused ifree %iused Mounted on /dev/disk0s2
		 * 487546976 94835168 392455808 20% 23772790 98113952 20% / devfs 184
		 * 184 0 100% 639 0 100% /dev /dev/disk1s2 487546976 172275116 315271860
		 * 36% 43068777 78817965 35% /Volumes/Server HD map -hosts 0 0 0 100% 0
		 * 0 100% /net map auto_home 0 0 0 100% 0 0 100% /home
		 * 
		 */
	 ...
	 * 
	 * @see
	 * com.queryio.sysmoncommon.sysmon.AbstractDataParser#parseTopMemCommand(
	 * java.lang.String)
	 */
		...
		// after disconnect
		{
			try {
				...
				while (st.hasMoreTokens()) {
					...
					/*
					 * Load Avg: 0.21, 0.12, 0.05 CPU usage: 2.9% user, 7.6%
	...
	public void parseNSFStatCommand(String clientStats, String serverStats) {
		...
		if (clientStats != null && clientStats.trim().length() > 0) {
			 ...
			 * 
			 * @formatter:off Client Info: RPC Counts: Getattr Setattr Lookup
			 * Readlink Read Write Create Remove 6480 493 112205 0 16577 160253
			 * 7790 7743 Rename Link Symlink Mkdir Rmdir Readdir RdirPlus Access
			 * 42 2 0 18 2 10030 0 145815 Mknod Fsstat Fsinfo PathConf Commit 0
			 * 1328 4 2 0 RPC Info: TimedOut Invalid X Replies Retries Requests
			 * 0 0 0 0 468788 Cache Info: Attr Hits Misses Lkup Hits Misses BioR
			 * Hits Misses BioW Hits Misses 408938 37912 13625 97102 36569 8881
			 * 981 160253 BioRLHits Misses BioD Hits Misses DirE Hits Misses 0 0
			 * 93 10093 10123 0
			 * 
			 * 
			 * @formatter:on
			...
			for (int i = 0; i < nfsstatsLines.length; i++) {
				...
				if (nfsstatsLines[i].startsWith("RPC Counts")) {
					/*
					 * RPC Counts: Getattr Setattr Lookup Readlink Read Write
					 * Create Remove 6480 493 112205 0 16577 160253 7790 7743
					 * Rename Link Symlink Mkdir Rmdir Readdir RdirPlus Access
					 * 42 2 0 18 2 10030 0 145815 Mknod Fsstat Fsinfo PathConf
					 * Commit 0 1328 4 2 0
					 */
					stats = "RPC Counts";
					...
					/*
					 * RPC Info: TimedOut Invalid X Replies Retries Requests 0 0
					 * 0 0 468788
					 */
					stats = "RPC Info";
					if (i + 2 < nfsstatsLines.length) {
					...
					/*
					 * Cache Info: Attr Hits Misses Lkup Hits Misses BioR Hits
					 * Misses BioW Hits Misses 408938 37912 13625 97102 36569
					 * 8881 981 160253 BioRLHits Misses BioD Hits Misses DirE
					 * Hits Misses 0 0 93 10093 10123 0
					 */
					stats = "Cache Info";
					for (int l = 0; l < 2; l++) {
						if (i + 2 < nfsstatsLines.length) {
							...
							for (int j = 0, k = 0; j < attrs.length && k < values.length; j++, k++) {
								...
								if (attr.startsWith("BioRL")) {
									group = "BioRL";
									attr = "Hits";
								} else if (attr.equals("Attr") || attr.equals("Lkup") || attr.equals("BioR")
										|| attr.equals("BioW") || attr.equals("BioD") || attr.equals("DirE")) {
									group = attr;
									++j;
									attr = attrs[j];
								}
		...
		if (serverStats != null && serverStats.trim().length() > 0) {
			/*
			 * Server Info: RPC Counts: Getattr Setattr Lookup Readlink Read
			 * Write Create Remove 0 0 0 0 0 0 0 0 Rename Link Symlink Mkdir
			 * Rmdir Readdir RdirPlus Access 0 0 0 0 0 0 0 0 Mknod Fsstat Fsinfo
			 * PathConf Commit 0 0 0 0 0 Server Ret-Failed 0 Server Faults 0
			 * Server Cache Stats: Inprog Idem Non-idem Misses 0 0 0 0 Server
			 * Write Gathering: WriteOps WriteRPC Opsaved 0 0 0
			 * 
			 */
			...
			for (int i = 0; i < nfsstatsLines.length; i++) {
				...
				if (nfsstatsLines[i].startsWith("RPC Counts")) {
					/*
					 * RPC Counts: Getattr Setattr Lookup Readlink Read Write
					 * Create Remove 0 0 0 0 0 0 0 0 Rename Link Symlink Mkdir
					 * Rmdir Readdir RdirPlus Access 0 0 0 0 0 0 0 0 Mknod
					 * Fsstat Fsinfo PathConf Commit 0 0 0 0 0
					 */
					stats = "RPC Counts";
					...
					/*
					 * Server Write Gathering: WriteOps WriteRPC Opsaved 0 0 0
					 */
