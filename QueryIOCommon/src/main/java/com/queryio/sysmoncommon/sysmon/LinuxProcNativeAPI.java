...
class LinuxProcNativeAPI extends AbstractUnixNativeAPI {
	...
	public LinuxProcNativeAPI(final String hostName, final String user, final String pwd, final String key,
			...
			final boolean monitorProcess, final String ioDisks, final boolean monitorNFS, final int refreshInterval) {
		...
		try {
			if (hostName.equals("127.0.0.1") || hostName.equals("localhost")) {
				...
				this.wrapperForProc = new SSHWrapper(hostName, user, pwd, key, authMethod, port,
						IOSProtocolConstants.LINUX);
				if (processFilter == null)
					this.wrapperForDisk = new SSHWrapper(hostName, user, pwd, key, authMethod, port,
							IOSProtocolConstants.LINUX);
			}
			setFirewallDetails(new AbstractProtocolWrapper[] { this.wrapperForProc, this.wrapperForDisk });
	...
	public boolean ping() throws Exception {
		if (!this.wrapperForProc.isConnected()) {
			...
			// this.sVersion = this.wrapperForProc.execute(VERSION_INFO_CMD,
			// false);
		}
		if (processFilter == null) {
	...
	/**
	 * Get Disk info of the remote machine from /proc/diskstats or
	 * /proc/partitions And parse the returned string. Extract the disk info.
	 */
	...
	public void collectData() throws Exception {
		...
		if (this.wrapperForProc != null) {
			...
			List pidList = dataParser.parseProcessIdList(result); // also used
																	// for
																	// finding
																	// count of
																	// running
			...
			if (monitorProcesses) {
				dataParser.resetProcessInfoList();
				/*
				 * for (Iterator iterator = pidList.iterator();
				 * iterator.hasNext();) { String pid = (String) iterator.next();
				 * result = this.wrapperForProc.execute("cat /proc/" + pid
				 * +"/stat", false); assertCommandNotFound(result, pid,
				 * command);
				 * 
	...
	public DriveInfo[] getDiskSpaceInfo() throws Exception {
		String result = null;
		if (this.wrapperForProc != null) {
			// NOTE: DF uses system call statfs to get the info. Could not find
			// required info in /proc
