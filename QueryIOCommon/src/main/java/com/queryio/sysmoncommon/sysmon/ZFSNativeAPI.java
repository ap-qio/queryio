...
class ZFSNativeAPI extends AbstractUnixNativeAPI 
{
	...
	*/
	...
	public ZFSNativeAPI(String hostName, String user, String pwd, String key, int authMethod, int port, int iProtocol, ProcessFilter processFilter, String fUserName,
		String fPassword, String superUserName, String superUserPassword, boolean monitorProcess, int interval) 
	{
		super(hostName, user, pwd, key, authMethod, port, iProtocol, processFilter, fUserName, fPassword, superUserName, superUserPassword, monitorProcess, interval);
		...
		try
		{
			...
			else if (iProtocol == IOSProtocolConstants.SSH)
			{
				for (int i = 0; i < wrappers.length; i++) 
				{
					wrappers[i] = new SSHWrapper(hostName, user, pwd, key, authMethod, port, IOSProtocolConstants.SOLARIS);
				}
			}
