...
public class ShellWrapper extends AbstractProtocolWrapper
{
	...
	public void disconnect() throws IOException
	{
		...
		{
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Disconnecting...");
			this.connected = false;
			
			try {
				Thread th = new Thread() {		
					public void run() {
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Closing line reader");
						try {
							lineReader.close();
						} catch (IOException e) {
						...
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Closing input stream");
						try {
							in.close();
						} catch (IOException e) {
							AppLogger.getLogger().fatal(e.getMessage(), e);
						...
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Closing output stream");
						try {
							out.close();
						} catch (IOException e) {
							AppLogger.getLogger().fatal(e.getMessage(), e);
				...
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Destroying process");
				this.process.destroy();
			}
				
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Disconnect complete");
		}
	}
	...
	public String execute(final String cmd, boolean bApplyTimeOut) throws IOException
	{
		...
		if (bApplyTimeOut)
		{
//			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("echo $PS1: " + AbstractProtocolWrapper.APPPERFECT_PROMPT);
...
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("cmd: " + cmd + " bApplyTimeOut: " + bApplyTimeOut);
		return result;
	}
	 ...
	 * int i = 0; i < searchElements.length; i++ ) { // initialize the handlers
	 * handlers[i] = new ScriptHandler(); handlers[i].setup( searchElements[i] ); }
	 * 
