...
/**
 * This class provides functions for reading the names of SpanReceivers from the
 * Hadoop configuration, adding those SpanReceivers to the Tracer, and closing
 * those SpanReceivers when appropriate. This class does nothing If no
 * SpanReceiver is configured.
 */
...
public class SpanReceiverHost implements TraceAdminProtocol {
	...
	private static final HashMap<String, SpanReceiverHost> hosts = new HashMap<String, SpanReceiverHost>(1);
	private final TreeMap<Long, SpanReceiver> receivers = new TreeMap<Long, SpanReceiver>();
	private final String confPrefix;
	private Configuration config;
	private boolean closed = false;
	private long highestId = 1;

	private final static String LOCAL_FILE_SPAN_RECEIVER_PATH_SUFFIX = "local-file-span-receiver.path";
	...
	private static String getUniqueLocalTraceFileName() {
		...
		try {
			// On Linux we can get a unique local file name by reading the
			// process id
	 ...
	 * "hadoop.htrace.spanreceiver.classes" property and instantiates and
	 * registers them with the Tracer as SpanReceiver's.
	 *
	 * The nullary constructor is called during construction, but if the classes
	 * specified implement the Configurable interface, setConfiguration() will
	 * be called on them. This allows SpanReceivers to use values from the
	...
	private synchronized SpanReceiver loadInstance(String className, List<ConfigurationPair> extraConfig)
			throws IOException {
		SpanReceiverBuilder builder = new SpanReceiverBuilder(
				TraceUtils.wrapHadoopConf(confPrefix, config, extraConfig));
