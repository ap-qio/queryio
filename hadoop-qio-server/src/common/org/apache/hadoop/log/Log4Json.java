 ...
 * <li>Time is published as a time_t event since 1/1/1970 -this is the fastest
 * to generate.</li>
...
public class Log4Json extends Layout {
	 ...
	 * @throws IOException
	 *             on problems generating the JSON
	 */
	...
	public Writer toJson(final Writer writer, final LoggingEvent event) throws IOException {
		...
		toJson(writer, event.getLoggerName(), event.getTimeStamp(), event.getLevel().toString(), event.getThreadName(),
				event.getRenderedMessage(), ti);
		return writer;
	}
	 ...
	 *             on any problem
	 */
	public Writer toJson(final Writer writer, final String loggerName, final long timeStamp, final String level,
			final String threadName, final String message, final ThrowableInformation ti) throws IOException {
		...
		if (ti != null) {
			// there is some throwable info, but if the log event has been sent
			// over the wire,
