...
public class Utils {
	 ...
	 * {@literal -112 <= i <= 127}, only one byte is used with the actual value.
	 * For other values of i, the first byte value indicates whether the long is
	 * positive or negative, and the number of bytes that follow. If the first
	 * byte value v is between -113 and -120, the following long is positive,
	 * with number of bytes that follow are -(v+112). If the first byte value v
	 * is between -121 and -128, the following long is negative, with number of
	 * bytes that follow are -(v+120). Bytes are stored in the
