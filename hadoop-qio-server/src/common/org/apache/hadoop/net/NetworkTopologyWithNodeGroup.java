...
/**
 * The class extends NetworkTopology to represents a cluster of computer with a
 * 4-layers hierarchical network topology. In this network topology, leaves
 * represent data nodes (computers) and inner nodes represent switches/routers
 * that manage traffic in/out of data centers, racks or physical host (with
 * virtual switch).
 * 
 * @see NetworkTopology
 */
...
public class NetworkTopologyWithNodeGroup extends NetworkTopology {
	 ...
	 * here is: each nodes are leaf nodes.
	 * 
	 * @param node1
	...
	public void add(Node node) {
		...
		try {
			...
			// note: rack's null parent case is: node's topology only has one
			// layer,
	 ...
	 * <p/>
	 * This is the same as
	 * {@link NetworkTopology#sortByDistance(Node, Node[], int)} except with a
	 * four-level network topology which contains the additional network
	 * distance of a "node group" which is between local and same rack.
	 *
	 * @param reader
