 ...
 * the work: reading the configuration parameters, executing any defined script,
 * handling errors and such like. The outer class extends
 * {@link CachedDNSToSwitchMapping} to cache the delegated queries.
 * <p/>
 * This DNS mapper's {@link #isSingleSwitch()} predicate returns true if and
 * only if a script is defined.
 */
...
public class ScriptBasedMapping extends CachedDNSToSwitchMapping {
	...
	/**
	 * key to the argument count that the script supports {@value}
	 */
	 ...
	 * </p>
	 * Calling {@link #setConf(Configuration)} will trigger a re-evaluation of
	 * the configuration settings and so be used to set up the mapping script.
	 *
	 */
	...
	/**
	 * This is the uncached script mapping that is fed into the cache managed by
	 * the superclass {@link CachedDNSToSwitchMapping}
	 */
	protected static class RawScriptBasedMapping extends AbstractDNSToSwitchMapping {
		...
		/**
		 * Build and execute the resolution command. The command is executed in
		 * the directory specified by the system property "user.dir" if set;
		 * otherwise the current working directory is used
		 * 
		 * @param args
		 *            a list of arguments
		 * @return null if the number of arguments is out of range, or the
		...
		protected String runResolveCommand(List<String> args, String commandScriptName) {
			...
			if (maxArgs < MIN_ALLOWABLE_ARGS) {
				LOG.warn("Invalid value " + Integer.toString(maxArgs) + " for " + SCRIPT_ARG_COUNT_KEY + "; must be >= "
