...
public class DomainSocket implements Closeable {
	 ...
	 * unprivileged users to perform a man-in-the-middle attack against it. For
	 * example, one way to perform a man-in-the-middle attack would be for a
	 * malicious user to move the server socket out of the way and create his
	 * own socket in the same place. Not good.
	 * 
	 * Note that we only check the path once. It's possible that the permissions
	 * on the path could change, perhaps to something more relaxed, immediately
	 * after the path passes our validation test-- hence creating a security
	 * hole. However, the purpose of this check is to spot common
	 ...
	 *            the number of starting path components to skip validation for
	 ...
	 * occurrences of _PORT with the port. This is mainly to make it possible to
	 * run multiple DataNodes locally for testing purposes.
	 *
	 * @param path
	 *            The source path
	 * @param port
	 ...
	 *
	 * @param path
	 *            The path to bind and listen on.
	...
	/**
	 * Create a pair of UNIX domain sockets which are connected to each other by
	 * calling socketpair(2).
	 *
	 * @return An array of two UNIX domain sockets connected to each other.
	 * @throws IOException
	 *             on error.
	 */
	public static DomainSocket[] socketpair() throws IOException {
		int fds[] = socketpair0();
		return new DomainSocket[] { new DomainSocket("(anonymous0)", fds[0]),
	 ...
	 * @throws IOException
	 *             If there was an I/O error performing the accept-- such as the
	...
	public void close() throws IOException {
		...
		while (count > 0) {
			if (!didShutdown) {
				try {
					// Calling shutdown on the socket will interrupt blocking
					// system
	...
	private native static void sendFileDescriptors0(int fd, FileDescriptor descriptors[], byte jbuf[], int offset,
	 ...
	 * @param jbuf
	 *            Some bytes to send. You must send at least one byte.
	 * @param offset
	 *            The offset in the jbuf array to start at.
	 * @param length
	 *            Length of the jbuf array to use.
	 */
	public void sendFileDescriptors(FileDescriptor descriptors[], byte jbuf[], int offset, int length)
			throws IOException {
		refCount.reference();
		boolean exc = true;
		try {
	...
	private static native int receiveFileDescriptors0(int fd, FileDescriptor[] descriptors, byte jbuf[], int offset,
			int length) throws IOException;
	 ...
	 *
	 * @param descriptors
	 *            (output parameter) Array of FileDescriptors. We will fill as
	 *            many slots as possible with file descriptors passed from the
	 *            remote process. The other slots will contain NULL.
	 * @param jbuf
	 *            (output parameter) Buffer to read into. The UNIX domain
	 *            sockets API requires you to read at least one byte from the
	 *            remote process, even if all you care about is the file
	 *            descriptors you will receive.
	 * @param offset
	 *            Offset into the byte buffer to load data
	 * @param length
	 *            Length of the byte buffer to use for data
	 *
	 * @return The number of bytes read. This will be -1 if we reached EOF
	 *         (similar to SocketInputStream); otherwise, it will be positive.
	 * @throws IOException
	 *             if there was an I/O error.
	 */
	public int receiveFileDescriptors(FileDescriptor[] descriptors, byte jbuf[], int offset, int length)
			throws IOException {
		refCount.reference();
		boolean exc = true;
		try {
	...
	private native static int readByteBufferDirect0(int fd, ByteBuffer dst, int position, int remaining)
			throws IOException;
	...
	public class DomainChannel implements ReadableByteChannel {
		...
		public int read(ByteBuffer dst) throws IOException {
			...
			try {
				int nread = 0;
				if (dst.isDirect()) {
					nread = DomainSocket.readByteBufferDirect0(DomainSocket.this.fd, dst, dst.position(),
							dst.remaining());
				} else if (dst.hasArray()) {
					nread = DomainSocket.readArray0(DomainSocket.this.fd, dst.array(),
							dst.position() + dst.arrayOffset(), dst.remaining());
				} else {
					throw new AssertionError("we don't support "
							+ "using ByteBuffers that aren't either direct or backed by " + "arrays");
				}
				if (nread > 0) {
