...
public final class DomainSocketWatcher implements Closeable {
	...
	public interface Handler {
		 ...
		 *
		 * @param sock
		 *            The socket that the event occurred on.
	...
	/**
	 * The FdSet is a set of file descriptors that gets passed to poll(2). It
	 * contains a native memory segment, so that we don't have to copy in the
	 * poll0 function.
	 */
	private static class FdSet {
		...
		/**
		 * Get an array containing all the FDs marked as readable. Also clear
		 * the state of all FDs.
		 *
	...
	/**
	 * Maximum length of time to go between checking whether the interrupted bit
	 * has been set for this thread.
	 */
	 ...
	 * write anything else to the socket until the notification handler has had
	 * a chance to run. Otherwise, our thread might block, causing deadlock. See
	...
	public void close() throws IOException {
		...
		// Close notificationSockets[0], so that notificationSockets[1] gets an
		// EOF
		// event. This will wake up the thread immediately if it is blocked
		// inside
	 ...
	 *
	 * @param sock
	 *            The socket to add. It is an error to re-add a socket that we
	 *            are already watching.
	 * @param handler
	 *            The handler to associate with this socket. This may be called
	 *            any time after this function is called.
	 */
	 ...
	 *            reason for call
	 * @param entries
	 *            mapping of file descriptor to entry
	 * @param fdSet
		...
		if (entry.getHandler().handle(sock)) {
			...
			try {
				...
				Preconditions.checkArgument(false,
						this + ": file descriptor " + sock.fd + " was closed while " + "still in the poll(2) loop.");
			}
			IOUtils.cleanup(LOG, sock);
			fdSet.remove(fd);
			return true;
	...
	final Thread watcherThread = new Thread(new Runnable() {
		...
		public void run() {
			...
			try {
				while (true) {
					...
					try {
						...
						if (!(toAdd.isEmpty() && toRemove.isEmpty())) {
							// Handle pending additions (before pending
							// removes).
							for (Iterator<Entry> iter = toAdd.iterator(); iter.hasNext();) {
								...
								Preconditions.checkState(prevEntry == null,
										this + ": tried to watch a file descriptor that we " + "were already watching: "
												+ sock);
								if (LOG.isTraceEnabled()) {
						...
						// Check if the thread should terminate. Doing this
						// check now is
						// easier than at the beginning of the loop, since we
						// know toAdd and
						// toRemove are now empty and processedCond has been
						// notified if it
						// needed to be.
						if (closed) {
							if (LOG.isDebugEnabled()) {
						...
						// Check if someone sent our thread an
						// InterruptedException while we
				...
				lock.lock();
				try {
					kick(); // allow the handler for notificationSockets[0] to
							// read a byte
					for (Entry entry : entries.values()) {
						// We do not remove from entries as we iterate, because
						// that can
	...
	private void addNotificationSocket(final TreeMap<Integer, Entry> entries, FdSet fdSet) {
		entries.put(notificationSockets[1].fd, new Entry(notificationSockets[1], new NotificationHandler()));
		try {
	...
	private static native int doPoll0(int maxWaitMs, FdSet readFds) throws IOException;
}
