...
public class NetworkTopology {
	...
	/**
	 * Get an instance of NetworkTopology based on the value of the
	 * configuration parameter net.topology.impl.
	 * 
	 * @param conf
	...
	public static NetworkTopology getInstance(Configuration conf) {
		return ReflectionUtils.newInstance(conf.getClass(CommonConfigurationKeysPublic.NET_TOPOLOGY_IMPL_KEY,
				NetworkTopology.class, NetworkTopology.class), conf);
	}

	/**
	 * InnerNode represents a switch/router of a data center or rack. Different
	 * from a leaf node, it has non-null children.
	 */
	static class InnerNode extends NodeBase {
		...
		/**
		 * Creates a parent node to be added to the list of children. Creates a
		 * node using the InnerNode four argument constructor specifying the
		 * name, location, parent, and level of this node.
		 * 
		 * <p>
		 * To be overridden in subclasses for specific InnerNode
		 * implementations, as alternative to overriding the full
		 * {@link #add(Node)} method.
		 * 
		 * @param parentName
		...
		boolean remove(Node n) {
			...
			if (!isAncestor(n))
				throw new IllegalArgumentException(
						n.getName() + ", which is located at " + parent + ", is not a descendent of " + currentPath);
			if (isParent(n)) {
		 ...
		 * 
		 * @param loc
		 *            string location of the form /rack/node
	...
	public void add(Node node) {
		...
		try {
			...
			if ((depthOfAllLeaves != -1) && (depthOfAllLeaves != newDepth)) {
				LOG.error("Error: can't add leaf node " + NodeBase.getPath(node) + " at depth " + newDepth
						+ " to topology:\n" + oldTopoStr);
			...
			if (rack != null && !(rack instanceof InnerNode)) {
				throw new IllegalArgumentException(
						"Unexpected data node " + node.toString() + " at an illegal network location");
			}
	...
	/**
	 * Return a reference to the node given its string representation. Default
	 * implementation delegates to {@link #getNode(String)}.
	 * 
	 * <p>
	 ...
	 * 
	 * @param loc
	 *            a path-like string representation of a rack
	 ...
	 *         the same or {@link Integer#MAX_VALUE} if node1 or node2 do not
	...
	private Node chooseRandom(String scope, String excludedScope) {
		...
		if (numOfDatanodes == 0) {
			throw new InvalidTopologyException("Failed to find datanode (scope=\"" + String.valueOf(scope)
	...
	/**
	 * Returns an integer weight which specifies how far away {node} is away
	 ...
	 * or on a different rack from the reader. Sorting the nodes based on
	 * network distance from the reader reduces network traffic and improves
	 * performance.
	 * <p/>
	 ...
	 *            Node where data will be read
	 * @param nodes
	 *            Available replicas with the requested data
	 * @param activeLen
