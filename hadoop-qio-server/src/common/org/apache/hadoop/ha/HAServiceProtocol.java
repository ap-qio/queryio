...
public interface HAServiceProtocol {
	 ...
	 * state and can no longer return to active/standby states.
	 */
	public enum HAServiceState {
		INITIALIZING("initializing"), ACTIVE("active"), STANDBY("standby"), STOPPING("stopping");
	...
	/**
	 * Information describing the source for a request to change state. This is
	 * used to differentiate requests from automatic vs CLI failover
	 * controllers, and in the future may include epoch information.
	 */
	 ...
	 * 
	 * Service is expected to perform checks to ensure it is functional. If the
	 * service is not healthy due to failure or partial failure, it is expected
	 * to throw {@link HealthCheckFailedException}. The definition of service
	 * not healthy is left to the service.
	 * 
	 * Note that when health check of an Active service fails, failover to
	 * standby may be done.
	 * 
	...
	/**
	 * Return the current status of the service. The status indicates the
	 * current <em>state</em> (e.g ACTIVE/STANDBY) as well as some additional
	 * information. {@see HAServiceStatus}
	 * 
	 * @throws AccessControlException
	 *             if access is denied.
	 * @throws IOException
