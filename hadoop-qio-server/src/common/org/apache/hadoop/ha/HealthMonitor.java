...
/**
 * This class is a daemon which runs in a loop, periodically heartbeating with
 * an HA service. It is responsible for keeping track of that service's health
 * and exposing callbacks to the failover controller when the health status
 * changes.
 * 
 * Classes which need callbacks should implement the {@link Callback} interface.
 */
...
public class HealthMonitor {
	 ...
	 */
	private List<Callback> callbacks = Collections.synchronizedList(new LinkedList<Callback>());

	private List<ServiceStateCallback> serviceStateCallbacks = Collections
			.synchronizedList(new LinkedList<ServiceStateCallback>());

	private HAServiceStatus lastServiceState = new HAServiceStatus(HAServiceState.INITIALIZING);
	...
	HealthMonitor(Configuration conf, HAServiceTarget target) {
		...
		this.connectRetryInterval = conf.getLong(HA_HM_CONNECT_RETRY_INTERVAL_KEY,
	...
	/**
	 * @return the current proxy object to the underlying service. Note that
	 *         this may return null in the case that the service is not
	 *         responding. Also note that, even if the last indicated state is
	 *         healthy, the service may have gone down in the meantime.
	 */
	...
	private boolean isHealthCheckFailedException(Throwable t) {
		return ((t instanceof HealthCheckFailedException) || (t instanceof RemoteException && ((RemoteException) t)
				.unwrapRemoteException(HealthCheckFailedException.class) instanceof HealthCheckFailedException));
	}
		...
		public void run() {
			while (shouldRun) {
				try {
					...
					Preconditions.checkState(!shouldRun, "Interrupted but still supposed to run");
				}
			}
		}
	}
