...
/**
 * The FailOverController is responsible for electing an active service on
 * startup or when the current active is changing (eg due to failure),
 * monitoring the health of a service, and performing a fail-over when a new
 * active service is either manually selected by a user or elected.
 */
...
public class FailoverController {
	...
	/*
	 * Need a copy of conf for graceful fence to set configurable retries for
	 * IPC client. Refer HDFS-3561
	 */
	private final Configuration gracefulFenceConf;

	private final RequestSource requestSource;

	public FailoverController(Configuration conf, RequestSource source) {
		this.conf = conf;
		...
		int gracefulFenceConnectRetries = conf.getInt(CommonConfigurationKeys.HA_FC_GRACEFUL_FENCE_CONNECTION_RETRIES,
				CommonConfigurationKeys.HA_FC_GRACEFUL_FENCE_CONNECTION_RETRIES_DEFAULT);
		gracefulFenceConf.setInt(CommonConfigurationKeys.IPC_CLIENT_CONNECT_MAX_RETRIES_KEY,
				gracefulFenceConnectRetries);
		gracefulFenceConf.setInt(CommonConfigurationKeys.IPC_CLIENT_CONNECT_MAX_RETRIES_ON_SOCKET_TIMEOUTS_KEY,
				gracefulFenceConnectRetries);
	}

	static int getGracefulFenceTimeout(Configuration conf) {
		return conf.getInt(CommonConfigurationKeys.HA_FC_GRACEFUL_FENCE_TIMEOUT_KEY,
				CommonConfigurationKeys.HA_FC_GRACEFUL_FENCE_TIMEOUT_DEFAULT);
	}

	static int getRpcTimeoutToNewActive(Configuration conf) {
		return conf.getInt(CommonConfigurationKeys.HA_FC_NEW_ACTIVE_TIMEOUT_KEY,
				CommonConfigurationKeys.HA_FC_NEW_ACTIVE_TIMEOUT_DEFAULT);
	}

	/**
	 * Perform pre-failover checks on the given service we plan to failover to,
	 * eg to prevent failing over to a service (eg due to it being inaccessible,
	 * already active, not healthy, etc).
	 *
	 * An option to ignore toSvc if it claims it is not ready to become active
	 * is provided in case performing a failover will allow it to become active,
	 * eg because it triggers a log roll so the standby can learn about new
	 * blocks and leave safemode.
	 *
	 * @param from
	 ...
	 * @param forceActive
	 *            ignore toSvc if it reports that it is not ready
	 * @throws FailoverFailedException
	 *             if we should avoid failover
	 */
	private void preFailoverChecks(HAServiceTarget from, HAServiceTarget target, boolean forceActive)
			throws FailoverFailedException {
	...
	/**
	 * Try to get the HA state of the node at the given address. This function
	 * is guaranteed to be "quick" -- ie it has a short timeout and no retries.
	 * Its only purpose is to avoid fencing a node that has already restarted.
	 */
	...
	/**
	 * Failover from service 1 to service 2. If the failover fails then try to
	 * failback.
	 *
	 * @param fromSvc
	 *            currently active service
	 ...
	 *            service to make active
	 * @param forceFence
	 *            to fence fromSvc even if not strictly necessary
	 * @param forceActive
	 *            try to make toSvc active even if it is not ready
	 * @throws FailoverFailedException
	 *             if the failover fails
	 */
	public void failover(HAServiceTarget fromSvc, HAServiceTarget toSvc, boolean forceFence, boolean forceActive)
			throws FailoverFailedException {
