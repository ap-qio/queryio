 ...
 * protocol {@link HAServiceProtocolPB}. This class translates the PB data types
...
public class HAServiceProtocolServerSideTranslatorPB implements HAServiceProtocolPB {
	...
	private static final MonitorHealthResponseProto MONITOR_HEALTH_RESP = MonitorHealthResponseProto.newBuilder()
			.build();
	private static final TransitionToActiveResponseProto TRANSITION_TO_ACTIVE_RESP = TransitionToActiveResponseProto
			.newBuilder().build();
	private static final TransitionToStandbyResponseProto TRANSITION_TO_STANDBY_RESP = TransitionToStandbyResponseProto
			.newBuilder().build();
	...
	@Override
	public MonitorHealthResponseProto monitorHealth(RpcController controller, MonitorHealthRequestProto request)
	...
	@Override
	public TransitionToActiveResponseProto transitionToActive(RpcController controller,
			TransitionToActiveRequestProto request) throws ServiceException {
		try {
	...
	@Override
	public TransitionToStandbyResponseProto transitionToStandby(RpcController controller,
			TransitionToStandbyRequestProto request) throws ServiceException {
		try {
	...
	public GetServiceStatusResponseProto getServiceStatus(RpcController controller,
		...
		GetServiceStatusResponseProto.Builder ret = GetServiceStatusResponseProto.newBuilder().setState(retState)
	...
	public ProtocolSignature getProtocolSignature(String protocol, long clientVersion, int clientMethodsHash)
		...
		if (!protocol.equals(RPC.getProtocolName(HAServiceProtocolPB.class))) {
			throw new IOException("Serverside implements " + RPC.getProtocolName(HAServiceProtocolPB.class)
					+ ". The following requested protocol is unknown: " + protocol);
		}

		return ProtocolSignature.getProtocolSignature(clientMethodsHash,
				RPC.getProtocolVersion(HAServiceProtocolPB.class), HAServiceProtocolPB.class);
	}
}
