...
public abstract class HAServiceTarget {
	 ...
	 *             if the fencing configuration appears to be invalid. This is
	 *             divorced from the above {@link #getFencer()} method so that
	 *             the configuration can be checked during the pre-flight phase
	 *             of failover.
	 */
	...
	public HAServiceProtocol getProxy(Configuration conf, int timeoutMs) throws IOException {
		...
		return new HAServiceProtocolClientSideTranslatorPB(getAddress(), confCopy, factory, timeoutMs);
	}
	...
	/**
	 * Hook to allow subclasses to add any parameters they would like to expose
	 * to fencing implementations/scripts. Fencing methods are free to use this
	 * map as they see fit -- notably, the shell script implementation takes
	 * each entry, prepends 'target_', substitutes '_' for '.', and adds it to
