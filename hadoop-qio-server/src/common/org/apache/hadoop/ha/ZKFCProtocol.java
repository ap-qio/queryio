...
public interface ZKFCProtocol {
	 ...
	 * 
	 * If the node is not currently active, it simply prevents any attempts to
	 * become active for the specified time period. Otherwise, it first tries to
	 * transition the local service to standby state, and then quits the
	 * election.
	 * 
	 * If the attempt to transition to standby succeeds, then the ZKFC receiving
	 * this RPC will delete its own breadcrumb node in ZooKeeper. Thus, the next
	 * node to become active will not run any fencing process. Otherwise, the
	 * breadcrumb will be left, such that the next active will fence this node.
	 * 
	 ...
	 * allow the target node to immediately rejoin the election, so long as it
	 * is healthy.
	 * 
	 * @param millisToCede
	 *            period for which the node should not attempt to become active
	 * @throws IOException
