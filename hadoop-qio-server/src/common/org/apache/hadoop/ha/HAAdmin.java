...
/**
 * A command-line tool for making calls in the HAServiceProtocol. For example,.
 * this can be used to force a service to standby or active mode, or to trigger
 * a health-check.
 */
...
public abstract class HAAdmin extends Configured implements Tool {
	...
	private int transitionToActive(final CommandLine cmd) throws IOException, ServiceFailedException {
		...
		/*
		 * returns true if other target node is active or some exception
		 * occurred and forceActive was not set
		 */
	 ...
	 * service. If automatic failover is configured, then the automatic failover
	 * controllers should be doing state management, and it is generally an
	 * error to use the HAAdmin command line to do so.
	 * 
	 * @param target
	 *            the target to check
	...
	private int failover(CommandLine cmd) throws IOException, ServiceFailedException {
		...
		if (fromNode.isAutoFailoverEnabled()) {
			if (forceFence || forceActive) {
				// -forceActive doesn't make sense with auto-HA, since, if the
				// node
				// is not healthy, then its ZKFC will immediately quit the
				// election
				// again the next time a health check runs.
				//
				// -forceFence doesn't seem to have any real use cases with
				// auto-HA
	...
	/**
	 * Initiate a graceful failover by talking to the target node's ZKFC. This
	 * sends an RPC to the ZKFC, which coordinates the failover.
	 * 
	 * @param toNode
	 *            the node to fail to
	 * @return status code (0 for success)
	 * @throws IOException
	 *             if failover does not succeed
	 */
	...
	/**
	 * Return the serviceId as is, we are assuming it was given as a service
	 * address of form <host:ipcport>.
	 */
