...
/**
 * Fencing method that runs a shell command. It should be specified in the
 * fencing configuration like:<br>
 * <code>
 *   shell(/path/to/my/script.sh arg1 arg2 ...)
 * </code><br>
 * The string between '(' and ')' is passed directly to a bash shell (cmd.exe on
 * Windows) and may not include any closing parentheses.
 * <p>
 * 
 * The shell command will be run with an environment set up to contain all of
 * the current Hadoop configuration variables, with the '_' character replacing
 * any '.' characters in the configuration keys.
 * <p>
 * 
 * If the shell command returns an exit code of 0, the fencing is determined to
 * be successful. If it returns any other exit code, the fencing was not
 * successful and the next fencing method in the list will be attempted.
 * <p>
 * 
 * <em>Note:</em> this fencing method does not implement any timeout. If
 * timeouts are necessary, they should be implemented in the shell script itself
 * (eg by forking a subshell to kill its parent in some number of seconds).
 */
public class ShellCommandFencer extends Configured implements FenceMethod {
	...
	/**
	 * Abbreviate a string by putting '...' in the middle of it, in an attempt
	 * to keep logs from getting too messy.
	 * 
	 * @param cmd
	 *            the string to abbreviate
	 * @param len
	 *            maximum length to abbreviate to
	...
	/**
	 * Attempt to use evil reflection tricks to determine the pid of a launched
	 * process. This is helpful to ops if debugging a fencing process that might
	 * have gone wrong. If running on a system or JVM where this doesn't work,
	...
	/**
	 * Set the environment of the subprocess to be the Configuration, with '.'s
	 * replaced by '_'s.
	 */
