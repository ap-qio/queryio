...
public class ActiveStandbyElector implements StatCallback, StringCallback {
	 ...
	 * application should return from these callbacks quickly so as not to
	 * impede Zookeeper client library performance and notifications. The app
	 * will typically remember the state change and return from the callback. It
	 * will then proceed with implementing actions around that state change. It
	 * is possible to be called back again while these actions are in flight and
	 * the app should handle this scenario.
	 */
	public interface ActiveStandbyElectorCallback {
		/**
		 * This method is called when the app becomes the active leader. If the
		 * service fails to become active, it should throw
		 * ServiceFailedException. This will cause the elector to sleep for a
		 * short period, then re-join the election.
		 * 
		 * Callback implementations are expected to manage their own timeouts
		 * (e.g. when making an RPC to a remote node).
		 */
		...
		/**
		 * If the elector gets disconnected from Zookeeper and does not know
		 * about the lock state, then it will notify the service via the
		 * enterNeutralMode interface. The service may choose to ignore this or
		 * stop doing state changing operations. Upon reconnection, the elector
		 * verifies the leader status and calls back on the becomeActive and
		 * becomeStandby app interfaces. <br/>
		 ...
		 * errors or Zookeeper persistent unavailability) then notifyFatalError
		 * is called to notify the app about it.
		 */
		void notifyFatalError(String errorMessage);

		/**
		 * If an old active has failed, rather than exited gracefully, then the
		 * new active may need to take some fencing actions against it before
		 * proceeding with failover.
		 * 
		 * @param oldActiveData
	 ...
	 * The elector is created by providing to it the Zookeeper configuration,
	 * the parent znode under which to create the znode and a reference to the
	 ...
	 * very short session timeout may result in frequent transitions between
	 * active and standby states during issues like network outages/GS pauses.
	 * 
	 ...
	 *            ZooKeeper ACL's
	 * @param authInfo
	 *            a list of authentication credentials to add to the ZK
	 *            connection
	 * @param app
	 ...
	 * will be notified by a callback on either the becomeActive or
	 * becomeStandby app interfaces. <br/>
	...
	/**
	 * Utility function to ensure that the configured base znode exists. This
	 * recursively creates the znode as well as all of its parents.
	 */
	...
	/**
	 * Clear all of the state held within the parent ZNode. This recursively
	 * deletes everything within the znode as well as the parent znode itself.
	 * It should only be used when it's certain that no electors are currently
	 ...
	 * quitElection. <br/>
	 * This will lose any leader status, if held, and stop monitoring of the
	 * lock node. <br/>
	 * If the instance wants to participate in election again, then it needs to
	 * call joinElection(). <br/>
	 * This allows service instances to take themselves out of rotation for
	 * known impending unavailable states (e.g. long GC pause or software
	 * upgrade).
	 * 
	 * @param needFence
	 *            true if the underlying daemon may need to be fenced if a
	 *            failover occurs due to dropping out of the election.
	 */
	 ...
	 */
	public synchronized byte[] getActiveData()
	...
	public synchronized void processResult(int rc, String path, Object ctx, String name) {
		if (isStaleClient(ctx))
			return;
		LOG.debug("CreateNode result: " + rc + " for path: " + path + " connectionState: " + zkConnectionState
				+ "  for " + this);

		Code code = Code.get(rc);
		if (isSuccess(code)) {
			// we successfully created the znode. we are the leader. start
			// monitoring
			if (becomeActive()) {
				monitorActiveStatus();
			} else {
		...
		if (isNodeExists(code)) {
			if (createRetryCount == 0) {
				// znode exists and we did not retry the operation. so a
				// different
				// instance has created it. become standby and monitor lock.
				becomeStandby();
			}
			// if we had retried then the znode could have been created by our
			// first
			// attempt to the server (that we lost) and this node exists
			// response is
			// for the second attempt. verify this case via ephemeral node
			// owner. this
	...
	public synchronized void processResult(int rc, String path, Object ctx, Stat stat) {
		...
		LOG.debug("StatNode result: " + rc + " for path: " + path + " connectionState: " + zkConnectionState + " for "
				+ this);
	...
	synchronized void processWatchEvent(ZooKeeper zk, WatchedEvent event) {
		...
		if (eventType == Event.EventType.None) {
			...
			case SyncConnected:
				...
				// if the listener was asked to move to safe state then it needs
				// to
				// be undone
			...
			case Disconnected:
				...
				// ask the app to move to safe state because zookeeper
				// connection
		...
		// a watch on lock path in zookeeper has fired. so something has changed
		// on
		// the lock. ideally we should check that the path is the same as the
		// lock
	 ...
	 * @throws IOException
	 * @throws KeeperException
	 *             zookeeper connectionloss exception
	 */
	...
	/**
	 * Sleep for the given number of milliseconds. This is non-static, and
	 * separated out, so that unit tests can override the behavior not to sleep.
	 */
	@VisibleForTesting
	...
	/**
	 * Write the "ActiveBreadCrumb" node, indicating that this node may need to
	 * be fenced on failover.
	 * 
	 ...
	 * active status. If this fails, it will simply warn, since the graceful
	 * release behavior is only an optimization.
	 */
	 ...
	 * 
	 * @return the Stat of the breadcrumb node that was read, or null if no
	 *         breadcrumb node existed
	 */
	...
	private byte[] getDataWithRetries(final String path, final boolean watch, final Stat stat)
			throws InterruptedException, KeeperException {
	...
	/**
	 * The callbacks and watchers pass a reference to the ZK client which made
	 * the original call. We don't want to take action based on any callbacks
	 * from prior clients after we quit the election.
	 * 
	 * @param ctx
	 *            the ZK client passed into the watcher
	...
	private final class WatcherWithClientRef implements Watcher {
		...
		/**
		 * Latch fired whenever any event arrives. This is used in order to wait
		 * for the Connected event when the client is first created.
		 */
		 ...
		 * @throws KeeperException
		 *             if the connection attempt times out. This will be a
		 *             ZooKeeper ConnectionLoss exception code.
		 * @throws IOException
		 *             if interrupted while connecting to ZooKeeper
		 */
