 ...
 * getChildren on the node and adds empty nodes to an internally managed
 * {@link Reaper}
 */
...
public class ChildReaper implements Closeable {
	 ...
	 *
	 * <p>
	 * Unlike {@code HashSet}, this class does NOT allow {@code null} to be used
	 * as an element. The set is serializable.
	 *
	 ...
	 * @param path
	 *            path to reap children from
	 * @param reapingThresholdMs
	 *            threshold in milliseconds that determines that a path can be
	 *            deleted
	 * @param mode
	 *            reaping mode
	 */
	 ...
	 *            reaping mode
	 */
	public ChildReaper(CuratorFramework client, String path, Reaper.Mode mode, ScheduledExecutorService executor,
	 ...
	 *            reaping mode
	 * @param leaderPath
	 *            if not null, uses a leader selection so that only 1 reaper is
	...
	public void start() throws Exception {
		...
		}, reapingThresholdMs, reapingThresholdMs, TimeUnit.MILLISECONDS);
