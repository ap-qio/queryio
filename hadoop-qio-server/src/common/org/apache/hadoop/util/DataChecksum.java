...
/**
 * This class provides interface and utilities for processing checksums for DFS
 * data transfers.
 */
...
public class DataChecksum implements Checksum {
	...
	public static enum Type {
		NULL(CHECKSUM_NULL, 0), CRC32(CHECKSUM_CRC32, 4), CRC32C(CHECKSUM_CRC32C, 4), DEFAULT(CHECKSUM_DEFAULT, 0), // This
																													// cannot
																													// be
																													// used
																													// to
	 ...
	 * 
	 * The 'mark' of the ByteBuffer parameters may be modified by this
	 * function,. but the position is maintained.
	 * 
	 * @param data
	 ...
	 * @param fileName
	 *            the name of the file being read, for error-reporting
	 * @param basePos
	 *            the file position to which the start of 'data' corresponds
	 * @throws ChecksumException
	 *             if the checksums do not match
	 */
	public void verifyChunkedSums(ByteBuffer data, ByteBuffer checksums, String fileName, long basePos)
			throws ChecksumException {
		if (type.size == 0)
			return;

		if (data.hasArray() && checksums.hasArray()) {
			verifyChunkedSums(data.array(), data.arrayOffset() + data.position(), data.remaining(), checksums.array(),
					checksums.arrayOffset() + checksums.position(), fileName, basePos);
			return;
		}
		...
		try {
			...
			while (data.remaining() > 0) {
				...
				int stored = (sum[0] << 24 & 0xff000000) | (sum[1] << 16 & 0xff0000) | (sum[2] << 8 & 0xff00)
						| sum[3] & 0xff;
				if (calculated != stored) {
					long errPos = basePos + data.position() - startDataPos - n;
					throw new ChecksumException(
							"Checksum error: " + fileName + " at " + errPos + " exp: " + stored + " got: " + calculated,
							errPos);
				}
			}
		} finally {
	 ...
	 * backing.
	 */
	private void verifyChunkedSums(byte[] data, int dataOff, int dataLen, byte[] checksums, int checksumsOff,
			String fileName, long basePos) throws ChecksumException {
		if (type.size == 0)
			return;

		if (NativeCrc32.isAvailable()) {
			NativeCrc32.verifyChunkedSumsByteArray(bytesPerChecksum, type.id, checksums, checksumsOff, data, dataOff,
					dataLen, fileName, basePos);
			return;
		}
		...
		while (remaining > 0) {
			...
			int calculated = (int) summer.getValue();
			int stored = (checksums[checksumsOff] << 24 & 0xff000000) | (checksums[checksumsOff + 1] << 16 & 0xff0000)
	 ...
	 * 
	 * @param data
	 *            the DirectByteBuffer pointing to the data to checksum.
	 * @param checksums
	 *            the DirectByteBuffer into which checksums will be stored.
	 *            Enough space must be available in this buffer to put the
	 *            checksums.
	 */
	...
	public void calculateChunkedSums(byte[] data, int dataOffset, int dataLength, byte[] sums, int sumsOffset) {
		...
		if (NativeCrc32.isAvailable()) {
			NativeCrc32.calculateChunkedSumsByteArray(bytesPerChecksum, type.id, sums, sumsOffset, data, dataOffset,
					dataLength);
			return;
		}
