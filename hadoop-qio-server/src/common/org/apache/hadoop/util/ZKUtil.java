...
public class ZKUtil {
	...
	/**
	 * Parse ACL permission string, partially borrowed from ZooKeeperMain
	 * private method
	 */
	...
	/**
	 * Helper method to remove a subset of permissions (remove) from a given set
	 * (perms).
	 * 
	 * @param perms
	 *            The permissions flag to remove from. Should be an OR of a some
	 *            combination of {@link ZooDefs.Perms}
	 * @param remove
	 *            The permissions to be removed. Should be an OR of a some
	 *            combination of {@link ZooDefs.Perms}
	 * @return A permissions flag that is an OR of {@link ZooDefs.Perms} present
	 *         in perms and not present in remove
	 */
	 ...
	 * @return ACL list
	 * @throws {@link
	 *             BadAclFormatException} if an ACL is invalid
	 */
	...
	/**
	 * Parse a comma-separated list of authentication mechanisms. Each such
	 * mechanism should be of the form 'scheme:auth' -- the same syntax used for
	 * the 'addAuth' command in the ZK CLI.
	 * 
	 * @param authString
	 *            the comma-separated auth mechanisms
	 * @return a list of parsed authentications
	 * @throws {@link
	 *             BadAuthFormatException} if the auth format is invalid
	 */
	...
	/**
	 * Because ZK ACLs and authentication information may be secret, allow the
	 * configuration values to be indirected through a file by specifying the
	 * configuration as "@/path/to/file". If this syntax is used, this function
	 * will return the contents of the file as a String.
	 * 
	 * @param valInConf
	 *            the value from the Configuration
	 * @return either the same value, or the contents of the referenced file if
	 *         the configured value starts with "@"
	 * @throws IOException
