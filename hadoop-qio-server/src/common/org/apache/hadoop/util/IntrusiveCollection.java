 ...
 * responsible for storing the pointers to previous and next elements. This can
 * save a lot of memory if there are many elements in the list or many lists.
 */
...
public class IntrusiveCollection<E extends IntrusiveCollection.Element> implements Collection<E> {
	 ...
	 * #{IntrusiveIterator#remove()}, but modifying the collection in other ways
	 * during the iteration is not supported.
	 */
	...
	/**
	 * Get an iterator over the list. This can be used to remove elements. It is
	 * not safe to do concurrent modifications from other threads while using
	 * this iterator.
	 * 
	 * @return The iterator.
	 */
