...
public abstract class ProtoUtil {
	 ...
	 * as the old connection context as was done for writable where the
	 * effective and real users are set based on the auth method.
	 *
	 */
	public static IpcConnectionContextProto makeIpcConnectionContext(final String protocol,
		...
		if (ugi != null) {
			 ...
			 * is not derived from the authentication done during connection
			 * setup.
			 */
	...
	public static RpcRequestHeaderProto makeRpcRequestHeader(RPC.RpcKind rpcKind,
			RpcRequestHeaderProto.OperationProto operation, int callId, int retryCount, byte[] uuid) {
		RpcRequestHeaderProto.Builder result = RpcRequestHeaderProto.newBuilder();
		result.setRpcKind(convert(rpcKind)).setRpcOp(operation).setCallId(callId).setRetryCount(retryCount)
		...
		if (Trace.isTracing()) {
			...
			result.setTraceInfo(
					RPCTraceInfoProto.newBuilder().setParentId(s.getSpanId()).setTraceId(s.getTraceId()).build());
		}
