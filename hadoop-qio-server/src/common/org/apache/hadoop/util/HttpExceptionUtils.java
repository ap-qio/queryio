...
/**
 * HTTP utility class to help propagate server side exception to the client over
 * HTTP as a JSON payload.
 * <p/>
...
public class HttpExceptionUtils {
	 ...
	 * @throws IOException
	 *             thrown if there was an error while creating the response
	 */
	public static void createServletExceptionResponse(HttpServletResponse response, int status, Throwable ex)
			throws IOException {
	 ...
	 * @param ex
	 *            the exception to serialize in the response
	 * @return the JAX-RPC response with the set error and JSON encoded
	 *         exception
	 */
	 ...
	 * <p/>
	 * <b>NOTE:</b> this method will throw the deserialized exception even if
	 * not declared in the <code>throws</code> of the method signature.
	 *
	 * @param conn
			...
			try {
				...
				if (exClass != null) {
					...
					toThrow = new IOException(
							String.format("HTTP status [%d], message [%s]", conn.getResponseCode(), msg));
				}
			} catch (Exception ex) {
