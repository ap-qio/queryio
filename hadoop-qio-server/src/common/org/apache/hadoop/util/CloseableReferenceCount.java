...
public class CloseableReferenceCount {
	 ...
	 * The status bits.
	 *
	 * Bit 30: 0 = open, 1 = closed. Bits 29 to 0: the reference count.
	 */
	 ...
	 *
	 * @return True if the object is closed and has no outstanding references.
	 */
	public boolean unreference() {
		int newVal = status.decrementAndGet();
		Preconditions.checkState(newVal != 0xffffffff, "called unreference when the reference count was already at 0.");
	 ...
	 * @throws ClosedChannelException
	 *             If someone else closes the object before we do.
	 */
