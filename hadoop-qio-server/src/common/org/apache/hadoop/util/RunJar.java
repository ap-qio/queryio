...
public class RunJar {
	 ...
	 * application classloader that provides the isolation of the user class
	 * space from the hadoop classes and their dependencies. It forms a class
	 * space for the user jar as well as the HADOOP_CLASSPATH. Otherwise, it
	 * creates a classloader that simply adds the user jar to the classpath.
	 */
	private ClassLoader createClassLoader(File file, final File workDir) throws MalformedURLException {
		...
		if (useClientClassLoader()) {
			...
			sb.append(workDir + "/").append(File.pathSeparator).append(file).append(File.pathSeparator)
					.append(workDir + "/classes/").append(File.pathSeparator).append(workDir + "/lib/*");
