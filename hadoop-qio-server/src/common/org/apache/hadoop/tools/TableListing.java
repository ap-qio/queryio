 ...
 * Example:
 *
 * NAME OWNER GROUP MODE WEIGHT pool1 andrew andrew rwxr-xr-x 100 pool2 andrew
 * andrew rwxr-xr-x 100 pool3 andrew andrew rwxr-xr-x 100
 *
 */
...
public class TableListing {
	...
	public static class Builder {
		 ...
		 *            Field title.
		 * @param justification
		 *            Right or left justification. Defaults to left.
		 * @param wrap
	...
	public void addRow(String... row) {
		if (row.length != columns.length) {
			throw new RuntimeException(
					"trying to add a row with " + row.length + " columns, but we have " + columns.length + " columns.");
		}
