...
public class SaslRpcClient {
	...
	private static final RpcRequestHeaderProto saslHeader = ProtoUtil.makeRpcRequestHeader(RpcKind.RPC_PROTOCOL_BUFFER,
			OperationProto.RPC_FINAL_PACKET, AuthProtocol.SASL.callId, RpcConstants.INVALID_RETRY_COUNT,
			RpcConstants.DUMMY_CLIENT_ID);
	private static final RpcSaslProto negotiateRequest = RpcSaslProto.newBuilder().setState(SaslState.NEGOTIATE)
			.build();

	/**
	 * Create a SaslRpcClient that can be used by a RPC client to negotiate SASL
	 * authentication with a RPC server
	 * 
	 * @param ugi
	 *            - connecting user
	 * @param protocol
	 *            - RPC protocol
	 * @param serverAddr
	 *            - InetSocketAddress of remote server
	 * @param conf
	 *            - Configuration
	 */
	public SaslRpcClient(UserGroupInformation ugi, Class<?> protocol, InetSocketAddress serverAddr,
			Configuration conf) {
		this.ugi = ugi;
	...
	/**
	 * Instantiate a sasl client for the first supported auth type in the given
	 * list. The auth type must be defined, enabled, and the user must possess
	 * the required credentials, else the next auth is tried.
	 * 
	 * @param authTypes
	 *            to attempt in the given order
	...
	private SaslAuth selectSaslClient(List<SaslAuth> authTypes)
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(
					"Use " + selectedAuthType.getMethod() + " authentication for protocol " + protocol.getSimpleName());
		}
	...
	/**
	 * Try to create a SaslClient for an authentication type. May return null if
	 * the type isn't supported or the client lacks the required credentials.
	 * 
	 * @param authType
	...
	private SaslClient createSaslClient(SaslAuth authType) throws SaslException, IOException {
		...
		// SASL requires the client and server to use the same proto and
		// serverId
		...
		case KERBEROS: {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("RPC Server's Kerberos principal name for protocol=" + protocol.getCanonicalName() + " is "
						+ serverPrincipal);
			}
			break;
		}
		...
		return Sasl.createSaslClient(new String[] { mechanism }, saslUser, saslProtocol, saslServerName, saslProperties,
				saslCallback);
	}
	 ...
	 * @throws IOException
	 *             - token selector cannot be instantiated
	 */
	...
	/**
	 * Get the remote server's principal. The value will be obtained from the
	 * config and cross-checked against the server's advertised principal.
	 * 
	 * @param authType
	 *            of the SASL client
	 * @return String of the server's principal
	 * @throws IOException
	 *             - error determining configured principal
	 */
	@VisibleForTesting
	String getServerPrincipal(SaslAuth authType) throws IOException {
		...
		if (serverKey == null) {
			throw new IllegalArgumentException(
					"Can't obtain server Kerberos config key from protocol=" + protocol.getCanonicalName());
		}
		// construct server advertised principal for comparision
		String serverPrincipal = new KerberosPrincipal(authType.getProtocol() + "/" + authType.getServerId(),
		...
		if (serverKeyPattern != null && !serverKeyPattern.isEmpty()) {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("getting serverKey: " + serverKey + " conf value: " + conf.get(serverKey) + " principal: "
						+ confPrincipal);
			}
			...
			if (name.getHostName() == null) {
				throw new IllegalArgumentException(
						"Kerberos principal name does NOT have the expected hostname part: " + confPrincipal);
			}
	 ...
	 * @throws IOException
	 */
	public AuthMethod saslConnect(InputStream inS, OutputStream outS) throws IOException {
		...
		do {
			...
			case NEGOTIATE: {
				...
				// define auth being attempted, caller can query if connect
				// fails
	...
	private void sendSaslMessage(DataOutputStream out, RpcSaslProto message) throws IOException {
		if (LOG.isDebugEnabled()) {
			LOG.debug("Sending sasl message " + message);
		}
		RpcRequestMessageWrapper request = new RpcRequestMessageWrapper(saslHeader, message);
	...
	/**
	 * Evaluate the server provided challenge. The server must send a token if
	 * it's not done. If the server is done, the challenge token is optional
	 * because not all mechanisms send a final token for the client to update
	 * its internal state. The client must also be done after evaluating the
	 * optional token to ensure a malicious server doesn't prematurely end the
	 * negotiation with a phony success.
	 * 
	 * @param saslResponse
	...
	private RpcSaslProto.Builder createSaslReply(SaslState state, byte[] responseToken) {
	...
	/**
	 * Get SASL wrapped InputStream if SASL QoP requires unwrapping, otherwise
	 * return original stream. Can be called only after saslConnect() has been
	 * called.
	 * 
	 * @param in
	 *            - InputStream used to make the connection
	...
	/**
	 * Get SASL wrapped OutputStream if SASL QoP requires wrapping, otherwise
	 * return original stream. Can be called only after saslConnect() has been
	 * called.
	 * 
	 * @param in
	...
	class WrappedOutputStream extends FilterOutputStream {
		...
		public void write(byte[] buf, int off, int len) throws IOException {
			...
			RpcSaslProto saslMessage = RpcSaslProto.newBuilder().setState(SaslState.WRAP)
					.setToken(ByteString.copyFrom(buf, 0, buf.length)).build();
