 ...
 * 
 * {@link Groups} allows for server to get the various group memberships of a
 * given user via the {@link #getGroups(String)} call, thus ensuring a
 * consistent user-to-groups mapping and protects against vagaries of different
...
public class Groups {
	...
	private final Map<String, List<String>> staticUserToGroupsMap = new HashMap<String, List<String>>();
	...
	public Groups(Configuration conf, final Timer timer) {
		impl = ReflectionUtils.newInstance(conf.getClass(CommonConfigurationKeys.HADOOP_SECURITY_GROUP_MAPPING,
				ShellBasedUnixGroupsMapping.class, GroupMappingServiceProvider.class), conf);

		cacheTimeout = conf.getLong(CommonConfigurationKeys.HADOOP_SECURITY_GROUPS_CACHE_SECS,
				CommonConfigurationKeys.HADOOP_SECURITY_GROUPS_CACHE_SECS_DEFAULT) * 1000;
		negativeCacheTimeout = conf.getLong(CommonConfigurationKeys.HADOOP_SECURITY_GROUPS_NEGATIVE_CACHE_SECS,
				CommonConfigurationKeys.HADOOP_SECURITY_GROUPS_NEGATIVE_CACHE_SECS_DEFAULT) * 1000;
		warningDeltaMs = conf.getLong(CommonConfigurationKeys.HADOOP_SECURITY_GROUPS_CACHE_WARN_AFTER_MS,
				CommonConfigurationKeys.HADOOP_SECURITY_GROUPS_CACHE_WARN_AFTER_MS_DEFAULT);
		parseStaticMapping(conf);

		this.timer = timer;
		this.cache = CacheBuilder.newBuilder().refreshAfterWrite(cacheTimeout, TimeUnit.MILLISECONDS)
				.ticker(new TimerToTickerAdapter(timer)).expireAfterWrite(10 * cacheTimeout, TimeUnit.MILLISECONDS)
		...
		if (LOG.isDebugEnabled())
			LOG.debug("Group mapping impl=" + impl.getClass().getName() + "; cacheTimeout=" + cacheTimeout
		...
		for (String users : mappings) {
			...
			if (userToGroups.size() < 1 || userToGroups.size() > 2) {
				throw new HadoopIllegalArgumentException(
						"Configuration " + CommonConfigurationKeys.HADOOP_USER_GROUP_STATIC_OVERRIDES + " is invalid");
			}
	...
	/**
	 * Get the group memberships of a given user. If the user's group is not
	 * cached, this method may block.
	 * 
	 * @param user
	 *            User's name
	...
	private class GroupCacheLoader extends CacheLoader<String, List<String>> {
		/**
		 * This method will block if a cache entry doesn't exist, and any
		 * subsequent requests for the same user will wait on this request to
		 * return. If a user already exists in the cache, this will be run in
		 * the background.
		 * 
		 * @param user
		 *            key of cache
		 ...
		 * @throws IOException
		 *             to prevent caching negative entries
		 */
		@Override
		...
		private List<String> fetchGroupList(String user) throws IOException {
			...
			if (deltaMs > warningDeltaMs) {
				LOG.warn("Potential performance problem: getGroups(user=" + user + ") " + "took " + deltaMs
						+ " milliseconds.");
			}
