...
/**
 * Abstract class for implementing credential providers that are based on Java
 * Keystores as the underlying credential store.
 *
...
public abstract class AbstractJavaKeyStoreProvider extends CredentialProvider {
	...
	public List<String> getAliases() throws IOException {
		...
		try {
			...
			try {
				...
				throw new IOException("Can't get alias " + alias + " from " + getPathAsString(), e);
			}
			return list;
		} finally {
			readLock.unlock();
	...
	CredentialEntry innerSetCredential(String alias, char[] material) throws IOException {
		writeLock.lock();
		try {
			keyStore.setKeyEntry(alias, new SecretKeySpec(new String(material).getBytes("UTF-8"), "AES"), password,
					null);
		} catch (KeyStoreException e) {
