...
public class DefaultImpersonationProvider implements ImpersonationProvider {
	...
	public void init(String configurationPrefix) {
		...
		for (Entry<String, String> entry : allMatchKeys.entrySet()) {
			...
			if (!proxyUserAcl.containsKey(aclKey)) {
				proxyUserAcl.put(aclKey, new AccessControlList(allMatchKeys.get(aclKey + CONF_USERS),
	...
	public void authorize(UserGroupInformation user, String remoteAddress) throws AuthorizationException {
		...
		if (acl == null || !acl.isUserAllowed(user)) {
			throw new AuthorizationException(
					"User: " + realUser.getUserName() + " is not allowed to impersonate " + user.getUserName());
		}
		...
		if (MachineList == null || !MachineList.includes(remoteAddress)) {
			throw new AuthorizationException(
					"Unauthorized connection for super-user: " + realUser.getUserName() + " from IP " + remoteAddress);
		}
	}
	...
	/**
	 * Returns configuration key for effective usergroups allowed for a
	 * superuser
	 * 
	 * @param userName
	...
	public Map<String, Collection<String>> getProxyHosts() {
		Map<String, Collection<String>> tmpProxyHosts = new HashMap<String, Collection<String>>();
