...
/**
 * An authorization manager which handles service-level authorization for
...
public class ServiceAuthorizationManager {
	...
	private volatile Map<Class<?>, AccessControlList[]> protocolToAcls = new IdentityHashMap<Class<?>, AccessControlList[]>();
	// For each class, first MachineList in the array specifies the allowed
	// entries
	// and second MachineList specifies blocked entries.
	private volatile Map<Class<?>, MachineList[]> protocolToMachineLists = new IdentityHashMap<Class<?>, MachineList[]>();
	...
	public static final Log AUDITLOG = LogFactory
			.getLog("SecurityLogger." + ServiceAuthorizationManager.class.getName());
			...
			if (clientKey != null && !clientKey.isEmpty()) {
				try {
					...
					throw (AuthorizationException) new AuthorizationException(
							"Can't figure out Kerberos principal name for connection from " + addr + " for user=" + user
									+ " protocol=" + protocol).initCause(e);
				}
			}
		}
		if ((clientPrincipal != null && !clientPrincipal.equals(user.getUserName())) || acls.length != 2
				|| !acls[0].isUserAllowed(user) || acls[1].isUserAllowed(user)) {
	...
	public void refreshWithLoadedConfiguration(Configuration conf, PolicyProvider provider) {
		...
		String defaultAcl = conf.get(CommonConfigurationKeys.HADOOP_SECURITY_SERVICE_AUTHORIZATION_DEFAULT_ACL,
				AccessControlList.WILDCARD_ACL_VALUE);

		String defaultBlockedAcl = conf
				.get(CommonConfigurationKeys.HADOOP_SECURITY_SERVICE_AUTHORIZATION_DEFAULT_BLOCKED_ACL, "");
		...
		if (services != null) {
			for (Service service : services) {
				...
				AccessControlList blockedAcl = new AccessControlList(
