...
/**
 * Class that caches the netgroups and inverts group-to-user map to
 * user-to-group map, primarily intended for use with netgroups (as returned by
 * getent netgrgoup) which only returns group to user mapping.
 */
@InterfaceAudience.LimitedPrivate({ "HDFS", "MapReduce" })
@InterfaceStability.Unstable
public class NetgroupCache {
	private static ConcurrentHashMap<String, Set<String>> userToNetgroupsMap = new ConcurrentHashMap<String, Set<String>>();
	 ...
	 * @param groups
	 *            put groups into this List
	 */
	public static void getNetgroups(final String user, List<String> groups) {
	 ...
	 *
	 * @param group
	 *            name of the group to add to cache
	 * @param users
	...
	public static void add(String group, List<String> users) {
		for (String user : users) {
			...
			// So null value check can be used to check if the key exists
			if (userGroups == null) {
				// Generate a ConcurrentHashSet (backed by the keyset of the
				// ConcurrentHashMap)
