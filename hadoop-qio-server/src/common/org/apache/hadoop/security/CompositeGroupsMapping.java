...
/**
 * An implementation of {@link GroupMappingServiceProvider} which composites
 * other group mapping providers for determining group membership. This allows
 * to combine existing provider implementations and composite a virtually new
 * provider without customized development to deal with complex situation.
 */
...
public class CompositeGroupsMapping implements GroupMappingServiceProvider, Configurable {
	...
	private void addMappingProvider(String providerName, Class<?> providerClass) {
		...
		GroupMappingServiceProvider provider = (GroupMappingServiceProvider) ReflectionUtils.newInstance(providerClass,
				newConf);
	 ...
	 * configured as
	 * "hadoop.security.group.mapping.provider.PROVIDER-X.ldap.url", so that a
	 * provider such as LdapGroupsMapping can be used to composite a complex one
	...
	private Configuration prepareConf(String providerName) {
		...
		while (entries.hasNext()) {
			...
			// get a property like
			// "hadoop.security.group.mapping.provider.PROVIDER-X.ldap.url"
			if (key.startsWith(providerKey) && !key.equals(providerKey)) {
				// restore to be the one like
