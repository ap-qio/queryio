...
public abstract class DelegationTokenAuthenticationHandler implements AuthenticationHandler {
	...
	static {
		DELEGATION_TOKEN_OPS
				.add(KerberosDelegationTokenAuthenticator.DelegationTokenOperation.GETDELEGATIONTOKEN.toString());
		DELEGATION_TOKEN_OPS
				.add(KerberosDelegationTokenAuthenticator.DelegationTokenOperation.RENEWDELEGATIONTOKEN.toString());
		DELEGATION_TOKEN_OPS
				.add(KerberosDelegationTokenAuthenticator.DelegationTokenOperation.CANCELDELEGATIONTOKEN.toString());
	}
	...
	public boolean managementOperation(AuthenticationToken token, HttpServletRequest request,
		...
		if (DELEGATION_TOKEN_OPS.contains(op) && !request.getMethod().equals("OPTIONS")) {
			KerberosDelegationTokenAuthenticator.DelegationTokenOperation dtOp = KerberosDelegationTokenAuthenticator.DelegationTokenOperation
					.valueOf(op);
			if (dtOp.getHttpMethod().equals(request.getMethod())) {
					...
					case RENEWDELEGATIONTOKEN:
						...
						if (tokenToRenew == null) {
							response.sendError(HttpServletResponse.SC_BAD_REQUEST,
									MessageFormat.format("Operation [{0}] requires the parameter [{1}]", dtOp,
											KerberosDelegationTokenAuthenticator.TOKEN_PARAM));
							requestContinues = false;
						} else {
						...
						if (tokenToCancel == null) {
							...
							try {
								...
								tokenManager.cancelToken(dt,
										(requestUgi != null) ? requestUgi.getShortUserName() : null);
							} catch (IOException ex) {
				...
				response.sendError(HttpServletResponse.SC_BAD_REQUEST,
						MessageFormat.format("Wrong HTTP method [{0}] for operation [{1}], it should be " + "[{2}]",
	...
	private static Map delegationTokenToJSON(Token token) throws IOException {
		...
		json.put(KerberosDelegationTokenAuthenticator.DELEGATION_TOKEN_URL_STRING_JSON, token.encodeToUrlString());
		Map response = new LinkedHashMap();
		response.put(KerberosDelegationTokenAuthenticator.DELEGATION_TOKEN_JSON, json);
		return response;
	}
	...
	private String getDelegationToken(HttpServletRequest request) throws IOException {
		String dToken = request.getHeader(DelegationTokenAuthenticator.DELEGATION_TOKEN_HEADER);
		if (dToken == null) {
			dToken = ServletUtils.getParameter(request, KerberosDelegationTokenAuthenticator.DELEGATION_PARAM);
		}
		return dToken;
	}
