...
public class DelegationTokenManager {
	...
	public static final String REMOVAL_SCAN_INTERVAL = PREFIX + "removal-scan-interval.sec";
	...
	private static class DelegationTokenSecretManager
		...
		@Override
		public DelegationTokenIdentifier decodeTokenIdentifier(Token<DelegationTokenIdentifier> token)
				throws IOException {
	...
	public void init() {
		if (managedSecretManager) {
			try {
				...
				throw new RuntimeException("Could not start " + secretManager.getClass() + ": " + ex.toString(), ex);
			}
		}
	}
	...
	public Token<? extends AbstractDelegationTokenIdentifier> createToken(UserGroupInformation ugi, String renewer) {
		...
		AbstractDelegationTokenIdentifier tokenIdentifier = (AbstractDelegationTokenIdentifier) secretManager
				.createIdentifier();
	...
	@SuppressWarnings("unchecked")
	public long renewToken(Token<? extends AbstractDelegationTokenIdentifier> token, String renewer)
			throws IOException {
		return secretManager.renewToken(token, renewer);
	}

	@SuppressWarnings("unchecked")
	public void cancelToken(Token<? extends AbstractDelegationTokenIdentifier> token, String canceler)
			throws IOException {
	...
	@SuppressWarnings("unchecked")
	public UserGroupInformation verifyToken(Token<? extends AbstractDelegationTokenIdentifier> token)
			throws IOException {
	...
	private static DelegationTokenIdentifier decodeToken(Token<DelegationTokenIdentifier> token, Text tokenKind)
			throws IOException {
