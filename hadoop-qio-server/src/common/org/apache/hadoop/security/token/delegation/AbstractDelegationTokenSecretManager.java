...
public abstract class AbstractDelegationTokenSecretManager<TokenIdent extends AbstractDelegationTokenIdentifier>
	...
	/**
	 * Sequence number to create DelegationTokenIdentifier. Protected by this
	 * object lock.
	 */
	...
	/**
	 * Whether to store a token's tracking ID in its TokenInformation. Can be
	...
	public AbstractDelegationTokenSecretManager(long delegationKeyUpdateInterval, long delegationTokenMaxLifetime,
	...
	/**
	 * Add a previously used master key to cache (when NN restarts), should be
	...
	/**
	 * For subclasses externalizing the storage, for example Zookeeper based
	 * implementations
	 */
	 ...
	 * tokens This method must be called before this secret manager is activated
	 * (before startThreads() is called)
	 * 
	 * @param identifier
	 *            identifier read from persistent storage
	 * @param renewDate
	 *            token renew time
	 * @throws IOException
	 */
	public synchronized void addPersistedDelegationToken(TokenIdent identifier, long renewDate) throws IOException {
		...
		if (getTokenInfo(identifier) == null) {
			currentTokens.put(identifier,
					new DelegationTokenInformation(renewDate, password, getTrackingIdIfEnabled(identifier)));
		} else {
	 ...
	 * tokenRemoverThread is created, and only by tokenRemoverThread afterwards.
	 */
	private void updateCurrentKey() throws IOException {
		...
		DelegationKey newKey = new DelegationKey(newCurrentId,
				System.currentTimeMillis() + keyUpdateInterval + tokenMaxLifetime, generateSecret());
	...
	/**
	 * Update the current master key for generating delegation tokens It should
	 * be called only by tokenRemoverThread.
	 */
	void rollMasterKey() throws IOException {
		synchronized (this) {
			 ...
			 * updateMasterKey() isn't called at expected interval. Add it back
			 * to allKeys just in case.
			 */
	...
	private synchronized void removeExpiredKeys() {
		long now = Time.now();
		for (Iterator<Map.Entry<Integer, DelegationKey>> it = allKeys.entrySet().iterator(); it.hasNext();) {
			Map.Entry<Integer, DelegationKey> e = it.next();
			if (e.getValue().getExpiryDate() < now) {
				it.remove();
				// ensure the tokens generated by this current key can be
				// recovered
	...
	protected synchronized byte[] createPassword(TokenIdent identifier) {
		...
		DelegationTokenInformation tokenInfo = new DelegationTokenInformation(now + tokenRenewInterval, password,
	...
	/**
	 * Find the DelegationTokenInformation for the given token id, and verify
	 * that if the token is expired. Note that this method should be called with
	 * acquiring the secret manager's monitor.
	 */
	protected DelegationTokenInformation checkToken(TokenIdent identifier) throws InvalidToken {
	 ...
	 * @throws InvalidToken
	 */
	public synchronized void verifyToken(TokenIdent identifier, byte[] password) throws InvalidToken {
	 ...
	 */
	public synchronized long renewToken(Token<TokenIdent> token, String renewer) throws InvalidToken, IOException {
		ByteArrayInputStream buf = new ByteArrayInputStream(token.getIdentifier());
		DataInputStream in = new DataInputStream(buf);
		TokenIdent id = createIdentifier();
		id.readFields(in);
		LOG.info("Token renewal for identifier: " + id + "; total currentTokens " + currentTokens.size());
		...
		if (!id.getRenewer().toString().equals(renewer)) {
			throw new AccessControlException(renewer + " tries to renew a token with renewer " + id.getRenewer());
		}
	...
	public synchronized TokenIdent cancelToken(Token<TokenIdent> token, String canceller) throws IOException {
		...
		if (!canceller.equals(owner)
				&& (renewer == null || renewer.toString().isEmpty() || !cancelerShortName.equals(renewer.toString()))) {
	...
	private void removeExpiredToken() throws IOException {
		...
		// don't hold lock on 'this' to avoid edit log updates blocking token
		// ops
	...
	private class ExpiredTokenRemover extends Thread {
		...
		public void run() {
			LOG.info("Starting expired delegation token remover thread, " + "tokenRemoverScanInterval="
					+ tokenRemoverScanInterval / (60 * 1000) + " min(s)");
			try {
				while (running) {
					long now = Time.now();
					...
					try {
						Thread.sleep(Math.min(5000, keyUpdateInterval)); // 5
																			// seconds
	 ...
	 * 
	 * @param token
	 *            the token where to extract the identifier
