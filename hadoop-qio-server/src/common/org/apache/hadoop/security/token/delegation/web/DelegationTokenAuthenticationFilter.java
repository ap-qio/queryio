 ...
 * {@link AbstractDelegationTokenSecretManager}. For situations where an
 * external <code>AbstractDelegationTokenSecretManager</code> is required (i.e.
 * one that shares the secret with
 * <code>AbstractDelegationTokenSecretManager</code> instance running in other
 * services), the external <code>AbstractDelegationTokenSecretManager</code>
 * must be set as an attribute in the {@link ServletContext} of the web
 * application using the {@link #DELEGATION_TOKEN_SECRET_MANAGER_ATTR} attribute
 * name ( 'hadoop.http.delegation-token-secret-manager').
 */
...
public class DelegationTokenAuthenticationFilter extends AuthenticationFilter {
	...
	private static final ThreadLocal<UserGroupInformation> UGI_TL = new ThreadLocal<UserGroupInformation>();
	 ...
	 * type is set to <code>simple</code> or <code>kerberos</code> in order to
	 * use the corresponding implementation with delegation token support.
	 *
	 * @param configPrefix
	 *            parameter not used.
	 * @param filterConfig
	...
	protected void setAuthHandlerClass(Properties props) throws ServletException {
		...
		if (authType == null) {
			throw new ServletException("Config property " + AUTH_TYPE + " doesn't exist");
		}
	...
	protected Configuration getProxyuserConfiguration(FilterConfig filterConfig) throws ServletException {
		// this filter class gets the configuration from the filter configs, we
		// are
		// creating an empty configuration and injecting the proxyuser settings
		// in
		// it. In the initialization of the filter, the returned configuration
		// is
		// passed to the ProxyUsers which only looks for 'proxyusers.'
		// properties.
			...
			if (ugi == null) {
				...
				if (doAsUser != null) {
					...
					try {
						...
						HttpExceptionUtils.createServletExceptionResponse(response, HttpServletResponse.SC_FORBIDDEN,
								ex);
