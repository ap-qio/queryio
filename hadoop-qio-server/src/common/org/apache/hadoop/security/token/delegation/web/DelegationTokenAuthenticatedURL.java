 ...
 * {@link DelegationTokenAuthenticator} implementations.
 * <p/>
 * The default {@link DelegationTokenAuthenticator} is the
 * {@link KerberosDelegationTokenAuthenticator} class which supports automatic
 * fallback from Kerberos SPNEGO to Hadoop Simple authentication via the
 * {@link PseudoDelegationTokenAuthenticator} class.
 * <p/>
...
public class DelegationTokenAuthenticatedURL extends AuthenticatedURL {
	...
	/**
	 * Sets the default {@link DelegationTokenAuthenticator} class to use when
	...
	/**
	 * Returns the default {@link DelegationTokenAuthenticator} class to use
	 * when an {@link DelegationTokenAuthenticatedURL} instance is created
	...
	private static DelegationTokenAuthenticator obtainDelegationTokenAuthenticator(DelegationTokenAuthenticator dta,
	 ...
	 *
	 * @param authenticator
	 *            the {@link DelegationTokenAuthenticator} instance to use, if
	 ...
	 *            string, <code>FALSE</code> if the delegation token is
	 *            transmitted using the
	 *            {@link DelegationTokenAuthenticator#DELEGATION_TOKEN_HEADER}
	 *            HTTP header.
	 */
	@Deprecated
	 ...
	 *         string, <code>FALSE</code> if the delegation token is transmitted
	 *         using the
	 *         {@link DelegationTokenAuthenticator#DELEGATION_TOKEN_HEADER} HTTP
	 *         header.
	 */
	 ...
	 * Token only if the given auth token is an instance of {@link Token} and it
	 * contains a Delegation Token, otherwise use the configured
	 ...
	 * Returns an authenticated {@link HttpURLConnection}. If the Delegation
	 * Token is present, it will be used taking precedence over the configured
	 * <code>Authenticator</code>. If the <code>doAs</code> parameter is not
	 * NULL, the request will be done on behalf of the specified
	 * <code>doAs</code> user.
	 *
	 * @param url
	 *            the URL to connect to. Only HTTP/S URLs are supported.
	 ...
	 *            the authentication token being used for the user.
	 * @param doAs
	 *            user to do the the request on behalf of, if NULL the request
	 *            is as self.
	 * @return an authenticated {@link HttpURLConnection}.
	 * @throws IOException
	 *             if an IO error occurred.
	...
	public HttpURLConnection openConnection(URL url, Token token, String doAs)
		...
		// if we have valid auth token, it takes precedence over a delegation
		// token
			...
			if (!creds.getAllTokens().isEmpty()) {
				...
				if (dToken != null) {
					if (useQueryStringForDelegationToken()) {
						// delegation token will go in the query string,
						// injecting it
						extraParams.put(KerberosDelegationTokenAuthenticator.DELEGATION_PARAM,
								dToken.encodeToUrlString());
					} else {
						// delegation token will go as request header, setting
						// it in the
						// auth-token to ensure no authentication handshake is
						// triggered
						// (if we have a delegation token, we are authenticated)
						// the delegation token header is injected in the
						// connection request
	 ...
	 * <code>Authenticator</code> for authentication.
	 *
	 * @param url
	 ...
	 *            the renewer user.
	 * @param doAsUser
	 *            the user to do as, which will be the token owner.
	 * @return a delegation token.
	 * @throws IOException
	 *             if an IO error occurred.
	 * @throws AuthenticationException
	 ...
	 */
	public org.apache.hadoop.security.token.Token<AbstractDelegationTokenIdentifier> getDelegationToken(URL url,
			Token token, String renewer, String doAsUser) throws IOException, AuthenticationException {
		Preconditions.checkNotNull(url, "url");
		Preconditions.checkNotNull(token, "token");
		try {
			token.delegationToken = ((KerberosDelegationTokenAuthenticator) getAuthenticator()).getDelegationToken(url,
	 ...
	 *
	 * @param url
	 *            the URL to renew the delegation token from. Only HTTP/S URLs
	 *            are supported.
	 * @param token
	...
	public long renewDelegationToken(URL url, Token token, String doAsUser)
		...
		try {
			return ((KerberosDelegationTokenAuthenticator) getAuthenticator()).renewDelegationToken(url, token,
					token.delegationToken, doAsUser);
		} catch (IOException ex) {
			token.delegationToken = null;
			throw ex;
		}
	 ...
	 *            are supported.
	 * @param token
	 *            the authentication token with the Delegation Token to cancel.
	 * @throws IOException
	 *             if an IO error occurred.
	 */
	...
	public void cancelDelegationToken(URL url, Token token, String doAsUser) throws IOException {
		...
		Preconditions.checkNotNull(token.delegationToken, "No delegation token available");
		try {
			((KerberosDelegationTokenAuthenticator) getAuthenticator()).cancelDelegationToken(url, token,
					token.delegationToken, doAsUser);
		} finally {
			token.delegationToken = null;
		}
