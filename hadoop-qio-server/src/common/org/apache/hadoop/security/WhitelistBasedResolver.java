...
/**
 * An implementation of the SaslPropertiesResolver. Uses a white list of IPs. If
 * the connection's IP address is in the list of IP addresses, the salProperties
 * will be unchanged. If the connection's IP is not in the list of IP addresses,
 * then QOP for the connection will be restricted to
 * "hadoop.rpc.protection.non-whitelist"
 *
 * Uses 3 IPList implementations together to form an aggregate whitelist. 1.
 * ConstantIPList - to check against a set of hardcoded IPs 2. Fixed IP List -
 * to check against a list of IP addresses which are specified externally, but
 * will not change over runtime. 3. Variable IP List - to check against a list
 * of IP addresses which are specified externally and could change during
 * runtime. A connection IP address will checked against these 3 IP Lists in the
 * order specified above. Once a match is found , the IP address is determined
 * to be in whitelist.
 *
...
public class WhitelistBasedResolver extends SaslPropertiesResolver {
	...
	/**
	 * Path to the file to containing subnets and ip addresses to form fixed
	 * whitelist.
	 */
	...
	/**
	 * time in seconds by which the variable whitelist file is checked for
	 * updates
	 */
	...
	/**
	 * comma separated list containing alternate hadoop.rpc.protection values
	 * for clients which are not in whitelist
	 */
	...
	public void setConf(Configuration conf) {
		...
		if (conf.getBoolean(HADOOP_SECURITY_SASL_VARIABLEWHITELIST_ENABLE, false)) {
			...
			expiryTime = conf.getLong(HADOOP_SECURITY_SASL_VARIABLEWHITELIST_CACHE_SECS, 3600) * 1000;
		}
