...
/**
 * Factory that creates SSLEngine and SSLSocketFactory instances using Hadoop
 * configuration information.
 * <p/>
 * This SSLFactory uses a {@link ReloadingX509TrustManager} instance, which
 * reloads public keys if the truststore file changes.
 * <p/>
...
public class SSLFactory implements ConnectionConfigurator {
	 ...
	 *
	 * @param mode
	 *            SSLFactory mode, client or server.
	 * @param conf
	 ...
	 *             thrown if an SSL initialization error happened.
	 * @throws IOException
	...
	public void init() throws GeneralSecurityException, IOException {
		...
		context.init(keystoresFactory.getKeyManagers(), keystoresFactory.getTrustManagers(), null);
	...
	private HostnameVerifier getHostnameVerifier(Configuration conf) throws GeneralSecurityException, IOException {
		return getHostnameVerifier(StringUtils.toUpperCase(conf.get(SSL_HOSTNAME_VERIFIER_KEY, "DEFAULT").trim()));
	}
	 ...
	 * @throws GeneralSecurityException
	 *             thrown if the SSL engine could not be initialized.
	 * @throws IOException
	 *             thrown if and IO error occurred while loading the server
	 *             keystore.
	 */
	 ...
	 * @throws GeneralSecurityException
	 *             thrown if the SSLSocketFactory could not be initialized.
	 * @throws IOException
	 *             thrown if and IO error occurred while loading the server
	 *             keystore.
	 */
