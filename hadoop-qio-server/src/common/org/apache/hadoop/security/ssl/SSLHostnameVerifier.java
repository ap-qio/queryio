 ...
 * http://juliusdavies.ca/commons-ssl/ This project is not yet in Apache, but it
 ...
 * Instead we added several check() methods that take SSLSocket, or
 * X509Certificate, or ultimately (they all end up calling this one), String.
 * (It's easier to supply JUnit with Strings instead of mock SSLSession
 * objects!)
 * </p>
 * <p>
 * Our check() methods throw exceptions if the name is invalid, whereas
 * javax.net.ssl.HostnameVerifier just returns true/false.
 * <p/>
...
public interface SSLHostnameVerifier extends javax.net.ssl.HostnameVerifier {
	 ...
	 * "a.b.foo.com".
	 */
	public final static SSLHostnameVerifier DEFAULT = new AbstractVerifier() {
		@Override
	 ...
	 */
	public final static SSLHostnameVerifier DEFAULT_AND_LOCALHOST = new AbstractVerifier() {
		@Override
		public final void check(final String[] hosts, final String[] cns, final String[] subjectAlts)
				throws SSLException {
	 ...
	 * "a.b.foo.com".
	 */
	public final static SSLHostnameVerifier STRICT = new AbstractVerifier() {
		@Override
	 ...
	 * to IE6's behaviour.
	 */
	public final static SSLHostnameVerifier STRICT_IE6 = new AbstractVerifier() {
		@Override
		public final void check(final String[] host, final String[] cns, final String[] subjectAlts)
				throws SSLException {
	 ...
	 */
	public final static SSLHostnameVerifier ALLOW_ALL = new AbstractVerifier() {
		@Override
		public final void check(final String[] host, final String[] cns, final String[] subjectAlts) {
	...
	abstract class AbstractVerifier implements SSLHostnameVerifier {
		 ...
		 *
		 * @param host
		 *            'hostname' we used to create our socket
		 * @param session
		...
		public void check(String host[], SSLSocket ssl) throws IOException {
			...
			if (session == null) {
				 ...
				 * 
				 * #1. Clean up the certificate chain that your server is
				 * presenting (e.g. edit "/etc/apache2/server.crt" or wherever
				 * it is your server's certificate chain is defined).
				 * 
				 * OR
				 * 
		...
		public void check(final String[] hosts, final String[] cns, final String[] subjectAlts, final boolean ie6,
			...
			boolean match = false;
			out: for (Iterator<String> it = names.iterator(); it.hasNext();) {
				...
				// [*.org.uk], etc...
				boolean doWildcard = cn.startsWith("*.") && cn.lastIndexOf('.') >= 0 && !isIP4Address(cn)
						&& acceptableCountryWildcard(cn);

				for (int i = 0; i < hosts.length; i++) {
					final String hostName = StringUtils.toLowerCase(hosts[i].trim());
					if (doWildcard) {
	...
	static class Certificates {
		public static String[] getCNs(X509Certificate cert) {
			 ...
			 * "Common Name" field. This is done a bit sloppy right now and
			 * should probably be done a bit more according to <code>RFC
			 * 2253</code>.
			 * 
			 ...
			 * 
			 * For example, getName() gives me this: 1.2.840.113549.1.9.1=#
			 * 16166a756c6975736461766965734063756362632e636f6d
			 * 
