 ...
 * keystore files.
 * <p/>
 * if the trust certificates keystore file changes, the {@link TrustManager} is
 * refreshed with the new trust certificate entries (using a
...
public class FileBasedKeyStoresFactory implements KeyStoresFactory {
	 ...
	 * @throws IOException
	 *             thrown if the keystores could not be initialized due to an IO
	 *             error.
	 * @throws GeneralSecurityException
	 *             thrown if the keystores could not be initialized due to a
	 *             security error.
	 */
	@Override
	public void init(SSLFactory.Mode mode) throws IOException, GeneralSecurityException {
		...
		if (requireClientCert || mode == SSLFactory.Mode.SERVER) {
			...
			// compatibility with legacy configurations that did not use a
			// separate
		...
		if (!truststoreLocation.isEmpty()) {
			...
			long truststoreReloadInterval = conf.getLong(
					resolvePropertyName(mode, SSL_TRUSTSTORE_RELOAD_INTERVAL_TPL_KEY),
					DEFAULT_SSL_TRUSTSTORE_RELOAD_INTERVAL);

			LOG.debug(mode.toString() + " TrustStore: " + truststoreLocation);

			trustManager = new ReloadingX509TrustManager(truststoreType, truststoreLocation, truststorePassword,
