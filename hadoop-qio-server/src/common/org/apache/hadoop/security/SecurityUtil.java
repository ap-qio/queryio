...
public class SecurityUtil {
	 ...
	 * 
	 * @param ticket
	 *            the original TGT (the ticket that is obtained when a kinit is
	 *            done)
	 * @return true or false
	 */
	 ...
	 * @throws IOException
	 *             if the client address cannot be determined
	 */
	...
	/**
	 * Convert Kerberos principal name pattern to valid Kerberos principal
	 * names. This method is similar to
	 * {@link #getServerPrincipal(String, String)}, except 1) the reverse DNS
	 * lookup from addr to hostname is done only when necessary, 2) param addr
	 * can't be null (no default behavior of using local hostname when addr is
	 * null).
	 * 
	 * @param principalConfig
	...
	/**
	 * Login as a principal specified in config. Substitute $host in user's
	 * Kerberos principal name with a dynamically looked-up fully-qualified
	...
	/**
	 * Login as a principal specified in config. Substitute $host in user's
	 * Kerberos principal name with hostname. If non-secure mode - return. If no
	 * keytab available - bail out with an exception
	 * 
	 * @param conf
	 *            conf to use
	 * @param keytabFileKey
	 ...
	 * @throws IOException
	 *             if the config doesn't specify a keytab
	 */
	@InterfaceAudience.Public
	@InterfaceStability.Evolving
	public static void login(final Configuration conf, final String keytabFileKey, final String userNameKey,
	 ...
	 * 
	 * @param providers
	 *            a list of high priority providers to use
	 */
	@InterfaceAudience.Private
	 ...
	 * 
	 * @param protocol
	 *            the protocol class to get the information for
	 * @param conf
	 ...
	 * 
	 * @param uri
	 *            of remote connection with a token
	 * @return "ip:port" or "host:port" depending on the value of
	 *         hadoop.security.token.service.use_ip
	 */
	...
	/**
	 * Perform the given action as the daemon's login user. If the login user
	 * cannot be determined, this will log a FATAL error and exit the whole JVM.
	 */
	...
	/*
	 * Check if a given port is privileged. The ports with number smaller than
	 * 1024 are treated as privileged ports in unix/linux system. For other
	 * operating systems, use this method with care. For example, Windows
	 * doesn't have the concept of privileged ports. However, it may be used at
	 * Windows client to check port of linux server.
	 * 
