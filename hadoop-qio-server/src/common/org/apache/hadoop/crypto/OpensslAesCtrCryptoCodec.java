...
public class OpensslAesCtrCryptoCodec extends AesCtrCryptoCodec {
	private static final Log LOG = LogFactory.getLog(OpensslAesCtrCryptoCodec.class.getName());
	...
	public void setConf(Configuration conf) {
		this.conf = conf;
		final Class<? extends Random> klass = conf.getClass(HADOOP_SECURITY_SECURE_RANDOM_IMPL_KEY,
				OsSecureRandom.class, Random.class);
		try {
	...
	private static class OpensslAesCtrCipher implements Encryptor, Decryptor {
		 ...
		 * in the destination buffer to encrypt entire input buffer.
		 */
		@Override
		...
		/**
		 * AES-CTR will consume all of the input data. It requires enough space
		 * in the destination buffer to decrypt entire input buffer.
		 */
		@Override
		...
		private void process(ByteBuffer inBuffer, ByteBuffer outBuffer) throws IOException {
			try {
				...
				if (n < inputSize) {
					/**
					 * Typically code will not get here. OpensslCipher#update
					 * will consume all input data and put result in outBuffer.
