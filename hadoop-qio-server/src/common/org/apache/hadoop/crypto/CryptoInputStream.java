 ...
 * mapping. The decryption is buffer based. The key points of the decryption are
 * (1) calculating the counter and (2) padding through stream position:
 * <p/>
...
public class CryptoInputStream extends FilterInputStream implements Seekable, PositionedReadable, ByteBufferReadable,
		HasFileDescriptor, CanSetDropBehind, CanSetReadahead, HasEnhancedByteBufferAccess, ReadableByteChannel {
	...
	public CryptoInputStream(InputStream in, CryptoCodec codec, int bufferSize, byte[] key, byte[] iv)
			throws IOException {
	...
	public int read(byte[] b, int off, int len) throws IOException {
		...
		if (remaining > 0) {
			...
			/*
			 * Check whether the underlying stream is {@link
			 * ByteBufferReadable}, it can avoid bytes copy.
			 */
	...
	/**
	 * Do the decryption using inBuffer as input and outBuffer as output. Upon
	 * return, inBuffer is cleared; the decrypted data starts at
	 * outBuffer.position() and ends at outBuffer.limit();
	 */
	private void decrypt(Decryptor decryptor, ByteBuffer inBuffer, ByteBuffer outBuffer, byte padding)
			throws IOException {
	 ...
	 */
	private byte afterDecryption(Decryptor decryptor, ByteBuffer inBuffer, long position, byte[] iv)
			throws IOException {
		byte padding = 0;
		if (decryptor.isContextReset()) {
			/*
			 * This code is generally not executed since the decryptor usually
			 * maintains decryption context (e.g. the counter) internally.
			 * However, some implementations can't maintain context so a re-init
			 * is necessary after each decryption call.
			 */
	 ...
	 * {@link #outBuffer}. This Typically happens during {@link #seek(long)} or
	 * {@link #skip(long)}.
	 */
	...
	public void seek(long pos) throws IOException {
		...
		try {
			/*
			 * If data of target pos in the underlying stream has already been
			 * read and decrypted in outBuffer, we just need to re-position
			 * outBuffer.
			 */
	...
	/**
	 * Decrypt all data in buf: total n bytes from given start position. Output
	...
	public void setReadahead(Long readahead) throws IOException, UnsupportedOperationException {
		try {
			...
			throw new UnsupportedOperationException(
					"This stream does not support " + "setting the readahead caching strategy.");
		}
	}
