...
public class CryptoOutputStream extends FilterOutputStream implements Syncable, CanSetDropBehind {
	...
	public CryptoOutputStream(OutputStream out, CryptoCodec codec, int bufferSize, byte[] key, byte[] iv)
			throws IOException {
	...
	private void encrypt() throws IOException {
		...
		/*
		 * If underlying stream supports {@link ByteBuffer} write in future,
		 * needs refine here.
		 */
	...
	private void updateEncryptor() throws IOException {
		final long counter = streamOffset / codec.getCipherSuite().getAlgorithmBlockSize();
		padding = (byte) (streamOffset % codec.getCipherSuite().getAlgorithmBlockSize());
