...
public interface Encryptor {
	 ...
	 * <p/>
	 * Certain modes, like CTR, require a different IV depending on the position
	 * in the stream. Generally, the encryptor maintains any necessary context
	 * for calculating the IV and counter so that no reinit is necessary during
	 * the encryption. Reinit before each operation is inefficient.
	 * 
	 ...
	 * potentially need to be called multiple times to process an entire buffer.
	 * The object may hold the encryption context internally.
	 * <p/>
	 ...
	 * 
	 * @param inBuffer
	 *            a direct {@link ByteBuffer} to read from. inBuffer may not be
	 *            null and inBuffer.remaining() must be > 0
	 * @param outBuffer
	 *            a direct {@link ByteBuffer} to write to. outBuffer may not be
	 *            null and outBuffer.remaining() must be > 0
	 * @throws IOException
	 *             if encryption fails
	 */
