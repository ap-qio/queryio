...
public class KeyShell extends Configured implements Tool {
	final static private String USAGE_PREFIX = "Usage: hadoop key " + "[generic options]\n";
	 ...
	 * @return 0 on success and 1 on failure. This value is passed back to the
	 *         unix shell, so we must follow shell return code conventions: the
	 *         return code is an unsigned character, and 0 means success, and
	...
	private class ListCommand extends Command {
		...
		public void execute() throws IOException {
			try {
				...
				out.println("Cannot list keys for KeyProvider: " + provider + ": " + e.toString());
				throw e;
			}
		}
	...
	private class RollCommand extends Command {
		...
		public void execute() throws NoSuchAlgorithmException, IOException {
			try {
				...
				out.println("Rolling key version from KeyProvider: " + provider + "\n  for key name: " + keyName);
				try {
					...
					out.println(
							"Cannot roll key: " + keyName + " within KeyProvider: " + provider + ". " + e.toString());
					throw e;
				}
			} catch (IOException e1) {
				out.println("Cannot roll key: " + keyName + " within KeyProvider: " + provider + ". " + e1.toString());
				throw e1;
			}
		}
	...
	private class DeleteCommand extends Command {
		public static final String USAGE = "delete <keyname> [-provider <provider>] [-f] [-help]";
		...
		public boolean validate() {
			...
			if (interactive) {
				try {
					cont = ToolRunner.confirmPrompt("You are about to DELETE all versions of " + " key " + keyName
							+ " from KeyProvider " + provider + ". Continue? ");
					if (!cont) {
		...
		public void execute() throws IOException {
			warnIfTransientProvider();
			out.println("Deleting key: " + keyName + " from KeyProvider: " + provider);
			if (cont) {
				try {
	...
	private class CreateCommand extends Command {
		...
		public boolean validate() {
			...
			if (keyName == null) {
				out.println("Please provide a <keyname>. See the usage description" + " with -help.");
				rc = false;
			}
			return rc;
		}
		...
		public void execute() throws IOException, NoSuchAlgorithmException {
			...
			try {
				...
				provider.flush();
				out.println(keyName + " has been successfully created with options " + options.toString() + ".");
				printProviderWritten();
	...
	/**
	 * main() entry point for the KeyShell. While strictly speaking the return
	 * is void, it will System.exit() with a return code: 0 is for success and 1
	 * for failure.
	 *
	 * @param args
	 *            Command line arguments.
