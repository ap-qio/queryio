...
/**
 * A simple LoadBalancing KMSClientProvider that round-robins requests across a
 * provided array of KMSClientProviders. It also retries failed requests on the
 * next available provider in the load balancer group. It only retries failed
 * requests that result in an IOException, sending back all other Exceptions to
 * the caller without retry.
 */
public class LoadBalancingKMSClientProvider extends KeyProvider
	...
	private <T> T doOp(ProviderCallable<T> op, int currPos) throws IOException {
		...
		for (int i = 0; i < providers.length; i++) {
			...
			try {
				...
				LOG.warn("KMS provider at [{}] threw an IOException [{}]!!", provider.getKMSUrl(), ioe.getMessage());
				ex = ioe;
			} catch (Exception e) {
	...
	@Override
	public EncryptedKeyVersion generateEncryptedKey(final String encryptionKeyName)
	...
	@Override
	public KeyVersion decryptEncryptedKey(final EncryptedKeyVersion encryptedKeyVersion)
			throws IOException, GeneralSecurityException {
		try {
			return doOp(new ProviderCallable<KeyVersion>() {
				@Override
				public KeyVersion call(KMSClientProvider provider) throws IOException, GeneralSecurityException {
	...
	public List<KeyVersion> getKeyVersions(final String name) throws IOException {
		return doOp(new ProviderCallable<List<KeyVersion>>() {
			@Override
			public List<KeyVersion> call(KMSClientProvider provider) throws IOException {
	...
	public KeyVersion createKey(final String name, final Options options) throws NoSuchAlgorithmException, IOException {
		try {
			return doOp(new ProviderCallable<KeyVersion>() {
				@Override
				public KeyVersion call(KMSClientProvider provider) throws IOException, NoSuchAlgorithmException {
