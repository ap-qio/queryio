...
/**
 * A KeyProvider with Cryptographic Extensions specifically for generating and
 * decrypting encrypted encryption keys.
 *
 */
...
public class KeyProviderCryptoExtension extends KeyProviderExtension<KeyProviderCryptoExtension.CryptoExtension> {
	...
	/**
	 * Designates a decrypted encrypted encryption key, that is, an encryption
	 * key (EK).
	 */
	...
	public static class EncryptedKeyVersion {
		 ...
		 *
		 * @param keyName
		 *            Name of the encryption key used to encrypt the encrypted
		 *            key.
		 * @param encryptionKeyVersionName
		 *            Version name of the encryption key used to encrypt the
		 *            encrypted key.
		 * @param encryptedKeyIv
		 *            Initialization vector of the encrypted key. The IV of the
		 *            encryption key used to encrypt the encrypted key is
		 *            derived from this IV.
		 ...
		 * returned EncryptedKeyVersion will only partially be populated; it is
		 * not necessarily suitable for operations besides decryption.
		 *
		 * @param keyName
		 ...
		 *            Key material of the encrypted key.
		...
		/**
		 * @return Version name of the encryption key used to encrypt the
		 *         encrypted key.
		 */
		 ...
		 *         encryption key used to encrypt the encrypted key is derived
		 *         from this IV.
		 */
		...
		/**
		 * Derive the initialization vector (IV) for the encryption key from the
		 * IV of the encrypted key. This derived IV is used with the encryption
		 * key to decrypt the encrypted key.
		 * <p/>
		 * The alternative to this is using the same IV for both the encryption
		 * key and the encrypted key. Even a simple symmetric transformation
		 * like this improves security by avoiding IV re-use. IVs will also be
		 * fairly unique among different EEKs.
		 *
		 * @param encryptedKeyIV
		 *            of the encrypted key (i.e. {@link #getEncryptedKeyIv()})
	...
	public interface CryptoExtension extends KeyProviderExtension.Extension {
		 ...
		 *
		 * @param keyName
		 *            the key to drain the Queue for
		 */
		public void drain(String keyName);

		/**
		 * Generates a key material and encrypts it using the given key version
		 * name and initialization vector. The generated key material is of the
		 * same length as the <code>KeyVersion</code> material of the latest key
		 * version of the key and is encrypted using the same cipher.
		 * <p/>
		 * NOTE: The generated key is not stored by the <code>KeyProvider</code>
		 *
		 * @param encryptionKeyName
		 *            The latest KeyVersion of this key's material will be
		 *            encrypted.
		 * @return EncryptedKeyVersion with the generated key material, the
		 *         version name is 'EEK' (for Encrypted Encryption Key)
		 * @throws IOException
		 *             thrown if the key material could not be generated
		 * @throws GeneralSecurityException
		 *             thrown if the key material could not be encrypted because
		 *             of a cryptographic issue.
		 */
		public EncryptedKeyVersion generateEncryptedKey(String encryptionKeyName)
				throws IOException, GeneralSecurityException;

		/**
		 * Decrypts an encrypted byte[] key material using the given a key
		 * version name and initialization vector.
		 *
		 * @param encryptedKeyVersion
		 *            contains keyVersionName and IV to decrypt the encrypted
		 *            key material
		 * @return a KeyVersion with the decrypted key material, the version
		 *         name is 'EK' (For Encryption Key)
		 * @throws IOException
		 *             thrown if the key material could not be decrypted
		 * @throws GeneralSecurityException
		 *             thrown if the key material could not be decrypted because
		 *             of a cryptographic issue.
		 */
	...
	private static class DefaultCryptoExtension implements CryptoExtension {
		...
		public EncryptedKeyVersion generateEncryptedKey(String encryptionKeyName)
			...
			return new EncryptedKeyVersion(encryptionKeyName, encryptionKey.getVersionName(), iv,
	 ...
	 * {@link KeyProviderCryptoExtension}
	 * 
	 * @param keyProvider
	 * @param extension
	 */
	 ...
	 *             thrown if the key material could not be generated
	 * @throws GeneralSecurityException
	 *             thrown if the key material could not be encrypted because of
	 ...
	 *             thrown if the key material could not be decrypted
	 * @throws GeneralSecurityException
	 *             thrown if the key material could not be decrypted because of
	 *             a cryptographic issue.
	 */
	...
	public static KeyProviderCryptoExtension createKeyProviderCryptoExtension(KeyProvider keyProvider) {
		CryptoExtension cryptoExtension = (keyProvider instanceof CryptoExtension) ? (CryptoExtension) keyProvider
