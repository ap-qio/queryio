 ...
 * available for the client to consume for a particular key. It also uses an
 * underlying Cache to evict queues for keys that have not been accessed for a
 * configurable period of time. Implementing classes are required to implement
 * the <code>QueueRefiller</code> interface that exposes a method to refill the
 * queue, when empty
 */
...
public class ValueQueue<E> {
	...
	public interface QueueRefiller<E> {
		 ...
		 * @param numValues
		 *            number of Values to be added to the queue.
		 * @throws IOException
		 */
		public void fillQueueForKey(String keyName, Queue<E> keyQueue, int numValues) throws IOException;
	}
	 ...
	 * Queue accepts a task only if the task is not currently in the process of
	 * being run by a thread which is implied by the presence of the key in the
	 * <code>keysInProgress</code> set.
	 *
	 ...
	 * "n" values and Queue is empty. This decides how many values to return
	 * when client calls "getAtMost"
	 */
	 ...
	 * 
	 * @param numValues
	 *            The number of values cached in the Queue for a particular key.
	 * @param lowWatermark
	 *            The ratio of (number of current entries/numValues) below which
	 *            the <code>fillQueueForKey()</code> funciton will be invoked to
	 *            fill the Queue.
	 * @param expiry
	 *            Expiry time after which the Key and associated Queue are
	 *            evicted from the cache.
	 * @param numFillerThreads
	 *            Number of threads to use for the filler thread
	 * @param policy
	 *            The SyncGenerationPolicy to use when client calls "getAtMost"
	 * @param refiller
	 *            implementation of the QueueRefiller
	 */
	public ValueQueue(final int numValues, final float lowWatermark, long expiry, int numFillerThreads,
			SyncGenerationPolicy policy, final QueueRefiller<E> refiller) {
		...
		executor = new ThreadPoolExecutor(numFillerThreads, numFillerThreads, 0L, TimeUnit.MILLISECONDS, queue,
				new ThreadFactoryBuilder().setDaemon(true).setNameFormat(REFILL_THREAD).build());
	}

	public ValueQueue(final int numValues, final float lowWaterMark, long expiry, int numFillerThreads,
			QueueRefiller<E> fetcher) {
		this(numValues, lowWaterMark, expiry, numFillerThreads, SyncGenerationPolicy.ALL, fetcher);
	}

	/**
	 * Initializes the Value Queues for the provided keys by calling the fill
	 * Method with "numInitValues" values
	 * 
	 * @param keyNames
	 *            Array of key Names
	 ...
	 * provided key. Will immediately fire the Queue filler function if key does
	 * not exist. If Queue exists but all values are drained, It will ask the
	 * generator function to add 1 value to Queue and then drain it.
	 * 
	 * @param keyName
	 *            String key name
	 ...
	 * provided key. Will immediately fire the Queue filler function if key does
	 * not exist How many values are actually returned is governed by the
	...
	public List<E> getAtMost(String keyName, int num) throws IOException, ExecutionException {
		...
		try {
			for (int i = 0; i < num; i++) {
				E val = keyQueue.poll();
				// If queue is empty now, Based on the provided
				// SyncGenerationPolicy,
				// figure out how many new values need to be generated
				// synchronously
				if (val == null) {
