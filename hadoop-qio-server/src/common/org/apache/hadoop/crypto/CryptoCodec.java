...
public abstract class CryptoCodec implements Configurable {
		...
		for (Class<? extends CryptoCodec> klass : klasses) {
			try {
				...
				if (c.getCipherSuite().getName().equals(cipherSuite.getName())) {
					...
					PerformanceAdvisory.LOG.debug("Crypto codec {} doesn't meet the cipher suite {}.", klass.getName(),
							cipherSuite.getName());
				}
			} catch (Exception e) {
	...
	private static List<Class<? extends CryptoCodec>> getCodecClasses(Configuration conf, CipherSuite cipherSuite) {
		...
		String configName = HADOOP_SECURITY_CRYPTO_CODEC_CLASSES_KEY_PREFIX + cipherSuite.getConfigSuffix();
	...
	/**
	 * This interface is only for Counter (CTR) mode. Generally the Encryptor or
	 * Decryptor calculates the IV and maintain encryption context internally.
	 ...
	 *
	 * @param bytes
	 *            byte array to populate with random data
	 */
