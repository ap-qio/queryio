...
/**
 * Used to registry custom methods to refresh at runtime. Each identifier maps
 * to one or more RefreshHandlers.
 */
...
public class RefreshRegistry {
	...
	/**
	 * Registers an object as a handler for a given identity. Note: will prevent
	 * handler from being GC'd, object should unregister itself when done
	 * 
	 * @param identifier
	 *            a unique identifier for this resource, such as
	 *            org.apache.hadoop.blacklist
	 * @param handler
	...
	/**
	 * Lookup the responsible handler and return its result. This should be
	 * called by the RPC server when it gets a refresh request.
	 * 
	 * @param identifier
	 *            the resource to refresh
	 * @param args
	 *            the arguments to pass on, not including the program name
	 * @throws IllegalArgumentException
	 *             on invalid identifier
	...
	public synchronized Collection<RefreshResponse> dispatch(String identifier, String[] args) {
		...
		for (RefreshHandler handler : handlers) {
			...
			try {
				...
				LOG.info(handlerName(handler) + " responds to '" + identifier + "', says: '" + response.getMessage()
						+ "', returns " + response.getReturnCode());
			} catch (Exception e) {
