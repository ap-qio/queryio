...
public abstract class Server {
	 ...
	 * terse exception group for concise logging messages
	 */
	...
	public static void registerProtocolEngine(RPC.RpcKind rpcKind, Class<? extends Writable> rpcRequestWrapperClass,
		...
		LOG.debug("rpcKind=" + rpcKind + ", rpcRequestWrapperClass=" + rpcRequestWrapperClass + ", rpcInvoker="
				+ rpcInvoker);
	}
	...
	/**
	 * Returns the RPC remote user when invoked inside an RPC. Note this may be
	 * different than the current user if called within another doAs
	 * 
	...
	private int readerPendingConnectionQueue; // number of connections to queue
												// per read thread
	...
	/**
	 * Refresh the service authorization ACL for the service handled by this
	 * server using the specified Configuration.
	 */
	@Private
	...
	private class Listener extends Thread {
		...
		private int backlogLength = conf.getInt(CommonConfigurationKeysPublic.IPC_SERVER_LISTEN_QUEUE_SIZE_KEY,
		...
		private class Reader extends Thread {
			...
			private synchronized void doRunLoop() {
				while (running) {
					...
					try {
						// consume as many connections as currently queued to
						// avoid
			 ...
			 * thread-safe, so the connection must be queued. The reader will
			 * drain the queue and update its readSelector before performing the
			 * next select
			 */
		...
		void doRead(SelectionKey key) throws InterruptedException {
			...
			try {
				...
				// a WrappedRpcServerException is an exception that has been
				// sent
				// to the client, so the stacktrace is unnecessary; any other
				// exceptions are unexpected internal server errors and thus the
				// stacktrace should be logged
				LOG.info(Thread.currentThread().getName() + ": readAndProcess from client " + c.getHostAddress()
						+ " threw exception [" + e + "]", (e instanceof WrappedRpcServerException) ? null : e);
			...
			try {
				synchronized (responseQueue) {
					...
					if (!call.rpcResponse.hasRemaining()) {
						...
						if (LOG.isDebugEnabled()) {
							LOG.debug(Thread.currentThread().getName() + ": responding to " + call + " Wrote "
									+ numBytes + " bytes.");
						}
					} else {
						//
						...
						if (LOG.isDebugEnabled()) {
							LOG.debug(Thread.currentThread().getName() + ": responding to " + call + " Wrote partial "
									+ numBytes + " bytes.");
						}
					}
	...
	/**
	 * Wrapper for RPC IOExceptions to be returned to the client. Used to let
	 * exceptions bubble up to top of processOneRpc where the correct callId can
	 * be associated with the response. Also used to prevent unnecessary stack
	 * trace logging if it's not an internal server error.
	 */
	...
	public class Connection {
		private boolean connectionHeaderRead = false; // connection header is
														// read?
		private boolean connectionContextRead = false; // if connection context
														// that
		...
		private final Call authFailedCall = new Call(AUTHORIZATION_FAILED_CALL_ID, RpcConstants.INVALID_RETRY_COUNT,
				null, this);
		private ByteArrayOutputStream authFailedResponse = new ByteArrayOutputStream();

		private final Call saslCall = new Call(AuthProtocol.SASL.callId, RpcConstants.INVALID_RETRY_COUNT, null, this);
		...
		private UserGroupInformation getAuthorizedUgi(String authorizedId) throws InvalidToken, AccessControlException {
		...
		private void saslReadAndProcess(DataInputStream dis)
			...
			case WRAP: {
				if (!saslContextEstablished || !useWrap) {
					throw new WrappedRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER,
		...
		private Throwable getCauseForInvalidToken(IOException e) {
			...
			while (cause != null) {
				if (cause instanceof RetriableException) {
					...
					// callbacks to only returning InvalidToken, but some
					// services
					// need to throw other exceptions (ex. NN +
					// StandyException),
		...
		private void saslProcess(RpcSaslProto saslMessage)
			...
			try {
				try {
					...
					AUDITLOG.warn(AUTH_FAILED_FOR + this.toString() + ":" + attemptingUser + " ("
		...
		private RpcSaslProto processSaslMessage(RpcSaslProto saslMessage) throws IOException, InterruptedException {
		...
		private RpcSaslProto processSaslToken(RpcSaslProto saslMessage) throws SaslException {
		...
		private RpcSaslProto buildSaslResponse(SaslState state, byte[] replyToken) {
			if (LOG.isDebugEnabled()) {
				LOG.debug("Will send " + state + " token of size " + ((replyToken != null) ? replyToken.length : null)
						+ " from saslServer.");
			}
		...
		private void doSaslReply(Message message) throws IOException {
			...
			setupResponse(saslResponse, saslCall, RpcStatusProto.SUCCESS, null, new RpcResponseWrapper(message), null,
					null);
		...
		private void doSaslReply(Exception ioe) throws IOException {
			setupResponse(authFailedResponse, authFailedCall, RpcStatusProto.FATAL,
					RpcErrorCodeProto.FATAL_UNAUTHORIZED, null, ioe.getClass().getName(), ioe.getLocalizedMessage());
			...
			case NONE: {
				...
				if (!isSimpleEnabled) {
					IOException ioe = new AccessControlException(
							"SIMPLE authentication is not enabled." + "  Available:" + enabledAuthMethods);
					doSaslReply(ioe);
					throw ioe;
				}
				break;
		...
		private SaslServer createSaslServer(AuthMethod authMethod) throws IOException, InterruptedException {
			final Map<String, ?> saslProps = saslPropsResolver.getServerProperties(addr);
		 ...
		 * incompatible with the server. This can contain special-case code to
		 * speak enough of past IPC protocols to pass back an exception to the
		 * caller.
		 * 
		 * @param clientVersion
		...
		private void setupBadVersionResponse(int clientVersion) throws IOException {
			String errMsg = "Server IPC version " + CURRENT_VERSION + " cannot communicate with client version "
					+ clientVersion;
			...
			if (clientVersion >= 9) {
				...
				setupResponse(buffer, fakeCall, RpcStatusProto.FATAL, RpcErrorCodeProto.FATAL_VERSION_MISMATCH, null,
		...
		private void setupHttpRequestOnIpcPortResponse() throws IOException {
			Call fakeCall = new Call(0, RpcConstants.INVALID_RETRY_COUNT, null, this);
			fakeCall.setResponse(ByteBuffer.wrap(RECEIVED_HTTP_REQ_RESPONSE.getBytes(Charsets.UTF_8)));
			responder.doRespond(fakeCall);
		}
		 ...
		 * 
		 * @param dis
		 *            - DataInputStream from which to read the header
		 * @throws WrappedRpcServerException
		 *             - if the header cannot be deserialized, or the user is
		 *             not authorized
		 */
		 ...
		 * @throws IOException
		 *             - SASL packet cannot be unwrapped
		 ...
		 * 
		 * @param buf
		 *            - contains the RPC request header and the rpc request
		 * @throws IOException
		 *             - internal error that should not be returned to client,
		 *             typically failure to respond to client
		 * @throws WrappedRpcServerException
		 *             - an exception to be sent back to the client that does
		 *             not require verbose logging by the Listener thread
		 * @throws InterruptedException
		 */
		private void processOneRpc(byte[] buf) throws IOException, WrappedRpcServerException, InterruptedException {
			...
			try {
				...
				if (callId < 0) { // callIds typically used during connection
									// setup
				...
				setupResponse(authFailedResponse, call, RpcStatusProto.FATAL, wrse.getRpcErrorCodeProto(), null,
		 ...
		 */
		private void checkRpcHeaders(RpcRequestHeaderProto header) throws WrappedRpcServerException {
			if (!header.hasRpcOp()) {
			...
			// (Note it would make more sense to have the handler deserialize
			// but
		 ...
		 *            - RPC request header
		 * @param dis
		 *            - stream to request payload
		 * @throws WrappedRpcServerException
		 *             - due to fatal rpc layer issues such as invalid header or
		 *             deserialization error. In this case a RPC fatal status
		 *             response will later be sent back to client.
		 * @throws InterruptedException
		 */
		private void processRpcRequest(RpcRequestHeaderProto header, DataInputStream dis)
			...
			try { // Read the rpc request
				...
				rpcRequest.readFields(dis);
									...
									// newInstance
				LOG.warn("Unable to read call parameters for client " + getHostAddress() + "on connection protocol "
			...
			if (header.hasTraceInfo()) {
				// If the incoming RPC included tracing info, always continue
				// the trace
		 ...
		 *            - stream to request payload
		 * @throws WrappedRpcServerException
		 *             - setup failed due to SASL negotiation failure, premature
		 *             or invalid connection context, or other state errors
		 * @throws IOException
		 *             - failed to send a response back to the client
		 * @throws InterruptedException
		 */
		private void processRpcOutOfBandRequest(RpcRequestHeaderProto header, DataInputStream dis)
				throws WrappedRpcServerException, IOException, InterruptedException {
		...
		private void authorizeConnection() throws WrappedRpcServerException {
			try {
				...
				LOG.info("Connection from " + this + " for protocol " + connectionContext.getProtocol()
		 ...
		 * 
		 * @param builder
		 *            - Builder of the protobuf to decode
		 * @param dis
		...
		private <T extends Message> T decodeProtobufFromStream(Builder builder, DataInputStream dis)
				throws WrappedRpcServerException {
			try {
				...
				try {
					...
					try {
						...
						if (call.connection.user == null) {
							...
							value = call.connection.user.doAs(new PrivilegedExceptionAction<Writable>() {
								...
								public Writable run() throws Exception {
									// make the call
									return call(call.rpcKind, call.connection.protocolName, call.rpcRequest,
											call.timestamp);
						...
						if (exceptionsHandler.isTerse(e.getClass())) {
							...
							// These exception types indicate something is
							// probably wrong
					...
					synchronized (call.connection.responseQueue) {
						...
						// Discard the large buf and reset it back to smaller
						// size
						// to free up heap
	...
	protected Server(String bindAddress, int port, Class<? extends Writable> rpcRequestClass, int handlerCount,
		...
		this(bindAddress, port, rpcRequestClass, handlerCount, numReaders, queueSizePerHandler, conf, serverName,
				secretManager, null);
	}
	 ...
	 * calls. If queueSizePerHandler or numReaders are not -1 they will be used
	 * instead of parameters from configuration. Otherwise the configuration
	 * will be picked up.
	 * 
	 * If rpcRequestClass is null then the rpcRequestClass must have been
	 * registered via
	 * {@link #registerProtocolEngine(RpcPayloadHeader.RpcKind, Class, RPC.RpcInvoker)}
	...
	private RpcSaslProto buildNegotiateResponse(List<AuthMethod> authMethods) throws IOException {
		...
		if (authMethods.contains(AuthMethod.SIMPLE) && authMethods.size() == 1) {
			...
			for (AuthMethod authMethod : authMethods) {
				...
				SaslAuth.Builder builder = negotiateBuilder.addAuthsBuilder().setMethod(authMethod.toString())
	 ...
	 * @throws IOException
	 */
	private void setupResponseOldVersionFatal(ByteArrayOutputStream response, Call call, Writable rv, String errorClass,
	...
	private void wrapWithSasl(ByteArrayOutputStream response, Call call) throws IOException {
		if (call.connection.saslServer != null) {
			...
			RpcResponseHeaderProto saslHeader = RpcResponseHeaderProto.newBuilder().setCallId(AuthProtocol.SASL.callId)
					.setStatus(RpcStatusProto.SUCCESS).build();
			RpcSaslProto saslMessage = RpcSaslProto.newBuilder().setState(SaslState.WRAP)
					.setToken(ByteString.copyFrom(token, 0, token.length)).build();
	 ...
	 * 
	 * @deprecated Use
	 *             {@link #call(RpcPayloadHeader.RpcKind, String, Writable, long)}
	 *             instead
	 */
	@Deprecated
	...
	/** Called for each call. */
	public abstract Writable call(RPC.RpcKind rpcKind, String protocol, Writable param, long receiveTime)
			throws Exception;
	 ...
	 */
	private void authorize(UserGroupInformation user, String protocolName, InetAddress addr)
			throws AuthorizationException {
		if (authorize) {
	...
	private class ConnectionManager {
		...
		ConnectionManager() {
			...
			this.idleScanInterval = conf.getInt(CommonConfigurationKeys.IPC_CLIENT_CONNECTION_IDLESCANINTERVAL_KEY,
			...
			this.maxIdleToClose = conf.getInt(CommonConfigurationKeysPublic.IPC_CLIENT_KILL_MAX_KEY,
					CommonConfigurationKeysPublic.IPC_CLIENT_KILL_MAX_DEFAULT);
			this.maxConnections = conf.getInt(CommonConfigurationKeysPublic.IPC_SERVER_MAX_CONNECTIONS_KEY,
		...
		Connection register(SocketChannel channel) {
			...
			add(connection);
			if (LOG.isDebugEnabled()) {
				LOG.debug("Server connection from " + connection + "; # active connections: " + size()
		...
		boolean close(Connection connection) {
			...
			if (exists) {
				if (LOG.isDebugEnabled()) {
					LOG.debug(Thread.currentThread().getName() + ": disconnecting client " + connection
		...
		synchronized void closeIdle(boolean scanAll) {
			...
			for (Connection connection : connections) {
				...
				if (connection.isIdle() && connection.getLastContact() < minLastContact && close(connection) && !scanAll
			...
			TimerTask idleScanTask = new TimerTask() {
				...
				public void run() {
					...
					try {
						...
						// explicitly reschedule so next execution occurs
						// relative
