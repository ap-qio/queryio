...
public interface RpcEngine {
	 ...
	 * @param <T>
	 */
	<T> ProtocolProxy<T> getProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
	 ...
	 * @param instance
	 *            the instance of protocol whose methods will be called
	 * @param conf
	 ...
	 *            the size of the queue per hander thread
	 * @param verbose
	 *            whether each call should be logged
	 * @param secretManager
	 *            The secret manager to use to validate incoming requests.
	 * @param portRangeConfig
	 *            A config parameter that can be used to restrict the range of
	 *            ports used when port is 0 (an ephemeral port)
	 * @return The Server instance
	 * @throws IOException
	 *             on any error
	 */
	RPC.Server getServer(Class<?> protocol, Object instance, String bindAddress, int port, int numHandlers,
	 ...
	 * 
	 * @param connId,
	 *            ConnectionId to be used for the proxy.
	 * @param conf,
	 *            Configuration.
	 * @param factory,
	 *            Socket factory.
	 ...
	 * @throws IOException
	 */
	ProtocolProxy<ProtocolMetaInfoPB> getProtocolMetaInfoProxy(ConnectionId connId, Configuration conf,
