...
public class FairCallQueue<E extends Schedulable> extends AbstractQueue<E> implements BlockingQueue<E> {
	 ...
	 * 
	 * @param capacity
	 *            the maximum size of each sub-queue
	 * @param ns
	 *            the prefix to use for configuration
	 * @param conf
	 *            the configuration to read from Notes: the FairCallQueue has no
	 *            fixed capacity. Rather, it has a minimum capacity of
	 *            `capacity` and a maximum capacity of `capacity *
	 *            number_queues`
	 */
	...
	/**
	 * Read the number of queues from the configuration. This will affect the
	 * FairCallQueue's overall capacity.
	 * 
	 * @throws IllegalArgumentException
	 *             on invalid queue count
	 */
	...
	/**
	 * Returns the first non-empty queue with equal or lesser priority than
	 * <i>startIdx</i>. Wraps around, searching a maximum of N queues, where N
	 * is this.queues.size().
	 *
	 * @param startIdx
	 *            the queue number to start searching at
	...
	/**
	 * Put and offer follow the same pattern: 1. Get a priorityLevel from the
	 * scheduler 2. Get the nth sub-queue matching this priorityLevel 3.
	 * delegate the call to this sub-queue.
	 *
	 * But differ in how they handle overflow: - Put will move on to the next
	 * queue until it lands on the last queue - Offer does not attempt other
	 * queues on overflow
	 */
	@Override
		...
		while (true) {
			...
			if (!res) {
				...
				if (priorityLevel == numLevels) {
					// That was the last one, we will block on put in the last
					// queue
	 ...
	 * capacity. Note: size provides no strict consistency, and should not be
	 * used to control queue IO.
	 */
	@Override
	public int size() {
		int size = 0;
	 ...
	 * to another FairCallQueue will likely fail, since the incoming calls may
	 * be scheduled differently in the new FairCallQueue. Nonetheless this
	...
	/**
	 * Returns maximum remaining capacity. This does not reflect how much you
	 * can ideally fit in this FairCallQueue, as that would depend on the
	...
	/**
	 * MetricsProxy is a singleton because we may init multiple FairCallQueues,
	 * but the metrics system cannot unregister beans cleanly.
	 */
