...
public class RetryCache {
	...
	public static class CacheEntry implements LightWeightCache.Entry {
		...
		CacheEntry(byte[] clientId, int callId, long expirationTime) {
			...
			Preconditions.checkArgument(clientId.length == ClientId.BYTE_LENGTH,
					"Invalid clientId - length is " + clientId.length + " expected length " + ClientId.BYTE_LENGTH);
	 ...
	 * previous response to be used for generating response for retried
	 * requests.
	 */
	 ...
	 * completion state is {@link CacheEntry#FAILED}, the expectation is that
	 * the thread that waited for completion, retries the request. the
	...
	private CacheEntry waitForCompletion(CacheEntry newEntry) {
		...
		try {
			...
			if (mapEntry == null) {
				if (LOG.isTraceEnabled()) {
					LOG.trace("Adding Rpc request clientId " + newEntry.clientIdMsb + newEntry.clientIdLsb + " callId "
		...
		// Entry already exists in cache. Wait for completion and return its
		// state
		...
		synchronized (mapEntry) {
			...
			// Previous request has failed, the expectation is is that it will
			// be
			// retried again.
	...
	private static CacheEntryWithPayload newEntry(Object payload, long expirationTime) {
		return new CacheEntryWithPayload(Server.getClientId(), Server.getCallId(), payload,
	...
	public static CacheEntryWithPayload waitForCompletion(RetryCache cache, Object payload) {
		...
		return (CacheEntryWithPayload) (cache != null ? cache.waitForCompletion(newEntry(payload, cache.expirationTime))
				: null);
	}
