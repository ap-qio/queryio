...
public class RPC {
	...
	interface RpcInvoker {
		 ...
		 * @param protocol
		 *            - the protocol name (the class of the client proxy used to
		 *            make calls to the rpc server.
		 * @param rpcRequest
		 *            - deserialized
		 * @param receiveTime
		 ...
		 * @throws IOException
		 **/
		public Writable call(Server server, String protocol, Writable rpcRequest, long receiveTime) throws Exception;
	}
	...
	/**
	 * Get all interfaces that the given protocol implements or extends which
	 * are assignable from VersionedProtocol.
	 */
	...
	/**
	 * Get the protocol name. If the protocol class has a ProtocolAnnotation,
	 * then get the protocol name from the annotation; otherwise the class name
	...
	/**
	 * Get the protocol version from protocol class. If the protocol class has a
	 * ProtocolAnnotation, then get the protocol name from the annotation;
	 * otherwise the class name is the protocol name.
	 */
	 ...
	 */
	public static <T> T waitForProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr, Configuration conf)
			throws IOException {
	 ...
	 *             if the far end through a RemoteException
	 */
	public static <T> ProtocolProxy<T> waitForProtocolProxy(Class<T> protocol, long clientVersion,
	 ...
	 */
	public static <T> T waitForProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr, Configuration conf,
			int rpcTimeout, long timeout) throws IOException {
	 ...
	 */
	public static <T> ProtocolProxy<T> waitForProtocolProxy(Class<T> protocol, long clientVersion,
			InetSocketAddress addr, Configuration conf, int rpcTimeout, RetryPolicy connectionRetryPolicy, long timeout)
			throws IOException {
		long startTime = Time.now();
		IOException ioe;
		while (true) {
			try {
				return getProtocolProxy(protocol, clientVersion, addr, UserGroupInformation.getCurrentUser(), conf,
				...
				ioe = te;
			} catch (NoRouteToHostException nrthe) { // perhaps a VIP is failing
														// over
	 ...
	 *             if the far end through a RemoteException
	 */
	public static <T> ProtocolProxy<T> getProtocolProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
	...
	/**
	 * Construct a client-side proxy that implements the named protocol, talking
	 * to a server at the named address.
	 * 
	 * @param <T>
	 * 
	 * @param protocol
	 ...
	 *            socket factory
	 * @param rpcTimeout
	 *            max time for each rpc; 0 means no timeout
	 * @return the proxy
	 * @throws IOException
	 *             if any error occurs
	 */
	public static <T> T getProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
			UserGroupInformation ticket, Configuration conf, SocketFactory factory, int rpcTimeout) throws IOException {
	...
	public static <T> ProtocolProxy<T> getProtocolProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
		...
		return getProtocolProxy(protocol, clientVersion, addr, ticket, conf, factory, rpcTimeout, connectionRetryPolicy,
				null);
	}
	...
	public static <T> ProtocolProxy<T> getProtocolProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
		...
		return getProtocolEngine(protocol, conf).getProxy(protocol, clientVersion, addr, ticket, conf, factory,
	...
	/**
	 * Return the connection ID of the given object. If the provided object is
	 * in fact a protocol translator, we'll get the connection ID of the
	 * underlying proxy object.
	 * 
	 * @param proxy
	 *            the proxy object to get the connection ID of.
	...
	/**
	 * Stop the proxy. Proxy must either implement {@link Closeable} or must
	 * have associated {@link RpcInvocationHandler}.
	 * 
	 * @param proxy
	 *            the RPC proxy object to be stopped
	 * @throws HadoopIllegalArgumentException
	 *             if the proxy does not implement {@link Closeable} interface
	 *             or does not have closeable {@link InvocationHandler}
	 */
	...
	public static class Builder {
		...
		public Server build() throws IOException, HadoopIllegalArgumentException {
			...
			return getProtocolEngine(this.protocol, this.conf).getServer(this.protocol, this.instance, this.bindAddress,
					this.port, this.numHandlers, this.numReaders, this.queueSizePerHandler, this.verbose, this.conf,
	...
	public abstract static class Server extends org.apache.hadoop.ipc.Server {
		...
		ArrayList<Map<ProtoNameVer, ProtoClassProtoImpl>> protocolImplMapArray = new ArrayList<Map<ProtoNameVer, ProtoClassProtoImpl>>(
				RpcKind.MAX_INDEX);
		...
		void registerProtocolAndImpl(RpcKind rpcKind, Class<?> protocolClass, Object protocolImpl) {
			...
			LOG.debug("RpcKind = " + rpcKind + " Protocol Name = " + protocolName + " version=" + version
					+ " ProtocolImpl=" + protocolImpl.getClass().getName() + " protocolClass="
		...
		VerProtocolImpl[] getSupportedProtocolVersions(RPC.RpcKind rpcKind, String protocolName) {
			...
			int i = 0;
			for (Map.Entry<ProtoNameVer, ProtoClassProtoImpl> pv : getProtocolImplMap(rpcKind).entrySet()) {
		 ...
		 *            - the protocol class
		 * @param protocolImpl
		 *            - the impl of the protocol that will be called
		...
		@Override
		public Writable call(RPC.RpcKind rpcKind, String protocol, Writable rpcRequest, long receiveTime)
				throws Exception {
