...
/**
 * The decay RPC scheduler counts incoming requests in a map, then decays the
 * counts at a fixed time interval. The scheduler is optimized for large periods
 * (on the order of seconds), as it offloads work to the decay sweep.
 */
public class DecayRpcScheduler implements RpcScheduler, DecayRpcSchedulerMXBean {
	 ...
	 * 10, 40, 80 implies 4 queues, with - q3 from 80% up - q2 from 40 up to 80
	 * - q1 from 10 up to 40 - q0 otherwise.
	 */
	...
	private final ConcurrentHashMap<Object, AtomicLong> callCounts = new ConcurrentHashMap<Object, AtomicLong>();
	...
	private final AtomicReference<Map<Object, Integer>> scheduleCacheRef = new AtomicReference<Map<Object, Integer>>();
	...
	private final int numQueues; // affects scheduling decisions, from 0 to
									// numQueues - 1
	private final double[] thresholds;
	private final IdentityProvider identityProvider;

	/**
	 * This TimerTask will call decayCurrentCounts until the scheduler has been
	 * garbage collected.
	 */
	public static class DecayTask extends TimerTask {
		...
		public void run() {
			...
			if (sched != null) {
				...
				// Our scheduler was garbage collected since it is no longer in
				// use,
	 ...
	 * 
	 * @param numQueues
	 *            number of queues to schedule for
	 * @param ns
	 *            config prefix, so that we can configure multiple schedulers in
	 *            a single instance.
	 * @param conf
	...
	private IdentityProvider parseIdentityProvider(String ns, Configuration conf) {
		List<IdentityProvider> providers = conf.getInstances(
				ns + "." + CommonConfigurationKeys.IPC_CALLQUEUE_IDENTITY_PROVIDER_KEY, IdentityProvider.class);
	...
	private static long parseDecayPeriodMillis(String ns, Configuration conf) {
		long period = conf.getLong(ns + "." + IPC_CALLQUEUE_DECAYSCHEDULER_PERIOD_KEY,
	...
	private static double[] parseThresholds(String ns, Configuration conf, int numQueues) {
		...
		if (percentages.length == 0) {
			...
			throw new IllegalArgumentException(
					"Number of thresholds should be " + (numQueues - 1) + ". Was: " + percentages.length);
		}
	...
	/**
	 * Generate default thresholds if user did not specify. Strategy is to halve
	 * each time, since queue usage tends to be exponential. So if numQueues is
	 * 4, we would generate: double[]{0.125, 0.25, 0.5} which specifies the
	 * boundaries between each queue's usage.
	 * 
	 * @param numQueues
	 *            number of queues to compute for
	...
	/**
	 * Decay the stored counts for each user and clean as necessary. This method
	 * should be called periodically in order to keep counts current.
	 */
	private void decayCurrentCounts() {
		...
		while (it.hasNext()) {
			...
			if (nextValue == 0) {
				// We will clean up unused keys here. An interesting
				// optimization might
	 ...
	 * 
	 * @param identity
	 *            the identity of the user to increment
	...
	private long getAndIncrement(Object identity) throws InterruptedException {
		...
		if (count == null) {
			...
			// Put it in, or get the AtomicInteger that was put in by another
			// thread
	...
	/**
	 * Returns the priority level for a given identity by first trying the
	 * cache, then computing it.
	 * 
	 * @param identity
	 *            an object responding to toString and hashCode
	...
	/**
	 * Compute the appropriate priority for a schedulable based on past
	 * requests.
	 * 
	 * @param obj
	...
	/**
	 * MetricsProxy is a singleton because we may init multiple schedulers and
	 * we want to clean up resources when a new scheduler replaces the old one.
	 */
