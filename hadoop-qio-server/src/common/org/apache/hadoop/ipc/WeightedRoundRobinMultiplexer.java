 ...
 * [9, 4, 1] for 3 queues:
 *
 * The cycle is (a minimum of) 9+4+1=14 reads. Queue 0 is read (at least) 9
 * times Queue 1 is read (at least) 4 times Queue 2 is read (at least) 1 time
 * Repeat
 *
...
public class WeightedRoundRobinMultiplexer implements RpcMultiplexer {
	...
	private final AtomicInteger currentQueueIndex; // Current queue we're
													// serving
	private final AtomicInteger requestsLeft; // Number of requests left for
												// this queue
	...
	public WeightedRoundRobinMultiplexer(int aNumQueues, String ns, Configuration conf) {
		...
		if (this.queueWeights.length == 0) {
			...
			throw new IllegalArgumentException(ns + "." + IPC_CALLQUEUE_WRRMUX_WEIGHTS_KEY + " must specify exactly "
	...
	/**
	 * Advances the index, which will change the current index if called enough
	 * times.
	 */
	private void advanceIndex() {
		...
		if (requestsLeftVal == 0) {
			// This is guaranteed to be called exactly once per
			// currentQueueIndex
	...
	/**
	 * Gets the current index. Should be accompanied by a call to advanceIndex
	 * at some point.
	 */
