...
public class RpcMetrics {
	...
	RpcMetrics(Server server, Configuration conf) {
		...
		rpcQuantileEnable = (intervals.length > 0)
				&& conf.getBoolean(CommonConfigurationKeys.RPC_METRICS_QUANTILE_ENABLE,
		...
		if (rpcQuantileEnable) {
			...
			for (int i = 0; i < intervals.length; i++) {
				...
				rpcQueueTimeMillisQuantiles[i] = registry.newQuantiles("rpcQueueTime" + interval + "s",
						"rpc queue time in milli second", "ops", "latency", interval);
				rpcProcessingTimeMillisQuantiles[i] = registry.newQuantiles("rpcProcessingTime" + interval + "s",
