 ...
 * signatures, keyed by server address, protocol and rpc kind. The cache is
 * lazily populated.
 */
public class RpcClientUtil {
	...
	private static class ProtoSigCacheKey {
		...
		public boolean equals(Object other) {
			...
			if (other instanceof ProtoSigCacheKey) {
				...
				return (serverAddress.equals(otherKey.serverAddress) && protocol.equals(otherKey.protocol)
	...
	private static ConcurrentHashMap<ProtoSigCacheKey, Map<Long, ProtocolSignature>> signatureMap = new ConcurrentHashMap<ProtoSigCacheKey, Map<Long, ProtocolSignature>>();

	private static void putVersionSignatureMap(InetSocketAddress addr, String protocol, String rpcKind,
			Map<Long, ProtocolSignature> map) {
		signatureMap.put(new ProtoSigCacheKey(addr, protocol, rpcKind), map);
	}

	private static Map<Long, ProtocolSignature> getVersionSignatureMap(InetSocketAddress addr, String protocol,
			String rpcKind) {
		return signatureMap.get(new ProtoSigCacheKey(addr, protocol, rpcKind));
	}

	/**
	 * Returns whether the given method is supported or not. The protocol
	 * signatures are fetched and cached. The connection id for the proxy
	 * provided is re-used.
	 * 
	 * @param rpcProxy
	 *            Proxy which provides an existing connection id.
	 * @param protocol
	 *            Protocol for which the method check is required.
	 * @param rpcKind
	 *            The RpcKind for which the method check is required.
	 * @param version
	 *            The version at the client.
	 * @param methodName
	 *            Name of the method.
	 * @return true if the method is supported, false otherwise.
	 * @throws IOException
	 */
	public static boolean isMethodSupported(Object rpcProxy, Class<?> protocol, RPC.RpcKind rpcKind, long version,
			String methodName) throws IOException {
		InetSocketAddress serverAddress = RPC.getServerAddress(rpcProxy);
		Map<Long, ProtocolSignature> versionMap = getVersionSignatureMap(serverAddress, protocol.getName(),
				rpcKind.toString());
	...
	private static ProtocolMetaInfoPB getProtocolMetaInfoProxy(Object proxy, Configuration conf) throws IOException {
		...
		return RPC.getProtocolEngine(ProtocolMetaInfoPB.class, conf)
				.getProtocolMetaInfoProxy(inv.getConnectionId(), conf, NetUtils.getDefaultSocketFactory(conf))
				.getProxy();
	}

	/**
	 * Convert an RPC method to a string. The format we want is
	 * 'MethodOuterClassShortName#methodName'.
	 *
	 ...
	 *
	 * the format we want is: ClientNamenodeProtocol#getServerDefaults
	 */
