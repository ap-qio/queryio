...
public class MutableMetricsFactory {
	...
	MutableMetric newForField(Field field, Metric annotation, MetricsRegistry registry) {
		...
		if (cls == MutableStat.class) {
			return registry.newStat(info.name(), info.description(), annotation.sampleName(), annotation.valueName(),
					annotation.always());
		}
		throw new MetricsException(
				"Unsupported metric field " + field.getName() + " of type " + field.getType().getName());
	}
