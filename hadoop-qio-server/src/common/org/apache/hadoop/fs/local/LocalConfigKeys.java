...
/**
 * This class contains constants for configuration keys used in the local file
 * system, raw local fs and checksum fs.
 *
 * Note that the settings for unimplemented features are ignored. E.g. checksum
 * related settings are just place holders. Even when wrapped with
 * {@link ChecksumFileSystem}, these settings are not used.
 */
...
public class LocalConfigKeys extends CommonConfigurationKeys {
	...
	public static FsServerDefaults getServerDefaults() throws IOException {
		return new FsServerDefaults(BLOCK_SIZE_DEFAULT, BYTES_PER_CHECKSUM_DEFAULT, CLIENT_WRITE_PACKET_SIZE_DEFAULT,
				REPLICATION_DEFAULT, STREAM_BUFFER_SIZE_DEFAULT, ENCRYPT_DATA_TRANSFER_DEFAULT,
				FS_TRASH_INTERVAL_DEFAULT, CHECKSUM_TYPE_DEFAULT);
	}
}
