...
public abstract class FileSystem extends Configured implements Closeable {
	 ...
	 * 
	 * The default implementation simply calls {@link #canonicalizeUri(URI)} on
	 * the filesystem's own URI, so subclasses typically only need to implement
	 * that method.
	 *
	 ...
	 * canonicalizing the hostname using DNS and adding the default port if not
	 * specified.
	 * 
	 * The default implementation simply fills in the default port if not
	 * specified and if the filesystem has a default port.
	 *
	 * @return URI
	...
	/**
	 * Get a canonical service name for this file system. The token cache is the
	 * only user of the canonical service name, and uses it to lookup this
	 * filesystem's service tokens. If file system provides a token of its own
	 * then it must have a canonical name, otherwise canonical name can be null.
	 * 
	 * Default Impl: If the file system has child file systems (such as an
	 * embedded file system) then it is assumed that the fs has no tokens of its
	 * own and hence returns a null name; otherwise a service name is built
	 * using Uri and port.
	 * 
	...
	/**
	 * Get a new delegation token for this file system. This is an internal
	 * method that should have been declared protected but wasn't historically.
	...
	/**
	 * Obtain all delegation tokens used by this FileSystem that are not already
	 * present in the given Credentials. Existing tokens will neither be
	 * verified as valid nor having the given renewer. Missing tokens will be
	 * acquired and added to the given Credentials.
	 * 
	 * Default Impl: works for simple fs with its own token and also for an
	 * embedded fs whose tokens are those of its children file system (i.e. the
	 * embedded fs has not tokens of its own).
	 * 
	 * @param renewer
	...
	private void collectDelegationTokens(final String renewer, final Credentials credentials,
		...
		// Collect token of the this filesystem and then of its embedded
		// children
	...
	/**
	 * Get all the immediate child FileSystems embedded in this FileSystem. It
	 * does not recurse and get grand children. If a FileSystem has multiple
	 * child FileSystems, then it should return a unique list of those
	 * FileSystems. Default is to return null to signify no children.
	 * 
	 ...
	 *
	 * @param p
	 *            path is used to identify an FS since an FS could have another
	 *            FS that it could be delegating the call to
	 * @param start
	 *            offset into the given file
	 * @param len
	 *            length for which to get locations for
	...
	/**
	 * Create an FSDataOutputStream at the indicated Path. Files are overwritten
	 * by default.
	 * 
	 * @param f
	 *            the file to create
	 ...
	 *            file will be overwritten, and if false an exception will be
	 *            thrown.
	 */
	...
	/**
	 * Create an FSDataOutputStream at the indicated Path with a custom checksum
	 * option
	 * 
	 * @param f
	 *            the file name to open
	 ...
	 * @param progress
	 * @param checksumOpt
	 *            checksum parameter. If null, the values found in conf will be
	 *            used.
	 * @throws IOException
	 * @see #setPermission(Path, FsPermission)
	 */
	...
	public FSDataOutputStream createNonRecursive(Path f, boolean overwrite, int bufferSize, short replication,
		...
		return this.createNonRecursive(f, FsPermission.getFileDefault(), overwrite, bufferSize, replication, blockSize,
				progress);
	}
	...
	public ContentSummary getContentSummary(Path f) throws IOException {
		...
		for (FileStatus s : listStatus(f)) {
			...
			ContentSummary c = s.isDirectory() ? getContentSummary(s.getPath())
					: new ContentSummary.Builder().length(length).fileCount(1).directoryCount(0).spaceConsumed(length)
							.build();
	...
	/**
	 * Returns a remote iterator so that followup calls are made on demand while
	 * consuming the entries. Each file system implementation should override
	 * this method and provide a more efficient implementation, if possible.
	 *
	 * @param p
	 *            target path
	 ...
	 * 
	 * If the path is a directory, if recursive is false, returns files in the
	 * directory; if recursive is true, return files in the subtree rooted at
	 * the path. If the path is a file, return the file's status and block
	 * locations.
	 * 
	 * @param f
	 *            is the path
	...
	public RemoteIterator<LocatedFileStatus> listFiles(final Path f, final boolean recursive)
		...
		return new RemoteIterator<LocatedFileStatus>() {
			...
			/**
			 * Process the input stat. If it is a file, return the file stat. If
			 * it is a directory, traverse the directory if recursive is true;
	 ...
	 * no built in notion of an initial workingDir.
	 * 
	 ...
	 * The src file is under FS, and the dst is on the local disk. Copy it from
	 * FS control to the local dst name. delSrc indicates if the src will be
	 * removed or not. useRawLocalFileSystem indicates whether to use
	 * RawLocalFileSystem as local file system or not. RawLocalFileSystem is non
	 * crc file system.So, It will not create any crc files at local.
	 * 
	 * @param delSrc
	 *            whether to delete the src
	 * @param src
	...
	/**
	 * Return the number of bytes that large input files should be optimally be
	 * split into to minimize i/o time. The given path will be used to locate
	 * the actual filesystem. The full path does not have to exist.
	 * 
	 * @param f
	 *            path of file
	...
	/**
	 * Set the write checksum flag. This is only applicable if the corresponding
	 * FileSystem supports checksum. By default doesn't do anything.
	 * 
	 * @param writeChecksum
	 */
	...
	/**
	 * Get all of the xattr name/value pairs for a file or directory. Only those
	 * xattrs which the logged-in user has permissions to view are returned.
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 * @param path
	...
	/**
	 * Get all of the xattrs name/value pairs for a file or directory. Only
	 * those xattrs which the logged-in user has permissions to view are
	 * returned.
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	...
	static class Cache {
		...
		synchronized void closeAll(UserGroupInformation ugi) throws IOException {
			...
			// we cannot close inline since close() removes the entry from the
			// Map
	 ...
	 * statistics is done infrequently by most programs, and not at all by
	 * others. Hence, this is optimized for writes.
	 * 
	...
	public static final class Statistics {
		 ...
		 * Hence, volatile is adequate here-- we do not need AtomicLong or
		 * similar to prevent lost updates. The Java specification guarantees
		 * that updates to volatile longs will be perceived as atomic with
		 * respect to other threads, which is all we need.
		 */
		 ...
		 * to the totals. This is useful for making copies of Statistics
		 * objects, and for storing data that pertains to threads that have been
		 * garbage collected. Protected by the Statistics lock.
		 */
		...
		/**
		 * List of all thread-local data areas. Protected by the Statistics
		 * lock.
		 */
		...
		/**
		 * Get or create the thread-local data associated with the current
		 * thread.
		 */
		...
		/**
		 * Apply the given aggregator to all StatisticsData objects associated
			...
			if (allData != null) {
				for (Iterator<StatisticsData> iter = allData.iterator(); iter.hasNext();) {
					...
					if (data.owner.get() == null) {
						 ...
						 * longer exists, remove the StatisticsData from our
						 * list and fold the values into rootData.
						 */
						rootData.add(data);
						iter.remove();
					}
		...
		/**
		 * Get the number of large file system read operations such as list
		 * files under a large directory
		 * 
		 ...
		 *
		 * In order to reset, we add up all the thread-local statistics data,
		 * and set rootData to the negative of that.
		 *
		 * This may seem like a counterintuitive way to reset the statsitics.
		 * Why can't we just zero out all the thread-local data? Well,
		 * thread-local data can only be modified by the thread that owns it. If
		 * we tried to modify the thread-local data from this thread, our
		 * modification might get interleaved with a read-modify-write operation
		 * done by the thread that owns the data. That would result in our
		 * update getting lost.
		 *
