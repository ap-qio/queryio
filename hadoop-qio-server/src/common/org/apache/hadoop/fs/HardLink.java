...
/**
 * Class for creating hardlinks. Supports Unix/Linux, Windows via winutils , and
 * Mac OS X.
 * 
 * The HardLink class was formerly a static inner class of FSUtil, and the
 * methods provided were blatantly non-thread-safe. To enable volume-parallel
 * Update snapshots, we now provide static threadsafe methods that allocate new
 * buffer string arrays upon each call. We also provide an API to hardlink all
 * files in a directory with a single command, which is up to 128 times more
...
public class HardLink {
	 ...
	 * needed functionality for querying link counts. The particular
	 * implementation class is chosen during static initialization phase of the
	 * HardLink class. The "getter" methods construct shell command strings.
	 */
	...
	private static class HardLinkCGUnix extends HardLinkCommandGetter {
		...
		/*
		 * @see
		 * org.apache.hadoop.fs.HardLink.HardLinkCommandGetter#linkCount(java.io
		 * .File)
		 */
		@Override
	...
	/*
	 * **************************************************** Complexity is above.
	 * User-visible functionality is below
	 * ****************************************************
	 */
	...
	private static IOException createIOException(File f, String message, String error, int exitvalue, Exception cause) {
		...
		final String s = "Failed to get link count on file " + f + ": message=" + message + "; error=" + error
				+ "; exit value=" + exitvalue;
		return (cause == null) ? new IOException(s) : new IOException(s, cause);
	}

	/**
	 * HardLink statistics counters and methods. Not multi-thread safe,
	 * obviously. Init is called during HardLink instantiation, above.
	 * 
	...
	public static class LinkStats {
		...
		public String report() {
			return "HardLinkStats: " + countDirs + " Directories, including " + countEmptyDirs + " Empty Directories, "
					+ countSingleLinks + " single Link operations, " + countMultLinks
