 ...
 * separate class since FsShell is getting too large.
 */
...
public class FsShellPermissions extends FsCommand {
	 ...
	 * also enforce octal mode specifications of either 3 digits without a
	 * sticky bit setting or four digits with a sticky bit setting.
	 */
	public static class Chmod extends FsShellPermissions {
		...
		public static final String DESCRIPTION = "Changes permissions of a file. "
		...
		protected void processOptions(LinkedList<String> args) throws IOException {
			...
			try {
				...
				// TODO: remove "chmod : " so it's not doubled up in output, but
				// it's
	...
	// used by chown/chgrp
	static private String allowedChars = Shell.WINDOWS ? "[-_./@a-zA-Z0-9 ]" : "[-_./@a-zA-Z0-9]";
	...
	public static class Chown extends FsShellPermissions {
		...
		public static final String DESCRIPTION = "Changes owner and group of a file. "
				...
				+ "The owner and group names may only consist of digits, alphabet, " + "and any of " + allowedChars
		...
		static private final Pattern chownPattern = Pattern
		...
		protected void processPath(PathData item) throws IOException {
			...
			String newOwner = (owner == null || owner.equals(item.stat.getOwner())) ? null : owner;
			String newGroup = (group == null || group.equals(item.stat.getGroup())) ? null : group;
	...
	public static class Chgrp extends Chown {
		...
		static private final Pattern chgrpPattern = Pattern.compile("^\\s*(" + allowedChars + "+)\\s*$");
