...
class Globber {
	...
	/**
	 * Translate an absolute path into a list of path components. We merge
	 * double slashes into a single slash here. POSIX root path, i.e. '/', does
	 * not get an entry in the list.
	 */
	...
	public FileStatus[] glob() throws IOException {
		...
		// Now loop over all flattened patterns. In every case, we'll be trying
		// to
		...
		for (String flatPattern : flattenedPatterns) {
			...
			// Now we break the flattened, absolute pattern into path
			// components.
			...
			// To get the "real" FileStatus of root, we'd have to do an
			// expensive
			...
			if (Path.WINDOWS && !components.isEmpty()
				...
				rootPlaceholder = new FileStatus(0, true, 0, 0, 0,
						new Path(scheme, authority, Path.SEPARATOR + driveLetter + Path.SEPARATOR));
			} else {
			...
			for (int componentIdx = 0; componentIdx < components.size(); componentIdx++) {
				...
				if ((componentIdx < components.size() - 1) && (!globFilter.hasPattern())) {
					// Optimization: if this is not the terminal path component,
					// and we
					// are not matching against a glob, assume that it exists.
					// If it
					// doesn't exist, we'll find out later when resolving a
					// later glob
				...
				for (FileStatus candidate : candidates) {
					if (globFilter.hasPattern()) {
						...
						if (children.length == 1) {
							// If we get back only one result, this could be
							// either a listing
							// of a directory with one entry, or it could
							// reflect the fact
							// that what we listed resolved to a file.
							//
							// Unfortunately, we can't just compare the returned
							// paths to
							// figure this out. Consider the case where you have
							// /a/b, where
							// b is a symlink to "..". In that case, listing
							// /a/b will give
							// back "/a/b" again. If we just went by returned
							// pathname, we'd
							// incorrectly conclude that /a/b was a file and
							// should not match
							// /a/*/*. So we use getFileStatus of the path we
							// just listed to
							// disambiguate.
						...
						// When dealing with non-glob components, use
						// getFileStatus
						// instead of listStatus. This is an optimization, but
						// it also
						// is necessary for correctness in HDFS, since there are
						// some
						// special HDFS directories like .reserved and .snapshot
						// that are
			...
			for (FileStatus status : candidates) {
				// Use object equality to see if this status is the root
				// placeholder.
				// See the explanation for rootPlaceholder above for more
				// information.
				...
				// HADOOP-3497 semantics: the user-defined filter is applied at
				// the
		 ...
		 * can't find it, we return null rather than an empty array. This is a
		 * special case which the shell relies on.
		 *
