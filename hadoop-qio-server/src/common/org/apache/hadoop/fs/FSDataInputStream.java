...
public class FSDataInputStream extends DataInputStream implements Seekable, PositionedReadable, ByteBufferReadable,
		HasFileDescriptor, CanSetDropBehind, CanSetReadahead, HasEnhancedByteBufferAccess, CanUnbuffer {
	/**
	 * Map ByteBuffers that we have handed out to readers to ByteBufferPool
	 * objects
	 */
	private final IdentityHashStore<ByteBuffer, ByteBufferPool> extendedReadBuffers = new IdentityHashStore<ByteBuffer, ByteBufferPool>(
			0);
	 ...
	 * @return total number of bytes read into the buffer, or <code>-1</code> if
	 ...
	 * @throws EOFException
	 *             If the end of stream is reached while reading. If an
	 *             exception is thrown an undetermined number of bytes in the
	...
	public void unbuffer() {
		try {
			...
			throw new UnsupportedOperationException("this stream does not " + "support unbuffering.");
		}
	}
}
