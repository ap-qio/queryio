...
/**
 * Used primarily by {@link FileContext} to operate on and resolve symlinks in a
 * path. Operations can potentially span multiple {@link AbstractFileSystem}s.
 * 
...
public abstract class FSLinkResolver<T> {
	...
	/**
	 * Return a fully-qualified version of the given symlink target if it has no
	 * scheme and authority. Partially and fully-qualified paths are returned
	 * unmodified.
	 * 
	 * @param pathURI
	 *            URI of the filesystem of pathWithLink
	 * @param pathWithLink
	 *            Path that contains the symlink
	 * @param target
	 *            The symlink's absolute target
	 * @return Fully qualified version of the target.
	 */
	public static Path qualifySymlinkTarget(final URI pathURI, Path pathWithLink, Path target) {
	...
	/**
	 * Generic helper function overridden on instantiation to perform a specific
	 * operation on the given file system using the given path which may result
	 * in an UnresolvedLinkException.
	 * 
	 * @param fs
	 *            AbstractFileSystem to perform the operation on.
	 * @param p
			...
			try {
				...
				if (!fc.resolveSymlinks) {
					throw new IOException(
							"Path " + path + " contains a symlink" + " and symlink resolution is disabled ("
