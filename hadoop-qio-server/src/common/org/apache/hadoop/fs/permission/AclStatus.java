...
public class AclStatus {
	...
	public boolean equals(Object o) {
		...
		return Objects.equal(owner, other.owner) && Objects.equal(group, other.group) && stickyBit == other.stickyBit
	...
	public String toString() {
		return new StringBuilder().append("owner: ").append(owner).append(", group: ").append(group).append(", acl: {")
				.append("entries: ").append(entries).append(", stickyBit: ").append(stickyBit).append('}').toString();
	}
	...
	public static class Builder {
		...
		/**
		 * Sets sticky bit. If this method is not called, then the builder
		 * assumes false.
		 *
		 * @param stickyBit
		 *            boolean sticky bit
	 ...
	 *
	 * @param file
	 *            Path file associated to this ACL
	 * @param owner
	 *            String file owner
	 * @param group
	 *            String file group
	 ...
	 * 
	 * @param entry
	 *            AclEntry to get the effective action
	 */
	 ...
	 *            communicating with old namenode, then this argument will not
	 *            have any preference.
	 * @return Returns the effective permission for the entry.
	 * @throws IllegalArgumentException
	 *             If the client communicating with old namenode and permission
	...
	public FsAction getEffectivePermission(AclEntry entry, FsPermission permArg) throws IllegalArgumentException {
		...
		if ((entry.getName() != null || entry.getType() == AclEntryType.GROUP)) {
			if (entry.getScope() == AclEntryScope.ACCESS) {
				...
				// default mask entry for effective permission calculation will
				// be the
				// penultimate entry. This can be mask entry in case of extended
				// ACLs.
				// In case of minimal ACL, this is the owner group entry, and we
				// end up
