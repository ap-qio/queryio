...
public final class ScopedAclEntries {
	 ...
	 * the list is already sorted such that all access entries precede all
	 * default entries.
	 *
	 * @param aclEntries
	 *            List<AclEntry> to separate
	 */
	public ScopedAclEntries(List<AclEntry> aclEntries) {
		...
		if (pivot != PIVOT_NOT_FOUND) {
			accessEntries = pivot != 0 ? aclEntries.subList(0, pivot) : Collections.<AclEntry>emptyList();
	 ...
	 *
	 * @return List<AclEntry> containing just access entries, or an empty list
	 ...
	 *         if there are no default entries
	 */
