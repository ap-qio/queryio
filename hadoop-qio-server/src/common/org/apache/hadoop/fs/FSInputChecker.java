...
abstract public class FSInputChecker extends FSInputStream {
	...
	/**
	 * Reads in checksum chunks into <code>buf</code> at <code>offset</code> and
	 * checksum into <code>checksum</code>. Since checksums can be disabled,
	 * there are two cases implementors need to worry about:
	 *
	 * (a) needChecksum() will return false: - len can be any positive value -
	 * checksum will be null Implementors should simply pass through to the
	 * underlying data stream. or (b) needChecksum() will return true: - len >=
	 * maxChunkSize - checksum.length is a multiple of CHECKSUM_SIZE
	 * Implementors should read an integer number of data chunks into buf. The
	 * amount read should be bounded by len or by checksum.length /
	 * CHECKSUM_SIZE * maxChunkSize. Note that len may be a value that is not a
	 * multiple of maxChunkSize, in which case the implementation may return
	 * less than len.
	 *
	...
	/*
	 * Read up one or more checksum chunk to array <i>b</i> at pos <i>off</i> It
	 * requires at least one checksum chunk boundary in between <cur_pos,
	 * cur_pos+len> and it stops reading at the last boundary or at the end of
	 * the stream; Otherwise an IllegalArgumentException is thrown. This makes
	 * sure that all data read are checksum verified.
	 * 
	...
	/**
	 * Convert a checksum byte array to a long This is deprecated since 0.22
	 * since it is no longer in use by this class.
	 */
	@Deprecated
