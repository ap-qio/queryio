...
/**
 * Wrapper for the Unix stat(1) command. Used to workaround the lack of lstat(2)
 * in Java 6.
 */
...
public class Stat extends Shell {
	...
	public Stat(Path path, long blockSize, boolean deref, FileSystem fs) throws IOException {
		super(0L, true);
		// Original path
		this.original = path;
	...
	protected void parseExecResult(BufferedReader lines) throws IOException {
		...
		// 6,symbolic
		// link,6,1373584236,1373584236,lrwxrwxrwx,andrew,andrew,`link' ->
		// `target'
		// OR
		// 6,symbolic
		// link,6,1373584236,1373584236,lrwxrwxrwx,andrew,andrew,'link' ->
		...
		try {
			...
			// Set stat
			stat = new FileStatus(length, isDir, 1, blockSize, modTime, accessTime, perms, owner, group, symlink,
					qualified);
