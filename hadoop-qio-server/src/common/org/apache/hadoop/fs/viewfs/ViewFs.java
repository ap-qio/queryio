 ...
 * customized view of a file system namespace that is composed from one or more
 * individual file systems (a localFs or Hdfs, S3fs, etc). For example one could
 * have a mount table that provides links such as
 * <ul>
 ...
 * <p>
 * To use viewfs one would typically set the default file system in the config
 * (i.e. fs.default.name< = viewfs:///) along with the mount table config
 ...
 * <p>
 * 
 * The file system is initialized from the standard Hadoop config through config
 * variables. See {@link FsConstants} for URI and Scheme constants; See
 * {@link Constants} for config var constants; see {@link ConfigUtil} for
 * convenient lib.
 * 
 * <p>
 * All the mount table config entries for view fs are prefixed by
 * <b>fs.viewfs.mounttable.</b> For example the above example can be specified
 * with the following config variables:
 * <ul>
 * <li>fs.viewfs.mounttable.default.link./user= hdfs://nnContainingUserDir/user
 ...
 * <li>fs.viewfs.mounttable.default.link./tmp= hdfs://nnTmp/privateTmpForUserXXX
 * </ul>
 * 
 * The default mount table (when no authority is specified) is from config
 * variables prefixed by <b>fs.viewFs.mounttable.default </b> The authority
 * component of a URI can be used to specify a different mount table. For
 * example,
 * <ul>
 ...
 * sometimes called union-mounts or junction-mounts in the literature. For
 * example of the home directories are stored on say two file systems (because
 * they do not fit on one) then one could specify a mount entry such as
 * following merges two dirs:
 * <ul>
 * <li>/user -> hdfs://nnUser1/user,hdfs://nnUser2/user
 * </ul>
 * Such a mergeLink can be specified with the following config var where "," is
 * used as the separator for each of links to be merged:
 * <ul>
 * <li>fs.viewfs.mounttable.default.linkMerge./user=
 * hdfs://nnUser1/user,hdfs://nnUser1/user
 * </ul>
 * A special case of the merge mount is where mount table's root is merged with
 * the root (slash) of another file system:
 * <ul>
...
public class ViewFs extends AbstractFileSystem {
	...
	static AccessControlException readOnlyMountTable(final String operation, final String p) {
		return new AccessControlException(
				"InternalDir of ViewFileSystem is readonly; operation=" + operation + "Path=" + p);
	}
	...
	static public class MountPoint {
		...
		private URI[] targets; // target of the mount; Multiple targets imply
								// mergeMount
	...
	ViewFs(final URI theUri, final Configuration conf) throws IOException, URISyntaxException {
		...
		config = conf;
		// Now build client side view (i.e. client side mount table) from
		// config.
		...
		fsState = new InodeTree<AbstractFileSystem>(conf, authority) {
			...
			@Override
			protected AbstractFileSystem getTargetFileSystem(final INodeDir<AbstractFileSystem> dir)
					throws URISyntaxException {
	...
	public FSDataOutputStream createInternal(final Path f, final EnumSet<CreateFlag> flag,
		...
		return res.targetFileSystem.createInternal(res.remainingPath, flag, absolutePermission, bufferSize, replication,
	...
	public FileStatus getFileStatus(final Path f)
		...
		// FileStatus#getPath to be unchanged so that it can get owner when
		// needed.
	...
	public void access(Path path, FsAction mode)
			throws AccessControlException, FileNotFoundException, UnresolvedLinkException, IOException {
		InodeTree.ResolveResult<AbstractFileSystem> res = fsState.resolve(getUriPath(path), true);
	...
	public FileStatus getFileLinkStatus(final Path f)
		...
		InodeTree.ResolveResult<AbstractFileSystem> res = fsState.resolve(getUriPath(f), false); // do
																									// not
																									// follow
																									// mount
																									// link
	...
	public RemoteIterator<FileStatus> listStatusIterator(final Path f)
		...
		return new RemoteIterator<FileStatus>() {
			...
			public FileStatus next() throws IOException {
				...
				return new ViewFsFileStatus(status,
						makeQualified(suffix.length() == 0 ? f : new Path(res.resolvedPath, suffix)));
			}
		};
	}
	...
	public FileStatus[] listStatus(final Path f)
		...
		if (!res.isInternalDir()) {
			...
			for (FileStatus status : statusLst) {
				String suffix = targetFs.stripOutRoot(status.getPath());
				statusLst[i++] = new ViewFsFileStatus(status,
	...
	public void mkdir(final Path dir, final FsPermission permission, final boolean createParent)
			...
			IOException {
		InodeTree.ResolveResult<AbstractFileSystem> res = fsState.resolve(getUriPath(dir), false);
	...
	@Override
	public void renameInternal(final Path src, final Path dst, final boolean overwrite)
		...
		/**
		 * // Alternate 1: renames within same file system - valid but we
		 * disallow // Alternate 2: (as described in next para - valid but we
		 * have disallowed it // // Note we compare the URIs. the URIs include
		 * the link targets. // hence we allow renames across mount links as
		 * long as the mount links // point to the same target. if
		 * (!resSrc.targetFileSystem.getUri().equals(
		 * resDst.targetFileSystem.getUri())) { throw new IOException("Renames
		 * across Mount points not supported"); }
		 */
	...
	/*
	 * An instance of this class represents an internal dir of the viewFs ie
	 * internal dir of the mount table. It is a ready only mount tbale and
	 * create, mkdir or delete operations are not allowed. If called on create
	 * or mkdir then this target is the parent of the directory in which one is
	 * trying to create or mkdir; hence in this case the path name passed in is
	 * the last component. Otherwise this target is the end point of the path
	 * and hence the path name passed in is null.
	 */
	static class InternalDirOfViewFs extends AbstractFileSystem {
		...
		public InternalDirOfViewFs(final InodeTree.INodeDir<AbstractFileSystem> dir, final long cTime,
				final UserGroupInformation ugi, final URI uri) throws URISyntaxException {
		...
		@Override
		public FSDataOutputStream createInternal(final Path f, final EnumSet<CreateFlag> flag,
				final FsPermission absolutePermission, final int bufferSize, final short replication,
		...
		public FileStatus getFileStatus(final Path f) throws IOException {
			checkPathIsSlash(f);
			return new FileStatus(0, true, 0, 0, creationTime, creationTime, PERMISSION_555, ugi.getUserName(),
					ugi.getGroupNames()[0], new Path(theInternalDir.fullPath).makeQualified(myUri, null));
		}

		@Override
		public FileStatus getFileLinkStatus(final Path f) throws FileNotFoundException {
			// look up i internalDirs children - ignore first Slash
			INode<AbstractFileSystem> inode = theInternalDir.children.get(f.toUri().toString().substring(1));
			if (inode == null) {
				throw new FileNotFoundException("viewFs internal mount table - missing entry:" + f);
			}
			FileStatus result;
			if (inode instanceof INodeLink) {
				INodeLink<AbstractFileSystem> inodelink = (INodeLink<AbstractFileSystem>) inode;
				result = new FileStatus(0, false, 0, 0, creationTime, creationTime, PERMISSION_555, ugi.getUserName(),
						ugi.getGroupNames()[0], inodelink.getTargetLink(),
						new Path(inode.fullPath).makeQualified(myUri, null));
			} else {
				result = new FileStatus(0, true, 0, 0, creationTime, creationTime, PERMISSION_555, ugi.getUserName(),
						ugi.getGroupNames()[0], new Path(inode.fullPath).makeQualified(myUri, null));
			}
			return result;
		}
		...
		public FileStatus[] listStatus(final Path f) throws AccessControlException, IOException {
			...
			for (Entry<String, INode<AbstractFileSystem>> iEntry : theInternalDir.children.entrySet()) {
				...
				if (inode instanceof INodeLink) {
					...
					result[i++] = new FileStatus(0, false, 0, 0, creationTime, creationTime, PERMISSION_555,
							ugi.getUserName(), ugi.getGroupNames()[0], link.getTargetLink(),
							new Path(inode.fullPath).makeQualified(myUri, null));
				} else {
					result[i++] = new FileStatus(0, true, 0, 0, creationTime, creationTime, PERMISSION_555,
		...
		public AclStatus getAclStatus(Path path) throws IOException {
			checkPathIsSlash(path);
			return new AclStatus.Builder().owner(ugi.getUserName()).group(ugi.getGroupNames()[0])
					.addEntries(AclUtil.getMinimalAcl(PERMISSION_555)).stickyBit(false).build();
		}
