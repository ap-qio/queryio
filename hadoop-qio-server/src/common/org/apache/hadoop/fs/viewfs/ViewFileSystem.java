...
public class ViewFileSystem extends FileSystem {
	...
	/**
	 * Make the path Absolute and get the path-part of a pathname. Checks that
	 * URI matches this file system and that the path-part is a valid name.
	 * 
	 * @param p
	 *            path
	...
	public FSDataOutputStream createNonRecursive(Path f, FsPermission permission, EnumSet<CreateFlag> flags,
		...
		return res.targetFileSystem.createNonRecursive(res.remainingPath, permission, flags, bufferSize, replication,
				blockSize, progress);
	}
	...
	public FSDataOutputStream create(final Path f, final FsPermission permission, final boolean overwrite,
		...
		assert (res.remainingPath != null);
		return res.targetFileSystem.create(res.remainingPath, permission, overwrite, bufferSize, replication, blockSize,
				progress);
	}
	...
	/*
	 * An instance of this class represents an internal dir of the viewFs that
	 * is internal dir of the mount table. It is a read only mount tables and
	...
	static class InternalDirOfViewFs extends FileSystem {
		...
		public InternalDirOfViewFs(final InodeTree.INodeDir<FileSystem> dir, final long cTime,
				final UserGroupInformation ugi, URI uri) throws URISyntaxException {
			myUri = uri;
			try {
		...
		@Override
		public BlockLocation[] getFileBlockLocations(final FileStatus fs, final long start, final long len)
		...
		public FileStatus[] listStatus(Path f) throws AccessControlException, FileNotFoundException, IOException {
			...
			int i = 0;
			for (Entry<String, INode<FileSystem>> iEntry : theInternalDir.children.entrySet()) {
