...
class ChRootedFileSystem extends FilterFileSystem {
	...
	protected Path fullPath(final Path path) {
		...
		return path.isAbsolute()
				? new Path((chRootPathPart.isRoot() ? "" : chRootPathPartString) + path.toUri().getPath())
	 ...
	 * @throws IOException
	 */
	public ChRootedFileSystem(final URI uri, Configuration conf) throws IOException {
	 ...
	 * @throws IOException
	 *             if the p is not prefixed with root
	 */
	...
	protected Path getInitialWorkingDirectory() {
		/*
		 * 3 choices here: null or / or /user/<uname> or strip out the root out
		 * of myFs's inital wd. Only reasonable choice for initialWd for
		 * chrooted fds is null so that the default rule for wd is applied
		 */
		return null;
	}
	...
	public FSDataOutputStream create(final Path f, final FsPermission permission, final boolean overwrite,
			...
			throws IOException {
		return super.create(fullPath(f), permission, overwrite, bufferSize, replication, blockSize, progress);
	}
	...
	public FSDataOutputStream createNonRecursive(Path f, FsPermission permission, EnumSet<CreateFlag> flags,
		...
		return super.createNonRecursive(fullPath(f), permission, flags, bufferSize, replication, blockSize, progress);
	}
	...
	public BlockLocation[] getFileBlockLocations(final FileStatus fs, final long start, final long len)
			throws IOException {
		return super.getFileBlockLocations(new ViewFsFileStatus(fs, fullPath(fs.getPath())), start, len);
	}
