...
/**
 * InodeTree implements a mount-table as a tree of inodes. It is used to
 * implement ViewFs and ViewFileSystem. In order to use it the caller must
 * subclass it and implement the abstract methods
 * {@link #getTargetFileSystem(INodeDir)}, etc.
 * 
 * The mountable is initialized from the config variables as specified in
 * {@link ViewFs}
 *
 * @param <T>
 *            is AbstractFileSystem or FileSystem
 * 
 *            The three main methods are {@link #InodeTreel(Configuration)} //
 *            constructor {@link #InodeTree(Configuration, String)} //
 *            constructor {@link #resolve(String, boolean)}
 */
...
abstract class InodeTree<T> {
	...
	static class INodeDir<T> extends INode<T> {
		...
		INode<T> resolveInternal(final String pathComponent) throws FileNotFoundException {
			return children.get(pathComponent);
		}

		INodeDir<T> addDir(final String pathComponent, final UserGroupInformation aUgi)
	...
	/**
	 * In internal class to represent a mount link A mount link can be single
	 * dir link or a merge dir link.
	 * 
	 * A merge dir link is a merge (junction) of links to dirs: example : <merge
	 * of 2 dirs /users -> hdfs:nn1//users /users -> hdfs:nn2//users
	 * 
	...
	static class INodeLink<T> extends INode<T> {
		 ...
		 */
		INodeLink(final String pathToNode, final UserGroupInformation aUgi, final T targetMergeFs,
				final URI[] aTargetDirLinkList) {
			super(pathToNode, aUgi);
		 ...
		 */
		INodeLink(final String pathToNode, final UserGroupInformation aUgi, final T targetFs,
		...
		/**
		 * Get the target of the link If a merge link then it returned as ","
		 * separated URI list.
		 */
		Path getTargetLink() {
	...
	private void createLink(final String src, final String target, final boolean isLinkMerge,
			final UserGroupInformation aUgi)
			throws URISyntaxException, IOException, FileAlreadyExistsException, UnsupportedFileSystemException {
	...
	/**
	 * The user of this class must subclass and implement the following 3
	 * abstract methods.
	 * 
	 * @throws IOException
	 */
	...
	protected abstract T getTargetFileSystem(final INodeDir<T> dir) throws URISyntaxException;
	...
	protected InodeTree(final Configuration config, final String viewName)
			throws UnsupportedFileSystemException, URISyntaxException, FileAlreadyExistsException, IOException {
		...
		if (!gotMountTableEntry) {
			throw new IOException("ViewFs: Cannot initialize: Empty Mount table in config for " + vName == null
					? "viewfs:///" : ("viewfs://" + vName + "/"));
		}
	}

	/**
	 * Resolve returns ResolveResult. The caller can continue the resolution of
	 * the remainingPath in the targetFileSystem.
	 * 
	 * If the input pathname leads to link to another file system then the
	 * targetFileSystem is the one denoted by the link (except it is file system
	 * chrooted to link target. If the input pathname leads to an internal
	 * mount-table entry then the target file system is one that represents the
	 * internal inode.
	 */
	...
	ResolveResult<T> resolve(final String p, final boolean resolveLastComponent) throws FileNotFoundException {
		...
		if (path.length <= 1) { // special case for when path is "/"
			ResolveResult<T> res = new ResolveResult<T>(ResultKind.isInternalDir, root.InodeDirFs, root.fullPath,
					SlashPath);
			return res;
		}
		...
		for (i = 1; i < path.length - (resolveLastComponent ? 0 : 1); i++) {
			...
			if (nextInode instanceof INodeLink) {
				...
				final ResolveResult<T> res = new ResolveResult<T>(ResultKind.isExternalDir, link.targetFileSystem,
		...
		if (resolveLastComponent) {
			...
			// that follows will do a children.get(remaningPath) and will have
			// to
		...
		final ResolveResult<T> res = new ResolveResult<T>(ResultKind.isInternalDir, curInode.InodeDirFs,
				curInode.fullPath, remainingPath);
		return res;
	}
	...
	/**
	 * 
	 * @return home dir value from mount table; null if no config value was
	 *         found.
	 */
