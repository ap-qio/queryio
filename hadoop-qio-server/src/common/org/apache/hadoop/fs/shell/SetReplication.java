...
class SetReplication extends FsCommand {
	...
	public static final String DESCRIPTION = "Set the replication level of a file. If <path> is a directory "
	...
	private void waitForReplication() throws IOException {
		for (PathData item : waitList) {
			...
			while (!done) {
				item.refreshStatus();
				BlockLocation[] locations = item.fs.getFileBlockLocations(item.stat, 0, item.stat.getLen());
