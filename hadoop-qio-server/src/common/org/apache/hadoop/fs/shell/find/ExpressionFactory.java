...
final class ExpressionFactory {
	 ...
	 * given class. This method abstracts the contract between the factory and
	 * the expression class. Do not assume that directly invoking
	 * registerExpression on the given class will have the same effect.
	 *
	...
	Expression createExpression(String expressionClassname, Configuration conf) {
		try {
			Class<? extends Expression> expressionClass = Class.forName(expressionClassname)
					.asSubclass(Expression.class);
