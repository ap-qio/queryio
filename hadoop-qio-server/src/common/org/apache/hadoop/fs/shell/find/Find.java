...
public class Find extends FsCommand {
	 ...
	 * 
	 * @param factory
	 *            the command factory that will instantiate this class
	 */
	...
	private static String[] HELP = { "Finds all files that match the specified expression and",
	...
	protected void processOptions(LinkedList<String> args) throws IOException {
		...
		// search for first non-path argument (ie starts with a "-") and capture
		// and
	...
	protected void recursePath(PathData item) throws IOException {
		...
		if (item.stat.isSymlink() && getOptions().isFollowLink()) {
			...
			if (isAncestor(item, linkedItem)) {
				getOptions().getErr()
						.println("Infinite loop ignored: " + item.toString() + " -> " + linkedItem.toString());
				return;
			}
