 ...
 * PathData ensures that the returned path string will be the same as the one
 * passed in during initialization (unlike Path objects which can modify the
 * path string). The stat field will be null if the path does not exist.
 */
...
public class PathData implements Comparable<PathData> {
	...
	/**
	 * Creates an object to wrap the given parameters as fields. The string used
	 * to create the path will be recorded since the Path object does not return
	 * exactly the same string used to initialize it
	 * 
	 * @param pathString
	 *            a string for a path
	 * @param conf
	...
	/**
	 * Looks up the file status for a path. If the path doesn't exist, then the
	 * status will be null
	 * 
	 * @param fs
	 ...
	 *             if file doesn't exist or the type does not match what was
	 *             specified in typeRequirement.
	 */
	...
	protected enum PathType {
		HAS_SCHEME, SCHEMELESS_ABSOLUTE, RELATIVE
	};

	/**
	 * Expand the given path as a glob pattern. Non-existent paths do not throw
	 * an exception because creation commands like touch and mkdir need to
	 * create them. The "stat" field will be null if the path does not exist.
	 * 
	 * @param pattern
	 ...
	 *         and does not exist, the list will contain a single PathData with
	 *         a null stat
	 * @throws IOException
	...
	/**
	 * Returns the printable version of the path that is either the path as
	 * given on the commandline, or the full path
	 * 
	...
	private static String uriToString(URI uri, boolean inferredSchemeFromPath) {
		...
		if ((scheme == null) || (inferredSchemeFromPath)) {
			if (Path.isWindowsAbsolutePath(decodedRemainder, true)) {
				// Strip the leading '/' added in stringToUri so users see a
				// valid
				// Windows path.
	...
	/**
	 * Normalize the given Windows path string. This does the following: 1. Adds
	 * "file:" scheme for absolute paths. 2. Ensures the scheme-specific part
	 * starts with '/' per RFC2396. 3. Replaces backslash path separators with
	 * forward slashes.
	 * 
	 * @param pathString
	 *            Path string supplied by the user.
	 * @return normalized absolute path string. Returns the input string if it
	 *         is not a Windows absolute path.
	 */
	...
	/**
	 * Construct a URI from a String with unescaped special characters that have
	 * non-standard semantics. e.g. /, ?, #. A custom parsing is needed to
	 * prevent misbehavior.
	 * 
	 * @param pathString
	...
	public boolean equals(Object o) {
		return (o != null) && (o instanceof PathData) && path.equals(((PathData) o).path);
	}
