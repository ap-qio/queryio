...
/**
 * Base class for commands related to viewing filesystem usage, such as du and
 * df
 */
...
class FsUsage extends FsCommand {
	...
	public static class Df extends FsUsage {
		...
		public static final String DESCRIPTION = "Shows the capacity, free and used space of the filesystem. "
				+ "If the filesystem has multiple partitions, and no path to a "
				+ "particular partition is specified, then the status of the root " + "partitions will be shown.\n"
		...
		protected void processOptions(LinkedList<String> args) throws IOException {
			...
			if (args.isEmpty())
				args.add(Path.SEPARATOR);
		}

		@Override
		protected void processArguments(LinkedList<PathData> args) throws IOException {
			usagesTable = new TableBuilder("Filesystem", "Size", "Used", "Available", "Use%");
		...
		protected void processPath(PathData item) throws IOException {
			...
			usagesTable.addRow(item.fs.getUri(), formatSize(size), formatSize(used), formatSize(free),
	...
	public static class Du extends FsUsage {
		...
		public static final String DESCRIPTION = "Show the amount of space, in bytes, used by the files that "
		...
		public void printToStream(PrintStream out) {
			...
			for (int i = 0; i < widths.length; i++) {
				...
				if (rightAlign[i]) {
					...
					// prevent trailing spaces if the final column is
					// left-aligned
					fmt.append("%s");
				}
			}
