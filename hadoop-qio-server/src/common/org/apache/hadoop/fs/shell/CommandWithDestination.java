...
/**
 * Provides: argument processing to ensure the destination is valid for the
 * number of source arguments. A processPaths that accepts both a source and
 * resolved target. Sources are resolved as children of a destination directory.
 */
abstract class CommandWithDestination extends FsCommand {
	...
	/**
	 * If true, the last modified time, last access time, owner, group and
	 * permission information of the source file will be preserved as far as
	 * target {@link FileSystem} implementation allows.
	 */
	 ...
	 *             if path doesn't exist or matches too many times
	 */
	...
	protected void recursePath(PathData src) throws IOException {
		...
		try {
			...
			if (dst.exists) {
				...
				dst.refreshStatus(); // need to update stat to know it exists
										// now
			}
	 ...
	 * /.reserved/raw or neither in /.reserved/raw. If neither src nor target
	 * are in /.reserved/raw, then return false, indicating not to preserve
	 * raw.* xattrs. If both src/target are in /.reserved/raw, then return true,
	 ...
	 *             is only one of src/target are in /.reserved/raw.
	 */
	private boolean checkPathsForReservedRaw(Path src, Path target) throws PathOperationException {
		final boolean srcIsRR = Path.getPathWithoutSchemeAndAuthority(src).toString().startsWith(RESERVED_RAW);
		final boolean dstIsRR = Path.getPathWithoutSchemeAndAuthority(target).toString().startsWith(RESERVED_RAW);
		boolean preserveRawXattrs = false;
		if (srcIsRR && !dstIsRR) {
			final String s = "' copy from '" + RESERVED_RAW + "' to non '" + RESERVED_RAW
					+ "'. Either both source and target must be in '" + RESERVED_RAW + "' or neither.";
			throw new PathOperationException("'" + src.toString() + s);
		} else if (!srcIsRR && dstIsRR) {
			final String s = "' copy from non '" + RESERVED_RAW + "' to '" + RESERVED_RAW
					+ "'. Either both source and target must be in '" + RESERVED_RAW + "' or neither.";
	 ...
	 * successful, the temporary file will be renamed to the real path, else the
	 * temporary file will be deleted.
	 * 
	 * @param in
	 *            the input stream for the copy
	 * @param target
	 *            where to store the contents of the stream
	 * @throws IOException
	 *             if copy fails
	 */
	protected void copyStreamToTarget(InputStream in, PathData target) throws IOException {
		...
		try {
			...
			targetFs.close(); // last ditch effort to ensure temp file is
								// removed
		}
	}

	/**
	 * Preserve the attributes of the source to the target. The method calls
	 * {@link #shouldPreserve(FileAttribute)} to check what attribute to
	 * preserve.
	 * 
	 * @param src
	 *            source to preserve
	 * @param target
	 *            where to preserve attributes
	 * @param preserveRawXAttrs
	 *            true if raw.* xattrs should be preserved
	 * @throws IOException
	 *             if fails to preserve attributes
	 */
	...
	private static class TargetFileSystem extends FilterFileSystem {
		...
		FSDataOutputStream create(PathData item, boolean lazyPersist) throws IOException {
			try {
				if (lazyPersist) {
					...
					return create(item.path, FsPermission.getFileDefault().applyUMask(FsPermission.getUMask(getConf())),
							createFlags, getConf().getInt("io.file.buffer.size", 4096),
							lazyPersist ? 1 : getDefaultReplication(item.path), getDefaultBlockSize(), null, null);
				} else {
		...
		void rename(PathData src, PathData target) throws IOException {
			// the rename method with an option to delete the target is
			// deprecated
