...
class AclCommands extends FsCommand {
	...
	public static class GetfaclCommand extends FsCommand {
		 ...
		 * permissions of the entry, then also prints the restricted version as
		 * the effective permissions. The mask applies to all named entries and
		 * also the unnamed group entry.
		 * 
		 * @param aclStatus
		 *            AclStatus for the path
		 * @param fsPerm
		 *            FsPermission for the path
		 * @param entry
		 *            AclEntry extended ACL entry to print
		 */
		private void printExtendedAclEntry(AclStatus aclStatus, FsPermission fsPerm, AclEntry entry) {
			if (entry.getName() != null || entry.getType() == AclEntryType.GROUP) {
				...
				if (entryPerm != effectivePerm) {
					out.println(String.format("%s\t#effective:%s", entry, effectivePerm.SYMBOL));
				} else {
					out.println(entry);
				}
			} else {
	...
	public static class SetfaclCommand extends FsCommand {
		...
		protected void processOptions(LinkedList<String> args) throws IOException {
			...
			// In recursive mode, save a separate list of just the access ACL
			// entries.
			// Only directories may have a default ACL. When a recursive
			// operation
		...
		/**
		 * Returns the ACL entries to use in the API call for the given path.
		 * For a recursive operation, returns all specified ACL entries if the
		 * item is a directory or just the access ACL entries if the item is a
		 * file. For a non-recursive operation, returns all specified ACL
		 * entries.
		 *
		 * @param item
		 *            PathData path to check
