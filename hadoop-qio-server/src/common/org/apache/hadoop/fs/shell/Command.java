...
abstract public class Command extends Configured {
	...
	/**
	 * The exit code to be returned if any errors occur during execution. This
	 * method is needed to account for the inconsistency in the exit codes
	 * returned by various commands.
	 * 
	...
	/**
	 * Allows commands that don't use paths to handle the raw arguments. Default
	 * behavior is to expand the arguments via
	 ...
	 * behavior is to call {@link #expandArgument(String)} on each element which
	 * by default globs the argument. The loop catches IOExceptions, increments
	 * the error count, and displays the exception.
	 * 
	 * @param args
	...
	/**
	 * Expand the given argument into a list of {@link PathData} objects. The
	 * default behavior is to expand globs. Commands may override to perform
	 * other expansions on an argument.
	 * 
	 * @param arg
	 ...
	 * {@link #processArgument(PathData)} will be invoked with each item in the
	 * list. The loop catches IOExceptions, increments the error count, and
	 ...
	 * {@link #processPath(PathData)} loop. Ex. ls and du use this to expand out
	 * directories.
	 * 
	 * @param item
	...
	/**
	 * Provides a hook for handling paths that don't exist. By default it will
	 * throw an exception. Primarily overriden by commands that create paths
	 * such as mkdir or touch.
	 * 
	 * @param item
	...
	/**
	 * Hook for commands to implement an operation to be applied on each path
	 * for the command. Note implementation of this method is optional if
	 * earlier methods in the chain handle the operation.
	 * 
	 * @param item
	 *            a {@link PathData} object
	...
	/**
	 * Hook for commands to implement an operation to be applied on each path
	 * for the command after being processed successfully
	 * 
	 * @param item
	 *            a {@link PathData} object
	 * @throws IOException
	...
	/**
	 * Display an exception prefaced with the command name. Also increments the
	 * error count for the command which will result in a non-zero exit code.
	 * 
	 * @param e
	...
	public void displayError(Exception e) {
		...
		if (errorMessage == null) {
			// this is an unexpected condition, so dump the whole exception
			// since
	...
	/**
	 * The name of the command. Will first try to use the assigned name else
	 * fallback to the command's preferred name
	 * 
	 * @return name of the command
	 */
	public String getName() {
		return (name == null) ? getCommandField("NAME") : name.startsWith("-") ? name.substring(1) : name;
	}
