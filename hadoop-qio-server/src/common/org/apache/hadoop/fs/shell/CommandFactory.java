...
public class CommandFactory extends Configured {
	...
	/**
	 * Invokes "static void registerCommands(CommandFactory)" on the given
	 * class. This method abstracts the contract between the factory and the
	 * command class. Do not assume that directly invoking registerCommands on
	 * the given class will have the same effect.
	 * 
	...
	public void addClass(Class<? extends Command> cmdClass, String... names) {
		for (String name : names)
			classMap.put(name, cmdClass);
	}

	/**
	 * Register the given object as handling the given list of command names.
	 * Avoid calling this method and use {@link #addClass(Class, String...)}
	 * whenever possible to avoid startup overhead from excessive command object
	 * instantiations. This method is intended only for handling nested
	...
	public void addObject(Command cmdObject, String... names) {
		for (String name : names) {
			...
			classMap.put(name, null); // just so it shows up in the list of
										// commands
		}
	}
	 ...
	 * class must have been registered via {@link #addClass(Class, String...)}
	 * 
	 * @param cmd
	 *            name of the command
