...
public class ContentSummary implements Writable {
	...
	public static class Builder {
		...
		public ContentSummary build() {
			return new ContentSummary(length, fileCount, directoryCount, quota, spaceConsumed, spaceQuota, typeConsumed,
					typeQuota);
		}
	...
	/**
	 * Constructor, deprecated by ContentSummary.Builder This constructor
	 * implicitly set spaceConsumed the same as length. spaceConsumed and length
	 * must be set explicitly with ContentSummary.Builder
	 */
	@Deprecated
	...
	private ContentSummary(long length, long fileCount, long directoryCount, long quota, long spaceConsumed,
			long spaceQuota, long typeConsumed[], long typeQuota[]) {
		this.length = length;
		this.fileCount = fileCount;
		this.directoryCount = directoryCount;
		this.quota = quota;
	...
	/** The header string */
	private static final String HEADER = String.format(STRING_FORMAT.replace('d', 's'), "directories", "files",
			"bytes");

	private static final String QUOTA_HEADER = String.format(QUOTA_STRING_FORMAT + SPACE_QUOTA_STRING_FORMAT,
	 ...
	 * Return the string representation of the object in the output format. if
	 * qOption is false, output directory count, file count, and content size;
	 * if qOption is true, output quota and remaining quota as well. if hOption
	 * is false file sizes are returned in bytes if hOption is true file sizes
	 * are returned in human readable
	 * 
	 * @param qOption
	 *            a flag indicating if quota needs to be printed or not
	 * @param hOption
