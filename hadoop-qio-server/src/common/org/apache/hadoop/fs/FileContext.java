 ...
 * using the Hadoop file system. It provides a set of methods for the usual
 * operation: create, open, list, etc
 * 
 * <p>
 * <b> *** Path Names *** </b>
 * <p>
 * 
 * The Hadoop file system supports a URI name space and URI names. It offers a
 * forest of file systems that can be referenced using fully qualified URIs. Two
 * common Hadoop file systems implementations are
 * <ul>
 * <li>the local file system: file:///path
 * <li>the hdfs file system hdfs://nnAddress:nnPort/path
 * </ul>
 * 
 * While URI names are very flexible, it requires knowing the name or address of
 * the server. For convenience one often wants to access the default system in
 * one's environment without knowing its name/address. This has an additional
 * benefit that it allows one to change one's default fs (e.g. admin moves
 * application from cluster1 to cluster2).
 * <p>
 * 
 * To facilitate this, Hadoop supports a notion of a default file system. The
 * user can set his default file system, although this is typically set up for
 * you in your environment via your default config. A default file system
 * implies a default scheme and authority; slash-relative names (such as
 * /for/bar) are resolved relative to that default FS. Similarly a user can also
 * have working-directory-relative names (i.e. names not starting with a slash).
 * While the working directory is generally in the same default FS, the wd can
 * be in a different FS.
 * <p>
 ...
 * <p>
 * The FileContext provides file namespace context for resolving file names; it
 * also contains the umask for permissions, In that sense it is like the
 * per-process file-related state in Unix system. These two properties
 * <ul>
 * <li>default file system i.e your slash)
 * <li>umask
 * </ul>
 * in general, are obtained from the default configuration file in your
 * environment, (@see {@link Configuration}).
 * 
 * No other configuration parameters are obtained from the default config as far
 * as the file context layer is concerned. All file system instances (i.e.
 * deployments of file systems) have default properties; we call these server
 * side (SS) defaults. Operation like create allow one to select many
 * properties: either pass them in as explicit parameters or use the SS
 * properties.
 * <p>
 ...
 * <li>...
 * </ul>
 * Example 4: Use a specific config, ignoring $HADOOP_CONFIG Generally you
 * should not need use a config unless you are doing
 * <ul>
 * <li>configX = someConfigSomeOnePassedToYou.
 * <li>myFContext = getFileContext(configX); // configX is not changed, // is
 * passed down
 * <li>myFContext.create(path, ...);
 * <li>...
 * </ul>
...
public class FileContext {
	...
	/**
	 * Default permission for directory and symlink In previous versions, this
	 * default permission was also used to create files, so files created end up
	 * with ugo+x permission. See HADOOP-9155 for detail. Two new constants are
	 * added to solve this, please use {@link FileContext#DIR_DEFAULT_PERM} for
	 * directory, and use {@link FileContext#FILE_DEFAULT_PERM} for file. This
	...
	private FileContext(final AbstractFileSystem defFs, final FsPermission theUmask, final Configuration aConf) {
		...
		/*
		 * Init the wd. WorkingDir is implemented at the FileContext layer NOT
		 * at the AbstractFileSystem layer. If the DefaultFS, such as
		 * localFilesystem has a notion of builtin WD, we use that as the
		 * initial WD. Otherwise the WD is initialized to the home directory.
		 */
	...
	/*
	 * Remove relative part - return "absolute": If input is relative path
	 * ("foo/bar") add wd: ie "/<workingDir>/foo/bar" A fully qualified uri
	 * ("hdfs://nn:p/foo/bar") or a slash-relative path ("/foo/bar") are
	 * returned unchanged.
	 * 
	 * Applications that use FileContext should use #makeQualified() since they
	 * really want a fully qualified URI. Hence this method is not called
	 * makeAbsolute() and has been deliberately declared private.
	 */
	 ...
	 *             If the file system for <code>absOrFqPath</code> is not
	 *             supported.
	 * @throws IOExcepton
	 *             If the file system for <code>absOrFqPath</code> could not be
	 *             instantiated.
	 */
	...
	private static AbstractFileSystem getAbstractFileSystem(UserGroupInformation user, final URI uri,
			final Configuration conf) throws UnsupportedFileSystemException, IOException {
		try {
	...
	/**
	 * Protected Static Factory methods for getting a FileContexts that take a
	 * AbstractFileSystem as input. To be used for testing.
	 */
	 ...
	 * 
	 * @param defaultFS
	 * @return a FileContext with the specified AbstractFileSystem as the
	 *         default FS.
	 */
	...
	/**
	 * Static Factory methods for getting a FileContext. Note new file contexts
	 * are created for each call. The only singleton is the local FS context
	 ...
	 * construction. Changes to the config after the call are ignore by the
	 * FileContext layer. The conf is passed to lower layers like
	 * AbstractFileSystem and HDFS which pick up their own config variables.
	 */
	 ...
	 * 
	 * @throws UnsupportedFileSystemException
	 *             If the file system from the default configuration is not
	 *             supported
	 */
	 ...
	 * @throws UnsupportedFileSystemException
	 *             If the file system for {@link FsConstants#LOCAL_FS_URI} is
	 *             not supported.
	 */
	 ...
	 * 
	 * @throws UnsupportedFileSystemException
	 *             If the file system for <code>defaultFsUri</code> is not
	 *             supported
	 */
	 ...
	 * @throws UnsupportedFileSystemException
	 *             If the file system with specified is not supported
	 * @throws RuntimeException
	 *             If the file system specified is supported but could not be
	 *             instantiated, or if login fails.
	 */
	...
	/**
	 * Create a FileContext using the passed config. Generally it is better to
	 * use {@link #getFileContext(URI, Configuration)} instead of this one.
	 * 
	 * 
	 * @param aConf
	 * @return new FileContext
	 * @throws UnsupportedFileSystemException
	 *             If file system in the config is not supported
	 */
	public static FileContext getFileContext(final Configuration aConf) throws UnsupportedFileSystemException {
		final URI defaultFsUri = URI.create(aConf.get(FS_DEFAULT_NAME_KEY, FS_DEFAULT_NAME_DEFAULT));
		if (defaultFsUri.getScheme() != null && !defaultFsUri.getScheme().trim().isEmpty()) {
			return getFileContext(defaultFsUri, aConf);
		}
		throw new UnsupportedFileSystemException(
				String.format("%s: URI configured via %s carries no scheme", defaultFsUri, FS_DEFAULT_NAME_KEY));
	}

	/**
	 * @param aConf
	 *            - from which the FileContext is configured
	 * @return a FileContext for the local file system using the specified
	 *         config.
	 * 
	 * @throws UnsupportedFileSystemException
	 *             If default file system in the config is not supported
	 * 
	 */
	 ...
	 * Working directory feature is provided by simply prefixing relative names
	 * with the working dir. Note this is different from Unix where the wd is
	 * actually set to the inode. Hence setWorkingDir does not follow symlinks
	 * etc. This works better in a distributed environment that has multiple
	 * independent roots. {@link #getWorkingDirectory()} should return what
	 * setWorkingDir() set.
	 * 
	 * @param newWDir
	...
	public void setWorkingDirectory(final Path newWDir) throws IOException {
		...
		/*
		 * wd is stored as a fully qualified path. We check if the given path is
		 * not relative first since resolve requires and returns an absolute
		 * path.
		 */
	...
	/**
	 * Return the current user's home directory in this file system. The default
	 ...
	 *             If server implementation throws undeclared exception to RPC
	 *             server
	 * 
	 *             RuntimeExceptions:
	...
	/**
	 * Make the path fully qualified if it is isn't. A Fully-qualified path has
	 * scheme and authority specified and an absolute path. Use the default file
	 * system and working dir in this FileContext to qualify.
	 * 
	 * @param path
	 ...
	 *            <ul>
	 *            <li>Progress - to report progress on the operation - default
	 *            null
	 *            <li>Permission - umask is applied against permisssion: default
	 *            is FsPermissions:getDefault()
	 * 
	 *            <li>CreateParent - create missing parent path; default is to
	 *            not to create parents
	 *            <li>The defaults for the following are SS defaults of the file
	 *            server implementing the target path. Not all parameters make
	 *            sense for all kinds of file system - eg. localFS ignores
	 *            Blocksize, replication, checksum
	 *            <ul>
	 *            <li>BufferSize - buffersize used in FSDataOutputStream
	 *            <li>Blocksize - block size for file blocks
	 *            <li>ReplicationFactor - replication for blocks
	 *            <li>ChecksumParam - Checksum parameters. server default is
	 ...
	 * @throws FileNotFoundException
	 *             If parent of <code>f</code> does not exist and
	 *             <code>createParent</code> is false
	 * @throws ParentNotDirectoryException
	 *             If parent of <code>f</code> is not a directory.
	 * @throws UnsupportedFileSystemException
	 *             If file system for <code>f</code> is not supported
	 * @throws IOException
	 ...
	 * @param createParent
	 *            - if true then missing parent dirs are created if false then
	 *            parent must exist
	 * 
	 * @throws AccessControlException
	 *             If access is denied
	 * @throws FileAlreadyExistsException
	 *             If directory <code>dir</code> already exists
	 * @throws FileNotFoundException
	 *             If parent of <code>dir</code> does not exist and
	 *             <code>createParent</code> is false
	 * @throws ParentNotDirectoryException
	 *             If parent of <code>dir</code> is not a directory
	 * @throws UnsupportedFileSystemException
	 *             If file system for <code>dir</code> is not supported
	 * @throws IOException
	 *             If an I/O error occurred
	 * 
	 *             Exceptions applicable to file systems accessed over RPC:
	...
	/**
	 * Opens an FSDataInputStream at the indicated Path using default
	 * buffersize.
	 * 
	 * @param f
	 *            the file name to open
	 ...
	 * 
	 * @param f
	 *            The path to the file to be truncated
	 * @param newLength
	 *            The size the file is to be truncated to
	 *
	 * @return <code>true</code> if the file has been truncated to the desired
	 *         <code>newLength</code> and is immediately available to be reused
	 *         for write operations such as <code>append</code>, or
	 ...
	 * @throws ParentNotDirectoryException
	 *             If parent of <code>dst</code> is not a directory
	 * @throws UnsupportedFileSystemException
	 *             If file system for <code>src</code> and <code>dst</code> is
	 *             not supported
	 * @throws IOException
	 *             If an I/O error occurred
	 * 
	...
	public void rename(final Path src, final Path dst, final Options.Rename... options)
		...
		try {
			...
			/*
			 * We do not know whether the source or the destination path was
			 * unresolved. Resolve the source path up until the final path
			 * component, then fully resolve the destination.
			 */
	 ...
	 *             If <code>username</code> or <code>groupname</code> is
	 *             invalid.
	 */
	 ...
	 *            milliseconds since epoch (Jan 1, 1970). A value of -1 means
	 *            that this call should not set modification time.
	 * @param atime
	 ...
	 * This is only applicable if the corresponding FileSystem supports
	 * checksum. By default doesn't do anything.
	 * 
	 ...
	 * {@link #getFileStatus(Path)} and checks the returned permissions against
	 * the requested permissions. Note that the getFileStatus call will be
	 * subject to authorization checks. Typically, this requires search
	 * (execute) permissions on each directory in the path's prefix, but this is
	 * implementation-defined. Any file system that provides a richer
	 * authorization model (such as ACLs) may override the default
	 * implementation so that it checks against that model instead.
	 * <p>
	 * In general, applications should avoid using this method, due to the risk
	 * of time-of-check/time-of-use race conditions. The permissions on a file
	 * may change immediately after the access call returns. Most applications
	 * should prefer running specific file system actions as the desired user
	 * represented by a {@link UserGroupInformation}.
	 *
	 * @param path
	 *            Path to check
	 * @param mode
	 ...
	 * @throws UnsupportedFileSystemException
	 *             if file system for <code>path</code> is not supported
	 * @throws IOException
	...
	/**
	 * Return a file status object that represents the path. If the path refers
	 * to a symlink then the FileStatus of the symlink is returned. The behavior
	 * is equivalent to #getFileStatus() if the underlying file system does not
	...
	/**
	 * Returns the target of the given symbolic link as it was specified when
	 * the link was created. Links in the path leading up to the final path
	 * component are resolved transparently.
	 *
	 * @param f
	 ...
	 *             If file system for <code>f</code> is not supported
	 * @throws IOException
	 *             If the given path does not refer to a symlink or an I/O error
	 *             occurred
	 */
	...
	/**
	 * Return blockLocation of the given file for the given offset and len. For
	 * a nonexistent file or regions, null will be returned.
	 *
	 ...
	 * system denoted by the Parh argument p. If the file system has multiple
	 * partitions, the use and capacity of the partition pointed to by the
	 * specified path is reflected.
	 * 
	 * @param f
	 *            Path for which status should be obtained. null means the root
	 *            partition of the default file system.
	 *
	 ...
	 * 
	 * Symlink permissions are ignored, access to a symlink is determined by the
	 ...
	 * functions operate on the symlink itself, these are: - delete(f) and
	 * deleteOnExit(f) - Deletes the symlink. - rename(src, dst) - If src refers
	 * to a symlink, the symlink is renamed. If dst refers to a symlink, the
	 * symlink is over-written. - getLinkTarget(f) - Returns the target of the
	 * symlink. - getFileLinkStatus(f) - Returns a FileStatus object describing
	 * the symlink. Some functions, create() and mkdir(), expect the final path
	 * component does not exist. If they are given a path that refers to a
	 * symlink that does exist they behave as if the path referred to an
	 * existing file or directory. All other functions fully resolve, ie follow,
	 * the symlink. These are: open, setReplication, setOwner, setTimes,
	 * setWorkingDirectory, setPermission, getFileChecksum, setVerifyChecksum,
	 * getFileBlockLocations, getFsStatus, getFileStatus, exists, and
	 * listStatus.
	 * 
	 ...
	 * @throws ParentNotDirectoryException
	 *             If parent of <code>link</code> is not a directory.
	 * @throws UnsupportedFileSystemException
	 *             If file system for <code>target</code> or <code>link</code>
	 *             is not supported
	 * @throws IOException
	 *             If an I/O error occurred
	 */
	 ...
	 *
	 * @return an iterator that traverses statuses of the files/directories in
	 *         the given path If any IO exception (for example the input
	 *         directory gets deleted while listing is being executed), next()
	 *         or hasNext() of the returned iterator may throw a
	...
	/**
	 * Utility/library methods built over the basic FileContext methods. Since
	 * this are library functions, the oprtation are not atomic and some of them
	 * may partially complete if other threads are making changes to the same
	 * part of the name space.
	 */
	public class Util {
		/**
		 * Does the file exist? Note: Avoid using this method if you already
		 * have FileStatus in hand. Instead reuse the FileStatus
		 * 
		 * @param f
		 ...
		 *             If an exception occurred in the RPC server
		 * @throws UnexpectedServerException
		 *             If server implementation throws undeclared exception to
		 *             RPC server
		 */
		...
		/**
		 * Filter files/directories in the given path using the user-supplied
		 * path filter.
		 * 
		 * @param f
		 *            is the path name
		 ...
		 *
		 * @return an array of FileStatus objects for the files under the given
		 *         path after applying the filter
		 *
		 * @throws AccessControlException
		 *             If access is denied
		 * @throws FileNotFoundException
		 *             If <code>f</code> does not exist
		 * @throws UnsupportedFileSystemException
		 *             If file system for <code>pathPattern</code> is not
		 *             supported
		 * @throws IOException
		 *             If an I/O error occurred
		 * 
		 ...
		 *             If access is denied
		 * @throws FileNotFoundException
		 *             If a file in <code>files</code> does not exist
		 * @throws IOException
		 *             If an I/O error occurred
		 * 
		 *             Exceptions applicable to file systems accessed over RPC:
		...
		/*
		 * Filter files/directories in the given path using the user-supplied
		 * path filter. Results are added to the given array
		 * <code>results</code>.
		 */
		 ...
		 *            is the path
		 *
		 * @return an array that contains statuses of the files/directories in
		 *         the given path
		 *
		 * @throws AccessControlException
		 *             If access is denied
		 ...
		 * 
		 * If the path is a directory, if recursive is false, returns files in
		 * the directory; if recursive is true, return files in the subtree
		 * rooted at the path. The subtree is traversed in the depth-first
		 ...
		 *
		 * @return an iterator that traverses statuses of the files If any IO
		 *         exception (for example a sub-directory gets deleted while
		 *         listing is being executed), next() or hasNext() of the
		 *         returned iterator may throw a RuntimeException with the IO
		...
		public RemoteIterator<LocatedFileStatus> listFiles(final Path f, final boolean recursive)
			...
			return new RemoteIterator<LocatedFileStatus>() {
				 ...
				 * @throws AccessControlException
				 *             if not allowed to access next file's status or
				 *             locations
				 * @throws FileNotFoundException
				 *             if next file does not exist any more
				 * @throws UnsupportedFileSystemException
				 *             if next file's fs is unsupported
				 * @throws IOException
				 *             for all other IO errors for example, NameNode is
				 *             not avaialbe or NameNode throws IOException due
				 *             to an error while getting the status or block
				 *             locations
				 */
				@Override
				...
				/**
				 * Process the input stat. If it is a file, return the file
				 * stat. If it is a directory, traverse the directory if
				 * recursive is true; ignore it if recursive is false. If it is
				 * a symlink, resolve the symlink first and then process it
		 ...
		 * pathPattern and is accepted by the user-supplied path filter. Results
		 * are sorted by their path names. Return null if pathPattern has no
		 * glob and the path does not exist. Return an empty array if
		 ...
		 */
		public boolean copy(final Path src, final Path dst) throws AccessControlException, FileAlreadyExistsException,
				FileNotFoundException, ParentNotDirectoryException, UnsupportedFileSystemException, IOException {
		 ...
		 *            - delete src if true
		 * @param overwrite
		 *            overwrite dst if true; throw IOException if dst exists and
		 *            overwrite is false.
		 *
		 ...
		 *             If parent of <code>dst</code> is not a directory
		 * @throws UnsupportedFileSystemException
		 *             If file system for <code>src</code> or <code>dst</code>
		 *             is not supported
		 * @throws IOException
		 *             If an I/O error occurred
		 * 
		...
		public boolean copy(final Path src, final Path dst, boolean deleteSource, boolean overwrite)
			...
			if (fs.isDirectory()) {
				...
				for (FileStatus content : contents) {
					copy(makeQualified(content.getPath()), makeQualified(new Path(qDst, content.getPath().getName())),
	...
	/**
	 * Check if copying srcName to dst would overwrite an existing file or
	 * directory.
	 * 
	 * @param srcName
	 ...
	 *             If access is denied.
	 * @throws IOException
	 *             If dst is an existing directory, or dst is an existing file
	...
	/**
	 * Resolves all symbolic links in the specified path. Returns the new path
	 * object.
	 */
	protected Path resolve(final Path f)
			throws FileNotFoundException, UnresolvedLinkException, AccessControlException, IOException {
		return new FSLinkResolver<Path>() {
			@Override
			public Path next(final AbstractFileSystem fs, final Path p) throws IOException, UnresolvedLinkException {
	...
	/**
	 * Resolves all symbolic links in the specified path leading up to, but not
	...
	/**
	 * Returns the list of AbstractFileSystems accessed in the path. The list
	 * may contain more than one AbstractFileSystems objects in case of
	 * symlinks.
	 * 
	 * @param f
	 ...
	 * 
	 * @param uri
	 *            the uri to lookup the statistics. Only scheme and authority
	 *            part of the uri are used as the key to store and lookup.
	...
	/**
	 * Prints the statistics to standard output. File System is identified by
	 ...
	 *         uri consists of scheme and authority for the filesystem.
	 */
	 ...
	 * ACL entries or modify the permissions on existing ACL entries. All
	 * existing ACL entries that are not specified in this call are retained
	 * without changes. (Modifications are merged into the current ACL.)
	 *
	 * @param path
	 *            Path to modify
	 * @param aclSpec
	 ...
	 * entries for user, group, and others are retained for compatibility with
	 * permission bits.
	 *
	 * @param path
	 *            Path to modify
	...
	/**
	 * Set an xattr of a file or directory. The name must be prefixed with the
	 * namespace followed by ".". For example, "user.attr".
	 * <p/>
	...
	/**
	 * Get an xattr for a file or directory. The name must be prefixed with the
	 * namespace followed by ".". For example, "user.attr".
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	...
	/**
	 * Get all of the xattrs for a file or directory. Only those xattrs for
	 * which the logged-in user has permissions to view are returned.
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 * @param path
	 *            Path to get extended attributes
	 * @return Map<String, byte[]> describing the XAttrs of the file or
	 *         directory
	 * @throws IOException
	 */
	...
	/**
	 * Remove an xattr of a file or directory. The name must be prefixed with
	 * the namespace followed by ".". For example, "user.attr".
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 * @param path
	...
	/**
	 * Get all of the xattr names for a file or directory. Only those xattr
	 * names which the logged-in user has permissions to view are returned.
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 * @param path
