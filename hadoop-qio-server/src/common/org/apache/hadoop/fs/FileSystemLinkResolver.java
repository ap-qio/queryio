...
public abstract class FileSystemLinkResolver<T> {
	 ...
	 * Overridden on instantiation to perform the actual method call, which
	 * throws an UnresolvedLinkException if called on an unresolved
	 * {@link Path}.
	 * 
	 * @param p
	 *            Path on which to perform an operation
	...
	/**
	 * Attempt calling overridden {@link #doCall(Path)} method with specified
	 * {@link FileSystem} and {@link Path}. If the call fails with an
	 * UnresolvedLinkException, it will try to resolve the path and retry the
	 * call by calling {@link #next(FileSystem, Path)}.
	 * 
	 * @param filesys
	 *            FileSystem with which to try call
	 * @param path
	 *            Path with which to try call
	 * @return Generic type determined by implementation
	 * @throws IOException
	 */
