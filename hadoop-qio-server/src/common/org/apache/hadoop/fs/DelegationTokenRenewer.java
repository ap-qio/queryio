...
public class DelegationTokenRenewer extends Thread {
	...
	/**
	 * An action that will renew and replace the file system's delegation tokens
	 * automatically.
	 */
	public static class RenewAction<T extends FileSystem & Renewable> implements Delayed {
		...
		/**
		 * Set a new time for the renewal. It can only be called when the action
		 * is not in the queue or any collection because the hashCode may change
		 * 
		 * @param newTime
		 *            the new time
		 */
		...
		/**
		 * Renew or replace the delegation token for this file system. It can
		 * only be called when the action is not in the queue.
		 * 
		 * @return
		 * @throws IOException
		 */
	 ...
	 * in {@link #addRenewAction(FileSystem)}
	 */
