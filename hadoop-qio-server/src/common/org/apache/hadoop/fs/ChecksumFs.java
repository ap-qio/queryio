...
/**
 * Abstract Checksumed Fs. It provide a basic implementation of a Checksumed Fs,
 * which creates a checksum file for each raw file. It generates & verifies
 * checksums at the client side.
 */
...
public abstract class ChecksumFs extends FilterFs {
	...
	private int getSumBufferSize(int bytesPerSum, int bufferSize) throws IOException {
	...
	private static class ChecksumFSInputChecker extends FSInputChecker {
		...
		protected int readChunk(long pos, byte[] buf, int offset, int len, byte[] checksum) throws IOException {
			...
			if (needChecksum()) {
				...
				assert checksum.length % CHECKSUM_SIZE == 0; // it is sane
																// length
				assert len >= bytesPerSum; // we must read at least one chunk

				final int checksumsToRead = Math.min(len / bytesPerSum, // number
																		// of
																		// checksums
																		// based
																		// on
																		...
																		// to
																		// read
						checksum.length / CHECKSUM_SIZE); // size of checksum
															// buffer
				...
				if (sumLenRead >= 0 && sumLenRead % CHECKSUM_SIZE != 0) {
					throw new EOFException("Checksum file not a length multiple of checksum size " + "in " + file
							+ " at " + pos + " checksumpos: " + checksumPos + " sumLenread: " + sumLenRead);
				}
				if (sumLenRead <= 0) { // we're at the end of the file
					eof = true;
				} else {
					// Adjust amount of data to read based on how many checksum
					// chunks we read
	...
	private static class ChecksumFSOutputSummer extends FSOutputSummer {
		...
		public ChecksumFSOutputSummer(final ChecksumFs fs, final Path file, final EnumSet<CreateFlag> createFlag,
			...
			this.datas = fs.getRawFs().createInternal(file, createFlag, absolutePermission, bufferSize, replication,
			...
			this.sums = fs.getRawFs().createInternal(fs.getChecksumFile(file),
					EnumSet.of(CreateFlag.CREATE, CreateFlag.OVERWRITE), absolutePermission, sumBufferSize, replication,
					blockSize, progress, checksumOpt, createParent);
		...
		@Override
		protected void writeChunk(byte[] b, int offset, int len, byte[] checksum, int ckoff, int cklen)
				throws IOException {
	...
	@Override
	public FSDataOutputStream createInternal(Path f, EnumSet<CreateFlag> createFlag, FsPermission absolutePermission,
			int bufferSize, short replication, long blockSize, Progressable progress, ChecksumOpt checksumOpt,
			boolean createParent) throws IOException {
		final FSDataOutputStream out = new FSDataOutputStream(new ChecksumFSOutputSummer(this, f, createFlag,
				absolutePermission, bufferSize, replication, blockSize, progress, checksumOpt, createParent), null);
		return out;
	}
	 ...
	 */
	public boolean reportChecksumFailure(Path f, FSDataInputStream in, long inPos, FSDataInputStream sums,
			long sumsPos) {
		return false;
	}
