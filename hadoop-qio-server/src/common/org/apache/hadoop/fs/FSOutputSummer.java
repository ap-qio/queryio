...
abstract public class FSOutputSummer extends OutputStream {
	...
	// We want this value to be a multiple of 3 because the native code
	// checksums
	 ...
	 * with a length of <code>len > 0</code>, and its checksum
	 */
	...
	private int write1(byte b[], int off, int len) throws IOException {
		if (count == 0 && len >= buf.length) {
			// local buffer is empty and user buffer size >= local buffer size,
			// so
			// simply checksum the user buffer and send it directly to the
			// underlying
			// stream
	...
	/*
	 * Forces buffered output bytes to be checksummed and written out to the
	 * underlying output stream. If there is a trailing partial chunk in the
	 * buffer, 1) flushPartial tells us whether to flush that chunk 2) if
	 * flushPartial is true, keep tells us whether to keep that chunk in the
	 ...
	 * checksums to the underlying output stream.
	 */
