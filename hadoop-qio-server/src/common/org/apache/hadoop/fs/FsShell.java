...
public class FsShell extends Configured implements Tool {
	 ...
	 * {@link #setConf(Configuration)} with a valid configuration prior to
	 * running commands.
	 */
	public FsShell() {
		this(null);
	...
	/*
	 * The following are helper methods for getInfo(). They are defined outside
	 * of the scope of the Help/Usage class because the run() method needs to
	 * invoke them too.
	 */
	...
	public int run(String argv[]) throws Exception {
		...
		if (argv.length < 1) {
			...
			try {
				...
				// instance.run catches IOE, so something is REALLY wrong if
				// here
				LOG.debug("Error", e);
