...
/**
 * This is a wrapper class. It wraps a Writable implementation around an array
 * of primitives (e.g., int[], long[], etc.), with optimized wire format, and
 * without creating new objects per element.
 * 
 * This is a wrapper class only; it does not make a copy of the underlying
 * array.
 */
...
public class ArrayPrimitiveWritable implements Writable {
	...
	private void checkDeclaredComponentType(Class<?> componentType) {
		if ((declaredComponentType != null) && (componentType != declaredComponentType)) {
			throw new HadoopIllegalArgumentException("input array component type " + componentType.getName()
