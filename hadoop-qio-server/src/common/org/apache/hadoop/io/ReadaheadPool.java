...
/**
 * Manages a pool of threads which can issue readahead requests on file
 * descriptors.
 */
...
public class ReadaheadPool {
	 ...
	 * 
	 * @param identifier
	 *            a textual identifier that will be used in error messages (e.g.
	 *            the file name)
	 * @param fd
	 *            the file descriptor to read ahead
	 * @param curPos
	 *            the current offset at which reads are being issued
	 * @param readaheadLength
	 *            the configured length to read ahead
	 * @param maxOffsetToRead
	 *            the maximum offset that will be readahead (useful if, for
	 *            example, only some segment of the file is requested by the
	 *            user). Pass {@link Long.MAX_VALUE} to allow readahead to the
	 *            end of the file.
	 * @param lastReadahead
	 *            the result returned by the previous invocation of this
	 *            function on this file descriptor, or null if this is the first
	 *            call
	 * @return an object representing this outstanding request, or null if no
	 *         readahead was performed
	 */
	 ...
	 * This request may be pending or may have been completed.
	 */
	public interface ReadaheadRequest {
		/**
		 * Cancels the request for readahead. This should be used if the reader
		 * no longer needs the requested data, <em>before</em> closing the
		 * related file descriptor.
		 * 
		 * It is safe to use even if the readahead request has already been
		 * fulfilled.
		 */
		public void cancel();
	...
	private static class ReadaheadRequestImpl implements Runnable, ReadaheadRequest {
		...
		public void run() {
			...
			try {
				NativeIO.POSIX.getCacheManipulator().posixFadviseIfPossible(identifier, fd, off, len,
		...
		public String toString() {
			return "ReadaheadRequestImpl [identifier='" + identifier + "', fd=" + fd + ", off=" + off + ", len=" + len
					+ "]";
		}
	}
}
