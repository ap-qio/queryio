...
/**
 * This is a simple ByteBufferPool which just creates ByteBuffers as needed. It
 * also caches ByteBuffers after they're released. It will always return the
 * smallest cached buffer with at least the capacity you request. We don't try
 * to do anything clever here like try to limit the maximum cache size.
 */
...
public final class ElasticByteBufferPool implements ByteBufferPool {
	private static final class Key implements Comparable<Key> {
		...
		public int compareTo(Key other) {
			return ComparisonChain.start().compare(capacity, other.capacity).compare(insertionTime, other.insertionTime)
					.result();
		}
	...
	public synchronized void putBuffer(ByteBuffer buffer) {
		...
		while (true) {
			...
			// If our key is not unique on the first try, we try again, since
			// the
