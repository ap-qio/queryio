...
public class MapFile {
	...
	public static class Writer implements java.io.Closeable {
		...
		public Writer(Configuration conf, FileSystem fs, String dirName, Class<? extends WritableComparable> keyClass,
				...
				throws IOException {
			this(conf, new Path(dirName), keyClass(keyClass), valueClass(valClass), compression(compress, codec),
					progressable(progress));
		}
		...
		public static SequenceFile.Writer.Option compression(CompressionType type, CompressionCodec codec) {
		...
		public Writer(Configuration conf, Path dirName, SequenceFile.Writer.Option... opts) throws IOException {
			...
			SequenceFile.Writer.Option[] dataOptions = Options.prependOptions(opts, SequenceFile.Writer.file(dataFile),
					SequenceFile.Writer.keyClass(keyClass));
			this.data = SequenceFile.createWriter(conf, dataOptions);

			SequenceFile.Writer.Option[] indexOptions = Options.prependOptions(opts,
	...
	public static class Reader implements java.io.Closeable {
		...
		protected synchronized void open(Path dir, WritableComparator comparator, Configuration conf,
	...
	public static long fix(FileSystem fs, Path dir, Class<? extends Writable> keyClass,
		...
		if (!dryrun) {
			indexWriter = SequenceFile.createWriter(conf, SequenceFile.Writer.file(index),
	...
	public static class Merger {
		...
		/*
		 * Open all input files for reading and verify the key and value types.
		 ...
		 * 4. Replace the already written key/value in keys/values arrays with
		 * the next key/value from the selected input <br>
		...
		private void mergePass() throws IOException {
			...
			do {
				...
				// Replace the already written key/value in keys/values arrays
				// with the
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			writer = new MapFile.Writer(conf, fs, out, reader.getKeyClass().asSubclass(WritableComparable.class),
