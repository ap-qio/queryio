...
public class ObjectWritable implements Writable, Configurable {
	...
	public static Object readObject(DataInput in, ObjectWritable objectWritable, Configuration conf)
		...
		if (declaredClass.isPrimitive()) { // primitive types
			...
			// Always allow the read, even if write is disabled by
			// allowCompactArrays.
	...
	/**
	 * Try to instantiate a protocol buffer of the given message class from the
	 * given input stream.
	 * 
	 * @param protoClass
	 *            the class of the generated protocol buffer
	 * @param dataIn
	...
	private static Message tryInstantiateProtobuf(Class<?> protoClass, DataInput dataIn) throws IOException {
		...
		try {
			if (dataIn instanceof InputStream) {
				// We can use the built-in parseDelimitedFrom and not have to
				// re-copy
				// the data
				...
				// Have to read it into a buffer first, since protobuf doesn't
				// deal
