...
public class EnumSetWritable<E extends Enum<E>> extends AbstractCollection<E> implements Writable, Configurable {
	...
	/**
	 * Construct a new EnumSetWritable. If the <tt>value</tt> argument is null
	 * or its size is zero, the <tt>elementType</tt> argument must not be null.
	 * If the argument <tt>value</tt>'s size is bigger than zero, the argument
	 ...
	 * <tt>value</value> and <tt>elementType</tt>. If the <tt>value</tt>
	 * argument is null or its size is zero, the <tt>elementType</tt> argument
	 * must not be null. If the argument <tt>value</tt>'s size is bigger than
	 * zero, the argument <tt>elementType</tt> is not be used.
	 * 
	 * @param value
	 * @param elementType
	 */
