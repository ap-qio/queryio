 ...
 * 
 * <p>
 * <code>SequenceFile</code> provides {@link SequenceFile.Writer},
 * {@link SequenceFile.Reader} and {@link Sorter} classes for writing, reading
 ...
 * <ol>
 * <li><code>Writer</code> : Uncompressed records.</li>
 * <li><code>RecordCompressWriter</code> : Record-compressed files, only
 * compress values.</li>
 * <li><code>BlockCompressWriter</code> : Block-compressed files, both keys &
 ...
 * above <code>SequenceFile</code> formats.
 * </p>
 *
 ...
 * <li>keyClassName -key class</li>
 * <li>valueClassName - value class</li>
 * <li>compression - A boolean which specifies if compression is turned on for
 * keys/values in this file.</li>
 * <li>blockCompression - A boolean which specifies if block-compression is
 * turned on for keys/values in this file.</li>
 ...
 * </ul>
 * </li>
 * <li>A sync-marker every few <code>100</code> bytes or so.</li>
 * </ul>
 *
 ...
 * </ul>
 * </li>
 * <li>A sync-marker every block.</li>
 * </ul>
 * 
 * <p>
...
public class SequenceFile {
	...
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass)
			throws IOException {
		return createWriter(conf, Writer.filesystem(fs), Writer.file(name), Writer.keyClass(keyClass),
	...
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
		...
		return createWriter(conf, Writer.filesystem(fs), Writer.file(name), Writer.keyClass(keyClass),
				Writer.valueClass(valClass), Writer.compression(compressionType));
	}
	...
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
		...
		return createWriter(conf, Writer.file(name), Writer.filesystem(fs), Writer.keyClass(keyClass),
				Writer.valueClass(valClass), Writer.compression(compressionType), Writer.progressable(progress));
	}
	...
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
		...
		return createWriter(conf, Writer.file(name), Writer.filesystem(fs), Writer.keyClass(keyClass),
				Writer.valueClass(valClass), Writer.compression(compressionType, codec));
	}
	...
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
			...
			throws IOException {
		return createWriter(conf, Writer.file(name), Writer.filesystem(fs), Writer.keyClass(keyClass),
				Writer.valueClass(valClass), Writer.compression(compressionType, codec), Writer.progressable(progress),
	...
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
		...
		return createWriter(conf, Writer.file(name), Writer.filesystem(fs), Writer.keyClass(keyClass),
				Writer.valueClass(valClass), Writer.bufferSize(bufferSize), Writer.replication(replication),
				Writer.blockSize(blockSize), Writer.compression(compressionType, codec), Writer.progressable(progress),
				Writer.metadata(metadata));
	}
	...
	@Deprecated
	public static Writer createWriter(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass,
			int bufferSize, short replication, long blockSize, boolean createParent, CompressionType compressionType,
	 ...
	 * 
	 * @param fc
	 *            The context for the specified file.
	 * @param conf
	 *            The configuration.
	 * @param name
	 *            The name of the file.
	 ...
	 * @param opts
	 *            file creation options; see {@link CreateOpts}.
	 * @return Returns the handle to the constructed SequenceFile Writer.
	 * @throws IOException
	 */
	...
	public static Writer createWriter(Configuration conf, FSDataOutputStream out, Class keyClass, Class valClass,
		...
		return createWriter(conf, Writer.stream(out), Writer.keyClass(keyClass), Writer.valueClass(valClass),
				Writer.compression(compressionType, codec), Writer.metadata(metadata));
	}
	...
	public static class Writer implements java.io.Closeable, Syncable {
		...
		/**
		 * @deprecated only used for backwards-compatibility in the createWriter
		 *             methods that take FileSystem.
		 */
		@Deprecated
		...
		Writer(Configuration conf, Option... opts) throws IOException {
			...
			if (ownStream) {
				...
				short replication = replicationOption == null ? fs.getDefaultReplication(p)
		...
		public Writer(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass) throws IOException {
			...
			init(conf, fs.create(name), true, keyClass, valClass, null, new Metadata());
		}
		...
		public Writer(FileSystem fs, Configuration conf, Path name, Class keyClass, Class valClass, int bufferSize,
			...
			this.compress = CompressionType.NONE;
			init(conf, fs.create(name, true, bufferSize, replication, blockSize, progress), true, keyClass, valClass,
					null, metadata);
		}
		...
		void init(Configuration conf, FSDataOutputStream out, boolean ownStream, Class keyClass, Class valClass,
				CompressionCodec codec, Metadata metadata) throws IOException {
			this.conf = conf;
			this.out = out;
			...
			if (this.keySerializer == null) {
				throw new IOException("Could not find a serializer for the Key class: '" + keyClass.getCanonicalName()
			...
			if (this.uncompressedValSerializer == null) {
				throw new IOException("Could not find a serializer for the Value class: '" + valClass.getCanonicalName()
						+ "'. " + "Please ensure that the configuration '"
						+ CommonConfigurationKeys.IO_SERIALIZATIONS_KEY + "' is "
						+ "properly configured, if you're using" + "custom serialization.");
			}
	...
	static class RecordCompressWriter extends Writer {
		...
		RecordCompressWriter(Configuration conf, Option... options) throws IOException {
			super(conf, options);
		}
	...
	static class BlockCompressWriter extends Writer {
		...
		BlockCompressWriter(Configuration conf, Option... options) throws IOException {
			super(conf, options);
	...
	public static class Reader implements java.io.Closeable {
		 ...
		 */
		@Deprecated
		public Reader(FSDataInputStream in, int buffersize, long start, long length, Configuration conf)
				throws IOException {
			this(conf, stream(in), start(start), length(length));
		}

		/** Common work of the constructors. */
		private void initialize(Path filename, FSDataInputStream in, long start, long length, Configuration conf,
		 ...
		 * @param length
		 *            The length being read if it is >= 0. Otherwise, the length
		 *            is not available.
	...
	public static class Sorter {
		...
		private class SortPass {
				...
				while (!atEof) {
					...
					while (!atEof && bytesProcessed < memoryLimit && count < recordLimit) {
						...
						ValueBytes rawValue = (count == keyOffsets.length || rawValues[count] == null)
								? in.createValueBytes() : rawValues[count];
			...
			private void flush(int count, int bytesProcessed, CompressionType compressionType, CompressionCodec codec,
				...
				Writer writer = createWriter(conf, Writer.stream(out), Writer.keyClass(keyClass),
						Writer.valueClass(valClass), Writer.compression(compressionType, codec),
		...
		public Writer cloneFileAttributes(Path inputFile, Path outputFile, Progressable prog) throws IOException {
			...
			Writer writer = createWriter(conf, Writer.file(outputFile), Writer.keyClass(keyClass),
					Writer.valueClass(valClass), Writer.compression(compress, codec), Writer.progressable(prog));
			return writer;
		}
			...
			public RawKeyValueIterator merge() throws IOException {
				...
				do {
					...
					if (numSegments <= factor) {
						...
						SegmentDescriptor tempSegment = new SegmentDescriptor(0, fs.getFileStatus(outputFile).getLen(),
								outputFile);
		...
		public class SegmentDescriptor implements Comparable {
			...
			public boolean nextRawKey() throws IOException {
				if (in == null) {
					...
					Reader reader = new Reader(conf, Reader.file(segmentPathName), Reader.bufferSize(bufferSize),
							Reader.start(segmentOffset), Reader.length(segmentLength));
