...
public class RetryPolicies {
	...
	/**
	 * Retry up to maxRetries. The actual sleep time of the n-th retry is f(n,
	 * sleepTime), where f is a function provided by the subclass
	 * implementation.
	 *
	 * The object of the subclasses should be immutable; otherwise, the subclass
	 * must override hashCode(), equals(..) and toString().
	 */
	 ...
	 * the first n0 retries sleep t0 milliseconds on average, the following n1
	 * retries sleep t1 milliseconds on average, and so on.
	 * 
	...
	public static class MultipleLinearRandomRetry implements RetryPolicy {
		...
		public RetryAction shouldRetry(Exception e, int curRetry, int failovers, boolean isIdempotentOrAtMostOnce)
			...
			final double ratio = RANDOM.get().nextDouble() + 0.5;// 0.5 <= ratio
																	// <=1.5
		 ...
		 * 
		 * @return the corresponding pair, or null if the current number of
		 *         retry > maximum number of retry.
		 */
		...
		/**
		 * Parse the given string as a MultipleLinearRandomRetry object. The
		 * format of the string is "t_1, n_1, t_2, n_2, ...", where t_i and n_i
		 * are the i-th pair of sleep time and number of retires. Note that the
		 * white spaces in the string are ignored.
		 *
		 ...
		 * 
		 * @return -1 if the parsing fails or the parsed value <= 0; otherwise,
		...
		private static int parsePositiveInt(final String[] elements, final int i, final String originalString) {
			...
			try {
				...
				LOG.warn("Failed to parse \"" + s + "\", which is the index " + i + " element in \"" + originalString
						+ "\"", nfe);
				return -1;
			}

			if (n <= 0) {
				LOG.warn("The value " + n + " <= 0: it is parsed from the string \"" + s + "\" which is the index " + i
						+ " element in \"" + originalString + "\"");
				return -1;
			}
			return n;
		}
	...
	/**
	 * Fail over and retry in the case of: Remote StandbyException (server is
	 * up, but is not the active server) Immediate socket exceptions (e.g. no
	 * route to host, econnrefused) Socket exceptions after initial connection
	 ...
	 * 
	 * Fail immediately in the case of: Socket exceptions after initial
	 * connection when operation is not idempotent
	 * 
	...
	static class FailoverOnNetworkExceptionRetry implements RetryPolicy {
		...
		/**
		 * @return 0 if this is our first failover/retry (i.e., retry
		 *         immediately), sleep exponentially otherwise
		 */
	 ...
	 * 
	 * @param time
	 *            the base amount of time to work with
	 * @param retries
	 *            the number of retries that have so occurred so far
	 * @param cap
	 *            value at which to cap the base sleep time
