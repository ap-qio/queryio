...
/**
 * This class provides output and input streams for bzip2 compression and
 * decompression. It uses the native bzip2 library on the system if possible,
 * else it uses a pure-Java implementation of the bzip2 algorithm. The
 * configuration parameter io.compression.codec.bzip2.library can be used to
 * control this behavior.
 *
 * In the pure-Java mode, the Compressor and Decompressor interfaces are not
 * implemented. Therefore, in that mode, those methods of CompressionCodec which
 * have a Compressor or Decompressor type argument, throw
 * UnsupportedOperationException.
 *
 * Currently, support for splittability is available only in the pure-Java mode;
 * therefore, if a SplitCompressionInputStream is requested, the pure-Java
 * implementation is used, regardless of the setting of the configuration
...
public class BZip2Codec implements Configurable, SplittableCompressionCodec {
	 ...
	 *            The InputStream
	 * @param start
	 *            The start offset into the compressed stream
	 * @param end
	 *            The end offset into the compressed stream
	 * @param readMode
	 *            Controls whether progress is reported continuously or only at
	 *            block boundaries.
	 *
	 * @return CompressionInputStream for BZip2 aligned at block boundaries
	 */
	public SplitCompressionInputStream createInputStream(InputStream seekableIn, Decompressor decompressor, long start,
		...
		// ........................................^^[current position of
		// stream]
		// .....................^^[We go back 10 Bytes in stream and find a
		// Block marker]
		// ........................................^^[We align at wrong
		// position!]
		// ...........................................................^^[While
	...
	/**
	 * This class is capable to de-compress BZip2 data in two modes; CONTINOUS
	 * and BYBLOCK. BYBLOCK mode makes it possible to do decompression starting
	 * any arbitrary position in the stream.
	 *
	 * So this facility can easily be used to parallelize decompression of a
	 * large BZip2 file for performance reasons. (It is exactly done so for
	 * Hadoop framework. See LineRecordReader for an example). So one can break
	 * the file (of course logically) into chunks for parallel processing. These
	 * "splits" should be like default Hadoop splits (e.g as in FileInputFormat
	 * getSplit metod). So this code is designed and tested for
	 * FileInputFormat's way of splitting only.
	 */
	...
	private static class BZip2CompressionInputStream extends SplitCompressionInputStream {
		 ...
		 * [m] represent end of block, \n is line delimiter and . represent
		 * compressed data.
		 *
		 * ............[m]......\n.......
		 *
		 ...
		 * will stop at \n and there is no need to read one more line. (To see
		 * the reason of reading one more line in the next() method is explained
		 * in LineRecordReader.) While in the second example LineRecordReader
		 * needs to read one more line (till the second \n). Now since
		 * BZip2Codecs only update position at least one byte passed a maker, so
		 * it is straight forward to differentiate between the two cases
		 * mentioned.
		 *
		 */
		...
		/*
		 * As the comments before read method tell that compressed stream is
		 * advertised when at least one byte passed EOB have been read off. But
		 * there is an exception to this rule. When we construct the stream we
		 * advertise the position exactly at EOB. In the following method
