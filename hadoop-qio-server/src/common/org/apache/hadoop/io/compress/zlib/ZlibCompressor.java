...
public class ZlibCompressor implements Compressor {
	...
	protected final void construct(CompressionLevel level, CompressionStrategy strategy, CompressionHeader header,
	...
	public ZlibCompressor(Configuration conf) {
		this(ZlibFactory.getCompressionLevel(conf), ZlibFactory.getCompressionStrategy(conf),
				CompressionHeader.DEFAULT_HEADER, DEFAULT_DIRECT_BUFFER_SIZE);
	}
	...
	public boolean needsInput() {
		...
		if (uncompressedDirectBuf.remaining() > 0) {
			...
			if (userBufLen <= 0) {
				...
				// copy enough data from userBuf to uncompressedDirectBuf
				setInputFromSavedData();
				if (uncompressedDirectBuf.remaining() > 0) // uncompressedDirectBuf
															// is not full
					return true;
				else
					return false;
