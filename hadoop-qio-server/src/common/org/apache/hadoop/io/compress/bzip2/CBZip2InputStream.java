...
public class CBZip2InputStream extends InputStream implements BZip2Constants {
	...
	public static final long EOS_DELIMITER = 0X177245385090L;// end of bzip2
																// stream
	 ...
	 *
	 * @param count
	 *            count is the number of bytes to be added to raw processed
	 *            bytes
	 */
	...
	/**
	 * This method is called by the client of this class in case there are any
	 * corrections in the stream position. One common example is when client of
	 * this code removes starting BZ characters from the compressed stream.
	 *
	 * @param count
	...
	/**
	 * This method tries to find the marker (passed to it as the first
	 * parameter) in the stream. It can find bit patterns of length <= 63 bits.
	 ...
	 * of the stream. If marker is found, the stream position will be right
	 * after marker at the end of this call.
	 *
	 * @param marker
	 ...
	 *             if <tt>in == null</tt>
	 */
	public CBZip2InputStream(final InputStream in, READ_MODE readMode) throws IOException {
		this(in, readMode, false);
	}

	private CBZip2InputStream(final InputStream in, READ_MODE readMode, boolean skipDecompression) throws IOException {
	...
	/**
	 * Returns the number of bytes between the current stream position and the
	 * immediate next BZip2 block marker.
	 *
	 * @param in
	 *            The InputStream
	 *
	 * @return long Number of bytes between current stream position and the next
	 *         BZip2 block start marker.
	 * @throws IOException
	 *
	 */
	...
	/**
	 * In CONTINOUS reading mode, this read method starts from the start of the
	 * compressed stream and end at the end of file by emitting un-compressed
	 * data. In this mode stream positioning is not announced and should be
	 * ignored.
	 *
	 * In BYBLOCK reading mode, this read method informs about the end of a
	 * BZip2 block by returning EOB. At this event, the compressed stream
	 * position is also announced. This announcement tells that how much of the
	 * compressed stream has been de-compressed and read out of this class. In
	 * between EOB events, the stream position is not updated.
	 *
	 *
	 * @throws IOException
	 *             if the stream content is malformed or an I/O error occurs.
	...
	private void init() throws IOException {
		...
		if (magic2 != 'h') {
			throw new IOException(
					"Stream is not BZip2 formatted: expected 'h'" + " as first byte but got '" + (char) magic2 + "'");
		}
