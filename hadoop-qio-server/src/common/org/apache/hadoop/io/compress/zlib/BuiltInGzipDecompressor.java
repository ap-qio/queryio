...
public class BuiltInGzipDecompressor implements Decompressor {
	 ...
	 * extra-field bytes are simply skipped.)
	 * </p>
	 *
	 ...
	 * we copy trailer bytes (all 8 of 'em) to a local buffer.
	 * </p>
	 *
	 ...
	 * except in finished state with concatenated data.)
	 * </p>
	 *
	...
	private void copyBytesToLocal(int len) {
		...
		userBufOff += len;
		userBufLen -= len;
		if (state == GzipStateLabel.TRAILER_CRC || state == GzipStateLabel.TRAILER_SIZE) {
	...
	private long readUIntLE(byte[] b, int off) {
		return ((((long) (b[off + 3] & 0xff) << 24) | ((long) (b[off + 2] & 0xff) << 16)
				| ((long) (b[off + 1] & 0xff) << 8) | ((long) (b[off] & 0xff))) & 0xffffffffL);
	}
