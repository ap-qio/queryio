...
/**
 * A collection of factories to create the right bzip2 compressor/decompressor
 * instances.
 * 
 */
public class Bzip2Factory {
	...
	/**
	 * Check if native-bzip2 code is loaded & initialized correctly and can be
	 * loaded for this job.
	 * 
	 * @param conf
	 *            configuration
	 * @return <code>true</code> if native-bzip2 is loaded & initialized and can
	...
	public static boolean isNativeBzip2Loaded(Configuration conf) {
		...
		if (!bzip2LibraryName.equals(libname)) {
			...
			} else if (conf.getBoolean(CommonConfigurationKeys.IO_NATIVE_LIB_AVAILABLE_KEY,
					CommonConfigurationKeys.IO_NATIVE_LIB_AVAILABLE_DEFAULT) && NativeCodeLoader.isNativeCodeLoaded()) {
				try {
					...
					LOG.warn("Failed to load/initialize native-bzip2 library " + libname
	...
	public static Class<? extends Compressor> getBzip2CompressorType(Configuration conf) {
		return isNativeBzip2Loaded(conf) ? Bzip2Compressor.class : BZip2DummyCompressor.class;
	}
	...
	public static Class<? extends Decompressor> getBzip2DecompressorType(Configuration conf) {
		return isNativeBzip2Loaded(conf) ? Bzip2Decompressor.class : BZip2DummyDecompressor.class;
	}
	...
	public static int getBlockSize(Configuration conf) {
		return conf.getInt("bzip2.compress.blocksize", Bzip2Compressor.DEFAULT_BLOCK_SIZE);
	}
	...
	public static int getWorkFactor(Configuration conf) {
		return conf.getInt("bzip2.compress.workfactor", Bzip2Compressor.DEFAULT_WORK_FACTOR);
	}
