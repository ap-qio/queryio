...
public class GzipCodec extends DefaultCodec {
	...
	protected static class GzipOutputStream extends CompressorStream {
		...
		private static class ResetableGZIPOutputStream extends GZIPOutputStream {
			...
			private static final boolean HAS_BROKEN_FINISH = (IBM_JAVA && JVMVersion.contains("1.6.0"));
	...
	public Compressor createCompressor() {
		return (ZlibFactory.isNativeZlibLoaded(conf)) ? new GzipZlibCompressor(conf) : null;
	}

	@Override
	public Class<? extends Compressor> getCompressorType() {
		return ZlibFactory.isNativeZlibLoaded(conf) ? GzipZlibCompressor.class : null;
	}
	...
	public Decompressor createDecompressor() {
		return (ZlibFactory.isNativeZlibLoaded(conf)) ? new GzipZlibDecompressor() : new BuiltInGzipDecompressor();
	}

	@Override
	public Class<? extends Decompressor> getDecompressorType() {
		return ZlibFactory.isNativeZlibLoaded(conf) ? GzipZlibDecompressor.class : BuiltInGzipDecompressor.class;
	}
	...
	static final class GzipZlibCompressor extends ZlibCompressor {
		...
		public GzipZlibCompressor(Configuration conf) {
			super(ZlibFactory.getCompressionLevel(conf), ZlibFactory.getCompressionStrategy(conf),
