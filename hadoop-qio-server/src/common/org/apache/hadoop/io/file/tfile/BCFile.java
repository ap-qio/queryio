...
final class BCFile {
	...
	static public class Writer implements Closeable {
		...
		private static interface BlockRegister {
			 ...
			 * @param offsetEnd
			 *            One byte after the end of the block. Compressed block
			 *            size is offsetEnd - offsetStart.
			 */
		...
		private static final class WBlockState {
			 ...
			 *            The compression algorithm to be used to for
			 *            compression.
			 * @throws IOException
			 */
			public WBlockState(Algorithm compressionAlgo, FSDataOutputStream fsOut, BytesWritable fsOutputBuffer,
		 ...
		 * 
		 * TODO: Change DataOutputStream to something else that tracks the size
		 * as long instead of int. Currently, we will wrap around if the row
		 * block size is greater than 4GB.
		 */
		public class BlockAppender extends DataOutputStream {
			 ...
			 * 
			 * @param register
			 *            the block register, which is called when the block is
			 *            closed.
			 * @param wbs
			...
			public long getRawSize() throws IOException {
				/**
				 * Expecting the size() of a block not exceeding 4GB. Assuming
				 * the size() will wrap to negative integer if it exceeds 2GB.
				 */
			 ...
			 * 
			 * @return the number of compressed bytes written to the underlying
			 *         FS file. The size may be smaller than actual need to
			 *         compress the all data written due to internal buffering
			...
			/**
			 * Signaling the end of write to the block. The block register will
			 * be called for registering the finished block.
			 */
			@Override
		 ...
		 *            Name of the compression algorithm, which will be used for
		 *            all data blocks.
		 * @throws IOException
		 ...
		 * <code>close</code> is not allowed and may lead to undetermined
		 * results.
		 */
		@Override
		public void close() throws IOException {
			...
			try {
				if (errorCount == 0) {
					...
					// Meta Index and the trailing section are written out
					// directly.
		...
		/**
		 * Create a Meta Block and obtain an output stream for adding data into
		 * the block. There can only be one BlockAppender stream active at any
		 * time. Regular Blocks may not be created after the first Meta Blocks.
		 * The caller must call BlockAppender.close() to conclude the block
		 * creation.
		 * 
		 * @param name
		 *            The name of the Meta Block. The name must not conflict
		 *            with existing Meta Blocks.
		 * @param compressionName
		...
		/**
		 * Create a Meta Block and obtain an output stream for adding data into
		 * the block. The Meta Block will be compressed with the same
		 * compression algorithm as data blocks. There can only be one
		 * BlockAppender stream active at any time. Regular Blocks may not be
		 * created after the first Meta Blocks. The caller must call
		 * BlockAppender.close() to conclude the block creation.
		 * 
		 * @param name
		 *            The name of the Meta Block. The name must not conflict
		 *            with existing Meta Blocks.
		...
		/**
		 * Create a Data Block and obtain an output stream for adding data into
		 * the block. There can only be one BlockAppender stream active at any
		 * time. Data Blocks may not be created after the first Meta Blocks. The
		 * caller must call BlockAppender.close() to conclude the block
		 * creation.
		 * 
		 * @return The BlockAppender stream
		 * @throws IOException
		...
		/**
		 * Callback to make sure a meta block is added to the internal list when
	...
	static public class Reader implements Closeable {
		...
		static private final class RBlockState {
			...
			public RBlockState(Algorithm compressionAlgo, FSDataInputStream fsin, BlockRegion region,
				...
				try {
					this.in = compressAlgo
							.createDecompressionStream(
									new BoundedRangeFileInputStream(fsin, this.region.getOffset(),
		...
		public static class BlockReader extends DataInputStream {
			...
			public void close() throws IOException {
				...
				try {
					// Do not set rBlkState to null. People may access stats
					// after calling
					// close().
					rBlkState.finish();
				} finally {
			...
			/**
			 * Get the name of the compression algorithm used to compress the
			 * block.
			 * 
		...
		public Reader(FSDataInputStream fin, long fileLength, Configuration conf) throws IOException {
			...
			// move the cursor to the beginning of the tail, containing: offset
			// to the
		...
		/**
		 * Find the smallest Block index whose starting offset is greater than
		 * or equal to the specified offset.
		 * 
		 * @param offset
	...
	static class DataIndex {
		...
		// for data blocks, each entry specifies a block's offset, compressed
		// size
		// and raw size
	...
	static final class Magic {
		private final static byte[] AB_MAGIC_BCFILE = {
				// ... total of 16 bytes
				(byte) 0xd1, (byte) 0x11, (byte) 0xd3, (byte) 0x68, (byte) 0x91, (byte) 0xb5, (byte) 0xd7, (byte) 0xb6,
				(byte) 0x39, (byte) 0xdf, (byte) 0x41, (byte) 0x40, (byte) 0x92, (byte) 0xba, (byte) 0xe1,
				(byte) 0x50 };
