...
public class TFile {
	...
	public static class Writer implements Closeable {
		 ...
		 * @param minBlockSize
		 *            Minimum compressed block size in bytes. A compression
		 *            block will not be closed until it reaches this size except
		 *            for the last block.
		 * @param compressName
		 *            Name of the compression algorithm. Must be one of the
		 *            strings returned by
		 *            {@link TFile#getSupportedCompressionAlgorithms()}.
		 * @param comparator
		 *            Leave comparator as null or empty string if TFile is not
		 *            sorted. Otherwise, provide the string name for the
		 *            comparison algorithm for keys. Two kinds of comparators
		 *            are supported.
		 *            <ul>
		 *            <li>Algorithmic comparator: binary comparators that is
		 *            language independent. Currently, only "memcmp" is
		 *            supported.
		 *            <li>Language-specific comparator: binary comparators that
		 *            can only be constructed in specific language. For Java,
		 *            the syntax is "jclass:", followed by the class name of the
		 *            RawComparator. Currently, we only support RawComparators
		 *            that can be constructed through the default constructor
		 *            (with no parameters). Parameterized RawComparators such as
		 *            {@link WritableComparator} or
		 *            {@link JavaSerializationComparator} may not be directly
		 *            used. One should write a wrapper class that inherits from
		 *            such classes and use its default constructor to perform
		 ...
		 * @throws IOException
		 */
		public Writer(FSDataOutputStream fsdos, int minBlockSize, String compressName, String comparator,
		...
		/**
		 * Close the Writer. Resources will be released regardless of the
		 * exceptions being thrown. Future close calls will have no effect.
		 * 
		 ...
		 *             Upon IO errors.
		 *             <p>
		 *             If an exception is thrown, the TFile will be in an
		 *             inconsistent state. The only legitimate call after that
		 *             would be close
		 */
		...
		/**
		 * Helper class to register value after close call on value append
		 * stream.
		 */
		...
		/**
		 * Obtain an output stream for writing a key into TFile. This may only
		 * be called when there is no active Key appending stream or value
		 * appending stream.
		 * 
		 * @param length
		 *            The expected length of the key. If length of the key is
		 *            not known, set length = -1. Otherwise, the application
		 *            must write exactly as many bytes as specified here before
		 *            calling close on the returned output stream.
		...
		/**
		 * Obtain an output stream for writing a value into TFile. This may only
		 * be called right after a key appending operation (the key append
		 * stream must be closed).
		 * 
		 * @param length
		 *            The expected length of the value. If length of the value
		 *            is not known, set length = -1. Otherwise, the application
		 *            must write exactly as many bytes as specified here before
		 *            calling close on the returned output stream. Advertising
		 *            the value size up-front guarantees that the value is
		 *            encoded in one chunk, and avoids intermediate chunk
		 *            buffering.
		 * @throws IOException
		 * 
		 */
		 ...
		 * not be called when there is a key append stream or value append
		 * stream active. No more key-value insertion is allowed after a meta
		 * data block has been added to TFile.
		 * 
		 * @param name
		 *            Name of the meta block.
		 * @param compressName
		 *            Name of the compression algorithm to be used. Must be one
		 *            of the strings returned by
		 *            {@link TFile#getSupportedCompressionAlgorithms()}.
		 ...
		 * not be called when there is a key append stream or value append
		 * stream active. No more key-value insertion is allowed after a meta
		 * data block has been added to TFile. Data will be compressed using the
		 * default compressor as defined in Writer's constructor.
		 * 
		 * @param name
		 *            Name of the meta block.
		 * @return A DataOutputStream that can be used to write Meta Block data.
		...
		void finishDataBlock(boolean bForceFinish) throws IOException {
			...
			if (bForceFinish || blkAppender.getCompressedSize() >= sizeMinBlock) {
				...
				// for now
				TFileIndexEntry keyLast = new TFileIndexEntry(lastKeyBufferOS.getBuffer(), 0, lastKeyBufferOS.size(),
						blkRecordCount);
	...
	/**
	 * TFile Reader. Users may only read TFiles by creating
	 * TFile.Reader.Scanner. objects. A scanner may scan the whole TFile
	 * ({@link Reader#createScanner()} ) , a portion of TFile based on byte
	 * offsets ( {@link Reader#createScannerByByteRange(long, long)}), or a
	 * portion of TFile with keys fall in a certain key range (for sorted TFile
	 * only, {@link Reader#createScannerByKey(byte[], byte[])} or
	...
	public static class Reader implements Closeable {
		 ...
		 * @param fileLength
		 *            The length of TFile. This is required because we have no
		 *            easy way of knowing the actual size of the input file
		 *            through the File input stream.
		 * @param conf
		 * @throws IOException
		 */
		 ...
		 * 
		 * @return If TFile is not empty, the location of the first key-value
		 *         pair. Otherwise, it returns end().
		 */
		Location begin() {
			return begin;
		}
		...
		/**
		 * Get a Comparator object to compare Entries. It is useful when you
		 * want stores the entries in a collection (such as PriorityQueue) and
		 * perform sorting or comparison among entries based on the keys without
		 * copying out the key.
		 * 
		...
		public Comparator<Scanner.Entry> getEntryComparator() {
			...
			return new Comparator<Scanner.Entry>() {
				/**
				 * Provide a customized comparator for Entries. This is useful
				 * if we have a collection of Entry objects. However, if the
				 * Entry objects come from different TFiles, users must ensure
				 * that those TFiles share the same RawComparator.
				 */
				@Override
				public int compare(Scanner.Entry o1, Scanner.Entry o2) {
					return comparator.compare(o1.getKeyBuffer(), 0, o1.getKeyLength(), o2.getKeyBuffer(), 0,
							o2.getKeyLength());
				}
			};
		}
		 ...
		 * containing the key strictly greater than input key. if greater is
		 * false then returns the beginning location of the block greater than
		...
		/**
		 * Get the location pointing to the beginning of the first key-value
		 * pair in a compressed block whose byte offset in the TFile is greater
		 ...
		 * whose byte offset in the TFile is greater than or equal to the
		 * specified offset.
		 * 
		 * @param offset
		 *            the user supplied offset.
		...
		/**
		 * Get a sample key that is within a block whose starting offset is
		 * greater than or equal to the specified offset.
		 * 
		 * @param offset
		 *            The file offset.
		 * @return the key that fits the requirement; or null if no such key
		 *         exists (which could happen if the offset is close to the end
		 *         of the TFile).
		 * @throws IOException
		 */
		 ...
		 * 
		 * @return The scanner object. A valid Scanner is always returned even
		 *         if the TFile is empty.
		 * @throws IOException
		 */
		 ...
		 * @return The actual coverage of the returned scanner tries to match
		 *         the specified byte-region but always round up to the
		 *         compression block boundaries. It is possible that the
		 *         returned scanner contains zero key-value pairs even if length
		 *         is positive.
		 * @throws IOException
		 */
		 ...
		 * 
		 * @param beginKey
		 *            Begin key of the scan (inclusive). If null, scan from the
		 *            first key-value entry of the TFile.
		 * @param endKey
		 *            End key of the scan (exclusive). If null, scan up to the
		 *            last key-value entry of the TFile.
		 * @return The actual coverage of the returned scanner will cover all
		 *         keys greater than or equal to the beginKey and less than the
		 *         endKey.
		 * @throws IOException
		 * 
		...
		public Scanner createScannerByKey(byte[] beginKey, byte[] endKey) throws IOException {
			return createScannerByKey((beginKey == null) ? null : new ByteArray(beginKey, 0, beginKey.length),
					(endKey == null) ? null : new ByteArray(endKey, 0, endKey.length));
		}
		 ...
		 * 
		 * @deprecated Use
		 *             {@link #createScannerByKey(RawComparable, RawComparable)}
		 *             instead.
		 */
		@Deprecated
		 ...
		 * @param endRecNum
		 *            The RecordNum for the last record (exclusive). To scan the
		 *            whole file, either specify endRecNum==-1 or
		 ...
		 * creation, points to the first key-value pair in the scan range. If
		 * the scan range is empty, the cursor will point to the end of the scan
		 * range.
		 * <p>
		 * Use {@link Scanner#atEnd()} to test whether the cursor is at the end
		 * location of the scanner.
		 * <p>
		 * Use {@link Scanner#advance()} to move the cursor to the next
		 * key-value pair (or end if none exists). Use seekTo methods (
		 ...
		 * {@link Scanner#rewind()} to seek back to the beginning of the
		 * scanner. Use {@link Scanner#seekToEnd()} to seek to the end of the
		 * scanner.
		 * <p>
		...
		public static class Scanner implements Closeable {
			 ...
			 * @throws IOException
			 * 
			 *             The offsets will be rounded to the beginning of a
			 *             compressed block whose offset is greater than or
			...
			Scanner(Reader reader, Location begin, Location end) throws IOException {
				this.reader = reader;
				// ensure the TFile index is loaded throughout the life of
				// scanner.
				...
				// TODO: remember the longest key in a TFile, and use it to
				// replace
				// MAX_KEY_SIZE.
			 ...
			 * @param endKey
			 *            End key of the scan. If null, scan up to the last <K,
			 *            V> entry of the TFile.
			 * @throws IOException
			 */
			protected Scanner(Reader reader, RawComparable beginKey, RawComparable endKey) throws IOException {
				this(reader, (beginKey == null) ? reader.begin() : reader.getBlockContainsKey(beginKey, false),
						reader.end());
				if (beginKey != null) {
			...
			/**
			 * Move the cursor to the first entry whose key is greater than or
			 * equal to the input key. Synonymous to seekTo(key, 0, key.length).
			 * The entry returned by the previous entry() call will be invalid.
			 * 
			 * @param key
			 *            The input key
			...
			/**
			 * Move the cursor to the first entry whose key is greater than or
			 * equal to the input key. The entry returned by the previous
			...
			private boolean seekTo(RawComparable key, boolean beyond) throws IOException {
				...
				// check if what we are seeking is in the later part of the
				// current
				// block.
			...
			/**
			 * Move the cursor to the new location. The entry returned by the
			 * previous entry() call will be invalid.
			 * 
			 * @param l
			 *            new cursor location. It must fall between the begin
			 *            and end location of the scanner.
			 * @throws IOException
			 */
			private void seekTo(Location l) throws IOException {
				...
				if (l.getBlockIndex() != currentLocation.getBlockIndex()) {
					...
					if (valueChecked) {
						// may temporarily go beyond the last record in the
						// block (in which
			...
			/**
			 * Rewind to the first entry in the scanner. The entry returned by
			...
			/**
			 * Seek to the end of the scanner. The entry returned by the
			 * previous entry() call will be invalid.
			 * 
			 * @throws IOException
			 */
			...
			/**
			 * Move the cursor to the first entry whose key is greater than or
			 * equal to the input key. Synonymous to lowerBound(key, 0,
			 * key.length). The entry returned by the previous entry() call will
			 * be invalid.
			 * 
			 * @param key
			 *            The input key
			...
			/**
			 * Move the cursor to the first entry whose key is strictly greater
			 * than the input key. Synonymous to upperBound(key, 0, key.length).
			 * The entry returned by the previous entry() call will be invalid.
			 * 
			 * @param key
			 *            The input key
			...
			/**
			 * Move the cursor to the first entry whose key is strictly greater
			 * than the input key. The entry returned by the previous entry()
			...
			/**
			 * Move the cursor to the next key-value pair. The entry returned by
			 * the previous entry() call will be invalid.
			 * 
			 * @return true if the cursor successfully moves. False when cursor
			 *         is already at the end location and cannot be advanced.
			 * @throws IOException
			 */
			...
			/**
			 * Load a compressed block for reading. Expecting blockIndex is
			 * valid.
			 * 
			 * @throws IOException
			 */
			...
			/**
			 * Close the scanner. Release all resources. The behavior of using
			 * the scanner after calling close is not defined. The entry
			...
			/**
			 * check whether we have already successfully obtained the key. It
			 * also initializes the valueInputStream.
			 */
			...
			/**
			 * Get the RecordNum corresponding to the entry pointed by the
			 * cursor.
			 * 
			 * @return The RecordNum corresponding to the entry pointed by the
			 *         cursor.
			 * @throws IOException
			 */
			 ...
			 * @param other
			 *            user-specified key.
			 * @return negative if key at cursor is smaller than user key; 0 if
			 *         equal; and positive if key at cursor greater than user
			 *         key.
			 * @throws IOException
			 */
			...
			public class Entry implements Comparable<RawComparable> {
				...
				/**
				 * Copy the key and value in one shot into BytesWritables. This
				 * is equivalent to getKey(key); getValue(value);
				 * 
				 * @param key
				...
				/**
				 * Copy the key into BytesWritable. The input BytesWritable will
				 * be automatically resized to the actual key size.
				 * 
				 * @param key
				...
				/**
				 * Copy the value into BytesWritable. The input BytesWritable
				 * will be automatically resized to the actual value size. The
				 * implementation directly uses the buffer inside BytesWritable
				 * for storing the value. The call does not require the value
				 * length to be known.
				 * 
				 * @param value
				 * @throws IOException
				...
				/**
				 * Writing the key to the output stream. This method avoids
				 * copying key buffer from Scanner into user buffer, then
				...
				/**
				 * Writing the value to the output stream. This method avoids
				 * copying value data from Scanner into user buffer, then
				 * writing to the output stream. It does not require the value
				 * length to be known.
				 * 
				 * @param out
				 *            The output stream
				 ...
				 * 
				 * @param buf
				 *            The buffer supplied by user. The length of the
				 *            buffer must not be shorter than the key length.
				 * @return The length of the key.
				 * 
				 * @throws IOException
				 */
				 ...
				 * @param offset
				 *            The starting offset of the user buffer where we
				 *            should copy the key into. Requiring the key-length
				 *            + offset no greater than the buffer length.
				 * @return The length of the key.
				 * @throws IOException
				 */
				...
				/**
				 * Streaming access to the key. Useful for desrializing the key
				 * into user objects.
				 * 
				 * @return The input stream.
				 */
				...
				/**
				 * Get the length of the value. isValueLengthKnown() must be
				 * tested true.
				 * 
				 * @return the length of the value.
				 */
				...
				/**
				 * Copy value into user-supplied buffer. User supplied buffer
				 * must be large enough to hold the whole value. The value part
				 * of the key-value pair pointed by the current cursor is not
				 * cached and can only be examined once. Calling any of the
				 * following functions more than once without moving the cursor
				 * will result in exception: {@link #getValue(byte[])},
				...
				/**
				 * Copy value into user-supplied buffer. User supplied buffer
				 * must be large enough to hold the whole value (starting from
				 * the offset). The value part of the key-value pair pointed by
				 * the current cursor is not cached and can only be examined
				 * once. Calling any of the following functions more than once
				...
				public int getValue(byte[] buf, int offset) throws IOException {
					...
					try {
						...
						if (dis.read() >= 0) {
							// attempt to read one more byte to determine
							// whether we reached
							// the
							// end or not.
							throw new IndexOutOfBoundsException("Buffer too small to hold value");
				...
				/**
				 * Stream access to value. The value part of the key-value pair
				 * pointed by the current cursor is not cached and can only be
				 * examined once. Calling any of the following functions more
				 * than once without moving the cursor will result in exception:
				 * {@link #getValue(byte[])}, {@link #getValue(byte[], int)},
				 * {@link #getValueStream}.
				 * 
				 ...
				 * 
				 * @return true if value length is known before hand. Values
				 *         less than the chunk size will always have their
				 *         lengths known before hand. Values that are written
				 *         out as a whole (with advertised length up-front) will
				...
				/**
				 * Compare the entry key to another key. Synonymous to
				 * compareTo(key, 0, key.length).
				 * 
				 * @param buf
				 *            The key buffer.
				 * @return comparison result between the entry key with the
				 *         input key.
				 */
				...
				/**
				 * Compare the entry key to another key. Synonymous to
				 * compareTo(new ByteArray(buf, offset, length)
				 * 
				 * @param buf
				 *            The key buffer
				 * @param offset
				...
				/**
				 * Compare an entry with a RawComparable object. This is useful
				 * when Entries are stored in a collection, and we want to
				 * compare a user supplied key.
				 */
				@Override
			...
			/**
			 * Advance cursor in block until we find a key that is greater than
			 * or equal to the input key.
			 * 
			 * @param key
			 *            Key to compare.
			 * @param greater
			 *            advance until we find a key greater than the input
			 *            key.
			...
			if (comparator.equals(COMPARATOR_MEMCMP)) {
				...
				try {
					...
					throw new IllegalArgumentException(
							"Failed to instantiate comparator: " + comparator + "(" + e.toString() + ")");
				}
			} else {
	...
	static class TFileIndex {
		...
		public TFileIndex(int entryCount, DataInput in, BytesComparator comparator) throws IOException {
			...
			if (size > 0) {
				...
				for (int i = 0; i < entryCount; i++) {
					...
					TFileIndexEntry idx = new TFileIndexEntry(
		 ...
		 * @param key
		 *            input key.
		 * @return the ID of the first block that contains key >= input key. Or
	...
	/**
	 * TFile Data Index entry. We should try to make the memory footprint of
	 * each index entry as small as possible.
	 */
	...
	public static void main(String[] args) {
		System.out.printf("TFile Dumper (TFile %s, BCFile %s)%n", TFile.API_VERSION.toString(),
				BCFile.API_VERSION.toString());
		if (args.length == 0) {
