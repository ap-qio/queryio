...
final class Chunk {
	...
	static public class ChunkDecoder extends InputStream {
		 ...
		 * 
		 * @return false if the chunk encoded stream has more data to read (in
		 *         which case available() will be greater than 0); true
		 *         otherwise.
		 * @throws java.io.IOException
		 *             on I/O errors.
		 */
		...
		@Override
		/*
		 * This method never blocks the caller. Returning 0 does not mean we
	 ...
	 * chunks. Non-terminal chunks have their lengths negated. Non-terminal
	 * chunks cannot have length 0. All lengths are in the range of 0 to
	...
	static public class ChunkEncoder extends OutputStream {
		...
		/**
		 * The internal buffer that is only used when we do not know the
		 * advertised size.
		 */
		private byte buf[];
		 ...
		 * @param buf
		 *            user-supplied buffer. The buffer would be used exclusively
		 *            by the ChunkEncoder during its life cycle.
		 */
		...
		public void write(byte b[], int off, int len) throws IOException {
			if ((len + count) >= buf.length) {
				/*
				 * If the input data do not fit in buffer, flush the output
				 * buffer and then write the data directly. In this way buffered
