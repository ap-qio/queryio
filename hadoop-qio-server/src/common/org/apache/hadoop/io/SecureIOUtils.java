 ...
 * tasktracker.</li>
 * <li>The tasktracker is unaware of the symlink, and simply streams the
 * contents of the jobToken file. The malicious user can now access potentially
 * sensitive map outputs, etc. of the target user's job.</li>
 * </ol>
 * A similar attack is possible involving task log truncation, but in that case
 * due to an insecure write to a file. <br/>
 */
public class SecureIOUtils {

	/**
	 * Ensure that we are set up to run with the appropriate native support
	 * code. If security is disabled, and the support code is unavailable, this
	 * class still tries its best to be secure, but is vulnerable to some race
	 * condition attacks.
	 *
	 ...
	 * @param mode
	 *            mode in which we want to open the random access file
	 * @param expectedOwner
	 *            the expected user owner for the file
	 * @param expectedGroup
	 *            the expected group owner for the file
	 * @throws IOException
	 *             if an IO error occurred or if the user/group does not match
	 *             when security is enabled.
	 */
	public static RandomAccessFile openForRandomRead(File f, String mode, String expectedOwner, String expectedGroup)
			throws IOException {
	...
	/**
	 * Same as openForRandomRead except that it will run even if security is
	 * off. This is used by unit tests.
	 */
	@VisibleForTesting
	protected static RandomAccessFile forceSecureOpenForRandomRead(File f, String mode, String expectedOwner,
	 ...
	 *            the expected group owner for the file
	 * @throws IOException
	 *             if an IO Error occurred or the user/group does not match if
	 *             security is enabled
	 */
	public static FSDataInputStream openFSDataInputStream(File file, String expectedOwner, String expectedGroup)
			throws IOException {
		if (!UserGroupInformation.isSecurityEnabled()) {
	 ...
	 */
	@VisibleForTesting
	protected static FSDataInputStream forceSecureOpenFSDataInputStream(File file, String expectedOwner,
			String expectedGroup) throws IOException {
	 ...
	 *
	 * Note that this function provides no additional checks if Hadoop security
	 * is disabled, since doing the checks would be too expensive when native
	 * libraries are not available.
	 *
	 * @param f
	 *            the file that we are trying to open
	 * @param expectedOwner
	 *            the expected user owner for the file
	 * @param expectedGroup
	 *            the expected group owner for the file
	 * @throws IOException
	 *             if an IO Error occurred, or security is enabled and the
	...
	/**
	 * Open the specified File for write access, ensuring that it does not
	 * exist.
	 * 
	 * @param f
	...
	private static void checkStat(File f, String owner, String group, String expectedOwner, String expectedGroup)
			throws IOException {
		...
		if (!success) {
			throw new IOException("Owner '" + owner + "' for path " + f + " did not match " + "expected owner '"
					+ expectedOwner + "'");
		}
	}
