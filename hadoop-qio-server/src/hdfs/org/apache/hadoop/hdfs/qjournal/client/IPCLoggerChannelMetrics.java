...
class IPCLoggerChannelMetrics {
	 ...
	 * JournalNode may change over the lifetime of the process. However,
	 * metrics2 doesn't have a function to unregister a set of metrics and fails
	 * if a new metrics class is registered with the same name as the existing
	 * one. Hence, we have to maintain our own registry ("multiton") here, so
	 * that we have exactly one metrics instance per JournalNode, and switch out
	 * the pointer to the underlying IPCLoggerChannel instance.
	 */
	...
	private IPCLoggerChannelMetrics(IPCLoggerChannel ch) {
		...
		if (intervals != null) {
			...
			for (int i = 0; i < writeEndToEndLatencyQuantiles.length; i++) {
				...
				writeEndToEndLatencyQuantiles[i] = registry.newQuantiles("writesE2E" + interval + "s",
