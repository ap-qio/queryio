 ...
 * 
 * @param <KEY>
 *            a key used to identify each of the outgoing calls
 * @param <RESULT>
...
class QuorumCall<KEY, RESULT> {
	...
	/**
	 * Interval, in milliseconds, at which a log message will be made while
	 * waiting for a quorum call.
	 */
	private static final int WAIT_PROGRESS_INTERVAL_MILLIS = 1000;

	/**
	 * Start logging messages at INFO level periodically after waiting for this
	 * fraction of the configured timeout for any call.
	 */
	private static final float WAIT_PROGRESS_INFO_THRESHOLD = 0.3f;
	/**
	 * Start logging messages at WARN level after waiting for this fraction of
	 * the configured timeout for any call.
	 */
	 ...
	 * 
	 * Note that, even after this returns, more responses may arrive, causing
	 * the return value of other methods in this class to change.
	 *
	 * @param minResponses
	 *            return as soon as this many responses have been received,
	 *            regardless of whether they are successes or exceptions
	 * @param minSuccesses
	 *            return as soon as this many successful (non-exception)
	 *            responses have been received
	 * @param maxExceptions
	 *            return as soon as this many exception responses have been
	 *            received. Pass 0 to return immediately if any exception is
	 *            received.
	 * @param millis
	 ...
	 *             if the specified timeout elapses before achieving the desired
	 *             conditions
	 */
	...
	/**
	 * Check if any of the responses came back with an AssertionError. If so, it
	 * re-throws it, even if there was a quorum of responses. This code only
	 * runs if assertions are enabled for this class, otherwise it should JIT
	 * itself away.
	 * 
	 * This is done since AssertionError indicates programmer confusion rather
	 * than some kind of expected issue, and thus in the context of test cases
	 * we'd like to actually fail the test case instead of continuing through.
	 */
	 ...
	 *         regardless of whether it threw an exception or returned a
	 *         successful result.
	 */
	 ...
	 *         map will not be further mutated, even if further results arrive
	 *         for the quorum.
	 */
	...
	/**
	 * Return a string suitable for displaying to the user, containing any
	 * exceptions that have been received so far.
	 */
