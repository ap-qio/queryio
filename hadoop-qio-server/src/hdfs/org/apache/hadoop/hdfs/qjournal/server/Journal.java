...
/**
 * A JournalNode can manage journals for several clusters at once. Each such
 * journal is entirely independent despite being hosted by the same JVM.
 */
public class Journal implements Closeable {
	...
	/**
	 * When a new writer comes along, it asks each node to promise to ignore
	 * requests from any previous writer, as identified by epoch number. In
	 * order to make such a promise, the epoch number of that writer is stored
	 * persistently on disk.
	 */
	private PersistentLongFile lastPromisedEpoch;

	/**
	 * Each IPC that comes from a given client contains a serial number which
	 * only increases from the client's perspective. Whenever we switch epochs,
	 * we reset this back to -1. Whenever an IPC comes from a client, we ensure
	 * that it is strictly higher than any previous IPC. This guards against any
	 * bugs in the IPC layer that would re-order IPCs or cause a stale retry
	 ...
	 * of a segment. See the the 'testNewerVersionOfSegmentWins' test case.
	 */
	private PersistentLongFile lastWriterEpoch;

	/**
	 * Lower-bound on the last committed transaction ID. This is not depended
	 * upon for correctness, but acts as a sanity check during the recovery
	 * procedures, and as a visibility mark for clients reading in-progress
	 * logs.
	 */
	 ...
	 * first load the Journal, but also after any formatting operation, since
	 * the cached data is no longer relevant.
	 */
	...
	/**
	 * Scan the local storage directory, and return the segment containing the
	 * highest transaction.
	 * 
	 * @return the EditLogFile with the highest transactions, or null if no
	 *         files exist.
	 */
	...
	/**
	 * @return the last epoch which this node has promised not to accept any
	 *         lower epoch, or 0 if no promises have been made.
	 */
	 ...
	 * @throws IOException
	 *             if the node has already made a promise to another writer with
	 *             a higher epoch number, if the namespace is inconsistent, or
	...
	synchronized void journal(RequestInfo reqInfo, long segmentTxId, long firstTxnId, int numTxns, byte[] records)
		...
		if (curSegmentTxId != segmentTxId) {
			...
			JournalOutOfSyncException e = new JournalOutOfSyncException(
					"Writer out of sync: it thinks it is writing segment " + segmentTxId + " but current segment is "
							+ curSegmentTxId);
			abortCurSegment();
			throw e;
		}
		...
		if (isLagging) {
			// This batch of edits has already been committed on a quorum of
			// other
	...
	private synchronized void checkWriteRequest(RequestInfo reqInfo) throws IOException {
		...
		if (reqInfo.getEpoch() != lastWriterEpoch.get()) {
			throw new IOException(
					"IPC's epoch " + reqInfo.getEpoch() + " is not the current writer epoch  " + lastWriterEpoch.get());
		}
	}
	 ...
	 *             if the given expression is not true. The message of the
	 *             exception is formatted using the 'msg' and 'formatArgs'
	 *             parameters.
	 */
	 ...
	 *             parameters.
	 * 
	 *             This should be used in preference to Java's built-in assert
	 *             in non-performance-critical paths, where a failure of this
	 *             invariant might cause the protocol to lose data.
	 */
	...
	public synchronized void startLogSegment(RequestInfo reqInfo, long txid, int layoutVersion) throws IOException {
		...
		if (curSegment != null) {
			LOG.warn("Client is requesting a new log segment " + txid + " though we are already writing " + curSegment
		...
		// Paranoid sanity check: we should never overwrite a finalized log
		// file.
		// Additionally, if it's in-progress, it should have at most 1
		// transaction.
		// This can happen if the writer crashes exactly at the start of a
		// segment.
	...
	/**
	 * Remove the previously-recorded 'accepted recovery' information for a
	 * given log segment, once it is no longer necessary.
	 * 
	 * @param segmentTxId
	 *            the transaction ID to purge
	 * @throws IOException
	...
	SegmentStateProto getSegmentInfo(long segmentTxId) throws IOException {
		...
		SegmentStateProto ret = SegmentStateProto.newBuilder().setStartTxId(segmentTxId).setEndTxId(elf.getLastTxId())
				.setIsInProgress(elf.isInProgress()).build();
	...
	/**
	 * @see QJournalProtocol#acceptRecovery(RequestInfo,
	 *      QJournalProtocolProtos.SegmentStateProto, URL)
	 */
	public synchronized void acceptRecovery(RequestInfo reqInfo, SegmentStateProto segment, URL fromUrl)
		...
		if (currentSegment == null || currentSegment.getEndTxId() != segment.getEndTxId()) {
			if (currentSegment == null) {
				...
				// we
				// currently have, we should not end up discarding any
				// transactions
	...
	synchronized void discardSegments(long startTxId) throws IOException {
		...
		// we delete all the segments after the startTxId. let's reset
		// committedTxnId
	...
	public synchronized void doUpgrade(StorageInfo sInfo) throws IOException {
		...
		PersistentLongFile prevLastPromisedEpoch = new PersistentLongFile(new File(previousDir, LAST_PROMISED_FILENAME),
				0);
		PersistentLongFile prevLastWriterEpoch = new PersistentLongFile(new File(previousDir, LAST_WRITER_EPOCH), 0);
		BestEffortLongFile prevCommittedTxnId = new BestEffortLongFile(new File(previousDir, COMMITTED_TXID_FILENAME),
				HdfsConstants.INVALID_TXID);
