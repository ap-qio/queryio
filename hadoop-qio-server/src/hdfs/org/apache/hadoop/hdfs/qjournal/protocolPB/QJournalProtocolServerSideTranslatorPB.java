 ...
 * {@link JournalProtocolPB} to the {@link JournalProtocol} server
 * implementation.
 */
...
public class QJournalProtocolServerSideTranslatorPB implements QJournalProtocolPB {
	...
	private final static StartLogSegmentResponseProto VOID_START_LOG_SEGMENT_RESPONSE = StartLogSegmentResponseProto
			.newBuilder().build();
	...
	@Override
	public IsFormattedResponseProto isFormatted(RpcController controller, IsFormattedRequestProto request)
	...
	@Override
	public GetJournalStateResponseProto getJournalState(RpcController controller, GetJournalStateRequestProto request)
			throws ServiceException {
		try {
	...
	public JournalResponseProto journal(RpcController unused, JournalRequestProto req) throws ServiceException {
		try {
			impl.journal(convert(req.getReqInfo()), req.getSegmentTxnId(), req.getFirstTxnId(), req.getNumTxns(),
	...
	@Override
	public StartLogSegmentResponseProto startLogSegment(RpcController controller, StartLogSegmentRequestProto req)
			throws ServiceException {
		try {
	...
	@Override
	public FinalizeLogSegmentResponseProto finalizeLogSegment(RpcController controller,
			FinalizeLogSegmentRequestProto req) throws ServiceException {
		try {
	...
	@Override
	public GetEditLogManifestResponseProto getEditLogManifest(RpcController controller,
	...
	@Override
	public PrepareRecoveryResponseProto prepareRecovery(RpcController controller, PrepareRecoveryRequestProto request)
			throws ServiceException {
		try {
	...
	@Override
	public AcceptRecoveryResponseProto acceptRecovery(RpcController controller, AcceptRecoveryRequestProto request)
			throws ServiceException {
		try {
			impl.acceptRecovery(convert(request.getReqInfo()), request.getStateToAccept(),
	...
	private RequestInfo convert(QJournalProtocolProtos.RequestInfoProto reqInfo) {
		return new RequestInfo(reqInfo.getJournalId().getIdentifier(), reqInfo.getEpoch(), reqInfo.getIpcSerialNumber(),
				reqInfo.hasCommittedTxId() ? reqInfo.getCommittedTxId() : HdfsConstants.INVALID_TXID);
	}

	@Override
	public DiscardSegmentsResponseProto discardSegments(RpcController controller, DiscardSegmentsRequestProto request)
			throws ServiceException {
		try {
	...
	@Override
	public DoPreUpgradeResponseProto doPreUpgrade(RpcController controller, DoPreUpgradeRequestProto request)
			throws ServiceException {
		try {
	...
	@Override
	public DoFinalizeResponseProto doFinalize(RpcController controller, DoFinalizeRequestProto request)
			throws ServiceException {
		try {
	...
	@Override
	public CanRollBackResponseProto canRollBack(RpcController controller, CanRollBackRequestProto request)
			throws ServiceException {
		try {
	...
	@Override
	public GetJournalCTimeResponseProto getJournalCTime(RpcController controller, GetJournalCTimeRequestProto request)
			throws ServiceException {
		try {
