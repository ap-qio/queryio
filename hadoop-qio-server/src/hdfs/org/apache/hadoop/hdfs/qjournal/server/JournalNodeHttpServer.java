...
public class JournalNodeHttpServer {
	...
	void start() throws IOException {
		...
		final String httpsAddrString = conf.get(DFSConfigKeys.DFS_JOURNALNODE_HTTPS_ADDRESS_KEY,
				DFSConfigKeys.DFS_JOURNALNODE_HTTPS_ADDRESS_DEFAULT);
		InetSocketAddress httpsAddr = NetUtils.createSocketAddr(httpsAddrString);

		HttpServer2.Builder builder = DFSUtil.httpServerTemplateForNNAndJN(conf, httpAddr, httpsAddr, "journal",
	...
	URI getServerURI() {
		...
		// matches the behavior that the first connector is a HTTPS connector
		// only
	...
	private static InetSocketAddress getAddress(Configuration conf) {
		...
		return NetUtils.createSocketAddr(addr, DFSConfigKeys.DFS_JOURNALNODE_HTTP_PORT_DEFAULT,
