...
public class DFSUtil {
	 ...
	 * states. Decommissioned/stale nodes are moved to the end of the array on
	 * sorting with this comparator.
	 */
	@InterfaceAudience.Private
	public static class DecomStaleComparator implements Comparator<DatanodeInfo> {
		 ...
		 * 
		 * @param interval
		 *            The time interval for marking datanodes as stale is passed
		 *            from outside, since the interval may be changed
		 *            dynamically
		 */
	...
	public static boolean isValidName(String src) {
		...
		for (int i = 0; i < components.length; i++) {
			...
			if (element.equals("..")) {
				if (components.length > 4 && components[1].equals(FSDirectory.DOT_RESERVED_STRING)
	 ...
	 * e.g. "/.reserved" is reserved, while "/hadoop/.reserved" is not.
	 * 
	...
	/**
	 * Namenode HighAvailability related configuration. Returns collection of
	 * namenode Ids from the configuration. One logical id for each namenode in
	 * the in the HA setup.
	 * 
	 * @param conf
	 *            configuration
	 * @param nsId
	 *            the nameservice ID to look at, or null for non-federated
	...
	/**
	 * Given a list of keys in the order of preference, returns a value for the
	 * key in the given order from the configuration.
	 * 
	 * @param defaultValue
	 *            default value to return, when key was not found
	 * @param keySuffix
	 *            suffix to add to the key, if it is not null
	 * @param conf
	 *            Configuration
	 * @param keys
	 *            list of keys in the order of preference
	 ...
	 * @param keys
	 *            Set of keys to look for in the order of preference
	 * @return a map(nameserviceId to map(namenodeId to InetSocketAddress))
	 */
	private static Map<String, Map<String, InetSocketAddress>> getAddresses(Configuration conf, String defaultAddress,
			String... keys) {
	 ...
	 *            default address to return in case key is not found.
	 * @param keys
	 *            Set of keys to look for in the order of preference @return a
	 *            map(nameserviceId to map(namenodeId to InetSocketAddress))
	 */
	private static Map<String, Map<String, InetSocketAddress>> getAddressesForNsIds(Configuration conf,
	...
	/**
	 * Get all of the RPC addresses of the individual NNs in a given
	 * nameservice.
	 * 
	 * @param conf
	 *            Configuration
	 * @param nsId
	 *            the nameservice whose NNs addresses we want.
	 * @param defaultValue
	 *            default address to return in case key is not found.
	 * @return A map from nnId -> RPC address of each NN in the nameservice.
	 */
	public static Map<String, InetSocketAddress> getRpcAddressesForNameserviceId(Configuration conf, String nsId,
			String defaultValue) {
		return getAddressesForNameserviceId(conf, nsId, defaultValue, DFS_NAMENODE_RPC_ADDRESS_KEY);
	}

	private static Map<String, InetSocketAddress> getAddressesForNameserviceId(Configuration conf, String nsId,
		...
		for (String nnId : emptyAsSingletonNull(nnIds)) {
			...
			if (address != null) {
				...
				if (isa.isUnresolved()) {
					LOG.warn("Namenode for " + nsId + " remains unresolved for ID " + nnId
	 ...
	 *             on error
	 */
	public static Map<String, Map<String, InetSocketAddress>> getBackupNodeAddresses(Configuration conf)
			throws IOException {
		Map<String, Map<String, InetSocketAddress>> addressList = getAddresses(conf, null,
				DFS_NAMENODE_BACKUP_ADDRESS_KEY);
		if (addressList.isEmpty()) {
			throw new IOException("Incorrect configuration: backup node address " + DFS_NAMENODE_BACKUP_ADDRESS_KEY
					+ " is not configured.");
		}
		return addressList;
	}
	...
	/**
	 * Returns list of InetSocketAddresses of corresponding to secondary
	 * namenode http addresses from the configuration.
	 * 
	 * @param conf
	 *            configuration
	 * @return list of InetSocketAddresses
	 ...
	 *             on error
	 */
	public static Map<String, Map<String, InetSocketAddress>> getSecondaryNameNodeAddresses(Configuration conf)
			throws IOException {
		Map<String, Map<String, InetSocketAddress>> addressList = getAddresses(conf, null,
	 ...
	 * service ports), if found. If not, regular RPC address configured for
	 * other clients is returned.
	 * 
	 * @param conf
	 *            configuration
	 ...
	 *             on error
	 */
	public static Map<String, Map<String, InetSocketAddress>> getNNServiceRpcAddresses(Configuration conf)
			throws IOException {
		...
		Map<String, Map<String, InetSocketAddress>> addressList = getAddresses(conf, defaultAddress,
				DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY, DFS_NAMENODE_RPC_ADDRESS_KEY);
		if (addressList.isEmpty()) {
			throw new IOException("Incorrect configuration: namenode address " + DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY
					+ " or " + DFS_NAMENODE_RPC_ADDRESS_KEY + " is not configured.");
		}
		return addressList;
	}

	/**
	 * Returns list of InetSocketAddresses corresponding to the namenode that
	 * manages this cluster. Note this is to be used by datanodes to get the
	 * list of namenode addresses to talk to.
	 *
	 * Returns namenode address specifically configured for datanodes (using
	 * service ports), if found. If not, regular RPC address configured for
	 * other clients is returned.
	 ...
	 *             on error
	 */
	public static Map<String, Map<String, InetSocketAddress>> getNNServiceRpcAddressesForCluster(Configuration conf)
			throws IOException {
		// Use default address as fall back
		String defaultAddress;
		try {
		...
		Collection<String> parentNameServices = conf
				.getTrimmedStringCollection(DFSConfigKeys.DFS_INTERNAL_NAMESERVICES_KEY);

		if (parentNameServices.isEmpty()) {
			parentNameServices = conf.getTrimmedStringCollection(DFSConfigKeys.DFS_NAMESERVICES);
		} else {
			// Ensure that the internal service is ineed in the list of all
			// available
			// nameservices.
			Set<String> availableNameServices = Sets
					.newHashSet(conf.getTrimmedStringCollection(DFSConfigKeys.DFS_NAMESERVICES));
		...
		Map<String, Map<String, InetSocketAddress>> addressList = getAddressesForNsIds(conf, parentNameServices,
				defaultAddress, DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY, DFS_NAMENODE_RPC_ADDRESS_KEY);
		if (addressList.isEmpty()) {
			throw new IOException("Incorrect configuration: namenode address " + DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY
					+ " or " + DFS_NAMENODE_RPC_ADDRESS_KEY + " is not configured.");
		}
	...
	/**
	 * Flatten the given map, as returned by other functions in this class, into
	 * a flat list of {@link ConfiguredNNAddress} instances.
	 */
	...
	/**
	 * Format the given map, as returned by other functions in this class, into
	 * a string suitable for debugging display. The format of this string should
	 * not be considered an interface, and is liable to change.
	 */
	...
	public static Collection<URI> getNsServiceRpcUris(Configuration conf) {
		return getNameServiceUris(conf, DFSConfigKeys.DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY,
	...
	public static Collection<URI> getNameServiceUris(Configuration conf, String... keys) {
		...
		// We're passed multiple possible configuration keys for any given NN or
		// HA
		// nameservice, and search the config in order of these keys. In order
		// to
		// make sure that a later config lookup (e.g. fs.defaultFS) doesn't add
		// a
		// URI for a config key for which we've already found a preferred entry,
		// we
		...
		if (defaultUri != null && HdfsConstants.HDFS_URI_SCHEME.equals(defaultUri.getScheme())
	 ...
	 * corresponding to the key with matching address, by doing a reverse lookup
	 * on the list of nameservices until it finds a match.
	 * 
	 ...
	 *            Configured addresses are typically given as URIs, but we may
	 *            have to compare against a URI typed in by a human, or the
	 *            server name may be aliased, so we compare unambiguous
	 *            InetSocketAddresses instead of just comparing URI substrings.
	 * @param keys
	 *            - list of configured communication parameters that should be
	 *            checked for matches. For example, to compare against RPC
	 *            addresses, provide the list
	 ...
	 */
	public static String getNameServiceIdFromAddress(final Configuration conf, final InetSocketAddress address,
			String... keys) {
	...
	/**
	 * return server http or https address from the configuration for a given
	 ...
	 * @throws IOException
	 */
	public static URI getInfoServerWithDefaultHost(String defaultHost, Configuration conf, final String scheme)
			throws IOException {
	 ...
	 * @param defaultHost
	 *            the host to substitute with, if configuredAddress is a
	 *            local/wildcard address.
	 ...
	 * @param nameserviceId
	 *            nameservice Id to construct the node specific key. Pass null
	 *            if federation is not configuration.
	 * @param nnId
	...
	public static ClientDatanodeProtocol createClientDatanodeProtocolProxy(DatanodeID datanodeid, Configuration conf,
			int socketTimeout, boolean connectToDnViaHostname, LocatedBlock locatedBlock) throws IOException {
	...
	public static ClientDatanodeProtocol createClientDatanodeProtocolProxy(DatanodeID datanodeid, Configuration conf,
			int socketTimeout, boolean connectToDnViaHostname) throws IOException {
		return new ClientDatanodeProtocolTranslatorPB(datanodeid, conf, socketTimeout, connectToDnViaHostname);
	}

	/** Create a {@link ClientDatanodeProtocol} proxy */
	public static ClientDatanodeProtocol createClientDatanodeProtocolProxy(InetSocketAddress addr,
	...
	/**
	 * Get the nameservice Id by matching the {@code addressKey} with the the
	 ...
	 * determines the nameservice Id by matching the local node's address with
	 * the configured addresses. When a match is found, it returns the
	 ...
	 *            Configuration
	 * @param addressKey
	 *            configuration key corresponding to the address.
	 * @param knownNsId
	 *            only look at configs for the given nameservice, if not-null
	 * @param knownNNId
	 *            only look at configs for the given namenode, if not null
	 * @param matcher
	 *            matching criteria for matching the address
	 * @return Array with nameservice Id and namenode Id on success. First
	 *         element in the array is nameservice Id and second element is
	 *         namenode Id. Null value indicates that the configuration does not
	 *         have the the Id.
	 * @throws HadoopIllegalArgumentException
	 *             on error
	 */
	static String[] getSuffixIDs(final Configuration conf, final String addressKey, String knownNsId, String knownNNId,
		...
		if (found > 1) { // Only one address must match the local address
			String msg = "Configuration has multiple addresses that match "
					+ "local node's address. Please configure the system with " + DFS_NAMESERVICE_ID + " and "
	...
	/**
	 * For given set of {@code keys} adds nameservice Id and or namenode Id and
	 * returns {nameserviceId, namenodeId} when address match is found.
	 * 
	 ...
	 * {@link org.apache.hadoop.ipc.RPC.Server}
	 * 
	 * @param conf
	 *            configuration
	 * @param protocol
	 ...
	 *            Configuration
	 * @param nsId
	 *            which nameservice nnId is a part of, optional
	 * @param nnId
	 *            the namenode ID to get the service addr for
	...
	/**
	 * If the configuration refers to only a single nameservice, return the name
	 * of that nameservice. If it refers to 0 or more than 1, return null.
	 */
	 ...
	 *            whether to print the generic command usage defined in
	 *            ToolRunner
	...
	public static float getInvalidateWorkPctPerIteration(Configuration conf) {
		...
		Preconditions.checkArgument((blocksInvalidateWorkPct > 0 && blocksInvalidateWorkPct <= 1.0f),
				DFSConfigKeys.DFS_NAMENODE_INVALIDATE_WORK_PCT_PER_ITERATION + " = '" + blocksInvalidateWorkPct
	...
	public static int getReplWorkMultiplier(Configuration conf) {
		int blocksReplWorkMultiplier = conf.getInt(DFSConfigKeys.DFS_NAMENODE_REPLICATION_WORK_MULTIPLIER_PER_ITERATION,
		...
		Preconditions.checkArgument((blocksReplWorkMultiplier > 0),
				DFSConfigKeys.DFS_NAMENODE_REPLICATION_WORK_MULTIPLIER_PER_ITERATION + " = '" + blocksReplWorkMultiplier
						+ "' is invalid. " + "It should be a positive, non-zero integer value.");
	 ...
	 *            Configuration
	 * @param defaultKey
	 *            default key to be used for config lookup
	 * @return DFS_WEB_AUTHENTICATION_KERBEROS_KEYTAB_KEY if the key is not
	 *         empty else return defaultKey
	 */
	 ...
	 * <li>This configuration is overridden by dfs.https.enable configuration,
	 * if it is set to true. In that case, both http and https endpoints are
	 * stared.</li>
	...
	public static HttpConfig.Policy getHttpPolicy(Configuration conf) {
		...
		if (policyStr == null) {
			...
			boolean hadoopSsl = conf.getBoolean(CommonConfigurationKeys.HADOOP_SSL_ENABLED_KEY,
					CommonConfigurationKeys.HADOOP_SSL_ENABLED_DEFAULT);

			if (hadoopSsl) {
				LOG.warn(CommonConfigurationKeys.HADOOP_SSL_ENABLED_KEY + " is deprecated. Please use "
						+ DFSConfigKeys.DFS_HTTP_POLICY_KEY + ".");
			}
			if (https) {
				LOG.warn(DFSConfigKeys.DFS_HTTPS_ENABLE_KEY + " is deprecated. Please use "
						+ DFSConfigKeys.DFS_HTTP_POLICY_KEY + ".");
			}
		...
		if (policy == null) {
			throw new HadoopIllegalArgumentException(
					"Unregonized value '" + policyStr + "' for " + DFSConfigKeys.DFS_HTTP_POLICY_KEY);
		}
	...
	public static HttpServer2.Builder loadSslConfToHttpServerBuilder(HttpServer2.Builder builder,
		...
		return builder
				.needsClientAuth(sslConf.getBoolean(DFS_CLIENT_HTTPS_NEED_AUTH_KEY, DFS_CLIENT_HTTPS_NEED_AUTH_DEFAULT))
	 ...
	 *
	 */
	public static HttpServer2.Builder httpServerTemplateForNNAndJN(Configuration conf, final InetSocketAddress httpAddr,
			final InetSocketAddress httpsAddr, String name, String spnegoUserNameKey, String spnegoKeytabFileKey)
			throws IOException {
	 ...
	 * passwords from the CredentialProvider API before falling back to clear
	 * text in config - if falling back is allowed.
	 * 
	 * @param conf
	 *            Configuration instance
	 * @param alias
	 *            name of the credential to retreive
	...
	public static long parseRelativeTime(String relTime) throws IOException {
		...
		try {
			...
			throw new IOException(
					"Unable to parse relative time value of " + relTime + ": " + ttlString + " is not a number");
		}
	 ...
	 * 
	 * @param objects
	 *            the collection of objects to check for equality.
	 */
	 ...
	 * @throws IOException
	 *             if the KeyProvider is improperly specified in the
	 *             Configuration
	 */
	public static KeyProvider createKeyProvider(final Configuration conf) throws IOException {
		...
		if (keyProvider == null) {
			throw new IOException("Could not instantiate KeyProvider from "
					+ DFSConfigKeys.DFS_ENCRYPTION_KEY_PROVIDER_URI + " setting of '" + providerUriStr + "'");
		}
		if (keyProvider.isTransient()) {
			throw new IOException(
					"KeyProvider " + keyProvider.toString() + " was found but it is a transient provider.");
		}
		return keyProvider;
	}

	/**
	 * Creates a new KeyProviderCryptoExtension by wrapping the KeyProvider
	 ...
	 *             Configuration
	 */
	public static KeyProviderCryptoExtension createKeyProviderCryptoExtension(final Configuration conf)
			throws IOException {
	...
	/**
	 * Probe for HDFS Encryption being enabled; this uses the value of the
	 * option {@link DFSConfigKeys.DFS_ENCRYPTION_KEY_PROVIDER_URI}, returning
	 * true if that property contains a non-empty, non-whitespace string.
	 * 
	 * @param conf
