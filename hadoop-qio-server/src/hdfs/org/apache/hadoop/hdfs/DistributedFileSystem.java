...
public class DistributedFileSystem extends FileSystem {
	...
	/**
	 * Checks that the passed URI belongs to this filesystem and returns just
	 * the path component. Expects a URI with an absolute path.
	 * 
	 * @param file
	...
	/**
	 * Used to query storage location information for a list of blocks. This
	 * list of blocks is normally constructed via a series of calls to
	 ...
	 * resides. The VolumeId associated with a replica may be null because
	 * volume information can be unavailable if the corresponding datanode is
	 * down or if the requested block is not found.
	 * 
	 * This API is unstable, and datanode-side support is disabled by default.
	 * It can be enabled by setting "dfs.datanode.hdfs-blocks-metadata.enabled"
	 * to true.
	 * 
	 * @param blocks
	 *            List of target BlockLocations to query volume location
	 *            information
	 * @return volumeBlockLocations Augmented array of
	 *         {@link BlockStorageLocation}s containing additional volume
	 *         location information for each replica of each block.
	 */
	@InterfaceStability.Unstable
	public BlockStorageLocation[] getFileBlockStorageLocations(List<BlockLocation> blocks)
		...
		return new FileSystemLinkResolver<Boolean>() {
			...
			public Boolean next(final FileSystem fs, final Path p) throws IOException {
				...
				throw new UnsupportedOperationException(
						"Cannot recoverLease through" + " a symlink to a non-DistributedFileSystem: " + f + " -> " + p);
			}
		}.resolve(this, absF);
	}
	...
	/**
	 * Same as
	 * {@link #create(Path, FsPermission, boolean, int, short, long, Progressable)}
	 ...
	 * HDFS. Hence it may be honored at the creation time only. And with favored
	 * nodes, blocks will be pinned on the datanodes to prevent balancing move
	 * the block. HDFS could move the blocks during replication, to move the
	 * blocks from favored nodes. A value of null means no favored nodes for
	 * this create
	 */
	...
	@Override
	protected HdfsDataOutputStream primitiveCreate(Path f, FsPermission absolutePermission, EnumSet<CreateFlag> flag,
			int bufferSize, short replication, long blockSize, Progressable progress, ChecksumOpt checksumOpt)
			throws IOException {
		statistics.incrementWriteOps(1);
		final DFSOutputStream dfsos = dfs.primitiveCreate(getPathName(fixRelativePart(f)), absolutePermission, flag,
				true, replication, blockSize, progress, bufferSize, checksumOpt);
		return dfs.createWrappedOutputStream(dfsos, statistics);
	}
	...
	/**
	 * Move blocks from srcs to trg and delete srcs afterwards. The file block
	 ...
	 * @param type
	 *            storage type of the specific storage type quota to be
	 *            modified.
	 * @param quota
	 *            value of the specific storage type quota to be modified. Maybe
	 *            {@link HdfsConstants#QUOTA_RESET} to clear quota by storage
	 *            type.
	 */
	public void setQuotaByStorageType(Path src, final StorageType type, final long quota) throws IOException {
		...
		new FileSystemLinkResolver<Void>() {
			...
			@Override
			public Void next(final FileSystem fs, final Path p) throws IOException {
				// setQuotaByStorageType is not defined in FileSystem, so we
				// only can resolve
				// within this DFS
				return doCall(p);
			}
	...
	protected RemoteIterator<LocatedFileStatus> listLocatedStatus(final Path p, final PathFilter filter)
		...
		return new FileSystemLinkResolver<RemoteIterator<LocatedFileStatus>>() {
			...
			public RemoteIterator<LocatedFileStatus> next(final FileSystem fs, final Path p) throws IOException {
				...
				// symlink resolution for this methos does not work cross file
				// systems
				// because it is a protected method.
				throw new IOException(
						"Link resolution does not work with multiple " + "file systems for listLocatedStatus(): " + p);
			}
		}.resolve(this, absF);
	}
	 ...
	 * consuming the entries. This reduces memory consumption during listing of
	 * a large directory.
	 *
	 * @param p
	 *            target path
	...
	/**
	 * This class defines an iterator that returns the file status of each
	 * file/subdirectory of a directory
	 * 
	 * if needLocation, status contains block location if it is a file throws a
	 * RuntimeException with the error as its cause.
	 * 
	 * @param <T>
	 ...
	 *
	 * See {@link FsPermission#applyUMask(FsPermission)} for details of how the
	 * permission is applied.
	 *
	 * @param f
	 *            The path to create
	 * @param permission
	 *            The permission. See FsPermission#applyUMask for details about
	 *            how this is used to calculate the effective permission.
	 */
	...
	/**
	 * Rolls the edit log on the active NameNode. Requires super-user
	 * privileges.
	 * 
	 ...
	 * 
	 * @see org.apache.hadoop.hdfs.protocol.ClientProtocol#restoreFailedStorage(String
	 *      arg)
	 */
	 ...
	 * bandwidth value for dfs.balance.bandwidthPerSec. The bandwidth parameter
	 * is the max number of bytes per second of network bandwidth to be used by
	 * a datanode during balancing.
	 *
	 * @param bandwidth
	...
	/**
	 * Utility function that returns if the NameNode is in safemode or not. In
	 * HA mode, this API will return only ActiveNN's safemode status.
	 * 
			...
			public Void next(final FileSystem fs, final Path p) throws IOException {
				if (fs instanceof DistributedFileSystem) {
					...
					throw new UnsupportedOperationException("Cannot perform snapshot"
							+ " operations on a symlink to a non-DistributedFileSystem: " + path + " -> " + p);
				}
				return null;
			}
		}.resolve(this, absF);
		...
		new FileSystemLinkResolver<Void>() {
			...
			public Void next(final FileSystem fs, final Path p) throws IOException {
				if (fs instanceof DistributedFileSystem) {
					...
					throw new UnsupportedOperationException("Cannot perform snapshot"
							+ " operations on a symlink to a non-DistributedFileSystem: " + snapshotDir + " -> " + p);
				}
				return null;
			}
		}.resolve(this, absF);
	 ...
	 */
	public SnapshotDiffReport getSnapshotDiffReport(final Path snapshotDir, final String fromSnapshot,
			...
			public Boolean next(final FileSystem fs, final Path p) throws IOException {
				if (fs instanceof DistributedFileSystem) {
					...
					throw new UnsupportedOperationException("Cannot call isFileClosed"
							+ " on a symlink to a non-DistributedFileSystem: " + src + " -> " + p);
				}
			}
		}.resolve(this, absF);
	}
	...
	public void modifyCacheDirective(CacheDirectiveInfo info, EnumSet<CacheFlag> flags) throws IOException {
		if (info.getPath() != null) {
			info = new CacheDirectiveInfo.Builder(info).setPath(new Path(getPathName(fixRelativePart(info.getPath())))
	...
	public RemoteIterator<CacheDirectiveEntry> listCacheDirectives(CacheDirectiveInfo filter) throws IOException {
		...
		return new RemoteIterator<CacheDirectiveEntry>() {
			...
			public CacheDirectiveEntry next() throws IOException {
				// Although the paths we get back from the NameNode should
				// always be
				// absolute, we call makeQualified to add the scheme and
				// authority of
