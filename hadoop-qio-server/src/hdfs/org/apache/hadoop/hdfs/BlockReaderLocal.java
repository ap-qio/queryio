...
class BlockReaderLocal implements BlockReader {
	 ...
	 *
	 * This is used to allocate dataBuf and checksumBuf, in the event that we
	 * need them.
	 */
	 ...
	 * The reason is because we need to do a certain amount of buffering in
	 * order to do checksumming.
	 * 
	 ...
	 * The rationale is that allocating a lot of buffers of different sizes
	 * would make it very difficult for the DirectBufferPool to re-use buffers.
	 */
	private final int maxReadaheadLength;

	/**
	 * Buffers data starting at the current dataPos and extending on for
	 * dataBuf.limit().
	 *
	...
	private synchronized void freeDataBufIfExists() {
		if (dataBuf != null) {
			// When disposing of a dataBuf, we have to move our stored file
			// index
			// backwards.
	 ...
	 *
	 * @param buf
	 *            The buffer to read into. May be dataBuf. The position and
	 *            limit of this buffer should be set to multiples of the
	 *            checksum size.
	 * @param canSkipChecksum
					...
					while (checksumBuf.hasRemaining()) {
						...
						if (nRead < 0) {
							throw new IOException(
									"Got unexpected checksum file EOF at " + checksumPos + ", block file position "
	...
	private boolean createNoChecksumContext() {
		if (verifyChecksum) {
			if (storageType != null && storageType.isTransient()) {
				// Checksums are not stored for replicas on transient storage.
				// We do not
	...
	public synchronized int read(ByteBuffer buf) throws IOException {
		...
		try {
			...
			if (LOG.isTraceEnabled()) {
				traceString = new StringBuilder().append("read(").append("buf.remaining=").append(buf.remaining())
						.append(", block=").append(block).append(", filename=").append(filename)
						.append(", canSkipChecksum=").append(canSkipChecksum).append(")").toString();
	 ...
	 * checksums.
	 * 
	 * We always want the offsets of the data contained in dataBuf to be aligned
	 * to the chunk boundary. If we are validating checksums, we accomplish this
	 * by seeking backwards in the file until we're on a chunk boundary. (This
	 * is necessary because we can't checksum a partial chunk.) If we are not
	 * validating checksums, we simply only fill the latter part of dataBuf.
	 * 
	 * @param canSkipChecksum
	 *            true if we can skip checksumming.
	...
	private synchronized boolean fillDataBuf(boolean canSkipChecksum) throws IOException {
		...
		if (LOG.isTraceEnabled()) {
			LOG.trace("loaded " + dataBuf.remaining() + " bytes into bounce " + "buffer from offset " + oldDataPos
					+ " of " + block);
		}
	 ...
	 * bytes from the slow read buffer. After this the read is guaranteed to be
	 * on a checksum chunk boundary. If there are still bytes to read, the fast
	 * direct path is used for as many remaining bytes as possible, up to a
	 * multiple of the checksum chunk size. Finally, any 'odd' bytes remaining
	 * at the end of the read cause another slow read to be issued, which
	...
	private synchronized int readWithBounceBuffer(ByteBuffer buf, boolean canSkipChecksum) throws IOException {
		...
		do {
			if (buf.isDirect() && (buf.remaining() >= maxReadaheadLength) && ((dataPos % bytesPerChecksum) == 0)) {
				// Fast lane: try to read directly into user-supplied buffer,
				// bypassing
				// bounce buffer.
	...
	public synchronized int read(byte[] arr, int off, int len) throws IOException {
		...
		try {
			String traceString = null;
			if (LOG.isTraceEnabled()) {
				traceString = new StringBuilder().append("read(arr.length=").append(arr.length).append(", off=")
						.append(off).append(", len=").append(len).append(", filename=").append(filename)
						.append(", block=").append(block).append(", canSkipChecksum=").append(canSkipChecksum)
						.append(")").toString();
	...
	private synchronized int readWithBounceBuffer(byte arr[], int off, int len, boolean canSkipChecksum)
			throws IOException {
		createDataBufIfNeeded();
	...
	public synchronized long skip(long n) throws IOException {
		...
		if (LOG.isTraceEnabled()) {
			LOG.trace("skip(n=" + n + ", block=" + block + ", filename=" + filename + "): discarded " + discardedFromBuf
					+ " bytes from " + "dataBuf and advanced dataPos by " + remaining);
		}
		dataPos += remaining;
		return n;
	}
	 ...
	 * not to munlock the block until this ClientMmap is closed. If we fetch the
	 * latter, we don't bother with anchoring.
	 *
	 * @param opts
