...
/**
 * This is a wrapper around connection to datanode and understands checksum,
 * offset etc.
 *
 * Terminology:
 * <dl>
 * <dt>block</dt>
 * <dd>The hdfs block, typically large (~64MB).</dd>
 * <dt>chunk</dt>
 * <dd>A block is divided into chunks, each comes with a checksum. We want
 * transfers to be chunk-aligned, to be able to verify checksums.</dd>
 * <dt>packet</dt>
 * <dd>A grouping of chunks used for transport. It contains a header, followed
 * by checksum data, followed by real data.</dd>
 * </dl>
 * Please see DataNode for the RPC specification.
 *
 * This is a new implementation introduced in Hadoop 0.23 which is more
 * efficient and simpler than the older BlockReader implementation. It should be
 * renamed to RemoteBlockReader once we are confident in it.
 */
...
public class RemoteBlockReader2 implements BlockReader {
	...
	public synchronized int read(byte[] buf, int off, int len) throws IOException {
		...
		if (LOG.isTraceEnabled()) {
			...
			LOG.trace(String.format("Starting read #%s file %s from datanode %s", randomId.toString(), this.filename,
	...
	private void readNextPacket() throws IOException {
		...
		if (curHeader.getDataLen() > 0) {
			...
			if (verifyChecksum && curDataSlice.remaining() > 0) {
				...
				// relative to the start of the block, not the start of the
				// file.
	...
	protected RemoteBlockReader2(String file, String bpid, long blockId, DataChecksum checksum, boolean verifyChecksum,
			long startOffset, long firstChunkOffset, long bytesToRead, Peer peer, DatanodeID datanodeID,
			PeerCache peerCache) {
	 ...
	 *            The Peer to use
	 * @param datanodeID
	 *            The DatanodeID this peer is connected to
	 * @return New BlockReader instance, or null on error.
	 */
	public static BlockReader newBlockReader(String file, ExtendedBlock block, Token<BlockTokenIdentifier> blockToken,
			long startOffset, long len, boolean verifyChecksum, String clientName, Peer peer, DatanodeID datanodeID,
			PeerCache peerCache, CachingStrategy cachingStrategy) throws IOException {
