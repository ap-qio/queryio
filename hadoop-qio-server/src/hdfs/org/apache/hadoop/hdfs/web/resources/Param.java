...
public abstract class Param<T, D extends Param.Domain<T>> {
	 ...
	 *
	 * @param separator
	 *            URI parameter separator character
	 * @param parameters
	 *            parameters to encode into a string
	...
	public static String toSortedString(final String separator, final Param<?, ?>... parameters) {
		...
		try {
			for (Param<?, ?> p : parameters) {
				if (p.getValue() != null) {
					b.append(separator).append(URLEncoder.encode(p.getName(), "UTF-8") + "="
	...
	static abstract class Domain<T> {
		...
		public final T parse(final String varName, final String str) {
			try {
				...
				throw new IllegalArgumentException("Failed to parse \"" + str + "\" for the parameter " + varName
