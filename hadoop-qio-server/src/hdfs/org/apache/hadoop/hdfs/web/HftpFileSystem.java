...
public class HftpFileSystem extends FileSystem
	...
	/**
	 * We generate the address with one of the following ports, in order of
	 * preference. 1. Port from the hftp URI e.g. hftp://namenode:4000/ will
	 * return 4000. 2. Port configured via DFS_NAMENODE_HTTP_PORT_KEY 3.
	 * DFS_NAMENODE_HTTP_PORT_DEFAULT i.e. 50070.
	 *
	 * @param uri
	 */
	 ...
	 * to be overridden by HsFtpFileSystem.
	 */
	...
	public synchronized <T extends TokenIdentifier> void setDelegationToken(Token<T> token) {
		 ...
		 * We use the token for renewal, since the reflection utilities needs
		 * the value of the kind field to correctly renew the token.
		 *
		 * For other operations, however, the client has to send a
		 * HDFS_DELEGATION_KIND token over the wire so that it can talk to
		 * Hadoop 0.20.203 clusters. Later releases fix this problem. See
		 * HDFS-5440 for more details.
		 */
		renewToken = token;
	...
	class LsParser extends DefaultHandler {
		...
		public void startElement(String ns, String localname, String qname, Attributes attrs) throws SAXException {
			...
			FileStatus fs = "file".equals(qname)
					? new FileStatus(Long.parseLong(attrs.getValue("size")), false,
							Short.valueOf(attrs.getValue("replication")).shortValue(),
							Long.parseLong(attrs.getValue("blocksize")), modif, atime,
							FsPermission.valueOf(attrs.getValue("permission")), attrs.getValue("owner"),
							attrs.getValue("group"),
							HftpFileSystem.this.makeQualified(new Path(getUri().toString(), attrs.getValue("path"))))
	...
	private class ChecksumParser extends DefaultHandler {
		...
		private FileChecksum getFileChecksum(String f) throws IOException {
			final HttpURLConnection connection = openConnection("/fileChecksum" + ServletUtil.encodePath(f),
	...
	private class ContentSummaryParser extends DefaultHandler {
		...
		private ContentSummary getContentSummary(String path) throws IOException {
			final HttpURLConnection connection = openConnection("/contentSummary" + ServletUtil.encodePath(path),
					"ugi=" + getEncodedUgiParameter());
			InputStream in = null;
			try {
