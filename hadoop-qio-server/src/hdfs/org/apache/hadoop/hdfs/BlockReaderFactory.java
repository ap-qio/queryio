...
public class BlockReaderFactory implements ShortCircuitReplicaCreator {
	 ...
	 * However, if the socket has been timed out on the remote end, our attempt
	 * to reuse the socket may end with an IOException. For that reason, we
	 * limit our attempts at socket reuse to dfs.client.cached.conn.retry times.
	 * After that, we create new sockets. This avoids the problem where a thread
	 * tries to talk to a peer that it hasn't talked to in a while, and has to
	 * clean out every entry in a socket cache full of stale entries.
	 *
	 * @return The new BlockReader. We will not return null.
	 *
	 * @throws InvalidToken
	 *             If the block token was invalid. InvalidEncryptionKeyException
	 *             If the encryption key was invalid. Other IOException If there
	 *             was another problem.
	 */
	...
	/**
	 * Get {@link BlockReaderLocalLegacy} for short circuited local reads. This
	 * block reader implements the path-based style of local reads first
	 * introduced in HDFS-2246.
	 */
	private BlockReader getLegacyBlockReaderLocal() throws IOException {
		...
		if ((!(ioe instanceof AccessControlException)) && isSecurityException(ioe)) {
			...
			// BlockReaderLocalLegacy#newBlockReader uses that exception to
			// indicate
			// that the user is not in dfs.block.local-path-access.user, a
			// condition
	...
	private BlockReader getBlockReaderLocal() throws InvalidToken {
		...
		if (!pathInfo.getPathState().getUsableForShortCircuit()) {
			PerformanceAdvisory.LOG.debug(
					this + ": " + pathInfo + " is not " + "usable for short circuit; giving up on BlockReaderLocal.");
			return null;
		}
	 ...
	 *
	 * @return Null if we could not communicate with the datanode, a new
	 *         ShortCircuitReplicaInfo object otherwise. ShortCircuitReplicaInfo
	 *         objects may contain either an InvalidToken exception, or a
	 *         ShortCircuitReplica object ready to use.
	 */
	@Override
		...
		while (true) {
			...
			try {
				...
				if (curPeer.fromCache) {
					...
					// These are considered less serious, because the socket may
					// be stale.
					...
					// Handle an I/O error we got when using a newly created
					// socket.
					// We temporarily disable the domain socket path for a few
					// minutes in
	 ...
	 * @param slot
	 *            If non-null, the shared memory slot to associate with the new
	 *            ShortCircuitReplica.
	 * 
	 * @return A ShortCircuitReplica object if we could communicate with the
	 *         datanode; null, otherwise.
	 * @throws IOException
	 *             If we encountered an I/O exception while communicating with
	 *             the datanode.
	 */
	private ShortCircuitReplicaInfo requestFileDescriptors(DomainPeer peer, Slot slot) throws IOException {
		...
		case SUCCESS:
			...
			try {
				...
				// Since
				// it's not a socket communication problem, we return null
				// rather than
		...
		case ERROR_UNSUPPORTED:
			if (!resp.hasShortCircuitAccessVersion()) {
				...
				LOG.warn("short-circuit read access for the file " + fileName + " is disabled for DataNode " + datanode
	 ...
	 *
	 * @throws InvalidToken
	 *             If the block token was invalid. Potentially other
	 *             security-related execptions.
	 */
		...
		while (true) {
			...
			try {
				...
				if (isSecurityException(ioe)) {
					if (LOG.isTraceEnabled()) {
						LOG.trace(
								this + ": got security exception while constructing "
				...
				if (curPeer.fromCache) {
					// Handle an I/O error we got when using a cached peer.
					// These are
					// considered less serious, because the underlying socket
					// may be stale.
					if (LOG.isDebugEnabled()) {
	 ...
	 *
	 * @throws IOException
	 *             If there was an error while constructing the peer (such as an
	 ...
	 * there is some action the client needs to take, such as refetching block
	 * tokens, renewing encryption keys, etc.
	 *
	 * @param ioe
	 *            The exception
