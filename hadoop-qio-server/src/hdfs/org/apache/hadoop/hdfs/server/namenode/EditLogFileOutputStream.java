...
public class EditLogFileOutputStream extends EditLogOutputStream {
	 ...
	 * 
	 * @param layoutVersion
	 *            the LayoutVersion of the EditLog
	 * @param out
	 *            the output stream to write the header to.
	 * @throws IOException
	 *             in the event of error writing to the stream.
	 */
	@VisibleForTesting
	...
	/**
	 * Flush ready buffer to persistent store. currentBuffer is not flushed as
	 * it accumulates new log records while readyBuffer will be flushed and
	 * synced.
	 */
	@Override
	...
	/**
	 * @return true if the number of buffered data exceeds the intial buffer
	 *         size
	 */
	@Override
	...
	/**
	 * For the purposes of unit tests, we don't need to actually write durably
	 * to disk. So, we can skip the fsync() calls for a speed improvement.
	 * 
	 * @param skip
	 *            true if fsync should <em>not</em> be called
	 */
	@VisibleForTesting
