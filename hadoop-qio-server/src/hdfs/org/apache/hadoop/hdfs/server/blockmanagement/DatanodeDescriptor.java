...
public class DatanodeDescriptor extends DatanodeInfo {
	...
	/**
	 * The blocks which we know are cached on this datanode. This list is
	 * updated by periodic cache reports.
	 */
	...
	/**
	 * The time when the last batch of caching directives was sent, in monotonic
	 * milliseconds.
	 */
	 ...
	 * this storage. This count is approximate and might be slightly bigger in
	 * case of errors (e.g. datanode does not report if an error occurs while
	 * writing the block).
	 */
	...
	/**
	 * When set to true, the node is not in include list and is not allowed to
	 ...
	 */
	public void updateHeartbeat(StorageReport[] reports, long cacheCapacity, long cacheUsed, int xceiverCount,
			int volFailures, VolumeFailureSummary volumeFailureSummary) {
	 ...
	 */
	public void updateHeartbeatState(StorageReport[] reports, long cacheCapacity, long cacheUsed, int xceiverCount,
			int volFailures, VolumeFailureSummary volumeFailureSummary) {
		...
		// Decide if we should check for any missing StorageReport and mark it
		// as
		...
		// accuracy.
		// 2. After DN restarts, volFailures might not increase and it is
		// possible
	...
	/**
	 * Remove stale storages from storageMap. We must not remove any storages as
	 * long as they have associated block replicas.
	 */
		...
		synchronized (storageMap) {
			...
			for (final DatanodeStorageInfo storageInfo : excessStorages.values()) {
				if (storageInfo.numBlocks() == 0) {
					...
					LOG.debug("Deferring removal of stale storage " + storageInfo + " with " + storageInfo.numBlocks()
							+ " blocks");
				}
			}
		}
	 ...
	 *         the given storage type of this datanode.
	 */
	...
	/**
	 * Set the flag to indicate if this datanode is disallowed from
