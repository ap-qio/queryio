...
public class EditLogTailer {
	 ...
	 * from finalized log segments, the Standby will only be as up-to-date as
	 * how often the logs are rolled.
	 */
	...
	public EditLogTailer(FSNamesystem namesystem, Configuration conf) {
		...
		if (logRollPeriodMs >= 0) {
			...
			Preconditions.checkArgument(activeAddr.getPort() > 0,
					"Active NameNode must have an IPC port configured. " + "Got address '%s'", activeAddr);
			LOG.info("Will roll logs on active node at " + activeAddr + " every " + (logRollPeriodMs / 1000)
					+ " seconds.");
		} else {
			LOG.info("Not going to trigger log rolls on active node because " + DFSConfigKeys.DFS_HA_LOGROLL_PERIOD_KEY
					+ " is negative.");
		}
	...
	private NamenodeProtocol getActiveNodeProxy() throws IOException {
		if (cachedActiveProxy == null) {
			int rpcTimeout = conf.getInt(DFSConfigKeys.DFS_HA_LOGROLL_RPC_TIMEOUT_KEY,
					DFSConfigKeys.DFS_HA_LOGROLL_RPC_TIMEOUT_DEFAULT);
			NamenodeProtocolPB proxy = RPC.waitForProxy(NamenodeProtocolPB.class,
					RPC.getProtocolVersion(NamenodeProtocolPB.class), activeAddr, conf, rpcTimeout, Long.MAX_VALUE);
	...
	public void catchupDuringFailover() throws IOException {
		...
		SecurityUtil.doAsLoginUser(new PrivilegedExceptionAction<Void>() {
			...
			public Void run() throws Exception {
				try {
					// It is already under the full name system lock and the
					// checkpointer
					// thread is already stopped. No need to acqure any other
					// lock.
					doTailEdits();
				} catch (InterruptedException e) {
					throw new IOException(e);
	...
	void doTailEdits() throws IOException, InterruptedException {
		...
		try {
			...
			try {
				...
				// This is acceptable. If we try to tail edits in the middle of
				// an edits
			...
			// Once we have streams to load, errors encountered are legitimate
			// cause
			// for concern, so we don't catch them here. Simple errors reading
			// from
			// disk are ignored.
			long editsLoaded = 0;
			try {
	...
	private class EditLogTailerThread extends Thread {
		...
		private void doWork() {
			while (shouldRun) {
				try {
					// There's no point in triggering a log roll if the Standby
					// hasn't
					...
					/**
					 * Check again in case someone calls
					 * {@link EditLogTailer#stop} while we're triggering an edit
					 * log roll, since ipc.Client catches and ignores
					 * {@link InterruptedException} in a few places. This fixes
					 * the bug described in HDFS-2823.
					 */
					if (!shouldRun) {
						break;
					}
					// Prevent reading of name system while being modified. The
					// full
					// name system lock will be acquired to further block even
					// the block
					// state updates.
