...
class DataXceiverServer implements Runnable {
	...
	public void run() {
		...
		while (datanode.shouldRun && !datanode.shutdownForUpgrade) {
			try {
				...
				// another thread closed our listener socket - that's expected
				// during shutdown,
				...
				IOUtils.cleanup(null, peer);
				// DataNode can run out of memory if there is too many
				// transfers.
				// Log the event, Sleep for 30 seconds, other transfers may
				// complete by
				// then.
	...
	void kill() {
		assert (datanode.shouldRun == false
				|| datanode.shutdownForUpgrade) : "shoudRun should be set to false or restarting should be true"
						+ " before killing";
		try {
