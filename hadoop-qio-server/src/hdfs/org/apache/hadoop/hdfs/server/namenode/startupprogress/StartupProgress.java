 ...
 * to associate a step or phase with optional information, such as a file name
 * or file size. It also provides counters, which can be incremented by the
...
public class StartupProgress {
	 ...
	 * method to obtain a counter once and then increment that instance
	 * repeatedly within a loop. This prevents redundant lookup operations and
	 * object creation within the tight loop. Incrementing the counter is an
	 * atomic operation, so there is no risk of lost updates even if multiple
	 * threads increment the same counter.
	 * 
	 * @param phase
	 *            Phase to get
	 * @param step
	 ...
	 * example, this can be used while loading fsimage to indicate the full path
	 * to the fsimage file.
	 * 
	 * @param phase
	 *            Phase to set
	 ...
	 * example, this can be used while loading fsimage to indicate the size of
	 * the fsimage file.
	 * 
	 * @param phase
	 *            Phase to set
	 ...
	 * this can be used while loading edits to indicate the number of operations
	 * to be applied.
	 * 
	 * @param phase
	 *            Phase to set
	 ...
	 * require aggregation, such as overall percent complete, will not be
	 * impacted by mutations performed in other threads mid-way through the
	 * calculation.
	 * 
	 ...
	 * phase and step. Returns either the newly initialized data structure or
	 * the existing one. Initialization is atomic, so there is no risk of lost
	 * updates even if multiple threads attempt to initialize the same step
	 * simultaneously.
	 * 
	 * @param phase
	 *            Phase to initialize
