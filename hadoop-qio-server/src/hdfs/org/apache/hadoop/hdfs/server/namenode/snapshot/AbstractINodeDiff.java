 ...
 * current directory state is stored in the field of {@link INode}. The snapshot
 * state can be obtained by applying the diffs one-by-one in reversed
 * chronological order. Let s_1, s_2, ..., s_n be the corresponding snapshots.
 * Then,
 * 
 * <pre>
...
abstract class AbstractINodeDiff<N extends INode, A extends INodeAttributes, D extends AbstractINodeDiff<N, A, D>>
	...
	A snapshotINode;
	/**
	 * Posterior diff is the diff happened after this diff. The posterior diff
	 * should be first applied to obtain the posterior snapshot and then apply
	 * this diff in order to obtain this snapshot. If the posterior diff is
	 * null, the posterior state is the current state.
	 */
	...
	abstract QuotaCounts combinePosteriorAndCollectBlocks(final BlockStoragePolicySuite bsps, final N currentINode,
			final D posterior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes);

	/**
	 * Delete and clear self.
	 * 
	 * @param bsps
	 *            The block storage policy suite used to retrieve storage policy
	 * @param currentINode
	 *            The inode where the deletion happens.
	 * @param collectedBlocks
	 *            Used to collect blocks for deletion.
	 * @param removedINodes
	 *            INodes removed
	 * @return quota usage delta
	 */
	abstract QuotaCounts destroyDiffAndCollectBlocks(final BlockStoragePolicySuite bsps, final N currentINode,
