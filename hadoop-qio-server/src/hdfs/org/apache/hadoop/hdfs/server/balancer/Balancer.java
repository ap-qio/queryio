 ...
 * of bytes that are being moved in this iteration. Normally "Bytes Already
 * Moved" is increasing while "Bytes Left To Move" is decreasing.
 * 
 * <p>
 ...
 * <li>No block has been moved for specified consecutive iterations (5 by
 * default);
 ...
 * <li>No block has been moved for specified iterations (5 by default).
 * Exiting...
...
public class Balancer {
	...
	/**
	 * Construct a balancer. Initialize balancer. It sets the value of the
	 * threshold, and builds the communication proxies to namenode as a client
	 * and a secondary namenode and retry proxies when connection fails.
	 */
	 ...
	 * over-utilized storages, above average utilized storages, below average
	 * utilized storages, and underutilized storages. The input datanode storage
	 * set is shuffled in order to randomize to the storage matching later on.
	 *
	 * @return the number of bytes needed to move in order to balance the
	 *         cluster.
	 */
	private long init(List<DatanodeStorageReport> reports) {
		...
		for (DatanodeStorageReport r : reports) {
			...
			for (StorageType t : StorageType.getMovableTypes()) {
				...
				if (utilization == null) { // datanode does not have such
											// storage type
					continue;
				}
		...
		Preconditions
				.checkState(
						dispatcher.getStorageGroupMap().size() == overUtilized.size() + underUtilized.size()
	 ...
	 * source to a target Maximum bytes to be moved per storage group is min(1
	 * Band worth of bytes, MAX_SIZE_TO_MOVE).
	 * 
	 * @return total number of bytes to move in this iteration
	 */
	private long chooseStorageGroups() {
		// First, match nodes on the same node group if cluster is node group
		// aware
	...
	private void chooseStorageGroups(final Matcher matcher) {
		...
		/*
		 * match each remaining underutilized datanode (target) to above average
		 * utilized datanodes (source). Note only underutilized datanodes that
		 * have not had that max bytes to move satisfied in step 1 are selected.
		 */
	 ...
	 * datanodes or the candidates are source nodes with (utilization > Avg),
	 * and the others are target nodes with (utilization < Avg).
	 */
	...
	static class Result {
		...
		void print(int iteration, PrintStream out) {
			out.printf("%-24s %10d  %19s  %18s  %17s%n", DateFormat.getDateTimeInstance().format(new Date()), iteration,
					StringUtils.byteDesc(bytesAlreadyMoved), StringUtils.byteDesc(bytesLeftToMove),
	...
	Result runOneIteration() {
		try {
			 ...
			 * the number of bytes that need to be moved from one node to
			 * another in this iteration. Maximum bytes to be moved per node is
			 ...
			 * decide a block to be moved and its proxy source, then initiates
			 * the move until all bytes are moved or no more block available to
			 * move. Exit no byte has been moved for 5 consecutive iterations.
			 */
	 ...
	 * {@link Balancer} to work through all datanodes once.
	 */
	...
	static class Parameters {
		...
		public String toString() {
			return Balancer.class.getSimpleName() + "." + getClass().getSimpleName() + "[" + policy + ", threshold="
					+ threshold + ", max idle iteration = " + maxIdleIteration + ", number of nodes to be excluded = "
					+ nodesToBeExcluded.size() + ", number of nodes to be included = " + nodesToBeIncluded.size() + "]";
		}
	}
