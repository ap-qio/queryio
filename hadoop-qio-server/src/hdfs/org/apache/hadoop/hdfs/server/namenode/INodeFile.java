...
public class INodeFile extends INodeWithAdditionalFields implements INodeFileAttributes, BlockCollection {
	...
	/**
	 * Bit format: [4-bit storagePolicyID][12-bit replication][48-bit
	 * preferredBlockSize]
	 */
	static enum HeaderFormat {
		PREFERRED_BLOCK_SIZE(null, 48, 1), REPLICATION(PREFERRED_BLOCK_SIZE.BITS, 12,
				1), STORAGE_POLICY_ID(REPLICATION.BITS, BlockStoragePolicySuite.ID_BIT_LENGTH, 0);
	...
	INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime, long atime, BlockInfoContiguous[] blklist,
	...
	public boolean metadataEquals(INodeFileAttributes other) {
		return other != null && getHeaderLong() == other.getHeaderLong()
	...
	/**
	 * Convert the file to a complete file, i.e., to remove the
	...
	private void assertAllBlocksComplete() {
		...
		for (int i = 0; i < blocks.length; i++) {
			Preconditions.checkState(blocks[i].isComplete(),
					"Failed to finalize" + " %s %s since blocks[%s] is non-complete, where blocks=%s.",
	...
	public BlockInfoContiguousUnderConstruction setLastBlock(BlockInfoContiguous lastBlock,
		...
		BlockInfoContiguousUnderConstruction ucBlock = lastBlock
				.convertToBlockUnderConstruction(BlockUCState.UNDER_CONSTRUCTION, locations);
	...
	public void recordModification(final int latestSnapshotId, boolean withBlocks) {
		if (isInLatestSnapshot(latestSnapshotId) && !shouldRecordInSrcSnapshot(latestSnapshotId)) {
			// the file is in snapshot, create a snapshot feature if it does not
			// have
			FileWithSnapshotFeature sf = this.getFileWithSnapshotFeature();
			if (sf == null) {
	...
	@Override
	public QuotaCounts cleanSubtree(BlockStoragePolicySuite bsps, final int snapshot, int priorSnapshotId,
		...
		if (snapshot == CURRENT_STATE_ID) {
			if (priorSnapshotId == NO_SNAPSHOT_ID) {
				// this only happens when deleting the current file and the file
				// is not
				// in any snapshot
				computeQuotaUsage(bsps, counts, false);
				destroyAndCollectBlocks(bsps, collectedBlocks, removedINodes);
			} else {
				// when deleting the current file and the file is in snapshot,
				// we should
	...
	/**
	 * Compute file size of the current file size but not including the last
	 * block if it is under construction.
	 */
	 ...
	 *            If the last block is under construction, should it be
	 *            included?
	 * @param usePreferredBlockSize4LastUcBlock
	 *            If the last block is under construction, should we use actual
	 *            block size or preferred block size? Note that
	 *            usePreferredBlockSize4LastUcBlock is ignored if
	 *            includesLastUcBlock == false.
	 * @return file size
	 */
	...
	/**
	 * Compute size consumed by all blocks of the current file, including blocks
	 * in its snapshots. Use preferred block size for the last block if it is
	 * under construction.
	 */
	...
	long computeQuotaDeltaForTruncate(final long newLength) {
		...
		if (sf != null) {
			...
			if (sblocks != null) {
				for (int i = (onBoundary ? n : n - 1); i < blocks.length && i < sblocks.length
						&& blocks[i].equals(sblocks[i]); i++) {
					truncateSize -= blocks[i].getNumBytes();
				}
			}
		}
