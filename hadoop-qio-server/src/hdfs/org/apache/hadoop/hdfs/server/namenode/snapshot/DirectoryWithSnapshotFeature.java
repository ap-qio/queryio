...
public class DirectoryWithSnapshotFeature implements INode.Feature {
	/**
	 * The difference between the current state and a previous snapshot of the
	 * children list of an INodeDirectory.
	 */
	static class ChildrenDiff extends Diff<byte[], INode> {
		 ...
		 * 
		 * @return true if the child is replaced; false if the child is not
		 *         found.
		 */
		...
		private QuotaCounts destroyCreatedList(final BlockStoragePolicySuite bsps, final INodeDirectory currentINode,
		...
		private QuotaCounts destroyDeletedList(final BlockStoragePolicySuite bsps,
				final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
			QuotaCounts counts = new QuotaCounts.Builder().build();
		...
		private void writeCreated(DataOutput out) throws IOException {
			...
			for (INode node : created) {
				// For INode in created list, we only need to record its local
				// name
	...
	public static class DirectoryDiff
		...
		DirectoryDiff(int snapshotId, INodeDirectoryAttributes snapshotINode, DirectoryDiff posteriorDiff,
				int childrenSize, List<INode> createdList, List<INode> deletedList, boolean isSnapshotRoot) {
		...
		@Override
		QuotaCounts combinePosteriorAndCollectBlocks(final BlockStoragePolicySuite bsps,
		...
		/**
		 * @return The children list of a directory in a snapshot. Since the
		 *         snapshot is read-only, the logical view of the list is never
		 *         changed although the internal data structure may mutate.
		 */
			...
			return new ReadOnlyList<INode>() {
				...
				private List<INode> initChildren() {
					if (children == null) {
						...
						children = combined.apply2Current(
								ReadOnlyList.Util.asList(currentDir.getChildrenList(Snapshot.CURRENT_STATE_ID)));
					}
					return children;
				}
		...
		INode getChild(byte[] name, boolean checkPosterior, INodeDirectory currentDir) {
			for (DirectoryDiff d = this;; d = d.getPosterior()) {
				...
				if (returned != null) {
					...
					// Since checkPosterior is false, return null, i.e. not
					// found.
					return null;
		...
		void write(DataOutput out, ReferenceMap referenceMap) throws IOException {
			...
			// Write diff. Node need to write poseriorDiff, since diffs is a
			// list.
			diff.write(out, referenceMap);
		}

		@Override
		QuotaCounts destroyDiffAndCollectBlocks(BlockStoragePolicySuite bsps, INodeDirectory currentINode,
	...
	public static class DirectoryDiffList
		 ...
		 * inode.
		 * 
		 * @return the id of the snapshot. {@link Snapshot#NO_SNAPSHOT_ID} if
		 *         the given inode is not in any of the snapshot.
		 */
	 ...
	 */
	public static void destroyDstSubtree(final BlockStoragePolicySuite bsps, INode inode, final int snapshot,
			final int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes)
		...
		if (inode.isReference()) {
			if (inode instanceof INodeReference.WithName && snapshot != Snapshot.CURRENT_STATE_ID) {
				// this inode has been renamed before the deletion of the
				// DstReference
				// subtree
				inode.cleanSubtree(bsps, snapshot, prior, collectedBlocks, removedINodes);
			} else {
				// for DstReference node, continue this process to its subtree
				destroyDstSubtree(bsps, inode.asReference().getReferredINode(), snapshot, prior, collectedBlocks,
						removedINodes);
			}
	 ...
	 * 
	 * @param bsps
	 *            The block storage policy suite.
	 * @param inode
	 *            The inode to clean.
	 * @param post
	 *            The post snapshot.
	 * @param prior
	 *            The id of the prior snapshot.
	 * @param collectedBlocks
	 *            Used to collect blocks for later deletion.
	 * @return Quota usage update.
	 */
	private static QuotaCounts cleanDeletedINode(final BlockStoragePolicySuite bsps, INode inode, final int post,
			final int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
		QuotaCounts counts = new QuotaCounts.Builder().build();
		...
		while (!queue.isEmpty()) {
			...
			if (topNode instanceof INodeReference.WithName) {
				...
				// For DstReference node, since the node is not in the created
				// list of
				...
				if (sf != null) {
					...
					// files/dirs, along with inode, were deleted right after
					// post.
				...
				for (INode child : dir.getChildrenList(prior)) {
					if (priorChildrenDiff != null
							&& priorChildrenDiff.search(ListType.DELETED, child.getLocalNameBytes()) != null) {
						continue;
					}
					queue.addLast(child);
				}
	 ...
	 * current children list. These directories are equivalent to the
	 * directories stored in the deletes lists.
	 */
	...
	public boolean removeChild(INodeDirectory parent, INode child, int latestSnapshotId) {
		// For a directory that is not a renamed node, if isInLatestSnapshot
		// returns
		// false, the directory is not in the latest snapshot, thus we do not
		// need
		// to record the removed child in any snapshot.
		// For a directory that was moved/renamed, note that if the directory is
		// in
		// any of the previous snapshots, we will create a reference node for
		// the
		// directory while rename, and isInLatestSnapshot will return true in
		// that
		// scenario (if all previous snapshots have been deleted,
		// isInLatestSnapshot
		...
		// previous src tree). Thus we do not need to record the removed child
		// in
		// any snapshot.
		ChildrenDiff diff = diffs.checkAndAddLatestSnapshotDiff(latestSnapshotId, parent).diff;
	 ...
	 *         returned for the snapshot. Otherwise we calculate the children
	 *         list for the snapshot and return it.
	 */
	public ReadOnlyList<INode> getChildrenList(INodeDirectory currentINode, final int snapshotId) {
		...
		return diff != null ? diff.getChildrenList(currentINode)
	...
	public INode saveChild2Snapshot(INodeDirectory currentINode, final INode child, final int latestSnapshotId,
	...
	public void computeContentSummary4Snapshot(final BlockStoragePolicySuite bsps, final ContentCounts counts) {
		// Create a new blank summary context for blocking processing of
		// subtree.
	 ...
	 *
	 * @param fromSnapshot
	 *            Start point of the diff computation. Null indicates current
	 *            tree.
	 * @param toSnapshot
	 ...
	 *            that the diff still represents (later_snapshot -
	 *            earlier_snapshot) although toSnapshot can be before
	 *            fromSnapshot.
	 * @param currentINode
	 *            The {@link INodeDirectory} this feature belongs to.
	 * @return Whether changes happened between the startSnapshot and
	 *         endSnaphsot.
	 */
	...
	public QuotaCounts cleanDirectory(final BlockStoragePolicySuite bsps, final INodeDirectory currentINode,
			final int snapshot, int prior, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
		QuotaCounts counts = new QuotaCounts.Builder().build();
		...
		if (snapshot == Snapshot.CURRENT_STATE_ID) { // delete the current
			...
			counts.add(currentINode.cleanSubtreeRecursively(bsps, snapshot, prior, collectedBlocks, removedINodes,
					priorDeleted));
		} else {
			// update prior
			prior = getDiffs().updatePrior(snapshot, prior);
			// if there is a snapshot diff associated with prior, we need to
			// record
			...
			counts.add(
					getDiffs().deleteSnapshotDiff(bsps, snapshot, prior, currentINode, collectedBlocks, removedINodes));
			counts.add(currentINode.cleanSubtreeRecursively(bsps, snapshot, prior, collectedBlocks, removedINodes,
					priorDeleted));

			// check priorDiff again since it may be created during the diff
			// deletion
			if (prior != Snapshot.NO_SNAPSHOT_ID) {
				DirectoryDiff priorDiff = this.getDiffs().getDiffById(prior);
				if (priorDiff != null && priorDiff.getSnapshotId() == prior) {
					// For files/directories created between "prior" and
					// "snapshot",
					// we need to clear snapshot copies for "snapshot". Note
					// that we must
					// use null as prior in the cleanSubtree call.
					...
					if (priorCreated != null) {
						// we only check the node originally in prior's created
						// list
						for (INode cNode : priorDiff.getChildrenDiff().getList(ListType.CREATED)) {
							if (priorCreated.containsKey(cNode)) {
								counts.add(cNode.cleanSubtree(bsps, snapshot, Snapshot.NO_SNAPSHOT_ID, collectedBlocks,
										removedINodes));
							}
						}
					}

					// When a directory is moved from the deleted list of the
					// posterior
					// diff to the deleted list of this diff, we need to destroy
					// its
					// descendants that were 1) created after taking this diff
					// and 2)
					// deleted after taking posterior diff.

					// For files moved from posterior's deleted list, we also
					// need to
					// delete its snapshot copy associated with the posterior
					// snapshot.
