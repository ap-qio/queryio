 ...
 * percent complete, will not be impacted by mutations performed in other
 * threads mid-way through the calculation.
 * 
...
public class StartupProgressView {
	...
	/**
	 * Returns the sum of the counter values for all steps in the specified
	 * phase.
	 * 
	 * @param phase
	 *            Phase to get
	...
	/**
	 * Returns elapsed time for the specified phase, calculated as (end - begin)
	 * if phase is complete or (now - begin) if phase is running or 0 if the
	...
	/**
	 * Returns elapsed time for the specified phase and step, calculated as (end
	 * - begin) if step is complete or (now - begin) if step is running or 0 if
	...
	/**
	 * Returns the optional file name associated with the specified phase,
	 * possibly null.
	 * 
	 * @param phase
	 *            Phase to get
	...
	/**
	 * Returns overall percent complete, calculated by aggregating percent
	 * complete of all phases. This is an approximation that assumes all phases
	 * have equal running time. In practice, this isn't true, but there isn't
	...
	/**
	 * Returns percent complete for the specified phase, calculated by
	 * aggregating the counter values and totals for all steps within the phase.
	 * 
	 * @param phase
	 *            Phase to get
	 * @return float percent complete
	 ...
	 * (now - begin) if end is undefined or 0 if both are undefined. Begin and
	 * end time come from the same AbstractTracking instance.
	 * 
	 * @param tracking
	 ...
	 * Returns elapsed time, calculated as (end - begin) if both are defined or
	 * (now - begin) if end is undefined or 0 if both are undefined. Begin and
	 * end time may come from different AbstractTracking instances.
	 * 
	 * @param beginTracking
	...
	private long getElapsedTime(AbstractTracking beginTracking, AbstractTracking endTracking) {
		final long elapsed;
		if (beginTracking != null && beginTracking.beginTime != Long.MIN_VALUE && endTracking != null
