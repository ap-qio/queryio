...
public class EditLogFileInputStream extends EditLogInputStream {
	...
	/**
	 * Open an EditLogInputStream for the given file. The file is
	 * pretransactional, so has no txids
	 * 
	 * @param name
	 *            filename to open
	 * @throws LogHeaderCorruptException
	 *             if the header is either missing or appears to be
	 *             corrupt/truncated
	 * @throws IOException
	 *             if an actual IO error occurs while reading the header
	 */
	 ...
	 *            filename to open
	 * @param firstTxId
	 *            first transaction found in file
	 * @param lastTxId
	 ...
	 */
	public static EditLogInputStream fromUrl(URLConnectionFactory connectionFactory, URL url, long startTxId,
		...
		case OPEN:
			...
			if ((op != null) && (op.hasTransactionId())) {
				...
				if ((txId >= lastTxId) && (lastTxId != HdfsConstants.INVALID_TXID)) {
					...
					// edit log. In that case, you can end up with an
					// unfinalized edit log
					// which has some garbage at the end.
					// JournalManager#recoverUnfinalizedSegments will finalize
					// these
					// unfinished edit logs, giving them a defined final
					// transaction
					// ID. Then they will be renamed, so that any subsequent
					// readers will have this information.
					//
					// Since there may be garbage at the end of these "cleaned
					// up"
					// logs, we want to be sure to skip it here if we've read
					// everything
	...
	static FSEditLogLoader.EditLogValidation validateEditLog(File file) throws IOException {
		...
		try {
			...
			// If the header is malformed or the wrong value, this indicates a
			// corruption
		...
		try {
			while (true) {
				...
				try {
					...
					FSImage.LOG.warn("Caught exception after scanning through " + numValid + " ops from " + in
	...
	static int readLogVersion(DataInputStream in, boolean verifyLayoutVersion)
		...
		if (verifyLayoutVersion && (logVersion < HdfsConstants.NAMENODE_LAYOUT_VERSION || // future
																							// version
			...
			throw new LogHeaderCorruptException("Unexpected version of the file system log file: " + logVersion
					+ ". Current version = " + HdfsConstants.NAMENODE_LAYOUT_VERSION + ".");
		}
		return logVersion;
	}

	/**
	 * Exception indicating that the header of an edits log file is corrupted.
	 * This can be because the header is not present, or because the header data
	 * is invalid (eg claims to be over a newer version than the running
	 * NameNode)
	 */
