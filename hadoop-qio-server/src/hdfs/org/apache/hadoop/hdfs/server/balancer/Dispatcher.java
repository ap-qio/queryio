...
public class Dispatcher {
	...
	/**
	 * the period of time to delay the usage of a DataNode after hitting errors
	 * when using it for migrating data
	 */
	...
	private static class GlobalBlockMap {
		...
		/**
		 * Get the block from the map; if the block is not found, create a new
		 * block and put it in the map.
		 */
	...
	public class PendingMove {
		...
		private boolean chooseProxySource() {
			...
			// if node group is supported, first try add nodes in the same node
			// group
		...
		private void dispatch() {
			...
			try {
				...
				// Proxy or target may have some issues, delay before using
				// these nodes
				// further in order to avoid a potential storm of "threads quota
				// exceeded" warnings when the dispatcher gets out of sync with
				// work
	...
	public class Source extends DDatanode.StorageGroup {
		 ...
		 * source's block list & {@link Dispatcher#globalBlocks}.
		 * 
		 ...
		 * are determined too. When choosing proxy and target, source & target
		...
		private void dispatchBlocks() {
			...
			while (!isTimeUp && getScheduledSize() > 0 && (!srcBlocks.isEmpty() || blocksToReceive > 0)) {
				...
				if (shouldFetchMoreBlocks()) {
					...
					// source node cannot find a pending block to move,
					// iteration +1
				...
				try {
					synchronized (Dispatcher.this) {
						Dispatcher.this.wait(1000); // wait for targets/sources
													// to be idle
					}
	...
	public Dispatcher(NameNodeConnector nnc, Set<String> includedNodes, Set<String> excludedNodes, long movedWinWidth,
			int moverThreads, int dispatcherThreads, int maxConcurrentMovesPerNode, Configuration conf) {
		this.nnc = nnc;
	...
	private boolean shouldIgnore(DatanodeInfo dn) {
		...
		if (decommissioned || decommissioning || excluded || notIncluded) {
			if (LOG.isTraceEnabled()) {
				LOG.trace("Excluding datanode " + dn + ": " + decommissioned + ", " + decommissioning + ", " + excluded
						+ ", " + notIncluded);
			}
			return true;
		}
	...
	/**
	 * Decide if the block is a good candidate to be moved from source to
	 * target. A block is a good candidate if 1. the block is not in the process
	 * of being moved/has not been moved; 2. the block does not have a replica
	 * on the target; 3. doing the move does not reduce the number of racks that
	 * the block has
	 */
	 ...
	 *
	 * @return true if there are any replica (other than source) on the same
	 *         node group with target
	 */
	...
	static class Util {
		 ...
		 * number.
		 * 
		 * @return true if the datanode's transfer address matches the set of
		 *         nodes.
		 */
