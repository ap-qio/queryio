...
public class StartupProgressMetrics implements MetricsSource {
	...
	private static final MetricsInfo STARTUP_PROGRESS_METRICS_INFO = info("StartupProgress",
	...
	public StartupProgressMetrics(StartupProgress startupProgress) {
		...
		DefaultMetricsSystem.instance().register(STARTUP_PROGRESS_METRICS_INFO.name(),
	 ...
	 *            long counter value
	 */
	private static void addCounter(MetricsRecordBuilder builder, Phase phase, String nameSuffix, String descSuffix,
			long value) {
	...
	/**
	 * Adds a gauge with a name built by using the specified phase's name as
	 * prefix and then appending the specified suffix.
	 * 
	 * @param builder
	 *            MetricsRecordBuilder to receive counter
	 ...
	 *            float gauge value
	 */
	private static void addGauge(MetricsRecordBuilder builder, Phase phase, String nameSuffix, String descSuffix,
			float value) {
		MetricsInfo metricsInfo = info(phase.getName() + nameSuffix, phase.getDescription() + descSuffix);
