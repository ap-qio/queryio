...
public class FSImage implements Closeable {
	...
	/**
	 * The last transaction ID that was either loaded from an image or loaded by
	 * loading edits files.
	 */
	 ...
	 * The checkpoint here could be one of the following operations. a.
	 * checkpoint when NN is in standby. b. admin saveNameSpace operation. c.
	 * download checkpoint file from any remote checkpointer.
	 */
	 ...
	 *            Configuration
	 * @throws IOException
	 *             if default directories are invalid.
	 */
	 ...
	 * @param editsDirs
	 *            Directories the editlog can be stored in.
	 * @throws IOException
	 *             if directories are invalid.
	 */
	...
	/**
	 * Check whether the storage directories and non-file journals exist. If
	 * running in interactive mode, will prompt the user for each directory to
	 * allow them to format anyway. Otherwise, returns false, unless 'force' is
	 * specified.
	 * 
	 * @param force
	 *            if true, format regardless of whether dirs exist
	 * @param interactive
	...
	boolean recoverTransitionRead(StartupOption startOpt, FSNamesystem target, MetaRecoveryContext recovery)
		...
		if (startOpt != StartupOption.UPGRADE && startOpt != StartupOption.UPGRADEONLY
			...
			throw new IOException("\nFile system image contains an old layout version " + storage.getLayoutVersion()
					...
					+ "Please restart NameNode with the \"" + RollingUpgradeStartupOption.STARTED.getOptionString()
					+ "\" option if a rolling upgrade is already started;" + " or restart NameNode with the \""
					+ StartupOption.UPGRADE.getName() + "\" option to start" + " a new upgrade.");
		}
	 ...
	 * 
	 * @param dataDirStates
	 *            output of storage directory states
	...
	public static boolean recoverStorageDirs(StartupOption startOpt, NNStorage storage,
		...
		// This loop needs to be over all storage dirs, even shared dirs, to
		// make
	...
	/**
	 * @return true if there is rollback fsimage (for rolling upgrade) in
	 *         NameNode directory.
	 */
	...
	void doUpgrade(FSNamesystem target) throws IOException {
		...
		if (!storage.getRemovedStorageDirs().isEmpty()) {
			...
			throw new IOException("Upgrade failed in " + storage.getRemovedStorageDirs().size()
	...
	void doRollback(FSNamesystem fsns) throws IOException {
		...
		try {
			...
			if (fsns.isHaEnabled()) {
				// If HA is enabled, check if the shared log can be rolled back
				// as well.
			...
			for (Iterator<StorageDirectory> it = storage.dirIterator(false); it.hasNext();) {
				...
				LOG.info("Rolling back storage directory " + sd.getRoot() + ".\n   new LV = "
						+ prevState.getStorage().getLayoutVersion() + "; new CTime = "
	...
	void finalizeUpgrade(boolean finalizeEditLog) throws IOException {
		...
		if (finalizeEditLog) {
			// We only do this in the case that HA is enabled and we're active.
			// In any
			// other case the NN will have done the upgrade of the edits
			// directories
	 ...
	 * @throws IOException
	 */
	private boolean loadFSImage(FSNamesystem target, StartupOption startOpt, MetaRecoveryContext recovery)
			throws IOException {
		...
		if (NameNodeLayoutVersion.supports(LayoutVersion.Feature.TXID_BASED_LAYOUT, getLayoutVersion())) {
			// If we're open for write, we're either non-HA or we're the active
			// NN, so
			// we better be able to load all the edits. If we're the standby NN,
			// it's
			// OK to not be able to read all of edits right now.
			// In the meanwhile, for HA upgrade, we will still write editlog
			// thus need
			// this toAtLeastTxId to be set to the max-seen txid
			// For rollback in rolling upgrade, we need to set the toAtLeastTxId
			// to
			// the txid right before the upgrade marker.
			long toAtLeastTxId = editLog.isOpenForWrite() ? inspector.getMaxSeenTxId() : 0;
			if (rollingRollback) {
				// note that the first image in imageFiles is the special
				// checkpoint
	...
	private void rollingRollback(long discardSegmentTxId, long ckptId) throws IOException {
		// discard discard unnecessary editlog segments starting from the given
		// id
		...
		if (NameNodeLayoutVersion.supports(LayoutVersion.Feature.TXID_BASED_LAYOUT, getLayoutVersion())) {
			...
			if (md5 == null) {
				throw new InconsistentFSStateException(sdForProperties.getRoot(),
						"Message digest property " + NNStorage.DEPRECATED_MESSAGE_DIGEST_PROPERTY
	...
	public void initEditLog(StartupOption startOpt) throws IOException {
		...
		} else if (HAUtil.isHAEnabled(conf, nameserviceId) && (startOpt == StartupOption.UPGRADE
				|| startOpt == StartupOption.UPGRADEONLY || RollingUpgradeStartupOption.ROLLBACK.matches(startOpt))) {
			// This NN is HA, but we're doing an upgrade or a rollback of
			// rolling
			...
			if (startOpt == StartupOption.UPGRADE || startOpt == StartupOption.UPGRADEONLY) {
				...
				if (this.storage.getCTime() < sharedLogCTime) {
					throw new IOException("It looks like the shared log is already "
							...
							+ "should restart this NameNode with the '" + StartupOption.BOOTSTRAPSTANDBY.getName()
	...
	private boolean needsResaveBasedOnStaleCheckpoint(File imageFile, long numEditsLoaded) {
		final long checkpointPeriod = conf.getLong(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_PERIOD_KEY,
	...
	private long loadEdits(Iterable<EditLogInputStream> editStreams, FSNamesystem target, StartupOption startOpt,
		...
		try {
			...
			for (EditLogInputStream editIn : editStreams) {
				...
				// If we are in recovery mode, we may have skipped over some
				// txids.
	...
	private static void updateCountForQuotaRecursively(BlockStoragePolicySuite bsps, byte blockStoragePolicyId,
		...
		if (dir.isQuotaSet()) {
			...
			if (Quota.isViolated(nsQuota, namespace)) {
				LOG.warn("Namespace quota violation in image for " + dir.getFullPathName() + " quota = " + nsQuota
						+ " < consumed = " + namespace);
			}
			...
			if (Quota.isViolated(ssQuota, ssConsumed)) {
				LOG.warn("Storagespace quota violation in image for " + dir.getFullPathName() + " quota = " + ssQuota
						+ " < consumed = " + ssConsumed);
			}
	...
	private void loadFSImage(File curFile, MD5Hash expectedMd5, FSNamesystem target, MetaRecoveryContext recovery,
			boolean requireSameLayoutVersion) throws IOException {
		// BlockPoolId is required when the FsImageLoader loads the rolling
		// upgrade
		...
		if (expectedMd5 != null && !expectedMd5.equals(readImageMd5)) {
			throw new IOException("Image file " + curFile + " is corrupt with MD5 checksum of " + readImageMd5
	...
	/**
	 * Save FSimage in the legacy format. This is not for NN consumption, but
	 * for tools like OIV.
	 */
	...
	/**
	 * FSImageSaver is being run in a separate thread when saving FSImage. There
	 * is one thread per each copy of the image.
	 *
	 * FSImageSaver assumes that it was launched from a thread that holds
	 * FSNamesystem lock and waits for the execution of FSImageSaver thread to
	 * finish. This way we are guaranteed that the namespace is not being
	 * updated while multiple instances of FSImageSaver are traversing it and
	 * writing it out.
	 */
		...
		try {
			try {
				...
				if (editLogWasOpen) {
					...
					// for startup. So, it doesn't hurt to update it
					// unnecessarily.
	...
	private synchronized void saveFSImageInAllDirs(FSNamesystem source, NameNodeFile nnf, long txid, Canceler canceler)
			throws IOException {
		StartupProgress prog = NameNode.getStartupProgress();
	 ...
	 */
	private void renameCheckpoint(long txid, NameNodeFile fromNnf, NameNodeFile toNnf, boolean renameMD5)
			throws IOException {
	...
	/**
	 * Deletes the checkpoint file in every storage directory, since the
	...
	NamenodeCommand startCheckpoint(NamenodeRegistration bnReg, // backup node
		...
		if (bnReg.getNamespaceID() != storage.getNamespaceID())
			msg = "Name node " + bnReg.getAddress() + " has incompatible namespace id: " + bnReg.getNamespaceID()
			...
			msg = "Name node " + bnReg.getAddress() + " role " + bnReg.getRole() + ": checkpoint is not allowed.";
				...
				|| (bnReg.getLayoutVersion() == storage.getLayoutVersion() && bnReg.getCTime() > storage.getCTime()))
	 ...
	 * 
	 * @param sig
	 *            to validate the current storage info against
	 * @throws IOException
	 *             if the checkpoint fields are inconsistent
	 */
	...
	/**
	 * This is called by the 2NN after having downloaded an image, and by the NN
	 * after having received a new image from the 2NN. It renames the image from
	 * fsimage_N.ckpt to fsimage_N and also saves the related .md5 file into
	 * place.
	 */
	public synchronized void saveDigestAndRenameCheckpointImage(NameNodeFile nnf, long txid, MD5Hash digest)
			throws IOException {
	...
	static List<URI> getCheckpointEditsDirs(Configuration conf, String defaultName) {
		Collection<String> dirNames = conf
				.getTrimmedStringCollection(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_EDITS_DIR_KEY);
	...
	// Should be OK for this to not be synchronized since all of the places
	// which
	// mutate this value are themselves synchronized so it shouldn't be possible
	// to see this flop back and forth. In the worst case this will just return
	// an
	// old value.
