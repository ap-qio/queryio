...
class ClusterJspHelper {
	...
	ClusterStatus generateClusterHealthReport() {
		...
		for (ConfiguredNNAddress cnn : nns) {
			...
			try {
				...
				if (cs.clusterid.isEmpty() || cs.clusterid.equals("")) { // Set
																			// clusterid
																			// only
																			// once
	 ...
	 * each Namenode over http via JmxJsonServlet to collect the data nodes
	 * status.
	 */
	DecommissionStatus generateDecommissioningReport() {
		...
		try {
			...
			// catch any exception encountered other than connecting to
			// namenodes
	 ...
	 * 
	 * @param statusMap
	 *            map whose key is datanode, value is an inner map with key
	 *            being namenode, value being decommission state.
	 */
	private void getDecommissionNodeClusterState(Map<String, Map<String, String>> statusMap) {
		...
		while (it.hasNext()) {
			...
			if ((inservice + dead + unknown) == nns) {
				// Do not display this data node. Remove this entry from status
				// map.
				it.remove();
			} else if (isUnknown) {
	...
	/**
	 * Class for connecting to Namenode over http via JmxJsonServlet to get JMX
	 * attributes exposed by the MXBean.
	 */
	static class NamenodeMXBeanHelper {
		...
		public NamenodeStatus getNamenodeStatus(String props)
				throws IOException, MalformedObjectNameException, NumberFormatException {
		 ...
		 *            string
		 */
		private void getDecomNodeInfoForReport(Map<String, Map<String, String>> statusMap, String props)
				throws IOException, MalformedObjectNameException {
		...
		/**
		 * Store the live datanode status information into datanode status map
		 * and DecommissionNode.
		 * 
		 * @param statusMap
		 *            Map of datanode status. Key is datanode, value is an inner
		 *            map whose key is namenode, value is datanode status.
		 ...
		 * @throws IOException
		 */
		private static void getLiveNodeStatus(Map<String, Map<String, String>> statusMap, String namenodeHost,
			...
			if (nodeMap != null && !nodeMap.isEmpty()) {
				...
				for (Map.Entry<String, Map<String, Object>> entry : nodeMap.entrySet()) {
					...
					if (innerMap != null) {
						...
						// the inner map key is namenode, value is datanode
						// status.
		 ...
		 * 
		 * @param statusMap
		 *            map with key being datanode, value being an inner map
		 *            (key:namenode, value:decommisionning state).
		 * @param host
		 *            datanode hostname
		 * @param json
		 *            String
		 * @throws IOException
		 */
		private static void getDeadNodeStatus(Map<String, Map<String, String>> statusMap, String host, String json)
				throws IOException {
			Map<String, Map<String, Object>> nodeMap = getNodeMap(json);
			if (nodeMap == null || nodeMap.isEmpty()) {
				return;
		 ...
		 *            String
		 */
		private static void getDecommissionNodeStatus(Map<String, Map<String, String>> dataNodeStatusMap, String host,
				String json) throws IOException {
			Map<String, Map<String, Object>> nodeMap = getNodeMap(json);
			if (nodeMap == null || nodeMap.isEmpty()) {
				return;
	...
	static class ClusterStatus {
		...
		public void toXML(XMLOutputter doc) throws IOException {
			if (error != null) {
				// general exception, only print exception message onto web
				// page.
			...
			for (NamenodeStatus nn : nnList) {
				...
				toXmlItemBlockWithLink(doc, nn.liveDatanodeCount + " (" + nn.liveDecomCount + ")",
						new URL(nn.httpAddress, "/dfsnodelist.jsp?whatNodes=LIVE"), "Live Datanode (Decommissioned)");
				toXmlItemBlockWithLink(doc, nn.deadDatanodeCount + " (" + nn.deadDecomCount + ")",
						new URL(nn.httpAddress, "/dfsnodelist.jsp?whatNodes=DEAD"), "Dead Datanode (Decommissioned)");
	...
	/**
	 * This class stores namenode statistics to be used to generate cluster web
	 * console report.
	 */
	...
	public enum DecommissionStates {
		...
		/*
		 * If datanode state is not known at a namenode, due to problems in
		 * getting the datanode state from the namenode.
		 */
		UNKNOWN("Unknown");
	...
	static class DecommissionStatus {
		...
		private DecommissionStatus(Map<String, Map<String, String>> statusMap, String cid, int httpPort,
		...
		private DecommissionStatus(Map<String, Map<String, String>> statusMap, String cid, int httpPort,
				Map<String, Exception> exceptions, Exception error) {
		...
		public void toXML(XMLOutputter doc) throws IOException {
			...
			if (statusMap == null || statusMap.isEmpty()) {
				// none of the namenodes has reported, print exceptions from
				// each nn.
				doc.startTag("cluster");
			...
			for (String dnhost : dnSet) {
				...
				if (overallStatus != null && (overallStatus.equals(AdminStates.DECOMMISSION_INPROGRESS.toString())
						|| overallStatus.equals(AdminStates.DECOMMISSIONED.toString())
						|| overallStatus.equals(DecommissionStates.PARTIALLY_DECOMMISSIONED.toString())
	...
	/**
	 * Generate a XML block as such,
	 * <item label="Node" value="hostname" link="http://hostname:50070" />
	 */
	private static void toXmlItemBlockWithLink(XMLOutputter doc, String value, URL url, String label)
			throws IOException {
		doc.startTag("item");
	 ...
	 * namenode.
	 */
	private static void createNamenodeExceptionMsg(XMLOutputter doc, Map<String, Exception> exceptionMsg)
			throws IOException {
	 ...
	 * "http://hostname/jmx?qry=Hadoop:service=NameNode,name=NameNodeInfo" is
	 * sent to namenode. JMX attributes are exposed via JmxJsonServelet on the
	 * namenode side.
	 */
