 ...
 * logs. It may correspond to multiple files, a backup node, etc. Even when the
 * actual underlying storage is rolled, or failed and restored, each conceptual
 * place of storage corresponds to exactly one instance of this class, which is
 * created when the EditLog is first opened.
 */
...
public interface JournalManager extends Closeable, LogsPurgeable, FormatConfirmable {
	...
	/**
	 * Finalize the upgrade. JMs should purge any state that they had been
	 * keeping around during the upgrade process. After this is completed,
	 * rollback is no longer allowed.
	 */
	 ...
	 * 
	 * @param startTxId
	 *            The given txid should be right at the segment boundary, i.e.,
	 *            it should be the first txid of some segment, if segment
	 *            corresponding to the txid exists.
	 */
	 ...
	 * edits. This exception occurs in the case of a gap in the transactions, or
	 * a corrupt edit file.
	 */
