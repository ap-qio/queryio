...
public class BackupNode extends NameNode {
	...
	public void stop() {
		...
		if (namenode != null && getRegistration() != null) {
			// Exclude this node from the list of backup streams on the
			// name-node
			try {
	...
	static class BackupNodeRpcServer extends NameNodeRpcServer implements JournalProtocol {
		...
		private void verifyJournalRequest(JournalInfo journalInfo) throws IOException {
			...
			if (journalInfo.getNamespaceId() != expectedNamespaceID) {
				errorMsg = "Invalid namespaceID in journal request - expected " + expectedNamespaceID + " actual "
			...
			if (!journalInfo.getClusterId().equals(namesystem.getClusterId())) {
				errorMsg = "Invalid clusterId in journal request - expected " + journalInfo.getClusterId() + " actual "
		...
		@Override
		public void journal(JournalInfo journalInfo, long epoch, long firstTxId, int numTxns, byte[] records)
				throws IOException {
			namesystem.checkOperation(OperationCategory.JOURNAL);
			verifyJournalRequest(journalInfo);
	...
	boolean shouldCheckpointAtStartup() {
		...
		// BN always checkpoints on startup in order to get in sync with
		// namespace
		return true;
	}
	...
	private NamespaceInfo handshake(Configuration conf) throws IOException {
		...
		this.namenode = NameNodeProxies
				.createNonHAProxy(conf, nnAddress, NamenodeProtocol.class, UserGroupInformation.getCurrentUser(), true)
				.getProxy();
	...
	private static NamespaceInfo handshake(NamenodeProtocol namenode) throws IOException, SocketTimeoutException {
		...
		assert HdfsConstants.NAMENODE_LAYOUT_VERSION == nsInfo
				.getLayoutVersion() : "Active and backup node layout versions must be the same. Expected: "
	...
	private class BNHAContext extends NameNodeHAContext {
		 ...
		 * <p>
		 * The following services should be muted (not run or not pass any
		 * control commands to DataNodes) on BackupNode:
