...
public class NameNode implements NameNodeStatusMXBean {
	 ...
	 * nameservice.
	 */
	public static final String[] NAMENODE_SPECIFIC_KEYS = { DFS_NAMENODE_RPC_ADDRESS_KEY,
			DFS_NAMENODE_RPC_BIND_HOST_KEY, DFS_NAMENODE_NAME_DIR_KEY, DFS_NAMENODE_EDITS_DIR_KEY,
			DFS_NAMENODE_SHARED_EDITS_DIR_KEY, DFS_NAMENODE_CHECKPOINT_DIR_KEY, DFS_NAMENODE_CHECKPOINT_EDITS_DIR_KEY,
			DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY, DFS_NAMENODE_SERVICE_RPC_BIND_HOST_KEY, DFS_NAMENODE_HTTP_ADDRESS_KEY,
			DFS_NAMENODE_HTTPS_ADDRESS_KEY, DFS_NAMENODE_HTTP_BIND_HOST_KEY, DFS_NAMENODE_HTTPS_BIND_HOST_KEY,
	 ...
	 * name service. For HA configurations using logical URI, it will be the
	 * logical address.
	 */
	 ...
	 * namenode or name service. This needs to be called before the config is
	 * overriden.
	 */
	...
	/**
	 * Given a configuration get the bind host of the service rpc server If the
	 * bind host is not configured returns null.
	 */
	...
	/**
	 * Given a configuration get the bind host of the client rpc server If the
	 * bind host is not configured returns null.
	 */
	...
	/**
	 * HTTP server address for binding the endpoint. This method is for use by
	 * the NameNode and its derivatives. It may return a different address than
	 * the one that should be used by clients to connect to the NameNode. See
	...
	public static InetSocketAddress getHttpAddress(Configuration conf) {
		return NetUtils
				.createSocketAddr(conf.getTrimmed(DFS_NAMENODE_HTTP_ADDRESS_KEY, DFS_NAMENODE_HTTP_ADDRESS_DEFAULT));
	}
	 ...
	 *
	 * @param conf
	 *            Supplies the original configuration.
	 * @return Cloned configuration without the shared edit dirs.
	 * @throws IOException
	 *             on failure to generate the configuration.
	 */
	...
	/**
	 * Format a new shared edits dir and copy in enough edit log segments so
	 * that the standby NN can start up.
	 * 
	 * @param conf
	 *            configuration
	 * @param force
	 *            format regardless of whether or not the shared edits dir
	 *            exists
	 * @param interactive
	...
	private static boolean initializeSharedEdits(Configuration conf, boolean force, boolean interactive)
		...
		try {
			...
			// Call Storage.format instead of FSImage.format here, since we
			// don't
			...
			// Have to unlock storage explicitly for the case when we're running
			// in a
				...
				for (i = i + 1; i < argsLen; i++) {
					if (args[i].equalsIgnoreCase(StartupOption.CLUSTERID.getName())) {
						...
						if (i >= argsLen) {
							...
							LOG.error("Must specify a valid cluster ID after the " + StartupOption.CLUSTERID.getName()
									+ " flag");
							return null;
						}
	...
	/**
	 * Verify that configured directories exist, then print the metadata
	 * versions of the software and the image.
	 *
	 * @param conf
	 *            configuration to use
	 * @throws IOException
	...
	protected synchronized void doImmediateShutdown(Throwable t) throws ExitException {
		...
		try {
			...
			// This is unlikely to happen, but there's nothing we can do if it
			// does.
		}
		terminate(1, t);
	}
