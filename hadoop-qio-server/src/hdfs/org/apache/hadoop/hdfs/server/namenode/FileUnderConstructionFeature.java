...
public class FileUnderConstructionFeature implements INode.Feature {
	...
	void updateLengthOfLastBlock(INodeFile f, long lastBlockLength) throws IOException {
		...
		assert (lastBlock != null) : "The last block for path " + f.getFullPathName()
				+ " is null when updating its length";
		assert (lastBlock instanceof BlockInfoContiguousUnderConstruction) : "The last block for path "
				+ f.getFullPathName() + " is not a BlockInfoUnderConstruction when updating its length";
		lastBlock.setNumBytes(lastBlockLength);
	}

	/**
	 * When deleting a file in the current fs directory, and the file is
	 * contained in a snapshot, we should delete the last block if it's under
	 * construction and its size is 0.
	 */
	void cleanZeroSizeBlock(final INodeFile f, final BlocksMapUpdateInfo collectedBlocks) {
		...
		if (blocks != null && blocks.length > 0
			...
			BlockInfoContiguousUnderConstruction lastUC = (BlockInfoContiguousUnderConstruction) blocks[blocks.length
					- 1];
