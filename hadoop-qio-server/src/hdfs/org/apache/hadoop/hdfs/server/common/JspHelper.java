...
public class JspHelper {
	...
	private static class NodeRecord extends DatanodeInfo {
		...
		public boolean equals(Object obj) {
			// Sufficient to use super equality as datanodes are uniquely
			// identified
			// by DatanodeID
	 ...
	 * Validate filename.
	 * 
	 * @return null if the filename is invalid. Otherwise, return the validated
	 *         filename.
	 */
	 ...
	 * 
	 * @return null if the value is invalid. Otherwise, return the validated
	 *         Long object.
	 */
	...
	public static UserGroupInformation getUGI(ServletContext context, HttpServletRequest request, Configuration conf,
		...
		if (UserGroupInformation.isSecurityEnabled()) {
			...
			if (tokenString != null) {
				// Token-based connections need only verify the effective user,
				// and
				// disallow proxying to different user. Proxy authorization
				// checks
			...
			remoteUser = (usernameFromQuery == null) ? getDefaultWebUserName(conf) // not
																					// specified
																					// in
																					// request
					: usernameFromQuery;
		...
		if (ugi == null) { // security is off, or there's no token
			...
			checkUsername(ugi.getShortUserName(), usernameFromQuery);
			if (UserGroupInformation.isSecurityEnabled()) {
				// This is not necessarily true, could have been auth'ed by
				// user-facing
				// filter
	...
	private static UserGroupInformation getTokenUGI(ServletContext context, HttpServletRequest request,
			String tokenString, Configuration conf) throws IOException {
	 ...
	 *            parameter value
	 * @param firstParam
	 *            true if this is the first parameter in the list, false
	 *            otherwise
	 * @return url parameter
	 */
