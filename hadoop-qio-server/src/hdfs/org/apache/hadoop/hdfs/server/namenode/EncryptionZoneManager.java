 ...
 * lock being held for many operations. The FSDirectory lock should not be taken
 * if the manager lock is already held.
 */
public class EncryptionZoneManager {
	 ...
	 * The external representation of an EZ is embodied in an EncryptionZone and
	 ...
	 *
	 * @param dir
	 *            Enclosing FSDirectory
	 */
	public EncryptionZoneManager(FSDirectory dir, Configuration conf) {
		this.dir = dir;
		encryptionZones = new TreeMap<Long, EncryptionZoneInt>();
		maxListEncryptionZonesResponses = conf.getInt(DFSConfigKeys.DFS_NAMENODE_LIST_ENCRYPTION_ZONES_NUM_RESPONSES,
		...
		Preconditions.checkArgument(maxListEncryptionZonesResponses >= 0,
				DFSConfigKeys.DFS_NAMENODE_LIST_ENCRYPTION_ZONES_NUM_RESPONSES + " " + "must be a positive integer.");
	}
	...
	/**
	 * Returns an EncryptionZone representing the ez for a given path. Returns
	 * an empty marker EncryptionZone if path is not in an ez.
	 *
	 * @param iip
	 *            The INodesInPath of the path to check
	...
	EncryptionZone getEZINodeForPath(INodesInPath iip) {
		...
		if (ezi == null) {
			...
			return new EncryptionZone(ezi.getINodeId(), getFullPathName(ezi), ezi.getSuite(), ezi.getVersion(),
					ezi.getKeyName());
		}
	}
	 ...
	 *            destination IIP
	 * @param src
	 *            source path, used for debugging
	 * @throws IOException
	 *             if the src cannot be renamed to the dst
	 */
		...
		if (srcInEZ) {
			if (!dstInEZ) {
				if (srcEZI.getINodeId() == srcIIP.getLastINode().getId()) {
					// src is ez root and dest is not in an ez. Allow the
					// rename.
					return;
				}
	...
	XAttr createEncryptionZone(String src, CipherSuite suite, CryptoProtocolVersion version, String keyName)
		...
		if (ezi != null) {
			throw new IOException(
					"Directory " + src + " is already in an " + "encryption zone. (" + getFullPathName(ezi) + ")");
		}
	...
	BatchedListEntries<EncryptionZone> listEncryptionZones(long prevId) throws IOException {
		...
		for (EncryptionZoneInt ezi : tailMap.values()) {
			/*
			 * Skip EZs that are only present in snapshots. Re-resolve the path
			 * to see if the path's current inode ID matches EZ map's INode ID.
			 * 
			 * INode#getFullPathName simply calls getParent recursively, so will
			 * return the INode's parents at the time it was snapshotted. It
			 * will not contain a reference INode.
			 */
			...
			zones.add(
					new EncryptionZone(ezi.getINodeId(), pathName, ezi.getSuite(), ezi.getVersion(), ezi.getKeyName()));
			count++;
