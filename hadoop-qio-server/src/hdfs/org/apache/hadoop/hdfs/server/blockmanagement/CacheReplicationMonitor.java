 ...
 *
 * The CacheReplicationMonitor does a full scan when the NameNode first starts
 * up, and at configurable intervals afterwards.
 */
...
public class CacheReplicationMonitor extends Thread implements Closeable {
	...
	public CacheReplicationMonitor(FSNamesystem namesystem, CacheManager cacheManager, long intervalMs,
			ReentrantLock lock) {
	...
	/**
	 * Indicates to the CacheReplicationMonitor that there have been
	 * CacheManager changes that require a rescan.
	 */
	public void setNeedsRescan() {
		...
		if (curScanCount >= 0) {
			// If there is a scan in progress, we need to wait for the scan
			// after
			// that.
			neededScanCount = curScanCount + 1;
		} else {
			// If there is no scan in progress, we need to wait for the next
			// scan.
	...
	/**
	 * Scan all CacheDirectives. Use the information to figure out what cache
	 * replication factor each block should have.
	 */
	private void rescanCacheDirectives() {
		...
		for (CacheDirective directive : cacheManager.getCacheDirectives()) {
			...
			try {
				...
				LOG.debug("Directive {}: got UnresolvedLinkException while resolving " + "path {}", directive.getId(),
						path);
				continue;
			}
			if (node == null) {
	...
	private void rescanFile(CacheDirective directive, INodeFile file) {
		...
		for (BlockInfoContiguous blockInfo : blockInfos) {
			...
			if (ocblock == null) {
				...
				// Assumptions: we assume that all the blocks are the same
				// length
				// on each datanode. We can assume this because we're only
				// caching
				// blocks in state COMPLETE.
				// Note that if two directives are caching the same block(s),
				// they will
				...
				if ((mark != ocblock.getMark()) || (ocblock.getReplication() < directive.getReplication())) {
					...
					//
					// 1. If the mark on the CachedBlock is different from the
					// mark for
					// this scan, that means the block hasn't been updated
					// during this
					// scan, and we should overwrite whatever is there, since it
					// is no
					// longer valid.
					//
					// 2. If the replication in the CachedBlock is less than
					// what the
					// directive asks for, we want to increase the block's
					// replication
			...
			LOG.trace("Directive {}: setting replication for block {} to {}", directive.getId(), blockInfo,
		...
		LOG.debug("Directive {}: caching {}: {}/{} bytes", directive.getId(), file.getFullPathName(), cachedTotal,
				neededTotal);
	}
	...
	private String findReasonForNotCaching(CachedBlock cblock, BlockInfoContiguous blockInfo) {
		if (blockInfo == null) {
			...
			// Alternately, the NameNode might have invalidated the block, but
			// the
			...
			// When a cached block changes state from complete to some other
			// state
			...
			// there may be a period of time when incomplete blocks remain
			// cached
			// on the DataNodes.
			return "not complete";
			...
			// Although the block was needed in the past, we didn't reach it
			// during
			// the current sweep. Therefore, it doesn't need to be cached any
			// more.
			// Need to set the replication to 0 so it doesn't flip back to
			// cached
	...
	/**
	 * Scan through the cached block map. Any blocks which are under-replicated
	 * should be assigned new Datanodes. Blocks that are over-replicated should
	 * be removed from Datanodes.
	 */
	private void rescanCachedBlockMap() {
		for (Iterator<CachedBlock> cbIter = cachedBlocks.iterator(); cbIter.hasNext();) {
			...
			// Remove nodes from PENDING_UNCACHED if they were actually
			// uncached.
			...
			if ((neededCached == 0) && pendingUncached.isEmpty() && pendingCached.isEmpty()) {
				...
				LOG.trace("Block {}: removing from cachedBlocks, since neededCached "
						+ "== 0, and pendingUncached and pendingCached are empty.", cblock.getBlockId());
				cbIter.remove();
			}
		}
	}
	 ...
	 *
	 * @param neededUncached
	 *            The number of replicas that need to be uncached.
	 * @param cachedBlock
	 *            The block which needs to be uncached.
	 * @param cached
	 *            A list of DataNodes currently caching the block.
	 * @param pendingUncached
	 *            A list of DataNodes that will soon uncache the block.
	 */
	 ...
	 *
	 * @param neededCached
	 *            The number of replicas that need to be cached.
	 * @param cachedBlock
	 *            The block which needs to be cached.
	 * @param cached
	 *            A list of DataNodes currently caching the block.
	 * @param pendingCached
	 *            A list of DataNodes that will soon cache the block.
	 */
	private void addNewPendingCached(final int neededCached, CachedBlock cachedBlock, List<DatanodeDescriptor> cached,
		...
		for (int i = 0; i < numReplicas; i++) {
			...
			if (pendingCapacity < blockInfo.getNumBytes()) {
				LOG.trace(
						...
						blockInfo.getBlockId(), datanode.getDatanodeUuid(), blockInfo.getNumBytes(), pendingCapacity,
						pendingBytes, datanode.getCacheRemaining());
				outOfCapacity++;
				continue;
			}
			possibilities.add(datanode);
		}
		List<DatanodeDescriptor> chosen = chooseDatanodesForCaching(possibilities, neededCached,
		...
		if (neededCached > chosen.size()) {
			LOG.debug(
					...
					blockInfo.getBlockId(), (cachedBlock.getReplication() - neededCached + chosen.size()),
					cachedBlock.getReplication(), outOfCapacity);
		}
	}

	/**
	 * Chooses datanode locations for caching from a list of valid
	 * possibilities. Non-stale nodes are chosen before stale nodes.
	 * 
	 * @param possibilities
	 *            List of candidate datanodes
	 * @param neededCached
	 ...
	 */
	private static List<DatanodeDescriptor> chooseDatanodesForCaching(final List<DatanodeDescriptor> possibilities,
			final int neededCached, final long staleInterval) {
		...
		while (chosen.size() < neededCached) {
			// Try to use stale nodes if we're out of non-stale nodes, else
			// we're done
			if (targets.isEmpty()) {
				if (!stale.isEmpty()) {
					targets = stale;
	...
	/**
	 * Choose a single datanode from the provided list of possible targets,
	 * weighted by the percentage of free space remaining on the node.
	 * 
	...
	private static DatanodeDescriptor chooseRandomDatanodeByRemainingCapacity(final List<DatanodeDescriptor> targets) {
		...
		// Choose a number from [0, offset), which is the total amount of
		// weight,
