...
/**
 * Static utility functions for serializing various pieces of data in the
 * correct format for the FSImage file.
 *
...
public class FSImageSerialization {
	...
	/**
	 * In order to reduce allocation, we reuse some static objects. However, the
	 * methods in this class should be thread-safe since image-saving is
	 * multithreaded, so we need to keep the static objects in a thread-local.
	 */
	...
	static INodeFile readINodeUnderConstruction(DataInput in, FSNamesystem fsNamesys, int imgVersion)
		...
		long inodeId = NameNodeLayoutVersion.supports(LayoutVersion.Feature.ADD_INODE_ID, imgVersion) ? in.readLong()
		...
		if (numBlocks > 0) {
			blk.readFields(in);
			blocks[i] = new BlockInfoContiguousUnderConstruction(blk, blockReplication, BlockUCState.UNDER_CONSTRUCTION,
					null);
		}
		...
		INodeFile file = new INodeFile(inodeId, name, perm, modificationTime, modificationTime, blocks,
	 ...
	 *            The node to write
	 * @param out
	 *            The {@link DataOutputStream} where the fields are written
	 * @param writeBlock
	 *            Whether to write block information
	 */
	 ...
	 *            The node to write
	 * @param out
	 *            The {@link DataOutput} where the fields are written
	 */
	 ...
	 * delta-encoding for the generation stamp and size, following the principle
	 * that genstamp increases relatively slowly, and size is equal for all but
