...
public class Step implements Comparable<Step> {
	...
	public int compareTo(Step other) {
		// Sort steps by file and then sequentially within the file to achieve
		// the
		// desired order. There is no concurrent map structure in the JDK that
		// maintains insertion order, so instead we attach a sequence number to
		// each
		// step and sort on read.
		return new CompareToBuilder().append(file, other.file).append(sequenceNumber, other.sequenceNumber)
				.toComparison();
	}
	...
	public boolean equals(Object otherObj) {
		...
		return new EqualsBuilder().append(this.file, other.file).append(this.size, other.size)
