...
/**
 * A double-buffer for edits. New edits are written into the first buffer while
 * the second is available to be flushed. Each time the double-buffer is
 * flushed, the two internal buffers are swapped. This allows edits to progress
 * concurrently to flushes without allocating new buffers each time.
 */
...
public class EditsDoubleBuffer {
	...
	/**
	 * Writes the content of the "ready" buffer to the given output stream, and
	 * resets it. Does not swap any buffers.
	 */
