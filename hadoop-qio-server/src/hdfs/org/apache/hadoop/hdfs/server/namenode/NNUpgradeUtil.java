...
public abstract class NNUpgradeUtil {
	 ...
	 */
	static boolean canRollBack(StorageDirectory sd, StorageInfo storage, StorageInfo prevStorage,
	 ...
	 * 
	 * @param sd
	 *            the storage directory to finalize
	 * @throws IOException
	 *             in the event of error
	 */
	...
	/**
	 * Perform any steps that must succeed across all storage
	 * dirs/JournalManagers involved in an upgrade before proceeding onto the
	 * actual upgrade stage. If a call to any JM's or local storage dir's
	 * doPreUpgrade method fails, then doUpgrade will not be called for any JM.
	 * The existing current dir is renamed to previous.tmp, and then a new,
	 * empty current dir is created.
	 *
	 * @param conf
	 *            configuration for creating {@link EditLogFileOutputStream}
	 * @param sd
	 *            the storage directory to perform the pre-upgrade procedure.
	 * @throws IOException
	 *             in the event of error
	 */
	...
	public static void renameCurToTmp(StorageDirectory sd) throws IOException {
		...
		Preconditions.checkState(!tmpDir.exists(),
				"Previous.tmp directory must not exist for preupgrade." + "Consider restarting for recovery.");
	 ...
	 * 
	 * @param sd
	 *            the storage directory to upgrade
	 * @param storage
	 *            info about the new upgraded versions.
	 * @throws IOException
	 *             in the event of error
	 */
	 ...
	 * 
	 * @param sd
	 *            the storage directory to roll back.
	 * @throws IOException
	 *             in the event of error
	 */
