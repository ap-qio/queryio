...
class FSDirRenameOp {
	@Deprecated
	static RenameOldResult renameToInt(FSDirectory fsd, final String srcArg, final String dstArg, boolean logRetryCache)
			throws IOException {
		String src = srcArg;
		String dst = dstArg;
	 ...
	 * @deprecated See
	 *             {@link #renameToInt(FSDirectory, String, String, boolean, Options.Rename...)}
	 */
	@Deprecated
	static boolean unprotectedRenameTo(FSDirectory fsd, String src, String dst, final INodesInPath srcIIP,
			final INodesInPath dstIIP, long timestamp) throws IOException {
		...
		NameNode.stateChangeLog
				.warn("DIR* FSDirectory.unprotectedRenameTo: " + "failed to rename " + src + " to " + dst);
		return false;
	}
	 ...
	 */
	static Map.Entry<BlocksMapUpdateInfo, HdfsFileStatus> renameToInt(FSDirectory fsd, final String srcArg,
			final String dstArg, boolean logRetryCache, Options.Rename... options) throws IOException {
		String src = srcArg;
		String dst = dstArg;
		if (NameNode.stateChangeLog.isDebugEnabled()) {
	 ...
	 */
	static void renameTo(FSDirectory fsd, FSPermissionChecker pc, String src, String dst,
			BlocksMapUpdateInfo collectedBlocks, boolean logRetryCache, Options.Rename... options) throws IOException {
		final INodesInPath srcIIP = fsd.getINodesInPath4Write(src, false);
		final INodesInPath dstIIP = fsd.getINodesInPath4Write(dst, false);
	 ...
	 * {@link org.apache.hadoop.hdfs.server .namenode.FSEditLogLoader} only.
	 * <br>
	 *
	 * @param fsd
	 *            FSDirectory
		...
		try {
			...
			if (tx.addSourceToDestination()) {
				...
				if (snapshottableDirs.size() > 0) {
					// There are snapshottable directories (without snapshots)
					// to be
	...
	@Deprecated
	@SuppressWarnings("deprecation")
	private static boolean renameTo(FSDirectory fsd, FSPermissionChecker pc, String src, String dst,
	...
	private static void validateOverwrite(String src, String dst, boolean overwrite, INode srcInode, INode dstInode)
			throws IOException {
	...
	private static void validateRenameSource(INodesInPath srcIIP) throws IOException {
		...
		// srcInode and its subtree cannot contain snapshottable directories
		// with
		// snapshots
	...
	private static class RenameOperation {
		...
		RenameOperation(FSDirectory fsd, String src, String dst, INodesInPath srcIIP, INodesInPath dstIIP)
				throws QuotaExceededException {
			this.fsd = fsd;
			this.src = src;
			this.dst = dst;
			...
			// snapshot is taken on the dst tree, changes will be recorded in
			// the
			...
			if (isSrcInSnapshot) {
				final INodeReference.WithName withName = srcParent.replaceChild4ReferenceWithName(srcChild,
						srcLatestSnapshotId);
		...
		boolean removeSrc4OldRename() {
			...
			if (removedSrc == -1) {
				NameNode.stateChangeLog.warn("DIR* FSDirRenameOp.unprotectedRenameTo: " + "failed to rename " + src
		...
		boolean addSourceToDestination() {
			...
			if (withCount == null) {
				...
				toDst = new INodeReference.DstReference(dstParent.asDirectory(), withCount,
		...
		void restoreSource() throws QuotaExceededException {
			...
			if (withCount == null) {
				...
				// the withCount node will no longer be used thus no need to
				// update
			...
			if (isSrcInSnapshot) {
				...
				// srcParent is not an INodeDirectoryWithSnapshot, we only need
				// to add
				// the srcChild back
		...
		void restoreDst(BlockStoragePolicySuite bsps) throws QuotaExceededException {
			...
			if (oldDstChild != null && oldDstChild.isReference()) {
				...
				final INodeReference.WithCount wc = (INodeReference.WithCount) removedDstRef.getReferredINode()
						.asReference();
		...
		boolean cleanDst(BlockStoragePolicySuite bsps, BlocksMapUpdateInfo collectedBlocks)
			...
			if (!oldDstChild.isInLatestSnapshot(dstIIP.getLatestSnapshotId())) {
				...
				filesDeleted = oldDstChild.cleanSubtree(bsps, Snapshot.CURRENT_STATE_ID, dstIIP.getLatestSnapshotId(),
						collectedBlocks, removedINodes).getNameSpace() >= 0;
			}
