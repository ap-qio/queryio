...
/**
 * Helper classes for reading the ops from an InputStream. All ops derive from
 * FSEditLogOp and are only instantiated from Reader#readOp()
 */
...
public abstract class FSEditLogOp {
	...
	final public static class OpInstanceCache {
		private final EnumMap<FSEditLogOpCodes, FSEditLogOp> inst = new EnumMap<FSEditLogOpCodes, FSEditLogOp>(
	...
	void readRpcIdsFromXml(Stanza st) {
		this.rpcClientId = st.hasChildren("RPC_CLIENTID") ? ClientId.toBytes(st.getValue("RPC_CLIENTID"))
				: RpcConstants.DUMMY_CLIENT_ID;
		this.rpcCallId = st.hasChildren("RPC_CALLID") ? Integer.parseInt(st.getValue("RPC_CALLID"))
	...
	private static void appendRpcIdsToXml(ContentHandler contentHandler, final byte[] clientId, final int callId)
			throws SAXException {
	...
	private static final class AclEditLogUtil {
		...
		private static List<AclEntry> read(DataInputStream in, int logVersion) throws IOException {
			...
			for (int i = 0; i < size; ++i) {
				...
				aclEntries.add(new AclEntry.Builder().setName(name).setPermission(FSACTION_VALUES[p])
		...
		private static void write(List<AclEntry> aclEntries, DataOutputStream out) throws IOException {
			...
			for (AclEntry e : aclEntries) {
				...
				int v = (e.getScope().ordinal() << ACL_EDITLOG_ENTRY_SCOPE_OFFSET)
						| (e.getType().ordinal() << ACL_EDITLOG_ENTRY_TYPE_OFFSET) | e.getPermission().ordinal();
	...
	static abstract class AddCloseOp extends FSEditLogOp implements BlockListUpdatingOp {
		...
		void readFields(DataInputStream in, int logVersion) throws IOException {
			...
			if ((-17 < logVersion && length != 4) || (logVersion <= -17 && length != 5
					&& !NameNodeLayoutVersion.supports(LayoutVersion.Feature.EDITLOG_OP_OPTIMIZATION, logVersion))) {
	...
	static class AddBlockOp extends FSEditLogOp {
		...
		public String toString() {
			...
			sb.append("AddBlockOp [path=").append(path).append(", penultimateBlock=")
					.append(penultimateBlock == null ? "NULL" : penultimateBlock).append(", lastBlock=")
					.append(lastBlock);
		...
		void readFields(DataInputStream in, int logVersion) throws IOException {
			...
			if (srcSize < 0) {
				throw new IOException("Incorrect data format. "
						+ "ConcatDeleteOp cannot have a negative number of data " + " sources.");
			} else if (srcSize > MAX_CONCAT_SRC) {
				throw new IOException("Incorrect data format. " + "ConcatDeleteOp can have at most " + MAX_CONCAT_SRC
	...
	static class MkdirOp extends FSEditLogOp {
		...
		public void writeFields(DataOutputStream out) throws IOException {
			...
			FSImageSerialization.writeLong(timestamp, out); // atime, unused at
															// this
			permissions.write(out);
			AclEditLogUtil.write(aclEntries, out);
			XAttrEditLogProto.Builder b = XAttrEditLogProto.newBuilder();
	...
	/**
	 * Operation corresponding to creating a snapshot. {@literal @AtMostOnce}
	 * for {@link ClientProtocol#createSnapshot}.
	 */
	...
	static class RollingUpgradeOp extends FSEditLogOp { // @Idempotent
		...
		@Override
		public String toString() {
			return new StringBuilder().append("RollingUpgradeOp [").append(name).append(", time=").append(time)
					.append("]").toString();
		}
	...
	public static class Writer {
		...
		public void writeOp(FSEditLogOp op) throws IOException {
			...
			// write the op code first to make padding and terminator
			// verification
			// work
			...
			// write the length back: content of the op + 4 bytes checksum -
			// op_code
	...
	public static class Reader {
		 ...
		 * @param logVersion
		 *            The version of the data coming from the stream.
		 */
		public Reader(DataInputStream in, StreamLimiter limiter, int logVersion) {
			...
			// It is possible that the logVersion is actually a future
			// layoutversion
			// during the rolling upgrade (e.g., the NN gets upgraded first). We
			// assume future layout will also support length of editlog op.
			this.supportEditLogLength = NameNodeLayoutVersion.supports(NameNodeLayoutVersion.Feature.EDITLOG_LENGTH,
					logVersion) || logVersion < NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION;
		 ...
		 * 
		 * Note that the objects returned from this method may be re-used by
		 * future calls to the same method.
		 * 
		 * @param skipBrokenEdits
		 *            If true, attempt to skip over damaged parts of the input
		 *            stream, rather than throwing an IOException
		 * @return the operation read from the stream, or null at the end of the
		 *         file
		 * @throws IOException
		 *             on error. This function should only throw an exception
		 *             when skipBrokenEdits is false.
		 */
		public FSEditLogOp readOp(boolean skipBrokenEdits) throws IOException {
			while (true) {
				try {
					...
					// FSEditLogOp#decodeOp is not supposed to throw
					// RuntimeException.
					// However, we handle it here for recovery mode, just to be
					// more
					// robust.
					in.reset();
					if (!skipBrokenEdits) {
		...
		private void verifyTerminator() throws IOException {
			 ...
			 * If it contains other bytes, the log itself may be corrupt. It is
			 * important to check this; if we don't, a stray OP_INVALID byte
			 * could make us stop reading the edit log halfway through, and we'd
			 * never know that we had lost data.
			 */
			...
			while (true) {
				try {
					...
					// After reading each group of bytes, we reposition the mark
					// one
					// byte before the next group. Similarly, if there is an
					// error, we
		 ...
		 *
		 *         If an exception is thrown, the stream's mark will be set to
		 *         the first problematic byte. This usually means the beginning
		 *         of the opcode.
		 */
		...
		/**
		 * Similar with decodeOp(), but instead of doing the real decoding, we
		 * skip the content of the op if the length of the editlog is supported.
		 * 
	...
	public static DelegationTokenIdentifier delegationTokenFromXml(Stanza st) throws InvalidXmlException {
		...
		DelegationTokenIdentifier token = new DelegationTokenIdentifier(new Text(owner), new Text(renewer),
				new Text(realuser));
