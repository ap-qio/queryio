...
class FSDirConcatOp {
	...
	static HdfsFileStatus concat(FSDirectory fsd, String target, String[] srcs, boolean logRetryCache)
			throws IOException {
	...
	private static void verifyTargetFile(FSDirectory fsd, final String target, final INodesInPath targetIIP)
			throws IOException {
		// check the target
	...
	private static INodeFile[] verifySrcFiles(FSDirectory fsd, String[] srcs, INodesInPath targetIIP,
		...
		for (String src : srcs) {
			...
			if (srcINode.isReference()
					&& ((INodeReference.WithCount) srcINode.asReference().getReferredINode()).getReferenceCount() > 1) {
			...
			// source file's preferred block size cannot be greater than the
			// target
			// file
	 ...
	 *            FSDirectory
	 */
	static void unprotectedConcat(FSDirectory fsd, INodesInPath targetIIP, INodeFile[] srcList, long timestamp)
			throws IOException {
