...
abstract class AbstractINodeDiffList<N extends INode, A extends INodeAttributes, D extends AbstractINodeDiff<N, A, D>>
	 ...
	 */
	public final QuotaCounts deleteSnapshotDiff(BlockStoragePolicySuite bsps, final int snapshot, final int prior,
			final N currentINode, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
		...
		if (snapshotIndex == 0) {
			...
			if (previous.getSnapshotId() != prior) {
				...
				counts.add(previous.combinePosteriorAndCollectBlocks(bsps, currentINode, removed, collectedBlocks,
						removedINodes));
	 ...
	 * 
	 * @param anchorId
	 *            The returned snapshot's id must be <= or < this given snapshot
	 *            id.
	 * @param exclusive
	 *            True means the returned snapshot's id must be < the given id,
	 *            otherwise <=.
	...
	/**
	 * Search for the snapshot whose id is 1) no less than the given id, and 2)
	...
	final int[] changedBetweenSnapshots(Snapshot from, Snapshot to) {
		...
		if (laterDiffIndex == -1 || laterDiffIndex == 0) {
			// if the laterSnapshot is the earliest SnapshotDiff stored in
			// diffs, or
	...
	/**
	 * @return the inode corresponding to the given snapshot. Note that the
	 *         current inode is returned if there is no change between the given
	...
	final D checkAndAddLatestSnapshotDiff(int latestSnapshotId, N currentINode) {
		final D last = getLast();
		return (last != null && Snapshot.ID_INTEGER_COMPARATOR.compare(last.getSnapshotId(), latestSnapshotId) >= 0)
