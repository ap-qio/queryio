...
public class DatanodeManager {
	...
	/**
	 * Whether or not to avoid using stale DataNodes for writing. Note that,
	 * even if this is configured, the policy may be temporarily disabled when a
	 * high percentage of the nodes are marked as stale.
	 */
	...
	/**
	 * Whether we should tell datanodes what to cache in replies to heartbeat
	 * messages.
	 */
	private boolean shouldSendCachingCommands = false;

	/**
	 * The number of datanodes for each software version. This list should
	 * change during rolling upgrades. Software version -> Number of datanodes
	 * with this version
	 */
	private HashMap<String, Integer> datanodesSoftwareVersions = new HashMap<String, Integer>(4, 0.75f);

	/**
	 * The minimum time between resending caching directives to Datanodes, in
	 * milliseconds.
	 *
	 * Note that when a rescan happens, we will send the new directives as soon
	 * as possible. This timeout only applies to resending directives that we've
	 * already sent.
	 */
	private final long timeBetweenResendingCachingDirectivesMs;

	DatanodeManager(final BlockManager blockManager, final Namesystem namesystem, final Configuration conf)
			throws IOException {
		...
		this.defaultXferPort = NetUtils.createSocketAddr(
				...
				.getPort();
		this.defaultInfoPort = NetUtils.createSocketAddr(conf.getTrimmed(DFSConfigKeys.DFS_DATANODE_HTTP_ADDRESS_KEY,
				DFSConfigKeys.DFS_DATANODE_HTTP_ADDRESS_DEFAULT)).getPort();
		this.defaultInfoSecurePort = NetUtils
				...
				.getPort();
		this.defaultIpcPort = NetUtils.createSocketAddr(conf.getTrimmed(DFSConfigKeys.DFS_DATANODE_IPC_ADDRESS_KEY,
		...
		this.dnsToSwitchMapping = ReflectionUtils
				.newInstance(conf.getClass(DFSConfigKeys.NET_TOPOLOGY_NODE_SWITCH_MAPPING_IMPL_KEY,
						ScriptBasedMapping.class, DNSToSwitchMapping.class), conf);

		this.rejectUnresolvedTopologyDN = conf.getBoolean(DFSConfigKeys.DFS_REJECT_UNRESOLVED_DN_TOPOLOGY_MAPPING_KEY,
		...
		final long heartbeatIntervalSeconds = conf.getLong(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY,
				DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_DEFAULT);
		final int heartbeatRecheckInterval = conf.getInt(DFSConfigKeys.DFS_NAMENODE_HEARTBEAT_RECHECK_INTERVAL_KEY,
		...
		this.avoidStaleDataNodesForRead = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_AVOID_STALE_DATANODE_FOR_READ_KEY,
		...
		Preconditions.checkArgument((ratioUseStaleDataNodesForWrite > 0 && ratioUseStaleDataNodesForWrite <= 1.0f),
				DFSConfigKeys.DFS_NAMENODE_USE_STALE_DATANODE_FOR_WRITE_RATIO_KEY + " = '"
	...
	private static long getStaleIntervalFromConf(Configuration conf, long heartbeatExpireInterval) {
		...
		Preconditions.checkArgument(staleInterval > 0, DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_KEY + " = '"
		...
		final long minStaleInterval = conf.getInt(DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_MINIMUM_INTERVAL_KEY,
				DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_MINIMUM_INTERVAL_DEFAULT) * heartbeatIntervalSeconds * 1000;
		if (staleInterval < minStaleInterval) {
			LOG.warn("The given interval for marking stale datanode = " + staleInterval + ", which is less than "
					...
					+ "due to temporary short-term failures. Reset stale interval to " + minStaleInterval + ".");
			staleInterval = minStaleInterval;
		}
		if (staleInterval > heartbeatExpireInterval) {
			LOG.warn("The given interval for marking stale datanode = " + staleInterval
					+ ", which is larger than heartbeat expire interval " + heartbeatExpireInterval + ".");
		}
	...
	public void sortLocatedBlocks(final String targethost, final List<LocatedBlock> locatedblocks) {
		...
		Comparator<DatanodeInfo> comparator = avoidStaleDataNodesForRead
				? new DFSUtil.DecomStaleComparator(staleInterval) : DFSUtil.DECOM_COMPARATOR;
	 ...
	 * the same, or if it doesn't find the datanode, it looks for a machine
	 * local and then rack local datanode, if a rack local datanode is not
	 * possible either, it returns the DatanodeDescriptor of any random node in
	 * the cluster.
	 *
	 * @param address
	...
	public DatanodeStorageInfo[] getDatanodeStorageInfos(DatanodeID[] datanodeID, String[] storageIDs)
			throws UnregisteredNodeException {
	...
	private void wipeDatanode(final DatanodeID node) {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(getClass().getSimpleName() + ".wipeDatanode(" + node + "): storage " + key
	 ...
	 * 
	 * @param node
	 *            to resolve to network location
	...
	private String resolveNetworkLocationWithFallBackToDefaultLocation(DatanodeID node) {
		...
		try {
			...
			LOG.error("Unresolved topology mapping. Using " + NetworkTopology.DEFAULT_RACK + " for host "
					+ node.getHostName());
	 ...
	 *             if the DNS to switch mapping fails to resolve network
	 *             location.
	 */
	...
	private List<String> getNetworkDependenciesWithDefault(DatanodeInfo node) {
		...
		try {
			...
			LOG.error("Unresolved dependency mapping for host " + node.getHostName()
	 ...
	 * to not display an already decommssioned data node to the operators. The
	 * operation procedure of making a already decommissioned data node not to
	 * be displayed is as following:
	 * <ol>
	 * <li>Host must have been in the include hosts list and the include hosts
	 * list must not be empty.</li>
	 * <li>Host is decommissioned by remaining in the include hosts list and
	 * added into the exclude hosts list. Name node is updated with the new
	 * information by issuing dfsadmin -refreshNodes command.</li>
	 * <li>Host is removed from both include hosts and exclude hosts lists. Name
	...
	/**
	 * Register the given datanode with the namenode. NB: the given registration
	 * is mutated and given back to the datanode.
	 *
	 * @param nodeReg
	 *            the datanode registration
	 * @throws DisallowedDatanodeException
	 *             if the registration request is denied because the datanode
	 *             does not match includes/excludes
	 * @throws UnresolvedTopologyException
	 *             if the registration request is denied because resolving
	...
	public void registerDatanode(DatanodeRegistration nodeReg)
		...
		if (dnAddress != null) {
			...
			if (checkIpHostnameInRegistration && !isNameResolved(dnAddress)) {
				// Reject registration of unresolved datanode to prevent
				// performance
			...
			// update node registration with the ip and hostname from rpc
			// request
		...
		try {
			...
			if (nodeS != null) {
				if (nodeN == nodeS) {
					// The same datanode has been just restarted to serve the
					// same data
					// storage. We do not need to remove old data blocks, the
					// delta will
					 ...
					 * existing data storage, which from now on will be served
					 * by a new node. If this message repeats, both nodes might
					 * have same storageID by (insanely rare) random chance.
					 * User needs to restart one of the nodes with its data
					 * cleared (or user can just remove the StorageID value in
					 * "VERSION" file under the data directory of the datanode,
					 * but this is might not work if VERSION file format has
					 * changed
					 */
	...
	/**
	 * Rereads conf to get hosts and exclude list file names. Rereads the files
	 * to update the hosts and exclude lists. It checks if any of the hosts have
	 * changed states:
	 */
	...
	private void refreshHostsReader(Configuration conf) throws IOException {
		...
		// Update the file names and refresh internal includes and excludes
		// list.
		if (conf == null) {
	...
	public boolean shouldAvoidStaleDataNodesForWrite() {
		...
		return avoidStaleDataNodesForWrite
				&& (numStaleNodes <= heartbeatManager.getLiveDatanodeCount() * ratioUseStaleDataNodesForWrite);
	}
	 ...
	 *
	 * @param numStaleStorages
	 *            The number of content stale storages.
	 */
	...
	public void fetchDatanodes(final List<DatanodeDescriptor> live, final List<DatanodeDescriptor> dead,
	...
	/**
	 * @return true if this cluster has ever consisted of multiple racks, even
	 *         if it is not now a multi-rack cluster.
	 */
	 ...
	 * 
	 * @param node
	 *            DN which caused cluster to become multi-rack. Used for
	 *            logging.
	 */
	@VisibleForTesting
	...
	public List<DatanodeDescriptor> getDatanodeListForReport(final DatanodeReportType type) {
		...
		if (listDeadNodes) {
			for (InetSocketAddress addr : includedNodes) {
				...
				// We can't ask the DataNode what it had configured, because
				// it's
				// dead.
				DatanodeDescriptor dn = new DatanodeDescriptor(new DatanodeID(addr.getAddress().getHostAddress(),
						addr.getHostName(), "", addr.getPort() == 0 ? defaultXferPort : addr.getPort(), defaultInfoPort,
	...
	public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg, StorageReport[] reports,
			final String blockPoolId, long cacheCapacity, long cacheUsed, int xceiverCount, int maxTransfers,
		...
		synchronized (heartbeatManager) {
			synchronized (datanodeMap) {
				...
				// If we are in safemode, do not send back any recovery /
				// replication
				...
				if (blocks != null) {
					...
					for (BlockInfoContiguousUnderConstruction b : blocks) {
						...
						// Skip stale nodes during recovery - not heart beated
						// for some time (30s by default).
						final List<DatanodeStorageInfo> recoveryLocations = new ArrayList<DatanodeStorageInfo>(
								storages.length);
						...
						// If we are performing a truncate recovery than set
						// recovery fields
						// to old block.
						...
						// If we only get 1 replica after eliminating stale
						// nodes, then choose all
						// replicas for recovery and let the primary data node
						// handle failures.
						...
						if (recoveryLocations.size() > 1) {
							...
							// If too many replicas are stale, then choose all
							// replicas to participate
							// in block recovery.
				...
				if (shouldSendCachingCommands && ((nowMs
						- nodeinfo.getLastCachingDirectiveSentTimeMs()) >= timeBetweenResendingCachingDirectivesMs)) {
					DatanodeCommand pendingCacheCommand = getCacheCommand(nodeinfo.getPendingCached(), nodeinfo,
							DatanodeProtocol.DNA_CACHE, blockPoolId);
					if (pendingCacheCommand != null) {
						cmds.add(pendingCacheCommand);
						sendingCachingCommands = true;
					}
					DatanodeCommand pendingUncacheCommand = getCacheCommand(nodeinfo.getPendingUncached(), nodeinfo,
	 ...
	 *
	 * @param list
	 *            The {@link CachedBlocksList}. This function clears the list.
	 * @param datanode
	 *            The datanode.
	 * @param action
	 *            The action to perform in the command.
	 * @param poolId
	 *            The block pool id.
	 * @return A DatanodeCommand to be sent back to the DN, or null if there is
	 *         nothing to be done.
	 */
	private DatanodeCommand getCacheCommand(CachedBlocksList list, DatanodeDescriptor datanode, int action,
			String poolId) {
	 ...
	 * (dfs.datanode.balance.bandwidthPerSec) dynamically by calling "dfsadmin
	 * -setBalanacerBandwidth newbandwidth", at which point the following
	 * 'bandwidth' variable gets updated with the new value for each node. Once
	 * the heartbeat command is issued to update the value on the specified
	...
	/**
	 * Clear any actions that are queued up to be sent to the DNs on their next
	 * heartbeats. This includes block invalidations, recoveries, and
