...
class BlockReceiver implements Closeable {
	...
	/**
	 * In the case that the client is writing with a different checksum
	 * polynomial than the block is stored with on disk, the DataNode needs to
	 * recalculate checksums before writing.
	 */
	...
	BlockReceiver(final ExtendedBlock block, final StorageType storageType, final DataInputStream in,
			final String inAddr, final String myAddr, final BlockConstructionStage stage, final long newGs,
			final long minBytesRcvd, final long maxBytesRcvd, final String clientname, final DatanodeInfo srcDataNode,
			final DataNode datanode, DataChecksum requestedChecksum, CachingStrategy cachingStrategy,
			final boolean allowLazyPersist, final boolean pinning) throws IOException {
		try {
			this.block = block;
			this.in = in;
			this.inAddr = inAddr;
			...
			if (isDatanode) { // replication or move
				...
				default:
					throw new IOException(
							"Unsupported stage " + stage + " while receiving block " + block + " from " + inAddr);
				}
			}
			...
			this.checksumOut = new DataOutputStream(
					new BufferedOutputStream(streams.getChecksumOut(), HdfsConstants.SMALL_BUFFER_SIZE));
	 ...
	 * this datanode unless it is caused by interruption.
	 */
	private void handleMirrorOutError(IOException ioe) throws IOException {
		String bpid = block.getBlockPoolId();
		LOG.info(datanode.getDNRegistrationForBP(bpid) + ":Exception writing " + block + " to mirror " + mirrorAddr,
				ioe);
	...
	/**
	 * Translate CRC chunks from the client's checksum implementation to the
	 * disk checksum implementation.
	 * 
	 * This does not verify the original checksums, under the assumption that
	...
	/**
	 * Check whether checksum needs to be verified. Skip verifying checksum iff
	 * this is not the last one in the pipeline and clientName is non-null. i.e.
	 * Checksum is verified on all the datanodes when the data is being written
	 * by a datanode rather than a client. Whe client is writing the data,
	...
	/**
	 * Receives and processes a packet. It can contain many chunks. returns the
	 * number of data bytes that the packet has.
	 */
	private int receivePacket() throws IOException {
		...
		if (lastPacketInBlock || len == 0) {
			...
			if (checksumReceivedLen > 0 && shouldVerifyChecksum()) {
				try {
					...
					// checksum error detected locally. there is no reason to
					// continue.
					if (responder != null) {
						try {
							((PacketResponder) responder.getRunnable()).enqueue(seqno, lastPacketInBlock, offsetInBlock,
			...
			try {
				...
				if (onDiskLen < offsetInBlock) {
					// Normally the beginning of an incoming packet is aligned
					// with the
					// existing data on disk. If the beginning packet data
					// offset is not
					// checksum chunk aligned, the end of packet will not go
					// beyond the
					// next chunk boundary.
					// When a failure-recovery is involved, the client state and
					// the
					// the datanode state may not exactly agree. I.e. the client
					// may
					// resend part of data that is already on disk. Correct
					// number of
					// bytes should be skipped when writing the data and
					// checksum
					// buffers out to disk.
					...
					// Since data is always appended, not overwritten, partial
					// CRC
					// recalculation is necessary if the on-disk data is not
					// chunk-
					// aligned, regardless of whether the beginning of the data
					// in
					...
					// aligned, the packet should terminate at or before the
					// next
					// chunk boundary.
					if (!alignedInPacket && len > bytesPerChecksum) {
						throw new IOException("Unexpected packet data length for " + block + " from " + inAddr
								+ ": a partial chunk must be " + " sent in an individual packet (data length = " + len
								+ " > bytesPerChecksum = " + bytesPerChecksum + ")");
					}

					// If the last portion of the block file is not a full
					// chunk,
					// then read in pre-existing partial data chunk and
					// recalculate
					// the checksum so that the checksum calculation can
					// continue
					...
					if (doPartialCrc) {
						if (LOG.isDebugEnabled()) {
							LOG.debug(
									"receivePacket for " + block + ": previous write did not end at the chunk boundary."
											+ " onDiskLen=" + onDiskLen);
						}
					...
					// The data buffer position where write will begin. If the
					// packet
					// data and on-disk data have no overlap, this will not be
					// at the
					...
					if (shouldNotWriteChecksum) {
						...
						// First, overwrite the partial crc at the end, if
						// necessary.
						...
						// Determine how many checksums need to be skipped up to
						// the last
						// boundary. The checksum after the boundary was already
						// counted
						// above. Only count the number of checksums skipped up
						// to the
						// boundary here.
						...
						// I.e. a partial chunk checksum rewrite happened and
						// there is no
		...
		/*
		 * Send in-progress responses for the replaceBlock() calls back to
		 * caller to avoid timeouts due to balancer throttling. HDFS-6247
		 */
			...
			if (outFd != null && offsetInBlock > lastCacheManagementOffset + CACHE_DROP_LAG_BYTES) {
				...
				if (syncBehindWrites) {
					if (syncBehindWritesInBackground) {
						this.datanode.getFSDataset().submitBackgroundSyncFileRangeRequest(block, outFd,
								lastCacheManagementOffset, offsetInBlock - lastCacheManagementOffset,
								NativeIO.POSIX.SYNC_FILE_RANGE_WRITE);
					} else {
						NativeIO.POSIX.syncFileRangeIfPossible(outFd, lastCacheManagementOffset,
				...
				if (dropPos > 0 && dropCacheBehindWrites) {
					NativeIO.POSIX.getCacheManipulator().posixFadviseIfPossible(block.getBlockName(), outFd, 0, dropPos,
	...
	void receiveBlock(DataOutputStream mirrOut, // output to next datanode
			...
			String mirrAddr, DataTransferThrottler throttlerArg, DatanodeInfo[] downstreams, boolean isReplaceBlock)
			throws IOException {
		...
		try {
			...
			if (datanode.isRestarting()) {
				// Do not throw if shutting down for restart. Otherwise, it will
				// cause
			...
			// If a shutdown for restart was initiated, upstream needs to be
			// notified.
			// There is no need to do anything special if the responder was
			// closed
			// normally.
				...
				if (responder != null) {
					...
					if (datanode.isRestarting() && isClient && !isTransfer) {
						...
						try (Writer out = new OutputStreamWriter(new FileOutputStream(restartMeta), "UTF-8")) {
							...
							// The worst case is not recovering this RBW
							// replica.
							// Client will fall back to regular pipeline
							// recovery.
						} finally {
							IOUtils.cleanup(LOG, out);
						}
						try {
							// Even if the connection is closed after the ack
							// packet is
							// flushed, the client can react to the connection
							// closure
							// first. Insert a delay to lower the chance of
							// client
							// missing the OOB ack.
							Thread.sleep(1000);
			...
			if (responder != null) {
				try {
					responder.interrupt();
					// join() on the responder should timeout a bit earlier than
					// the
					// configured deadline. Otherwise, the join() on this thread
					// will
	...
	private Checksum computePartialChunkCrc(long blkoff, long ckoff) throws IOException {
		...
		final Checksum partialCrc = DataChecksum.newDataChecksum(diskChecksum.getChecksumType(),
	...
	/**
	 * Processes responses from downstream datanodes in the pipeline and sends
	 * back replies to the originator.
	 */
	class PacketResponder implements Runnable, Closeable {
		/**
		 * queue for packets waiting for ack - synchronization using monitor
		 * lock
		 */
		...
		PacketResponder(final DataOutputStream upstreamOut, final DataInputStream downstreamIn,
			...
			final StringBuilder b = new StringBuilder(getClass().getSimpleName()).append(": ").append(block)
		 ...
		 *            if true, this is the last packet in block
		 * @param offsetInBlock
		 *            offset of this packet in block
		 */
		...
		/**
		 * Send an OOB response. If all acks have been sent already for the
		 * block and the responder is about to close, the delivery is not
		 * guaranteed. This is because the other end can close the connection
		 * independently. An OOB coming from downstream will be automatically
		 * relayed upstream by the responder. This method is used only by
		 * originating datanode.
		 *
		 * @param ackStatus
			...
			while (isRunning() && !lastPacketInBlock) {
				...
				try {
					...
					try {
						...
						if (seqno != PipelineAck.UNKOWN_SEQNO || type == PacketResponderType.LAST_IN_PIPELINE) {
							...
							if (type == PacketResponderType.HAS_DOWNSTREAM_IN_PIPELINE) {
								...
								// downstream
								// nodes.
								// The value is 0 if this responder doesn't have
								// a downstream
								// DN in the pipeline.
								totalAckTimeNanos = ackRecvNanoTime - pkt.ackEnqueueNanoTime;
								// Report the elapsed time from ack send to ack
								// receive minus
					...
					if (Thread.interrupted() || isInterrupted) {
						 ...
						 * upstream datanode will detect that this datanode is
						 * bad, and rightly so.
						 *
						 * The receiver thread can also interrupt this thread
						 * for sending an out-of-band response upstream.
						 */
					...
					if (running) {
						...
						LOG.info(myString, e);
						running = false;
						if (!Thread.interrupted()) { // failure not caused by
														// interruption
		 ...
		 * 
		 * @param startTime
		 *            time when BlockReceiver started receiving the block
		 */
		private void finalizeBlock(long startTime) throws IOException {
			...
			if (ClientTraceLog.isInfoEnabled() && isClient) {
				...
				ClientTraceLog.info(String.format(DN_CLIENTTRACE_FORMAT, inAddr, myAddr, block.getNumBytes(),
						"HDFS_WRITE", clientname, offset, dnR.getDatanodeUuid(), block, endTime - startTime));
			} else {
		...
		/**
		 * The wrapper for the unprotected version. This is only called by the
		 * responder's run() method.
		 *
		 * @param ack
		 *            Ack received from downstream
		 * @param seqno
		 *            sequence number of ack to be sent upstream
		 * @param totalAckTimeNanos
		 *            total ack time including all the downstream nodes
		 * @param offsetInBlock
		 *            offset in block for the data in packet
		 * @param myHeader
		 *            the local ack header
		 */
		private void sendAckUpstream(PipelineAck ack, long seqno, long totalAckTimeNanos, long offsetInBlock,
			...
			try {
				// Wait for other sender to finish. Unless there is an OOB being
				// sent,
		...
		private void sendAckUpstreamUnprotected(PipelineAck ack, long seqno, long totalAckTimeNanos, long offsetInBlock,
			...
			final int[] replies;
			if (ack == null) {
				// A new OOB response is being sent from this node. Regardless
				// of
				...
				if (ackLen > 0 && PipelineAck.getStatusFromHeader(replies[1]) == Status.ERROR_CHECKSUM) {
					throw new IOException("Shutting down writer and responder "
							+ "since the down streams reported the data sent by this " + "thread is corrupt");
				}
			}
			...
			if (duration > datanodeSlowLogThresholdMs) {
				LOG.warn("Slow PacketResponder send ack to upstream took " + duration + "ms (threshold="
						+ datanodeSlowLogThresholdMs + "ms), " + myString + ", replyAck=" + replyAck);
			} else if (LOG.isDebugEnabled()) {
				LOG.debug(myString + ", replyAck=" + replyAck);
			}

			// If a corruption was detected in the received data, terminate
			// after
			// sending ERROR_CHECKSUM back.
			Status myStatus = PipelineAck.getStatusFromHeader(myHeader);
			if (myStatus == Status.ERROR_CHECKSUM) {
				throw new IOException("Shutting down writer and responder "
						+ "due to a checksum error in received data. The error " + "response has been sent upstream.");
			}
		}
	...
	private static class Packet {
		...
		@Override
		public String toString() {
			return getClass().getSimpleName() + "(seqno=" + seqno + ", lastPacketInBlock=" + lastPacketInBlock
