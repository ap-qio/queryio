 ...
 * directories of the NN and enforcing a retention policy on checkpoints and
 * edit logs.
 * 
 * It delegates the actual removal of files to a StoragePurger implementation,
 * which might delete the files or instead copy them to a filer or HDFS for
 * later analysis.
 */
public class NNStorageRetentionManager {
	...
	public NNStorageRetentionManager(Configuration conf, NNStorage storage, LogsPurgeable purgeableLogs,
		...
		this.numCheckpointsToRetain = conf.getInt(DFSConfigKeys.DFS_NAMENODE_NUM_CHECKPOINTS_RETAINED_KEY,
				DFSConfigKeys.DFS_NAMENODE_NUM_CHECKPOINTS_RETAINED_DEFAULT);
		this.numExtraEditsToRetain = conf.getLong(DFSConfigKeys.DFS_NAMENODE_NUM_EXTRA_EDITS_RETAINED_KEY,
	...
	void purgeOldStorage(NameNodeFile nnf) throws IOException {
		...
		// new images.
		//
		// First, determine the target number of extra transactions to retain
		// based
		...
		// Next, adjust the number of transactions to retain if doing so would
		// mean
		...
		// Finally, ensure that we're not trying to purge any transactions that
		// we
		// actually need.
		if (purgeLogsFrom > minimumRequiredTxId) {
			throw new AssertionError("Should not purge more edits than required to " + "restore: " + purgeLogsFrom
					+ " should be <= " + minimumRequiredTxId);
		}
	...
	/**
	 * @param inspector
	 *            inspector that has already inspected all storage dirs
	 * @return the transaction ID corresponding to the oldest checkpoint that
	 *         should be retained.
	 */
	...
	/**
	 * Delete old OIV fsimages. Since the target dir is not a full blown storage
	 * directory, we simply list and keep the latest ones. For the same reason,
	 * no storage inspector is used.
	 */
