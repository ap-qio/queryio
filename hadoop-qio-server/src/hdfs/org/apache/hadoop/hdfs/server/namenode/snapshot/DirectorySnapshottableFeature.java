...
public class DirectorySnapshottableFeature extends DirectoryWithSnapshotFeature {
	...
	/**
	 * Snapshots of this directory in ascending order of snapshot names. Note
	 * that snapshots in ascending order of snapshot id are stored in
	...
	/**
	 * Simply add a snapshot into the {@link #snapshotsByNames}. Used when
	 * loading fsimage.
	 */
	...
	public Snapshot addSnapshot(INodeDirectory snapshotRoot, int id, String name)
		...
		if (n + 1 > snapshotQuota) {
			throw new SnapshotException("Failed to add snapshot: there are already " + n
					+ " snapshot(s) and the snapshot quota is " + snapshotQuota);
		}
	 ...
	 * @param snapshotName
	 *            The name of the snapshot to be removed
	 * @param collectedBlocks
	 *            Used to collect information to update blocksMap
	 * @return The removed snapshot. Null if no snapshot with the given name
	 *         exists.
	 */
	public Snapshot removeSnapshot(BlockStoragePolicySuite bsps, INodeDirectory snapshotRoot, String snapshotName,
		...
		if (i < 0) {
			...
			try {
				...
				if (parent != null) {
					// there will not be any WithName node corresponding to the
					// deleted
					// snapshot, thus only update the quota usage in the current
					// tree
			...
			// remove from snapshotsByNames after successfully cleaning the
			// subtree
	...
	public ContentSummaryComputationContext computeContentSummary(final BlockStoragePolicySuite bsps,
			final INodeDirectory snapshotRoot, final ContentSummaryComputationContext summary) {
	 ...
	 * current directory) of the directory.
	 *
	 * @param from
	 ...
	 * @throws SnapshotException
	 *             If there is no snapshot matching the starting point, or if
	 *             endSnapshotName is not null but cannot be identified as a
	 *             previous snapshot.
	 */
	SnapshotDiffInfo computeDiff(final INodeDirectory snapshotRoot, final String from, final String to)
			throws SnapshotException {
	 ...
	 *
	 * @param snapshotRoot
	 *            The directory where snapshots were taken.
	 * @param snapshotName
	 *            The name of the snapshot.
	 * @return The corresponding snapshot. Null if snapshotName is null or
	 *         empty.
	 * @throws SnapshotException
	 *             If snapshotName is not null or empty, but there is no
	 *             snapshot matching the name.
	 */
	private Snapshot getSnapshotByName(INodeDirectory snapshotRoot, String snapshotName) throws SnapshotException {
		...
		if (snapshotName != null && !snapshotName.isEmpty()) {
			...
			if (index < 0) {
				throw new SnapshotException("Cannot find the snapshot of directory " + snapshotRoot.getFullPathName()
	 ...
	 *            The directory where snapshots were taken.
	 * @param node
	 *            The directory/file under which the diff is computed.
	 * @param parentPath
	 *            Relative path (corresponding to the snapshot root) of the
	 *            node's parent.
	 * @param diffReport
	 *            data structure used to store the diff.
	 */
	private void computeDiffRecursively(final INodeDirectory snapshotRoot, INode node, List<byte[]> parentPath,
		...
		if (node.isDirectory()) {
			...
			for (INode child : children) {
				...
				if (!toProcess && child instanceof INodeReference.WithName) {
					byte[][] renameTargetPath = findRenameTargetPath(snapshotRoot, (WithName) child,
			...
			boolean change = file.getFileWithSnapshotFeature().changedBetweenSnapshots(file, earlierSnapshot,
					laterSnapshot);
			if (change) {
	...
	/**
	 * We just found a deleted WithName node as the source of a rename
	 * operation. However, we should include it in our snapshot diff report as
	 * rename only if the rename target is also under the same snapshottable
	 * directory.
	 */
	private byte[][] findRenameTargetPath(final INodeDirectory snapshotRoot, INodeReference.WithName wn,
				...
				public Iterator<SnapshotAndINode> iterator() {
					return new Iterator<SnapshotAndINode>() {
						...
						public SnapshotAndINode next() {
							final SnapshotAndINode pair = new SnapshotAndINode(next.getSnapshotId(),
									getSnapshotById(next.getSnapshotId()).getRoot());
							next = findNext();
							return pair;
						}
