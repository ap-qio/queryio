...
public final class DirectoryWithQuotaFeature implements INode.Feature {
	...
	public static class Builder {
		...
		public Builder() {
			this.quota = new QuotaCounts.Builder().nameSpace(DEFAULT_NAMESPACE_QUOTA)
					.storageSpace(DEFAULT_STORAGE_SPACE_QUOTA).typeSpaces(DEFAULT_STORAGE_SPACE_QUOTA).build();
	 ...
	 * @param ssQuota
	 *            Storagespace quota to be set
	 * @param type
	 *            Storage type of the storage space quota to be set. To set
	 *            storagespace/namespace quota, type must be null.
	 */
	 ...
	 * 
	 * @param counts
	 *            counts to be added with current quota usage
	...
	private void checkStoragespace(final INodeDirectory dir, final long computed) {
		if (-1 != quota.getStorageSpace() && usage.getStorageSpace() != computed) {
			NameNode.LOG.error("BUG: Inconsistent storagespace for directory " + dir.getFullPathName() + ". Cached = "
					+ usage.getStorageSpace() + " != Computed = " + computed);
		}
	}

	void addSpaceConsumed(final INodeDirectory dir, final QuotaCounts counts, boolean verify)
			throws QuotaExceededException {
		if (dir.isQuotaSet()) {
			// The following steps are important:
			// check quotas in this inode and all ancestors before changing
			// counts
	...
	/**
	 * Sets namespace and storagespace take by the directory rooted at this
	 * INode. This should be used carefully. It does not check for quota
	 * violations.
	 * 
	 * @param namespace
	 *            size of the directory to be set
	 * @param storagespace
	 *            storage space take by all the nodes under this directory
	 * @param typespaces
	 *            counters of storage type usage
	 */
	...
	private void verifyQuotaByStorageType(EnumCounters<StorageType> typeDelta)
		...
		for (StorageType t : StorageType.getTypesSupportingQuota()) {
			...
			if (Quota.isViolated(quota.getTypeSpace(t), usage.getTypeSpace(t), typeDelta.get(t))) {
				throw new QuotaByStorageTypeExceededException(quota.getTypeSpace(t),
						usage.getTypeSpace(t) + typeDelta.get(t), t);
			}
		}
	}
	...
	/**
	 * @throws QuotaExceededException
	 *             if namespace, storagespace or storage type space quota is
	...
	private String storagespaceString() {
		return "storagespace: "
				+ (quota.getStorageSpace() < 0 ? "-" : usage.getStorageSpace() + "/" + quota.getStorageSpace());
	}
	...
	private String typeSpaceString() {
		...
		for (StorageType t : StorageType.getTypesSupportingQuota()) {
			sb.append("StorageType: " + t
					+ (quota.getTypeSpace(t) < 0 ? "-" : usage.getTypeSpace(t) + "/" + usage.getTypeSpace(t)));
		}
		return sb.toString();
	}
