...
/**
 * The Checkpointer is responsible for supporting periodic checkpoints of the
 * HDFS metadata.
 *
 * The Checkpointer is a daemon that periodically wakes up up (determined by the
 * schedule specified in the configuration), triggers a periodic checkpoint and
 * then goes back to sleep.
 * 
 * The start of a checkpoint is triggered by one of the two factors: (1) time or
 * (2) the size of the edits file.
 */
class Checkpointer extends Daemon {
	...
	private void initialize(Configuration conf) throws IOException {
		...
		// Pull out exact http address for posting url to avoid ip aliasing
		// issues
	...
	void doCheckpoint() throws IOException {
		...
		if (!manifest.getLogs().isEmpty()) {
			...
			if (firstRemoteLog.getStartTxId() > lastApplied + 1) {
				...
				bnImage.saveDigestAndRenameCheckpointImage(NameNodeFile.IMAGE, sig.mostRecentCheckpointTxId,
						downloadedHash);
		...
		LOG.info("Checkpoint completed in " + (monotonicNow() - startTime) / 1000 + " seconds." + " New Image Size: "
				+ imageSize);
	}
