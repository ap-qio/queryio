...
class FSDirAclOp {
	static HdfsFileStatus modifyAclEntries(FSDirectory fsd, final String srcArg, List<AclEntry> aclSpec)
			throws IOException {
		String src = srcArg;
	...
	static HdfsFileStatus removeAclEntries(FSDirectory fsd, final String srcArg, List<AclEntry> aclSpec)
			throws IOException {
		String src = srcArg;
		checkAclsConfigFlag(fsd);
		FSPermissionChecker pc = fsd.getPermissionChecker();
	...
	static AclStatus getAclStatus(FSDirectory fsd, String src) throws IOException {
		...
		fsd.readLock();
		try {
			// There is no real inode for the path ending in ".snapshot", so
			// return a
			...
			List<AclEntry> acl = AclStorage
					.readINodeAcl(fsd.getAttributes(src, inode.getLocalNameBytes(), inode, snapshotId));
			...
			return new AclStatus.Builder().owner(inode.getUserName()).group(inode.getGroupName())
					.stickyBit(fsPermission.getStickyBit()).setPermission(fsPermission).addEntries(acl).build();
		} finally {
			fsd.readUnlock();
		}
	}

	static List<AclEntry> unprotectedSetAcl(FSDirectory fsd, String src, List<AclEntry> aclSpec, boolean fromEdits)
			throws IOException {
	...
	private static void unprotectedRemoveAcl(FSDirectory fsd, INodesInPath iip) throws IOException {
		...
		if (featureEntries.get(0).getScope() == AclEntryScope.ACCESS) {
			...
			AclEntry groupEntryKey = new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.GROUP)
					.build();
			int groupEntryIndex = Collections.binarySearch(featureEntries, groupEntryKey,
