...
class InvalidateBlocks {
	...
	private void printBlockDeletionTime(final Logger log) {
		log.info(DFSConfigKeys.DFS_NAMENODE_STARTUP_DELAY_BLOCK_DELETION_SEC_KEY + " is set to "
	 ...
	 *         invalidation. Blocks are compared including their generation
	 *         stamps: if a block is pending invalidation but with a different
	 *         generation stamp, returns false.
	 */
	...
	synchronized void add(final Block block, final DatanodeInfo datanode, final boolean log) {
		...
		if (set.add(block)) {
			numBlocks++;
			if (log) {
				NameNode.blockStateChangeLog.info("BLOCK* {}: add {} to {}", getClass().getSimpleName(), block,
						datanode);
			}
		}
	}
