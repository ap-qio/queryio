...
public class BlockScanner {
	...
	static class Conf {
		...
		@VisibleForTesting
		static final long INTERNAL_DFS_BLOCK_SCANNER_MAX_STALENESS_MS_DEFAULT = TimeUnit.MILLISECONDS.convert(15,
				TimeUnit.MINUTES);
		...
		@VisibleForTesting
		static final long INTERNAL_DFS_BLOCK_SCANNER_CURSOR_SAVE_INTERVAL_MS_DEFAULT = TimeUnit.MILLISECONDS.convert(10,
				TimeUnit.MINUTES);
		 ...
		 *
		 * For compatibility with prior releases of HDFS, if the configured
		 * value is zero then the scan period is set to 3 weeks.
		 *
		 * If the configured value is less than zero then the scanner is
		 * disabled.
		 *
		 * @param conf
		...
		private static long getConfiguredScanPeriodMs(Configuration conf) {
			long tempScanPeriodMs = getUnitTestLong(conf, INTERNAL_DFS_DATANODE_SCAN_PERIOD_MS,
					TimeUnit.MILLISECONDS.convert(
							conf.getLong(DFS_DATANODE_SCAN_PERIOD_HOURS_KEY, DFS_DATANODE_SCAN_PERIOD_HOURS_DEFAULT),
							TimeUnit.HOURS));

			if (tempScanPeriodMs == 0) {
				tempScanPeriodMs = TimeUnit.MILLISECONDS.convert(DFS_DATANODE_SCAN_PERIOD_HOURS_DEFAULT,
						TimeUnit.HOURS);
			}
		...
		Conf(Configuration conf) {
			this.targetBytesPerSec = Math.max(0L, conf.getLong(DFS_BLOCK_SCANNER_VOLUME_BYTES_PER_SECOND,
					DFS_BLOCK_SCANNER_VOLUME_BYTES_PER_SECOND_DEFAULT));
			this.maxStalenessMs = Math.max(0L, getUnitTestLong(conf, INTERNAL_DFS_BLOCK_SCANNER_MAX_STALENESS_MS,
					INTERNAL_DFS_BLOCK_SCANNER_MAX_STALENESS_MS_DEFAULT));
			this.scanPeriodMs = getConfiguredScanPeriodMs(conf);
			this.cursorSaveMs = Math.max(0L, getUnitTestLong(conf, INTERNAL_DFS_BLOCK_SCANNER_CURSOR_SAVE_INTERVAL_MS,
					INTERNAL_DFS_BLOCK_SCANNER_CURSOR_SAVE_INTERVAL_MS_DEFAULT));
			if (allowUnitTestSettings) {
				this.resultHandler = (Class<? extends ScanResultHandler>) conf
						.getClass(INTERNAL_VOLUME_SCANNER_SCAN_RESULT_HANDLER, ScanResultHandler.class);
			} else {
	 ...
	 *
	 * @param bpid
	 *            The block pool id to enable scanning for.
	 */
	 ...
	 *
	 * @param bpid
	 *            The block pool id to disable scanning for.
	 */
	 ...
	 * VolumeScanner keeps a list of recently suspicious blocks, which it uses
	 * to avoid rescanning the same block over and over in a short time frame.
	 *
	 * @param storageId
	 *            The ID of the storage where the block replica is being stored.
	 * @param block
	 *            The block's ID and block pool id.
	 */
	synchronized void markSuspectBlock(String storageId, ExtendedBlock block) {
		if (!isEnabled()) {
			LOG.info("Not scanning suspicious block {} on {}, because the block " + "scanner is disabled.", block,
					storageId);
			return;
		}
		VolumeScanner scanner = scanners.get(storageId);
		if (scanner == null) {
			// This could happen if the volume is in the process of being
			// removed.
