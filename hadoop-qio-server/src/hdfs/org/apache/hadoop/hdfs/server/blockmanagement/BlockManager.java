...
public class BlockManager {
	...
	/**
	 * Mapping: Block -> { BlockCollection, datanodes, self ref } Updated only
	 * in response to client-sent information.
	 */
	...
	/**
	 * After a failover, over-replicated blocks may not be handled until all of
	 * the replicas have done a block report to the new active. This is to make
	 * sure that this NameNode has been notified of all block deletions that
	 * might have been pending when the failover happened.
	 */
	private final Set<Block> postponedMisreplicatedBlocks = Sets.newHashSet();

	/**
	 * Maps a StorageID to the set of blocks that are "extra" for this DataNode.
	 * We'll eventually remove these extras.
	 */
	public final Map<String, LightWeightLinkedSet<Block>> excessReplicateMap = new TreeMap<String, LightWeightLinkedSet<Block>>();

	/**
	 * Store set of Blocks that need to be replicated 1 or more times. We also
	 * store pending replication-orders.
	 */
	 ...
	 * have at one time considering all but the highest priority replications
	 * needed.
	 */
	...
	public BlockManager(final Namesystem namesystem, final Configuration conf) throws IOException {
		...
		this.replicationStreamsHardLimit = conf.getInt(DFSConfigKeys.DFS_NAMENODE_REPLICATION_STREAMS_HARD_LIMIT_KEY,
	...
	private static BlockTokenSecretManager createBlockTokenSecretManager(final Configuration conf) {
		...
		if (isHaEnabled) {
			...
			return new BlockTokenSecretManager(updateMin * 60 * 1000L, lifetimeMin * 60 * 1000L,
					thisNnId.compareTo(otherNnId) < 0 ? 0 : 1, null, encryptionAlgorithm);
		} else {
			return new BlockTokenSecretManager(updateMin * 60 * 1000L, lifetimeMin * 60 * 1000L, 0, null,
					encryptionAlgorithm);
		}
	}
	...
	private void dumpBlockMeta(Block block, PrintWriter out) {
		...
		// containingLiveReplicasNodes can include READ_ONLY_SHARED replicas
		// which are
		...
		out.print(block + ((usableReplicas > 0) ? "" : " MISSING") + " (replicas:" + " l: " + numReplicas.liveReplicas()
				+ " d: " + numReplicas.decommissionedReplicas() + " c: " + numReplicas.corruptReplicas() + " e: "
	 ...
	 * @throws IOException
	 *             if the block does not have at least a minimal number of
	...
	private BlockInfoContiguous completeBlock(final BlockCollection bc, final BlockInfoContiguous block, boolean force)
			throws IOException {
	 ...
	 * regardless of whether enough replicas are present. This is necessary when
	 * tailing edit logs as a Standby.
	 */
	 ...
	 * <p>
	 * The block is converted only if the file has blocks and the last one is a
	 ...
	 * The methods returns null if there is no partial block at the end. The
	 * client is supposed to allocate a new block with the next call.
	 *
	 * @param bc
	 *            file
	 * @param bytesToRemove
	...
	private List<LocatedBlock> createLocatedBlockList(final BlockInfoContiguous[] blocks, final long offset,
			final long length, final int nrBlocksToReturn, final AccessMode mode) throws IOException {
		int curBlk = 0;
	...
	public LocatedBlocks createLocatedBlocks(final BlockInfoContiguous[] blocks,
			final long fileSizeExcludeBlocksUnderConstruction, final boolean isFileUnderConstruction, final long offset,
			final long length, final boolean needBlockToken, final boolean inSnapshot, FileEncryptionInfo feInfo)
			throws IOException {
	...
	private BlocksWithLocations getBlocksWithLocations(final DatanodeID datanode, final long size)
		...
		int startBlock = DFSUtil.getRandom().nextInt(numBlocks); // starting
																	// from a
																	// random
																	// block
		// skip blocks
	...
	/**
	 * Remove all block invalidation tasks under this datanode UUID; used when a
	 * datanode registers with a new UUID and the old one is wiped.
	 */
	 ...
	 * @param reason
	 *            a textual reason why the block should be marked corrupt, for
	 *            logging purposes
	 */
	public void findAndMarkBlockAsCorrupt(final ExtendedBlock blk, final DatanodeInfo dn, String storageID,
		...
		markBlockAsCorrupt(
				new BlockToMarkCorrupt(storedBlock, blk.getGenerationStamp(), reason, Reason.CORRUPTION_REPORTED),
				storageID == null ? null : node.getStorageInfo(storageID), node);
	}
	 ...
	 * @param b
	 * @param storageInfo
	 *            storage that contains the block, if known. null otherwise.
	 * @throws IOException
	 */
	private void markBlockAsCorrupt(BlockToMarkCorrupt b, DatanodeStorageInfo storageInfo, DatanodeDescriptor node)
		...
		boolean hasMoreCorruptReplicas = minReplicationSatisfied
				&& (numberOfReplicas.liveReplicas() + numberOfReplicas.corruptReplicas()) > bc.getBlockReplication();
		...
		// In case of 3, rbw block will be deleted and valid block can be
		// replicated
	...
	private boolean invalidateBlock(BlockToMarkCorrupt b, DatanodeInfo dn) throws IOException {
		...
		if (nr.replicasOnStaleNodes() > 0) {
			...
			// If we have at least one copy on a live node, then we can delete
			// it.
			...
			synchronized (neededReplications) {
				for (int priority = 0; priority < blocksToReplicate.size(); priority++) {
					for (Block block : blocksToReplicate.get(priority)) {
						...
						if (srcNode == null) { // block can not be replicated
												// from any node
							LOG.debug("Block " + block + " cannot be repl from any node");
							continue;
						}

						// liveReplicaNodes can include READ_ONLY_SHARED
						// replicas which are
						// not included in the numReplicas.liveReplicas() count
						assert liveReplicaNodes.size() >= numReplicas.liveReplicas();

						// do not schedule more if enough replicas is already
						// pending
						...
						work.add(new ReplicationWork(block, bc, srcNode, containingNodes, liveReplicaNodes,
		...
		try {
			for (ReplicationWork rw : work) {
				...
				synchronized (neededReplications) {
					...
					// replications that fail after an appropriate amount of
					// time.
	 ...
	 */
	public DatanodeStorageInfo[] chooseTarget4AdditionalDatanode(String src, int numAdditionalNodes, Node clientnode,
	 ...
	 * @see BlockPlacementPolicy#chooseTarget(String, int, Node, Set, long,
	 *      List, BlockStoragePolicy)
	 */
	public DatanodeStorageInfo[] chooseTarget4NewBlock(final String src, final int numOfReplicas, final Node client,
		...
		if (targets.length < minReplication) {
			throw new IOException("File " + src + " could only be replicated to " + targets.length
					+ " nodes instead of minReplication (=" + minReplication + ").  There are "
	 ...
	 * random node among those that did not reach their replication limits.
	 * However, if the replication is of the highest priority and all nodes have
	 * reached their replication limits, we will choose a random node despite
	 * the replication limit.
	 *
	 ...
	 *            List to be populated with nodes found to contain the given
	 *            block
	 * @param nodesContainingLiveReplicas
	 *            List to be populated with nodes found to contain live replicas
	 *            of the given block
	 * @param numReplicas
	 *            NumberReplicas instance to be initialized with the counts of
	 *            live, corrupt, excess, and decommissioned replicas of the
	 *            given block.
	 * @param priority
	 *            integer representing replication priority of the given block
	 * @return the DatanodeDescriptor of the chosen node from which to replicate
	 *         the given block
	 */
	@VisibleForTesting
	DatanodeDescriptor chooseSourceDatanode(Block block, List<DatanodeDescriptor> containingNodes,
		...
		if (timedOutItems != null) {
			...
			try {
				for (int i = 0; i < timedOutItems.length; i++) {
					/*
					 * Use the blockinfo from the blocksmap to be certain we're
					 * working with the most up-to-date block information (e.g.
					 * genstamp).
					 */
					...
					if (isNeededReplication(bi, getReplication(bi), num.liveReplicas())) {
						neededReplications.add(bi, num.liveReplicas(), num.decommissionedReplicas(),
								getReplication(bi));
					}
				}
			} finally {
	...
	/**
	 * BlockToMarkCorrupt is used to build the "toCorrupt" list, which is a list
	 * of blocks that should be considered corrupt due to a block report.
	 */
	...
	/**
	 * The given storage is reporting all its blocks. Update the
	 * (storage-->block list) and (block-->storage list) maps.
	 *
	 * @return true if all known storages of the given DN have finished
	 *         reporting.
	 * @throws IOException
	 */
	public boolean processReport(final DatanodeID nodeID, final DatanodeStorage storage,
			final BlockListAsLongs newReport, BlockReportContext context, boolean lastStorageInRpc) throws IOException {
		namesystem.writeLock();
		final long startTime = Time.monotonicNow(); // after acquiring write
													// lock
		final long endTime;
		...
		try {
			...
			if (storageInfo.getBlockReportCount() == 0) {
				// The first block report can be processed a lot more
				// efficiently than
		...
		if (invalidatedBlocks != null) {
			for (Block b : invalidatedBlocks) {
				blockLog.info("BLOCK* processReport: {} on node {} size {} does not " + "belong to any file", b, node,
						b.getNumBytes());
			}
		}
		...
		blockLog.info(
				"BLOCK* processReport: from storage {} node {}, "
	...
	void rescanPostponedMisreplicatedBlocks() {
		...
		try {
			...
			// Randomly select blocksPerRescan consecutive blocks from the
			// HashSet
			// when the number of blocks remaining is larger than
			// blocksPerRescan.
			// The reason we don't always pick the first blocksPerRescan blocks
			// is to
			...
			for (; it.hasNext(); i++) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("BLOCK* rescanPostponedMisreplicatedBlocks: " + "Re-scanned block " + b + ", result is "
							+ res);
				}
			...
			LOG.info("Rescan of postponedMisreplicatedBlocks completed in "
					+ (Time.monotonicNow() - startTimeRescanPostponedMisReplicatedBlocks) + " msecs. "
	...
	private Collection<Block> processReport(final DatanodeStorageInfo storageInfo, final BlockListAsLongs report)
			throws IOException {
		// Normal case:
			...
			if (isCorrupt) {
				blockLog.info(
						"BLOCK* markBlockReplicasAsCorrupt: mark block replica"
								+ " {} on {} as corrupt because the dn is not in the new committed " + "storage list.",
						b, storage.getDatanodeDescriptor());
	 ...
	 * @throws IOException
	 */
	private void processFirstBlockReport(final DatanodeStorageInfo storageInfo, final BlockListAsLongs report)
			throws IOException {
		if (report == null)
			return;
		assert (namesystem.hasWriteLock());
		...
		for (BlockReportReplica iblk : report) {
			...
			if (c != null) {
				if (shouldPostponeBlocksFromFuture) {
					// In the Standby, we may receive a block report for a file
					// that we
					// just have an out-of-date gen-stamp or state for, for
					// example.
			...
			if (isBlockUnderConstruction(storedBlock, ucState, reportedState)) {
				((BlockInfoContiguousUnderConstruction) storedBlock).addReplicaIfNotPresent(storageInfo, iblk,
						reportedState);
				// OpenFileBlocks only inside snapshots also will be added to
				// safemode
	...
	private void reportDiff(DatanodeStorageInfo storageInfo, BlockListAsLongs newReport,
			...
			Collection<BlockToMarkCorrupt> toCorrupt, // add to corrupt replicas
														// list
			Collection<StatefulBlockInfo> toUC) { // add to under-construction
													// list
		...
		for (BlockReportReplica iblk : newReport) {
			...
			BlockInfoContiguous storedBlock = processReportedBlock(storageInfo, iblk, iState, toAdd, toInvalidate,
					toCorrupt, toUC);
	 ...
	 * <li>If the reported replica is valid that is has the same generation
	 * stamp and length as recorded on the name-node, then the replica location
	 * should be added to the name-node.</li>
	 * <li>If the reported replica is not valid, then it is marked as corrupt,
	 * which triggers replication of the existing valid replicas. Corrupt
	 * replicas are removed from the system when the block is fully
	 * replicated.</li>
	 ...
	 * @param toInvalidate
	 *            missing blocks (not in the blocks map) should be removed from
	 *            the data-node
	 * @param toCorrupt
	 *            replicas with unexpected length or generation stamp; add to
	 *            corrupt replicas
	 * @param toUC
	 *            replicas of blocks currently under construction
	 * @return the up-to-date stored block, if it should be kept. Otherwise,
	 *         null.
	 */
	private BlockInfoContiguous processReportedBlock(final DatanodeStorageInfo storageInfo, final Block block,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Reported block " + block + " on " + dn + " size " + block.getNumBytes() + " replicaState = "
					+ reportedState);
		}
		...
		if (c != null) {
			if (shouldPostponeBlocksFromFuture) {
				...
				// TODO: Pretty confident this should be s/storedBlock/block
				// below,
				// since we should be postponing the info of the reported block,
				// not
		...
		if (isBlockUnderConstruction(storedBlock, ucState, reportedState)) {
			toUC.add(new StatefulBlockInfo((BlockInfoContiguousUnderConstruction) storedBlock, new Block(block),
					reportedState));
			return storedBlock;
		}
	...
	/**
	 * Queue the given reported block for later processing in the standby
	 * node. @see PendingDataNodeMessages.
	 * 
	 * @param reason
	 *            a textual reason to report in the debug logs
	 */
	 ...
	 * block. This is called from FSEditLogLoader whenever a block's state in
	 * the namespace has changed or a new block has been created.
	 */
	...
	/**
	 * Process any remaining queued datanode messages after entering active
	 * state. At this point they will not be re-queued since we are the
	 * definitive master node and thus should be up-to-date with the namespace
	 * information.
	 */
	 ...
	 * the replica is corrupt, or under construction. These are laid out as
	 * switch statements, on the theory that it is easier to understand the
	 * combinatorics of reportedState and ucState that way. It should be at
	 * least as efficient as boolean expressions.
	 * 
	 * @return a BlockToMarkCorrupt object, or null if the replica is not
	 *         corrupt
	 */
	private BlockToMarkCorrupt checkReplicaCorrupt(Block reported, ReplicaState reportedState,
			BlockInfoContiguous storedBlock, BlockUCState ucState, DatanodeDescriptor dn) {
		...
		case FINALIZED:
			...
			case UNDER_CONSTRUCTION:
				if (storedBlock.getGenerationStamp() > reported.getGenerationStamp()) {
					final long reportedGS = reported.getGenerationStamp();
					return new BlockToMarkCorrupt(storedBlock, reportedGS,
							"block is " + ucState + " and reported state " + reportedState + ", But reported genstamp "
		...
		case RWR:
			if (!storedBlock.isComplete()) {
				...
				if (reportedState == ReplicaState.RBW) {
					// If it's a RBW report for a COMPLETE block, it may just be
					// that
					// the block report got a little bit delayed after the
					// pipeline
	 ...
	 * @throws IOException
	 */
	private void addStoredBlockImmediate(BlockInfoContiguous storedBlock, DatanodeStorageInfo storageInfo)
			throws IOException {
		...
		if (storedBlock.getBlockUCState() == BlockUCState.COMMITTED && numCurrentReplica >= minReplication) {
			...
			// In the case that the block just became complete above,
			// completeBlock()
	...
	private Block addStoredBlock(final BlockInfoContiguous block, DatanodeStorageInfo storageInfo,
		...
		if (block instanceof BlockInfoContiguousUnderConstruction) {
			// refresh our copy in case the block got completed in another
			// thread
		...
		// do not try to handle over/under-replicated blocks during first safe
		// mode
		if (!namesystem.isPopulatingReplQueues()) {
			return storedBlock;
		}
		...
		if (numCorruptNodes != corruptReplicasCount) {
			LOG.warn("Inconsistent number of corrupt replicas for " + storedBlock + "blockMap has " + numCorruptNodes
	 ...
	 * {@link #invalidateBlocks} so that they could be further deleted from the
	 * respective data-nodes, and remove the block from corruptReplicasMap.
	 * <p>
	...
	private void invalidateCorruptReplicas(BlockInfoContiguous blk) {
		...
		// ConcurrentModificationException, when the block is removed from the
		// node
	...
	/*
	 * Since the BlocksMapGset does not throw the
	 * ConcurrentModificationException and supports further iteration after
	 * modification to list, there is a chance of missing the newly added block
	 * while iterating. Since every addition to blocksMap will check for
	 * mis-replication, missing mis-replication check for new blocks will not be
	 * a problem.
	 */
	private void processMisReplicatesAsync() throws InterruptedException {
		...
		while (namesystem.isRunning() && !Thread.currentThread().isInterrupted()) {
			...
			try {
				...
				// there is a possibility that if any of the blocks
				// deleted/added during
				...
				namesystem.writeUnlock();
				// Make sure it is out of the write lock for sufficiently long
				// time.
		...
		for (DatanodeStorageInfo storage : blocksMap.getStorages(block, State.NORMAL)) {
			...
			if (storage.areBlockContentsStale()) {
				LOG.info("BLOCK* processOverReplicatedBlock: " + "Postponing processing of over-replicated " + block
						+ " since storage + " + storage + "datanode " + cur + " does not yet have up-to-date "
						+ "block information.");
				postponeBlock(block);
				return;
			}
	...
	static boolean useDelHint(boolean isFirst, DatanodeStorageInfo delHint, DatanodeStorageInfo added,
		...
		if (!isFirst) {
			...
			if (moreThan1Racks.contains(delHint)) {
				return true; // delHint and some other nodes are under the same
								// rack
	...
	private void processAndHandleReportedBlock(DatanodeStorageInfo storageInfo, Block block, ReplicaState reportedState,
			DatanodeDescriptor delHintNode) throws IOException {
		...
		for (Block b : toInvalidate) {
			blockLog.info("BLOCK* addBlock: block {} on node {} size {} does not " + "belong to any file", b, node,
					b.getNumBytes());
			addToInvalidates(b, node);
		}
		for (BlockToMarkCorrupt b : toCorrupt) {
	...
	public void processIncrementalBlockReport(final DatanodeID nodeID, final StorageReceivedDeletedBlocks srdb)
		...
		if (storageInfo == null) {
			// The DataNode is reporting an unknown storage. Usually the NN
			// learns
			// about new storages from heartbeats but during NN restart we may
			// receive a block report or incremental report before the
			// heartbeat.
	...
	/**
	 * Return the number of nodes hosting a given block, grouped by the state of
	 * those replicas.
	 */
	 ...
	 * number of live nodes. If in startup safemode (or its 30-sec extension
	 * period), then it gains speed by ignoring issues of excess replicas or
	 * nodes that are decommissioned or in process of becoming decommissioned.
	 ...
	 * are any excess replicas, call processOverReplicatedBlock(). Process over
	 * replicated blocks only when active NN is out of safe mode.
	 */
	...
	/**
	 * Check replication of the blocks in the collection. If any block is needed
	 * replication, insert it into the replication queue. Otherwise, if the
	 * block is more than the expected replication factor, process it as an over
	 * replicated block.
	 */
	...
	/**
	 * @return 0 if the block is not found; otherwise, return the replication
	 *         factor of the block.
	 */
	...
	/**
	 * Return an iterator over the set of blocks for which there are no
	 * replicas.
	 */
	...
	/**
	 * Get reason for certain corrupted replicas for a given block and a given
	 * dn.
	 */
	...
	private class ReplicationMonitor implements Runnable {
		...
		public void run() {
			while (namesystem.isRunning()) {
				try {
					// Process replication work only when active NN is out of
					// safe mode.
					if (namesystem.isPopulatingReplQueues()) {
						computeDatanodeWork();
