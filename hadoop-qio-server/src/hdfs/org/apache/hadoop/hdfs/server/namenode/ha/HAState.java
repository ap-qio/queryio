...
abstract public class HAState {
	...
	/**
	 * Internal method to transition the state of a given namenode to a new
	 * state.
	 * 
	 * @param nn
	 *            Namenode
	...
	/**
	 * Method to be overridden by subclasses to prepare to enter a state. This
	 * method is called <em>without</em> the context being locked, and after
	 * {@link #prepareToExitState(HAContext)} has been called for the previous
	 * state, but before {@link #exitState(HAContext)} has been called for the
	 * previous state.
	 * 
	 * @param context
	 *            HA context
	 ...
	 *            HA context
	 * @throws ServiceFailedException
	 *             on failure to enter the state.
	 */
	public abstract void enterState(final HAContext context) throws ServiceFailedException;

	/**
	 * Method to be overridden by subclasses to prepare to exit a state. This
	 * method is called <em>without</em> the context being locked. This is used
	 * by the standby state to cancel any checkpoints that are going on. It can
	 * also be used to check any preconditions for the state transition.
	 * 
	 * This method should not make any destructuve changes to the state (eg
	 * stopping threads) since {@link #prepareToEnterState(HAContext)} may
	 * subsequently cancel the state transition.
	 * 
	 * @param context
	 *            HA context
	 * @throws ServiceFailedException
	 ...
	 *             if a given type of operation is not supported in standby
	 *             state
	 */
