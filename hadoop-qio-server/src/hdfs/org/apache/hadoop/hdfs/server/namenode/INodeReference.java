 ...
 *
 * This class and its subclasses are used to support multiple access paths. A
 * file/directory may have multiple access paths when it is stored in some
 * snapshots and it is renamed/moved to other locations.
 * 
 * For example, (1) Suppose we have /abc/foo, say the inode of foo is
 * inode(id=1000,name=foo) (2) create snapshot s0 for /abc (3) mv /abc/foo
 * /xyz/bar, i.e. inode(id=1000,name=...) is renamed from "foo" to "bar" and its
 * parent becomes /xyz.
 * 
 * Then, /xyz/bar and /abc/.snapshot/s0/foo are two different access paths to
 * the same inode, inode(id=1000,name=bar).
 *
 * With references, we have the following - /abc has a child
 * ref(id=1001,name=foo). - /xyz has a child ref(id=1002) - Both
 * ref(id=1001,name=foo) and ref(id=1002) point to another reference,
 * ref(id=1003,count=2). - Finally, ref(id=1003,count=2) points to
 * inode(id=1000,name=bar).
 * 
 * Note 1: For a reference without name, e.g. ref(id=1002), it uses the name of
 * the referred inode. Note 2: getParent() always returns the parent in the
 * current state, e.g. inode(id=1000,name=bar).getParent() returns /xyz but not
 * /abc.
 */
public abstract class INodeReference extends INode {
	/**
	 * Try to remove the given reference and then return the reference count. If
	 * the given inode is not a reference, return -1;
	 */
	...
	/**
	 * Remove the given reference and then return the reference count. If the
	 * referred inode is not a WithCount, return -1;
	 */
	...
	public static class WithCount extends INodeReference {
		...
		/**
		 * Compare snapshot with IDs, where null indicates the current status
		 * thus is greater than any non-null snapshot.
		 */
		...
		/**
		 * @return the WithName/DstReference node contained in the given
		 *         snapshot.
		 */
	...
	public static class WithName extends INodeReference {
		...
		/**
		 * The id of the last snapshot in the src tree when this WithName node
		 * was generated. When calculating the quota usage of the referred node,
		 * only the files/dirs existing when this snapshot was taken will be
		 * counted for this WithName node and propagated along its ancestor
		 * path.
		 */
		...
		@Override
		public final QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId,
				QuotaCounts counts, boolean useCache, int lastSnapshotId) {
			// if this.lastSnapshotId < lastSnapshotId, the rename of the
			// referred
			// node happened before the rename of its ancestor. This should be
			// impossible since for WithName node we only count its children at
			// the
			// time of the rename.
			Preconditions
					.checkState(lastSnapshotId == Snapshot.CURRENT_STATE_ID || this.lastSnapshotId >= lastSnapshotId);
			final INode referred = this.getReferredINode().asReference().getReferredINode();
			// We will continue the quota usage computation using the same
			// snapshot id
			// as time line (if the given snapshot id is valid). Also, we cannot
			// use
			// cache for the referred node since its cached quota may have
			// already
		...
		@Override
		public QuotaCounts cleanSubtree(BlockStoragePolicySuite bsps, final int snapshot, int prior,
				final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
			// since WithName node resides in deleted list acting as a snapshot
			// copy,
			// the parameter snapshot must be non-null
			Preconditions.checkArgument(snapshot != Snapshot.CURRENT_STATE_ID);
			// if prior is NO_SNAPSHOT_ID, we need to check snapshot belonging
			// to the
			...
			if (snapshot < lastSnapshotId) {
				// for a WithName node, when we compute its quota usage, we only
				// count
				// in all the nodes existing at the time of the corresponding
				// rename op.
				// Thus if we are deleting a snapshot before/at the snapshot
				// associated
				// with lastSnapshotId, we do not need to update the quota
				// upwards.
			...
			if (removeReference(this) <= 0) {
				...
				if (snapshot != Snapshot.NO_SNAPSHOT_ID) {
					if (prior != Snapshot.NO_SNAPSHOT_ID && snapshot <= prior) {
						// the snapshot to be deleted has been deleted while
						// traversing
						// the src tree of the previous rename operation. This
						// usually
						// happens when rename's src and dst are under the same
						// snapshottable directory. E.g., the following
						// operation sequence:
	...
	public static class DstReference extends INodeReference {
		 ...
		 * later operations on the moved/renamed files/directories, if the
		 * latest snapshot is after this dstSnapshot, changes will be recorded
		 * to the latest snapshot. Otherwise changes will be recorded to the
		 * snapshot belonging to the src of the rename.
		 * 
		 * {@link Snapshot#NO_SNAPSHOT_ID} means no dstSnapshot (e.g., src of
		 * the first-time rename).
		 */
		...
		public QuotaCounts cleanSubtree(BlockStoragePolicySuite bsps, int snapshot, int prior,
			...
			if (snapshot == Snapshot.CURRENT_STATE_ID && prior == Snapshot.NO_SNAPSHOT_ID) {
				...
				// if prior is NO_SNAPSHOT_ID, we need to check snapshot
				// belonging to
				...
				// to-be-deleted snapshot, we can quit here and leave the
				// snapshot
		...
		public void destroyAndCollectBlocks(BlockStoragePolicySuite bsps, BlocksMapUpdateInfo collectedBlocks,
			...
			if (removeReference(this) <= 0) {
				...
				INode referred = getReferredINode().asReference().getReferredINode();
				if (referred.isFile()) {
					// if referred is a file, it must be a file with snapshot
					// since we did
