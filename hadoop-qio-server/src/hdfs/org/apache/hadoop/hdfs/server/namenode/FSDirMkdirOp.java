...
class FSDirMkdirOp {
	...
	static HdfsFileStatus mkdirs(FSNamesystem fsn, String src, PermissionStatus permissions, boolean createParent)
			throws IOException {
		...
		try {
			...
			if (lastINode == null) {
				...
				if (length > 1) {
					...
					// Ensure that the user can traversal the path by adding
					// implicit
				...
				if ((existing = createChildrenDirectories(fsd, existing, nonExisting.subList(length - 1, length),
						permissions)) == null) {
					throw new IOException("Failed to create directory: " + src);
				}
			}
	 ...
	 * u+wx permission. This is used by create() and addSymlink() for implicitly
	 * creating all directories along the path.
	 *
	 ...
	 *         existing and newly created directories) and the last component in
	 *         the relative path. Or return null if there are errors.
	 */
	 ...
	 *            ancestral INodes
	 * @param children
	 *            The relative path from the parent towards children, starting
	 *            with "/"
	 * @param perm
	 *            the permission of the directory. Note that all ancestors
	 *            created along the path has implicit {@code u+wx} permissions.
	 *
	 * @return {@link INodesInPath} which contains all inodes to the target
	 *         directory, After the execution parentPath points to the path of
	 *         the returned INodesInPath. The function return null if the
	 *         operation has failed.
	 */
	private static INodesInPath createChildrenDirectories(FSDirectory fsd, INodesInPath existing, List<String> children,
	...
	static void mkdirForEditLog(FSDirectory fsd, long inodeId, String src, PermissionStatus permissions,
			...
			throws QuotaExceededException, UnresolvedLinkException, AclException, FileAlreadyExistsException {
		assert fsd.hasWriteLock();
	...
	private static INodesInPath createSingleDirectory(FSDirectory fsd, INodesInPath existing, String localName,
			PermissionStatus perm) throws IOException {
		assert fsd.hasWriteLock();
		existing = unprotectedMkdir(fsd, fsd.allocateNewInodeId(), existing, localName.getBytes(Charsets.UTF_8), perm,
				null, now());
		if (existing == null) {
			return null;
		}
	...
	private static PermissionStatus addImplicitUwx(PermissionStatus parentPerm, PermissionStatus perm) {
		...
		FsPermission ancestorPerm = new FsPermission(p.getUserAction().or(FsAction.WRITE_EXECUTE), p.getGroupAction(),
				p.getOtherAction());
