...
public class FileDataServlet extends DfsServlet {
	...
	private URL createRedirectURL(String path, String encodedPath, HdfsFileStatus status, UserGroupInformation ugi,
			ClientProtocol nnproxy, HttpServletRequest request, String dt) throws IOException {
		String scheme = request.getScheme();
		final LocatedBlocks blks = nnproxy.getBlockLocations(status.getFullPath(new Path(path)).toUri().getPath(), 0,
				1);
		...
		return new URL(scheme, hostname, port, "/streamFile" + encodedPath + '?' + "ugi="
	...
	/**
	 * Service a GET request as described below. Request: {@code
