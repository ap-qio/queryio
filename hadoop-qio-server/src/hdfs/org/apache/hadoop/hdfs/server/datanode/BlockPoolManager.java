...
/**
 * Manages the BPOfferService objects for the data node. Creation, removal,
 * starting, stopping, shutdown on BPOfferService objects must be done via APIs
 * in this class.
 */
...
class BlockPoolManager {
	...
	/**
	 * Returns the array of BPOfferService objects. Caution: The BPOfferService
	 * returned could be shutdown any time.
	 */
	...
	private void doRefreshNamenodes(Map<String, Map<String, InetSocketAddress>> addrMap) throws IOException {
		...
		synchronized (this) {
			...
			// Step 2. Any nameservices we currently have but are no longer
			// present
			// need to be removed.
			toRemove = Sets.newHashSet(Sets.difference(bpByNameserviceId.keySet(), addrMap.keySet()));

			assert toRefresh.size() + toAdd.size() == addrMap.size() : "toAdd: "
					+ Joiner.on(",").useForNull("<default>").join(toAdd) + "  toRemove: "
					+ Joiner.on(",").useForNull("<default>").join(toRemove) + "  toRefresh: "
