...
public class RollingWindowManager {
	...
	/**
	 * Represents an operation within a TopWindow. It contains a ranked set of
	 * the top users for the operation.
	 */
	...
	/**
	 * Represents a user who called an Op within a TopWindow. Specifies the user
	 * and the number of times the user called the operation.
	 */
	...
	public RollingWindowManager(Configuration conf, int reportingPeriodMs) {
		...
		bucketsPerWindow = conf.getInt(DFSConfigKeys.NNTOP_BUCKETS_PER_WINDOW_KEY,
	 ...
	 * @param command
	 *            the metric that is updated, e.g., the operation name
	 * @param user
	 *            the user that updated the metric
	 * @param delta
	 *            the amount of change in the metric, e.g., +1
	 */
	...
	public TopWindow snapshot(long time) {
		...
		for (Map.Entry<String, RollingWindowMap> entry : metricMap.entrySet()) {
			...
			// since we'd like them sorted in descending rather than ascending
			// order
	...
	/**
	 * A pair of a name and its corresponding value. Defines a custom comparator
	 * so the TopN PriorityQueue sorts based on the count.
	 */
