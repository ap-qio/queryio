 ...
 * Only one proxy is used to connect to both servers and switching between the
 * servers is done by the environment/infrastructure, which guarantees clients
 * can consistently reach only one node at a time.
 *
 * Clients with a live connection will likely get connection reset after an IP
 * failover. This case will be handled by the FailoverOnNetworkExceptionRetry
 * retry policy. I.e. if the call is not idempotent, it won't get retried.
 *
...
public class IPFailoverProxyProvider<T> extends AbstractNNFailoverProxyProvider<T> {
	...
	public IPFailoverProxyProvider(Configuration conf, URI uri, Class<T> xface) {
		...
		int maxRetries = this.conf.getInt(DFSConfigKeys.DFS_CLIENT_FAILOVER_CONNECTION_RETRIES_KEY,
	...
	public synchronized ProxyInfo<T> getProxy() {
		...
		if (nnProxyInfo == null) {
			try {
				...
				nnProxyInfo = new ProxyInfo<T>(NameNodeProxies
						.createNonHAProxy(conf, nnAddr, xface, UserGroupInformation.getCurrentUser(), false).getProxy(),
						nnAddr.toString());
