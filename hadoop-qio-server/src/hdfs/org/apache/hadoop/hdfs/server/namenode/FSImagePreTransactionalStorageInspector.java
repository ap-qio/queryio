 ...
 * This format has the following data files: - fsimage - fsimage.ckpt (when
 * checkpoint is being uploaded) - edits - edits.new (when logs are "rolled")
 */
...
class FSImagePreTransactionalStorageInspector extends FSImageStorageInspector {
	...
	List<FSImageFile> getLatestImages() throws IOException {
		...
		FSImageFile file = new FSImageFile(latestNameSD, NNStorage.getStorageFile(latestNameSD, NameNodeFile.IMAGE),
	...
	@Override
	boolean needToSave() {
		return hasOutOfDateStorageDirs || checkpointTimes.size() != 1
				|| latestNameCheckpointTime > latestEditsCheckpointTime || needToSaveAfterRecovery;
	}
	...
	/**
	 * @return a list with the paths to EDITS and EDITS_NEW (if it exists) in a
