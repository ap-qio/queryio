...
public final class CachedBlock implements Element, LightWeightGSet.LinkedElement {
	 ...
	 *
	 * Since this CachedBlock can be in multiple CachedBlocksList objects, we
	 * need to be able to store multiple 'prev' and 'next' pointers. The
	 * triplets array does this.
	 *
	 * Each triplet contains a CachedBlockList object followed by a prev
	 * pointer, followed by a next pointer.
	 */
	...
	/**
	 * Get a list of the datanodes which this block is cached, planned to be
	 * cached, or planned to be uncached on.
	 *
	 * @param type
	 *            If null, this parameter is ignored. If it is non-null, we
	 *            match only datanodes which have it on this list. See
	 *            {@link DatanodeDescriptor.CachedBlocksList.Type} for a
	 *            description of all the lists.
	 * 
	 * @return The list of datanodes. Modifying this list does not alter the
	...
	public String toString() {
		return new StringBuilder().append("{").append("blockId=").append(blockId).append(", ").append("replication=")
				.append(getReplication()).append(", ").append("mark=").append(getMark()).append("}").toString();
	}
