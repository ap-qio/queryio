 ...
 * streams edits to a backup node.
 * 
 * @see org.apache.hadoop.hdfs.server.protocol.NamenodeProtocol#journal
 *      (org.apache.hadoop.hdfs.server.protocol.NamenodeRegistration, int, int,
 *      byte[])
 */
class EditLogBackupOutputStream extends EditLogOutputStream {
	...
	private final NamenodeRegistration bnRegistration; // backup node
														// registration
	...
	EditLogBackupOutputStream(NamenodeRegistration bnReg, // backup node
		...
		try {
			this.backupNode = NameNodeProxies.createNonHAProxy(new HdfsConfiguration(), bnAddress,
					JournalProtocol.class, UserGroupInformation.getCurrentUser(), true).getProxy();
		} catch (IOException e) {
