...
public class FileWithSnapshotFeature implements INode.Feature {
	...
	/**
	 * We need to distinguish two scenarios: 1) the file is still in the current
	 * file directory, it has been modified before while it is included in some
	 * snapshot 2) the file is not in the current file directory (deleted), but
	 * it is in some snapshot, thus we still keep this inode For both scenarios
	 * the file has snapshot feature. We set {@link #isCurrentFileDeleted} to
	 * true for 2).
	 */
	...
	boolean changedBetweenSnapshots(INodeFile file, Snapshot from, Snapshot to) {
		...
		final long laterLength = laterDiffIndex == diffList.size() ? file.computeFileSize(true, false)
		...
		INodeFileAttributes laterAttr = diffs
				.getSnapshotINode(Math.max(Snapshot.getSnapshotId(from), Snapshot.getSnapshotId(to)), file);
	...
	public QuotaCounts cleanFile(final BlockStoragePolicySuite bsps, final INodeFile file, final int snapshotId,
			int priorSnapshotId, final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
	...
	public QuotaCounts updateQuotaAndCollectBlocks(BlockStoragePolicySuite bsps, INodeFile file, FileDiff removed,
	...
	/**
	 * If some blocks at the end of the block list no longer belongs to any
	 * inode, collect them and update the block list.
	 */
