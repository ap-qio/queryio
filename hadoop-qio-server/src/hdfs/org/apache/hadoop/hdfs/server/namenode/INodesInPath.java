...
public class INodesInPath {
	...
	/**
	 * Retrieve existing INodes from a path. For non-snapshot path, the number
	 * of INodes is equal to the number of path components. For snapshot path
	 * (e.g., /foo/.snapshot/s1/bar), the number of INodes is
	 ...
	 * 
	 * <p>
	 * <code>getExistingPathINodes(["","c1","c2"])</code> should fill the array
	 * with [rootINode,c1,c2], <br>
	 * <code>getExistingPathINodes(["","c1","c2","c3"])</code> should fill the
	 * array with [rootINode,c1,c2,null]
	 * 
	 * @param startingDir
	 ...
	 * non-snapshot paths, it is the id of the latest snapshot found in the
	 * path; or {@link Snapshot#CURRENT_STATE_ID} if no snapshot is found.
	 */
	...
	private String toString(boolean vaildateObject) {
		...
		final StringBuilder b = new StringBuilder(getClass().getSimpleName()).append(": path = ")
				.append(DFSUtil.byteArray2PathString(path)).append("\n  inodes = ");
		if (inodes == null) {
			b.append("null");
