...
public class FSDirectory implements Closeable {
	...
	private static INodeDirectory createRoot(FSNamesystem namesystem) {
		final INodeDirectory r = new INodeDirectory(INodeId.ROOT_INODE_ID, INodeDirectory.ROOT_NAME,
				namesystem.createFsOwnerPermissions(new FsPermission((short) 0755)), 0L);
		r.addDirectoryWithQuotaFeature(new DirectoryWithQuotaFeature.Builder()
	...
	private volatile boolean skipQuotaCheck = false; // skip while consuming
														// edits
	...
	FSDirectory(FSNamesystem ns, Configuration conf) throws IOException {
		...
		this.aclsEnabled = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_ACLS_ENABLED_KEY,
				DFSConfigKeys.DFS_NAMENODE_ACLS_ENABLED_DEFAULT);
		LOG.info("ACLs enabled? " + aclsEnabled);
		this.xattrsEnabled = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_XATTRS_ENABLED_KEY,
		...
		this.quotaByStorageTypeEnabled = conf.getBoolean(DFS_QUOTA_BY_STORAGETYPE_ENABLED_KEY,
		...
		this.contentCountLimit = conf.getInt(DFSConfigKeys.DFS_CONTENT_SUMMARY_LIMIT_KEY,
				DFSConfigKeys.DFS_CONTENT_SUMMARY_LIMIT_DEFAULT);

		// filesystem limits
		this.maxComponentLength = conf.getInt(DFSConfigKeys.DFS_NAMENODE_MAX_COMPONENT_LENGTH_KEY,
				DFSConfigKeys.DFS_NAMENODE_MAX_COMPONENT_LENGTH_DEFAULT);
		this.maxDirItems = conf.getInt(DFSConfigKeys.DFS_NAMENODE_MAX_DIRECTORY_ITEMS_KEY,
		...
		// to 64MB. This means we can only store approximately 6.7 million
		// entries
	...
	private static INodeFile newINodeFile(long id, PermissionStatus permissions, long mtime, long atime,
	...
	private static INodeFile newINodeFile(long id, PermissionStatus permissions, long mtime, long atime,
		...
		return new INodeFile(id, null, permissions, mtime, atime, BlockInfoContiguous.EMPTY_ARRAY, replication,
	...
	INodesInPath addFile(INodesInPath existing, String localName, PermissionStatus permissions, short replication,
			long preferredBlockSize, String clientName, String clientMachine) throws FileAlreadyExistsException,
			QuotaExceededException, UnresolvedLinkException, SnapshotAccessControlException, AclException {

		long modTime = now();
		INodeFile newNode = newINodeFile(allocateNewInodeId(), permissions, modTime, modTime, replication,
				preferredBlockSize);
	...
	INodeFile addFileForEditLog(long id, INodesInPath existing, byte[] localName, PermissionStatus permissions,
			List<AclEntry> aclEntries, List<XAttr> xAttrs, short replication, long modificationTime, long atime,
			long preferredBlockSize, boolean underConstruction, String clientName, String clientMachine,
			byte storagePolicyId) {
		final INodeFile newNode;
		assert hasWriteLock();
		if (underConstruction) {
			newNode = newINodeFile(id, permissions, modificationTime, modificationTime, replication, preferredBlockSize,
					storagePolicyId);
			newNode.toUnderConstruction(clientName, clientMachine);
		} else {
	 ...
	 */
	BlockInfoContiguous addBlock(String path, INodesInPath inodesInPath, Block block, DatanodeStorageInfo[] targets)
			throws IOException {
		writeLock();
		try {
			...
			BlockInfoContiguousUnderConstruction blockInfo = new BlockInfoContiguousUnderConstruction(block,
					fileINode.getFileReplication(), BlockUCState.UNDER_CONSTRUCTION, targets);
	...
	boolean unprotectedRemoveBlock(String path, INodesInPath iip, INodeFile fileNode, Block block) throws IOException {
		...
		if (NameNode.stateChangeLog.isDebugEnabled()) {
			NameNode.stateChangeLog.debug("DIR* FSDirectory.removeBlock: " + path + " with " + block
	...
	/**
	 * This is a wrapper for resolvePath(). If the path passed is prefixed with
	 * /.reserved/raw, then it checks to ensure that the caller has super user
	 * privileges.
	 *
	 * @param pc
	 *            The permission checker used when resolving path.
	 * @param path
	 *            The path to resolve.
	 * @param pathComponents
	 *            path components corresponding to the path
	 ...
	 * @param replication
	 *            the replication factor of the block consumption change
	...
	void updateCount(INodesInPath iip, long nsDelta, long ssDelta, short replication, boolean checkQuota)
		...
		EnumCounters<StorageType> typeSpaceDeltas = getStorageTypeDeltas(fileINode.getStoragePolicyID(), ssDelta,
				replication, replication);
		;
		updateCount(iip, iip.length() - 1, new QuotaCounts.Builder().nameSpace(nsDelta)
				.storageSpace(ssDelta * replication).typeSpaces(typeSpaceDeltas).build(), checkQuota);
	}
	...
	void updateCount(INodesInPath iip, long nsDelta, long ssDelta, short oldRep, short newRep, boolean checkQuota)
		...
		EnumCounters<StorageType> typeSpaceDeltas = getStorageTypeDeltas(fileINode.getStoragePolicyID(), ssDelta,
				oldRep, newRep);
		updateCount(iip, iip.length() - 1, new QuotaCounts.Builder().nameSpace(nsDelta)
				.storageSpace(ssDelta * (newRep - oldRep)).typeSpaces(typeSpaceDeltas).build(), checkQuota);
	}
	 ...
	 * @param counts
	 *            the count of space/namespace/type usage to be update
	 * @param checkQuota
	 ...
	 * {@link #updateCount(INodesInPath, int, QuotaCounts, boolean)}
	 */
	...
	static void unprotectedUpdateCount(INodesInPath inodesInPath, int numOfINodes, QuotaCounts counts) {
		for (int i = 0; i < numOfINodes; i++) {
			if (inodesInPath.getINode(i).isQuotaSet()) { // a directory with
															// quota
	...
	public EnumCounters<StorageType> getStorageTypeDeltas(byte storagePolicyID, long dsDelta, short oldRep,
			short newRep) {
		EnumCounters<StorageType> typeSpaceDeltas = new EnumCounters<StorageType>(StorageType.class);
		// Storage type and its quota are only available when storage policy is
		// set
		...
		for (int i = 0; i < depth; i++) {
			if (inode == null) {
				NameNode.stateChangeLog
						.warn("Could not get full path." + " Corresponding file might have deleted already.");
				return null;
			}
	 ...
	 * storagespace to a given position.
	 * 
	 * @param iip
	 ...
	 * @param deltas
	 *            needed namespace, storagespace and storage types
	 * @param commonAncestor
	 *            Last node in inodes array that is a common ancestor for a
	 *            INode that is being moved from one location to the other. Pass
	 *            null if a node is not being moved.
	 * @throws QuotaExceededException
	 ...
	 *
	 * @param childName
	 *            byte[] containing new child name
	 * @param parentPath
	 *            String containing parent path
	...
	void verifyMaxComponentLength(byte[] childName, String parentPath) throws PathComponentTooLongException {
		...
		if (length > maxComponentLength) {
			final PathComponentTooLongException e = new PathComponentTooLongException(maxComponentLength, length,
					parentPath, DFSUtil.bytes2String(childName));
			if (namesystem.isImageLoaded()) {
				throw e;
			} else {
	 ...
	 */
	@VisibleForTesting
	public INodesInPath addLastINode(INodesInPath existing, INode inode, boolean checkQuota)
			throws QuotaExceededException {
		assert existing.getLastINode() != null && existing.getLastINode().isDirectory();

		final int pos = existing.length();
		// Disallow creation of /.reserved. This may be created when loading
		// editlog/fsimage during upgrade since /.reserved was a valid name in
		// older
	...
	/**
	 * Remove the last inode in the path from the namespace. Note: the caller
	 * needs to update the ancestors' quota count.
	 *
	 * @return -1 for failing to remove; 0 for removing a reference whose
	 *         referred inode has other reference nodes; 1 otherwise.
	 */
	@VisibleForTesting
	 ...
	 */
	void unprotectedTruncate(String src, String clientName, String clientMachine, long newLength, long mtime,
			Block truncateBlock)
			throws UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException, IOException {
		...
		assert onBlockBoundary == (truncateBlock == null) : "truncateBlock is null iff on block boundary: "
				+ truncateBlock;
		getFSNamesystem().removeBlocksAndUpdateSafemodeTotal(collectedBlocks);
	}

	boolean truncate(INodesInPath iip, long newLength, BlocksMapUpdateInfo collectedBlocks, long mtime,
			QuotaCounts delta) throws IOException {
		writeLock();
		try {
	...
	/**
	 * Truncate has the following properties: 1.) Any block deletions occur now.
	 * 2.) INode length is truncated now new clients can only read up to the
	 * truncated length. 3.) INode will be set to UC and lastBlock set to
	 * UNDER_RECOVERY. 4.) NN will trigger DN truncation recovery and waits for
	 * DNs to report. 5.) File is considered UNDER_RECOVERY until truncation
	 * recovery completes. 6.) Soft and hard Lease expiration require truncation
	 * recovery to complete.
	 *
	 * @return true if on the block boundary or false if recovery is need
	 */
	boolean unprotectedTruncate(INodesInPath iip, long newLength, BlocksMapUpdateInfo collectedBlocks, long mtime,
			QuotaCounts delta) throws IOException {
		assert hasWriteLock();
		INodeFile file = iip.getLastINode().asFile();
	...
	private void verifyQuotaForTruncate(INodesInPath iip, INodeFile file, long newLength, QuotaCounts delta)
			throws QuotaExceededException {
				...
				if (xaf != null) {
					...
					for (XAttr xattr : xattrs) {
						...
						if (CRYPTO_XATTR_ENCRYPTION_ZONE.equals(xaName)) {
							try {
								final HdfsProtos.ZoneEncryptionInfoProto ezProto = HdfsProtos.ZoneEncryptionInfoProto
										.parseFrom(xattr.getValue());
	...
	XAttr createEncryptionZone(String src, CipherSuite suite, CryptoProtocolVersion version, String keyName)
			throws IOException {
		writeLock();
		try {
	...
	/**
	 * This function combines the per-file encryption info (obtained from the
	 * inode's XAttrs), and the encryption info from its zone, and returns a
	 * consolidated FileEncryptionInfo instance. Null is returned for
	 * non-encrypted files.
	 *
	 * @param inode
	 *            inode of the file
	 * @param snapshotId
	 *            ID of the snapshot that we want to get encryption info from
	 * @param iip
	 *            inodes in the path containing the file, passed in to avoid
	 *            obtaining the list of inodes again; if iip is null then the
	 *            list of inodes will be obtained again
	...
	FileEncryptionInfo getFileEncryptionInfo(INode inode, int snapshotId, INodesInPath iip) throws IOException {
		...
		try {
			...
			XAttr fileXAttr = FSDirXAttrOp.unprotectedGetXAttrByName(inode, snapshotId,
	...
	/**
	 * Given an INode get all the path complents leading to it from the root. If
	 * an Inode corresponding to C is given in /A/B/C, the returned patch
	 * components will be {root, A, B, C}. Note that this method cannot handle
	 * scenarios where the inode is in a snapshot.
	 */
	 ...
	 * /ezone/a refers to the decrypted file and /.reserved/raw/ezone/a refers
	 * to the encrypted (raw) bytes of /ezone/a.
	 * <p/>
	...
	private static String resolveDotInodesPath(String src, byte[][] pathComponents, FSDirectory fsd)
			throws FileNotFoundException {
	...
	INode getINode4DotSnapshot(String src) throws UnresolvedLinkException {
		Preconditions.checkArgument(src.endsWith(HdfsConstants.SEPARATOR_DOT_SNAPSHOT_DIR), "%s does not end with %s",
				src, HdfsConstants.SEPARATOR_DOT_SNAPSHOT_DIR);
	...
	FSPermissionChecker getPermissionChecker(String fsOwner, String superGroup, UserGroupInformation ugi)
			throws AccessControlException {
		return new FSPermissionChecker(fsOwner, superGroup, ugi,
				attributeProvider == null ? DefaultINodeAttributesProvider.DEFAULT_PROVIDER : attributeProvider);
	}
	 ...
	 */
	void checkPermission(FSPermissionChecker pc, INodesInPath iip, boolean doCheckOwner, FsAction ancestorAccess,
	...
	HdfsFileStatus getAuditFileInfo(INodesInPath iip) throws IOException {
		return (namesystem.isAuditEnabled() && namesystem.isExternalInvocation())
				? FSDirStatAndListingOp.getFileInfo(this, iip.getPath(), iip, false, false) : null;
	}
	...
	INodeAttributes getAttributes(String fullPath, byte[] path, INode node, int snapshot) {
		...
		if (attributeProvider != null) {
			...
			fullPath = fullPath + (fullPath.endsWith(Path.SEPARATOR) ? "" : Path.SEPARATOR)
