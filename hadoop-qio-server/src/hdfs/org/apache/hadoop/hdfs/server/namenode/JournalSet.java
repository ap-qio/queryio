...
public class JournalSet implements JournalManager {
	...
	private static final Comparator<EditLogInputStream> LOCAL_LOG_PREFERENCE_COMPARATOR = new Comparator<EditLogInputStream>() {
		...
		public int compare(EditLogInputStream elis1, EditLogInputStream elis2) {
			// we want local logs to be ordered earlier in the collection, and
			// true
			// is considered larger than false, so we want to invert the
			// booleans here
	 ...
	 * 
	 * If a Journal gets disabled due to an error writing to its stream, then
	 * the stream will be aborted and set to null.
	 */
	...
	/**
	 * In this function, we get a bunch of streams from all of our
	 * JournalManager objects. Then we add these to the collection one by one.
	 * 
	 * @param streams
	 *            The collection to add the streams to. It may or may not be
	 *            sorted-- this is up to the caller.
	 * @param fromTxId
	...
	public static void chainAndMakeRedundantStreams(Collection<EditLogInputStream> outStreams,
		...
		// We want to group together all the streams that start on the same
		// start
		...
		// higher start transaction ID, we select a stream from the accumulator
		// and
		// clear it. Then we begin accumulating streams with the new, higher
		// start
		// transaction ID.
		...
		while ((elis = allStreams.poll()) != null) {
			if (acc.isEmpty()) {
				...
				if (accFirstTxId == elis.getFirstTxId()) {
					// if we have a finalized log segment available at this
					// txid,
					...
					// try to read from the local logs first since the
					// throughput should
					// be higher
	...
	/**
	 * Returns true if there are no journals, all redundant journals are
	 * disabled, or any required journals are disabled.
	 * 
		...
		for (JournalAndStream jas : journals) {
			try {
				...
				if (jas.isRequired()) {
					...
					LOG.fatal(msg, t);
					// If we fail on *any* of the required journals, then we
					// must not
					// continue on any of the other journals. Abort them to
					// ensure that
					// retry behavior doesn't allow them to keep going in any
					// way.
					abortAllJournals();
					// the current policy is to shutdown the NN on errors to
					// shared edits
					// dir. There are many code paths to shared edits failures -
					// syncs,
					// roll of edits etc. All of them go through this common
					// function
					// where the isRequired() check is made. Applying exit
					// policy here
	...
	/**
	 * An implementation of EditLogOutputStream that applies a requested method
	 * on all the journals that are currently active.
	 */
	...
	/**
	 * Return a manifest of what finalized edit logs are available. All
	 * available edit logs are returned starting from the transaction id passed.
	 * If 'fromTxId' falls in the middle of a log, that log is returned as well.
	 * 
	 * @param fromTxId
	...
	public synchronized RemoteEditLogManifest getEditLogManifest(long fromTxId) {
		...
		while (true) {
			...
			if (logGroup.isEmpty()) {
				// we have a gap in logs - for example because we recovered some
				// old
