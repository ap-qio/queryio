...
public abstract class HdfsAuditLogger implements AuditLogger {
	 ...
	 * @param userName
	 *            Name of the user executing the request.
	 * @param addr
	 *            Remote address of the request.
	 * @param cmd
	 *            The requested command.
	 * @param src
	 *            Path of affected source file.
	 * @param dst
	 *            Path of affected destination file (if any).
	 * @param stat
	 *            File information for operations that change the file's
	 *            metadata (permissions, owner, times, etc).
	 * @param ugi
	 *            UserGroupInformation of the current user, or null if not
	 *            logging token tracking information
	 * @param dtSecretManager
	 *            The token secret manager, or null if not logging token
	 *            tracking information
	 */
	public abstract void logAuditEvent(boolean succeeded, String userName, InetAddress addr, String cmd, String src,
			String dst, FileStatus stat, UserGroupInformation ugi, DelegationTokenSecretManager dtSecretManager);
}
