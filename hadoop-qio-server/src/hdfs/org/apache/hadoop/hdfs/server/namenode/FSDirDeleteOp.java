...
class FSDirDeleteOp {
	 ...
	 *
	 * @param iip
	 *            the INodesInPath instance containing all the INodes for the
	 *            path
	 * @param collectedBlocks
	 *            Blocks under the deleted directory
	 * @param removedINodes
	 *            INodes that should be removed from inodeMap
	 * @return the number of files that have been removed
	 */
	static long delete(FSDirectory fsd, INodesInPath iip, BlocksMapUpdateInfo collectedBlocks,
	 ...
	 *
	 */
	static BlocksMapUpdateInfo delete(FSNamesystem fsn, String src, boolean recursive, boolean logRetryCache)
			throws IOException {
	 ...
	 * directory with quota <br>
	 * Note: This is to be used by
	 * {@link org.apache.hadoop.hdfs.server.namenode.FSEditLog} only. <br>
	 * 
	 * @param src
	 ...
	 * For large directories, deletion is incremental. The blocks under the
	 * directory are collected and deleted a small number at a time holding the
	 * {@link org.apache.hadoop.hdfs.server.namenode.FSNamesystem} lock.
	 * <p>
	 * For small directory or file the deletion is done in one shot.
	 */
	static BlocksMapUpdateInfo deleteInternal(FSNamesystem fsn, String src, INodesInPath iip, boolean logRetryCache)
			throws IOException {
		...
		if (iip.length() < 1 || iip.getLastINode() == null) {
			if (NameNode.stateChangeLog.isDebugEnabled()) {
				NameNode.stateChangeLog.debug(
						"DIR* FSDirectory.unprotectedDelete: failed to remove " + src + " because it does not exist");
			}
			return false;
	 ...
	 * 
	 * @param iip
	 *            the inodes resolved from the path
	 * @param collectedBlocks
	 ...
	 */
	private static long unprotectedDelete(FSDirectory fsd, INodesInPath iip, BlocksMapUpdateInfo collectedBlocks,
		...
		if (!targetNode.isInLatestSnapshot(latestSnapshot)) {
			...
			QuotaCounts counts = targetNode.cleanSubtree(fsd.getBlockStoragePolicySuite(), CURRENT_STATE_ID,
