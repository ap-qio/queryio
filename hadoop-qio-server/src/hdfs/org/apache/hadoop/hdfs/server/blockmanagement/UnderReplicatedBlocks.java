...
/**
 * Keep prioritized queues of under replicated blocks. Blocks have replication
 * priority, with priority {@link #QUEUE_HIGHEST_PRIORITY} indicating the
 * highest priority.
 * </p>
 * Having a prioritised queue allows the {@link BlockManager} to select which
 * blocks to replicate first -it tries to give priority to data that is most at
 * risk or considered most valuable.
 *
 * <p/>
 * The policy for choosing which priority to give added blocks is implemented in
 * {@link #getPriority(Block, int, int, int)}.
 * </p>
 * <p>
 ...
 * first. That is blocks with only one copy, or blocks with zero live copies but
 * a copy in a node being decommissioned. These blocks are at risk of loss if
 * the disk or server on which they remain fails.</li>
 * <li>{@link #QUEUE_VERY_UNDER_REPLICATED}: blocks that are very
 * under-replicated compared to their expected values. Currently that means the
 * ratio of the ratio of actual:expected means that there is <i>less than</i>
 * 1:3.</li>. These blocks may not be at risk, but they are clearly considered
 * "important".
 * <li>{@link #QUEUE_UNDER_REPLICATED}: blocks that are also under replicated,
 * and the ratio of actual:expected is good enough that they do not need to go
 * into the {@link #QUEUE_VERY_UNDER_REPLICATED} queue.</li>
 * <li>{@link #QUEUE_REPLICAS_BADLY_DISTRIBUTED}: there are as least as many
 * copies of a block as required, but the blocks are not adequately distributed.
 * Loss of a rack/switch could take all copies off-line.</li>
 * <li>{@link #QUEUE_WITH_CORRUPT_BLOCKS} This is for blocks that are corrupt
 * and for which there are no-non-corrupt copies (currently) available. The
 * policy here is to keep those corrupt blocks replicated, but give blocks that
 * are not corrupt higher priority.</li>
 * </ol>
 */
class UnderReplicatedBlocks implements Iterable<Block> {
	...
	/**
	 * The queue for blocks that have the right number of replicas, but which
	 * the block manager felt were badly distributed: {@value}
	 */
		...
		if (priorityQueues.get(priLevel).add(block)) {
			...
			NameNode.blockStateChangeLog.debug(
					"BLOCK* NameSystem.UnderReplicationBlock.add: {}"
							...
							+ " neededReplications at priority level {}",
	...
	synchronized boolean remove(Block block, int oldReplicas, int decommissionedReplicas, int oldExpectedReplicas) {
		...
		if (priLevel == QUEUE_WITH_CORRUPT_BLOCKS && oldExpectedReplicas == 1 && removedBlock) {
			corruptReplOneBlocks--;
			assert corruptReplOneBlocks >= 0 : "Number of corrupt blocks with replication factor 1 "
	 ...
	 *
	 * The priLevel parameter is a hint of which queue to query first: if
	 * negative or &gt;= {@link #LEVEL} this shortcutting is not attmpted.
	 *
	...
	boolean remove(Block block, int priLevel) {
		if (priLevel >= 0 && priLevel < LEVEL && priorityQueues.get(priLevel).remove(block)) {
			NameNode.blockStateChangeLog.debug(
					"BLOCK* NameSystem.UnderReplicationBlock.remove: Removing block {}" + " from priority queue {}",
					block, priLevel);
			return true;
		} else {
	 ...
	 * remove it from the block queue. Regardless of whether or not the block is
	 * in the block queue of (recalculate) priority, an attempt is made to add
	 * it to that queue. This ensures that the block will be in its expected
	 ...
	 * represents its replication priority. Replication index will be tracked
	 * for each priority list separately in priorityToReplIdx map. Iterates
	 * through all priority lists and find the elements after replication index.
	 * Once the last priority lists reaches to end, all replication indexes will
	 * be set to 0 and start from 1st priority list to fulfill the blockToProces
	 * count.
	 * 
	...
	public synchronized List<List<Block>> chooseUnderReplicatedBlocks(int blocksToProcess) {
		...
		for (int priority = 0; priority < LEVEL; priority++) {
			// Go through all blocks that need replications with current
			// priority.
			...
			if (!neededReplicationsIterator.hasNext() && neededReplicationsIterator.getPriority() == LEVEL - 1) {
				// reset all priorities replication index to 0 because there is
				// no
