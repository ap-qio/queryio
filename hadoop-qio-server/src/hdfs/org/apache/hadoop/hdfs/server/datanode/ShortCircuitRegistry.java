 ...
 * ShortCircuitRegistry generates and manages these segments. Each segment has a
 * randomly generated 128-bit ID which uniquely identifies it. The segments each
 * contain several "slots."
 *
 ...
 * zero-copy reads on these replicas (the ZCR interface has no way of verifying
 * checksums.)
 * 
 * When a DN needs to munlock a block, it needs to first wait for the block to
 * be unanchored by clients doing a no-checksum read or a zero-copy read. The DN
 * also marks the block's slots as "unanchorable" to prevent additional clients
 * from initiating these operations in the future.
 * 
...
public class ShortCircuitRegistry {
	...
	public static class RegisteredShm extends ShortCircuitShm implements DomainSocketWatcher.Handler {
		...
		RegisteredShm(String clientName, ShmId shmId, FileInputStream stream, ShortCircuitRegistry registry)
				throws IOException {
			super(shmId, stream);
	...
	public ShortCircuitRegistry(Configuration conf) throws IOException {
		...
		try {
			int interruptCheck = conf.getInt(DFS_SHORT_CIRCUIT_SHARED_MEMORY_WATCHER_INTERRUPT_CHECK_MS,
					DFS_SHORT_CIRCUIT_SHARED_MEMORY_WATCHER_INTERRUPT_CHECK_MS_DEFAULT);
			if (interruptCheck <= 0) {
				throw new IOException(
						DFS_SHORT_CIRCUIT_SHARED_MEMORY_WATCHER_INTERRUPT_CHECK_MS + " was set to " + interruptCheck);
			}
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("created new ShortCircuitRegistry with interruptCheck=" + interruptCheck + ", shmPath="
						+ shmFactory.getPath());
			}
		} catch (IOException e) {
			if (LOG.isDebugEnabled()) {
	 ...
	 * not use the corresponding replica for new read or mmap operations
	 * (although existing, ongoing read or mmap operations will complete.)
	 *
	 * @param blockId
	 *            The block ID.
	 */
	 ...
	 *
	 * @param clientName
	 *            Client name as reported by the client.
	 * @param sock
	 *            The DomainSocket to associate with this memory segment. When
	 *            this socket is closed, or the other side writes anything to
	 *            the socket, the segment will be closed. This can happen at any
	 *            time, including right after this function returns.
	 * @return A NewShmInfo object. The caller must close the NewShmInfo object
