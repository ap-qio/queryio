...
public interface FsVolumeSpi {
	 ...
	 * volume.
	 *
	 * It is caller's responsibility to close {@link FsVolumeReference} to
	 * decrease the reference count on the volume.
	 */
	...
	/**
	 * Reserve disk space for an RBW block so a writer does not run out of space
	...
	/**
	 * BlockIterator will return ExtendedBlock entries from a block pool in this
	 * volume. The entries will be returned in sorted order.
	 * <p/>
	 *
	...
	public interface BlockIterator extends Closeable {
		 ...
		 * longer exists.
		 *
		 * @return The next block, or null if there are no more blocks. Null if
		 *         there was an error determining the next block.
		 *
		 * @throws IOException
		 *             If there was an error getting the next block in this
		 *             volume. In this case, EOF will be set on the iterator.
		 */
		...
		/**
		 * Save this block iterator to the underlying volume. Any existing saved
		 * block iterator with this name will be overwritten. maxStalenessMs
		 * will not be saved.
		 *
		 * @throws IOException
		 *             If there was an error when saving the block iterator.
		 */
	...
	/**
	 * Create a new block iterator. It will start at the beginning of the block
	 * set.
	 *
	 * @param bpid
	 *            The block pool id to iterate over.
	 * @param name
	 ...
	 * @throws IOException
	 *             If there was an IO error loading the saved block iterator.
	 */
