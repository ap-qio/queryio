...
public abstract class INode implements INodeAttributes, Diff.Element<byte[]> {
	...
	/**
	 * When {@link #recordModification} is called on a referred node, this
	 * method tells which snapshot the modification should be associated with:
	 * the snapshot that belongs to the SRC tree of the rename operation, or the
	 * snapshot belonging to the DST tree.
	 * 
	 * @param latestInDst
	 *            id of the latest snapshot in the DST tree above the reference
	 *            node
	 ...
	 *            The id of the latest snapshot that has been taken. Note that
	 *            it is {@link Snapshot#CURRENT_STATE_ID} if no snapshots have
	 *            been taken.
	 */
	...
	/**
	 * Clean the subtree under this inode and collect the blocks from the
	 * descents for further block deletion/update. The current inode can either
	 * resides in the current tree or be stored as a snapshot copy.
	 * 
	 * <pre>
	 ...
	 *            The id of the latest snapshot before the to-be-deleted
	 *            snapshot. When deleting a current inode, this parameter
	 ...
	 */
	public abstract QuotaCounts cleanSubtree(final BlockStoragePolicySuite bsps, final int snapshotId,
			int priorSnapshotId, BlocksMapUpdateInfo collectedBlocks, List<INode> removedINodes);
	 ...
	 * @param bsps
	 *            block storage policy suite to calculate intended storage type
	 *            usage This is needed because
	 *            INodeReference#destroyAndCollectBlocks() needs to call
	 *            INode#cleanSubtree(), which calls INode#computeQuotaUsage().
	 * @param collectedBlocks
	 *            blocks collected from the descents for further block
	 ...
	 *            inodeMap
	 */
	public abstract void destroyAndCollectBlocks(BlockStoragePolicySuite bsps, BlocksMapUpdateInfo collectedBlocks,
	...
	public final ContentSummary computeAndConvertContentSummary(ContentSummaryComputationContext summary) {
		...
		return new ContentSummary.Builder().length(counts.getLength())
				.fileCount(counts.getFileCount() + counts.getSymlinkCount()).directoryCount(counts.getDirectoryCount())
				.quota(q.getNameSpace()).spaceConsumed(counts.getStoragespace()).spaceQuota(q.getStorageSpace())
	...
	/**
	 * Check and add namespace/storagespace/storagetype consumed to itself and
	 * the ancestors.
	 * 
	 * @throws QuotaExceededException
	 *             if quote is violated.
	 */
	...
	public QuotaCounts getQuotaCounts() {
		return new QuotaCounts.Builder().nameSpace(HdfsConstants.QUOTA_RESET).storageSpace(HdfsConstants.QUOTA_RESET)
	...
	/**
	 * Count subtree {@link Quota#NAMESPACE} and {@link Quota#STORAGESPACE}
	 * usages. Entry point for FSDirectory where blockStoragePolicyId is given
	 * its initial value.
	 */
	public final QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps) {
		final byte storagePolicyId = isSymlink() ? BlockStoragePolicySuite.ID_UNSPECIFIED : getStoragePolicyID();
		return computeQuotaUsage(bsps, storagePolicyId, new QuotaCounts.Builder().build(), true,
	 ...
	 * {@link DstReference} node. To avoid circles while quota usage
	 * computation, we have the following rules:
	 * 
	 * <pre>
	 ...
	 * subtree) in the regular manner, i.e., including every inode in the
	 * current tree and in snapshot copies, as well as the size of diff list.
	 * 
	 ...
	 *            Block storage policy suite to calculate intended storage type
	 *            usage
	 * @param blockStoragePolicyId
	 *            block storage policy id of the current INode
	 * @param counts
	 *            The subtree counts for returning.
	 * @param useCache
	 *            Whether to use cached quota usage. Note that {@link WithName}
	 *            node never uses cache for its subtree.
	 * @param lastSnapshotId
	 *            {@link Snapshot#CURRENT_STATE_ID} indicates the computation is
	 *            in the current tree. Otherwise the id indicates the
	 *            computation range for a {@link WithName} node.
	...
	/**
	 * @return the parent as a reference if this is a referred inode; otherwise,
	 *         return null.
	 */
	...
	/**
	 * @return the latest block storage policy id of the INode. Specifically, if
	 *         a storage policy is directly specified on the INode then return
	 *         the ID of that policy. Otherwise follow the latest parental path
	 *         and return the ID of the first specified storage policy.
	 */
	 ...
	 * 
	 * @param parentStoragePolicyId
	 *            the storage policy ID of the parent directory
	 * @return the storage policy ID of this INode. Note that for an
	 *         {@link INodeSymlink} we return
	 *         {@link BlockStoragePolicySuite#ID_UNSPECIFIED} instead of
	 *         throwing Exception
	 */
	 ...
	 * 
	 * @param prefix
	 *            The prefix string that each line should print.
	 */
	@VisibleForTesting
	...
	/**
	 * INode feature such as {@link FileUnderConstructionFeature} and
	 * {@link DirectoryWithQuotaFeature}.
	 */
