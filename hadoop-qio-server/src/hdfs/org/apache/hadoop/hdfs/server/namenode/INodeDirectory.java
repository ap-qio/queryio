...
public class INodeDirectory extends INodeWithAdditionalFields implements INodeDirectoryAttributes {
	...
	/**
	 * If feature list contains a {@link DirectoryWithSnapshotFeature}, return
	...
	public Snapshot removeSnapshot(BlockStoragePolicySuite bsps, String snapshotName,
			BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) throws SnapshotException {
		return getDirectorySnapshottableFeature().removeSnapshot(bsps, this, snapshotName, collectedBlocks,
				removedINodes);
	}
	...
	public void replaceChild(INode oldChild, final INode newChild, final INodeMap inodeMap) {
		...
		Preconditions.checkState(oldChild == children.get(i)
				|| oldChild == children.get(i).asReference().getReferredINode().asReference().getReferredINode());
	...
	INodeReference.WithName replaceChild4ReferenceWithName(INode oldChild, int latestSnapshotId) {
		...
		final INodeReference.WithName ref = new INodeReference.WithName(this, withCount, oldChild.getLocalNameBytes(),
				latestSnapshotId);
	 ...
	 *            from the corresponding snapshot; otherwise, get the result
	 ...
	 *         list; {@link Snapshot#NO_SNAPSHOT_ID} if the inode is neither in
	 *         the children list nor in any snapshot; otherwise the snapshot id
	 *         of the corresponding snapshot diff list.
	 */
	 ...
	 *         otherwise, return the children list corresponding to the
	 *         snapshot. Note that the returned list is never null.
	 */
	...
	/**
	 * Remove the specified child from this directory. The basic remove method
	 * which actually calls children.remove(..).
	 *
	 * @param child
	 ...
	 *            INode to insert
	 * @param setModTime
	 *            set modification time for the parent node not needed when
	 *            replaying the addition and the parent already has the proper
	 *            mod time
	...
	/**
	 * Add the node to the children list at the given insertion point. The basic
	 * add method which actually calls children.add(..).
	 */
	...
	public QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, QuotaCounts counts,
		...
		// we are computing the quota usage for a specific snapshot here, i.e.,
		// the
		// computation only includes files/directories that exist at the time of
		// the
		// given snapshot
	...
	private QuotaCounts computeDirectoryQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId,
	...
	protected ContentSummaryComputationContext computeDirectoryContentSummary(ContentSummaryComputationContext summary,
			int snapshotId) {
	 ...
	 * (WithName instance) in both the children list and a created list, delete
	 * the reference node from the children list, and add it to the
	 * corresponding deleted list.
	 * 
	 ...
	 */
	public void undoRename4DstParent(final BlockStoragePolicySuite bsps, final INode deletedChild, int latestSnapshotId)
			throws QuotaExceededException {
		DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
		Preconditions.checkState(sf != null, "Directory does not have snapshot feature");
		...
		// update quota usage if adding is successfully and the old child has
		// not
	...
	public QuotaCounts cleanSubtreeRecursively(final BlockStoragePolicySuite bsps, final int snapshot, int prior,
			final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes,
			final Map<INode, INode> excludedNodes) {
		QuotaCounts counts = new QuotaCounts.Builder().build();
		// in case of deletion snapshot, since this call happens after we modify
		// the diff list, the snapshot to be deleted has been combined or
		// renamed
	...
	@Override
	public void destroyAndCollectBlocks(final BlockStoragePolicySuite bsps, final BlocksMapUpdateInfo collectedBlocks,
	...
	@Override
	public QuotaCounts cleanSubtree(final BlockStoragePolicySuite bsps, final int snapshotId, int priorSnapshotId,
			final BlocksMapUpdateInfo collectedBlocks, final List<INode> removedINodes) {
		DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
		...
		if (priorSnapshotId == Snapshot.NO_SNAPSHOT_ID && snapshotId == Snapshot.CURRENT_STATE_ID) {
			// destroy the whole subtree and collect blocks that should be
			// deleted
			QuotaCounts counts = new QuotaCounts.Builder().build();
	 ...
	 * 
	 * \- foo (INodeDirectory@33dd2717) \- sub1 (INodeDirectory@442172) +- file1
	 * (INodeFile@78392d4) +- file2 (INodeFile@78392d5) +- sub11
	 * (INodeDirectory@8400cff) \- file3 (INodeFile@78392d6) \- z_file4
	 * (INodeFile@45848712)
	 */
