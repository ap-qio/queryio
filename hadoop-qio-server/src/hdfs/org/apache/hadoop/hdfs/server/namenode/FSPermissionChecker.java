...
/**
 * Class that helps in checking file system permission. The state of this class
 * need not be synchronized as it has data structures that are read-only.
 * 
...
class FSPermissionChecker implements AccessControlEnforcer {
	...
	/** @return a string for throwing {@link AccessControlException} */
	private String toAccessControlString(INodeAttributes inodeAttrib, String path, FsAction access, FsPermission mode,
			boolean deniedFromAcl) {
		StringBuilder sb = new StringBuilder("Permission denied: ").append("user=").append(getUser()).append(", ")
				.append("access=").append(access).append(", ").append("inode=\"").append(path).append("\":")
				.append(inodeAttrib.getUserName()).append(':').append(inodeAttrib.getGroupName()).append(':')
				.append(inodeAttrib.isDirectory() ? 'd' : '-').append(mode);
		if (deniedFromAcl) {
			sb.append("+");
		}
		return sb.toString();
	 ...
	 * 
	 *             Guarded by {@link FSNamesystem#readLock()} Caller of this
	 *             method must hold that lock.
	 */
	void checkPermission(INodesInPath inodesInPath, boolean doCheckOwner, FsAction ancestorAccess,
		...
		enforcer.checkPermission(fsOwner, supergroup, callerUgi, inodeAttrs, inodes, pathByNameArr, snapshotId, path,
	...
	public void checkPermission(String fsOwner, String supergroup, UserGroupInformation callerUgi,
			...
			int ancestorIndex, boolean doCheckOwner, FsAction ancestorAccess, FsAction parentAccess, FsAction access,
	...
	private void checkOwner(INodeAttributes inode) throws AccessControlException {
		...
		throw new AccessControlException(
				"Permission denied. user=" + getUser() + " is not the owner of inode=" + inode);
	}
	...
	/** Guarded by {@link FSNamesystem#readLock()} */
	private void checkSubAccess(byte[][] pathByNameArr, int pathIdx, INode inode, int snapshotId, FsAction access,
	...
	/**
	 * Checks requested access against an Access Control List. This method
	 * relies on finding the ACL data in the relevant portions of
	 * {@link FsPermission} and {@link AclFeature} as implemented in the logic
	 * of {@link AclStorage}. This method also relies on receiving the ACL
	 * entries in sorted order. This is assumed to be true, because the ACL
	 * modification methods in {@link AclTransformation} sort the resulting
	 * entries.
	 *
	 * More specifically, this method depends on these invariants in an ACL: -
	 * The list must be sorted. - Each entry in the list must be unique by scope
	 * + type + name. - There is exactly one each of the unnamed
	 * user/group/other entries. - The mask entry must not have a name. - The
	 * other entry must not have a name. - Default entries may be present, but
	 * they are ignored during enforcement.
	 *
	 * @param inode
	 *            INodeAttributes accessed inode
	 * @param snapshotId
	 *            int snapshot ID
	 * @param access
	 *            FsAction requested permission
	 * @param mode
	 *            FsPermission mode from inode
	 * @param aclFeature
	 *            AclFeature of inode
	 * @throws AccessControlException
	 *             if the ACL denies permission
	 */
	private void checkAccessAcl(INodeAttributes inode, String path, FsAction access, FsPermission mode,
		...
		if (!foundMatch) {
			for (int pos = 0, entry; pos < aclFeature.getEntriesSize(); pos++) {
				...
				if (type == AclEntryType.USER) {
					// Use named user entry with mask from permission bits
					// applied if user
					// matches name.
					...
					// Use group entry (unnamed or named) with mask from
					// permission bits
					// applied if user is a member and entry grants access. If
					// user is a
					// member of multiple groups that have entries that grant
					// access, then
					// it doesn't matter which is chosen, so exit early after
					// first match.
	...
	private void checkStickyBit(INodeAttributes parent, INodeAttributes inode) throws AccessControlException {
		...
		throw new AccessControlException(
				"Permission denied by sticky bit setting:" + " user=" + getUser() + ", inode=" + inode);
	}
	 ...
	 *
	 * @param pool
	 *            CachePool being accessed
	 * @param access
	 *            type of action being performed on the cache pool
	 * @throws AccessControlException
	 *             if pool cannot be accessed
	 */
	public void checkPermission(CachePool pool, FsAction access) throws AccessControlException {
		...
		throw new AccessControlException("Permission denied while accessing pool " + pool.getPoolName() + ": user "
				+ getUser() + " does not have " + access.toString() + " permissions.");
	}
}
