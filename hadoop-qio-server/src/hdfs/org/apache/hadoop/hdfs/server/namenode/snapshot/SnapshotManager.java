...
public class SnapshotManager implements SnapshotStatsMXBean {
	...
	private void checkNestedSnapshottable(INodeDirectory dir, String path) throws SnapshotException {
		...
		for (INodeDirectory s : snapshottables.values()) {
			if (s.isAncestorDirectory(dir)) {
				throw new SnapshotException("Nested snapshottable directories not allowed: path=" + path
						+ ", the subdirectory " + s.getFullPathName() + " is already a snapshottable directory.");
			}
			if (dir.isAncestorDirectory(s)) {
				throw new SnapshotException("Nested snapshottable directories not allowed: path=" + path
						+ ", the ancestor " + s.getFullPathName() + " is already a snapshottable directory.");
			}
		}
	}

	/**
	 * Set the given directory as a snapshottable directory. If the path is
	 * already a snapshottable directory, update the quota.
	 */
	 ...
	 *             if there are snapshots in the directory.
	 */
	...
	/**
	 * Find the source root directory where the snapshot will be taken for a
	 * given path.
	 *
	...
	/**
	 * Create a snapshot of the given path. It is assumed that the caller will
	 * perform synchronization.
	 *
	 * @param iip
	 *            the INodes resolved from the snapshottable directory's path
	 * @param snapshotName
	...
	public String createSnapshot(final INodesInPath iip, String snapshotRoot, String snapshotName) throws IOException {
		...
		if (snapshotCounter == getMaxSnapshotID()) {
			// We have reached the maximum allowable snapshot ID and since we
			// don't
	 ...
	 * @throws IOException
	 */
	public void deleteSnapshot(final INodesInPath iip, final String snapshotName, BlocksMapUpdateInfo collectedBlocks,
			final List<INode> removedINodes) throws IOException {
		INodeDirectory srcRoot = getSnapshottableRoot(iip);
	 ...
	 * @throws IOException
	 *             Throw IOException when 1) the given path does not lead to an
	 *             existing snapshottable directory, and/or 2) the snapshot with
	 *             the old name does not exist for the directory, and/or 3)
	 *             there exists a snapshot with the new name for the directory
	 */
	public void renameSnapshot(final INodesInPath iip, final String snapshotRoot, final String oldSnapshotName,
	...
	/**
	 * Write {@link #snapshotCounter}, {@link #numSnapshots}, and all snapshots
	 * to the DataOutput.
	 */
	...
	/**
	 * Read values of {@link #snapshotCounter}, {@link #numSnapshots}, and all
	 * snapshots from the DataInput
	 */
	...
	/**
	 * List all the snapshottable directories that are owned by the current
	 * user.
	 * 
	 * @param userName
	 *            Current user name.
	 * @return Snapshottable directories that are owned by the current user,
	 *         represented as an array of {@link SnapshottableDirectoryStatus}.
	 *         If {@code userName} is null, return all the snapshottable dirs.
	 */
		...
		for (INodeDirectory dir : snapshottables.values()) {
			if (userName == null || userName.equals(dir.getUserName())) {
				SnapshottableDirectoryStatus status = new SnapshottableDirectoryStatus(dir.getModificationTime(),
						...
						dir.getLocalNameBytes(), dir.getId(), dir.getChildrenNum(Snapshot.CURRENT_STATE_ID),
						dir.getDirectorySnapshottableFeature().getNumSnapshots(),
						dir.getDirectorySnapshottableFeature().getSnapshotQuota(), dir.getParent() == null
								? DFSUtil.EMPTY_BYTES : DFSUtil.string2Bytes(dir.getParent().getFullPathName()));
	 ...
	 */
	public SnapshotDiffReport diff(final INodesInPath iip, final String snapshotRootPath, final String from,
		...
		final SnapshotDiffInfo diffs = snapshotRoot.getDirectorySnapshottableFeature().computeDiff(snapshotRoot, from,
				to);
		return diffs != null ? diffs.generateReport()
				: new SnapshotDiffReport(snapshotRootPath, from, to, Collections.<DiffReportEntry>emptyList());
	}
	...
	public static SnapshottableDirectoryStatus.Bean toBean(INodeDirectory d) {
		return new SnapshottableDirectoryStatus.Bean(d.getFullPathName(),
				d.getDirectorySnapshottableFeature().getNumSnapshots(),
				d.getDirectorySnapshottableFeature().getSnapshotQuota(), d.getModificationTime(),
				Short.valueOf(Integer.toOctalString(d.getFsPermissionShort())), d.getUserName(), d.getGroupName());
	}
