...
public class FSEditLogLoader {
	 ...
	 */
	long loadFSEdits(EditLogInputStream edits, long expectedStartingTxId, StartupOption startOpt,
	...
	long loadEditRecords(EditLogInputStream in, boolean closeOnExit, long expectedStartingTxId, StartupOption startOpt,
			MetaRecoveryContext recovery) throws IOException {
			...
			while (true) {
				try {
					...
					try {
						...
						MetaRecoveryContext.editLogLoaderPrompt("We failed to read txId " + expectedTxId, recovery,
					...
					if (op.hasTransactionId()) {
						if (op.getTransactionId() > expectedTxId) {
							MetaRecoveryContext.editLogLoaderPrompt(
									"There appears " + "to be a gap in the edit log.  We expected txid " + expectedTxId
											+ ", but got txid " + op.getTransactionId() + ".",
					...
					try {
						...
						MetaRecoveryContext.editLogLoaderPrompt(
								"Failed to " + "apply edit log operation " + op + ": error " + e.getMessage(), recovery,
								"applying edits");
					}
	...
	private long getAndUpdateLastInodeId(long inodeIdFromOp, int logVersion, long lastInodeId) throws IOException {
		...
		if (inodeId == INodeId.GRANDFATHER_INODE_ID) {
			...
			// need to reset lastInodeId. fsnamesys gets lastInodeId firstly
			// from
	...
	private long applyEditLogOp(FSEditLogOp op, FSDirectory fsDir, StartupOption startOpt, int logVersion,
		...
		case OP_ADD: {
			...
			if (FSNamesystem.LOG.isDebugEnabled()) {
				FSNamesystem.LOG.debug(op.opCode + ": " + path + " numblocks : " + addCloseOp.blocks.length
						+ " clientHolder " + addCloseOp.clientName + " clientMachine " + addCloseOp.clientMachine);
			}
			...
			if (oldFile == null) { // this is OP_ADD on a new file (case 1)
				...
				newFile = fsDir.addFileForEditLog(inodeId, iip.getExistingINodes(), iip.getLastLocalName(),
						addCloseOp.permissions, addCloseOp.aclEntries, addCloseOp.xAttrs, replication, addCloseOp.mtime,
						addCloseOp.atime, addCloseOp.blockSize, true, addCloseOp.clientName, addCloseOp.clientMachine,
				...
				if (!oldFile.isUnderConstruction()) {
					// This is case 3: a call to append() on an already-closed
					// file.
					if (FSNamesystem.LOG.isDebugEnabled()) {
		...
		case OP_CLOSE: {
			...
			// One might expect that you could use removeLease(holder, path)
			// here,
		...
		case OP_APPEND: {
			...
			if (!file.isUnderConstruction()) {
				LocatedBlock lb = fsNamesys.prepareFileForAppend(path, iip, appendOp.clientName, appendOp.clientMachine,
		...
		case OP_ADD_BLOCK: {
			...
			if (FSNamesystem.LOG.isDebugEnabled()) {
				FSNamesystem.LOG
						.debug(op.opCode + ": " + path + " new block id : " + addBlockOp.getLastBlock().getBlockId());
			}
		...
		case OP_SET_REPLICATION: {
			...
			FSDirAttrOp.unprotectedSetReplication(fsDir,
					renameReservedPathsOnUpgrade(setReplicationOp.path, logVersion), replication, null);
			break;
		}
		...
		case OP_MKDIR: {
			...
			FSDirMkdirOp.mkdirForEditLog(fsDir, inodeId, renameReservedPathsOnUpgrade(mkdirOp.path, logVersion),
		...
		case OP_SET_PERMISSIONS: {
			...
			FSDirAttrOp.unprotectedSetPermission(fsDir, renameReservedPathsOnUpgrade(setPermissionsOp.src, logVersion),
		...
		case OP_SET_QUOTA_BY_STORAGETYPE:
			...
			FSDirAttrOp.unprotectedSetQuota(fsDir,
					renameReservedPathsOnUpgrade(setQuotaByStorageTypeOp.src, logVersion), HdfsConstants.QUOTA_DONT_SET,
					setQuotaByStorageTypeOp.dsQuota, setQuotaByStorageTypeOp.type);
			break;
		...
		case OP_SYMLINK: {
			...
			FSDirSymlinkOp.unprotectedAddSymlink(fsDir, iip.getExistingINodes(), iip.getLastLocalName(), inodeId,
					symlinkOp.value, symlinkOp.mtime, symlinkOp.atime, symlinkOp.permissionStatus);
		...
		case OP_GET_DELEGATION_TOKEN: {
			...
			fsNamesys.getDelegationTokenSecretManager().addPersistedDelegationToken(getDelegationTokenOp.token,
					getDelegationTokenOp.expiryTime);
			break;
		}
		case OP_RENEW_DELEGATION_TOKEN: {
			RenewDelegationTokenOp renewDelegationTokenOp = (RenewDelegationTokenOp) op;
			fsNamesys.getDelegationTokenSecretManager().updatePersistedTokenRenewal(renewDelegationTokenOp.token,
		...
		case OP_DELETE_SNAPSHOT: {
			...
			INodesInPath iip = fsDir.getINodesInPath4Write(snapshotRoot);
			fsNamesys.getSnapshotManager().deleteSnapshot(iip, deleteSnapshotOp.snapshotName, collectedBlocks,
					removedINodes);
		...
		case OP_RENAME_SNAPSHOT: {
			...
			INodesInPath iip = fsDir.getINodesInPath4Write(snapshotRoot);
			fsNamesys.getSnapshotManager().renameSnapshot(iip, snapshotRoot, renameSnapshotOp.snapshotOldName,
		...
		case OP_ROLLING_UPGRADE_FINALIZE: {
			...
			if (fsNamesys.isRollingUpgrade()) {
				...
				// We can get FINALIZE without corresponding START, if NN is
				// restarted
	 ...
	 * @throws IOException
	 */
	private void updateBlocks(FSDirectory fsDir, BlockListUpdatingOp op, INodesInPath iip, INodeFile file)
			throws IOException {
		...
		for (int i = 0; i < oldBlocks.length && i < newBlocks.length; i++) {
			...
			if (oldBlock.getBlockId() != newBlock.getBlockId()
				...
				throw new IOException("Mismatched block IDs or generation stamps, " + "attempting to replace block "
						+ oldBlock + " with " + newBlock + " as block # " + i + "/" + newBlocks.length + " of " + path);
			}
			...
			if (changeMade) {
				// The state or gen-stamp of the block has changed. So, we may
				// be
				// able to process some messages from datanodes that we
				// previously
		...
		if (newBlocks.length < oldBlocks.length) {
			...
			for (int i = oldBlocks.length; i < newBlocks.length; i++) {
				...
				if (!op.shouldCompleteLastBlock()) {
					...
					// what about an old-version fsync() where fsync isn't
					// called
	...
	private void incrOpCount(FSEditLogOpCodes opCode, EnumMap<FSEditLogOpCodes, Holder<Integer>> opCounts, Step step,
			Counter counter) {
	...
	private void check203UpgradeFailure(int logVersion, Throwable e) throws IOException {
		// 0.20.203 version version has conflicting opcodes with the later
		// releases.
		// The editlog must be emptied by restarting the namenode, before
		// proceeding
		// with the upgrade.
	...
	/**
	 * Find the last valid transaction ID in the stream. If there are invalid or
	 * corrupt transactions in the middle of the stream, validateEditLog will
	 * skip over them. This reads through the stream but does not close it.
	 */
	 ...
	 * information from the given edits. The step always includes the log's
	 * name. If the log has a known length, then the length is included in the
	 * step too.
	 * 
	 * @param edits
