...
class RamDiskAsyncLazyPersistService {
	...
	/**
	 * Create a RamDiskAsyncLazyPersistService with a set of volumes (specified
	 * by their root directories).
	 *
	 * The RamDiskAsyncLazyPersistService uses one ThreadPool per volume to do
	 * the async disk operations.
	 */
	...
	/**
	 * Gracefully shut down all ThreadPool. Will wait for all lazy persist tasks
	 * to finish.
	 */
	...
	class ReplicaLazyPersistTask implements Runnable {
		...
		public String toString() {
			// Called in AsyncLazyPersistService.execute for displaying error
			// messages.
		...
		public void run() {
			...
			try {
				...
				FsDatasetImpl.LOG.warn(
						"LazyWriter failed to async persist RamDisk block pool id: " + bpId + "block Id: " + blockId,
						e);
			} finally {
				if (!succeeded) {
