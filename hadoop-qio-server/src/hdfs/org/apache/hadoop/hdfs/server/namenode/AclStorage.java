...
public final class AclStorage {
	...
	/**
	 * If a default ACL is defined on a parent directory, then copies that
	 * default ACL to a newly created child file or directory.
	 *
	 * @param child
	 *            INode newly created child
	 */
	public static void copyINodeDefaultAcl(INode child) {
		...
		for (AclEntry entry : parentDefaultEntries) {
			...
			AclEntry.Builder builder = new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(type)
					.setName(name);

			// The child's initial permission bits are treated as the mode
			// parameter,
			// which can filter copied permission values for owner, mask and
			// other.
			...
			if (type == AclEntryType.USER && name == null) {
				...
				// This only happens if the default ACL is a minimal ACL:
				// exactly 3
				// entries corresponding to owner, group and other. In this
				// case,
				// filter the group permissions.
				permission = entry.getPermission().and(childPerm.getGroupAction());
			} else if (type == AclEntryType.MASK) {
				// Group bits from mode parameter filter permission of mask
				// entry.
				permission = entry.getPermission().and(childPerm.getGroupAction());
	 ...
	 * logically has an ACL, even if no ACL has been set explicitly. If the
	 * inode does not have an extended ACL, then the result is a minimal ACL
	 * consising of exactly 3 entries that correspond to the owner, group and
	 * other permissions. This method always reads the inode's current state and
	 * does not support querying by snapshot ID. This is because the method is
	 * intended to support ACL modification APIs, which always apply a delta on
	 * top of current state.
	 *
	 * @param inode
	 *            INode to read
	...
	public static List<AclEntry> readINodeLogicalAcl(INode inode) {
		...
		// plus the 3 implicit entries (owner, group and other) from the
		// permission
		// bits.
		...
		if (!accessEntries.isEmpty()) {
			...
			existingAcl.add(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.USER)
			...
			existingAcl.add(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.MASK)
					.setPermission(perm.getGroupAction()).build());

			// Add other entry implied from other permission bits.
			existingAcl.add(new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.OTHER)
	 ...
	 *            INode to update
	 * @param newAcl
	 *            List<AclEntry> containing new ACL entries
	 * @param snapshotId
	 *            int latest snapshot ID of inode
	 * @throws AclException
	 *             if the ACL is invalid for the given inode
	...
	public static void updateINodeAcl(INode inode, List<AclEntry> newAcl, int snapshotId)
		...
		if (!AclUtil.isMinimalAcl(newAcl)) {
			...
			// This is a minimal ACL. Remove the ACL feature if it previously
			// had one.
			if (inode.getAclFeature() != null) {
				inode.removeAclFeature(snapshotId);
			}
	 ...
	 *
	 * @param accessEntries
	 *            List<AclEntry> access ACL entries
	 * @param defaultEntries
	 *            List<AclEntry> default ACL entries
	...
	private static AclFeature createAclFeature(List<AclEntry> accessEntries, List<AclEntry> defaultEntries) {
		// Pre-allocate list size for the explicit entries stored in the
		// feature,
		// which is all entries minus the 3 entries implicitly stored in the
		// permission bits.
		List<AclEntry> featureEntries = Lists
				.newArrayListWithCapacity((accessEntries.size() - 3) + defaultEntries.size());
	 ...
	 * the POSIX ACLs model, which presents the mask as the permissions of the
	 * group class.
	 *
	 * @param accessEntries
	 *            List<AclEntry> access ACL entries
	 * @param existingPerm
	 *            FsPermission existing permissions
	 * @return FsPermission new permissions
	 */
	private static FsPermission createFsPermissionForExtendedAcl(List<AclEntry> accessEntries,
		...
		return new FsPermission(accessEntries.get(0).getPermission(),
				...
				accessEntries.get(accessEntries.size() - 1).getPermission(), existingPerm.getStickyBit());
	}

	/**
	 * Creates the new FsPermission for an inode that is receiving a minimal
	 * ACL, based on its access ACL entries. For a correctly sorted ACL, the
	 * owner, group and other permissions are in order. Also preserve sticky bit
	 * and toggle ACL bit off.
	 *
	 * @param accessEntries
	 *            List<AclEntry> access ACL entries
	 * @param existingPerm
	 ...
	 * @return FsPermission new permissions
	 */
	private static FsPermission createFsPermissionForMinimalAcl(List<AclEntry> accessEntries,
			FsPermission existingPerm) {
		return new FsPermission(accessEntries.get(0).getPermission(), accessEntries.get(1).getPermission(),
				accessEntries.get(2).getPermission(), existingPerm.getStickyBit());
	}
