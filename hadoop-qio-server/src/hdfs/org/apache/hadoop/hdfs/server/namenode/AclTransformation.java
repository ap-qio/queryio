 ...
 * change. Different operations interpret the ACL spec differently. In the case
 * of adding an ACL to an inode that previously did not have one, the existing
 * ACL can be a "minimal ACL" containing exactly 3 entries for owner, group and
 * other, all derived from the {@link FsPermission} bits.
 *
 ...
 * is considered untrusted user input, so all operations pre-sort the ACL spec
 * as the first step.
 */
...
final class AclTransformation {
	...
	/**
	 * Filters (discards) any existing ACL entries that have the same scope,
	 * type and name of any entry in the ACL spec. If necessary, recalculates
	 * the mask entries. If necessary, default entries may be inferred by
	 * copying the permissions of the corresponding access entries. It is
	 * invalid to request removal of the mask entry from an ACL that would
	 * otherwise require a mask entry, due to existing named entries or an
	 * unnamed group entry.
	 *
	 * @param existingAcl
	 *            List<AclEntry> existing ACL
	 * @param inAclSpec
	 *            List<AclEntry> ACL spec describing entries to filter
	...
	public static List<AclEntry> filterDefaultAclEntries(List<AclEntry> existingAcl) throws AclException {
		...
		for (AclEntry existingEntry : existingAcl) {
			if (existingEntry.getScope() == DEFAULT) {
				// Default entries sort after access entries, so we can exit
				// early.
				break;
			}
			aclBuilder.add(existingEntry);
	 ...
	 *            List<AclEntry> existing ACL
	 * @param inAclSpec
	 *            List<AclEntry> ACL spec containing entries to merge
	 * @return List<AclEntry> new ACL
	 * @throws AclException
	 *             if validation fails
	 */
	 ...
	 * default entries, then the existing access entries are retained. If the
	 * ACL spec contains both access and default entries, then both are
	 * replaced.
	 *
	 * @param existingAcl
	 *            List<AclEntry> existing ACL
	 * @param inAclSpec
	 *            List<AclEntry> ACL spec containing replacement entries
	 * @return List<AclEntry> new ACL
	 * @throws AclException
	 *             if validation fails
	 */
	 ...
	 * -owner entry (unnamed user) -all named user entries (internal ordering
	 * undefined) -owning group entry (unnamed group) -all named group entries
	 * (internal ordering undefined) -mask entry -other entry All access ACL
	 ...
	 *
	 * @param aclBuilder
	 *            ArrayList<AclEntry> containing entries to build
	...
	private static List<AclEntry> buildAndValidateAcl(ArrayList<AclEntry> aclBuilder) throws AclException {
		if (aclBuilder.size() > MAX_ENTRIES) {
			throw new AclException("Invalid ACL: ACL has " + aclBuilder.size() + " entries, which exceeds maximum of "
					+ MAX_ENTRIES + ".");
		}
		...
		// Search for the required base access entries. If there is a default
		// ACL,
	 ...
	 * performed separately for each scope: access and default. This method is
	 * responsible for handling the following cases of mask calculation: 1.
	 * Throws an exception if the caller attempts to remove the mask entry of an
	 * existing ACL that requires it. If the ACL has any named entries, then a
	 * mask entry is required. 2. If the caller supplied a mask in the ACL spec,
	 * use it. 3. If the caller did not supply a mask, but there are ACL entry
	 * changes in this scope, then automatically calculate a new mask. The
	 * permissions of the new mask are the union of the permissions on the group
	 * entry and all named entries.
	 *
	 * @param aclBuilder
	 *            ArrayList<AclEntry> containing entries to build
	 * @param providedMask
	 *            EnumMap<AclEntryScope, AclEntry> mapping each scope to the
	 *            mask entry that was provided for that scope (if provided)
	 * @param maskDirty
	 *            EnumSet<AclEntryScope> which contains a scope if the mask
	 *            entry is dirty (added or deleted) in that scope
	 * @param scopeDirty
	 *            EnumSet<AclEntryScope> which contains a scope if any entry is
	 *            dirty (added or deleted) in that scope
	 * @throws AclException
	 *             if validation fails
	 */
	private static void calculateMasks(List<AclEntry> aclBuilder, EnumMap<AclEntryScope, AclEntry> providedMask,
		...
		for (AclEntryScope scope : scopeFound) {
			if (!providedMask.containsKey(scope) && maskNeeded.contains(scope) && maskDirty.contains(scope)) {
				...
				// Caller explicitly provided new mask, or we are preserving the
				// existing
				...
				aclBuilder.add(new AclEntry.Builder().setScope(scope).setType(MASK).setPermission(unionPerms.get(scope))
						.build());
			}
		}
	}
	...
	private static void copyDefaultsIfNeeded(List<AclEntry> aclBuilder) {
		...
		if (!scopedEntries.getDefaultEntries().isEmpty()) {
			...
			// Add all copied entries when done to prevent potential issues with
			// binary
	...
	private static final class ValidatedAclSpec implements Iterable<AclEntry> {
		...
		/**
		 * Creates a ValidatedAclSpec by pre-validating and sorting the given
		 * ACL entries. Pre-validation checks that it does not exceed the
		 * maximum entries. This check is performed before modifying the ACL,
		 * and it's actually insufficient for enforcing the maximum number of
		 * entries. Transformation logic can create additional entries
		 * automatically,such as the mask and some of the default entries, so we
		 * also need additional checks during transformation. The up-front check
		 * is still valuable here so that we don't run a lot of expensive
		 * transformation logic while holding the namesystem lock for an
		 * attacker who intentionally sent a huge ACL spec.
		 *
		 * @param aclSpec
		 *            List<AclEntry> containing unvalidated input ACL spec
		 * @throws AclException
		 *             if validation fails
		 */
