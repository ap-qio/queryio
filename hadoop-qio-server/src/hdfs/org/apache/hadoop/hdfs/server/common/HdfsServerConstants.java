...
public final class HdfsServerConstants {
	...
	public static enum RollingUpgradeStartupOption {
		...
		static RollingUpgradeStartupOption fromString(String s) {
			...
			throw new IllegalArgumentException(
					"Failed to convert \"" + s + "\" to " + RollingUpgradeStartupOption.class.getSimpleName());
		}
	...
	static public enum StartupOption {
		FORMAT("-format"), CLUSTERID("-clusterid"), GENCLUSTERID("-genclusterid"), REGULAR("-regular"), BACKUP(
				"-backup"), CHECKPOINT("-checkpoint"), UPGRADE("-upgrade"), ROLLBACK("-rollback"), FINALIZE(
						"-finalize"), ROLLINGUPGRADE("-rollingUpgrade"), IMPORT("-importCheckpoint"), BOOTSTRAPSTANDBY(
								"-bootstrapStandby"), INITIALIZESHAREDEDITS(
										"-initializeSharedEdits"), RECOVER("-recover"), FORCE("-force"), NONINTERACTIVE(
												"-nonInteractive"), RENAMERESERVED("-renameReserved"), METADATAVERSION(
														"-metadataVersion"), UPGRADEONLY("-upgradeOnly"),
		...
		// scenario.
		// TODO refactor StorageDirectory.analyzeStorage() so that we can do
		// away with
		...
		public String toString() {
			if (this == ROLLINGUPGRADE) {
				return new StringBuilder(super.toString()).append("(").append(getRollingUpgradeStartupOption())
						.append(")").toString();
			}
	...
	public static final int WRITE_TIMEOUT_EXTENSION = 5 * 1000; // for write
																// pipeline
	...
	static public enum BlockUCState {
		 ...
		 * The block has at least the configured minimal replication number of
		 * {@link ReplicaState#FINALIZED} replica(s), and is not going to be
		 * modified. NOTE, in some special cases, a block may be forced to
		 * COMPLETE state, even if it doesn't have required minimal
		 * replications.
		 */
		COMPLETE,
		/**
		 ...
		 * and needs to go through a recovery procedure, which synchronizes the
		 * existing replicas contents.
		 */
		UNDER_RECOVERY,
		/**
		 * The block is committed.<br>
		 * The client reported that all bytes are written to data-nodes with the
		 * given generation stamp and block length, but no
		 * {@link ReplicaState#FINALIZED} replicas has yet been reported by
