...
public abstract class EditLogOutputStream implements Closeable {
	...
	/**
	 * Write raw data to an edit log. This data should already have the
	 * transaction ID, checksum, etc included. It is for use within the
	 * BackupNode when replicating edits from the NameNode.
	 *
	 * @param bytes
	 *            the bytes to write.
	 * @param offset
	 *            offset in the bytes to write from
	 * @param length
	 ...
	 * 
	 * @param layoutVersion
	 *            The LayoutVersion of the journal
	 * @throws IOException
	 */
	 ...
	 * 
	 * @throws IOException
	 *             if the journal can't be closed, or if there are unflushed
	 *             edits
	 */
	@Override
	...
	/**
	 * Close the stream without necessarily flushing any pending data. This may
	 * be called after a previous write or close threw an exception.
	 */
	 ...
	 * The buffered edits can be flushed when the buffer becomes full or a
	...
	/**
	 * @return a short text snippet suitable for describing the current status
	 *         of the stream
	 */
