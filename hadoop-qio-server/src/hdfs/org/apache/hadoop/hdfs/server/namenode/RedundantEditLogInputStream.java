...
class RedundantEditLogInputStream extends EditLogInputStream {
	...
	RedundantEditLogInputStream(Collection<EditLogInputStream> streams, long startTxId) {
		...
		// EditLogInputStreams in a RedundantEditLogInputStream must be
		// finalized,
		...
		for (EditLogInputStream s : streams) {
			...
			if (first == null) {
				...
				Preconditions.checkArgument(s.getFirstTxId() == first.getFirstTxId(),
						"All streams in the RedundantEditLogInputStream must have the same " + "start transaction ID!  "
								+ first + " had start txId " + first.getFirstTxId() + ", but " + s + " had start txId "
								+ s.getFirstTxId());
			}
		}
		...
		// We sort the streams here so that the streams that end later come
		// first.
				...
				try {
					...
					if (op == null) {
						...
						if (streams[curIdx].getLastTxId() == prevTxId) {
							...
							throw new PrematureEOFException("got premature end-of-file " + "at txid " + prevTxId
									+ "; expected file to go up to " + streams[curIdx].getLastTxId());
						}
					}
			...
			case STREAM_FAILED:
				...
				if (newLast < oldLast) {
					throw new IOException("We encountered an error reading " + streams[curIdx].getName()
							...
							+ "remaining edit log ends at transaction " + newLast
							+ ", but we thought we could read up to transaction " + oldLast
