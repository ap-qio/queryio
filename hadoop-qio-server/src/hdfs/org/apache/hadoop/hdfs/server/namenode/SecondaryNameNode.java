 ...
 * and then goes back to sleep. The Secondary NameNode uses the NamenodeProtocol
...
public class SecondaryNameNode implements Runnable, SecondaryNameNodeInfoMXBean {
	...
	public String toString() {
		return getClass().getSimpleName() + " Status" + "\nName Node Address      : " + nameNodeAddr
				...
				+ (lastCheckpointTime == 0 ? "--" : ((Time.monotonicNow() - lastCheckpointTime) / 1000))
	...
	public SecondaryNameNode(Configuration conf, CommandLineOpts commandLineOpts) throws IOException {
		try {
			...
			if (HAUtil.isHAEnabled(conf, nsId)) {
				throw new IOException("Cannot use SecondaryNameNode in an HA cluster."
	...
	public static InetSocketAddress getHttpAddress(Configuration conf) {
		return NetUtils.createSocketAddr(conf.getTrimmed(DFSConfigKeys.DFS_NAMENODE_SECONDARY_HTTP_ADDRESS_KEY,
	...
	private void initialize(final Configuration conf, CommandLineOpts commandLineOpts) throws IOException {
		...
		JvmMetrics.create("SecondaryNameNode", conf.get(DFSConfigKeys.DFS_METRICS_SESSION_ID_KEY),
		...
		this.conf = conf;
		this.namenode = NameNodeProxies.createNonHAProxy(conf, nameNodeAddr, NamenodeProtocol.class,
		...
		final String httpsAddrString = conf.getTrimmed(DFSConfigKeys.DFS_NAMENODE_SECONDARY_HTTPS_ADDRESS_KEY,
				DFSConfigKeys.DFS_NAMENODE_SECONDARY_HTTPS_ADDRESS_DEFAULT);
		InetSocketAddress httpsAddr = NetUtils.createSocketAddr(httpsAddrString);

		HttpServer2.Builder builder = DFSUtil.httpServerTemplateForNNAndJN(conf, httpAddr, httpsAddr, "secondary",
		...
		LOG.info("Checkpoint Period   :" + checkpointConf.getPeriod() + " secs " + "(" + checkpointConf.getPeriod() / 60
				+ " min)");
	...
	public void doWork() {
		//
		// Poll the Namenode (once every checkpointCheckPeriod seconds) to find
		// the
		// number of transactions in the edit log that haven't yet been
		// checkpointed.
		//
	 ...
	 * @throws IOException
	 */
	static boolean downloadCheckpointFiles(final URL nnHostPort, final FSImage dstImage, final CheckpointSignature sig,
			...
			Boolean b = UserGroupInformation.getCurrentUser().doAs(new PrivilegedExceptionAction<Boolean>() {
				...
				public Boolean run() throws Exception {
					...
					if (sig.mostRecentCheckpointTxId == dstImage.getStorage().getMostRecentCheckpointTxId()) {
						...
						MD5Hash downloadedHash = TransferFsImage.downloadImageToStorage(nnHostPort,
								sig.mostRecentCheckpointTxId, dstImage.getStorage(), true);
						dstImage.saveDigestAndRenameCheckpointImage(NameNodeFile.IMAGE, sig.mostRecentCheckpointTxId,
								downloadedHash);
					}
					...
					// true if we haven't loaded all the transactions
					// represented by the
					// downloaded fsimage.
	...
	public boolean doCheckpoint() throws IOException {
		...
		boolean isSameCluster = (dstStorage.versionSupportsFederation(NameNodeLayoutVersion.FEATURES)
		...
		if (isFreshCheckpointer || (isSameCluster && !sig.storageVersionMatches(checkpointImage.getStorage()))) {
			// if we're a fresh 2NN, or if we're on the same cluster and our
			// storage
		...
		try {
			...
			case CHECKPOINT:
				...
				if (count > checkpointConf.getTxnCount() || opts.shouldForceCheckpoint()) {
					...
					System.err.println(
							"EditLog size " + count + " transactions is " + "smaller than configured checkpoint "
	...
	static class CommandLineOpts {
		...
		CommandLineOpts() {
			...
			checkpointOpt = OptionBuilder.withArgName("force").hasOptionalArg().withDescription("checkpoint on startup")
					.create("checkpoint");
			;
		...
		void usage() {
			String header = "The Secondary NameNode is a helper "
					...
					+ "The current design allows only one Secondary NameNode " + "per HDFS cluster.";
	...
	static class CheckpointStorage extends FSImage {
		...
		private static class CheckpointLogPurger implements LogsPurgeable {
			...
			@Override
			public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId,
		 ...
		 *            Node configuration.
		 * @param imageDirs
		 *            URIs of storage for image.
		 * @param editsDirs
		 *            URIs of storage for edit logs.
		 * @throws IOException
		 *             If storage cannot be access.
		 */
		CheckpointStorage(Configuration conf, Collection<URI> imageDirs, List<URI> editsDirs) throws IOException {
			...
			// Replace the archival manager with one that can actually work on
			// the
			// 2NN's edits storage.
		 ...
		 * exist. Recover from an unsuccessful checkpoint if necessary.
		 *
		 * @throws IOException
		 */
			...
			for (Iterator<StorageDirectory> it = storage.dirIterator(); it.hasNext();) {
				...
				try {
					...
					case NORMAL:
						...
						// (a) the VERSION file for each of the directories is
						// the same,
						// and (b) when we connect to a NN, we can verify that
						// the remote
						// node matches the same namespace that we ran on
						// previously.
	...
	static void doMerge(CheckpointSignature sig, RemoteEditLogManifest manifest, boolean loadImage, FSImage dstImage,
