...
class FSDirXAttrOp {
	...
	private static final XAttr UNREADABLE_BY_SUPERUSER_XATTR = XAttrHelper
	 ...
	 * @throws IOException
	 */
	static HdfsFileStatus setXAttr(FSDirectory fsd, String src, XAttr xAttr, EnumSet<XAttrSetFlag> flag,
	 ...
	 * toFilter and puts them into filtered. Upon completion, toFilter contains
	 * the filter XAttrs that were not found, while fitleredXAttrs contains the
	 * XAttrs that were found.
	 *
	 * @param existingXAttrs
	 *            Existing XAttrs to be filtered
	 * @param toFilter
	 *            XAttrs to filter from the existing XAttrs
	 * @param filtered
	 *            Return parameter, XAttrs that were filtered
	 * @return List of XAttrs that does not contain filtered XAttrs
	 */
	@VisibleForTesting
	static List<XAttr> filterINodeXAttrs(final List<XAttr> existingXAttrs, final List<XAttr> toFilter,
			final List<XAttr> filtered) throws AccessControlException {
	...
	static INode unprotectedSetXAttrs(FSDirectory fsd, final String src, final List<XAttr> xAttrs,
		...
		for (XAttr xattr : newXAttrs) {
			...
			/*
			 * If we're adding the encryption zone xattr, then add src to the
			 * list of encryption zones.
			 */
	...
	static List<XAttr> setINodeXAttrs(FSDirectory fsd, final List<XAttr> existingXAttrs, final List<XAttr> toSet,
			final EnumSet<XAttrSetFlag> flag) throws IOException {
		// Check for duplicate XAttrs in toSet
		// We need to use a custom comparator, so using a HashSet is not
		// suitable
		...
		if (userVisibleXAttrsNum > fsd.getInodeXAttrsLimit()) {
			throw new IOException(
					"Cannot add additional XAttr to inode, " + "would exceed limit of " + fsd.getInodeXAttrsLimit());
		}
	...
	private static void checkXAttrChangeAccess(FSDirectory fsd, INodesInPath iip, XAttr xAttr, FSPermissionChecker pc)
	...
	/**
	 * Verifies that the combined size of the name and value of an xattr is
	 * within the configured limit. Setting a limit of zero disables this check.
	 */
	private static void checkXAttrSize(FSDirectory fsd, XAttr xAttr) {
		...
		if (size > fsd.getXattrMaxSize()) {
			throw new HadoopIllegalArgumentException("The XAttr is too big. The maximum combined size of the"
					+ " name and value is " + fsd.getXattrMaxSize() + ", but the total size is " + size);
		}
	}
