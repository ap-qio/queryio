...
public class ConfiguredFailoverProxyProvider<T> extends AbstractNNFailoverProxyProvider<T> {
	...
	public synchronized ProxyInfo<T> getProxy() {
		...
		if (current.namenode == null) {
			try {
				current.namenode = NameNodeProxies
						.createNonHAProxy(conf, current.address, xface, ugi, false, fallbackToSimpleAuth).getProxy();
			} catch (IOException e) {
	 ...
	 * to an NN. Note that {@link AddressRpcProxyPair#namenode} may be null.
	 */
