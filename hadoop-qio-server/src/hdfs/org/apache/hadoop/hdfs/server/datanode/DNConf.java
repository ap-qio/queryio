...
/**
 * Simple class encapsulating all of the configuration that the DataNode loads
 * at startup time.
 */
...
public class DNConf {
	...
	public DNConf(Configuration conf) {
		...
		socketKeepaliveTimeout = conf.getInt(DFSConfigKeys.DFS_DATANODE_SOCKET_REUSE_KEEPALIVE_KEY,
		...
		readaheadLength = conf.getLong(DFSConfigKeys.DFS_DATANODE_READAHEAD_BYTES_KEY,
				DFSConfigKeys.DFS_DATANODE_READAHEAD_BYTES_DEFAULT);
		dropCacheBehindWrites = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_DROP_CACHE_BEHIND_WRITES_KEY,
		...
		this.dfsclientSlowIoWarningThresholdMs = conf.getLong(DFSConfigKeys.DFS_CLIENT_SLOW_IO_WARNING_THRESHOLD_KEY,
		...
		long initBRDelay = conf.getLong(DFS_BLOCKREPORT_INITIAL_DELAY_KEY, DFS_BLOCKREPORT_INITIAL_DELAY_DEFAULT)
				* 1000L;
		...
		// do we need to sync block file contents to disk when blockfile is
		// closed?
		...
		this.ignoreSecurePortsForTesting = conf.getBoolean(IGNORE_SECURE_PORTS_FOR_TESTING_KEY,
				IGNORE_SECURE_PORTS_FOR_TESTING_DEFAULT);

		this.xceiverStopTimeout = conf.getLong(DFS_DATANODE_XCEIVER_STOP_TIMEOUT_MILLIS_KEY,
				DFS_DATANODE_XCEIVER_STOP_TIMEOUT_MILLIS_DEFAULT);

		this.maxLockedMemory = conf.getLong(DFS_DATANODE_MAX_LOCKED_MEMORY_KEY, DFS_DATANODE_MAX_LOCKED_MEMORY_DEFAULT);

		this.restartReplicaExpiry = conf.getLong(DFS_DATANODE_RESTART_REPLICA_EXPIRY_KEY,
	 ...
	 *
	 * @return SaslPropertiesResolver configured for use with
	 ...
	 * DataTransferProtocol, or null if not configured.
	 *
	 * @return TrustedChannelResolver configured for use with
	 *         DataTransferProtocol
	 */
	...
	/**
	 * Returns true if configuration is set to skip checking for proper port
	 * configuration in a secured cluster. This is only intended for use in dev
	 * testing.
	 *
