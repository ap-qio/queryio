...
/**
 * A DN volume choosing policy which takes into account the amount of free space
 * on each of the available volumes when considering where to assign a new
 * replica allocation. By default this policy prefers assigning replicas to
...
public class AvailableSpaceVolumeChoosingPolicy<V extends FsVolumeSpi>
	...
	public synchronized V chooseVolume(List<V> volumes, long replicaSize) throws IOException {
		...
		if (volumesWithSpaces.areAllVolumesWithinFreeSpaceThreshold()) {
			// If they're actually not too far out of whack, fall back on pure
			// round
			// robin.
			V volume = roundRobinPolicyBalanced.chooseVolume(volumes, replicaSize);
			if (LOG.isDebugEnabled()) {
				LOG.debug("All volumes are within the configured free space balance " + "threshold. Selecting " + volume
						+ " for write of block size " + replicaSize);
			}
			return volume;
		} else {
			V volume = null;
			// If none of the volumes with low free space have enough space for
			// the
			...
			float scaledPreferencePercent = (highAvailableVolumes.size() * balancedPreferencePercent)
			...
			if (mostAvailableAmongLowVolumes < replicaSize || random.nextFloat() < scaledPreferencePercent) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug("Volumes are imbalanced. Selecting " + volume
							+ " from high available space volumes for write of block size " + replicaSize);
				}
			} else {
				volume = roundRobinPolicyLowAvailable.chooseVolume(lowAvailableVolumes, replicaSize);
				if (LOG.isDebugEnabled()) {
					LOG.debug("Volumes are imbalanced. Selecting " + volume
							+ " from low available space volumes for write of block size " + replicaSize);
				}
			}
			return volume;
		}
	...
	private class AvailableSpaceVolumeList {
		...
		/**
		 * @return true if all volumes' free space is within the configured
		...
		/**
		 * @return the maximum amount of space available across volumes with low
		 *         space.
		 */
	 ...
	 * the beginning of
	 * {@link AvailableSpaceVolumeChoosingPolicy#chooseVolume(List, long)}.
	 */
