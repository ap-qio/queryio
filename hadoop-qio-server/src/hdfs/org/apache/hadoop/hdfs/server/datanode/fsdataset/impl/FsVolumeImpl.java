...
public class FsVolumeImpl implements FsVolumeSpi {
	...
	/**
	 * Per-volume worker pool that processes new blocks to cache. The maximum
	 * number of workers per volume is bounded (configurable via
	 * dfs.datanode.fsdatasetcache.max.threads.per.volume) to limit resource
	 * contention.
	 */
	protected ThreadPoolExecutor cacheExecutor;

	FsVolumeImpl(FsDatasetImpl dataset, String storageID, File currentDir, Configuration conf, StorageType storageType)
			throws IOException {
	...
	protected ThreadPoolExecutor initializeCacheExecutor(File parent) {
		...
		ThreadFactory workerFactory = new ThreadFactoryBuilder().setDaemon(true)
				.setNameFormat("FsVolumeImplWorker-" + parent.toString() + "-%d").build();
		ThreadPoolExecutor executor = new ThreadPoolExecutor(1, maxNumThreads, 60, TimeUnit.SECONDS,
	 ...
	 * count on this volume.
	 * 
	 * @throws IOException
	 *             if the volume is closed or the waiting is interrupted.
	 */
	void closeAndWait() throws IOException {
		...
		while (this.reference.getReferenceCount() > 0) {
			if (FsDatasetImpl.LOG.isDebugEnabled()) {
				FsDatasetImpl.LOG.debug(String.format("The reference count for %s is %d, wait to be 0.", this,
						reference.getReferenceCount()));
			}
			try {
	...
	/**
	 * Temporary files. They get moved to the finalized block directory when the
	 * block is finalized.
	 */
	...
	public void releaseReservedSpace(long bytesToRelease) {
		if (bytesToRelease != 0) {
			...
			do {
				...
				if (newReservation < 0) {
					// Failsafe, this should never occur in practice, but if it
					// does we don't
	...
	private static class BlockIteratorState {
		...
		// The wall-clock ms since the epoch at which this iterator was last
		// saved.
		@JsonProperty
	...
	private class BlockIteratorImpl implements FsVolumeSpi.BlockIterator {
		 ...
		 *
		 * @return The next subdirectory within the block pool slice, or null if
		 *         there are no more.
		 */
		private String getNextSubDir(String prev, File dir) throws IOException {
			...
			cacheMs = 0;
			if (children.size() == 0) {
				LOG.trace("getNextSubDir({}, {}): no subdirectories found in {}", storageID, bpid,
						dir.getAbsolutePath());
				return null;
			}
			...
			if (nextSubDir == null) {
				LOG.trace("getNextSubDir({}, {}): no more subdirectories found in {}", storageID, bpid,
						dir.getAbsolutePath());
			} else {
				LOG.trace("getNextSubDir({}, {}): picking next subdirectory {} " + "within {}", storageID, bpid,
						nextSubDir, dir.getAbsolutePath());
			}
			return nextSubDir;
		}
		...
		private List<String> getSubdirEntries() throws IOException {
			...
			if (cache != null) {
				...
				if (delta < maxStalenessMs) {
					...
					LOG.trace("getSubdirEntries({}, {}): purging entries cache for {} " + "after {} ms.", storageID,
							bpid, state.curFinalizedSubDir, delta);
					cache = null;
				}
			}
			File dir = Paths.get(bpidDir.getAbsolutePath(), "current", "finalized", state.curFinalizedDir,
					state.curFinalizedSubDir).toFile();
			...
			if (entries == null) {
				...
				LOG.trace("getSubdirEntries({}, {}): listed {} entries in {}", storageID, bpid, entries.size(),
						dir.getAbsolutePath());
			}
			cache = entries;
			cacheMs = now;
		 ...
		 * entries in a subdirectory, we search for the next subdirectory. And
		 * so on.
		 */
		@Override
				...
				while (true) {
					...
					if (entries != null) {
						...
						if (state.curEntry == null) {
							LOG.trace("nextBlock({}, {}): advancing from {} to next " + "subdirectory.", storageID,
									bpid, state.curFinalizedSubDir);
						} else {
		...
		public void save() throws IOException {
			...
			try (BufferedWriter writer = new BufferedWriter(
					new OutputStreamWriter(new FileOutputStream(getTempSaveFile(), false), "UTF-8"))) {
		...
		public void load() throws IOException {
			...
			LOG.trace("load({}, {}): loaded iterator {} from {}: {}", storageID, bpid, name, file.getAbsoluteFile(),
					mapper.writerWithDefaultPrettyPrinter().writeValueAsString(state));
		}
	...
	/**
	 * RBW files. They get moved to the finalized block directory when the block
	 * is finalized.
	 */
	...
	/**
	 *
	 * @param bytesReservedForRbw
	 *            Space that was reserved during block creation. Now that the
	 *            block is being finalized we can free up this space.
	 * @return
	 * @throws IOException
	 */
	...
	void getVolumeMap(String bpid, ReplicaMap volumeMap, final RamDiskReplicaTracker ramDiskReplicaMap)
			throws IOException {
	...
	void deleteBPDirectories(String bpid, boolean force) throws IOException {
		...
		if (force) {
			...
			if (lazypersistDir.exists()
					&& ((!DatanodeUtil.dirNoFilesRecursive(lazypersistDir) || !FileUtil.fullyDelete(lazypersistDir)))) {
