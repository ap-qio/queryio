...
public class RamDiskReplicaLruTracker extends RamDiskReplicaTracker {
	...
	/**
	 * Queue of replicas that need to be written to disk. Stale entries are GC'd
	 * by dequeueNextReplicaToPersist.
	 */
	 ...
	 * the block is either deleted from the block space or the replica is no
	 * longer on transient storage.
	 *
	 * @param deleteSavedCopies
	 *            true if we should delete the saved copies on persistent
	 *            storage. This should be set by the caller when the block is no
	 *            longer needed.
	 */
	@Override
