...
public class KeyManager implements Closeable, DataEncryptionKeyFactory {
	...
	public KeyManager(String blockpoolID, NamenodeProtocol namenode, boolean encryptDataTransfer, Configuration conf)
			throws IOException {
		...
		if (isBlockTokenEnabled) {
			...
			LOG.info("Block token params received from NN: update interval=" + StringUtils.formatTime(updateInterval)
			...
			this.blockTokenSecretManager = new BlockTokenSecretManager(updateInterval, tokenLifetime, blockpoolID,
					encryptionAlgorithm);
			this.blockTokenSecretManager.addKeys(keys);

			// sync block keys with NN more frequently than NN updates its block
			// keys
