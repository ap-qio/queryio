...
public class VolumeScanner extends Thread {
	 ...
	 * and divide it up into one-minute periods. Each entry in the array
	 * represents how many bytes were scanned during that period.
	 */
	...
	/**
	 * Blocks which were suspect which we have scanned. This is used to avoid
	 * scanning the same suspect block over and over.
	 */
	private final Cache<ExtendedBlock, Boolean> recentSuspectBlocks = CacheBuilder.newBuilder().maximumSize(1000)
	...
	static class Statistics {
		...
		public String toString() {
			return new StringBuilder().append("Statistics{").append("bytesScannedInPastHour=")
					.append(bytesScannedInPastHour).append(", blocksScannedInCurrentPeriod=")
					.append(blocksScannedInCurrentPeriod).append(", blocksScannedSinceRestart=")
					.append(blocksScannedSinceRestart).append(", scansSinceRestart=").append(scansSinceRestart)
	...
	static class ScanResultHandler {
		...
		public void handle(ExtendedBlock block, IOException e) {
			...
			// The BlockSender got an old block path in rbw. BlockReceiver
			// removed
			// the rbw block from rbw to finalized but BlockSender tried to open
			// the
			// file before BlockReceiver updated the VolumeMap. The state of the
			// block can be changed again now, so ignore this error here. If
			// there
			// is a block really deleted by mistake, DirectoryScan should catch
			// it.
	...
	private void expireOldScannedBytesRecords(long monotonicMs) {
		...
		// If a minute or more has gone past since we last updated the
		// scannedBytes
		...
		for (long m = curMinute + 1; m <= newMinute; m++) {
			...
			LOG.trace("{}: updateScannedBytes is zeroing out slotIdx {}.  " + "curMinute = {}; newMinute = {}", this,
					slotIdx, curMinute, newMinute);
	 ...
	 * <p/>
	 *
	 * @return 0 if we found a usable block iterator; the length of time we
	 *         should delay before checking again otherwise.
	 */
	private synchronized long findNextUsableBlockIter() {
		...
		// not want to save a monotonic time in the cursor file, because it
		// resets
		...
		for (int i = 0; i < numBlockIters; i++) {
			...
			if (waitMs <= 0) {
				iter.rewind();
				LOG.info("Now rescanning bpid {} on volume {}, after more than " + "{} hour(s)", iter.getBlockPoolId(),
						volume.getBasePath(), TimeUnit.HOURS.convert(conf.scanPeriodMs, TimeUnit.MILLISECONDS));
				curBlockIter = iter;
				return 0L;
			}
	 ...
	 *            The block to scan.
	 * @param bytesPerSec
	 *            The bytes per second to scan at.
	 *
	 * @return The length of the block that was scanned, or -1 if the block
	 *         could not be scanned.
	 */
	private long scanBlock(ExtendedBlock cblock, long bytesPerSec) {
		// 'cblock' has a valid blockId and block pool id, but we don't yet know
		// the
		// genstamp the block is supposed to have. Ask the FsDatasetImpl for
		// this
		// information.
	...
	@VisibleForTesting
	static boolean calculateShouldScan(String storageId, long targetBytesPerSec, long scannedBytesSum, long startMinute,
			long curMinute) {
		...
		LOG.trace(
				"{}: calculateShouldScan: effectiveBytesPerSec = {}, and "
						+ "targetBytesPerSec = {}.  startMinute = {}, curMinute = {}, " + "shouldScan = {}",
				storageId, effectiveBytesPerSec, targetBytesPerSec, startMinute, curMinute, shouldScan);
		return shouldScan;
	}
	 ...
	 *
	 * @param suspectBlock
	 *            A suspect block which we should scan, or null to scan the next
	...
	private long runLoop(ExtendedBlock suspectBlock) {
		...
		try {
			...
			if (!calculateShouldScan(volume.getStorageID(), conf.targetBytesPerSec, scannedBytesSum, startMinute,
					curMinute)) {
				// If neededBytesPerSec is too low, then wait few seconds for
				// some old
			...
			if (suspectBlock != null) {
				...
				try {
					...
					// There was an error listing the next block in the volume.
					// This is a
					// serious issue.
					LOG.warn("{}: nextBlock error on {}", this, curBlockIter);
					// On the next loop iteration, curBlockIter#eof will be set
					// to true, and
	...
	/**
	 * If there are elements in the suspectBlocks list, removes and returns the
	 * first one. Otherwise, returns null.
	 */
	...
	public void run() {
		...
		try {
			...
			// When the VolumeScanner exits, release the reference we were
			// holding
	...
	public synchronized void markSuspectBlock(ExtendedBlock block) {
		...
		if (recent != null) {
			LOG.info("{}: Not scheduling suspect block {} for " + "rescanning, because we rescanned it recently.", this,
					block);
			return;
		}
