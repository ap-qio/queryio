...
/**
 * Manages datanode decommissioning. A background monitor thread periodically
 * checks the status of datanodes that are in-progress of decommissioning.
 * <p/>
 * A datanode can be decommissioned in a few situations:
 * <ul>
 * <li>If a DN is dead, it is decommissioned immediately.</li>
 * <li>If a DN is alive, it is decommissioned after all of its blocks are
 * sufficiently replicated. Merely under-replicated blocks do not block
 * decommissioning as long as they are above a replication threshold.</li>
 * </ul>
 * In the second case, the datanode transitions to a decommission-in-progress
 ...
 * determine if they can be decommissioned. The monitor also prunes this list as
 * blocks become replicated, so monitor scans will become more efficient over
 * time.
 * <p/>
...
public class DecommissionManager {
	 ...
	 * <p/>
	 * Note also that the reference to the list of under-replicated blocks will
	 * be null on initial add
	 * <p/>
	...
	DecommissionManager(final Namesystem namesystem, final BlockManager blockManager,
		...
		executor = Executors.newScheduledThreadPool(1,
				new ThreadFactoryBuilder().setNameFormat("DecommissionMonitor-%d").setDaemon(true).build());
	...
	void activate(Configuration conf) {
		...
		checkArgument(intervalSecs >= 0,
				"Cannot set a negative " + "value for " + DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_INTERVAL_KEY);

		// By default, the new configuration key overrides the deprecated one.
		// No # node limit is set.
		int blocksPerInterval = conf.getInt(DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BLOCKS_PER_INTERVAL_KEY,
		...
		checkArgument(blocksPerInterval > 0,
				"Must set a positive value for " + DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BLOCKS_PER_INTERVAL_KEY);

		final int maxConcurrentTrackedNodes = conf.getInt(
				DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_MAX_CONCURRENT_TRACKED_NODES,
				DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_MAX_CONCURRENT_TRACKED_NODES_DEFAULT);
		checkArgument(maxConcurrentTrackedNodes >= 0, "Cannot set a negative " + "value for "
				+ DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_MAX_CONCURRENT_TRACKED_NODES);
		...
		LOG.debug("Activating DecommissionManager with interval {} seconds, "
				+ "{} max blocks per interval, {} max nodes per interval, " + "{} max concurrently tracked nodes.",
				intervalSecs, blocksPerInterval, nodesPerInterval, maxConcurrentTrackedNodes);
	}

	/**
	 * Stop the decommission monitor thread, waiting briefly for it to
	 * terminate.
	 */
	void close() {
	...
	void stopDecommission(DatanodeDescriptor node) {
		if (node.isDecommissionInProgress() || node.isDecommissioned()) {
			...
			LOG.trace(
					"stopDecommission: Node {} is not decommission in progress " + "or decommissioned, nothing to do.",
					node);
		}
	}
	 ...
	 */
	private boolean isSufficientlyReplicated(BlockInfoContiguous block, BlockCollection bc,
		...
		if (numExpected > numLive) {
			if (bc.isUnderConstruction() && block.equals(bc.getLastBlock())) {
				// Can decom a UC block as long as there will still be
				// minReplicas
				if (numLive >= blockManager.minReplication) {
					LOG.trace("UC block {} sufficiently-replicated since numLive ({}) " + ">= minR ({})", block,
							numLive, blockManager.minReplication);
					return true;
				} else {
					LOG.trace("UC block {} insufficiently-replicated since numLive " + "({}) < minR ({})", block,
							numLive, blockManager.minReplication);
				}
			} else {
	...
	private static void logBlockReplicationInfo(Block block, BlockCollection bc, DatanodeDescriptor srcNode,
		...
		LOG.info("Block: " + block + ", Expected Replicas: " + curExpectedReplicas + ", live replicas: " + curReplicas
				...
				+ bc.isUnderConstruction() + ", Datanodes having this block: " + nodeList + ", Current Datanode: "
				+ srcNode + ", Is current datanode decommissioning: " + srcNode.isDecommissionInProgress());
	}
	 ...
	 * <p/>
	 * Since this is done while holding the namesystem lock, the amount of work
	 * per monitor tick is limited.
	 */
	private class Monitor implements Runnable {
		...
		/**
		 * Pop datanodes off the pending list and into decomNodeBlocks, subject
		 * to the maxConcurrentTrackedNodes limit.
		 */
		private void processPendingNodes() {
			while (!pendingNodes.isEmpty()
					&& (maxConcurrentTrackedNodes == 0 || decomNodeBlocks.size() < maxConcurrentTrackedNodes)) {
				decomNodeBlocks.put(pendingNodes.poll(), null);
			}
		}

		private void check() {
			final Iterator<Map.Entry<DatanodeDescriptor, AbstractList<BlockInfoContiguous>>> it = new CyclicIteration<>(
			...
			while (it.hasNext() && !exceededNumBlocksPerCheck() && !exceededNumNodesPerCheck()) {
				...
				if (blocks == null) {
					// This is a newly added datanode, run through its list to
					// schedule
					// under-replicated blocks for replication and collect the
					// blocks
					// that are insufficiently replicated for further tracking
					LOG.debug("Newly-added node {}, doing full scan to find " + "insufficiently-replicated blocks.",
							dn);
					blocks = handleInsufficientlyReplicated(dn);
					decomNodeBlocks.put(dn, blocks);
					fullScan = true;
				} else {
					// This is a known datanode, check if its # of
					// insufficiently
					// replicated blocks has dropped to zero and if it can be
					// decommed
				...
				if (blocks.size() == 0) {
					if (!fullScan) {
						// If we didn't just do a full scan, need to re-check
						// with the
						// full block map.
						//
						// We've replicated all the known insufficiently
						// replicated
						// blocks. Re-check with the full block map before
						// finally
					...
					LOG.debug("Node {} still has {} blocks to replicate "
							+ "before it is a candidate to finish decommissioning.", dn, blocks.size());
				}
				iterkey = dn;
			}
		 ...
		 * <p/>
		 * As part of this, it also schedules replication work for any
		...
		/**
		 * Used while checking if decommission-in-progress datanodes can be
		 * marked as decommissioned. Combines shared logic of
		 * pruneSufficientlyReplicated and handleInsufficientlyReplicated.
		 *
		 * @param datanode
		 *            Datanode
		 * @param it
		 *            Iterator over the blocks on the datanode
		 * @param insufficientlyReplicated
		 *            Return parameter. If it's not null, will contain the
		 *            insufficiently replicated-blocks from the list.
		 * @param pruneSufficientlyReplicated
		 *            whether to remove sufficiently replicated blocks from the
		 *            iterator
		 * @return true if there are under-replicated blocks in the provided
		 *         block iterator, else false.
		 */
		private void processBlocksForDecomInternal(final DatanodeDescriptor datanode,
				final Iterator<BlockInfoContiguous> it, final List<BlockInfoContiguous> insufficientlyReplicated,
			...
			while (it.hasNext()) {
				...
				// Remove the block from the list if it's no longer in the block
				// map,
				...
				// Schedule under-replicated blocks for replication if not
				// already
				// pending
				if (blockManager.isNeededReplication(block, bc.getBlockReplication(), liveReplicas)) {
					if (!blockManager.neededReplications.contains(block)
						...
						// Process these blocks only when active NN is out of
						// safe mode.
						blockManager.neededReplications.add(block, curReplicas, num.decommissionedReplicas(),
