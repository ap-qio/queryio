...
/**
 * Extension of FSImage for the backup node. This class handles the setup of the
 * journaling spool on the backup namenode.
 */
@InterfaceAudience.Private
public class BackupImage extends FSImage {
	/** Backup input stream for loading edits into memory */
	private final EditLogBackupInputStream backupInputStream = new EditLogBackupInputStream(
			"Data from remote NameNode");

	/**
	 * Current state of the BackupNode. The BackupNode's state transitions are
	 * as follows:
	 * 
	 * Initial: DROP_UNTIL_NEXT_ROLL - Transitions to JOURNAL_ONLY the next time
	...
	static enum BNState {
		/**
		 * Edits from the NN should be dropped. On the next log roll, transition
		 * to JOURNAL_ONLY state
		 */
		DROP_UNTIL_NEXT_ROLL,
		/**
		 * Edits from the NN should be written to the local edits log but not
		 * applied to the namespace.
		 */
		JOURNAL_ONLY,
		/**
		 * Edits should be written to the local edits log and applied to the
		 * local namespace.
		 */
		IN_SYNC;
	}
	...
	/**
	 * Flag to indicate that the next time the NN rolls, the BN should
	 * transition from to JOURNAL_ONLY state.
	 ...
	 *            Configuration
	 * @throws IOException
	 *             if storage cannot be initialised.
	 */
	...
	void recoverCreateRead() throws IOException {
		for (Iterator<StorageDirectory> it = storage.dirIterator(); it.hasNext();) {
			...
			try {
				...
				switch (curState) {
				case NON_EXISTENT:
					// fail if any of the configured storage dirs are
					// inaccessible
					throw new InconsistentFSStateException(sd.getRoot(),
							"checkpoint directory does not exist or is not accessible.");
				case NOT_FORMATTED:
					// for backup node all directories may be unformatted
					// initially
	...
	private synchronized void applyEdits(long firstTxId, int numTxns, byte[] data) throws IOException {
		Preconditions.checkArgument(firstTxId == lastAppliedTxId + 1,
				"Received txn batch starting at %s but expected %s", firstTxId, lastAppliedTxId + 1);
	...
	/**
	 * Transition the BackupNode from JOURNAL_ONLY state to IN_SYNC state. This
	 * is done by repeated invocations of tryConvergeJournalSpool until we are
	 * caught up to the latest in-progress edits file.
	 */
	...
	private boolean tryConvergeJournalSpool() throws IOException {
		...
		synchronized (this) {
			...
			Collection<EditLogInputStream> editStreams = getEditLog()
					.selectInputStreams(getEditLog().getCurSegmentTxId(), getEditLog().getCurSegmentTxId());
	...
	/**
	 * Receive a notification that the NameNode has begun a new edit log. This
	 * causes the BN to also start the new edit log in its local directories.
	 */
	synchronized void namenodeStartedLogSegment(long txid) throws IOException {
		...
		if (editLog.isSegmentOpen()) {
			if (editLog.getLastWrittenTxId() == txid - 1) {
				// We are in sync with the NN, so end and finalize the current
				// segment
				editLog.endCurrentLogSegment(false);
			} else {
				// We appear to have missed some transactions -- the NN probably
				// lost contact with us temporarily. So, mark the current
				// segment
				// as aborted.
	...
	/**
	 * Request that the next time the BN receives a log roll, it should stop
	 * applying the edits log to the local namespace. This is typically followed
	 * on by a call to {@link #waitUntilNamespaceFrozen()}
	 */
