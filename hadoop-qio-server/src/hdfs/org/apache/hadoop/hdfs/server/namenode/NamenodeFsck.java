...
public class NamenodeFsck implements DataEncryptionKeyFactory {
	...
	// ordered by block id; to allow continuation support, pass in the last
	// block
	// # from previous call
	 ...
	 * @param out
	 *            output stream to write the fsck output
	 * @param totalDatanodes
	 *            number of live datanodes
	 * @param remoteAddress
	 *            source address of the fsck request
	 */
	NamenodeFsck(Configuration conf, NameNode namenode, NetworkTopology networktopology, Map<String, String[]> pmap,
			PrintWriter out, int totalDatanodes, InetAddress remoteAddress) {
		this.conf = conf;
	...
	public void fsck() {
		...
		try {
			if (blockIds != null) {
				...
				sb.append("FSCK started by " + UserGroupInformation.getCurrentUser() + " from " + remoteAddress + " at "
						+ new Date());
			...
			String msg = "FSCK started by " + UserGroupInformation.getCurrentUser() + " from " + remoteAddress
					+ " for path " + path + " at " + new Date();
			LOG.info(msg);
			out.println(msg);
			...
			if (file != null) {
				...
				out.println(
						"FSCK ended at " + new Date() + " in " + (Time.monotonicNow() - startTime + " milliseconds"));

				// If there were internal errors during the fsck operation, we
				// want to
				// return FAILURE_STATUS, even if those errors were not
				// immediately
				// fatal. Otherwise many unit tests will pass even when there
				// are bugs.
				if (internalError) {
				...
				// output
				// string might break testcases. Also note this must be the last
				// line
				// of the report.
				if (res.isHealthy()) {
	...
	private void listCorruptFileBlocks() throws IOException {
		Collection<FSNamesystem.CorruptFileBlockInfo> corruptFiles = namenode.getNamesystem()
	...
	void check(String parent, HdfsFileStatus file, Result res) throws IOException {
		...
		for (LocatedBlock lBlk : blocks.getLocatedBlocks()) {
			...
			NumberReplicas numberReplicas = namenode.getNamesystem().getBlockManager()
					.countNodes(block.getLocalBlock());
	...
	private void copyBlocksToLostFound(String parent, HdfsFileStatus file, LocatedBlocks blocks) throws IOException {
		...
		try {
			...
			if (hdfsPathExists(target)) {
				LOG.warn("Fsck: can't copy the remains of " + fullName + " to " + "lost+found, because " + target
						+ " already exists.");
				return;
			}
			...
			if (copyError) {
				LOG.warn("Fsck: there were errors copying the remains of the " + "corrupted file " + fullName
						+ " to /lost+found");
			} else {
		...
		while (blockReader == null) {
			...
			try {
				...
				blockReader = new BlockReaderFactory(dfs.getConf()).setFileName(file).setBlock(block)
						.setBlockToken(lblock.getBlockToken()).setStartOffset(0).setLength(-1).setVerifyChecksum(true)
						.setClientName("fsck").setDatanodeInfo(chosenNode).setInetSocketAddress(targetAddr)
