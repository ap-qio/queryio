...
/**
 * Journal manager for the common case of edits files being written to a storage
 * directory.
 * 
...
public class FileJournalManager implements JournalManager {
	...
	private static final Pattern EDITS_INPROGRESS_REGEX = Pattern
			.compile(NameNodeFile.EDITS_INPROGRESS.getName() + "_(\\d+)");
	private static final Pattern EDITS_INPROGRESS_STALE_REGEX = Pattern
			.compile(NameNodeFile.EDITS_INPROGRESS.getName() + "_(\\d+).*(\\S+)");
	 ...
	 * @param inProgressOk
	 *            whether or not to include the in-progress edit log segment
	...
	public List<RemoteEditLog> getRemoteEditLogs(long firstTxId, boolean inProgressOk) throws IOException {
		...
		for (EditLogFile elf : allLogFiles) {
			...
			if (elf.getFirstTxId() >= firstTxId) {
				...
				// Return this
				// anyway and let the caller figure out whether it wants to use
				// it.
				ret.add(new RemoteEditLog(elf.firstTxId, elf.lastTxId, elf.isInProgress()));
			}
		}
	...
	private void discardEditLogSegments(long startTxId) throws IOException {
		...
		// go through the editlog files to make sure the startTxId is right at
		// the
		// segment boundary
		for (EditLogFile elf : allLogFiles) {
			...
			if (forPurging) {
				...
				if (staleInprogressEditsMatch.matches()) {
					try {
						...
						LOG.error(
								"In-progress stale edits file " + f + " has improperly " + "formatted transaction ID");
						// skip
					}
				}
			}
	...
	@Override
	synchronized public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId,
			boolean inProgressOk) throws IOException {
		List<EditLogFile> elfs = matchEditLogs(sd.getCurrentDir());
		LOG.debug(this + ": selecting input streams starting at " + fromTxId
				+ (inProgressOk ? " (inProgress ok) " : " (excluding inProgress) ") + "from among " + elfs.size()
				+ " candidate file(s)");
	...
	static void addStreamsToCollectionFromFiles(Collection<EditLogFile> elfs, Collection<EditLogInputStream> streams,
		...
		for (EditLogFile elf : elfs) {
			...
			if (elf.lastTxId < fromTxId) {
				...
				LOG.debug("passing over " + elf + " because it ends at " + elf.lastTxId
						+ ", but we only care about transactions " + "as new as " + fromTxId);
				continue;
			}
	...
	synchronized public void recoverUnfinalizedSegments() throws IOException {
		...
		for (EditLogFile elf : allLogFiles) {
			...
			if (elf.isInProgress()) {
				// If the file is zero-length, we likely just crashed after
				// opening the
				...
				if (elf.getLastTxId() == HdfsConstants.INVALID_TXID) {
					// If the file has a valid header (isn't corrupt) but
					// contains no
					// transactions, we likely just crashed after opening the
					// file and
					// writing the header, but before syncing any transactions.
					// Safe to
					// delete the file.
	...
	public static EditLogFile getLogFile(File dir, long startTxId) throws IOException {
		...
		if (ret.isEmpty()) {
			...
			throw new IllegalStateException("More than one log segment in " + dir + " starting at txid " + startTxId
					+ ": " + Joiner.on(", ").join(ret));
		}
	}
	...
	public static class EditLogFile {
		...
		/**
		 * Find out where the edit log ends. This will update the lastTxId of
		 * the EditLogFile or mark it as corrupt if it is.
		 */
