 ...
 * currently includes blockCollection it belongs to and the datanodes that store
 * the block.
 */
class BlocksMap {
	...
	BlocksMap(int capacity) {
		...
		this.blocks = new LightWeightGSet<Block, BlockInfoContiguous>(capacity) {
			...
			public Iterator<BlockInfoContiguous> iterator() {
				...
				/*
				 * Not tracking any modifications to set. As this set will be
				 * used always under FSNameSystem lock, modifications will not
				 * cause any ConcurrentModificationExceptions. But there is a
				 * chance of missing newly added elements during iteration.
				 */
	...
	/**
	 * Searches for the block in the BlocksMap and returns {@link Iterable} of
	 * the storages the block belongs to.
	 */
	...
	/**
	 * Searches for the block in the BlocksMap and returns {@link Iterable} of
	 * the storages the block belongs to <i>that are of the given
	 * {@link DatanodeStorage.State state}</i>.
	 * 
	 * @param state
	 *            DatanodeStorage state by which to filter the returned Iterable
	 */
	...
	/**
	 * For a block that has already been retrieved from the BlocksMap returns
	 * {@link Iterable} of the storages the block belongs to.
	 */
	...
	boolean removeNode(Block b, DatanodeDescriptor node) {
		...
		if (info.getDatanode(0) == null // no datanodes left
				&& info.getBlockCollection() == null) { // does not belong to a
														// file
	...
	/**
	 * Replace a block in the block map by a new block. The new block and the
	 * old one have the same key.
	 * 
	 * @param newBlock
