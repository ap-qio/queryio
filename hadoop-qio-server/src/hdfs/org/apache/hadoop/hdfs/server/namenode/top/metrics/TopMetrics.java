 ...
 * {@link org.apache.hadoop .hdfs.server.namenode.top.TopAuditLogger}, which
 * calls TopMetrics to increment per-operation, per-user counts on every audit
 * log call. These counts are used to show the top users by NameNode operation
 * as well as across all operations.
 * <p/>
 ...
 * {@link org.apache.hadoop .hdfs.server .namenode.metrics.FSNamesystemMBean#getTopWindows}.
 * This is done by calling
 * {@link org.apache.hadoop.hdfs.server.namenode.top.window .RollingWindowManager#snapshot}
...
public class TopMetrics {
	...
	/**
	 * A map from reporting periods to WindowManager. Thread-safety is provided
	 * by the fact that the mapping is not changed after construction.
	 */
	...
	/**
	 * Pick the same information that DefaultAuditLogger does before writing to
	 * a log file. This is to be consistent when {@link TopMetrics} is charged
	 * with data read back from log files instead of being invoked directly by
	 * the FsNamesystem
	 */
	public void report(boolean succeeded, String userName, InetAddress addr, String cmd, String src, String dst,
			FileStatus status) {
