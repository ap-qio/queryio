...
public interface NamenodeProtocol {
	/**
	 * Until version 6L, this class served as both the client interface to the
	 * NN AND the RPC protocol used to communicate with the NN.
	 * 
	 * This class is used by both the DFSClient and the NN server side to
	 * insulate from the protocol serialization.
	 * 
	 * If you are adding/changing NN's interface then you need to change both
	...
	/**
	 * Get a list of blocks belonging to <code>datanode</code> whose total size
	...
	/**
	 * @return The most recent transaction ID that has been synced to persistent
	 *         storage, or applied from persistent storage in the case of a
	 *         non-active node.
	 * @throws IOException
	 */
	@Idempotent
	 ...
	 * 
	 * @return {@link NamespaceInfo} identifying versions and storage
	 ...
	 *
	 * @return {@link NamenodeRegistration} of the node, which this node has
	...
	/**
	 * A request to the active name-node to start a checkpoint. The name-node
	 * should decide whether to admit it or reject. The name-node also decides
	 * what should be done with the backup node image before and after the
	 * checkpoint.
	 * 
	...
	/**
	 * A request to the active name-node to finalize previously started
	 * checkpoint.
	 * 
	 * @param registration
	 *            the requesting node
	 * @param sig
	 *            {@code CheckpointSignature} which identifies the checkpoint.
	 * @throws IOException
	 */
	@AtMostOnce
	 ...
	 * 
	 * @param sinceTxId
	 *            return only logs that contain transactions >= sinceTxId
	 */
	@Idempotent
