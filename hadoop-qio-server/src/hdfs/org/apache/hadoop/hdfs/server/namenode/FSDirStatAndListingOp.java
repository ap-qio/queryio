...
class FSDirStatAndListingOp {
	static DirectoryListing getListingInt(FSDirectory fsd, final String srcArg, byte[] startAfter, boolean needLocation)
			throws IOException {
	 ...
	 *
	 * We will stop when any of the following conditions is met: 1) this.lsLimit
	 * files have been added 2) needLocation is true AND enough files have been
	 * added such that at least this.lsLimit block locations are in the response
	 *
	 * @param fsd
	 *            FSDirectory
	 * @param iip
	 ...
	 */
	private static DirectoryListing getListing(FSDirectory fsd, INodesInPath iip, String src, byte[] startAfter,
	 ...
	 */
	private static DirectoryListing getSnapshotsListing(FSDirectory fsd, String src, byte[] startAfter)
			throws IOException {
		...
		final String dirPath = FSDirectory
				.normalizePath(src.substring(0, src.length() - HdfsConstants.DOT_SNAPSHOT_DIR.length()));
		...
		for (int i = 0; i < numOfListing; i++) {
			...
			listing[i] = createFileStatus(fsd, src, sRoot.getLocalNameBytes(), sRoot,
					BlockStoragePolicySuite.ID_UNSPECIFIED, Snapshot.CURRENT_STATE_ID, false,
	 ...
	 *            The string representation of the path to the file
	 * @param isRawPath
	 *            true if a /.reserved/raw pathname was passed by the user
	 * @param includeStoragePolicy
	 *            whether to include storage policy
	 * @return object containing information regarding the file or null if file
	 *         not found
	 */
	static HdfsFileStatus getFileInfo(FSDirectory fsd, String path, INodesInPath src, boolean isRawPath,
			boolean includeStoragePolicy) throws IOException {
		fsd.readLock();
		try {
			...
			byte policyId = includeStoragePolicy && i != null && !i.isSymlink() ? i.getStoragePolicyID()
					: BlockStoragePolicySuite.ID_UNSPECIFIED;
			return i == null ? null
					: createFileStatus(fsd, path, HdfsFileStatus.EMPTY_NAME, i, policyId, src.getPathSnapshotId(),
							isRawPath, src);
		} finally {
			fsd.readUnlock();
		}
	...
	static HdfsFileStatus getFileInfo(FSDirectory fsd, String src, boolean resolveLink, boolean isRawPath,
			boolean includeStoragePolicy) throws IOException {
		String srcs = FSDirectory.normalizePath(src);
		if (srcs.endsWith(HdfsConstants.SEPARATOR_DOT_SNAPSHOT_DIR)) {
			if (fsd.getINode4DotSnapshot(srcs) != null) {
				return new HdfsFileStatus(0, true, 0, 0, 0, 0, null, null, null, null, HdfsFileStatus.EMPTY_NAME, -1L,
						0, null, BlockStoragePolicySuite.ID_UNSPECIFIED);
			}
			return null;
		}
	...
	private static HdfsFileStatus getFileInfo4DotSnapshot(FSDirectory fsd, String src) throws UnresolvedLinkException {
		if (fsd.getINode4DotSnapshot(src) != null) {
			return new HdfsFileStatus(0, true, 0, 0, 0, 0, null, null, null, null, HdfsFileStatus.EMPTY_NAME, -1L, 0,
	 ...
	 *             if any error occurs
	 */
	static HdfsFileStatus createFileStatus(FSDirectory fsd, String fullPath, byte[] path, INode node,
			boolean needLocation, byte storagePolicy, int snapshot, boolean isRawPath, INodesInPath iip)
			throws IOException {
		if (needLocation) {
	 ...
	 */
	static HdfsFileStatus createFileStatus(FSDirectory fsd, String fullPath, byte[] path, INode node,
			byte storagePolicy, int snapshot, boolean isRawPath, INodesInPath iip) throws IOException {
		...
		if (node.isFile()) {
			...
			if (fileNode.getBlocks() != null) {
				Log.info("Server : fileNode.getBlocks() is not null");
				Log.info("Server : fileNode.getBlocks().length : " + fileNode.getBlocks().length);
				blocks = new Block[fileNode.getBlocks().length];
				for (int i = 0; i < fileNode.getBlocks().length; i++) {
					blocks[i] = new Block(fileNode.getBlocks()[i].getBlockId(), fileNode.getBlocks()[i].getNumBytes(),
							fileNode.getBlocks()[i].getGenerationStamp());
				}
			} else {
				Log.info("Server : fileNode.getBlocks() is NULL !!");
			}
		...
		return new HdfsFileStatus(size, node.isDirectory(), replication, blocksize, node.getModificationTime(snapshot),
				node.getAccessTime(snapshot), getPermissionForFileStatus(nodeAttrs, isEncrypted),
	 ...
	 */
	private static HdfsLocatedFileStatus createLocatedFileStatus(FSDirectory fsd, String fullPath, byte[] path,
			INode node, byte storagePolicy, int snapshot, boolean isRawPath, INodesInPath iip) throws IOException {
		...
		if (node.isFile()) {
			...
			loc = fsd.getFSNamesystem().getBlockManager().createLocatedBlocks(fileNode.getBlocks(snapshot), fileSize,
					isUc, 0L, size, false, inSnapshot, feInfo);
			if (loc == null) {
		...
		INodeAttributes nodeAttrs = fsd.getAttributes(fullPath, path, node, snapshot);
		HdfsLocatedFileStatus status = new HdfsLocatedFileStatus(size, node.isDirectory(), replication, blocksize,
				node.getModificationTime(snapshot), node.getAccessTime(snapshot),
				getPermissionForFileStatus(nodeAttrs, isEncrypted), nodeAttrs.getUserName(), nodeAttrs.getGroupName(),
				node.isSymlink() ? node.asSymlink().getSymlink() : null, path, node.getId(), loc, childrenNum, feInfo,
				storagePolicy, /* @QUERYIO@ */null);
	...
	private static ContentSummary getContentSummaryInt(FSDirectory fsd, INodesInPath iip) throws IOException {
		...
		try {
			...
			if (targetNode == null) {
				...
				// Make it relinquish locks everytime contentCountLimit entries
				// are
				// processed. 0 means disabled. I.e. blocking for the entire
				// duration.
				ContentSummaryComputationContext cscc = new ContentSummaryComputationContext(fsd, fsd.getFSNamesystem(),
