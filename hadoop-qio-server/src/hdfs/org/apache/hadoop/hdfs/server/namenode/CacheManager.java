 ...
 *
 * This class is instantiated by the FSNamesystem. It maintains the mapping of
 * cached blocks to datanodes via processing datanode cache reports. Based on
 * these reports and addition and removal of caching directives, we will
 * schedule caching and uncaching work.
 */
...
public final class CacheManager {
	 ...
	 *
	 * listCacheDirectives relies on the ordering of elements in this map to
	 * track what has already been listed by the client.
	 */
	...
	public static final class PersistState {
		...
		public PersistState(CacheManagerSection section, List<CachePoolInfoProto> pools,
	...
	CacheManager(FSNamesystem namesystem, Configuration conf, BlockManager blockManager) {
		...
		this.maxListCachePoolsResponses = conf.getInt(DFS_NAMENODE_LIST_CACHE_POOLS_NUM_RESPONSES,
				DFS_NAMENODE_LIST_CACHE_POOLS_NUM_RESPONSES_DEFAULT);
		this.maxListCacheDirectivesNumResponses = conf.getInt(DFS_NAMENODE_LIST_CACHE_DIRECTIVES_NUM_RESPONSES,
		...
		float cachedBlocksPercent = conf.getFloat(DFS_NAMENODE_PATH_BASED_CACHE_BLOCK_MAP_ALLOCATION_PERCENT,
	 ...
	 * to reset FSNamesystem state. See {@link FSNamesystem#clear()}.
	 */
	void clear() {
	 ...
	 * {@link CacheDirectiveInfo.Expiration}. This converts a relative
	 * Expiration into an absolute time based on the local clock.
	 * 
	 * @param info
	 *            to validate.
	 * @param maxRelativeExpiryTime
	 *            of the info's pool.
	 * @return the expiration time, or the pool's max absolute expiration if the
	 *         info's expiration was not set.
	 * @throws InvalidRequestException
	 *             if the info's Expiration is invalid.
	 */
	private static long validateExpiryTime(CacheDirectiveInfo info, long maxRelativeExpiryTime)
		...
		if (relExpiryTime > maxRelativeExpiryTime) {
			throw new InvalidRequestException("Expiration " + expiry.toString()
					+ " exceeds the max relative expiration time of " + maxRelativeExpiryTime + " ms.");
		}
	 ...
	 *
	 * @param pool
	 *            CachePool where the path is being cached
	 * @param path
	 *            Path that is being cached
	 * @param replication
	 *            Replication factor of the path
	 * @throws InvalidRequestException
	 *             if the pool does not have enough capacity
	 */
	private void checkLimit(CachePool pool, String path, short replication) throws InvalidRequestException {
		...
		if (pool.getBytesNeeded() + (stats.getBytesNeeded() * replication) > pool.getLimit()) {
			throw new InvalidRequestException(
					"Caching path " + path + " of size " + stats.getBytesNeeded() / replication
							+ " bytes at replication " + replication + " would exceed pool " + pool.getPoolName()
							+ "'s remaining capacity of " + (pool.getLimit() - pool.getBytesNeeded()) + " bytes.");
		}
	}
	...
	/**
	 * Adds a directive, skipping most error checking. This should only be
	 * called internally in special scenarios like edit log replay.
	 */
	...
	public CacheDirectiveInfo addDirective(CacheDirectiveInfo info, FSPermissionChecker pc, EnumSet<CacheFlag> flags)
			throws IOException {
		assert namesystem.hasWriteLock();
	...
	/**
	 * Factory method that makes a new CacheDirectiveInfo by applying fields in
	 * a CacheDirectiveInfo to an existing CacheDirective.
	 * 
	 * @param info
	 *            with some or all fields set.
	 * @param defaults
	 *            directive providing default values for unset fields in info.
	 * 
	 ...
	 * internal use only. modifyDirective can be non-deterministic since its
	 * error checking depends on current system time, which poses a problem for
	 * edit log replay.
	 */
	...
	public void modifyDirective(CacheDirectiveInfo info, FSPermissionChecker pc, EnumSet<CacheFlag> flags)
			throws IOException {
		assert namesystem.hasWriteLock();
		...
		try {
			...
			// Need to test the pool being set here to avoid rejecting a modify
			// for a
	...
	public BatchedListEntries<CacheDirectiveEntry> listCacheDirectives(long prevId, CacheDirectiveInfo filter,
		...
		if (id != null) {
			...
			// Since we use a tailMap on directivesById, setting prev to id-1
			// gets
		...
		for (Entry<Long, CacheDirective> cur : tailMap.entrySet()) {
			...
			// Hitting this case means the ID is not present, or we're on the
			// second
	...
	public final void processCacheReport(final DatanodeID datanodeID, final List<Long> blockIds) throws IOException {
		...
		LOG.debug("Processed cache report from {}, blocks: {}, " + "processing time: {} msecs", datanodeID,
				blockIds.size(), (endTime - startTime));
	}
	...
	/**
	 * Saves the current state of the CacheManager to the DataOutput. Used to
	 * persist CacheManager state in the FSImage.
	 * 
	 * @param out
	 *            DataOutput to persist state
	 * @param sdPath
	...
	public PersistState saveState() throws IOException {
		...
		CacheManagerSection s = CacheManagerSection.newBuilder().setNextDirectiveId(nextDirectiveId)
				.setNumPools(pools.size()).setNumDirectives(directives.size()).build();

		return new PersistState(s, pools, directives);
	}

	/**
	 * Reloads CacheManager state from the passed DataInput. Used during
	 * namenode startup to restore CacheManager state from an FSImage.
	 * 
	 * @param in
	 *            DataInput from which to restore state
	 * @throws IOException
	 */
	...
	public void loadState(PersistState s) throws IOException {
		...
		for (CacheDirectiveInfoProto p : s.directives) {
			...
			CacheDirective directive = new CacheDirective(p.getId(), new Path(p.getPath()).toUri().getPath(),
					(short) p.getReplication(), p.getExpiration().getMillis());
	...
	private final class SerializerCompat {
		...
		private void load(DataInput in) throws IOException {
			...
			// pools need to be loaded first since directives point to their
			// parent pool
			loadPools(in);
			loadDirectives(in);
		}
		...
		private void loadDirectives(DataInput in) throws IOException {
			...
			for (int i = 0; i < numDirectives; i++) {
				...
				CacheDirective directive = new CacheDirective(info.getId(), info.getPath().toUri().getPath(),
