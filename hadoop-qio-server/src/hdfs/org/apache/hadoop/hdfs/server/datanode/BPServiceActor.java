...
class BPServiceActor implements Runnable {
	 ...
	 * the NN. Access should be synchronized on this object.
	 */
	private final Map<DatanodeStorage, PerStoragePendingIncrementalBR> pendingIncrementalBRperStorage = Maps
			.newHashMap();
	...
	/**
	 * Perform the first part of the handshake with the NameNode. This calls
	 * <code>versionRequest</code> to determine the NN's namespace and version
	 * info. It automatically retries until the NN responds or the DN is
	 * shutting down.
	 * 
	...
	private void checkNNVersion(NamespaceInfo nsInfo) throws IncorrectVersionException {
		...
		if (VersionUtil.compareVersions(nnVersion, minimumNameNodeVersion) < 0) {
			IncorrectVersionException ive = new IncorrectVersionException(minimumNameNodeVersion, nnVersion, "NameNode",
					"DataNode");
		...
		if (!nnVersion.equals(dnVersion)) {
			LOG.info("Reported NameNode version '" + nnVersion + "' does not match " + "DataNode version '" + dnVersion
	...
	/**
	 * Report received blocks and delete hints to the Namenode for each storage.
	 *
	 * @throws IOException
	 */
	private void reportReceivedDeletedBlocks() throws IOException {

		// Generate a list of the pending reports for each storage under the
		// lock
		ArrayList<StorageReceivedDeletedBlocks> reports = new ArrayList<StorageReceivedDeletedBlocks>(
				pendingIncrementalBRperStorage.size());
			...
			if (!success) {
				synchronized (pendingIncrementalBRperStorage) {
					for (StorageReceivedDeletedBlocks report : reports) {
						// If we didn't succeed in sending the report, put all
						// of the
						// blocks back onto our queue, but only in the case
						// where we
						// didn't put something newer in the meantime.
						PerStoragePendingIncrementalBR perStorageMap = pendingIncrementalBRperStorage
								.get(report.getStorage());
	...
	/**
	 * Add a blockInfo for notification to NameNode. If another entry exists for
	 * the same block it is removed.
	 *
	...
	List<DatanodeCommand> blockReport() throws IOException {
		...
		// or we will report an RBW replica after the BlockReport already
		// reports
		// a FINALIZED one.
		...
		Map<DatanodeStorage, BlockListAsLongs> perVolumeBlockLists = dn.getFSDataset()
		...
		try {
			...
			LOG.info((success ? "S" : "Uns") + "uccessfully sent block report 0x" + Long.toHexString(reportId)
					+ ",  containing " + reports.length + " storage report(s), of which we sent " + numReportsSent + "."
	...
	DatanodeCommand cacheReport() throws IOException {
		...
		if (startTime - lastCacheReport > dnConf.cacheReportInterval) {
			...
			LOG.debug("CacheReport of " + blockIds.size() + " block(s) took " + createCost + " msec to generate and "
	...
	HeartbeatResponse sendHeartBeat() throws IOException {
		...
		int numFailedVolumes = volumeFailureSummary != null ? volumeFailureSummary.getFailedStorageLocations().length
				: 0;
	...
	private void offerService() throws Exception {
		LOG.info("For namenode " + nnAddr + " using" + " DELETEREPORT_INTERVAL of " + dnConf.deleteReportInterval
				+ " msec " + " BLOCKREPORT_INTERVAL of " + dnConf.blockReportInterval + "msec"
				+ " CACHEREPORT_INTERVAL of " + dnConf.cacheReportInterval + "msec" + " Initial delay: "
			...
			try {
				...
				if (sendHeartbeat) {
					...
					if (!dn.areHeartbeatsDisabledForTests()) {
						...
						// If the state of this NN has changed (eg
						// STANDBY->ACTIVE)
						// then let the BPOfferService update itself.
						//
						// Important that this happens before processCommand
						// below,
						// since the first heartbeat to a new active might have
						// commands
	 ...
	 * <p>
	 * The bpDatanode needs to register with the namenode on startup in order 1)
		...
		try {
			while (true) {
				...
				try {
					...
					// Initial handshake, storage recovery or registration
					// failed
					runningState = RunningState.INIT_FAILED;
					if (shouldRetryInit()) {
						// Retry until all namenode's of BPOS failed
						// initialization
	...
	private static class PerStoragePendingIncrementalBR {
		...
		ReceivedDeletedBlockInfo[] dequeueBlockInfos() {
			ReceivedDeletedBlockInfo[] blockInfos = pendingIncrementalBR.values()
					.toArray(new ReceivedDeletedBlockInfo[getBlockInfoCount()]);
		...
		/**
		 * Add blocks from blockArray to pendingIncrementalBR, unless the block
		 * already exists in pendingIncrementalBR.
		 * 
		 * @param blockArray
	...
	static class Scheduler {
		...
		/**
		 * Schedule the next block report after the block report interval. If
		 * the current block report was delayed then the next block report is
		 * sent per the original schedule. Numerical overflow is possible here.
		 */
		void scheduleNextBlockReport() {
			// If we have sent the first set of block reports, then wait a
			// random
