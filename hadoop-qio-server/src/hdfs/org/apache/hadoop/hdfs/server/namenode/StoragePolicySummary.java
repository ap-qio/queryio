		...
		for (Map.Entry<StorageTypeAllocation, Long> storageComboCount : sortByComparator(storageComboCounts)) {
			...
			if (sta.policyMatches()) {
				compliantFormatter.format("%-25s %10d  %20s%n", sta.getStoragePolicyDescriptor(),
						storageComboCount.getValue(), percentFormat.format(percent));
			} else {
				...
				nonCompliantFormatter.format("%-35s %-20s %10d  %20s%n", sta.getStoragePolicyDescriptor(),
						sta.getSpecifiedStoragePolicy().getName(), storageComboCount.getValue(),
	...
	static class StorageTypeAllocation {
		...
		String getStoragePolicyDescriptor() {
			...
			if (actualStoragePolicy != null) {
				storagePolicyDescriptorSB.append(getStorageAllocationAsString()).append("(")
						.append(actualStoragePolicy.getName()).append(")");
			} else {
		...
		public boolean equals(Object another) {
			return (another instanceof StorageTypeAllocation
					&& Objects.equals(specifiedStoragePolicy, ((StorageTypeAllocation) another).specifiedStoragePolicy)
