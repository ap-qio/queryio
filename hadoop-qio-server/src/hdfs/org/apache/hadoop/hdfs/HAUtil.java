...
public class HAUtil {
	...
	/**
	 * Get the namenode Id by matching the {@code addressKey} with the the
	...
	public static String getNameNodeIdOfOtherNode(Configuration conf, String nsId) {
		...
		Preconditions.checkArgument(nnIds != null, "Could not determine namenode ids in namespace '%s'. "
				+ "Please configure " + DFSUtil.addKeySuffixes(DFSConfigKeys.DFS_HA_NAMENODES_KEY_PREFIX, nsId), nsId);
		Preconditions.checkArgument(nnIds.size() == 2,
				"Expected exactly 2 NameNodes in namespace '%s'. " + "Instead, got only %s (NN ids were '%s'", nsId,
				nnIds.size(), Joiner.on("','").join(nnIds));
	 ...
	 * 
	 * @return true if the NN should allow read operations while in standby
	 *         mode.
	 */
	...
	/**
	 * Check whether the client has a failover proxy provider configured for the
	...
	/**
	 * Check whether logical URI is needed for the namenode and the
	 * corresponding failover proxy provider in the config.
	 *
	 * @param conf
	 *            Configuration
	 * @param nameNodeUri
	 ...
	 * @throws IOException
	 *             most likely due to misconfiguration.
	 */
	public static boolean useLogicalUri(Configuration conf, URI nameNodeUri) throws IOException {
		...
		AbstractNNFailoverProxyProvider<ClientProtocol> provider = NameNodeProxies.createFailoverProxyProvider(conf,
				nameNodeUri, ClientProtocol.class, false, null);
	 ...
	 * 
	 * @param uri
	 *            the logical URI of the cluster
	 * @param scheme
	 *            the scheme of the corresponding FileSystem
	...
	/**
	 * @return true if this token corresponds to a logical nameservice rather
	 *         than a specific namenode.
	 */
	...
	/**
	 * Locate a delegation token associated with the given HA cluster URI, and
	 * if one is found, clone it to also represent the underlying namenode
	 * address.
	 * 
	 * @param ugi
	 *            the UGI to modify
	 ...
	 * @param nnAddrs
	 *            collection of NNs in the cluster to which the token applies
	 */
	public static void cloneDelegationTokenForLogicalUri(UserGroupInformation ugi, URI haUri,
		...
		if (haToken != null) {
			for (InetSocketAddress singleNNAddr : nnAddrs) {
				...
				Token<DelegationTokenIdentifier> specificToken = new Token.PrivateToken<DelegationTokenIdentifier>(
						haToken);
				SecurityUtil.setTokenService(specificToken, singleNNAddr);
				Text alias = new Text(buildTokenServicePrefixForLogicalUri(HdfsConstants.HDFS_URI_SCHEME) + "//"
						+ specificToken.getService());
				ugi.addToken(alias, specificToken);
				LOG.debug(
						"Mapped HA service delegation token for logical URI " + haUri + " to namenode " + singleNNAddr);
			}
		} else {
	 ...
	 * be used, since callers of this method who connect directly to the NN
	 * using the resulting InetSocketAddress will not be able to connect to the
	 * active NN if a failover were to occur after this method has been called.
	 * 
	 * @param fs
	 *            the file system to get the active address of.
	 * @return the internet address of the currently-active NN.
	 * @throws IOException
	 *             if an error occurs while resolving the active NN.
	 */
	 ...
	 * call should be made on every NN in an HA nameservice, not just the
	 * active.
	 * 
	 * @param conf
	 *            configuration
	 * @param nsId
	 *            the nameservice to get all of the proxies for.
	...
	public static <T> List<ProxyAndInfo<T>> getProxiesForAllNameNodesInNameservice(Configuration conf, String nsId,
		...
		for (InetSocketAddress nnAddress : nnAddresses.values()) {
			...
			proxyInfo = NameNodeProxies.createNonHAProxy(conf, nnAddress, xface, UserGroupInformation.getCurrentUser(),
					false);
	 ...
	 * 
	 * @param namenodes
	 *            list of RPC proxies for each NN to check.
	 * @return true if at least one NN is active, false if all are in the
	 *         standby state.
	 * @throws IOException
	 *             in the event of error.
	 */
