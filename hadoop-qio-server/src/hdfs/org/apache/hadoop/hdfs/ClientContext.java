 ...
 * 
 * This allows us to share caches such as the socket cache across DFSClient
 * instances.
 */
...
public class ClientContext {
	 ...
	 *
	 * The legacy block reader local gets disabled completely whenever there is
	 * an error or miscommunication. The new block reader local code handles
	 * this case more gracefully inside DomainSocketFactory.
	 */
	 ...
	 * that didn't match its config values yet.
	 */
	...
	private ClientContext(String name, Conf conf) {
		...
		this.shortCircuitCache = new ShortCircuitCache(conf.shortCircuitStreamsCacheSize,
				conf.shortCircuitStreamsCacheExpiryMs, conf.shortCircuitMmapCacheSize,
				conf.shortCircuitMmapCacheExpiryMs, conf.shortCircuitMmapCacheRetryTimeout,
				conf.shortCircuitCacheStaleThresholdMs, conf.shortCircuitSharedMemoryWatcherInterruptCheckMs);
	...
	public static String confAsString(Conf conf) {
		...
		builder.append("shortCircuitStreamsCacheSize = ").append(conf.shortCircuitStreamsCacheSize)
				...
				.append(conf.socketCacheExpiry).append(", shortCircuitLocalReads = ")
				.append(conf.shortCircuitLocalReads).append(", useLegacyBlockReaderLocal = ")
				.append(conf.useLegacyBlockReaderLocal).append(", domainSocketDataTraffic = ")
				.append(conf.domainSocketDataTraffic).append(", shortCircuitSharedMemoryWatcherInterruptCheckMs = ")
				.append(conf.shortCircuitSharedMemoryWatcherInterruptCheckMs).append(", keyProviderCacheExpiryMs = ")
	 ...
	 *
	 * This method is less efficient than the version which takes a
	 * DFSClient#Conf object, and should be mostly used by tests.
	 */
	@VisibleForTesting
	public static ClientContext getFromConf(Configuration conf) {
		return get(conf.get(DFSConfigKeys.DFS_CLIENT_CONTEXT, DFSConfigKeys.DFS_CLIENT_CONTEXT_DEFAULT),
		...
		if (!existing.equals(requested)) {
			if (!printedConfWarning) {
				...
				LOG.warn("Existing client context '" + name + "' does not match "
						+ "requested configuration.  Existing: " + existing + ", Requested: " + requested);
			}
		}
	}
