...
/**
 * Stream for reading inotify events. DFSInotifyEventInputStreams should not be
...
public class DFSInotifyEventInputStream {
	...
	/**
	 * Returns the next batch of events in the stream or null if no new batches
	 * are currently available.
	 *
	 * @throws IOException
	 *             because of network error or edit log corruption. Also
	 *             possible if JournalNodes are unresponsive in the QJM setting
	 *             (even one unresponsive JournalNode is enough in rare cases),
	 *             so catching this exception and retrying at least a few times
	 *             is recommended.
	 * @throws MissingEventsException
	 *             if we cannot return the next batch in the stream because the
	 *             data for the events (and possibly some subsequent events) has
	 *             been deleted (generally because this stream is a very large
	 *             number of transactions behind the current state of the
	 *             NameNode). It is safe to continue reading from the stream
	 *             after this exception is thrown The next available batch of
	 *             events will be returned.
	 */
	public EventBatch poll() throws IOException, MissingEventsException {
		TraceScope scope = Trace.startSpan("inotifyPoll", traceSampler);
		try {
			// need to keep retrying until the NN sends us the latest committed
			// txid
			if (lastReadTxid == -1) {
			...
			if (!it.hasNext()) {
				...
				if (el.getLastTxid() != -1) {
					// we only want to set syncTxid when we were actually able
					// to read some
					// edits on the NN -- otherwise it will seem like edits are
					// being
					// generated faster than we can read them when the problem
					// is really
			...
			if (it.hasNext()) { // can be empty if el.getLastTxid != -1 but none
								// of the
	 ...
	 * current state this stream is. Clients should periodically call this
	 * method and check if its result is steadily increasing, which indicates
	 * that they are falling behind (i.e. transaction are being generated faster
	 * than the client is reading them). If a client falls too far behind events
	 * may be deleted before the client can read them.
	 * <p/>
	 * A return value of -1 indicates that an estimate could not be produced,
	 * and should be ignored. The value returned by this method is really only
	 * useful when compared to previous or subsequent returned values.
	 */
	public long getTxidsBehindEstimate() {
		if (syncTxid == 0) {
			...
			// this gives the difference between the last txid we have fetched
			// to the
	 ...
	 * amount of time for a new batch. Returns null if one is not available at
	 * the end of the specified amount of time. The time before the method
	 * returns may exceed the specified amount of time by up to the time
	 * required for an RPC to the NameNode.
	 *
	 * @param time
	 *            number of units of the given TimeUnit to wait
	 * @param tu
	...
	/**
	 * Returns the next batch of events in the stream, waiting indefinitely if a
	 * new batch is not immediately available.
	 *
	 * @throws IOException
	 *             see {@link DFSInotifyEventInputStream#poll()}
	 * @throws MissingEventsException
	...
	public EventBatch take() throws IOException, InterruptedException, MissingEventsException {
		...
		try {
			...
			while ((next = poll()) == null) {
				// sleep for a random period between nextWaitMin and nextWaitMin
				// * 2
