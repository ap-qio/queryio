...
class BlockReaderLocalLegacy implements BlockReader {
	...
	private static class LocalDatanodeInfo {
		...
		LocalDatanodeInfo() {
			...
			cache = Collections.synchronizedMap(
					new LinkedHashMap<ExtendedBlock, BlockLocalPathInfo>(hashTableCapacity, hashTableLoadFactor, true) {
		...
		private synchronized ClientDatanodeProtocol getDatanodeProxy(UserGroupInformation ugi, final DatanodeInfo node,
				...
				try {
					proxy = ugi.doAs(new PrivilegedExceptionAction<ClientDatanodeProtocol>() {
						...
						public ClientDatanodeProtocol run() throws Exception {
							return DFSUtil.createClientDatanodeProtocolProxy(node, conf, socketTimeout,
	...
	// Multiple datanodes could be running on the local machine. Store proxies
	// in
	 ...
	 * decremented (usually to 0) by subsequent reads. This avoids having to do
	 * a checksum read at construction to position the read cursor correctly.
	 */
	 ...
	 */
	static BlockReaderLocalLegacy newBlockReader(DFSClient.Conf conf, UserGroupInformation userGroupInformation,
			Configuration configuration, String file, ExtendedBlock blk, Token<BlockTokenIdentifier> token,
			DatanodeInfo node, long startOffset, long length, StorageType storageType) throws IOException {
		...
		if (pathinfo == null) {
			...
			pathinfo = getBlockPathInfo(userGroupInformation, blk, node, configuration, conf.socketTimeout, token,
		...
		try {
			...
			if (LOG.isDebugEnabled()) {
				LOG.debug("New BlockReaderLocalLegacy for file " + blkfile + " of size " + blkfile.length()
			...
			if (!skipChecksumCheck) {
				...
				final DataChecksum checksum = BlockMetadataHeader.readDataChecksum(new DataInputStream(checksumIn),
						blk);
				long firstChunkOffset = startOffset - (startOffset % checksum.getBytesPerChecksum());
				localBlockReader = new BlockReaderLocalLegacy(conf, file, blk, token, startOffset, length, pathinfo,
						checksum, true, dataIn, firstChunkOffset, checksumIn);
			} else {
				localBlockReader = new BlockReaderLocalLegacy(conf, file, blk, token, startOffset, length, pathinfo,
						dataIn);
			}
	...
	private static BlockLocalPathInfo getBlockPathInfo(UserGroupInformation ugi, ExtendedBlock blk, DatanodeInfo node,
			Configuration conf, int timeout, Token<BlockTokenIdentifier> token, boolean connectToDnViaHostname,
			StorageType storageType) throws IOException {
		LocalDatanodeInfo localDatanodeInfo = getLocalDatanodeInfo(node.getIpcPort());
		BlockLocalPathInfo pathinfo = null;
		ClientDatanodeProtocol proxy = localDatanodeInfo.getDatanodeProxy(ugi, node, conf, timeout,
				connectToDnViaHostname);
		try {
			...
			// storage.
			// If the replica gets evicted, then it moves to a different path.
			// Then,
			// our next attempt to read from the cached path would fail to find
			// the
			// file. Additionally, the failure would cause us to disable legacy
			// short-circuit read for all subsequent use in the ClientContext.
			// Unlike
			// the newer short-circuit read implementation, we have no
			// communication
			// channel for the DataNode to notify the client that the path has
			// been
	...
	private BlockReaderLocalLegacy(DFSClient.Conf conf, String hdfsfile, ExtendedBlock block,
			Token<BlockTokenIdentifier> token, long startOffset, long length, BlockLocalPathInfo pathinfo,
			FileInputStream dataIn) throws IOException {
		this(conf, hdfsfile, block, token, startOffset, length, pathinfo,
				DataChecksum.newDataChecksum(DataChecksum.Type.NULL, 4), false, dataIn, startOffset, null);
	}

	private BlockReaderLocalLegacy(DFSClient.Conf conf, String hdfsfile, ExtendedBlock block,
			Token<BlockTokenIdentifier> token, long startOffset, long length, BlockLocalPathInfo pathinfo,
			DataChecksum checksum, boolean verifyChecksum, FileInputStream dataIn, long firstChunkOffset,
	...
	public synchronized int read(ByteBuffer buf) throws IOException {
		...
		if (verifyChecksum) {
			...
			// bytes
			// to read, the fast direct path is used for as many remaining bytes
			// as
			// possible, up to a multiple of the checksum chunk size. Finally,
			// any
			// 'odd' bytes remaining at the end of the read cause another slow
			// read to
			// be issued, which involves an extra copy.

			// Every 'slow' read tries to fill the slow read buffer in one go
			// for
			// efficiency's sake. As described above, all
			// non-checksum-chunk-aligned
			// reads will be served from the slower read path.

			if (slowReadBuff.hasRemaining()) {
				// There are remaining bytes from a small read available. This
				// usually
				// means this read is unaligned, which falls back to the slow
				// path.
			...
			if (buf.remaining() >= bytesPerChecksum && offsetFromChunkBoundary == 0) {
				// Since we have drained the 'small read' buffer, we are
				// guaranteed to
				// be chunk-aligned
				int len = buf.remaining() - (buf.remaining() % bytesPerChecksum);

				// There's only enough checksum buffer space available to
				// checksum one
				// entire slow read buffer. This saves keeping the number of
				// checksum
				// chunks around.
			...
			// offsetFromChunkBoundary > 0 => unaligned read, use slow path to
			// read
			...
			// Non-checksummed reads are much easier; we can just fill the
			// buffer directly.
	...
	/**
	 * Tries to read as many bytes as possible into supplied buffer,
	 * checksumming each chunk if needed.
	 *
	 * <b>Preconditions:</b>
	 * <ul>
	 * <li>If checksumming is enabled, buf.remaining must be a multiple of
	 ...
	 *
	 * @param buf
	 *            byte buffer to write bytes to. If checksums are not required,
	 *            buf can have any number of bytes remaining, otherwise there
	 *            must be a multiple of the checksum chunk size remaining.
	...
	private synchronized int doByteBufferRead(ByteBuffer buf) throws IOException {
		...
		if (dataRead < offsetFromChunkBoundary) {
			// yikes, didn't even get enough bytes to honour offset. This can
			// happen
	 ...
	 * read buffer. The number of bytes available to read is returned. If the
	 * buffer is not already empty, the number of remaining bytes is returned
	 * and no actual read happens.
	 *
	 * @param len
	 *            the maximum number of bytes to make available. After len bytes
	 *            are read, the underlying bytestream <b>must</b> be at a
	 *            checksum boundary, or EOF. That is, (len + currentPosition) %
	 *            bytesPerChecksum == 0.
	...
	private synchronized int fillSlowReadBuffer(int len) throws IOException {
		...
		if (slowReadBuff.hasRemaining()) {
			...
			// Round a complete read of len bytes (plus any implicit offset) to
			// the
			// next chunk boundary, since we try and read in multiples of a
			// chunk
			...
			if (nRead > 0) {
				// So that next time we call slowReadBuff.hasRemaining(), we
				// don't get a
				// false positive.
	...
	public synchronized int read(byte[] buf, int off, int len) throws IOException {
		...
		if (nRead > 0) {
			// Possible that buffer is filled with a larger read than we need,
			// since
