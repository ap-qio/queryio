...
public class DFSAdmin extends FsShell {
		...
		static int run(DistributedFileSystem dfs, String[] argv, int idx) throws IOException {
			...
			if (action == null) {
				throw new IllegalArgumentException(
						"Failed to covert \"" + argv[1] + "\" to " + RollingUpgradeAction.class.getSimpleName());
			}
	...
	/**
	 * Common usage summary shared between "hdfs dfsadmin -help" and "hdfs
	 * dfsadmin"
	 */
	...
	public void report(String[] argv, int i) throws IOException {
		...
		// Truncate already handled arguments before parsing report()-specific
		// ones
	...
	/**
	 * Safe mode maintenance command. Usage: hdfs dfsadmin -safemode [enter |
	 * leave | get]
	 * 
	 * @param argv
	...
	public void setSafeMode(String[] argv, int idx) throws IOException {
		...
		if (isHaEnabled) {
			...
			List<ProxyAndInfo<ClientProtocol>> proxies = HAUtil.getProxiesForAllNameNodesInNameservice(dfsConf, nsId,
	...
	public int triggerBlockReport(String[] argv) throws IOException {
		...
		System.out.println(
				"Triggering " + (incremental ? "an incremental " : "a full ") + "block report on " + hostPort + ".");
		return 0;
	}

	/**
	 * Allow snapshot on a directory. Usage: hdfs dfsadmin -allowSnapshot
	 * snapshotDir
	 * 
	 * @param argv
	 *            List of of command line parameters.
	...
	/**
	 * Allow snapshot on a directory. Usage: hdfs dfsadmin -disallowSnapshot
	 * snapshotDir
	 * 
	 * @param argv
	 *            List of of command line parameters.
	 ...
	 * the namenode. Usage: hdfs dfsadmin -restoreFailedStorage true|false|check
	 * 
	 * @exception IOException
	 ...
	 * datanodes. Usage: hdfs dfsadmin -setBalancerBandwidth bandwidth
	 * 
	 * @param argv
	 *            List of of command line parameters.
	 * @param idx
	...
	/**
	 * Download the most recent fsimage from the name node, and save it to a
	 * local file in the given directory.
	 * 
	 * @param argv
	 *            List of of command line parameters.
	 * @param idx
	...
	public int fetchImage(final String[] argv, final int idx) throws IOException {
		...
		final URL infoServer = DFSUtil
				.getInfoServer(HAUtil.getAddressOfActive(getDFS()), conf, DFSUtil.getHttpClientScheme(conf)).toURL();
	 ...
	 *            The index of the command that is being processed.
	 * @exception IOException
	 *                if an error occurred while accessing the file or path.
	 */
	public int metaSave(String[] argv, int idx) throws IOException {
		...
		if (isHaEnabled) {
			...
			for (ProxyAndInfo<ClientProtocol> proxy : proxies) {
				...
				System.out.println("Created metasave file " + pathname + " in the log " + "directory of namenode "
						+ proxy.getAddress());
			}
		} else {
			dfs.metaSave(pathname);
			System.out.println(
					"Created metasave file " + pathname + " in the log " + "directory of namenode " + dfs.getUri());
		}
		return 0;
	}

	/**
	 * Display each rack and the nodes assigned to that rack, as determined by
	 * the NameNode, in a hierarchical manner. The nodes and racks are sorted
	 * alphabetically.
	 * 
	 * @throws IOException
	...
	public int refreshServiceAcl() throws IOException {
		...
		if (isHaEnabled) {
			...
			String nsId = dfsUri.getHost();
			List<ProxyAndInfo<RefreshAuthorizationPolicyProtocol>> proxies = HAUtil
					.getProxiesForAllNameNodesInNameservice(conf, nsId, RefreshAuthorizationPolicyProtocol.class);
			...
			RefreshAuthorizationPolicyProtocol refreshProtocol = NameNodeProxies
					.createProxy(conf, FileSystem.getDefaultUri(conf), RefreshAuthorizationPolicyProtocol.class)
					.getProxy();
	...
	public int refreshUserToGroupsMappings() throws IOException {
		...
		if (isHaEnabled) {
			...
			String nsId = dfsUri.getHost();
			List<ProxyAndInfo<RefreshUserMappingsProtocol>> proxies = HAUtil
					.getProxiesForAllNameNodesInNameservice(conf, nsId, RefreshUserMappingsProtocol.class);
			...
			RefreshUserMappingsProtocol refreshProtocol = NameNodeProxies
					.createProxy(conf, FileSystem.getDefaultUri(conf), RefreshUserMappingsProtocol.class).getProxy();
	...
	public int refreshSuperUserGroupsConfiguration() throws IOException {
		...
		if (isHaEnabled) {
			// Run refreshSuperUserGroupsConfiguration for all NNs if HA is
			// enabled
			String nsId = dfsUri.getHost();
			List<ProxyAndInfo<RefreshUserMappingsProtocol>> proxies = HAUtil
					.getProxiesForAllNameNodesInNameservice(conf, nsId, RefreshUserMappingsProtocol.class);
	...
	public int refreshCallQueue() throws IOException {
		...
		if (isHaEnabled) {
			...
			String nsId = dfsUri.getHost();
			List<ProxyAndInfo<RefreshCallQueueProtocol>> proxies = HAUtil.getProxiesForAllNameNodesInNameservice(conf,
					nsId, RefreshCallQueueProtocol.class);
			...
			// Create the client
			RefreshCallQueueProtocol refreshProtocol = NameNodeProxies
					.createProxy(conf, FileSystem.getDefaultUri(conf), RefreshCallQueueProtocol.class).getProxy();
		...
		if ("datanode".equals(nodeType)) {
			...
			try {
				...
				for (Map.Entry<PropertyChange, Optional<String>> result : status.getStatus().entrySet()) {
					...
					out.printf("Change property %s%n\tFrom: \"%s\"%n\tTo: \"%s\"%n", result.getKey().prop,
							result.getKey().oldVal, result.getKey().newVal);
					if (result.getValue().isPresent()) {
	...
	public int genericRefresh(String[] argv, int i) throws IOException {
		...
		GenericRefreshProtocolPB proxy = (GenericRefreshProtocolPB) RPC.getProxy(xface, RPC.getProtocolVersion(xface),
				address, ugi, conf, NetUtils.getDefaultSocketFactory(conf), 0);

		Collection<RefreshResponse> responses = null;
		try (GenericRefreshProtocolClientSideTranslatorPB xlator = new GenericRefreshProtocolClientSideTranslatorPB(
				proxy);) {
			// Refresh
			...
			for (RefreshResponse response : responses) {
				...
				if (returnCode == 0 && response.getReturnCode() != 0) {
					// This is the first non-zero return code, so we should
					// return this
	...
	private static void printUsage(String cmd) {
		if ("-report".equals(cmd)) {
			...
			System.err
					.println("Usage: hdfs dfsadmin" + " [-refresh <hostname:port> <resource_identifier> [arg1..argn]");
		} else if ("-printTopology".equals(cmd)) {
			...
			System.err
					.println("Usage: hdfs dfsadmin" + " [-triggerBlockReport [-incremental] <datanode_host:ipc_port>]");
		} else {
	...
	private ClientDatanodeProtocol getDataNodeProxy(String datanode) throws IOException {
		...
		// Create the client
		ClientDatanodeProtocol dnProtocol = DFSUtil.createClientDatanodeProtocolProxy(datanodeAddr, getUGI(), conf,
