 ...
 * Adding more options:
 * <ul>
 * <li>If adding a simple option to get a value corresponding to a key in the
 * configuration, use regular {@link GetConf.CommandHandler}. See
 * {@link GetConf.Command#EXCLUDE_FILE} example.</li>
 * <li>If adding an option that is does not return a value for a key, add a
 * subclass of {@link GetConf.CommandHandler} and set it up in
...
public class GetConf extends Configured implements Tool {
	...
	enum Command {
		NAMENODE("-namenodes", "gets list of namenodes in the cluster."), SECONDARY("-secondaryNameNodes",
				"gets list of secondary namenodes in the cluster."), BACKUP("-backupNodes",
						"gets list of backup nodes in the cluster."), INCLUDE_FILE("-includeFile",
								"gets the include file path that defines the datanodes "
										+ "that can join the cluster."), EXCLUDE_FILE(
												"-excludeFile",
												"gets the exclude file path that defines the datanodes "
														+ "that need to decommissioned."), NNRPCADDRESSES(
																"-nnRpcAddresses",
																"gets the namenode rpc addresses"), CONFKEY(
																		"-confKey [key]",
	...
	/**
	 * Handler for {@link Command#NNRPCADDRESSES} If rpc addresses are defined
	 * in configuration, we return them. Otherwise, return empty string.
	 */
	static class NNRpcAddressesCommandHandler extends CommandHandler {
		@Override
		public int doWorkInternal(GetConf tool, String[] args) throws IOException {
			Configuration config = tool.getConf();
			List<ConfiguredNNAddress> cnnlist = DFSUtil
					.flattenAddressMap(DFSUtil.getNNServiceRpcAddressesForCluster(config));
			if (!cnnlist.isEmpty()) {
