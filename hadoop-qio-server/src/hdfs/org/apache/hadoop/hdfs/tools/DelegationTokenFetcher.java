...
public class DelegationTokenFetcher {
				...
				if (renew) {
					...
					if (webUrl != null) {
						...
						for (Token<?> token : creds.getAllTokens()) {
							System.out.println("Fetched token via " + webUrl + " for " + token.getService() + " into "
									+ tokenFile);
						}
					} else {
	...
	static public void cancelDelegationToken(URLConnectionFactory factory, URI nnAddr,
		...
		StringBuilder buf = new StringBuilder(nnAddr.toString()).append(CancelDelegationTokenServlet.PATH_SPEC)
				.append("?").append(CancelDelegationTokenServlet.TOKEN).append("=").append(tok.encodeToUrlString());
	...
	static public long renewDelegationToken(URLConnectionFactory factory, URI nnAddr,
			Token<DelegationTokenIdentifier> tok) throws IOException, AuthenticationException {
		StringBuilder buf = new StringBuilder(nnAddr.toString()).append(RenewDelegationTokenServlet.PATH_SPEC)
				.append("?").append(RenewDelegationTokenServlet.TOKEN).append("=").append(tok.encodeToUrlString());
	...
	static private IOException getExceptionFromResponse(HttpURLConnection con) {
		...
		try {
			...
			java.lang.reflect.Constructor<? extends Exception> constructor = ec
