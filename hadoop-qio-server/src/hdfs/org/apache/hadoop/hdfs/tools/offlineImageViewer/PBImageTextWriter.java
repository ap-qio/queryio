...
/**
 * This class reads the protobuf-based fsimage and generates text output for
 * each inode to {@link PBImageTextWriter#out}. The sub-class can override
 ...
 * filename of each directory. It also scans the INode_Dir sections to loads the
 * relationships between a directory and its children. It uses these metadata to
 * build FS namespace and stored in {@link MetadataMap}</li>
 ...
 * {@link MetadataMap}, and generate output.</li>
 * </ol>
 *
 * Two various of {@link MetadataMap} are provided. {@link InMemoryMetadataDB}
 * stores all metadata in memory (O(n) memory) while {@link LevelDBMetadataMap}
 * stores metadata in LevelDB on disk (O(1) memory). User can choose between
 * them based on the time/space tradeoffs.
 */
abstract class PBImageTextWriter implements Closeable {
	 ...
	 * <p>
	 * <li>It maps each inode (inode Id) to its parent directory (inode
	 * Id).</li>
	...
	private static class LevelDBMetadataMap implements MetadataMap {
		...
		@Override
		public void putDir(INode dir) throws IOException {
			Preconditions.checkArgument(dir.hasDirectory(), "INode %s (%s) is not a directory.", dir.getId(),
					dir.getName());
		...
		public String getParentPath(long inode) throws IOException {
			...
			Preconditions.checkState(bytes != null && bytes.length == 8,
					"Can not find parent directory for inode %s, " + "fsimage might be corrupted", inode);
			...
			if (!dirPathCache.containsKey(parent)) {
				...
				if (parent != INodeId.ROOT_INODE_ID) {
					Preconditions.checkState(bytes != null,
							"Can not find parent directory for inode %s, " + ", the fsimage might be corrupted.",
							parent);
				}
	 ...
	 * 
	 * @param out
	 *            the writer to output text information of fsimage.
	 * @param tempPath
	 *            the path to store metadata. If it is empty, store metadata in
	 *            memory instead.
	 */
	...
	private void output(Configuration conf, FileSummary summary, FileInputStream fin,
			ArrayList<FileSummary.Section> sections) throws IOException {
		InputStream is;
	...
	private void loadDirectories(FileInputStream fin, List<FileSummary.Section> sections, FileSummary summary,
	...
	private void loadINodeDirSection(FileInputStream fin, List<FileSummary.Section> sections, FileSummary summary,
			Configuration conf) throws IOException {
