 ...
 *
 * These operations are only for debugging, and may change or disappear between
 * HDFS versions.
 */
...
public class DebugAdmin extends Configured implements Tool {
	 ...
	 */
	private DebugCommand DEBUG_COMMANDS[] = { new VerifyBlockChecksumCommand(), new RecoverLeaseCommand(),
			new HelpCommand() };
	...
	private class VerifyBlockChecksumCommand extends DebugCommand {
		VerifyBlockChecksumCommand() {
			super("verify", "verify [-meta <metadata-file>] [-block <block-file>]",
		...
		int run(List<String> args) throws IOException {
			...
			try {
				...
				try {
					...
					System.err.println("Failed to read HDFS metadata file header for " + metaFile + ": "
				...
				try {
					...
					System.err.println("Failed to open HDFS block file for " + blockFile + ": "
							+ StringUtils.stringifyException(e));
					return 1;
				}
				long offset = 0;
				while (true) {
					...
					try {
						...
						System.err.println("Got I/O error reading block file " + blockFile + "from disk at offset "
								+ dataChannel.position() + ": " + StringUtils.stringifyException(e));
						return 1;
					}
					try {
						int csumToRead = (((checksum.getBytesPerChecksum() - 1) + dataRead)
						...
						System.err.println("Got I/O error reading metadata file " + metaFile + "from disk at offset "
								+ metaChannel.position() + ": " + StringUtils.stringifyException(e));
						return 1;
					}
					try {
	...
	private class RecoverLeaseCommand extends DebugCommand {
		RecoverLeaseCommand() {
			super("recoverLease", "recoverLease [-path <path>] [-retries <num-retries>]",
		...
		int run(List<String> args) throws IOException {
			...
			if (retriesStr != null) {
				try {
					...
					System.err
							.println("Failed to parse the argument to -retries: " + StringUtils.stringifyException(e));
					return 1;
				}
			}
			FileSystem fs;
			...
			try {
				...
				System.err.println("Invalid filesystem for path " + pathStr + ": " + "needed scheme hdfs, but got: "
						+ fs.getScheme());
				return 1;
			}
			...
			System.err.println("Giving up on recoverLease for " + pathStr + " after " + maxRetries
