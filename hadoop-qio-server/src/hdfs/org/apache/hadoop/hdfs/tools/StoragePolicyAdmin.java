...
public class StoragePolicyAdmin extends Configured implements Tool {
	...
	private static class SetStoragePolicyCommand implements AdminHelper.Command {
		...
		public int run(Configuration conf, List<String> args) throws IOException {
			...
			if (path == null) {
				System.err.println(
						"Please specify the path for setting the storage " + "policy.\nUsage: " + getLongUsage());
				return 1;
			}
	...
	private static final AdminHelper.Command[] COMMANDS = { new ListStoragePoliciesCommand(),
			new SetStoragePolicyCommand(), new GetStoragePolicyCommand() };
}