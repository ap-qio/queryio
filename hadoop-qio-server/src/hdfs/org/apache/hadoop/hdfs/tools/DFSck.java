...
public class DFSck extends Configured implements Tool {
	...
	private static final String USAGE = "Usage: hdfs fsck <path> " + "[-list-corruptfileblocks | "
			...
			+ " indicates a snapshottable directory or there are " + "snapshottable directories under it\n"
			...
			+ " if the same file present in both original fs tree " + "and inside snapshots.";
	...
	/*
	 * To get the list, we need to call iteratively until the server says there
	 * is no more left.
	 */
		...
		while (!allDone) {
			...
			try {
				...
				while ((line = input.readLine()) != null) {
					...
					if ((line.endsWith(noCorruptLine)) || (line.endsWith(noMoreCorruptLine))
	...
	/**
	 * Derive the namenode http address from the current file system, either
	 * default or as set by "-fs" in the generic options.
	 * 
	...
	private int doWork(final String[] args) throws IOException {
		...
		if (namenodeAddress == null) {
			// Error message already output in {@link
		...
		url.append("&path=")
				.append(URLEncoder.encode(Path.getPathWithoutSchemeAndAuthority(dirpath).toString(), "UTF-8"));
