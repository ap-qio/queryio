...
/**
 * A DelimitedImageVisitor generates a text representation of the fsimage, with
 * each element separated by a delimiter string. All of the elements common to
 * both inodes and inodes-under-construction are included. When processing an
 * fsimage with a layout version that did not include an element, such as
 * AccessTime, the output file will include a column for the value, but no value
 * will be included.
 * 
 * Individual block information for each file is not currently included.
 * 
 * The default delimiter is tab, as this is an unlikely value to be included an
 * inode path or other text metadata. The delimiter value can be via the
 * constructor.
 */
class DelimitedImageVisitor extends TextWriterImageVisitor {
...

	{
		...
		Collections.addAll(elementsToTrack, ImageElement.INODE_PATH, ImageElement.REPLICATION,
				ImageElement.MODIFICATION_TIME, ImageElement.ACCESS_TIME, ImageElement.BLOCK_SIZE,
				ImageElement.NUM_BLOCKS, ImageElement.NUM_BYTES, ImageElement.NS_QUOTA, ImageElement.DS_QUOTA,
				ImageElement.PERMISSION_STRING, ImageElement.USER_NAME, ImageElement.GROUP_NAME);
	}
	...
	/**
	 * Reset the values of the elements we're tracking in order to handle the
	 * next file
	 */
	private void reset() {
		elements.clear();
	...
	void visit(ImageElement element, String value) throws IOException {
		...
		// Special case of file size, which is sum of the num bytes in each
		// block
