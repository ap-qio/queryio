...
public class CacheAdmin extends Configured implements Tool {
	...
	private static class AddCacheDirectiveInfoCommand implements AdminHelper.Command {
		...
		public String getShortUsage() {
			return "[" + getName() + " -path <path> -pool <pool-name> " + "[-force] "
		...
		public String getLongUsage() {
			...
			listing.addRow("<pool-name>", "The pool to which the directive will be "
					+ "added. You must have write permission on the cache pool " + "in order to add new directives.");
			...
			listing.addRow("<time-to-live>",
					"How long the directive is " + "valid. Can be specified in minutes, hours, and days, e.g. "
	...
	private static class RemoveCacheDirectiveInfoCommand implements AdminHelper.Command {
		...
		public String getLongUsage() {
			TableListing listing = AdminHelper.getOptionDescriptionListing();
			listing.addRow("<id>",
					"The id of the cache directive to remove.  " + "You must have write permission on the pool of the "
	...
	private static class RemoveCacheDirectiveInfosCommand implements AdminHelper.Command {
		...
		public String getLongUsage() {
			...
			listing.addRow("-path <path>", "The path of the cache directives to remove.  "
					...
					+ "to remove it.  To see a list of cache directives, use the " + "-listDirectives command.");
		...
		public int run(Configuration conf, List<String> args) throws IOException {
			...
			try {
				DistributedFileSystem dfs = AdminHelper.getDFS(conf);
				RemoteIterator<CacheDirectiveEntry> iter = dfs
						.listCacheDirectives(new CacheDirectiveInfo.Builder().setPath(new Path(path)).build());
				while (iter.hasNext()) {
	...
	private static class ListCacheDirectiveInfoCommand implements AdminHelper.Command {
		...
		public String getLongUsage() {
			...
			listing.addRow("<path>",
					"List only " + "cache directives with this path. "
		...
		public int run(Configuration conf, List<String> args) throws IOException {
			...
			TableListing.Builder tableBuilder = new TableListing.Builder().addField("ID", Justification.RIGHT)
					.addField("POOL", Justification.LEFT).addField("REPL", Justification.RIGHT)
					.addField("EXPIRY", Justification.LEFT).addField("PATH", Justification.LEFT);
			if (printStats) {
				tableBuilder.addField("BYTES_NEEDED", Justification.RIGHT).addField("BYTES_CACHED", Justification.RIGHT)
						.addField("FILES_NEEDED", Justification.RIGHT).addField("FILES_CACHED", Justification.RIGHT);
			}
	...
	private static class AddCachePoolCommand implements AdminHelper.Command {
		...
		public String getLongUsage() {
			...
			listing.addRow("<limit>", "The maximum number of bytes that can be "
					+ "cached by directives in this pool, in aggregate. By default, " + "no limit is set.");
			listing.addRow("<maxTtl>",
	...
	private static class ModifyCachePoolCommand implements AdminHelper.Command {
		...
		public String getLongUsage() {
			...
			return getShortUsage() + "\n"
					+ WordUtils.wrap("Modifies the metadata of an existing cache pool. " + "See usage of "
							+ AddCachePoolCommand.NAME + " for more details.", AdminHelper.MAX_LINE_WIDTH)
					+ "\n\n" + listing.toString();
		}
	...
	private static class ListCachePoolsCommand implements AdminHelper.Command {
		...
		public String getLongUsage() {
			...
			return getShortUsage() + "\n"
					+ WordUtils.wrap("Display information about one or more cache pools, "
							+ "e.g. name, owner, group, permissions, etc.", AdminHelper.MAX_LINE_WIDTH)
					+ "\n\n" + listing.toString();
		}
		...
		public int run(Configuration conf, List<String> args) throws IOException {
			...
			DistributedFileSystem dfs = AdminHelper.getDFS(conf);
			TableListing.Builder builder = new TableListing.Builder().addField("NAME", Justification.LEFT)
					.addField("OWNER", Justification.LEFT).addField("GROUP", Justification.LEFT)
					.addField("MODE", Justification.LEFT).addField("LIMIT", Justification.RIGHT)
					.addField("MAXTTL", Justification.RIGHT);
			if (printStats) {
				builder.addField("BYTES_NEEDED", Justification.RIGHT).addField("BYTES_CACHED", Justification.RIGHT)
						.addField("BYTES_OVERLIMIT", Justification.RIGHT).addField("FILES_NEEDED", Justification.RIGHT)
	...
	private static final AdminHelper.Command[] COMMANDS = { new AddCacheDirectiveInfoCommand(),
			new ModifyCacheDirectiveInfoCommand(), new ListCacheDirectiveInfoCommand(),
			new RemoveCacheDirectiveInfoCommand(), new RemoveCacheDirectiveInfosCommand(), new AddCachePoolCommand(),
			new ModifyCachePoolCommand(), new RemoveCachePoolCommand(), new ListCachePoolsCommand() };
}
