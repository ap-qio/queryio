...
/**
 * PBImageXmlWriter walks over an fsimage structure and writes out an equivalent
 * XML document that contains the fsimage's components.
 */
...
public final class PBImageXmlWriter {
	...
	private void dumpCacheManagerSection(InputStream is) throws IOException {
		...
		for (int i = 0; i < s.getNumPools(); ++i) {
			...
			out.print("<pool>");
			o("poolName", p.getPoolName()).o("ownerName", p.getOwnerName()).o("groupName", p.getGroupName())
					.o("mode", p.getMode()).o("limit", p.getLimit()).o("maxRelativeExpiry", p.getMaxRelativeExpiry());
			out.print("</pool>\n");
		}
	...
	private void dumpINodeReference(INodeReferenceSection.INodeReference r) {
		...
		o("referredId", r.getReferredId()).o("name", r.getName().toStringUtf8())
				.o("dstSnapshotId", r.getDstSnapshotId()).o("lastSnapshotId", r.getLastSnapshotId());
		out.print("</ref>\n");
	}

	private void dumpINodeFile(INodeSection.INodeFile f) {
		o("replication", f.getReplication()).o("mtime", f.getModificationTime()).o("atime", f.getAccessTime())
				.o("perferredBlockSize", f.getPreferredBlockSize()).o("permission", dumpPermission(f.getPermission()));
	...
	private void dumpNameSection(InputStream in) throws IOException {
		...
		o("genstampV1", s.getGenstampV1()).o("genstampV2", s.getGenstampV2())
				.o("genstampV1Limit", s.getGenstampV1Limit()).o("lastAllocatedBlockId", s.getLastAllocatedBlockId())
		...
		while (true) {
			...
			case DIRECTORYDIFF: {
				for (int i = 0; i < e.getNumOfDiff(); ++i) {
					...
					o("snapshotId", d.getSnapshotId()).o("isSnapshotroot", d.getIsSnapshotRoot())
							.o("childrenSize", d.getChildrenSize()).o("name", d.getName().toStringUtf8());
