 ...
 * 
 * <h3>Description.</h3> This is the tool for analyzing file sizes in the
 * namespace image. In order to run the tool one should define a range of
 * integers <tt>[0, maxSize]</tt> by specifying <tt>maxSize</tt> and a
 * <tt>step</tt>. The range of integers is divided into segments of size
 * <tt>step</tt>: <tt>[0, s<sub>1</sub>, ..., s<sub>n-1</sub>, maxSize]</tt>,
 * and the visitor calculates how many files in the system fall into each
 * segment <tt>[s<sub>i-1</sub>, s<sub>i</sub>)</tt>. Note that files larger
 * than <tt>maxSize</tt> always fall into the very last segment.
 * 
 * <h3>Input.</h3>
 * <ul>
