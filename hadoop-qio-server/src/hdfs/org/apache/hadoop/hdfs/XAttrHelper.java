...
public class XAttrHelper {
	...
	/**
	 * Build <code>XAttr</code> from name with prefix and value. Name can not be
	 * null. Value can be null. The name and prefix are validated. Both name and
	...
	public static XAttr buildXAttr(String name, byte[] value) {
		...
		XAttr xAttr = (new XAttr.Builder()).setNameSpace(ns).setName(name.substring(prefixIndex + 1)).setValue(value)
				.build();
	 ...
	 * prefix, and value is xattr value.
	 */
