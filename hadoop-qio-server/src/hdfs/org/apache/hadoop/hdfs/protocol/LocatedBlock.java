...
/**
 * Associates a block with the Datanodes that contain its replicas and other
 * block metadata (E.g. the file offset associated with this block, whether it
 * is corrupt, a location is cached in memory, security token, etc).
 */
...
public class LocatedBlock {
	...
	public LocatedBlock(ExtendedBlock b, DatanodeStorageInfo[] storages, long startOffset, boolean corrupt) {
		this(b, DatanodeStorageInfo.toDatanodeInfos(storages), DatanodeStorageInfo.toStorageIDs(storages),
				DatanodeStorageInfo.toStorageTypes(storages), startOffset, corrupt, EMPTY_LOCS); // startOffset
																									// is
																									// unknown
	}
		...
		if (locs == null) {
			...
			for (int i = 0; i < locs.length; i++) {
				...
				DatanodeInfoWithStorage storage = new DatanodeInfoWithStorage(di,
						storageIDs != null ? storageIDs[i] : null, storageTypes != null ? storageTypes[i] : null);
				this.locs[i] = storage;
			}
		}
	 ...
	 * not expected to be modified. If it is, caller must immediately invoke
	...
	/**
	 * Updates the cached StorageID and StorageType information. Must be called
	 * when the locations array is modified.
	 */
	 ...
	 * 
	 * @param loc
	 *            of datanode with the cached replica
	 */
