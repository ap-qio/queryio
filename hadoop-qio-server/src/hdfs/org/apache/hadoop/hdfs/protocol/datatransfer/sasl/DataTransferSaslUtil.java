...
public final class DataTransferSaslUtil {
	...
	public static void checkSaslComplete(SaslParticipant sasl, Map<String, String> saslProps) throws IOException {
		...
		if (!requestedQop.contains(negotiatedQop)) {
			throw new IOException(String.format("SASL handshake completed, but "
					+ "channel does not have acceptable quality of protection, " + "requested = %s, negotiated = %s",
	...
	/**
	 * Creates a SaslPropertiesResolver from the given configuration. This
	 * method works by cloning the configuration, translating configuration
	 * properties specific to DataTransferProtocol to what
	 * SaslPropertiesResolver expects, and then delegating to
	 * SaslPropertiesResolver for initialization. This method returns null if
	 * SASL protection has not been configured for DataTransferProtocol.
	 *
	 * @param conf
	...
	public static SaslPropertiesResolver getSaslPropertiesResolver(Configuration conf) {
		...
		Class<? extends SaslPropertiesResolver> resolverClass = conf.getClass(HADOOP_SECURITY_SASL_PROPS_RESOLVER_CLASS,
		...
		LOG.debug("DataTransferProtocol using SaslPropertiesResolver, configured QOP {} = {}, configured class {} = {}",
				DFS_DATA_TRANSFER_PROTECTION_KEY, qops, DFS_DATA_TRANSFER_SASL_PROPS_RESOLVER_CLASS_KEY, resolverClass);
		return resolver;
	}
	 ...
	 *            stream to read
	 * @param cipherOptions
	 *            list to store negotiation cipher options
	 * @return byte[] SASL negotiation message
	 * @throws IOException
	 *             for any error
	 */
	public static byte[] readSaslMessageAndNegotiationCipherOptions(InputStream in, List<CipherOption> cipherOptions)
			throws IOException {
		DataTransferEncryptorMessageProto proto = DataTransferEncryptorMessageProto.parseFrom(vintPrefixed(in));
		if (proto.getStatus() == DataTransferEncryptorStatus.ERROR_UNKNOWN_KEY) {
			throw new InvalidEncryptionKeyException(proto.getMessage());
	 ...
	 *            the configuration
	 * @param options
	 *            the cipher options which client supports
	 ...
	 *             for any error
	 */
	public static void sendSaslMessageAndNegotiatedCipherOption(OutputStream out, byte[] payload, CipherOption option)
			throws IOException {
	 ...
	 *            negotiated cipher option
	 * @param sasl
	 *            SASL participant representing server
	 ...
	 *            negotiated cipher option
	 * @param sasl
	 *            SASL participant representing client
	 * @return CipherOption negotiated cipher option which contains the
