...
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.negotiateCipherOption;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.readSaslMessage;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.readSaslMessageAndNegotiationCipherOptions;
...
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.sendSaslMessageAndNegotiatedCipherOption;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.wrap;
...
/**
 * Negotiates SASL for DataTransferProtocol on behalf of a server. There are two
...
public class SaslDataTransferServer {
	 ...
	 *
	 * @param dnConf
	 *            configuration of DataNode
	 * @param blockPoolTokenSecretManager
	 *            used for checking block access tokens and encryption keys
	 */
	 ...
	 * @param datanodeId
	 *            ID of DataNode accepting connection
	 * @return new pair of streams, wrapped after SASL negotiation
	 * @throws IOException
	 *             for any error
	 */
	public IOStreamPair receive(Peer peer, OutputStream underlyingOut, InputStream underlyingIn, int xferPort,
		...
		if (dnConf.getEncryptDataTransfer()) {
			...
			// ignore.secure.ports.for.testing. That's intended for dev use
			// only.
			// This code path is not expected to execute ever, because DataNode
			// startup
			// checks for invalid configuration and aborts.
			throw new IOException(String.format(
					"Cannot create a secured " + "connection if DataNode listens on unprivileged port (%d) and no "
	 ...
	 *             for any error
	 */
	private IOStreamPair getEncryptedStreams(Peer peer, OutputStream underlyingOut, InputStream underlyingIn)
			throws IOException {
	...
	private static final class SaslServerCallbackHandler implements CallbackHandler {
		 ...
		 *
		 * @param passwordFunction
		 *            for determing the user's password
		 */
	...
	/**
	 * Given a secret manager and a username encoded for the encrypted
	 * handshake, determine the encryption key.
	 * 
	 * @param userName
	 ...
	 *             for any error
	 */
	private IOStreamPair getSaslStreams(Peer peer, OutputStream underlyingOut, InputStream underlyingIn)
			throws IOException {
		if (peer.hasSecureChannel() || dnConf.getTrustedChannelResolver().isTrusted(getPeerAddress(peer))) {
			return new IOStreamPair(underlyingIn, underlyingOut);
		}
	...
	private IOStreamPair doSaslHandshake(OutputStream underlyingOut, InputStream underlyingIn,
		...
		try {
			...
			if (ioe instanceof SaslException && ioe.getCause() != null
				...
				// This could just be because the client is long-lived and
				// hasn't gotten
				// a new encryption key from the NN in a while. Upon receiving
				// this
				// error, the client will get a new encryption key from the NN
				// and retry
