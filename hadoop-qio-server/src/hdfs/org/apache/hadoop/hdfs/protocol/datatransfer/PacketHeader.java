 ...
 * actual data.
 * 
 * This data includes: - the offset in bytes into the HDFS block of the data in
 * this packet - the sequence number of this packet in the pipeline - whether or
 * not this is the last packet in the pipeline - the length of the data in this
 * packet - whether or not this packet should be synced by the DNs.
 * 
 * When serialized, this header is written out as a protocol buffer, preceded by
 * a 4-byte integer representing the full packet length, and a 2-byte short
 * representing the header length.
 */
@InterfaceAudience.Private
@InterfaceStability.Evolving
public class PacketHeader {
	private static final int MAX_PROTO_SIZE = PacketHeaderProto.newBuilder().setOffsetInBlock(0).setSeqno(0)
			.setLastPacketInBlock(false).setDataLen(0).setSyncBlock(false).build().getSerializedSize();
	...
	public PacketHeader(int packetLen, long offsetInBlock, long seqno, boolean lastPacketInBlock, int dataLen,
		...
		PacketHeaderProto.Builder builder = PacketHeaderProto.newBuilder().setOffsetInBlock(offsetInBlock)
				.setSeqno(seqno).setLastPacketInBlock(lastPacketInBlock).setDataLen(dataLen);

		if (syncBlock) {
			// Only set syncBlock if it is specified.
			// This is wire-incompatible with Hadoop 2.0.0-alpha due to
			// HDFS-3721
			// because it changes the length of the packet header, and
			// BlockReceiver
	...
	/**
	 * @return the number of bytes necessary to write out this header, including
	 *         the length-prefixing of the payload and header
	 */
	...
	/**
	 * Write the header into the buffer. This requires that PKT_HEADER_LEN bytes
	 * are available.
	 */
	public void putInBuffer(final ByteBuffer buf) {
		assert proto.getSerializedSize() <= MAX_PROTO_SIZE : "Expected " + (MAX_PROTO_SIZE) + " got: "
