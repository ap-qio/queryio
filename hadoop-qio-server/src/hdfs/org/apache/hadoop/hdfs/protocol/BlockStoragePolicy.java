...
/**
 * A block storage policy describes how to select the storage types for the
 * replicas of a block.
 */
...
public class BlockStoragePolicy {
	...
	/**
	 * Whether the policy is inherited during file creation. If set then the
	 * policy cannot be changed after file creation.
	 */
	private boolean copyOnCreateFile;

	@VisibleForTesting
	public BlockStoragePolicy(byte id, String name, StorageType[] storageTypes, StorageType[] creationFallbacks,
	...
	@VisibleForTesting
	public BlockStoragePolicy(byte id, String name, StorageType[] storageTypes, StorageType[] creationFallbacks,
			StorageType[] replicationFallbacks, boolean copyOnCreateFile) {
		this.id = id;
		this.name = name;
	...
	private List<StorageType> chooseStorageTypes(final short replication, final Iterable<StorageType> chosen,
			final List<StorageType> excess) {
	...
	/**
	 * Choose the storage types for storing the remaining replicas, given the
	 * replication number, the storage types of the chosen replicas and the
	 * unavailable storage types. It uses fallback storage in case that the
	 * desired storage type is unavailable.
	 *
	 * @param replication
	 *            the replication number.
	 * @param chosen
	 ...
	 * @param isNewBlock
	 *            Is it for new block creation?
	 * @return a list of {@link StorageType}s for storing the replicas of a
	 *         block.
	 */
	public List<StorageType> chooseStorageTypes(final short replication, final Iterable<StorageType> chosen,
			final EnumSet<StorageType> unavailables, final boolean isNewBlock) {
		...
		for (int i = storageTypes.size() - 1; i >= 0; i--) {
			...
			if (unavailables.contains(t)) {
				final StorageType fallback = isNewBlock ? getCreationFallback(unavailables)
		...
		if (storageTypes.size() < expectedSize) {
			LOG.warn("Failed to place enough replicas: expected size is " + expectedSize + " but only "
					+ storageTypes.size() + " storage types can be selected " + "(replication=" + replication
					+ ", selected=" + storageTypes + ", unavailable=" + unavailables + ", removed=" + removed
					+ ", policy=" + this + ")");
		}
		return storageTypes;
	}
	 ...
	 * computation we have: t = t - c; Further, if e is not null, set e = e + c
	 * - t;
	 */
	...
	/**
	 * Choose excess storage types for deletion, given the replication number
	 * and the storage types of the chosen replicas.
	 *
	 * @param replication
	 *            the replication number.
	 * @param chosen
	...
	public String toString() {
		return getClass().getSimpleName() + "{" + name + ":" + id + ", storageTypes=" + Arrays.asList(storageTypes)
				+ ", creationFallbacks=" + Arrays.asList(creationFallbacks) + ", replicationFallbacks="
