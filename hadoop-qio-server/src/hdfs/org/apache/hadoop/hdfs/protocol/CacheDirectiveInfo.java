...
public class CacheDirectiveInfo {
	...
	public static class Builder {
		 ...
		 * 
		 * @param replication
		 *            The replication used in this request.
		 * @return This builder, for call chaining.
		 */
		 ...
		 * {@link CacheDirectiveInfo.Expiration} can specify either an absolute
		 * or relative expiration time.
		 * 
		 * @param expiration
		 *            when this CacheDirective should expire
		 * @return This builder, for call chaining
		 */
	 ...
	 * and {@link CacheDirectiveInfo.Expiration#newRelative(long)} to create an
	 * Expiration.
	 * <p>
	...
	public static class Expiration {
		 ...
		 */
		public static final long MAX_RELATIVE_EXPIRY_MS = Long.MAX_VALUE / 4; // This
																				// helps
																				// prevent
																				// weird
																				// overflow
		 ...
		 * 
		 * @param ms
		 *            how long until the CacheDirective expires, in milliseconds
		 ...
		 * 
		 * @param ms
		 *            when the CacheDirective expires, in milliseconds since the
		 *            Unix epoch.
		 * @return An absolute Expiration
		 */
		...
		/**
		 * @return true if Expiration was specified as a relative duration,
		 *         false if specified as an absolute time.
		 */
		...
		/**
		 * @return The raw underlying millisecond value, either a relative
		 *         duration or an absolute time as milliseconds since the Unix
		 *         epoch.
		 */
		 ...
		 *         converts a relative Expiration into an absolute time based on
		 *         the local clock.
		 */
	...
	public boolean equals(Object o) {
		...
		return new EqualsBuilder().append(getId(), other.getId()).append(getPath(), other.getPath())
				.append(getReplication(), other.getReplication()).append(getPool(), other.getPool())
				.append(getExpiration(), other.getExpiration()).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(id).append(path).append(replication).append(pool).append(expiration)
				.hashCode();
	}
