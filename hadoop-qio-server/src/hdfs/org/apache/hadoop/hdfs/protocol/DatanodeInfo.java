...
/**
 * This class extends the primary identifier of a Datanode with ephemeral state,
 * eg usage information, current administrative state, and the network location
 * that is communicated to clients.
 */
...
public class DatanodeInfo extends DatanodeID implements Node {
	...
	public DatanodeInfo(DatanodeID nodeID, String location, final long capacity, final long dfsUsed,
			final long remaining, final long blockPoolUsed, final long cacheCapacity, final long cacheUsed,
		...
		this(nodeID.getIpAddr(), nodeID.getHostName(), nodeID.getDatanodeUuid(), nodeID.getXferPort(),
				nodeID.getInfoPort(), nodeID.getInfoSecurePort(), nodeID.getIpcPort(), capacity, dfsUsed, remaining,
				blockPoolUsed, cacheCapacity, cacheUsed, lastUpdate, lastUpdateMonotonic, xceiverCount, location,
				adminState);
	}

	/** Constructor */
	public DatanodeInfo(final String ipAddr, final String hostName, final String datanodeUuid, final int xferPort,
			final int infoPort, final int infoSecurePort, final int ipcPort, final long capacity, final long dfsUsed,
			final long remaining, final long blockPoolUsed, final long cacheCapacity, final long cacheUsed,
			final long lastUpdate, final long lastUpdateMonotonic, final int xceiverCount, final String networkLocation,
			final AdminStates adminState) {
	...
	/**
	 * Get the last update timestamp. Return value is suitable for Date
	 * conversion.
	 */
	 ...
	 * Ps: So return value is ideal for calculation of time differences. Should
	 * not be used to convert to Date.
	 */
	 ...
	 * received heartbeat msg from a datanode for more than staleInterval
	 * (default value is
	 * {@link DFSConfigKeys#DFS_NAMENODE_STALE_DATANODE_INTERVAL_DEFAULT}), the
	 * datanode will be treated as stale node.
	 * 
	 * @param staleInterval
	 *            the time interval for marking the node as stale. If the last
	 *            update time is beyond the given time interval, the node will
	 *            be marked as stale.
