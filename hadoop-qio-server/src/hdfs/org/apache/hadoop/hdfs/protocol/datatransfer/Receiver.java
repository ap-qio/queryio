...
public abstract class Receiver implements DataTransferProtocol {
	...
	private void opReadBlock() throws IOException {
		...
		try {
			readBlock(PBHelper.convert(proto.getHeader().getBaseHeader().getBlock()),
					PBHelper.convert(proto.getHeader().getBaseHeader().getToken()), proto.getHeader().getClientName(),
	...
	private void opWriteBlock(DataInputStream in) throws IOException {
		...
		try {
			writeBlock(PBHelper.convert(proto.getHeader().getBaseHeader().getBlock()),
					...
					PBHelper.convert(proto.getHeader().getBaseHeader().getToken()), proto.getHeader().getClientName(),
					targets, PBHelper.convertStorageTypes(proto.getTargetStorageTypesList(), targets.length),
	...
	private void opRequestShortCircuitFds(DataInputStream in) throws IOException {
		...
		try {
			requestShortCircuitFds(PBHelper.convert(proto.getHeader().getBlock()),
					PBHelper.convert(proto.getHeader().getToken()), slotId, proto.getMaxVersion(),
	...
	private void opReleaseShortCircuitFds(DataInputStream in) throws IOException {
		final ReleaseShortCircuitAccessRequestProto proto = ReleaseShortCircuitAccessRequestProto
	...
	private void opReplaceBlock(DataInputStream in) throws IOException {
		...
		try {
			replaceBlock(PBHelper.convert(proto.getHeader().getBlock()),
					PBHelper.convertStorageType(proto.getStorageType()), PBHelper.convert(proto.getHeader().getToken()),
