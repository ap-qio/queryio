...
public interface ClientProtocol {
	...
	/**
	 * Until version 69, this class ClientProtocol served as both the client
	 * interface to the NN AND the RPC protocol used to communicate with the NN.
	 * 
	 * This class is used by both the DFSClient and the NN server side to
	 * insulate from the protocol serialization.
	 * 
	 * If you are adding/changing this interface then you need to change both
	 * this class and ALSO related protocol buffer wire protocol definition in
	 * ClientNamenodeProtocol.proto.
	 * 
	 * For more details on protocol buffer wire protocol, please see
	 * .../org/apache/hadoop/hdfs/protocolPB/overview.html
	 * 
	 * The log of historical changes can be retrieved from the svn). 69:
	 * Eliminate overloaded method names.
	 * 
	 ...
	 * clients. Although, other clients cannot {@link #delete(String, boolean)},
	 ...
	 * files must also use {@link #addBlock}
	 *
	 * @param src
	 ...
	 *            exists or create if it does not exist or append.
	 * @param createParent
	 ...
	 *             If file creation violates disk space quota restriction
	 * @throws FileAlreadyExistsException
	 *             If file <code>src</code> already exists
	 * @throws FileNotFoundException
	 *             If parent of <code>src</code> does not exist and
	 *             <code>createParent</code> is false
	 * @throws ParentNotDirectoryException
	 *             If parent of <code>src</code> is not a directory.
	 * @throws NSQuotaExceededException
	 *             If file creation violates name space quota restriction
	 ...
	 *             <p>
	 *             <em>Note that create with {@link CreateFlag#OVERWRITE} is
	 *             idempotent.</em>
	 */
	@AtMostOnce
	 ...
	 *            name of the current client.
	 * @param flag
	 *            indicates whether the data is appended to a new block.
	 ...
	 *             {@link org.apache.hadoop.ipc.RemoteException}. Allows
	 *             appending to an existing file if the server is configured
	 *             with the parameter dfs.support.append set to true, otherwise
	 ...
	 * @throws DSQuotaExceededException
	 *             If append violates disk space quota restriction
	 * @throws SafeModeException
	...
	public LastBlockWithStatus append(String src, String clientName, EnumSetWritable<CreateFlag> flag)
			throws AccessControlException, DSQuotaExceededException, FileNotFoundException, SafeModeException,
	 ...
	 *             If access is denied
	 * @throws DSQuotaExceededException
	 *             If replication violates disk space quota restriction
	 * @throws FileNotFoundException
	 *             If file <code>src</code> is not found
	 * @throws SafeModeException
	 ...
	 *             If changes violate the quota restriction
	 */
	@Idempotent
	public void setStoragePolicy(String src, String policyName) throws SnapshotAccessControlException,
			UnresolvedLinkException, FileNotFoundException, QuotaExceededException, IOException;
	 ...
	 */
	@Idempotent
	public void setPermission(String src, FsPermission permission) throws AccessControlException, FileNotFoundException,
			SafeModeException, UnresolvedLinkException, SnapshotAccessControlException, IOException;
	...
	/**
	 * The client can give up on a block by calling abandonBlock(). The client
	 ...
	 *            Block to abandon
	 * @param fileId
	 *            The id of the file where the block resides. Older clients will
	 *            pass GRANDFATHER_INODE_ID here.
	 * @param src
	 *            The path of the file where the block resides.
	 * @param holder
	 *            Lease holder.
	 *
	 * @throws AccessControlException
	 ...
	 * replicated to.
	 * 
	 * addBlock() also commits the previous block by reporting to the name-node
	 * the actual generation stamp and the length of the block that the client
	 * has transmitted to data-nodes.
	 *
	 * @param src
	 ...
	 *            previous block
	 * @param excludeNodes
	 *            a list of nodes that should not be allocated for the current
	 *            block
	 * @param fileId
	 *            the id uniquely identifying a file
	 * @param favoredNodes
	 *            the list of nodes where the client wants the blocks. Nodes are
	 *            identified by either host name or address.
	 *
	 ...
	 *             previous blocks of the file are not replicated yet. Blocks
	 *             cannot be added until replication completes.
	 * @throws SafeModeException
	 *             create not allowed in safemode
	 * @throws UnresolvedLinkException
	 *             If <code>src</code> contains a symlink
	...
	public LocatedBlock addBlock(String src, String clientName, ExtendedBlock previous, DatanodeInfo[] excludeNodes,
			long fileId, String[] favoredNodes) throws AccessControlException, FileNotFoundException,
			NotReplicatedYetException, SafeModeException, UnresolvedLinkException, IOException;
	 ...
	 */
	@Idempotent
	public LocatedBlock getAdditionalDatanode(final String src, final long fileId, final ExtendedBlock blk,
			final DatanodeInfo[] existings, final String[] existingStorageIDs, final DatanodeInfo[] excludes,
	 ...
	 * 
	 * close() also commits the last block of file by reporting to the name-node
	 * the actual generation stamp and the length of the block that the client
	 * has transmitted to data-nodes.
	 *
	 ...
	 *            the id uniquely identifying a file
	 *
	 * @return true if all file blocks are minimally replicated or false
	 *         otherwise
	 *
	 * @throws AccessControlException
	 *             If access is denied
	 ...
	 * @throws UnresolvedLinkException
	 *             if <code>trg</code> or <code>srcs</code> contains a symlink
	 * @throws SnapshotAccessControlException
	 *             if path is in RO snapshot
	 */
	@AtMostOnce
	 ...
	 * </ul>
	 * <p>
	 * Without OVERWRITE option, rename fails if the dst already exists. With
	 * OVERWRITE option, rename overwrites the dst, if it is a file or an empty
	 * directory. Rename fails if dst is a non-empty directory.
	 * <p>
	 ...
	 *             If access is denied
	 * @throws DSQuotaExceededException
	 *             If rename violates disk space quota restriction
	 * @throws FileAlreadyExistsException
	 ...
	 * @throws NSQuotaExceededException
	 *             If rename violates namespace quota restriction
	 * @throws ParentNotDirectoryException
	 *             If parent of <code>dst</code> is not a directory
	 * @throws SafeModeException
	 *             rename not allowed in safemode
	 * @throws UnresolvedLinkException
	 *             If <code>src</code> or <code>dst</code> contains a symlink
	 * @throws SnapshotAccessControlException
	 *             if path is in RO snapshot
	 * @throws IOException
	 *             If an I/O error occurred
	...
	public void rename2(String src, String dst, Options.Rename... options)
			throws AccessControlException, DSQuotaExceededException, FileAlreadyExistsException, FileNotFoundException,
			NSQuotaExceededException, ParentNotDirectoryException, SafeModeException, UnresolvedLinkException,
			SnapshotAccessControlException, IOException;
	 ...
	 * </ul>
	 * <p>
	 * This implementation of truncate is purely a namespace operation if
	 * truncate occurs at a block boundary. Requires DataNode block recovery
	 * otherwise.
	 * <p>
	 * 
	 * @param src
	 ...
	 *             If file <code>src</code> is not found
	 * @throws SafeModeException
	 *             truncate not allowed in safemode
	 * @throws UnresolvedLinkException
	 *             If <code>src</code> contains a symlink
	 * @throws SnapshotAccessControlException
	 *             if path is in RO snapshot
	 ...
	 */
	@AtMostOnce
	public boolean delete(String src, boolean recursive) throws AccessControlException, FileNotFoundException,
			SafeModeException, UnresolvedLinkException, SnapshotAccessControlException, IOException;
	 ...
	 */
	@Idempotent
	public boolean mkdirs(String src, FsPermission masked, boolean createParent) throws AccessControlException,
			FileAlreadyExistsException, FileNotFoundException, NSQuotaExceededException, ParentNotDirectoryException,
			SafeModeException, UnresolvedLinkException, SnapshotAccessControlException, IOException;
	...
	/**
	 * Start lease recovery. Lightweight NameNode operation to trigger lease
	 * recovery
	 * 
	 * @param src
	...
	/**
	 * Get a set of statistics about the filesystem. Right now, only seven
	 * values are returned.
	 * <ul>
	 ...
	 * <li>[6] contains number of blocks which have replication factor 1 and
	 * have lost the only replica.</li>
	 * </ul>
	 ...
	 * each block must have at least <tt>dfs.namenode.replication.min</tt>
	 * replicas. When the threshold is reached the name node extends safe mode
	 * for a configurable amount of time to let the remaining data nodes to
	 * check in before it will start replicating missing blocks. Then the name
	 * node leaves safe mode.
	 * <p>
	 * If safe mode is turned on manually using
	 * {@link #setSafeMode(HdfsConstants.SafeModeAction,boolean)
	 * setSafeMode(SafeModeAction.SAFEMODE_ENTER,false)} then the name node
	 * stays in safe mode until it is manually turned off using
	 * {@link #setSafeMode(HdfsConstants.SafeModeAction,boolean)
	 * setSafeMode(SafeModeAction.SAFEMODE_LEAVE,false)}. Current state of the
	 * name node can be verified using
	 * {@link #setSafeMode(HdfsConstants.SafeModeAction,boolean)
	 * setSafeMode(SafeModeAction.SAFEMODE_GET,false)}
	 * <h4>Configuration parameters:</h4> <tt>dfs.safemode.threshold.pct</tt> is
	 * the threshold parameter.<br>
	 * <tt>dfs.safemode.extension</tt> is the safe mode extension parameter.<br>
	 * <tt>dfs.namenode.replication.min</tt> is the minimal replication
	 * parameter.
	 * 
	 * <h4>Special cases:</h4> The name node does not enter safe mode at startup
	 * if the threshold is set to 0 or if the name space is empty.<br>
	 ...
	 * @param action
	 *            <ul>
	 *            <li>0 leave safe mode;</li>
	 *            <li>1 enter safe mode;</li>
	 *            <li>2 get safe mode state.</li>
	 *            </ul>
	 * @param isChecked
	 ...
	 * @return
	 *         <ul>
	 *         <li>0 if the safe mode is OFF or</li>
	 *         <li>1 if the safe mode is ON.</li>
	 *         </ul>
	 * 
	 * @throws IOException
	 */
	 ...
	 * 
	 * @param action
	 *            either query, start or finailze.
	...
	/**
	 * Dumps namenode data structures into specified file. If the file already
	 * exists, then append.
	 *
	 * @throws IOException
	 */
	...
	/**
	 * Get the file info for a specific file or directory. If the path refers to
	 * a symlink then the FileStatus of the symlink is returned.
	 * 
	 * @param src
	 *            The string representation of the path to the file
	 *
	 ...
	 *            Limit on storage space occupied all the files under this
	 *            directory.
	 * @param type
	 *            StorageType that the space quota is intended to be set on. It
	 *            may be null when called by traditional space/namespace quota.
	 *            When type is is not null, the storagespaceQuota parameter is
	 *            for type specified and namespaceQuota must be
	 ...
	 *            {@link HdfsConstants#QUOTA_DONT_SET} implies the quota will
	 *            not be changed, and (3) {@link HdfsConstants#QUOTA_RESET}
	 *            implies the quota will be reset. Any other value is a runtime
	 *            error.
	 * 
	 * @throws AccessControlException
	 *             permission denied
	 ...
	 *            The string representation of the path
	 * @param inodeId
	 *            The inode ID, or GRANDFATHER_INODE_ID if the client is too old
	 *            to support fsync with inode IDs.
	 * @param client
	 *            The string representation of the client
	 * @param lastBlockLength
	 *            The length of the last block (under construction) to be
	 *            reported to NameNode
	 * @throws AccessControlException
	 *             permission denied
	 * @throws FileNotFoundException
	 *             file <code>src</code> is not found
	 ...
	 */
	@Idempotent
	public void setTimes(String src, long mtime, long atime) throws AccessControlException, FileNotFoundException,
			UnresolvedLinkException, SnapshotAccessControlException, IOException;
	 ...
	 * 
	 * @param target
	 *            The path of the destination that the link points to.
	 * @param link
	 ...
	 * @throws FileNotFoundException
	 *             If parent of <code>link</code> does not exist and
	 *             <code>createParent</code> is false
	 * @throws ParentNotDirectoryException
	...
	public void createSymlink(String target, String link, FsPermission dirPerm, boolean createParent)
			...
			ParentNotDirectoryException, SafeModeException, UnresolvedLinkException, SnapshotAccessControlException,
			IOException;
	 ...
	 */
	@AtMostOnce
	public void updatePipeline(String clientName, ExtendedBlock oldBlock, ExtendedBlock newBlock, DatanodeID[] newNodes,
			String[] newStorageIDs) throws IOException;
	 ...
	 * 
	 * @param snapshotRoot
	 *            the path that is being snapshotted
	 * @param snapshotName
	 ...
	 * 
	 * @param snapshotRoot
	 *            the directory to disallow snapshot
	 * @throws IOException
	 *             on error
	 */
	@Idempotent
	 ...
	 *            tree.
	 * @return The difference report represented as a
	 *         {@link SnapshotDiffReport}.
	 * @throws IOException
	 *             on error
	 */
	@Idempotent
	public SnapshotDiffReport getSnapshotDiffReport(String snapshotRoot, String fromSnapshot, String toSnapshot)
			throws IOException;
	 ...
	 * 
	 * @param id
	 *            of a CacheDirectiveInfo
	 * @throws IOException
	 *             if the cache directive could not be removed
	 */
	@AtMostOnce
	public void removeCacheDirective(long id) throws IOException;

	/**
	 * List the set of cached paths of a cache pool. Incrementally fetches
	 ...
	 *            listCacheDirectives.
	 * @param filter
	 *            Parameters to use to filter the list results, or null to
	 *            display all directives visible to us.
	 * @return A batch of CacheDirectiveEntry objects.
	 */
	@Idempotent
	public BatchedEntries<CacheDirectiveEntry> listCacheDirectives(long prevId, CacheDirectiveInfo filter)
			throws IOException;
	 ...
	 * 
	 * @param info
	 *            Description of the new cache pool
	 * @throws IOException
	...
	/**
	 * List the set of cache pools. Incrementally fetches results from the
	 * server.
	 * 
	 * @param prevPool
	...
	/**
	 * Set xattr of a file or directory. The name must be prefixed with the
	...
	/**
	 * Get xattrs of a file or directory. Values in xAttrs parameter are
	 * ignored. If xAttrs is null or empty, this is the same as getting all
	 * xattrs of the file or directory. Only those xattrs for which the
	 * logged-in user has permissions to view are returned.
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 * @param src
	...
	/**
	 * List the xattrs names for a file or directory. Only the xattr names for
	 * which the logged in user has the permissions to access will be returned.
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 * @param src
	 ...
	 * The name must be prefixed with the namespace followed by ".". For
	 * example, "user.attr".
	 * <p/>
	 * Refer to the HDFS extended attributes user documentation for details.
	 *
	 ...
	 * {@link AccessControlException}. In general, applications should avoid
	 * using this method, due to the risk of time-of-check/time-of-use race
	 * conditions. The permissions on a file may change immediately after the
	 * access call returns.
	 *
	 * @param path
	 *            Path to check
	...
	/**
	 * Get the highest txid the NameNode knows has been written to the edit log,
	 * or -1 if the NameNode's edit log is not yet open for write. Used as the
	 * starting point for the inotify event stream.
	 */
	@Idempotent
