...
public interface ClientDatanodeProtocol {
	/**
	 * Until version 9, this class ClientDatanodeProtocol served as both the
	 * client interface to the DN AND the RPC protocol used to communicate with
	 * the NN.
	 * 
	 * This class is used by both the DFSClient and the DN server side to
	 ...
	 * 
	 * The log of historical changes can be retrieved from the svn). 9: Added
	 * deleteBlockPool method
	 * 
	...
	/**
	 * Refresh the list of federated namenodes from updated configuration Adds
	 * new namenodes and stops the deleted namenodes.
	 * 
	 * @throws IOException
	 *             on error
	 **/
	...
	/**
	 * Retrieves the path names of the block file and metadata file stored on
	 * the local file system.
	 * 
	 * In order for this method to work, one of the following should be
	 * satisfied:
	 * <ul>
	 * <li>The client user must be configured at the datanode to be able to use
	 * this method.</li>
	 * <li>When security is enabled, kerberos authentication must be used to
	 * connect to the datanode.</li>
	 * </ul>
	 * 
	 * @param block
	 ...
	 * datanode. This is in the form of an opaque
	 * {@link org.apache.hadoop.fs.VolumeId} for each configured data directory,
	 * which is not guaranteed to be the same across DN restarts.
	 * 
	 * @param blockPoolId
	 *            the pool to query
	 * @param blockIds
	 ...
	 *         data directories
	 * @throws IOException
	 *             if datanode is unreachable, or replica is not found on
	 *             datanode
	 */
	HdfsBlocksMetadata getHdfsBlocksMetadata(String blockPoolId, long[] blockIds,
	 ...
	 *
	 * @param forUpgrade
	 *            If true, data node does extra prep work before shutting down.
	 *            The work includes advising clients to wait and saving certain
	 *            states for quick restart. This should only be used when the
	 *            stored data will remain the same during upgrade/restart.
	 * @throws IOException
	 */
