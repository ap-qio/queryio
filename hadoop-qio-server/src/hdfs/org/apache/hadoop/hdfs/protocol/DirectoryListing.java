...
/**
 * This class defines a partial listing of a directory to support iterative
 * directory listing.
 */
...
public class DirectoryListing {
	...
	public DirectoryListing(HdfsFileStatus[] partialListing, int remainingEntries) {
		...
		if (partialListing.length == 0 && remainingEntries != 0) {
			throw new IllegalArgumentException(
					"Partial listing is empty but " + "the number of remaining entries is not zero");
		}
	 ...
	 * 
	 * @return true if there are more entries that are left to be listed; return
	 *         false otherwise.
	 */
	 ...
	 * 
	 * @return the last name in the list if it is not empty; otherwise return
	 *         null
	 */
