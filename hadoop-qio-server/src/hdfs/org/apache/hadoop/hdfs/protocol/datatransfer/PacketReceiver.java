...
/**
 * Class to handle reading packets one-at-a-time from the wire. These packets
 * are used both for reading and writing data to/from DataNodes.
 */
@InterfaceAudience.Private
public class PacketReceiver implements Closeable {

	/**
	 * The max size of any single packet. This prevents OOMEs when invalid data
	 * is sent.
	 */
	...
	/**
	 * The entirety of the most recently read packet. The first PKT_LENGTHS_LEN
	 * bytes of this buffer are the length prefixes.
	 */
	 ...
	 * user data and corresponding checksums. The header will be parsed and set.
	 */
	...
	private void doRead(ReadableByteChannel ch, InputStream in) throws IOException {
		...
		if (checksumLen < 0) {
			throw new IOException("Invalid packet: data length in packet header "
					+ "exceeds data length received. dataPlusChecksumLen=" + dataPlusChecksumLen + " header: "
					+ curHeader);
		}
	...
	private void reslicePacket(int headerLen, int checksumsLen, int dataLen) {
		...
		assert curPacketBuf.limit() == lenThroughData : "headerLen= " + headerLen + " clen=" + checksumsLen + " dlen="
				+ dataLen + " rem=" + curPacketBuf.remaining();
