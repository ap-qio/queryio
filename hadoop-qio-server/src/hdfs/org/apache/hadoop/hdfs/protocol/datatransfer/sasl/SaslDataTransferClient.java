...
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.NAME_DELIMITER;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.SASL_TRANSFER_MAGIC_NUMBER;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.checkSaslComplete;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.createSaslPropertiesForEncryption;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.createStreamPair;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.encryptionKeyToPassword;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.getPeerAddress;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.readSaslMessage;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.readSaslMessageAndNegotiatedCipherOption;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.requestedQopContainsPrivacy;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.sendGenericSaslErrorMessage;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.sendSaslMessage;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.sendSaslMessageAndNegotiationCipherOptions;
import static org.apache.hadoop.hdfs.protocol.datatransfer.sasl.DataTransferSaslUtil.unwrap;
...
/**
 * Negotiates SASL for DataTransferProtocol on behalf of a client. There are two
 * possible supported variants of SASL negotiation: either a general-purpose
...
public class SaslDataTransferClient {
	 ...
	 * cluster, we can assume that all run under the same security
	 * configuration.
	 *
	 * @param conf
	 *            the configuration
	 * @param saslPropsResolver
	 *            for determining properties of SASL negotiation
	 * @param trustedChannelResolver
	 *            for identifying trusted connections that do not require SASL
	 *            negotiation
	 */
	 ...
	 *            checked on each attempt at general SASL handshake, if true
	 *            forces use of simple auth
	 */
	public SaslDataTransferClient(Configuration conf, SaslPropertiesResolver saslPropsResolver,
	...
	public Peer peerSend(Peer peer, DataEncryptionKeyFactory encryptionKeyFactory,
			Token<BlockTokenIdentifier> accessToken, DatanodeID datanodeId) throws IOException {
		IOStreamPair ios = checkTrustAndSend(getPeerAddress(peer), peer.getOutputStream(), peer.getInputStream(),
	...
	public IOStreamPair socketSend(Socket socket, OutputStream underlyingOut, InputStream underlyingIn,
			DataEncryptionKeyFactory encryptionKeyFactory, Token<BlockTokenIdentifier> accessToken,
			DatanodeID datanodeId) throws IOException {
		IOStreamPair ios = checkTrustAndSend(socket.getInetAddress(), underlyingOut, underlyingIn, encryptionKeyFactory,
	 ...
	 *             for any error
	 */
	private IOStreamPair checkTrustAndSend(InetAddress addr, OutputStream underlyingOut, InputStream underlyingIn,
			DataEncryptionKeyFactory encryptionKeyFactory, Token<BlockTokenIdentifier> accessToken,
			DatanodeID datanodeId) throws IOException {
		if (!trustedChannelResolver.isTrusted() && !trustedChannelResolver.isTrusted(addr)) {
			...
			LOG.debug("SASL client skipping handshake on trusted connection for addr = {}, " + "datanodeId = {}", addr,
					datanodeId);
			return null;
		}
	}
	 ...
	 *            connection input stream
	 * @param encryptionKey
	 *            for an encrypted SASL handshake
	 * @param accessToken
	 *            connection block access token
	 * @param datanodeId
	 *            ID of destination DataNode
	 ...
	 *             for any error
	 */
	private IOStreamPair send(InetAddress addr, OutputStream underlyingOut, InputStream underlyingIn,
			DataEncryptionKey encryptionKey, Token<BlockTokenIdentifier> accessToken, DatanodeID datanodeId)
			throws IOException {
		if (encryptionKey != null) {
			...
			// It's a secured cluster using non-privileged ports, but no SASL.
			// The
			// only way this can happen is if the DataNode has
			// ignore.secure.ports.for.testing configured, so this is a rare
			// edge case.
	 ...
	 *             for any error
	 */
	private IOStreamPair getEncryptedStreams(OutputStream underlyingOut, InputStream underlyingIn,
	 ...
	 * third encoded using Base64. The fields are each separated by a single
	 * space.
	 * 
	 * @param encryptionKey
	 ...
	 *             for any error
	 */
	private IOStreamPair getSaslStreams(InetAddress addr, OutputStream underlyingOut, InputStream underlyingIn,
			Token<BlockTokenIdentifier> accessToken, DatanodeID datanodeId) throws IOException {
	...
	/**
	 * Builds the client's user name for the general-purpose handshake,
	 * consisting of the base64-encoded serialized block access token
	 * identifier. Note that this includes only the token identifier, not the
	 * token itself, which would include the password. The password is a shared
	 * secret, and we must not write it on the network during the SASL
	 ...
	 *            for responding to SASL callbacks
	 * @return new pair of streams, wrapped after SASL negotiation
	 * @throws IOException
	 *             for any error
	 */
	private IOStreamPair doSaslHandshake(OutputStream underlyingOut, InputStream underlyingIn, String userName,
		...
		try {
			...
			if (requestedQopContainsPrivacy(saslProps)) {
				...
				// supported
				// cipher suite is AES/CTR/NoPadding, but the protocol allows
				// multiple
