...
public class PipelineAck {
	...
	public PipelineAck(long seqno, int[] replies, long downstreamAckTimeNanos) {
		...
		proto = PipelineAckProto.newBuilder().setSeqno(seqno).addAllReply(statusList).addAllFlag(flagList)
	 ...
	 * 
	 * @return time elapsed for downstream ack in nanoseconds, 0 if no next DN
	 *         in pipeline
	 */
