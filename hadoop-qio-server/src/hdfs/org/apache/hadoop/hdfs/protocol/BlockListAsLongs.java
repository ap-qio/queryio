...
public abstract class BlockListAsLongs implements Iterable<BlockReportReplica> {
	 ...
	 * 
	 * @param numBlocks
	 *            - blocks in the buffers
	 * @param blocksBufs
	 *            - list of ByteString encoded varints
	 * @return BlockListAsLongs
	 */
	...
	/**
	 * Prepare an instance to in-place decode the given list of Longs. Note it's
	 * much more efficient to decode ByteString buffers and only exists for
	 * compatibility.
	 * 
	 * @param blocksList
	 *            - list of longs
	 ...
	 * 
	 * The structure of the buffer is as follows: - each replica is represented
	 * by 4 longs: blockId, block length, genstamp, replica state
	 *
	...
	/**
	 * Convert block report to old-style list of longs. Only used to re-encode
	 * the block report when the DN detects an older NN. This is inefficient,
	 * but in practice a DN is unlikely to be upgraded first
	 * 
	 * The structure of the array is as follows: 0: the length of the finalized
	 * replica list; 1: the length of the under-construction replica list; -
	 * followed by finalized replica list where each replica is represented by 3
	 * longs: one for the blockId, one for the block length, and one for the
	 * generation stamp; - followed by the invalid replica represented with
	 * three -1s; - followed by the under-construction replica list where each
	 * replica is represented by 4 longs: three for the block id, length,
	 * generation stamp, and the fourth for the replica state.
	 * 
	...
	/**
	 * Returns a singleton iterator over blocks in the block report. Do not add
	 * the returned blocks to a collection.
	 * 
	 * @return Iterator
	 */
	...
	private static class BufferDecoder extends BlockListAsLongs {
		...
		public long[] getBlockListAsLongs() {
			...
			// an undecoded buffer into longs - ie. it will never happen but
			// let's
			// handle it anyway
			if (numFinalized == -1) {
				int n = 0;
		...
		public Iterator<BlockReportReplica> iterator() {
			return new Iterator<BlockReportReplica>() {
				...
				public BlockReportReplica next() {
					currentBlockIndex++;
					try {
						// zig-zag to reduce size of legacy blocks and mask off
						// bits
			...
			return new Iterator<BlockReportReplica>() {
				...
				public BlockReportReplica next() {
					if (currentBlockIndex == finalizedBlocks) {
						...
						readBlock();
						Preconditions.checkArgument(block.getBlockId() == -1 && block.getNumBytes() == -1
								&& block.getGenerationStamp() == -1, "Invalid delimiter block");
					}
