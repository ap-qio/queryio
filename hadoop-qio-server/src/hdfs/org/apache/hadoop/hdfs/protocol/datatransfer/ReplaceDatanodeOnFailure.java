...
public class ReplaceDatanodeOnFailure {
	...
	private static interface Condition {
		...
		static final Condition TRUE = new Condition() {
			@Override
			public boolean satisfy(short replication, DatanodeInfo[] existings, int nExistings, boolean isAppend,
					boolean isHflushed) {
				return true;
			}
		};
		...
		/**
		 * DEFAULT condition: Let r be the replication number. Let n be the
		 * number of existing datanodes. Add a new datanode only if r >= 3 and
		 * either (1) floor(r/2) >= n; or (2) r > n and the block is
		 * hflushed/appended.
		 */
		static final Condition DEFAULT = new Condition() {
			@Override
			public boolean satisfy(final short replication, final DatanodeInfo[] existings, final int n,
	...
	public void checkEnabled() {
		if (policy == Policy.DISABLE) {
			throw new UnsupportedOperationException("This feature is disabled.  Please refer to "
	 ...
	 * 
	 * @return Suppose the datanode replacement fails. false: An exception
	 *         should be thrown so that the write will fail. true : The write
	 *         should be resumed with the remaining datandoes.
	 */
	...
	public boolean satisfy(final short replication, final DatanodeInfo[] existings, final boolean isAppend,
	...
	private static Policy getPolicy(final Configuration conf) {
		final boolean enabled = conf.getBoolean(DFSConfigKeys.DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_KEY,
				DFSConfigKeys.DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_ENABLE_DEFAULT);
		if (!enabled) {
			return Policy.DISABLE;
		}

		final String policy = conf.get(DFSConfigKeys.DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_POLICY_KEY,
		...
		throw new HadoopIllegalArgumentException("Illegal configuration value for "
				+ DFSConfigKeys.DFS_CLIENT_WRITE_REPLACE_DATANODE_ON_FAILURE_POLICY_KEY + ": " + policy);
	}
