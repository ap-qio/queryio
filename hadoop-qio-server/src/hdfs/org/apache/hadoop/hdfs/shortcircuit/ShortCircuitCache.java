...
/**
 * The ShortCircuitCache tracks things which the client needs to access HDFS
 * block files via short-circuit.
 *
...
public class ShortCircuitCache implements Closeable {
	...
	/**
	 * Expiry thread which makes sure that the file descriptors get closed after
	 * a while.
	 */
	private class CacheCleaner implements Runnable, Closeable {
		 ...
		 * when another thread requests a ShortCircuitReplica object for the
		 * same block. So in that sense, there is no maximum size to the cache.
		 *
		 ...
		 * files kick the mmaped files out of the cache too quickly. Reusing an
		 * already-existing mmap gives a huge performance boost, since the page
		 * table entries don't have to be re-populated. Both the mmap and
		 * non-mmap evictable lists have maximum sizes and maximum lifespans.
		 */
		@Override
		public void run() {
			...
			try {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug(this + ": finishing cache cleaner run started at " + curMs + ".  Demoted " + numDemoted
							+ " mmapped replicas; " + "purged " + numPurged + " replicas.");
				}
			} finally {
	 ...
	 * changes to the mlock status of the replica associated with the slot. It
	 * will also allow us (the client) to re-use this slot for another replica.
	 * If we can't communicate with the DataNode for some reason, we tear down
	 * the shared memory segment to avoid being in an inconsistent state.
	 */
	private class SlotReleaser implements Runnable {
		...
		public void run() {
			...
			try {
				...
				LOG.error(ShortCircuitCache.this + ": failed to release " + "short-circuit shared memory slot " + slot
						+ " by sending " + "ReleaseShortCircuitAccessRequestProto to " + path
	...
	/**
	 * Maximum total size of the cache, including both mmapped and no$-mmapped
	 * elements.
	 */
	...
	/**
	 * The minimum number of milliseconds we'll wait after an unsuccessful mmap
	 * attempt before trying again.
	 */
	private final long mmapRetryTimeoutMs;

	/**
	 * How long we will keep replicas in the cache before declaring them to be
	 * stale.
	 */
	...
	public static ShortCircuitCache fromConf(Configuration conf) {
		return new ShortCircuitCache(
				...
				conf.getInt(DFSConfigKeys.DFS_CLIENT_MMAP_CACHE_SIZE, DFSConfigKeys.DFS_CLIENT_MMAP_CACHE_SIZE_DEFAULT),
	 ...
	 *
	 * @param replica
	 *            The replica we're removing.
	 */
	private void ref(ShortCircuitReplica replica) {
		lock.lock();
		try {
			Preconditions.checkArgument(replica.refCount > 0, "can't ref %s because its refCount reached %d", replica,
					replica.refCount);
	...
	void unref(ShortCircuitReplica replica) {
		lock.lock();
		try {
			// If the replica is stale or unusable, but we haven't purged it
			// yet,
			// let's do that. It would be a shame to evict a non-stale replica
			// so
			...
			if (newRefCount == 0) {
				...
				Preconditions.checkState(null == replica.getEvictableTimeNs(),
						"Replica %s had a refCount higher than 1, " + "but was still evictable (evictableTimeNs = %d)",
				...
				Preconditions.checkArgument(replica.refCount >= 0,
						"replica's refCount went negative (refCount = %d" + " for %s)", replica.refCount, replica);
			}
			if (LOG.isTraceEnabled()) {
				LOG.trace(this + ": unref replica " + replica + ": " + addedString + " refCount " + (newRefCount + 1)
	 ...
	 *
	 * @param evictionTimeNs
	 *            The eviction time in absolute nanoseconds.
	 * @param replica
	 *            The replica to insert.
	 * @param map
	 *            The map to insert it into.
	 */
	 ...
	 * outstanding references to it. However, it does mean the cache won't hand
	 * it out to anyone after this.
	 *
	 * You must hold the cache lock while calling this function.
	 *
	 * @param replica
	 ...
	 * @param creator
	 *            Replica creator callback. Will be called without the cache
	 *            lock being held.
	 *
	 * @return Null if no replica could be found or created. The replica,
	 *         otherwise.
	 */
	 ...
	 * @param waitable
	 *            The waitable object to wait on.
	 * @return The existing ReplicaInfo object, or null if there is none.
	 *
	 * @throws RetriableException
	 *             If the caller needs to retry.
	 */
	...
	public void accept(CacheVisitor visitor) {
		...
		try {
			...
			for (Entry<ExtendedBlockId, Waitable<ShortCircuitReplicaInfo>> entry : replicaInfoMap.entrySet()) {
				...
				if (waitable.hasVal()) {
					if (waitable.getVal().getReplica() != null) {
						...
						// The exception may be null here, indicating a failed
						// load that
			...
			if (LOG.isDebugEnabled()) {
				StringBuilder builder = new StringBuilder();
				builder.append("visiting ").append(visitor.getClass().getName()).append("with outstandingMmapCount=")
	 ...
	 *
	 * @param datanode
	 *            The datanode to allocate a shm slot with.
	 * @param peer
	 *            A peer connected to the datanode.
	 * @param usedPeer
	 *            Will be set to true if we use up the provided peer.
	 * @param blockId
	 *            The block id and block pool id of the block we're allocating
	 *            this slot for.
	 * @param clientName
	 *            The name of the DFSClient allocating the shared memory.
	 * @return Null if short-circuit shared memory is disabled; a short-circuit
	 *         memory slot otherwise.
	 * @throws IOException
	 *             An exception if there was an error talking to the datanode.
	 */
	public Slot allocShmSlot(DatanodeInfo datanode, DomainPeer peer, MutableBoolean usedPeer, ExtendedBlockId blockId,
