 ...
 * When the UNIX domain socket associated with this shared memory segment closes
 * unexpectedly, we mark the slots inside this segment as disconnected.
 * ShortCircuitReplica objects that contain disconnected slots are stale, and
 * will not be used to service new reads or mmap operations. However,
 * in-progress read or mmap operations will continue to proceed. Once the last
 * slot is deallocated, the segment can be safely munmapped.
 *
 * Slots may also become stale because the associated replica has been deleted
 * on the DataNode. In this case, the DataNode will clear the 'valid' bit. The
 * client will then see these slots as stale (see
...
public class DfsClientShm extends ShortCircuitShm implements DomainSocketWatcher.Handler {
	...
	/**
	 * The UNIX domain socket associated with this DfsClientShm. We rely on the
	 * DomainSocketWatcher to close the socket associated with this DomainPeer
	 * when necessary.
	 */
