...
public class DfsClientShmManager implements Closeable {
	...
	class EndpointShmManager {
		 ...
		 *
		 * @param blockId
		 *            The blockId to put inside the Slot object.
		 *
		 * @return null if none of our shared memory segments contain a free
		 *         slot; the slot object otherwise.
		 */
		...
		/**
		 * Ask the DataNode for a new shared memory segment. This function must
		 * be called with the manager lock held. We will release the lock while
		 * communicating with the DataNode.
		 *
		 * @param clientName
		 *            The current client name.
		 * @param peer
		 *            The peer to use to talk to the DataNode.
		 *
		 * @return Null if the DataNode does not support shared memory segments,
		 *         or experienced an error creating the shm. The shared memory
		 *         segment itself on success.
		 * @throws IOException
		 *             If there was an error communicating over the socket. We
		 *             will not throw an IOException unless the socket itself
		 *             (or the network) is the problem.
		 */
		private DfsClientShm requestNewShm(String clientName, DomainPeer peer) throws IOException {
			...
			ShortCircuitShmResponseProto resp = ShortCircuitShmResponseProto
			...
			case ERROR_UNSUPPORTED:
				// The DataNode just does not support short-circuit shared
				// memory
				...
				return null;
			default:
				// The datanode experienced some kind of unexpected error when
				// trying to
		 ...
		 *            The peer to use to talk to the DataNode.
		 * @param usedPeer
		 *            (out param) Will be set to true if we used the peer. When
		 *            a peer is used
		 *
		 * @param clientName
		 *            The client name.
		 ...
		 */
		Slot allocSlot(DomainPeer peer, MutableBoolean usedPeer, String clientName, ExtendedBlockId blockId)
				throws IOException {
			while (true) {
				if (closed) {
					if (LOG.isTraceEnabled()) {
				...
				// There are no free slots. If someone is loading more slots,
				// wait
				// for that to finish.
				if (loading) {
					if (LOG.isTraceEnabled()) {
					...
					try {
						...
						if (shm == null)
							continue;
						// See #{DfsClientShmManager#domainSocketWatcher} for
						// details
						// about why we do this before retaking the manager
						// lock.
						domainSocketWatcher.add(peer.getDomainSocket(), shm);
						// The DomainPeer is now our responsibility, and should
						// not be
					...
					if (shm.isDisconnected()) {
						// If the peer closed immediately after the shared
						// memory segment
						// was created, the DomainSocketWatcher callback might
						// already have
						// fired and marked the shm as disconnected. In this
						// case, we
						// obviously don't want to add the SharedMemorySegment
						// to our list
		...
		void freeSlot(Slot slot) {
			...
			if (shm.isDisconnected()) {
				...
				ShmId shmId = shm.getShmId();
				full.remove(shmId); // The shm can't be full if we just freed a
									// slot.
				if (shm.isEmpty()) {
					notFull.remove(shmId);

					// If the shared memory segment is now empty, we call
					// shutdown(2) on
					// the UNIX domain socket associated with it. The
					// DomainSocketWatcher,
					// which is watching this socket, will call
					// DfsClientShm#handle,
					// cleaning up this shared memory segment.
					//
					// See #{DfsClientShmManager#domainSocketWatcher} for
					// details about why
					// we don't want to call DomainSocketWatcher#remove directly
					// here.
					//
					// Note that we could experience 'fragmentation' here, where
					// the
					// DFSClient allocates a bunch of slots in different shared
					// memory
					// segments, and then frees most of them, but never fully
					// empties out
					// any segment. We make some attempt to avoid this
					// fragmentation by
					// always allocating new slots out of the shared memory
					// segment with the
					// lowest ID, but it could still occur. In most workloads,
					// fragmentation should not be a major concern, since it
					// doesn't impact
		 ...
		 * inside that segment.
		 *
		 * The DomainSocketWatcher calls this while holding the
		 * DomainSocketWatcher lock.
		 *
		 * @param shmId
		 *            The ID of the shared memory segment to unregister.
		 */
	 ...
	 */
	private final HashMap<DatanodeInfo, EndpointShmManager> datanodes = new HashMap<DatanodeInfo, EndpointShmManager>(
			1);
	 ...
	 * DomainSocketWatcher lock while holding the DfsClientShmManager lock, or
	 * else deadlock might result. This means that most DomainSocketWatcher
	...
	public Slot allocSlot(DatanodeInfo datanode, DomainPeer peer, MutableBoolean usedPeer, ExtendedBlockId blockId,
	...
	public static class PerDatanodeVisitorInfo {
		...
		PerDatanodeVisitorInfo(TreeMap<ShmId, DfsClientShm> full, TreeMap<ShmId, DfsClientShm> notFull,
				boolean disabled) {
			this.full = full;
