...
/**
 * A ShortCircuitReplica object contains file descriptors for a block that we
 * are reading via short-circuit local reads.
 *
 * The file descriptors can be shared between multiple threads because all the
 * operations we perform are stateless-- i.e., we use pread instead of read, to
 * avoid using the shared position state.
 */
...
public class ShortCircuitReplica {
	...
	/**
	 * Number of external references to this replica. Replicas are referenced by
	 * the cache, BlockReaderLocal instances, and by ClientMmap instances. The
	 * number starts at 2 because when we create a replica, it is referenced by
	 * both the cache and the requester.
	 *
	 * Protected by the cache lock.
	 */
	int refCount = 2;
	...
	/**
	 * The monotonic time in nanoseconds at which the replica became evictable,
	 * or null if it is not evictable.
	 *
	 * Protected by the cache lock.
	 */
	 ...
	 * Datanode. The DataNode will not munlock the block until the number of
	 * no-checksum anchors for the block reaches zero.
	 * 
	 ...
	 * Note: you must hold the cache lock to call this function.
	 *
	 * @param evictableTimeNs
	 *            The evictable time in nanoseconds, or null to set no evictable
	 *            time.
	 */
	...
	/**
	 * Convert the replica to a string for debugging purposes. Note that we
	 * can't take the lock here.
	 */
	@Override
