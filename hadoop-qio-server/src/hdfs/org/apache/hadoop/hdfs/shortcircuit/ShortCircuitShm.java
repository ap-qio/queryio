...
public class ShortCircuitShm {
	...
	/**
	 * Calculate the usable size of a shared memory segment. We round down to a
	 * multiple of the slot size and do some validation.
	 *
	 * @param stream
	 ...
	 *
	 * The format is: word 0 bit 0:32 Slot flags (see below). bit 33:63 Anchor
	 * count. word 1:7 Reserved for future use, such as statistics. Padding is
	 * also useful for avoiding false sharing.
	 *
	 * Little-endian versus big-endian is not relevant here since both the
	 * client and the server reside on the same computer and use the same
	 * orientation.
	 */
	public class Slot {
		/**
		 ...
		 * 
		 * The DFSClient sets this flag when it allocates a new slot within one
		 * of its shared memory regions.
		 * 
		 * The DataNode clears this flag when the replica associated with this
		 * slot is no longer valid. The client itself also clears this flag when
		 * it believes that the DataNode is no longer using this slot to
		 * communicate.
		 */
		 ...
		 *
		 * When a slot is anchored, we know that the block it refers to is
		 * resident in memory.
		 *
		...
		public boolean addAnchor() {
			...
			do {
				...
				if ((prev & 0x7fffffff) == 0x7fffffff) {
					// Too many other threads have anchored the slot (2
					// billion?)
					return false;
				}
	...
	/**
	 * The slots associated with this shared memory segment. slot[i] contains
	 * the slot at offset i * BYTES_PER_SLOT, or null if that slot is not
	 * allocated.
	 */
	 ...
	 *            The ID to use.
	 * @param stream
	 *            The stream that we're going to use to create this shared
	 *            memory segment.
	 * 
	 *            Although this is a FileInputStream, we are going to assume
	 *            that the underlying file descriptor is writable as well as
	 *            readable. It would be more appropriate to use a
	 *            RandomAccessFile here, but that class does not have any public
	 *            accessor which returns a FileDescriptor, unlike
	 *            FileInputStream.
	 */
	 ...
	 *
	 * This function chooses an empty slot, initializes it, and then returns the
	 ...
	 * another process), and registers it with us. Then, it returns the relevant
	 * Slot object.
	 *
	 * @return The slot.
	 *
