...
public class JournalProtocolServerSideTranslatorPB implements JournalProtocolPB {
	...
	public JournalResponseProto journal(RpcController unused, JournalRequestProto req) throws ServiceException {
		try {
			impl.journal(PBHelper.convert(req.getJournalInfo()), req.getEpoch(), req.getFirstTxnId(), req.getNumTxns(),
	...
	public FenceResponseProto fence(RpcController controller, FenceRequestProto req) throws ServiceException {
		try {
			...
			return FenceResponseProto.newBuilder().setInSync(resp.isInSync())
					.setLastTransactionId(resp.getLastTransactionId()).setPreviousEpoch(resp.getPreviousEpoch())
					.build();
		} catch (IOException e) {
			throw new ServiceException(e);
		}
