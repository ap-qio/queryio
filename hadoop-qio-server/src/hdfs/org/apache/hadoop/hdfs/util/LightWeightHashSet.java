...
public class LightWeightHashSet<T> implements Collection<T> {
	...
	public LightWeightHashSet(int initCapacity, float maxLoadFactor, float minLoadFactor) {
		...
		LOG.debug("initial capacity=" + initialCapacity + ", max load factor= " + maxLoadFactor + ", min load factor= "
				+ minLoadFactor);
	}
	...
	/**
	 * Return the element in this set which is equal to the given key, if such
	 * an element exists. Otherwise returns null.
	 */
	...
	/**
	 * Check if the set contains given element at given index. If it does,
	 * return that element.
	 *
	...
	/**
	 * Remove and return n elements from the hashtable. The order in which
	 * entries are removed is unspecified, and and may not correspond to the
	...
	public String toString() {
		...
		b.append("(size=").append(size).append(", modification=").append(modification).append(", entries.length=")
