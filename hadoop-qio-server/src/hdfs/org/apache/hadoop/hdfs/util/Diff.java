 ...
 *            The key type.
 * @param <E>
 *            The element type, which must implement {@link Element} interface.
 */
public class Diff<K, E extends Diff.Element<K>> {
	...
	/**
	 * Undo information for some operations such as delete(E) and
	 * {@link Diff#modify(Element, Element)}.
	 */
	 ...
	 *         defined in {@link Collections#binarySearch(List, Object)}. Note
	 *         that, when the list is null, -1 is the correct insertion point.
	 */
	...
	/**
	 * @return null if the element is not found; otherwise, return the element
	 *         in the created/deleted list.
	 */
	 ...
	 * 
	 * @param i
	 *            the insertion point defined in
	...
	public UndoInfo<E> modify(final E oldElement, final E newElement) {
		...
		Preconditions.checkArgument(oldElement.compareTo(newElement.getKey()) == 0,
				"The names do not match: oldElement=%s, newElement=%s", oldElement, newElement);
	...
	/**
	 * Undo the previous modify(E, E) operation. Note that the behavior is
	 * undefined if the previous operation is not modify(E, E).
	 */
	 ...
	 *         current state; otherwise, return a {@link Container} containing
	 *         the element in the previous state. Note that the element can
	 *         possibly be null which means that the element is not found in the
	 *         previous state.
	 */
	...
	private static <K, E extends Diff.Element<K>> Container<E> accessPrevious(final K name, final List<E> clist,
			final List<E> dlist) {
	 ...
	 * 
	 * @return null if the element cannot be determined in the current state
	 *         since no change is recorded and it should be determined in the
	 *         previous state; otherwise, return a {@link Container} containing
	 *         the element in the current state. Note that the element can
	 *         possibly be null which means that the element is not found in the
	 *         current state.
	 */
	...
	private static <K, E extends Diff.Element<K>> List<E> apply2Previous(final List<E> previous, final List<E> clist,
			final List<E> dlist) {
		// Assumptions:
		...
		final List<E> tmp = new ArrayList<E>(previous.size() - dlist.size());
		{
			...
			for (E deleted : dlist) {
				E e = i.next(); // since dlist is non-empty, e must exist by
								// (A2).
				int cmp = 0;
	...
	/**
	 * Apply the reverse of this diff to current state in order to obtain the
	 * previous state.
	 * 
	 ...
	 * 
	 * @param posterior
	 *            The posterior diff to combine with.
	 * @param deletedProcesser
	 *            process the deleted/overwritten elements in case 2.1, 2.3, 3.1
	 *            and 3.3.
	 */
	...
	public String toString() {
		return getClass().getSimpleName() + "{created=" + getList(ListType.CREATED) + ", deleted="
