...
public abstract class ByteArrayManager {
	...
	/**
	 * A counter with a time stamp so that it is reset automatically if there is
	 * no increment for the time period.
	 */
	static class Counter {
		...
		/**
		 * Increment the counter, and reset it if there is no increment for
	...
	static class CounterMap {
		...
		/**
		 * @return the counter for the given key; and create a new counter if it
		 *         does not exist.
		 */
	...
	static class FixedLengthManager {
		 ...
		 * blocked until the number of allocated arrays drops to below the
		 * maximum.
		 * 
		 * The byte array allocated by this method must be returned for
		 * recycling via the {@link FixedLengthManager#recycle(byte[])} method.
		 */
		 ...
		 *
		 * The byte array may or may not be allocated by the
		 * {@link FixedLengthManager#allocate()} method.
		 */
	...
	public static class Conf {
		/**
		 * The count threshold for each array length so that a manager is
		 * created only after the allocation count exceeds the threshold.
		 */
		 ...
		 * array length is reset to zero if there is no increment.
		 */
		private final long countResetTimePeriodMs;
	...
	/**
	 * Create a byte array for the given length, where the length of the
	 * returned array is larger than or equal to the given length.
	 *
	 * The current thread may be blocked if some resource is unavailable.
	 * 
	 * The byte array created by this method must be released via the
	 * {@link ByteArrayManager#release(byte[])} method.
	 *
	 ...
	 * 
	 * The byte array may or may not be created by the
	 * {@link ByteArrayManager#newByteArray(int)} method.
	 * 
	...
	static class Impl extends ByteArrayManager {
		...
		/**
		 * Allocate a byte array, where the length of the allocated array is the
		 * least power of two of the given length unless the given length is
		 * less than {@link #MIN_ARRAY_LENGTH}. In such case, the returned array
		 * length is equal to {@link #MIN_ARRAY_LENGTH}.
		 *
		 * If the number of allocated arrays exceeds the capacity, the current
		 * thread is blocked until the number of allocated arrays drops to below
		 * the capacity.
		 * 
		 * The byte array allocated by this method must be returned for
		 * recycling via the {@link Impl#release(byte[])} method.
		 *
		 * @return a byte array with length larger than or equal to the given
		 *         length.
		 */
		 ...
		 * 
		 * The byte array may or may not be allocated by the
		 * {@link Impl#newByteArray(int)} method.
		 * 
