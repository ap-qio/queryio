...
/**
 * @deprecated this is an old implementation that is being left around in case
 *             any issues spring up with the new {@link RemoteBlockReader2}
 *             implementation. It will be removed in the next release.
 */
...
public class RemoteBlockReader extends FSInputChecker implements BlockReader {
	...
	/**
	 * offset in block of of first chunk - may be less than startOffset if
	 * startOffset is not chunk-aligned
	 */
	...
	/**
	 * The total number of bytes we need to transfer from the DN. This is the
	 * amount that the user has requested plus some padding at the beginning so
	 * that the read can begin on a chunk boundary.
	 */
	...
	public synchronized long skip(long n) throws IOException {
		/*
		 * How can we make sure we don't throw a ChecksumException, at least in
		 * majority of the cases?. This one throws.
		 */
		if (skipBuf == null) {
			skipBuf = new byte[bytesPerChecksum];
		}
	...
	public boolean seekToNewSource(long targetPos) throws IOException {
		/*
		 * Checksum errors are handled outside the BlockReader. DFSInputStream
		 * does not always call 'seekToNewSource'. In the case of pread(), it
		 * just tries a different replica without seeking.
		 */
		return false;
	}
	...
	/**
	 * Makes sure that checksumBytes has enough capacity and limit is set to the
	 * number of checksum bytes needed to be read.
	 */
	...
	private synchronized int readChunkImpl(long pos, byte[] buf, int offset, int len, byte[] checksumBuf)
			throws IOException {
		// Read one chunk.
		if (eos) {
			// Already hit EOF
		...
		if (checksumSize > 0) {
			...
			bytesToRead = Math.min(checksumsToRead * bytesPerChecksum, // full
																		// chunks
	...
	private RemoteBlockReader(String file, String bpid, long blockId, DataInputStream in, DataChecksum checksum,
			boolean verifyChecksum, long startOffset, long firstChunkOffset, long bytesToRead, Peer peer,
			DatanodeID datanodeID, PeerCache peerCache) {
		// Path is used only for printing block and file information in debug
		super(new Path("/" + Block.BLOCK_FILE_PREFIX + blockId + ":" + bpid + ":of:"
				+ file)/* too non path-like? */, 1, verifyChecksum, checksum.getChecksumSize() > 0 ? checksum : null,
	...
	/**
	 * Create a new BlockReader specifically to satisfy a read. This method also
	 * sends the OP_READ_BLOCK request.
	 *
	 * @param file
	 *            File location
	 * @param block
	 ...
	 */
	public static RemoteBlockReader newBlockReader(String file, ExtendedBlock block,
			Token<BlockTokenIdentifier> blockToken, long startOffset, long len, int bufferSize, boolean verifyChecksum,
		...
		return new RemoteBlockReader(file, block.getBlockPoolId(), block.getBlockId(), in, checksum, verifyChecksum,
	...
	/**
	 * When the reader reaches end of the read, it sends a status response (e.g.
	 * CHECKSUM_OK) to the DN. Failure to do so could lead to the DN closing our
	 * connection (which we will re-open), but won't affect data correctness.
	 */
	void sendReadResult(Peer peer, Status statusCode) {
		...
		try {
			...
			// It's ok not to be able to send this. But something is probably
			// wrong.
