...
/****************************************************************
 * DFSInputStream provides bytes from a named file. It handles negotiation of
 * the namenode and various datanodes as necessary.
 ****************************************************************/
@InterfaceAudience.Private
public class DFSInputStream extends FSInputStream
		implements ByteBufferReadable, CanSetDropBehind, CanSetReadahead, HasEnhancedByteBufferAccess, CanUnbuffer {
	@VisibleForTesting
	...
	public static class ReadStatistics {
		...
		/**
		 * @return The total bytes read. This will always be at least as high as
		 *         the other numbers, since it includes all of them.
		 */
		...
		/**
		 * @return The total local bytes read. This will always be at least as
		 *         high as totalShortCircuitBytesRead, since all short-circuit
	...
	/**
	 * This variable tracks the number of failures since the start of the most
	 * recent user-facing operation. That is to say, it should be reset whenever
	 * the user makes a call on this stream, and if at any point during the
	 * retry logic, the failure count exceeds a threshold, the errors will be
	 * thrown back to the operation.
	 *
	 * Specifically this counts the number of times the client has gone back to
	 * the namenode to get a new list of block locations, and is capped at
	 * maxBlockAcquireFailures
	 */
	private int failures = 0;

	/*
	 * XXX Use of CocurrentHashMap is temp fix. Need to fix parallel accesses to
	 * DFSInputStream (through ptreads) properly
	 */
	...
	void openInfo() throws IOException, UnresolvedLinkException {
		synchronized (infoLock) {
			...
			while (retriesForLastBlockLength > 0) {
				// Getting last block length as -1 is a special case. When
				// cluster
				// restarts, DNs may not report immediately. At this time
				// partial block
				// locations will not be available with NN for getting the
				// length. Lets
				// retry for 3 times to get the length.
				if (lastBlockBeingWrittenLength == -1) {
					DFSClient.LOG.warn("Last block locations not available. "
							+ "Datanodes might not have reported blocks completely." + " Will retry for "
							+ retriesForLastBlockLength + " times");
			...
			if (last != null) {
				if (last.getLocations().length == 0) {
					if (last.getBlockSize() == 0) {
						// if the length is zero, then no data has been written
						// to
	...
	private long readBlockLength(LocatedBlock locatedblock) throws IOException {
		...
		for (DatanodeInfo datanode : locatedblock.getLocations()) {
			...
			try {
				cdp = DFSUtil.createClientDatanodeProtocolProxy(datanode, dfsClient.getConfiguration(),
						dfsClient.getConf().socketTimeout, dfsClient.getConf().connectToDnViaHostname, locatedblock);
				...
				if (ioe instanceof RemoteException
					...
					// special case : replica might not be on the DN, treat as 0
					// length
		...
		// Namenode told us about these locations, but none know about the
		// replica
	...
	private LocatedBlock getBlockAt(long offset) throws IOException {
		synchronized (infoLock) {
			...
			if (offset < 0 || offset >= getFileLength()) {
				throw new IOException("offset < 0 || offset >= getFileLength(), offset=" + offset + ", locatedBlocks="
						+ locatedBlocks);
	...
	/**
	 * Get blocks in the specified range. Fetch them from the namenode if not
	 * cached. This function will not get a read request beyond the EOF.
	 * 
	 * @param offset
	...
	/**
	 * Get blocks in the specified range. Includes only the complete blocks.
	...
	/**
	 * Open a DataInputStream to a DataNode so that it can be read from. We get
	 * block ID and the IDs of the destinations at startup, from the namenode.
	 */
	private synchronized DatanodeInfo blockSeekTo(long target) throws IOException {
		...
		int refetchEncryptionKey = 1; // only need to get a new encryption key
										// once
	...
	private synchronized int readBuffer(ReaderStrategy reader, int off, int len,
		...
		while (true) {
			...
			try {
				...
				if (!retryCurrentNode) {
					DFSClient.LOG.warn(
							"Exception while reading from " + getCurrentBlock() + " of " + src + " from " + currentNode,
							e);
				}
				ioe = e;
			}
			...
			if (retryCurrentNode) {
				 ...
				 * result in application level failures (e.g. Datanode could
				 * have closed the connection because the client is idle for too
				 * long).
				 */
	...
	private synchronized int readWithStrategy(ReaderStrategy strategy, int off, int len) throws IOException {
		...
		if (pos < getFileLength()) {
			...
			while (retries > 0) {
				try {
					// currentNode can be left as null if previous read had a
					// checksum
					...
					if (result >= 0) {
						...
						// got a EOS from reader though we expect more data on
						// it.
					...
					// Check if need to report block replicas corruption either
					// read
	...
	private DNAddrPair chooseDataNode(LocatedBlock block, Collection<DatanodeInfo> ignoredNodes) throws IOException {
		while (true) {
			try {
				...
				try {
					// Introducing a random factor to the wait time before
					// another retry.
					// The wait time is dependent on # of failures and a random
					// factor.
					// At the first time of getting a BlockMissingException, the
					// wait time
					// is a random number between 0..3000 ms. If the first retry
					// still fails, we will wait 3000 ms grace period before the
					// 2nd retry.
					// Also at the second retry, the waiting window is expanded
					// to 6000 ms
					// alleviating the request rate from the server. Similarly
					// the 3rd retry
					// will wait 6000ms grace period before retry and the
					// waiting window is
					// expanded to 9000ms.
					final int timeWindow = dfsClient.getConf().timeWindow;
					double waitTime = timeWindow * failures + // grace period
																// for the last
																// round of
																// attempt
							timeWindow * (failures + 1) * DFSUtil.getRandom().nextDouble(); // expanding
																							// time
																							// window
																							// for
																							// each
		...
		if (nodes != null) {
			for (int i = 0; i < nodes.length; i++) {
				if (!deadNodes.containsKey(nodes[i]) && (ignoredNodes == null || !ignoredNodes.contains(nodes[i]))) {
					chosenNode = nodes[i];
					// Storage types are ordered to correspond with nodes, so
					// use the same
	...
	private Callable<ByteBuffer> getFromOneDataNode(final DNAddrPair datanode, final LocatedBlock block,
			final long start, final long end, final ByteBuffer bb,
			final Map<ExtendedBlock, Set<DatanodeInfo>> corruptedBlockMap, final int hedgedReadId) {
	...
	private void actualGetFromOneDataNode(final DNAddrPair datanode, LocatedBlock block, final long start,
		...
		while (true) {
			...
			try {
				...
				String msg = "fetchBlockByteRange(). Got a checksum exception for " + src + " at " + block.getBlock()
	...
	/**
	 * Like
	 * {@link #fetchBlockByteRange(LocatedBlock, long, long, byte[], int, Map)}
	 * except we start up a second, parallel, 'hedged' read if the first read is
	 * taking longer than configured amount of time. We then wait on which ever
	 * read returns first.
	 */
	private void hedgedFetchBlockByteRange(LocatedBlock block, long start, long end, byte[] buf, int offset,
		...
		block = getBlockAt(block.getStartOffset());
		while (true) {
			// see HDFS-6591, this metric is used to verify/catch unnecessary
			// loops
			...
			if (futures.isEmpty()) {
				...
				// ongoing. Call getBestNodeDNAddrPair instead of
				// chooseDataNode.
				...
				// if not succeeded. Submit callables for each datanode in a
				// loop, wait
	 ...
	 * @return true if block access token has expired or invalid and it should
	 *         be refetched
	 */
	private static boolean tokenRefetchNeeded(IOException ex, InetSocketAddress targetAddr) {
		 ...
		 * both NN and DN re-started while DFSClient holding a cached access
		 * token. 2) In the case that NN fails to update its access key at
		 * pre-set interval (by a wide margin) and subsequently restarts. In
		 * this case, DN re-registers itself with NN and receives a new access
		 * key, but DN will delete the old access key from its memory since it's
		 * considered expired based on the estimated expiration date.
		 */
	...
	/**
	 * DFSInputStream reports checksum failure. Case I : client has tried
	 * multiple data nodes and at least one of the attempts has succeeded. We
	 * report the other failures as corrupted block to namenode. Case II: client
	 * has tried out all data nodes, but all failed. We only report if the total
	 * number of replica is 1. We do not report otherwise since this maybe due
	 * to the client is a handicapped client (who can not read).
	 * 
			...
			if (diff <= blockReader.available()) {
				try {
					...
					if (pos == targetPos) {
						...
						// The range was already checked. If the block reader
						// returns
						// something unexpected instead of throwing an
						// exception, it is
						// most likely a bug.
	...
	/**
	 * Seek to given position on a node other than the current node. If a node
	 * other than the current node is found, then returns true. If another node
	 * could not be found, then returns false.
	 */
	@Override
	...
	public synchronized ByteBuffer read(ByteBufferPool bufferPool, int maxLength, EnumSet<ReadOption> opts)
		...
		if ((blockReader == null) || (blockEnd == -1)) {
			...
			/*
			 * If we don't have a blockReader, or the one we have has no more
			 * bytes left to read, we call seekToBlockSource to get a new
			 * blockReader and recalculate blockEnd. Note that we assume we're
			 * not at EOF here (we check this above).
			 */
	...
	private synchronized ByteBuffer tryReadZeroCopy(int maxLength, EnumSet<ReadOption> opts) throws IOException {
		// Copy 'pos' and 'blockEnd' to local variables to make it easier for
		// the
			...
			if (length63 <= 0) {
				if (DFSClient.LOG.isDebugEnabled()) {
					DFSClient.LOG.debug("Unable to perform a zero-copy read from offset " + curPos + " of " + src + "; "
							+ length63 + " bytes left in block.  " + "blockPos=" + blockPos + "; curPos=" + curPos
							+ "; curEnd=" + curEnd);
				}
				return null;
			}
		...
		// Make sure that don't go beyond 31-bit offsets in the
		// MappedByteBuffer.
		int length;
		if (blockPos + length63 <= Integer.MAX_VALUE) {
			...
			if (length31 <= 0) {
				...
				// So we can't mmap the parts of the block higher than the 2 GB
				// offset.
			...
			if (DFSClient.LOG.isDebugEnabled()) {
				DFSClient.LOG
						.debug("Reducing read length from " + maxLength + " to " + length + " to avoid 31-bit limit.  "
	...
	public synchronized void releaseBuffer(ByteBuffer buffer) {
		...
		if (val == null) {
			throw new IllegalArgumentException(
					"tried to release a buffer " + "that was not created by this stream, " + buffer);
		}
