...
public class HdfsDataOutputStream extends FSDataOutputStream {
	public HdfsDataOutputStream(DFSOutputStream out, FileSystem.Statistics stats, long startPosition)
			throws IOException {
	 ...
	 * are currently being written to. Depending on the configuration, the
	 * client may continue to write to a block even if a few datanodes in the
	 * write pipeline have failed, or the client may add a new datanodes once a
	 * datanode has failed.
	 * 
	...
	public static enum SyncFlag {
		...
		/**
		 * When doing sync to DataNodes, also update the metadata (block length)
		 * in the NameNode.
		 */
		UPDATE_LENGTH,

		/**
		 * Sync the data to DataNode, close the current block, and allocate a
		 * new block
		 */
		END_BLOCK;
	}
