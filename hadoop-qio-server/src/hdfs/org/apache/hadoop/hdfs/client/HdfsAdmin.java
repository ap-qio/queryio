...
public class HdfsAdmin {
	 ...
	 * 
	 * @param uri
	 *            the unique URI of the HDFS file system to administer
	 * @param conf
	 *            configuration
	 * @throws IOException
	 *             in the event the file system could not be created
	 */
	...
	/**
	 * Set the namespace quota (count of files, directories, and sym links) for
	 * a directory.
	 * 
	 * @param src
	 *            the path to set the quota for
	 * @param quota
	...
	/**
	 * Set the quota by storage type for a directory. Note that directories and
	 * sym links do not occupy storage type quota.
	 *
	 * @param src
	 *            the target directory to set the quota by storage type
	 * @param type
	 *            the storage type to set for quota by storage type
	 * @param quota
	 *            the value to set for quota by storage type
	 * @throws IOException
	 *             in the event of error
	 */
	 ...
	 *
	 * @param src
	 *            the target directory to clear the quota by storage type
	 * @param type
	 *            the storage type to clear for quota by storage type
	 * @throws IOException
	 *             in the event of error
	 */
	...
	/**
	 * Create an encryption zone rooted at an empty existing directory, using
	 * the specified encryption key. An encryption zone has an associated
	 * encryption key used when reading and writing files within the zone.
	 *
	 * @param path
	 *            The path of the root of the encryption zone. Must refer to an
	 *            empty, existing directory.
	 * @param keyName
	 *            Name of key available at the KeyProvider.
	 * @throws IOException
	 *             if there was a general IO exception
	 * @throws AccessControlException
	 *             if the caller does not have access to path
	 ...
	 * <p/>
	 * Since the list is fetched in batches, it does not represent a consistent
	 * snapshot of the entire list of encryption zones.
	 * <p/>
	 ...
	 * stream is created are available. See
	 ...
	 * <p/>
	 * Inotify users may want to tune the following HDFS parameters to ensure
	 * that enough extra HDFS edits are saved to support inotify clients that
	 * fall behind the current state of the namespace while reading events. The
	 * default parameter values should generally be reasonable. If edits are
	 ...
	 * dfs.namenode.num.checkpoints.retained dfs.ha.log-roll.period
	 * <p/>
	 * It is recommended that local journaling be configured
	 * (dfs.namenode.edits.dir) for inotify (in addition to a shared journal) so
	 * that edit transfers from the shared journal can be avoided.
	 *
	 * @throws IOException
	 ...
	 * have access to an FSImage inclusive of lastReadTxid) and only want to
	 * read events after this point.
	 */
