...
public class BlockTokenSecretManager extends SecretManager<BlockTokenIdentifier> {
	 ...
	 * sync'ed their block keys with NN at least once during each interval.
	 */
	 ...
	 */
	public BlockTokenSecretManager(long keyUpdateInterval, long tokenLifetime, String blockPoolId,
	 ...
	 */
	public BlockTokenSecretManager(long keyUpdateInterval, long tokenLifetime, int nnIndex, String blockPoolId,
			String encryptionAlgorithm) {
	...
	private BlockTokenSecretManager(boolean isMaster, long keyUpdateInterval, long tokenLifetime, String blockPoolId,
			String encryptionAlgorithm) {
	...
	private synchronized void generateKeys() {
		if (!isMaster)
			return;
		/*
		 * Need to set estimated expiry dates for currentKey and nextKey so that
		 * if NN crashes, DN can still expire those keys. NN will stop using the
		 * newly generated currentKey after the first keyUpdateInterval, however
		 * it may still be used by DN and Balancer to generate new tokens before
		 * they get a chance to sync their keys with NN. Since we require
		 * keyUpdInterval to be long enough so that all live DN's and Balancer
		 * will sync their keys with NN at least once during the period, the
		 * estimated expiry date for currentKey is set to now() + 2 *
		 * keyUpdateInterval + tokenLifetime. Similarly, the estimated expiry
		 * date for nextKey is one keyUpdateInterval more.
		 */
	...
	synchronized boolean updateKeys() throws IOException {
		...
		currentKey = new BlockKey(nextKey.getKeyId(), Time.now() + 2 * keyUpdateInterval + tokenLifetime,
				nextKey.getKey());
		allKeys.put(currentKey.getKeyId(), currentKey);
	...
	public Token<BlockTokenIdentifier> generateToken(ExtendedBlock block, EnumSet<AccessMode> modes)
			throws IOException {
	...
	public Token<BlockTokenIdentifier> generateToken(String userId, ExtendedBlock block, EnumSet<AccessMode> modes)
			throws IOException {
	...
	public void checkAccess(BlockTokenIdentifier id, String userId, ExtendedBlock block, AccessMode mode)
			throws InvalidToken {
		if (LOG.isDebugEnabled()) {
			LOG.debug("Checking access for user=" + userId + ", block=" + block + ", access mode=" + mode + " using "
					+ id.toString());
		}
	...
	public void checkAccess(Token<BlockTokenIdentifier> token, String userId, ExtendedBlock block, AccessMode mode)
		...
		try {
			...
			throw new InvalidToken("Unable to de-serialize block token identifier for user=" + userId + ", block="
					+ block + ", access mode=" + mode);
		}
	...
	/**
	 * check if a token is expired. for unit test only. return true when token
	 ...
	 * BlockKey.
	 * 
	 * @return a data encryption key which may be used to encrypt traffic over
	 ...
	 * 
	 * @param keyId
	 *            identifier of the secret key used to generate the encryption
	 *            key.
	 * @param nonce
		...
		synchronized (this) {
			...
			if (key == null) {
				throw new InvalidEncryptionKeyException(
						"Can't re-compute encryption key" + " for nonce, since the required block key (keyID=" + keyId
