...
public class DelegationTokenSelector extends AbstractDelegationTokenSelector<DelegationTokenIdentifier> {
	...
	/**
	 * Select the delegation token for hdfs. The port will be rewritten to the
	 * port of hdfs.service.host_$nnAddr, or the default rpc namenode port. This
	 * method should only be called by non-hdfs filesystems that do not use the
	 * rpc port to acquire tokens. Ex. webhdfs, hftp
	 * 
	 * @param nnUri
	 ...
	 * @return Token
	 */
	public Token<DelegationTokenIdentifier> selectToken(final URI nnUri, Collection<Token<?>> tokens,
		...
		// rpc port unless a config key is set. there should be a way to
		// automatic
		...
		// use original hostname from the uri to avoid unintentional host
		// resolving
