...
/**
 * A HDFS specific delegation token secret manager. The secret manager is
 * responsible for generating and accepting the password for each token.
 */
...
public class DelegationTokenSecretManager extends AbstractDelegationTokenSecretManager<DelegationTokenIdentifier> {
	 ...
	 * @param storeTokenTrackingId
	 *            whether to store the token's tracking id
	 */
	public DelegationTokenSecretManager(long delegationKeyUpdateInterval, long delegationTokenMaxLifetime,
			long delegationTokenRenewInterval, long delegationTokenRemoverScanInterval, boolean storeTokenTrackingId,
	...
	public byte[] retrievePassword(DelegationTokenIdentifier identifier) throws InvalidToken {
		try {
			...
			// HA standby NN. non-token auths are allowed into the namespace
			// which
	...
	public byte[] retriableRetrievePassword(DelegationTokenIdentifier identifier)
		...
		try {
			...
			if (namesystem.inTransitionToActive()) {
				...
				// active state, let client retry since the corresponding
				// editlog may
	...
	public static class SecretManagerState {
		...
		public SecretManagerState(SecretManagerSection s, List<SecretManagerSection.DelegationKey> keys,
	...
	public synchronized SecretManagerState saveSecretManagerState() {
		SecretManagerSection s = SecretManagerSection.newBuilder().setCurrentId(currentId)
				.setTokenSequenceNumber(delegationTokenSequenceNumber).setNumKeys(allKeys.size())
				.setNumTokens(currentTokens.size()).build();
		...
		for (DelegationKey v : allKeys.values()) {
			SecretManagerSection.DelegationKey.Builder b = SecretManagerSection.DelegationKey.newBuilder()
					.setId(v.getKeyId()).setExpiryDate(v.getExpiryDate());
		...
		for (Entry<DelegationTokenIdentifier, DelegationTokenInformation> e : currentTokens.entrySet()) {
			...
			SecretManagerSection.PersistToken.Builder b = SecretManagerSection.PersistToken.newBuilder()
					.setOwner(id.getOwner().toString()).setRenewer(id.getRenewer().toString())
					.setRealUser(id.getRealUser().toString()).setIssueDate(id.getIssueDate())
					.setMaxDate(id.getMaxDate()).setSequenceNumber(id.getSequenceNumber())
	 ...
	 * 
	 * @param identifier
	 *            DelegationTokenIdentifier read from the edit logs or fsimage
	 * 
	 * @param expiryTime
	 *            token expiry time
	 * @throws IOException
