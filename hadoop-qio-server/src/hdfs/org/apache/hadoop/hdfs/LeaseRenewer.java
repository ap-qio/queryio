...
/**
 * <p>
 * Used by {@link DFSClient} for renewing file-being-written leases on the
 * namenode. When a file is opened for write (create or append), namenode stores
 * a file lease for recording the identity of the writer. The writer (i.e. the
 * DFSClient) is required to renew the lease periodically. When the lease is not
 * renewed before it expires, the namenode considers the writer as failed and
 * then it may either let another writer to obtain the lease or close the file.
 * </p>
 * <p>
 * This class also provides the following functionality:
 * <ul>
 * <li>It maintains a map from (namenode, user) pairs to lease renewers. The
 * same {@link LeaseRenewer} instance is used for renewing lease for all the
 * {@link DFSClient} to the same namenode and the same user.</li>
 * <li>Each renewer maintains a list of {@link DFSClient}. Periodically the
 * leases for all the clients are renewed. A client is removed from the list
 * when the client is closed.</li>
 * <li>A thread per namenode per user is used by the {@link LeaseRenewer} to
 * renew the leases.</li>
 * </ul>
 * </p>
 */
class LeaseRenewer {
	...
	static LeaseRenewer getInstance(final String authority, final UserGroupInformation ugi, final DFSClient dfsc)
			throws IOException {
	...
	/**
	 * A factory for sharing {@link LeaseRenewer} objects among
	 * {@link DFSClient} instances so that there is only one renewer per
	 * authority per user.
	 */
	...
	/**
	 * A period in milliseconds that the lease renewer thread should run after
	 * the map became empty. In other words, if the map is empty for a time
	 * period longer than the grace period, the renewer should terminate.
	 */
	...
	/**
	 * A stringified stack trace of the call stack when the Lease Renewer was
	 * instantiated. This is only generated if trace-level logging is enabled on
	 * this class.
	 */
	...
	void closeFile(final long inodeId, final DFSClient dfsc) {
		...
		synchronized (this) {
			...
			if (emptyTime == Long.MAX_VALUE) {
				...
				// discover the first time that all file-being-written maps are
				// empty.
	...
	/**
	 * Periodically check in with the namenode and renew all the leases when the
	 * lease period is half over.
	 */
	private void run(final int id) throws InterruptedException {
		for (long lastRenewed = Time.monotonicNow(); !Thread.interrupted(); Thread.sleep(getSleepPeriod())) {
			...
			synchronized (this) {
				...
				// if no clients are in running state or there is no more
				// clients
