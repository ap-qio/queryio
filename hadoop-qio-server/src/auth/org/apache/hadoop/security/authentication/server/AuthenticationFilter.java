...
/**
 * <p>
 * The {@link AuthenticationFilter} enables protecting web application resources
 * with different (pluggable) authentication mechanisms and signer secret
 * providers.
 * </p>
 * <p>
 ...
 * authentication and once it obtains an {@link AuthenticationToken} from it,
 * sets a signed HTTP cookie with the token. For client requests that provide
 * the signed HTTP cookie, it verifies the validity of the cookie, extracts the
 * user information and lets the request proceed to the target resource.
 * </p>
 * The supported configuration properties are:
 * <ul>
 * <li>config.prefix: indicates the prefix to be used by all other configuration
 * properties, the default value is no prefix. See below for details on how/why
 * this prefix is used.</li>
 * <li>[#PREFIX#.]type: simple|kerberos|#CLASS#, 'simple' is short for the
 * {@link PseudoAuthenticationHandler}, 'kerberos' is short for
 * {@link KerberosAuthenticationHandler}, otherwise the full class name of the
 * {@link AuthenticationHandler} must be specified.</li>
 ...
 * {@link AuthenticationHandler} implementation and the
 * {@link AuthenticationFilter} will take all the properties that start with the
 * prefix #PREFIX#, it will remove the prefix from it and it will pass them to
 * the the authentication handler for initialization. Properties that do not
 * start with the prefix will not be passed to the authentication handler
 * initialization.
 * </p>
 * <p>
 ...
 * mentioned above. The "random" implementation uses a randomly generated secret
 * that rolls over at the interval specified by the [#PREFIX#.]token.validity
 * mentioned above. The "zookeeper" implementation is like the "random" one,
 * except that it synchronizes the random secret and rollovers between multiple
 * servers; it's meant for HA services.
 * </p>
...
public class AuthenticationFilter implements Filter {
	...
	public static SignerSecretProvider constructSecretProvider(ServletContext ctx, Properties config,
		...
		if (!disallowFallbackToRandomSecretProvider && "file".equals(name)
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
		...
		if (unauthorizedResponse) {
			if (!httpResponse.isCommitted()) {
				...
				// If response code is 401. Then WWW-Authenticate Header should
				// be
