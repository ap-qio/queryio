 ...
 * default realm. It does not have a default value.</li>
 * <li>kerberos.keytab: the keytab file containing the credentials for the
 * Kerberos principal. It does not have a default value.</li>
...
public class KerberosAuthenticationHandler implements AuthenticationHandler {
	...
	/**
	 * Constant for the configuration property that indicates the keytab file
	 * path.
	 */
	 ...
	 *            the HTTP client response.
	 *
	 * @return an authentication token if the Kerberos SPNEGO sequence is
	 *         complete and valid, <code>null</code> if it is in progress (in
	 *         this case the handler handles the response to the client).
	 *
	 * @throws IOException
