...
/**
 * The <code>PseudoAuthenticationHandler</code> provides a pseudo authentication
 * mechanism that accepts the user name specified as a query string parameter.
 * <p>
 * This mimics the model of Hadoop Simple authentication which trust the
 * 'user.name' property provided in the configuration object.
 * <p>
...
public class PseudoAuthenticationHandler implements AuthenticationHandler {
	...
	/**
	 * Constant for the configuration property that indicates if anonymous users
	 * are allowed.
	 */
	...
	/**
	 * Creates a Hadoop pseudo authentication handler with the default
	 * auth-token type, <code>simple</code>.
	 */
	 ...
	 * query string and creates an {@link AuthenticationToken} with it.
	 * <p>
	 * If the HTTP client request does not contain the
	 * {@link PseudoAuthenticator#USER_NAME} parameter and the handler is
	 * configured to allow anonymous users it returns the
	 * {@link AuthenticationToken#ANONYMOUS} token.
	 * <p>
	 * If the HTTP client request does not contain the
	 * {@link PseudoAuthenticator#USER_NAME} parameter and the handler is
	 * configured to disallow anonymous users it throws an
	 ...
	 *            the HTTP client response.
	 *
	 * @return an authentication token if the HTTP client request is accepted
	 *         and credentials are valid.
	 *
	 * @throws IOException
	 *             thrown if an IO error occurred.
	 * @throws AuthenticationException
	 *             thrown if HTTP client request was not accepted as an
