...
/**
 * Interface for server authentication mechanisms. The
 * {@link AuthenticationFilter} manages the lifecycle of the authentication
 * handler. Implementations must be thread-safe as one instance is initialized
...
public interface AuthenticationHandler {
	...
	/**
	 * Returns the authentication type of the authentication handler. This
	 * should be a name that uniquely identifies the authentication type. For
	 * example 'simple' or 'kerberos'.
	 *
	 ...
	 * <p>
	 * This is useful for handling operations like get/renew/cancel delegation
	 * tokens which are being handled as operations of the service end-point.
	 * <p>
	 ...
	 *
	 * @param token
	 *            the authentication token if any, otherwise <code>NULL</code>.
	 * @param request
	 ...
	 * HTTP client request is not yet authenticated.
	 * <p>
	 * Depending upon the authentication mechanism being implemented, a
	 * particular HTTP client may end up making a sequence of invocations before
	 * authentication is successfully established (this is the case of Kerberos
	 * SPNEGO).
	 * <p>
	 * This method must return an {@link AuthenticationToken} only if the the
	 * HTTP client request has been successfully and fully authenticated.
	 * <p>
	 * If the HTTP client request has not been completely authenticated, this
	 * method must take over the corresponding HTTP response and it must return
	 * <code>null</code>.
	 *
	 * @param request
	 *            the HTTP client request.
	 ...
	 *            the HTTP client response.
	 *
	 * @return an {@link AuthenticationToken} if the HTTP client request has
	 *         been authenticated, <code>null</code> otherwise (in this case it
