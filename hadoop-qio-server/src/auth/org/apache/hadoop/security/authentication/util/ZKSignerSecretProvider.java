 ...
 * All ZKSignerSecretProviders looking at that znode will use those secrets and
 * next rollover time to ensure they are synchronized. There is no "leader" --
 * any of the ZKSignerSecretProviders can choose the next secret; which one is
 * indeterminate. Kerberos-based ACLs can also be enforced to prevent a
 * malicious third-party from getting or setting the secrets. It uses its own
 * CuratorFramework client for talking to ZooKeeper. If you want to use your own
 * Curator client, you can pass it to ZKSignerSecretProvider; see
 ...
 * <li>signer.secret.provider.zookeeper.path: indicates the ZooKeeper path to
 * use for storing and retrieving the secrets. All ZKSignerSecretProviders that
 * need to coordinate should point to the same path.</li>
 * <li>signer.secret.provider.zookeeper.auth.type: indicates the auth type to
 * use. Supported values are "none" and "sasl". The default value is "none"</li>
...
public class ZKSignerSecretProvider extends RolloverSignerSecretProvider {
	...
	/**
	 * Constant for the property that specifies whether or not the Curator
	 * client should disconnect from ZooKeeper on shutdown. The default is
	 * "true". Only set this to "false" if a custom Curator client is being
	 * provided and the disconnection is being handled elsewhere.
	 */
	 ...
	 * a custom CuratorFramework client. If set ZKSignerSecretProvider will use
	 * this Curator client instead of creating a new one. The providing class is
	 * responsible for creating and configuring the Curator client (including
	 * security and ACLs) in this case.
	 */
	public static final String ZOOKEEPER_SIGNER_SECRET_PROVIDER_CURATOR_CLIENT_ATTRIBUTE = CONFIG_PREFIX
			+ "curator.client";
	...
	private String path;
	/**
	 * Stores the next secret that will be used after the current one rolls
	 * over. We do this to help with rollover performance by actually deciding
	 * the next secret at the previous rollover. This allows us to switch to the
	 * next secret very quickly. Afterwards, we have plenty of time to decide on
	 * the next secret.
	 */
	...
	/**
	 * Stores the next date that the rollover will occur. This is only used for
	 * allowing new servers joining later to synchronize their rollover with
	 * everyone else.
	 */
	...
	public void init(Properties config, ServletContext servletContext, long tokenValidity) throws Exception {
		Object curatorClientObj = servletContext
				.getAttribute(ZOOKEEPER_SIGNER_SECRET_PROVIDER_CURATOR_CLIENT_ATTRIBUTE);
		...
		try {
			...
			// everyone tries to do this, only one will succeed and only when
			// the
			// znode doesn't already exist. Everyone else will synchronize on
			// the
			// data from the znode
			client.create().creatingParentsIfNeeded().forPath(path,
					generateZKData(generateRandomSecret(), generateRandomSecret(), null));
			zkVersion = 0;
	...
	protected synchronized void rollSecret() {
		super.rollSecret();
		// Try to push the information to ZooKeeper with a potential next
		// secret.
	 ...
	 * <p>
	 * [DATA_VERSION, newSecretLength, newSecret, currentSecretLength,
	 * currentSecret, previousSecretLength, previousSecret, nextRolloverDate]
	 * <p>
	 * Only previousSecret can be null, in which case the format looks like
	 * this:
	 * <p>
	 * [DATA_VERSION, newSecretLength, newSecret, currentSecretLength,
	...
	private synchronized byte[] generateZKData(byte[] newSecret, byte[] currentSecret, byte[] previousSecret) {
		...
		ByteBuffer bb = ByteBuffer.allocate(INT_BYTES + INT_BYTES + newSecretLength + INT_BYTES + currentSecretLength
				+ INT_BYTES + previousSecretLength + LONG_BYTES);
	 ...
	 * next secret and version. If isInit is true, it will also parse the
	 * current and previous secrets, and the next rollover date; it will also
	 * init the secrets. Hence, isInit should only be true on startup.
	 * 
	 * @param isInit
		...
		try {
			...
			if (dataVersion > DATA_VERSION) {
				throw new IllegalStateException(
						"Cannot load data from ZooKeeper; it" + "was written with a newer version");
			}
	...
	protected CuratorFramework createCuratorClient(Properties config) throws Exception {
		...
		CuratorFramework cf = CuratorFrameworkFactory.builder().connectString(connectionString).retryPolicy(retryPolicy)
	...
	/**
	 * Simple implementation of an {@link ACLProvider} that simply returns an
	 * ACL that gives all permissions only to a single principal.
	 */
	private static class SASLOwnerACLProvider implements ACLProvider {
		...
		private SASLOwnerACLProvider(String principal) {
			this.saslACL = Collections.singletonList(new ACL(Perms.ALL, new Id("sasl", principal)));
		}
	 ...
	 * be used for connecting to ZooKeeper.
	 */
	@InterfaceAudience.Private
	public static class JaasConfiguration extends Configuration {
		 ...
		 * principal, and keytab. The other necessary options will be set for
		 * you.
		 *
		 * @param entryName
