...
public final class AppLogger
{
			...
			final String loggerFileName, final String logFileProperty, final String logFileName)
	{
		...
		try
		{
			...
			if (file.exists())
			{
				...
				prop.load(bis);
				final File logFile = new File(logFileLocation, logFileName != null ? logFileName : prop
						.getProperty(logFileProperty));
	...
	public static final void setLoggerLevel(final int productId, final int level)
	{
		...
		if (logger != null)
		{
			...
			{
				case Priority.FATAL_INT: currentLevel = FATAL; break;
				case Priority.ERROR_INT: currentLevel = ERROR; break;
				case Priority.WARN_INT:  currentLevel = WARN; break;
				case Priority.INFO_INT: currentLevel = INFO; break;
				case Priority.DEBUG_INT: currentLevel = DEBUG; break;
				default: currentLevel = FATAL; break;
			}			
