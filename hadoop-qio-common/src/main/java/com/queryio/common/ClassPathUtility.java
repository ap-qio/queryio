...
public class ClassPathUtility {
	...
	public static synchronized void recycleClassLoder() {
		try {
			...
			String tagParserJarDir = EnvironmentalConstants.getAppHome() + File.separator + ".."
					...
					+ File.separator + QueryIOConstants.MAPREDRESOURCE
					+ File.separator + QueryIOConstants.TAGPARSER_JAR_DIR
					+ File.separator;
			
			String defaultJarFile = EnvironmentalConstants.getAppHome() + File.separator + ".."
					+ File.separator + ".." + File.separator + ".."
					+ File.separator + QueryIOConstants.MAPREDRESOURCE
					+ File.separator + QueryIOConstants.DEFAULT_USERDEFINEDTAGSJAR
	...
	public static synchronized void recycleClassLoderForUIServer() {
		try {
			closeClassLoder();
			String tagParserJarDir = EnvironmentalConstants.getAppHome()
					+ File.separator + QueryIOConstants.MAPREDRESOURCE
					+ File.separator + QueryIOConstants.TAGPARSER_JAR_DIR
					+ File.separator;
			
			String defaultJarFile = StartupParameters.getHadoopDirLocation()
					+ File.separator + QueryIOConstants.MAPREDRESOURCE
					+ File.separator + QueryIOConstants.DEFAULT_USERDEFINEDTAGSJAR
					+ File.separator + QueryIOConstants.DEFAULT_ONINGEST_PARSER_LIBJAR;
...
////		URLClassLoader webappClassLoader = (URLClassLoader) ClassLoader.getSystemClassLoader();
