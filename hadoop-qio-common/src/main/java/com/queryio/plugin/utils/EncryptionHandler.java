...
public class EncryptionHandler {
	...
	// FIXME Hardcoded for now, should be moved to some secure file with
	// restricted access.
	private static char[] password = "QueryIO123456qwert".toCharArray();

	private static byte[] initVector = new byte[] { (byte) 0x8E, 0x12, 0x39, (byte) 0x9C, 0x07, 0x72, 0x6F, 0x5A,
	...
	// We are using same key each time, so instead of creating new instance,
	// creating a static var.
	...
	public EncryptionHandler(int mode, boolean encrypt) throws NoSuchAlgorithmException, NoSuchPaddingException,
	...
	public InputStream getCompressedCipherInputStream(InputStream inputStream, boolean encrypt, int compressionType)
			throws IOException {
	...
	public OutputStream getCompressedCipherOutputStream(OutputStream outputStream, boolean encrypt, int compressionType)
			throws IOException {
