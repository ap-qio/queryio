...
public abstract class AbstractDataTagParser implements IDataTagParser {
	...
	// private HashMap<String, Boolean> doKeepCount = new HashMap<String,
	// Boolean>();
	// private HashMap<String, Boolean> doKeepDistinctCount = new
	// HashMap<String, Boolean>();
	// HashMap<String, Boolean> doKeepTotal = new HashMap<String, Boolean>();
	// private HashMap<String, Boolean> doKeepMax = new HashMap<String,
	// Boolean>();
	// private HashMap<String, Boolean> doKeepMin = new HashMap<String,
	// Boolean>();
	// private HashMap<String, Boolean> doKeepWordExist = new HashMap<String,
	// Boolean>();
	// private HashMap<String, Boolean> doKeepWordOccurrenceCount = new
	// HashMap<String, Boolean>();
	// private HashMap<String, Boolean> doKeepPatternExist = new HashMap<String,
	// Boolean>();
	// private HashMap<String, Boolean> doKeepPatternMatchCount = new
	// HashMap<String, Boolean>();
	// private HashMap<String, Boolean> doKeepTotalWordCount = new
	// HashMap<String, Boolean>();
	// private HashMap<String, Boolean> doKeepTotalLineCount = new
	// HashMap<String, Boolean>();
	...
	protected void setMethod(Tag tag, String method, boolean isLogical) {
		if (isLogical) {
			...
			if (method.startsWith("SUM")) {
				operatorColumnNames.put(tag.getName(), method.substring(method.indexOf("(") + 1, method.indexOf(")")));
				...
				if (method.contains("'")) {
					operatorColumnNames.put(tag.getName(),
							method.substring(method.indexOf("('") + 2, method.indexOf("')")));
				} else {
					operatorColumnNames.put(tag.getName(),
	...
	final protected void evaluateCurrentEntry(HashMap<String, String> curValueMap) {
		...
		for (int i = 0; i < tags.size(); i++) {
			...
			// System.out.println("Expressions: " + expressions + " for tag: " +
			// tag.getName() + " evaluated to " + expResults + " for values: " +
			// curValueMap);
	...
	final protected void evaluateCurrentEntry(HashMap<String, String> curValueMap, String line) {
		// String line = "IP,CPU,RAM,DiskRead,DiskWrite,NetRead,NetWrite"; //
		// Move this to arguments
		for (int i = 0; i < tags.size(); i++) {
			Tag tag = tags.get(i);
			...
			if ((expResults.size() == 0)
					|| (expResults.size() > 0 && evaluateBooleanExpression(expResults, tag.getRelations()))) {
				satisfied.put(tag.getName(), true);
				...
				case METHOD_SUM:
				case METHOD_AVG:
					total.put(tag.getName(), total.get(tag.getName())
							.add(new BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName())))));
					count.put(tag.getName(), count.get(tag.getName()) + 1);
					break;
				case METHOD_BOOLEAN:
					break;
				case METHOD_MAX:
					...
					if (max.get(tag.getName()) == null) {
						isMax = true;
					} else if (max.get(tag.getName())
							.compareTo(new BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName())))) == -1) {
						isMax = true;
					}
					if (isMax) {
				...
				case METHOD_DISTINCT_COUNT:
					if (!distinctCountArray.get(tag.getName())
							.contains(curValueMap.get(operatorColumnNames.get(tag.getName())))) {
						distinctCount.put(tag.getName(), ((Long) distinctCount.get(tag.getName())).longValue() + 1L);
						distinctCountArray.get(tag.getName())
								.add(curValueMap.get(operatorColumnNames.get(tag.getName())));
					}
					break;
				case METHOD_WORD_EXIST:
				case METHOD_WORD_OCCURRENCE_COUNT:
					Pattern pattern = Pattern.compile("\\b" + operatorColumnNames.get(tag.getName()) + "\\b",
				...
				// {
				// distinctCount.put(tag.getName(), ((Long)
				// distinctCount.get(tag.getName())).longValue() + 1L);
				// distinctCountArray.get(tag.getName()).add(curValueMap.get(operatorColumnNames.get(tag.getName())));
				// }
				// }
				// if(doKeepTotal.get(tag.getName())){
				// total.put(tag.getName(), total.get(tag.getName()).add(new
				// BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName())))));
				// }
				// if(doKeepMax.get(tag.getName())) {
				// boolean isMax = false;
				// if(max.get(tag.getName()) == null) {
				// isMax = true;
				// } else if(max.get(tag.getName()).compareTo(new
				// BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName()))))
				// == -1) {
				// isMax = true;
				// }
				// if(isMax) {
				// max.put(tag.getName(), new
				// BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName()))));
				// }
				// }
				...
				// if(min.get(tag.getName()) == null) {
				// isMin = true;
				// } else if(min.get(tag.getName()).compareTo(new
				// BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName()))))
				// == 1) {
				// isMin = true;
				// }
				// if(isMin) {
				// min.put(tag.getName(), new
				// BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName()))));
				// }
				// }
				// if(doKeepWordOccurrenceCount.get(tag.getName())) {
				// Pattern pattern = Pattern.compile("\\b" +
				// operatorColumnNames.get(tag.getName())+ "\\b",
				// Pattern.CASE_INSENSITIVE);
				// Matcher matcher = pattern.matcher(line);
				// int matchCount = 0;
				// while (matcher.find()) {
				// matchCount++;
				// }
				// wordMatchCount.put(tag.getName(),
				// wordMatchCount.get(tag.getName()) + matchCount);
				// }
				// if(doKeepPatternMatchCount.get(tag.getName())) {
				// Pattern pattern =
				// Pattern.compile(operatorColumnNames.get(tag.getName()));
				// Matcher matcher2 = pattern.matcher(line);
				// int matchCount = 0;
				// while (matcher2.find()) {
				// matchCount++;
				// }
				// patternMatchCount.put(tag.getName(),
				// patternMatchCount.get(tag.getName()) + matchCount);
				// }
				...
				// cnt++; sc.next();
				// }
				// totalWordCount.put(tag.getName(),
				// totalWordCount.get(tag.getName()) + cnt);
				// sc.close();
				// }
				// if(doKeepTotalLineCount.get(tag.getName())) {
				// totalLineCount.put(tag.getName(),
				// totalLineCount.get(tag.getName()) + 1);
				// }
			}
		}
	}
	...
	final protected void evaluateRecordEntry(HashMap<String, String> curValueMap, String line) {
		...
		for (int i = 0; i < tags.size(); i++) {
			...
			if ((expResults.size() == 0)
				...
				// }
				// if(doKeepDistinctCount.get(tag.getName())){
				// distinctCount.put(tag.getName(),
				// curValueMap.get(operatorColumnNames.get(tag.getName())));
				// }
				// if(doKeepTotal.get(tag.getName())){
				// total.put(tag.getName(), (new
				// BigDecimal(curValueMap.get(operatorColumnNames.get(tag.getName())))));
				// }
				// if(doKeepMax.get(tag.getName())) {
				// max.put(tag.getName(), new
	...
	public void parseTagData(String key, String value, boolean isReducer) throws Exception {
		...
		// if(!distinctCountArray.get(key).contains(value)) {
		// distinctCount.put(key, ((Long) distinctCount.get(key)).longValue() +
		...
		// wordMatchCount.put(key, wordMatchCount.get(key) +
		// Integer.parseInt(value));
		// }
		// if(doKeepPatternMatchCount.get(key)) {
		// patternMatchCount.put(key, patternMatchCount.get(key) +
		// Integer.parseInt(value));
		// }
		// if(doKeepTotalWordCount.get(key)) {
		// totalWordCount.put(key, totalWordCount.get(key) +
		// Integer.parseInt(value));
		// }
		// if(doKeepTotalLineCount.get(key)) {
		// totalLineCount.put(key, totalLineCount.get(key) +
		// Long.parseLong(value));
		// }
	}
			...
			if (satisfied.get(tag.getName())) {
				...
				if (tag.getMethod() != METHOD_NONE) {
					...
					case METHOD_COUNT:
						...
						break;
					case METHOD_AVG:
						value = total.get(tag.getName())
								.divide(new BigDecimal(count.get(tag.getName())), RoundingMode.HALF_UP).doubleValue(); // double
						break;
					case METHOD_MAX:
						value = max.get(tag.getName()).doubleValue(); // BigDecimal
																		// -
																		// change
																		// to
																		// double
						break;
					case METHOD_MIN:
						value = min.get(tag.getName()).doubleValue(); // BigDecimal
																		// -
																		// change
																		// to
																		// double
						break;
					case METHOD_DISTINCT_COUNT:
						Object tmp = distinctCount.get(tag.getName()); // long(reducer,
																		// combiner)
																		// /
																		// string(mapper)
					...
					// value = total.get(tag.getName()).divide(new
					// BigDecimal(count.get(tag.getName())),
					// RoundingMode.HALF_UP).doubleValue(); // double
					// } else if(method == METHOD_MAX) {
					// value = max.get(tag.getName()).doubleValue(); //
					// BigDecimal - change to double
					// } else if(method == METHOD_MIN) {
					// value = min.get(tag.getName()).doubleValue(); //
					// BigDecimal - change to double
					// } else if(method==METHOD_DISTINCT_COUNT) {
					// Object tmp = distinctCount.get(tag.getName()); // long /
					// string
					// if (tmp instanceof String)
					// value = (String) tmp;
					// else if (tmp instanceof Long)
		...
		for (int i = 0; i < tags.size(); i++) {
			...
			if (isSatisfied(tag.getName())) {
				if (!isMethod(tag.getName())) {
					metadata.add(alterSequence(tag.getName()), String.valueOf(tag.getSubstitutionValue()));
					continue;
				}
				...
				case OPERATION_WORD_EXISTS:
					metadata.add(alterSequence(tag.getName()), String.valueOf(wordMatchCount.get(tag.getName()) > 0));
					break;
				case OPERATION_WORD_MATCH_COUNT:
					metadata.add(alterSequence(tag.getName()), String.valueOf(wordMatchCount.get(tag.getName())));
					break;
				case OPERATION_PATTERN_EXISTS:
					metadata.add(alterSequence(tag.getName()),
							String.valueOf(patternMatchCount.get(tag.getName()) > 0));
					break;
				case OPERATION_PATTERN_MATCH_COUNT:
					metadata.add(alterSequence(tag.getName()), String.valueOf(patternMatchCount.get(tag.getName())));
					break;
				case OPERATION_TOTAL_WORD_COUNT:
					metadata.add(alterSequence(tag.getName()), String.valueOf(totalWordCount.get(tag.getName())));
					break;
				case OPERATION_TOTAL_LINE_COUNT:
					metadata.add(alterSequence(tag.getName()), String.valueOf(totalLineCount.get(tag.getName())));
					break;
				...
				case OPERATION_META_EXIST:
					...
					break;
				case OPERATION_COPY:
					metadata.add(alterSequence(tag.getName()), String.valueOf(copyValues.get(tag.getName())));
					break;
				}
			}
		}
	...
	protected void parseLine(String line, Metadata metadata) {
		...
		for (int i = 0; i < tags.size(); i++) {
			...
			case OPERATION_META_OCCURENCE_COUNT:
				...
				// System.out.println("match with value for column : " +
				// colNameForMatch);
				...
				for (int j = 0; j < metadata.names().length; j++) {
					...
					// System.out.println("Current metadata key: " +
					// alterSequence(key) + ", looking for column: " +
					// colNameForMatch);
				...
				while (sc.hasNext()) {
					...
					if (word.trim().equalsIgnoreCase(valueForColumn)) {
						// System.out.println("Current word: " + words[j].trim()
						// + ", looking for word: " + valueForColumn);
						count++;
					}
				}
				wordMatchCount.put(tag.getName(), wordMatchCount.get(tag.getName()) + count);
