...
public class UserDefinedTagUtils {
	...
	public static List<UserDefinedTag> generateDefaultTags(FileStatus status) throws IOException {
		List<UserDefinedTag> tags = new ArrayList<UserDefinedTag>();
			...
			tags.add(new UserDefinedTag(TableMetadata.DEFAULT_TAG_MODIFICATIONTIME, new Timestamp(status.getModificationTime())));
			...
			tags.add(new UserDefinedTag(TableMetadata.DEFAULT_TAG_BLOCKS, convert(status.getBlocks())));
		return tags;
	}
