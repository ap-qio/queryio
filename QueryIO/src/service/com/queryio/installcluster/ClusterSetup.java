...
public class ClusterSetup {
	...
	public HashMap<String, Boolean> installNodes(String cip, String cUsername, String cPassword, String queryioUser,
			String queryioPassword, boolean isLocal, String installUserHome, String installJavaHome,
			String ipPrivateKey, String port) {
	...
	public DWRResponse installHost(boolean isLocal, String installUserHome, String installJavaHome) {
		...
		DWRResponse response = RemoteManager.insertHostAutomatationInstaller(ip, userName, password, sshPrivateKey,
				"/default-rack", sshPort, ip, isLocal, installUserHome, installJavaHome, false);
		AppLogger.getLogger()
				.debug("host result : " + response.isTaskSuccess() + " : " + response.getResponseMessage());
	...
	public Boolean installNameNode(String ip, boolean isLocal, String installUserHome) {
		...
		DWRResponse response = RemoteManager.addNameNodeInstaller(host, "NameNode1", d, installDir,
				namenodePorts.get("serverPort").toString(), namenodePorts.get("http").toString(),
				namenodePorts.get("https").toString(), namenodePorts.get("s3serverPort").toString(),
				namenodePorts.get("s3SslserverPort").toString(), namenodePorts.get("hdfsOverFtpPort").toString(),
				namenodePorts.get("ftpServerPort").toString(), namenodePorts.get("ftpSslServerPort").toString(),
				namenodePorts.get("jmxPort").toString(), "MetaStore", "Hive", false);
		if (response.isTaskSuccess()) {
	...
	public Boolean installDataNode(String ip, boolean isLocal, String installUserHome) {
		...
		jmxPort = jmxPort.substring(jmxPort.indexOf("jmxremote.port=") + 1);
		datanodePorts.put("jmx",
				jmxPort.substring(jmxPort.indexOf('=') + 1, jmxPort.indexOf("-Dcom.sun.management")).trim());
		...
		DWRResponse response = RemoteManager.addDataNodeInstaller(host, "DataNode1",
				datanodePorts.get("serverPort").toString(), datanodePorts.get("http").toString(),
				datanodePorts.get("https").toString(), datanodePorts.get("ipc").toString(),
	...
	public Boolean installRM(String ip, boolean isLocal, String installUserHome) {
		...
		DWRResponse response = RemoteManager.addResourceManagerInstaller("ResourceManager1", host,
				rmPorts.get("serverPort").toString(), rmPorts.get("scheduler").toString(),
				rmPorts.get("webapp").toString(), rmPorts.get("adminPort").toString(),
				rmPorts.get("tracker").toString(), rmPorts.get("hostoryPort").toString(),
				rmPorts.get("webhostoryPort").toString(), rmPorts.get("jmxPort").toString(), installDir);
		if (response.isTaskSuccess()) {
	...
	public Boolean installNM(String ip, boolean isLocal, String installUserHome) {
		...
		DWRResponse response = RemoteManager.addNodeManagerInstaller("NodeManager1", host, "ResourceManager1",
				nmPorts.get("localizer").toString(), nmPorts.get("webapp").toString(),
