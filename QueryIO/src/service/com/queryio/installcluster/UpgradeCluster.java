...
public class UpgradeCluster {
	...
	public boolean startUpgrade(String qioInstallLoc) {
		...
		try {
			...
			// Now user cant upgrade from previous version (v3.6 or earlier) to
			// v4.0 as in v4.0 DB has been changed from postgres to HSQL. So we
			// dont need to update db configs.
			// Update queryio db
			...
			if (hostList.contains("127.0.0.1")) {
				...
				DWRResponse response = RemoteManager.insertHostInstaller(ip, username, password, "null", installDir, "",
						"", javaHome, "22", true, true, dbPort);
			...
			if (list.length > 4) {
				for (int i = 0; i < list.length; i++) {
					...
					sshPort = list[i];
					password = (password.equals("null")) ? null
							: com.queryio.common.util.SecurityHandler.decryptData(password);
					sshPrivateKey = (sshPrivateKey.equals("null")) ? null
							: com.queryio.common.util.SecurityHandler.decryptData(sshPrivateKey);

					int ind = hostList.indexOf(ip);
					String temp = hostList.substring(ind, hostList.indexOf(".!", ind));
					String installDir = temp.substring(temp.indexOf(":") + 1, temp.length());
					installDir = installDir.substring(0, installDir.indexOf(QueryIOConstants.QUERYIOPACKAGE_DIR));
					String javaHome = RemoteManager.getJavaHome(ip, username, password, sshPrivateKey,
					...
					DWRResponse response = RemoteManager.insertHostInstaller(ip, username, password, sshPrivateKey,
							installDir, "", "", javaHome, sshPort, false, true, dbPort);
	...
	public void updateSystemDB(String qioInstallLoc) {
		// Whenever we upgrade queryio, handling for additional scripts to be
		// executed needs to be added here
		String systemUpdateFileName = "upgrade-3.6-to-4.0."
				+ EnvironmentalConstants.getQueryIODatabaseType().toLowerCase() + "postgres.sql";
		String systemUpdateFolderPath = qioInstallLoc + File.separator + QueryIOConstants.SERVICES_DIR_NAME
				+ File.separator + QueryIOConstants.DB_UPGRADE_DIR_NAME + File.separator
				+ QueryIOConstants.SYSTEM_DIR_NAME;
		...
		try {
			...
			AppLogger.getLogger().fatal(
					"Error occured while executing script: " + systemUpdateFilePath + "\n" + errorLogWriter.toString(),
					e);
		} finally {
	...
	public static boolean backupHadoopEtc(String host, String user, String password, String sshPrivateKey, String port,
			String installDir, boolean isBackup, boolean isLocal) {
		...
		try {
			...
			if (!isLocal) {
				...
				if (isBackup) {
					...
					command = "cp -r " + etcBackUpDir + " " + etcDir + "; cp -r " + hiveSiteBak + " " + hiveConf
							+ "; cp -r " + hiveEnvBak + " " + hiveConf + "; cp -r " + hiveLogBak + " " + hiveConf
							+ "/../logs;";
				}
	...
	public static boolean stopAgent(String host, String user, String password, String sshPrivateKey, String port,
			String installDir, boolean isLocal) {
		Session session = null;
		boolean flag = false;
		String hostName = "";
		String command = "";
		try {
			...
			if (!isLocal) {
				...
				session.connect();
				command = "sh " + installDir + "bin/stop_hive.sh; sh " + installDir + "bin/stop_hadoop.sh; sh "
						+ installDir + "bin/stop_agent.sh; ";
				// System.out.println(command);
				executeCommand(session, command);
				String response = getOutput();
				// System.out.println(response);
	...
	public static boolean startHadoop(String host, String user, String password, String sshPrivateKey, String port,
			String installDir, boolean isLocal) {
		Session session = null;
		boolean flag = false;
		String hostName = "";
		...
		try {
			...
			String newBackUpDir = installDir + QueryIOConstants.QUERYIOPACKAGE_DIR_BAK
					+ new java.text.SimpleDateFormat("MM-dd-yyyy-h-mm-ss-a").format(new java.util.Date());
			installDir = installDir + QueryIOConstants.QUERYIOPACKAGE_DIR + File.separator;
			...
			if (!isLocal) {
				...
				session.connect();
				command = " sh " + installDir + "bin/start_hadoop.sh;  sh " + installDir + "bin/start_hive.sh; sh "
						+ installDir + "bin/start_queryio_services.sh; mv  " + backUpDir + " " + newBackUpDir + "; ";
				executeCommand(session, command);
				// String response = getOutput();
				// System.out.println(response);
			} else {
	...
	public static Session createSession(String hostName, String userName, String password, int portNumber)
			throws Exception {
		JSch jsch = null;
		Session session = null;
	...
	public static Session createSessionWithPrivateKeyFile(String hostName, String userName, String privateKeyFile,
	...
	public static void main(String args[]) {
		...
		// UpgradeCluster().startUpgrade("/Applications/QueryIO"));
		...
		// String command = "ls " + installDir + "QueryIOPackage_bak/ | grep
		// hive";
		// System.out.println(command);
		// executeCommand(command);
		// System.out.println("oldHiveDir: " + oldHiveDir);
		// backupHadoopEtc("", "", "", "", "", "/Applications/QueryIO", false,
		// true);
	}
}
