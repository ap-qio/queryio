...
public class SchedulerDAO {
	...
	private static String getAllJobDetailsByJobNameJobGroup = "SELECT * FROM " + SchedulerConstants.TABLE_PREFIX
			+ StdJDBCConstants.TABLE_JOB_DETAILS + " WHERE " + StdJDBCConstants.COL_SCHEDULER_NAME + " = '"
			+ SchedulerConstants.SCHEDULER_NAME + "' AND " + StdJDBCConstants.COL_JOB_NAME + " =  ? " + "AND "
			+ StdJDBCConstants.COL_JOB_GROUP + " = ?";

	private static String getTriggerDetails = "SELECT " + SchedulerConstants.ID + ", " + SchedulerConstants.STARTTIME
			+ ", " + SchedulerConstants.ENDTIME + ", " + SchedulerConstants.STATUS + ", "
			+ SchedulerConstants.REASON_FOR_FAILURE + " FROM " + TableConstants.TABLE_TRIGGERED_SCHEDULEJOB_STATE
			+ " WHERE " + StdJDBCConstants.COL_JOB_GROUP + " = ? AND " + StdJDBCConstants.COL_JOB_NAME + " = ? ";
	...
	private static String deleteSelectedTriggers = "DELETE FROM " + TableConstants.TABLE_TRIGGERED_SCHEDULEJOB_STATE
			+ " WHERE " + SchedulerConstants.ID + " = ?";

	public static void initializeScheduler() {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("initializing scheduler: ");
		try {
			scheduler = new StdSchedulerFactory(EnvironmentalConstants.getAppHome() + "quartz.properties")
					.getScheduler();// .getDefaultScheduler();
			initialized = true;
		} catch (Exception e) {
	...
	public static void scheduleMapRedJob(JobDetailImpl jobDetailImpl, SchedulerBean schedule) {
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Scheduling mapred job, group: " + grpName + ", trigger: " + triggerName);

			switch (schedule.getInterval()) {
			case SchedulerConstants.SCH_FREQUENCY_ONCE: // Once
			{
				trigger = new SimpleTriggerImpl(triggerName, grpName, new Date(schedule.getTime()),
						new Date(schedule.getTime() + 60000), 0, 0);
				break;
			}
			...
			case SchedulerConstants.SCH_FREQUENCY_TWELVE_HOURS: {
				trigger = new CalendarIntervalTriggerImpl(triggerName, grpName, DateBuilder.IntervalUnit.HOUR, 12);
				((CalendarIntervalTriggerImpl) trigger).setStartTime(new Date(schedule.getTime()));
				break;
			}
			default: // from interval
			{
				trigger = new CalendarIntervalTriggerImpl(triggerName, grpName, DateBuilder.IntervalUnit.MINUTE,
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("A job " + jobDetailImpl.getName()
						+ " is scheduled successfully and will be triggerd at " + trigger.getStartTime());
		} catch (Exception e) {
			AppLogger.getLogger().fatal("Error Occured while scheduling job: " + e.getMessage(), e);
		}
	}

	public static void scheduleJob(JobDetailImpl jobDetailImpl, SchedulerBean schedule, String grpName)
			throws Exception {
		try {
			Trigger trigger = null;
			String triggerName = schedQueryioTriggerName + SchedulerConstants.HIPHEN + String.valueOf(getLastJobId());
	...
	public static void scheduleJob(JobDetailImpl jobDetailImpl, QuerySchedulerBean querySchedule) {
		try {
			Trigger trigger = null;
			String grpName = SCHED_BIG_QUERY_JOB_NAME + SchedulerConstants.HIPHEN
					+ String.valueOf(querySchedule.getQuery());
			String triggerName = schedQueryioTriggerName + SchedulerConstants.HIPHEN + String.valueOf(getLastJobId());
			...
			{
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("Trriger set to once Time :" + querySchedule.getTime());

				trigger = new SimpleTriggerImpl(triggerName, grpName, new Date(querySchedule.getTime()),
						new Date(querySchedule.getTime() + 60000), 0, 0);
				break;
			}
			}
			scheduler.scheduleJob(jobDetailImpl, trigger);
	...
	public static void addJob(SchedulerBean schedule) {
		try {
			...
			String grpName = SCHED_SYS_REPORT_JOB_NAME + SchedulerConstants.HIPHEN
					+ String.valueOf(schedule.getReportId());
	...
	public static void addNamespaceDiagnosisSchedule(SchedulerBean schedule, String namenodeId, long startIndex,
	...
	public static void addQueryJob(QuerySchedulerBean querySchedule) {
		try {
			if (scheduler != null) {
				...
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("Scheduler is off.");
			}
	...
	public static SchedulerBean getMapredScheduleBean(Connection connection, String jobName) {
		...
		try {
			...
			if (rs.next()) {
				...
				JobDetail jdetail = scheduler.getJobDetail(new JobKey(rs.getString(StdJDBCDelegate.COL_JOB_NAME),
						rs.getString(StdJDBCDelegate.COL_JOB_GROUP)));
	...
	public static void getAllSysReportsSchedules(ArrayList scheduleList) {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("GET_ALL_JOB_DETAILS_SYS_REPORTS" + getAllJobDetailsSysReports);
			while (rs.next()) {
				SchedulerBean sBean = new SchedulerBean();
				JobDetail jdetail = scheduler.getJobDetail(new JobKey(rs.getString(StdJDBCDelegate.COL_JOB_NAME),
						rs.getString(StdJDBCDelegate.COL_JOB_GROUP)));
				JobDataMap jMap = jdetail.getJobDataMap();
				if ((String) jMap.get(SchedulerConstants.REPORT_ID) != null
						&& !((String) jMap.get(SchedulerConstants.REPORT_ID)).trim().equals("")) {
					sBean.setReportId(
							ReportManager.getReportName((String) jMap.get(SchedulerConstants.REPORT_ID)).toString());
				}
				if ((String) jMap.get(SchedulerConstants.FORMAT_TYPE) != null
						&& !((String) jMap.get(SchedulerConstants.FORMAT_TYPE)).trim().equals("")) {
					sBean.setSelectedFormat(ReportManager
							.getExportFormat((String) jMap.get(SchedulerConstants.FORMAT_TYPE)).toString());
				}

				sBean.setInterval(Integer.parseInt((String) jMap.get(SchedulerConstants.FREQUENCY)));
				sBean.setNotificationEnable(
						Boolean.parseBoolean(jMap.getString(SchedulerConstants.NOTIFICATION_ENABLE)));
				if (sBean.isNotificationEnable()) {
					if ((String) jMap.get(SchedulerConstants.EMAIL_ADDRESS) != null
							&& !((String) jMap.get(SchedulerConstants.EMAIL_ADDRESS)).trim().equals("")) {
						sBean.setEmailUserIds(RemoteManager
								.getUserDetailInfo((String) jMap.get(SchedulerConstants.EMAIL_ADDRESS)).toString());
					} else {
	...
	public static void getAllBigQuerySchedules(ArrayList scheduleList) {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("GET_ALL_JOB_DETAILS_BIG_QUERY: " + getAllJobDetailsBigQuery);
			while (rs.next()) {
				...
				qBean.setSelectedFormat(
						ReportManager.getExportFormat((String) jMap.get(SchedulerConstants.FORMAT_TYPE)).toString());
				qBean.setInterval((Integer) jMap.getInt(SchedulerConstants.FREQUENCY));
				qBean.setNotificationEnable(
						Boolean.parseBoolean((String) jMap.get(SchedulerConstants.NOTIFICATION_ENABLE)));
				if (qBean.isNotificationEnable()) {
					...
					if (qBean.getNotificationType().equals(SchedulerConstants.NOTIFICATION_EMAIL)) {
						qBean.setEmailUserIds(RemoteManager
								.getUserDetailInfo((String) jMap.get(SchedulerConstants.EMAIL_ADDRESS)).toString());
					}
				} else {
	...
	public static void updateJob(SchedulerBean schedule) {
		try {
			...
			JobDetailImpl jobDetailImpl = (JobDetailImpl) scheduler
					.getJobDetail(new JobKey(schedule.getName(), schedule.getGroup()));
	...
	public static void updateTrigger(JobDetailImpl jobDetailImpl, SchedulerBean schedule) {
		try {
			...
			String triggerName = schedQueryioTriggerName + SchedulerConstants.HIPHEN
					+ (schedule.getName().substring(schedule.getName().indexOf(SchedulerConstants.HIPHEN) + 1));
			...
			case SchedulerConstants.SCH_FREQUENCY_DAILY: // Daily
			{
				trigger = (new CalendarIntervalTriggerImpl(triggerName, schedule.getGroup(),
						DateBuilder.IntervalUnit.DAY, 1));
				((CalendarIntervalTriggerImpl) trigger).setStartTime(new Date(schedule.getTime()));
				// trigger.setName();
				break;
			}
			...
			{
				trigger = new CalendarIntervalTriggerImpl(triggerName, schedule.getGroup(),
						DateBuilder.IntervalUnit.DAY, 7);
				((CalendarIntervalTriggerImpl) trigger).setStartTime(new Date(schedule.getTime()));
				break;
			}
			case SchedulerConstants.SCH_FREQUENCY_TWELVE_HOURS: {
			...
			default: // Once
			{
				trigger = new SimpleTriggerImpl(triggerName, schedule.getGroup(), new Date(schedule.getTime()),
						new Date(schedule.getTime() + 60000), 0, 0);
				break;
			}
			}
			...
			scheduler.scheduleJob(jobDetailImpl, trigger);
			AppLogger.getLogger().fatal("A job " + jobDetailImpl.getName()
					+ " is re-scheduled successfully and will be triggerd at " + trigger.getStartTime());
		} catch (Exception e) {
			AppLogger.getLogger().fatal("Error Occured while scheduling job: " + e.getMessage(), e);
		}
	}
