...
public class QueryJobExecutor implements Job 
{
	...
	private String executeQueryJob(String queryID, String namenodeId,String userName, ArrayList usersIdList, String message, String format)
	{
		...
		if(object!=null)
		{
			...
			JSONObject queryObj = BigQueryManager.isQueryCompleteInternal(executionId, false, format, RemoteManager.getLoggedInUser());
			...
			while(queryObj.get("error")==null)
			{
				...
				queryObj = BigQueryManager.isQueryCompleteInternal(executionId, false, format, RemoteManager.getLoggedInUser());
	...
	private void mailGeneratedBigQueryReport(ArrayList fileList, String message, ArrayList usersList) throws Exception
	{
		...
		{
			NotificationHandler.generateEmailNotification(notifMgr, "Scheduled Big-Query Report", message, notify, usersList, true);
		}
		else
		{
	...
	private void logGeneratedBigQueryReport(ArrayList fileList, String message) throws Exception
	{
		...
		{
			NotificationHandler.generateLogNotification(notifMgr, "[Scheduled Big-Query Report] - " + message + fileList.toString(), notify, true);
		}
		else
		{
			throw new Exception("Error Generating Notification");
