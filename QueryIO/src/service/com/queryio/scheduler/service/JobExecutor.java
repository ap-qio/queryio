...
public class JobExecutor implements Job {
	...
	public void executeJob(JobExecutionContext jobExecutionContext) throws Exception {
		...
		try {
			...
			Date end = new Date();
			endDate = String.valueOf(end.getMonth() + 1) + "/" + String.valueOf(end.getDate()) + "/"
					+ String.valueOf(end.getYear() + 1900) + " " + String.valueOf(end.getHours()) + ":"
					+ String.valueOf(end.getMinutes()) + ":" + String.valueOf(end.getSeconds());
			...
			startDate = String.valueOf(start.getMonth() + 1) + "/" + String.valueOf(start.getDate()) + "/"
					+ String.valueOf(start.getYear() + 1900) + " " + String.valueOf(start.getHours()) + ":"
					+ String.valueOf(start.getMinutes()) + ":" + String.valueOf(start.getSeconds());
		} catch (Exception e) {
			AppLogger.getLogger().fatal(e.getMessage(), e);
		...
		if (Boolean.parseBoolean(notificationEnable)) {
			if (reportTypeList.size() > 0) {
				if (reportTypeList.contains(Integer.toString(QueryIOConstants.NN_DETAIL))
						|| reportTypeList.contains(Integer.toString(QueryIOConstants.DN_DETAIL))
						|| reportTypeList.contains(Integer.toString(QueryIOConstants.REPORT_RM_DETAIL))
						|| reportTypeList.contains(Integer.toString(QueryIOConstants.REPORT_NM_DETAIL))) {
					if (notificationType.equals(SchedulerConstants.NOTIFICATION_EMAIL)) {
						dwrResponse = ReportManager.mailNodeReport((String) jDataMap.get(SchedulerConstants.NODE_ID),
								exportFormatList, usersIdList, jobExecutionContext.getJobDetail().getKey().getName(),
								startDate, endDate);
					} else {
						dwrResponse = ReportManager.logNodeReport((String) jDataMap.get(SchedulerConstants.NODE_ID),
								exportFormatList, jobExecutionContext.getJobDetail().getKey().getName(), startDate,
								endDate);
					}
				} else {
					if (notificationType.equals(SchedulerConstants.NOTIFICATION_EMAIL)) {
						dwrResponse = ReportManager.mailGeneralReport(reportTypeList, exportFormatList, usersIdList,
								jobExecutionContext.getJobDetail().getKey().getName(), startDate, endDate);
					} else {
						dwrResponse = ReportManager.logGeneralReport(reportTypeList, exportFormatList,
								jobExecutionContext.getJobDetail().getKey().getName(), startDate, endDate);
					}
				}
			}
			...
			if (reportTypeList.size() > 0) {
				if (reportTypeList.contains(Integer.toString(QueryIOConstants.NN_DETAIL))
						...
						|| reportTypeList.contains(Integer.toString(QueryIOConstants.REPORT_NM_DETAIL))) {
					dwrResponse = ReportManager.mailNodeReport((String) jDataMap.get(SchedulerConstants.NODE_ID),
							exportFormatList, null, jobExecutionContext.getJobDetail().getKey().getName(), startDate,
							endDate);
				} else {
					dwrResponse = ReportManager.mailGeneralReport(reportTypeList, exportFormatList, null,
							jobExecutionContext.getJobDetail().getKey().getName(), startDate, endDate);
				}
			}
		}
