Rack_Summary={
		...
		ready : function(){
			...
			var temp =Navbar.selectedRack;
			Navbar.selectedRack = Navbar.selectedTabId;
			Navbar.selectedTabId = temp;
			RemoteManager.getAllDataNodesSummaryTableforRack(Navbar.selectedRack,false,Rack_Summary.populateRackDetailSummaryTable);
			RemoteManager.getAllDataNodeStatusSummaryforRack(Navbar.selectedRack,Rack_Summary.fillStatusTable);
			RemoteManager.getAllDataNodeMemoryInfoForRack(Navbar.selectedRack,Rack_Summary.fillSummaryChart);
			RemoteManager.getAllDataNodesSummaryTable(false,Rack_Summary.setDataSummaryValue);
		...
		fillStatusTable : function(list)
		{
	    	...
	    	var div_data='<div id="rack_status_title" class="header" style="width: 99%">Status summary</div>';
		...
		{
			if(document.getElementById('rack_summary_chart')==undefined||document.getElementById('rack_summary_chart')==null)return;
			if (list == null || list == undefined)
			{
				$('#rack_summary_chart').append('<div style="margin: 30px 0 0 15px; height:200px; background: url(images/noData.png) no-repeat"></div>');
				return;
			}
			...
			else
			{
				...
				$.jqplot("rack_summary_chart", [seriesArray], {
					title: {show: false},
		...
		populateRackDetailSummaryTable :function(summaryTable)
		{
			
		
			if(document.getElementById('rack_summary_table_div')==undefined||document.getElementById('rack_summary_table_div')==null)return;
			if (summaryTable == null || summaryTable == undefined)
			{
				$("#rack_summary_table_div").html('<span>DataNode Details not available. </span>');
				return;
			}
			var flag=true;
//			var diskstatusArray = new Array();
			...
			$('#rack_summary_table').html(table_data);
			$("#rack_summary_table").treeTable(
					{
						expandable: true,
