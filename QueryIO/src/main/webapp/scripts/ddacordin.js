...
var ddaccordion={
    ...
    jQuery(document).ready(function($){
        ...
        var persistedheaders=ddaccordion.getCookie(config.cookieprefix + config.headerclass)
        ...
        $('.'+config["headerclass"]).bind(config.revealtype, function(e){
            if (config.revealtype=="mouseenter"){
                ...
                // if user clicked console status link, return before toggling
                if (targ.className == 'consolestatuslink')
                {
                    return true;
                }
                $(this).trigger("evt_accordion")
                
                // if element contains an onclick, let it fire
                if ($(this).attr('class').indexOf('menuitemspan') != -1)
                {
                    if ($(this).attr('onclick') != null && $(this).attr('onclick').length > 0)
                    {
                ...
                // menu item contains this class and is href, fire it
                if ($(this).attr('class').indexOf('showhide') != -1 && $(this).get(0).href != null && $(this).get(0).href.length > 0)
                {
                    return true
                }
        ...
        $(window).bind('unload', function(){ //clean up and persist on page unload
            ...
            if (config.persiststate==true){ //persist state?
                ...
                ddaccordion.setCookie(config.cookieprefix + config.headerclass, expandedindices)
            }
        })
    })
    }
