...
public class ExportFromHDFSToDataBase implements Runnable {
	...
	public ExportFromHDFSToDataBase(String user, String group, MigrationInfo migrationInfo, PoolingDataSource ps,
			String responseJSON, Configuration conf, Path path) throws Exception {
	...
	public void parseLine(String line, PreparedStatement ps) throws Exception {
		String regex = null;
		int i = 1;
	...
	private Object castValueToCorrespondingObject(String value, int index) {
		...
		try {
			if (dataType[index].trim().toUpperCase().startsWith("VARCHAR")) {
				return value;
			} else if (dataType[index].trim().toUpperCase().startsWith("DATETIME")) { // To
																						// handle
																						// condition
																						// as
																						// SchemeDetection
