...
public class HDFSExportMigrationThread extends Thread {
	...
	public HDFSExportMigrationThread(String user, String group, MigrationInfo migrationInfo, String host, String path,
			String userHDFS, String groupHDFS, Configuration conf, boolean overwrite) throws Exception {
		super(user);
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("HDFS export request received");

		this.connect();

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Connected to HDFS");
						...
						try {
							...
							if (connectRetry == 3)
								...
								try {
									if (AppLogger.getLogger().isDebugEnabled())
										AppLogger.getLogger()
												.debug("IOException occurred...Reconnecting to FTP server...");
									this.connect();
									if (AppLogger.getLogger().isDebugEnabled())
	...
	private List getAllFilePaths(String str) throws Exception {
		...
		if (!stat.isDirectory()) {
			...
			for (int i = 0; i < stats.length; i++) {
				...
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("list: " + stats[i].getPath().getName());
			}
		}
		return list;
	}
	...
	public long writeToStream(InputStream stream, OutputStream baos) throws IOException {
		...
		while ((bytesIn = stream.read(readBuffer, 0, readBuffer.length)) != -1) {
			...
			baos.flush();
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("written " + readSoFar + " bytes");
		}
		return readSoFar;
	}
