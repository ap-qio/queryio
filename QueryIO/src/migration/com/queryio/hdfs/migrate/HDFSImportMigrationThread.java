...
public class HDFSImportMigrationThread extends Thread {
	...
	public HDFSImportMigrationThread(String user, String group, MigrationInfo migrationInfo, String host, String path,
			Configuration conf, boolean overwrite) throws Exception {
		this.loginUser = user;
							...
							if (connectRetry == 3) {
								...
								try {
									AppLogger.getLogger().fatal("IOException occurred... exception: " + e.getMessage(),
											e);
									if (AppLogger.getLogger().isDebugEnabled())
										AppLogger.getLogger().debug("Reconnecting to HDFS server...");
									...
									connectRetry++;
									AppLogger.getLogger()
											.fatal("Could not connect to HDFS, Exception: " + ex.getMessage(), ex);
								}
								i--;
								continue;
							}
