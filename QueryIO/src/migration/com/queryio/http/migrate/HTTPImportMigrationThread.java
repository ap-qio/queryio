...
public class HTTPImportMigrationThread extends Thread {
	...
	public HTTPImportMigrationThread(String user, String group, MigrationInfo migrationInfo, String baseURL,
			String userName, String password, String file, String encoding, Configuration conf, boolean overwrite)
			throws Exception {
		this.loginUser = user;
			...
			for (int i = 0; i < count; i++) {
				if (flag) {
					...
					try {
						if (is != null) {
							if (AppLogger.getLogger().isDebugEnabled())
								AppLogger.getLogger().debug("importedFilePath: " + importedFilePath);
						...
						AppLogger.getLogger().fatal(
								"Could not save file. Source: " + importedFilePath + ", exception: " + e.getMessage(),
								e);
	...
	private Path createObject(Path objectPath, boolean overwrite, boolean unzip) throws Exception {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Closing object: " + objectPath.toString());
			try {
	...
	public long writeToStream(OutputStream baos) throws IOException {
		...
		while ((bytesIn = is.read(readBuffer, 0, readBuffer.length)) != -1) {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug(bytesIn + " bytes");
			readSoFar += bytesIn;
			baos.flush();
		}
		return readSoFar;
	...
	public void connect() throws Exception {
		...
		httpclient.getCredentialsProvider()
				.setCredentials(new AuthScope(AuthScope.ANY_HOST, AuthScope.ANY_PORT, AuthScope.ANY_REALM), cred);
		if (file.startsWith("/")) {
