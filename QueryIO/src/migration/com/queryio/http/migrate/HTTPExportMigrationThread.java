...
public class HTTPExportMigrationThread extends Thread {
	...
	public HTTPExportMigrationThread(String user, String group, MigrationInfo migrationInfo, String userName, String password, Configuration conf, boolean overwrite) throws Exception{
		super(user);
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("HTTP export request received");
		
		this.connect();
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Connected to HTTP server");
					...
					try
					{
						...
						catch(Exception e)
						{
							...
							else 
							{
								try
								{
									if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("IOException occurred...Reconnecting to HTTP server...");
								...
								catch(Exception ex)
								{
									Thread.sleep(1000);
									connectRetry ++;
									AppLogger.getLogger().fatal("Could not connect to HTTP server, exception: " + ex.getMessage(), ex);
								}
								i--;
								continue;
							}
						...
						try
						{
							HttpFileObject obejct = (HttpFileObject) manager.resolveFile(destPath.toString(), fileSystemOptions);
							obejct.createFile();
