...
public class SFTPImportMigrationThread extends Thread {
	...
	public SFTPImportMigrationThread(String user, String group, MigrationInfo migrationInfo, String username,
	...
	public void run() {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Listing files");
					...
					try {
						...
						if (inputStream != null) {
							...
							throw new IOException(
									"Output stream could not be opened for source file: " + object.getName().getPath());
						}
					} catch (Exception e) {
						connectRetry = 0;
						AppLogger.getLogger().fatal("Could not save file. Source: " + object.getName().getPath()
	...
	public List getAllFilePaths(FileObject object) throws IOException {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Listing paths for: " + object.getName().getPath());
