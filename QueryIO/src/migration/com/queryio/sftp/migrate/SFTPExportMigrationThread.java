...
public class SFTPExportMigrationThread extends Thread {
	...
	public SFTPExportMigrationThread(String user, String group, MigrationInfo migrationInfo, String userName, String password, Configuration conf, boolean overwrite) throws Exception
	{
		super(user);
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("SFTP export request received");
		
		this.connect();
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Connected to SFTP server");
...
//						destPath = new Path(destinationPath + filePath.getName().substring(filePath.getName().lastIndexOf('/') + 1));
						
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Saving data: " + destPath.toString());
						
						try
						{
							SftpFileObject obejct = (SftpFileObject) manager.resolveFile(destPath.toString(), fileSystemOptions);
							obejct.createFile();
						...
						catch(FileSystemException e)
						{
							...
							else 
							{
								try
								{
									if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("IOException occurred. Reconnecting to SFTP server.");
									this.connect();
									if(manager.resolveFile(destPath.toString(), fileSystemOptions) == null || !manager.resolveFile(destPath.toString(), fileSystemOptions).exists())
										throw e;
								...
								catch(Exception ex)
								{
									Thread.sleep(1000);
									connectRetry ++;
									AppLogger.getLogger().fatal("Could not connect to SFTP server, exception: " + ex.getMessage(), ex);
								}
								i--;
								continue;
							}
						...
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Saving file: " + destPath.toString() + " successful");
						successCount ++;
					} 
					catch(Exception e)
					{
