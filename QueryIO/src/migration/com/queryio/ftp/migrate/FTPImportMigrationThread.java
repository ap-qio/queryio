...
public class FTPImportMigrationThread extends Thread {
	...
	public FTPImportMigrationThread(String user, String group, MigrationInfo migrationInfo, String userName,
	...
	public void run() {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("size of folder " + sourcePath + " " + filePaths.size());
			...
			for (int i = 0; i < filePaths.size(); i++) {
				if (flag) {
					...
					Path importedFilePath = getImportedFilePath(migrationInfo.getDestinationPath(),
					...
					try {
						try {
							...
							if (connectRetry == 3) {
								...
								try {
									AppLogger.getLogger().fatal("IOException occurred..., exception: " + e.getMessage(),
											e);
									...
									if (client.isConnected()) {
										...
										if (AppLogger.getLogger().isDebugEnabled())
											AppLogger.getLogger().debug("Reconnecting to FTP server...");
										this.connect();
										if (AppLogger.getLogger().isDebugEnabled())
									...
									AppLogger.getLogger().fatal(
											"Could not connect to FTP server, exception: " + ex.getMessage(), ex);
								}
								i--;
								continue;
							}
						...
						if (inputStream != null) {
							try {
								migrationInfo
										.setStatus("Migrating " + (i + 1) + " object of " + totalFileCount + " object");
						...
						if (AppLogger.getLogger().isDebugEnabled())
							AppLogger.getLogger().debug("Completing pending command");
						...
						if (AppLogger.getLogger().isDebugEnabled())
							AppLogger.getLogger().debug("Completed pending command");
					} catch (Exception e) {
						connectRetry = 0;
						AppLogger.getLogger().fatal(
								"Could not save file. Source: " + filePath + ", exception: " + e.getMessage(), e);
	...
	public List getAllFilePaths(String str) throws IOException {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Listing paths for: " + str);
		...
		for (int i = 0; i < fileList.length; i++) {
			...
			if (str.endsWith("/"))
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("File: " + new Path(str + fileList[i].getName()).toString());
				else if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("File: " + new Path(str + "/" + fileList[i].getName()).toString());

			if (str.endsWith("/"))
				list.add(new Path(str + fileList[i].getName()));
			else
				list.add(new Path(str + "/" + fileList[i].getName()));
		}
	...
	private InputStream getObject(Path path) throws Exception {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Retreiving input stream for file: " + path.toString());
	...
	private void createFolder(String bucketName) throws Exception {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("creating folder " + bucketName);
	...
	private Path createObject(Path objectPath, InputStream inputStream, boolean overwrite, boolean unzip)
			throws Exception {
		if (AppLogger.getLogger().isDebugEnabled())
			...
			try {
				...
				try {
					qioOutputStream = new QIODFSOutputStream(dfs, dfsOutputStream, migrationInfo.getCompressionType(),
							migrationInfo.getEncryptionType(), null, objectPath.toUri().toString());
				} catch (Exception e) {
					if (dfsOutputStream != null) {
						dfsOutputStream.close();
					}
