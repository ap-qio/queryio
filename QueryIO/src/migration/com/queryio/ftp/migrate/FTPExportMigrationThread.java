...
public class FTPExportMigrationThread extends Thread {
	...
	public FTPExportMigrationThread(String user, String group, MigrationInfo migrationInfo, String userName,
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("FTP export request received");
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Connected to FTP server");
			...
			for (int i = 0; i < filePaths.size(); i++) {
				if (flag) {
					...
					try {
						...
						if (AppLogger.getLogger().isDebugEnabled())
							AppLogger.getLogger().debug("Creating directory on remote server: " + parent);
						...
						if (AppLogger.getLogger().isDebugEnabled())
							AppLogger.getLogger().debug(
									"Saving file: " + destPath.toString() + " " + (success ? "successful" : "failed"));
						successCount++;
						connectRetry = 0;
					} catch (Exception e) {
						connectRetry = 0;
