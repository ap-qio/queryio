...
public class AmazonExportMigrationThread extends Thread {
	...
	public AmazonExportMigrationThread(String user, String group, MigrationInfo migrationInfo, String accessKey,
	...
	public void run() {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("size of bucket " + migrationInfo.getSourcePath() + " " + filePaths.size());
			...
			for (int i = 0; i < filePaths.size(); i++) {
				if (flag) {
					...
					try {
						...
						if (AppLogger.getLogger().isDebugEnabled())
							AppLogger.getLogger().debug("Destination path: " + migrationInfo.getDestinationPath());
						...
						try {
							createObject(bucketName, innerKey != null ? (innerKey + fileName) : fileName, data,
									metadata);
						} catch (Exception e) {
							AppLogger.getLogger().fatal(e.getMessage(), e);
							failCount++;
						...
						AppLogger.getLogger().fatal(
								"Could not save file. Source: " + filePaths.get(i) + ", exception: " + e.getMessage(),
								e);
					}
	...
	private Map getMetadata(String filePath) throws Exception {
		...
		try {
			...
			IDataTagParser tagParser = UserDefinedTagResourceFactory.getParserFromConstructor(this.conf, filePath, null,
					null);
	...
	private void createBucket(String bucketName) throws Exception {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("creating bucket " + bucketName);
		Response response = conn.createBucket(bucketName, null);
		if (response.connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
			throw new RuntimeException("couldn't create bucket: expected " + HttpURLConnection.HTTP_OK + " but got "
					+ response.connection.getResponseCode() + " Response Message: "
	...
	private void createObject(String bucketName, String key, byte[] data, Map metadata) throws Exception {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger()
					.debug("creating object " + key + " of size " + data.length + " bytes in bucket " + bucketName);
		Response response = conn.put(bucketName, key, new S3Object(data, metadata), null);
		if (response.connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
			throw new RuntimeException("couldn't create object: expected " + HttpURLConnection.HTTP_OK + " but got "
					+ response.connection.getResponseCode() + " Response Message: "
					+ response.connection.getResponseMessage());
		}
	}
	...
	private void deleteObject(String bucketName, String key) throws Exception {
		if (key.startsWith("/"))
			key = key.substring(1);
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("deleting object " + key + " from bucket " + bucketName);
		Response response = conn.delete(bucketName, key, null);
		if (response.connection.getResponseCode() != HttpURLConnection.HTTP_OK) {
			throw new RuntimeException("couldn't delete entry: expected " + HttpURLConnection.HTTP_NO_CONTENT
					+ " but got " + response.connection.getResponseCode() + " Response Message: "
					+ response.connection.getResponseMessage());
		}
	}
}
