...
public class SSHExportMigrationThread extends Thread
{
	...
	public SSHExportMigrationThread(String user, String group, MigrationInfo migrationInfo, String host, int port, String username, String password, String path, boolean isSSHKey, Configuration conf, boolean overwrite) throws Exception
	{
		this.loginUser = user;
...
//				UserGroupInformation.getLoginUser(us.getUserName(), SecurityHandler.decryptData(us.getPassword()));		
	...
	public void run()
	{
		...
		try
		{
			...
			if(flag)
			{
				...
				try
				{
					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Fetching object: " + destinationPath);
					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Fetching fileName: " + fileName);
					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Destination: " + importedFilePath);

					migrationInfo.setProgress(0);
					migrationInfo.setStatus("Migrated "+successCount +" of "+ totalFiles +" objects." + (failCount > 0 ? " Failed cases: " + failCount + "." : ""));
					updateMigrationStatusInDB();
					
					FileStatus fileStatus = dfs.getFileStatus(sourcePath);
					if(fileStatus.isDirectory())
					{
						createFolder(migrationInfo.getDestinationPath() + File.separator + fileStatus.getPath().getName().toString());
						exportRecursive(sourcePath, migrationInfo.getDestinationPath() + File.separator + fileStatus.getPath().getName().toString());//, fileStatus);
					}
					else
	...
	private void exportFile(Path sourcePath, Path importedFilePath) throws Exception{
		...
		migrationInfo.setStatus("Migrated "+successCount +" of "+ totalFiles +" objects." + (failCount > 0 ? " Failed cases: " + failCount + "." : ""));
		if((successCount + failCount) % unitCount == 0) {
			updateMigrationStatusInDB();
		}
	}
	...
	private void executeCommand(String commandToExecute, InputStream fis, long fileLength, Path filePath, boolean overwrite, boolean unzip) throws Exception
	{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Command: " + commandToExecute);
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("FilePath: " + filePath);
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("FileLength: " + fileLength);
		...
		try
		{
			...
			if (checkAck(in) != 0) {
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Error in IN");
			}
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Command1: " + command);
			...
			if (checkAck(in) != 0) 
			{
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Error in IN2");
			}
...
//				out.flush();
			...
			if (checkAck(in) != 0) {
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Error in IN3");
			}
			...
			finally
			{
				...
				finally
				{
					...
					finally
					{
						...
						{
							channel.disconnect();
							if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("RemoteScpTo complete");
						}
						catch(Exception ee)
						{ 
							AppLogger.getLogger().fatal(ee);
...
//      copying a file
	...
	public static void main(String args[])
	{
		...
		try 
		{
//			new SSHExportMigrationThread("a", "queryio", null, "192.168.0.23", 22, "admin", "App4ever#", "a", conf, true);
		} 
		catch (Exception e) 
		{
			e.printStackTrace();
