	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			while (iterator.hasNext()) {
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(index ++);
				FileItemStream	item = iterator.next();
				if (item.isFormField()) {
					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("item.getFieldName(): " + item.getFieldName());
					if(item.getFieldName().equals("tagName")){
						...
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("fileTagFT: " + fileTagFT);
					...
					if(headers!=null){
						...
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Content-Length: " + contentLength);
						
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Rcvd Headers");
						java.util.Iterator iter = headers.getHeaderNames();
						while(iter.hasNext()){
							String header = (String) iter.next();
							if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(header + " : " + headers.getHeader(header));
						}
					}
					...
					try{
						...
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("isIngest: " + isIngest);
						if(isIngest)
							file = new File(EnvironmentalConstants.getAppHome() + "/" + QueryIOConstants.MAPREDRESOURCE + "/"+ QueryIOConstants.TAGPARSER_JAR_DIR + "/" + name + "/" + item.getName());
						else
							file = new File(EnvironmentalConstants.getAppHome() + "/" + QueryIOConstants.MAPREDRESOURCE + "/"+ name + "/" +item.getName());
						if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("FilePath: " + file.getAbsolutePath());
					...
					try{
						...
						if(config != null){
							...
							for(String jarName : jars.split(",")){
								if(isIngest){
									jarName = EnvironmentalConstants.getAppHome() + "/" + QueryIOConstants.MAPREDRESOURCE + "/"+ QueryIOConstants.TAGPARSER_JAR_DIR + "/" + config.getTagName() + "/" + jarName;
								}else{
			...
			File libDir = null;
			if (isIngest)
				libDir = new File(EnvironmentalConstants.getAppHome() + "/" + QueryIOConstants.MAPREDRESOURCE + "/"+ QueryIOConstants.TAGPARSER_JAR_DIR + "/" + name);
			else
				libDir = new File(EnvironmentalConstants.getAppHome() + "/" + QueryIOConstants.MAPREDRESOURCE + "/"+ name);
			
			if (libDir.exists())
			{
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("extraJars: " + extraJars);
...
//			else{
//				try{
//					RemoteManager.insertTagParserConfig(filename, filedesc, jar, file_type, class_name, true);
//				}
//				catch(Exception e){
