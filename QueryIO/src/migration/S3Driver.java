...
public class S3Driver {
	...
	public static void main(String args[]) throws Exception {
		...
		QueryStringAuthGenerator generator = new QueryStringAuthGenerator(AWSACCESSKEYID, AWSSECRETACCESSKEY);
		...
		if (!conn.checkBucketExists(BUCKETNAME)) {
			System.out.println(conn.createBucket(BUCKETNAME, AWSAuthConnection.LOCATION_DEFAULT, null).connection
					.getResponseMessage());
			// sample creating an EU located bucket.
			// (note path-style urls will not work with location-constrained
			// buckets)
			// System.out.println(conn.createBucket(bucketName,
			// AWSAuthConnection.LOCATION_EU,
			// null).connection.getResponseMessage());
		}
		...
		System.out.println(conn.put(BUCKETNAME, KEYNAME + "-public", object, headers).connection.getResponseMessage());
		...
		// awsSecretAccessKey, true, Utils.DEFAULT_HOST,
		...
		System.out.println(conn.delete(BUCKETNAME, COPIEDKEYNAME, null).connection.getResponseMessage());
		System.out.println(conn.delete(BUCKETNAME, KEYNAME, null).connection.getResponseMessage());
		System.out.println(conn.delete(BUCKETNAME, KEYNAME + "-public", null).connection.getResponseMessage());
		...
		System.out.println(conn.deleteBucket(BUCKETNAME, null).connection.getResponseMessage());
	}
}
