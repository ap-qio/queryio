...
public class S3Test {
	...
	public static void main(String args[]) throws Exception {
		...
		// testing pure vanity domains (http://<vanity domain>/key) is not
		// covered here
	...
	private static void test(CallingFormat format, String location, boolean secure, String server) throws Exception {
		...
		System.out.println((secure ? "http" : "https") + " / " + server + " / "
				+ ((location == null) ? "<no-location>" : location) + " / " + format.getClass().getName());

		AWSAuthConnection conn = new AWSAuthConnection(AWSACCESSKEYID, AWSSECRETACCESSKEY, secure, server, format);
		QueryStringAuthGenerator generator = new QueryStringAuthGenerator(AWSACCESSKEYID, AWSSECRETACCESSKEY, secure,
				server, format);

		Response response = conn.createBucket(BUCKETNAME, location, null);
		assertEquals("couldn't create bucket", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());

		ListBucketResponse listBucketResponse = conn.listBucket(BUCKETNAME, null, null, null, null);
		assertEquals("couldn't get list", HttpURLConnection.HTTP_OK, listBucketResponse.connection.getResponseCode());
		...
		assertEquals("couldn't put simple object", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());
		...
		assertEquals("couldn't get list", listBucketResponse.connection.getResponseCode(), HttpURLConnection.HTTP_OK);
		...
		assertEquals("Unexpected list size", 1, listBucketResponse.entries.size());
		assertEquals("Unexpected common prefix size", 0, listBucketResponse.commonPrefixEntries.size());
		verifyBucketResponseParameters(listBucketResponse, BUCKETNAME, "test/", "", UNSPECIFIEDMAXKEYS, "/", false,
				null);

		// remove innerkey
		response = conn.delete(BUCKETNAME, innerKey, null);
		assertEquals("couldn't delete entry", HttpURLConnection.HTTP_NO_CONTENT, response.connection.getResponseCode());
		...
		response = conn.put(BUCKETNAME, key, new S3Object(text.getBytes(), null), null);
		assertEquals("couldn't reput simple object", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());
		...
		assertEquals("couldn't put complex object", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());

		GetResponse getResponse = conn.get(BUCKETNAME, key, null);
		assertEquals("couldn't get object", getResponse.connection.getResponseCode(), HttpURLConnection.HTTP_OK);
		...
		assertEquals("didn't get the right metadata back", 1, ((List) getResponse.object.metadata.get("title")).size());
		assertEquals("didn't get the right metadata back", "title",
				((List) getResponse.object.metadata.get("title")).get(0));
		assertEquals("didn't get the right content-length", "" + text.length(),
		...
		assertEquals("couldn't put metadata with leading and trailing spaces", HttpURLConnection.HTTP_OK,
				response.connection.getResponseCode());

		getResponse = conn.get(BUCKETNAME, key, null);
		assertEquals("couldn't get object", HttpURLConnection.HTTP_OK, getResponse.connection.getResponseCode());
		assertEquals("didn't get the right metadata back", getResponse.object.metadata.size(), 1);
		assertEquals("didn't get the right metadata back", 1, ((List) getResponse.object.metadata.get("title")).size());
		assertEquals("didn't get the right metadata back", titleWithSpaces.trim(),
				((List) getResponse.object.metadata.get("title")).get(0));

		String weirdKey = "&=//%# ++++";
		response = conn.put(BUCKETNAME, weirdKey, new S3Object(text.getBytes(), null), null);
		assertEquals("couldn't put weird key", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());

		getResponse = conn.get(BUCKETNAME, weirdKey, null);
		assertEquals("couldn't get weird key", HttpURLConnection.HTTP_OK, getResponse.connection.getResponseCode());

		// start acl test

		getResponse = conn.getACL(BUCKETNAME, key, null);
		assertEquals("couldn't get acl", HttpURLConnection.HTTP_OK, getResponse.connection.getResponseCode());

		byte[] acl = getResponse.object.data;

		response = conn.putACL(BUCKETNAME, key, new String(acl), null);
		assertEquals("couldn't put acl", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());

		getResponse = conn.getBucketACL(BUCKETNAME, null);
		assertEquals("couldn't get bucket acl", HttpURLConnection.HTTP_OK, getResponse.connection.getResponseCode());

		byte[] bucketACL = getResponse.object.data;

		response = conn.putBucketACL(BUCKETNAME, new String(bucketACL), null);
		assertEquals("couldn't put bucket acl", HttpURLConnection.HTTP_OK, response.connection.getResponseCode());

		// end acl test

		// bucket logging tests
		getResponse = conn.getBucketLogging(BUCKETNAME, null);
		assertEquals("couldn't get bucket logging config", HttpURLConnection.HTTP_OK,
		...
		assertEquals("couldn't put bucket logging config", HttpURLConnection.HTTP_OK,
				response.connection.getResponseCode());
		...
		assertEquals("couldn't get bucket request payment config", HttpURLConnection.HTTP_OK,
				getResponse.connection.getResponseCode());
		...
		assertEquals("couldn't put bucket request payment config", HttpURLConnection.HTTP_OK,
				response.connection.getResponseCode());
		...
		assertEquals("couldn't put simple object in a requester paid bucket", HttpURLConnection.HTTP_OK,
				response.connection.getResponseCode());
		...
		listBucketResponse = conn.listBucket(BUCKETNAME, null, null, null, null);
		assertEquals("couldn't list bucket", HttpURLConnection.HTTP_OK,
		...
		assertEquals("couldn't list bucket", HttpURLConnection.HTTP_OK,
				listBucketResponse.connection.getResponseCode());
		assertEquals("didn't get back the right number of entries", 1, listBucketResponse.entries.size());
		...
		for (Iterator it = entries.iterator(); it.hasNext();) {
			...
			assertEquals("couldn't delete entry", HttpURLConnection.HTTP_NO_CONTENT,
					response.connection.getResponseCode());
		}

		ListAllMyBucketsResponse listAllMyBucketsResponse = conn.listAllMyBuckets(null);
		assertEquals("couldn't list all my buckets", HttpURLConnection.HTTP_OK,
		...
		assertEquals("couldn't delete bucket", HttpURLConnection.HTTP_NO_CONTENT,
				response.connection.getResponseCode());

		listAllMyBucketsResponse = conn.listAllMyBuckets(null);
		assertEquals("couldn't list all my buckets", HttpURLConnection.HTTP_OK,
				listAllMyBucketsResponse.connection.getResponseCode());
		assertEquals("bucket count is incorrect", buckets.size() - 1, listAllMyBucketsResponse.entries.size());

		checkURL(generator.createBucket(BUCKETNAME, null), "PUT", HttpURLConnection.HTTP_OK, "couldn't create bucket");
		checkURL(generator.put(BUCKETNAME, key, new S3Object("test data".getBytes(), null), null), "PUT",
				HttpURLConnection.HTTP_OK, "put object", "test data");
		checkURL(generator.get(BUCKETNAME, key, null), "GET", HttpURLConnection.HTTP_OK, "get object");
		checkURL(generator.listBucket(BUCKETNAME, null, null, null, null), "GET", HttpURLConnection.HTTP_OK,
				"list bucket");
		checkURL(generator.listAllMyBuckets(null), "GET", HttpURLConnection.HTTP_OK, "list all my buckets");
		checkURL(generator.getACL(BUCKETNAME, key, null), "GET", HttpURLConnection.HTTP_OK, "get acl");
		checkURL(generator.putACL(BUCKETNAME, key, new String(acl), null), "PUT", HttpURLConnection.HTTP_OK, "put acl",
				new String(acl));
		checkURL(generator.getBucketACL(BUCKETNAME, null), "GET", HttpURLConnection.HTTP_OK, "get bucket acl");
		checkURL(generator.putBucketACL(BUCKETNAME, new String(bucketACL), null), "PUT", HttpURLConnection.HTTP_OK,
				"put bucket acl", new String(bucketACL));
		checkURL(generator.getBucketLogging(BUCKETNAME, null), "GET", HttpURLConnection.HTTP_OK, "get bucket logging");
		checkURL(generator.putBucketLogging(BUCKETNAME, new String(bucketLogging), null), "PUT",
				HttpURLConnection.HTTP_OK, "put bucket logging", new String(bucketLogging));
		checkURL(generator.delete(BUCKETNAME, key, null), "DELETE", HttpURLConnection.HTTP_NO_CONTENT, "delete object");
		checkURL(generator.deleteBucket(BUCKETNAME, null), "DELETE", HttpURLConnection.HTTP_NO_CONTENT,
				"delete bucket");

		System.out.println("OK (" + assertionCount + " tests passed)");
	}

	private static void verifyBucketResponseParameters(ListBucketResponse listBucketResponse, String bucketName,
			String prefix, String marker, int maxKeys, String delimiter, boolean isTruncated, String nextMarker) {
	...
	private static void assertEquals(String message, byte[] expected, byte[] actual) {
		...
		if (!Arrays.equals(expected, actual)) {
			throw new RuntimeException(
					message + ": expected " + new String(expected) + " but got " + new String(actual));
		}
	}
	...
	private static void checkURL(String url, String method, int code, String message, String data)
		...
		if ("PUT".equals(method)) {
			...
			// HttpURLConnection auto populates Content-Type, which we don't
			// want here
