...
public class XMLExporter extends AbstractExporter
{
	...
	public void exportResult(String location, ArrayList models) throws ExporterException 
	{
		...
		{
			throw new ExporterException(RM.getString("ERR_WRITING_FILE") + "Result_" + sCurrentProject + this.getFileExtension(), e);
		}
		finally
		{
	...
	private void writeNode(AbstractExportableNode node, TreeModel treeModel, String reportsLocation) throws ExporterException, IOException
	{
		this.node = node;
	...
	{
		if (!tabFolder.isXmlExport()) return;
	...
	private void exportTableModel(final TableModel model, String tableHeader)
	{
		...
		{
			if(tableHeader != null){
				this.writer.startElement("TableData", new String[] { "TableHeader", "NumRows", "NumColumns" }, new String[] {
						tableHeader, String.valueOf(model.getRowCount()), String.valueOf(model.getColumnCount()) });
			}else{
				this.writer.startElement("TableData", new String[] { "NumRows", "NumColumns" }, new String[] {
						String.valueOf(model.getRowCount()), String.valueOf(model.getColumnCount()) });
			}
			// write all rows
