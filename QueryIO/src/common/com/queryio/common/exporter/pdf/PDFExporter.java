...
public final class PDFExporter extends AbstractExporter {
	...
	public void exportNode(final AbstractExportableNode nodeToExport) throws ExporterException {
		...
		try {
			this.writer = PdfWriter.getInstance(this.document, new BufferedOutputStream(new FileOutputStream(
					new File(this.node.getFilePath(), this.node.getExportedFileName() + this.getFileExtension()))));
			this.addMetaData();
			this.document.open();
	...
	private void addMetaData() {
		// meta-data
		this.document.addTitle(this.sCurrentProject != null ? this.sCurrentProject
				: (this.node.getHeaderText() != null ? this.node.getHeaderText() : "AppPerfect Report"));
		this.document.addAuthor("AppPerfect Corporation"); //$NON-NLS-1$
		// document.addSubject("Generated by AppPerfect Corporation");
		// //$NON-NLS-1$
		this.document.addKeywords(
				"iText, AppPerfect, Load Test, App Test, Web Test, Code Test, Java Profiler, Unit Test, Agentless Monitor, Test Manager"); //$NON-NLS-1$
		...
		try {
					...
					for (int j = 0, k = 0; j < numCols; j++, ++k) {
						...
						if (j == 0) {
							PdfPCell cell = new PdfPCell(new Phrase(
									val == null ? IProductConstants.EMPTY_STRING : val.toString(), DEF_FONT));
							if (val == null)
								cell.setNoWrap(true);
							else
							...
							pdfTable.addCell(new Phrase(val == null ? IProductConstants.EMPTY_STRING : val.toString(),
									DEF_FONT));
						}
					}
				}
			...
			if (table.isTotal()) {
				...
				for (int j = 0, k = 0; j < numCols; j++, ++k) {
					...
					pdfTable.addCell(
							new Phrase(val == null ? IProductConstants.EMPTY_STRING : val.toString(), DEF_FONT));
	...
	protected void exportItem(final Chart chart) throws ExporterException {
		try {
			...
			// final int height = chart.getHeight() * this.iHeight /
			// (chart.isScrollable() ? 150:200);
			...
			if (chart.isScrollable()) {
				...
				if (scrollable) {
					...
					height = ((IScrollableChart) chart.getAppChart()).getTotalChartHeight(swingInterface, width,
							height);
					g2.dispose();
	...
	private void writeNode(Section section, AbstractExportableNode node, TreeModel treeModel, String reportsLocation)
			throws ExporterException {
		this.node = node;
