...
public final class HTMLExporter extends AbstractExporter implements HTMLConstants {
	...
	private void writeHeadTag() {
		...
		this.writer.writePlainTextLine(
				"\tdocument.getElementById(target).innerHTML = document.getElementById(id).innerHTML;"); //$NON-NLS-1$
		this.writer.writePlainTextLine("}"); //$NON-NLS-1$
		...
		if (this.bExportingResult) {
			this.writer.writePlainTextLine(
					"body { font-family: Tahoma, Arial, Helvetica, sans-serif; font-size: 75%; margin: 0; padding: 0; line-height: 1em; color: #000000; text-align: center; }"); //$NON-NLS-1$
			this.writer.writePlainTextLine(
					".bodyClass { position: relative; float: left; text-align: left; display: inline;  width: 100%; margin: 0; border-collapse: collapse; border: 1px solid #000000; }"); //$NON-NLS-1$
		} else {
			this.writer.writePlainTextLine(
					"body { font-family: Tahoma, Arial, Helvetica, sans-serif; font-size: 75%; margin: 5px 5px 5px 5px; padding: 0 0 0 0; line-height: 1em; color: #000000; text-align: center; }"); //$NON-NLS-1$
			this.writer.writePlainTextLine(
					".bodyClass { position: relative; float: left; text-align: left; display: inline;  width: 100%; margin: 0; border-collapse: collapse; border: 0px solid #000000;}"); //$NON-NLS-1$
		}
		this.writer.writePlainTextLine(
				"h1 { font-size: 3em; font-weight: normal; line-height: 1.3em; margin: 0 0 0.5em 0; padding: 0; }"); //$NON-NLS-1$
		this.writer.writePlainTextLine(
				"h2 { font-size: 1.3em; font-weight: normal; line-height: 1.3em; margin: 0 0 0.5em 0; padding: 0; }"); //$NON-NLS-1$
		this.writer.writePlainTextLine("h6 { font-size: 1em; line-height: 1.3em; margin: 0 0 0.25em 0; padding: 0; }"); //$NON-NLS-1$

		this.writer.writePlainTextLine(
				"table { font-size: 1em; margin: 0; padding: 0; border-collapse: collapse; border: 1px solid #a0a0a0; }"); //$NON-NLS-1$
		...
		this.writer.writePlainTextLine(
				"a.tab { background-image:url('images/menu.png') ; color: #FFF; font-weight: bold; line-height: 18px; margin-rigth: 10px; padding: 2px 10px 2px 10px; text-decoration: none; border: 1px solid #a0a0a0; font-family:Tahoma, Arial, Helvetica, sans-serif;font-size: 8pt; }"); //$NON-NLS-1$
		this.writer.writePlainTextLine(
				"a.tab:hover	{ color: #FFF; background-image:url('images/menu-hover.png'); font-family:Tahoma, Arial, Helvetica, sans-serif;font-size: 8pt;  }"); //$NON-NLS-1$
		this.writer.writePlainTextLine(
				"a.tab:active { color: #FFF; background-image:url('images/menu-active.png'); font-family:Tahoma, Arial, Helvetica, sans-serif;font-size: 8pt;  }"); //$NON-NLS-1$
	...
	private void startBodyTag() {
		...
		// this.writer.writeText(DateFormat.getDateTimeInstance(DateFormat.MEDIUM,
	...
	private void writeHeaderDiv() {
		...
		if (sHeaderImagePath != null) {
			try {
				...
				} else if (sHeaderImagePath.trim().length() > 0) // User has set
																	// it as
																	// some
																	// image
																	// link
	 ...
	 * @throws ExporterException
	 */
	private void createTree(final String reportsLocation, final TreeModel model, final String frameName,
			final int index) throws ExporterException {
		if (this.writer == null) {
			this.writer = new HTMLWriter();
		} else {
			this.writer.reInitialize();
	...
	private void createTabsFrame(final String reportsLocation, final String[] tabNames) throws ExporterException {
		...
		this.writer.startTag(HTML);
		this.writer.startTag(HEAD);
		this.writer.startTag(SCRIPT, new String[] { SRC, LANGUAGE },
				new String[] { "js/tabs.js", LANGUAGE_JAVASCRIPT });//$NON-NLS-1$
		this.writer.endTag(SCRIPT);
		this.writer.writeTag(LINK, new String[] { TYPE, REL, HREF },
				new String[] { TYPE_TEXT_CSS, STYLESHEET, "css/tree.css" }); //$NON-NLS-1$
		this.writer.endTag(HEAD);
		this.writer.startTag(BODY);
	...
	protected void exportItem(final Table table) {
		...
		if (model != null) {
			...
			if (table.getTableHeader() != null) {
				...
				this.writer.startTag(TD, new String[] { STYLE, COL_SPAN },
						new String[] { "text-align: center;", numCols + "" });
			...
			for (int i = 0; i < numCols; i++) {
				if ((colWidths.length > i) && (colWidths[i] > 0)) {
					colspan++;
					this.writer.startTag(TD, new String[] { WIDTH, ALIGN },
							new String[] { colWidths[i] + PERCT, LEFT }); // $NON-NLS-1$
			...
			for (int i = 0; i < numRows; i++) {
				...
				// else
				// {
				// this.writer.startTag(TR, new String[] { "bgcolor"}, new
				// String[] { "#F0F0F0"});
				// }
				for (int j = 0, k = 0; j < numCols; j++, ++k) {
					...
					if (val != null && val instanceof Color) {
						this.writer.startTag(TD, new String[] { "bgcolor" },
								new String[] { "#" + Color.toAWTColorString((Color) val) });
					} else {
			...
			if (table.isTotal()) {
				...
				for (int j = 0, k = 0; j < numCols; j++, ++k) {
					...
					this.writer
							.writeText((val == null) || (val.toString().trim().length() == 0) ? " " : val.toString()); //$NON-NLS-1$
					this.writer.endTag(TD);
				}
				this.writer.endTag(TR);
			}
	...
	protected void exportItem(final Label label) {
		this.writer.startTag(DIV, new String[] { STYLE },
				new String[] { this.getStyleString(new String[] { POSITION, FLOAT, WIDTH },
	...
	protected void exportItem(final Chart chart) throws ExporterException {
		String style = this.getStyleString(new String[] { POSITION, FLOAT, WIDTH, HEIGHT, BORDER, MARGIN },
				new String[] { POSITION_RELATIVE, FLOAT_LEFT, chart.getWidth() + PERCT,
						chart.getHeight() * this.iHeight / 100 + PX, BORDER_GRAY, "20 0 10 10" });
		this.writer.startTag(DIV, new String[] { STYLE }, new String[] { style });
		this.writer.writeTag(IMG, new String[] { SRC },
				new String[] {
						chart.getFileName(this.node.getCompleteFileName(ExportConstants.EXPORT_TYPE_HTML) + "_files", //$NON-NLS-1$
								chart.getWidth() * this.iWidth / 100, chart.getHeight() * this.iHeight / 100,
								this.imgCount++) });
		this.writer.endTag(DIV);
	}
	...
	private void exportTree(final Tree tree) {
		String style = this.getStyleString(new String[] { POSITION, FLOAT, WIDTH, HEIGHT }, new String[] {
				POSITION_RELATIVE, FLOAT_LEFT, tree.getWidth() + PERCT, tree.getHeight() * this.iHeight / 100 + PX });
		this.writer.startTag(DIV, new String[] { STYLE }, new String[] { style });
	...
	protected void exportItem(final TabFolder tabFolder) throws ExporterException {
		...
		if (tabNames != null && tabNames.length > 0) {
			...
			for (int i = 0; i < numTabs; i++) {
				...
				this.values = new String[] { "#", "tab", //$NON-NLS-1$ //$NON-NLS-2$
						"javascript:changeTab('" + targetId + "', '" + tabFolder.getId() + '_' + tabNames[i] + "');" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			...
			String style = this.getStyleString(new String[] { WIDTH, DISPLAY },
					new String[] { tabFolder.getWidth() + PERCT, DISPLAY_INLINE });
			...
			this.writer.startTag(SCRIPT, this.attrs, this.values);
			this.writer.writePlainTextLine(
					"changeTab('" + targetId + "', '" + tabFolder.getId() + '_' + tabNames[0] + "');"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			this.writer.endTag(SCRIPT);
		}
	}
 ...
 * final AbstractExportableNode root = (AbstractExportableNode) model.getRoot();
 * if (root.getItems() != null) { this.writeNodeDiv(root, model); } else { for
 * (int i = 0; i < model.getChildCount(root); i++) {
 * this.writeNodeDiv((AbstractExportableNode) root.getChildAt(i), model); } } }
 * 
 * private void writeNodeDiv(final AbstractExportableNode node, final TreeModel
 * model) throws ExporterException { this.node = node; final String style =
 * this.getStyleString(new String[] { POSITION, FLOAT, WIDTH, HEIGHT }, new
 * String[] { POSITION_RELATIVE, FLOAT_LEFT, 0 + PERCT, 0 + PX }); this.attrs =
 * new String[] { ID, STYLE }; this.values = new String[] { "view_" +
 * node.getExportedFileName(), style }; //$NON-NLS-1$ this.writer.startTag(DIV,
 * this.attrs, this.values); this.exportNodeItems(); this.writer.endTag(DIV);
 * 
 * for (int i = 0; i < model.getChildCount(node); i++) {
 * this.writeNodeDiv((AbstractExportableNode) node.getChildAt(i), model); } }
 * private void writeTree(final TreeModel model, final int index) { final
 * AbstractExportableNode root = (AbstractExportableNode) model.getRoot(); final
 * String imageName = ImageConstants.getImageName(root.getImageID()); final
 * String fName = StaticUtilities.jsEncode(root.getExportedFileName()); final
 * StringBuffer buff = new StringBuffer("var rootNode"); //$NON-NLS-1$
 * buff.append(index); buff.append(" = new Node(null, 'node_"); //$NON-NLS-1$
 * buff.append(fName); buff.append("', '"); //$NON-NLS-1$
 * buff.append(StaticUtilities.jsEncode(root.getDisplayName()));
 * buff.append("', 'view_"); //$NON-NLS-1$ if (root.getItems() != null) {
 * buff.append(fName); } else {
 * buff.append(StaticUtilities.jsEncode(this.getNextNodeId(root, model))); }
 * buff.append("', '', 'images/"); //$NON-NLS-1$ buff.append(imageName); if
 * (!this.alImages.contains(imageName)) { this.alImages.add(imageName); }
 * buff.append("');"); //$NON-NLS-1$
 * this.writer.writePlainTextLine(buff.toString()); for (int i = 0; i <
 * model.getChildCount(root); i++) { this.writeNode("node_" + fName,
 * (AbstractExportableNode) root.getChildAt(i), model); //$NON-NLS-1$ } }
 * 
 * 
 * public void exportResult_Old(final String location, final ArrayList models)
 * throws ExporterException { final int numTrees = models.size(); if (numTrees >
 * 1) {
 *
 * attrs = new String[] { POSITION, LEFT, TOP, WIDTH, HEIGHT, OVERFLOW, BORDER,
 * BACKGROUND_COLOR }; values = new String[] { DEF_POSITION, 0 + PX, 0 + PX,
 * WIDTH_LEFT * iWidth / 100 + PX, iHeight - 25 + PX, AUTO, DEF_BORDER,
 * BGCOLOR_WHITE };
 *
 * this.attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW, BORDER,
 * BACKGROUND_COLOR }; this.values = new String[] { POSITION_RELATIVE,
 * FLOAT_LEFT, WIDTH_LEFT + PERCT, this.iHeight - 25 + PX, DISPLAY_INLINE,
 * DEF_BORDER, BGCOLOR_WHITE }; String style = this.getStyleString(this.attrs,
 * this.values); this.attrs = new String[] { ID, STYLE }; this.values = new
 * String[] { "MainTreeSpan", style }; //$NON-NLS-1$ this.writer.startTag(DIV,
 * this.attrs, this.values); this.writer.endTag(DIV);
 *
 * attrs = new String[] { POSITION, LEFT, TOP, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; values = new String[] { DEF_POSITION, 0 + PX, iHeight - 25 + PX,
 * WIDTH_LEFT * iWidth / 100 + PX, 25 + PX, AUTO, DEF_BORDER };
 * 
 * this.attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; this.values = new String[] { POSITION_RELATIVE, FLOAT_LEFT, WIDTH_LEFT +
 * PERCT, 25 + PX, DISPLAY_INLINE, DEF_BORDER }; style =
 * this.getStyleString(this.attrs, this.values); this.attrs = new String[] {
 * STYLE }; this.values = new String[] { style }; this.writer.startTag(DIV,
 * this.attrs, this.values); for (int i = 0; i < numTrees; i++) { this.attrs =
 * new String[] { HREF, ONCLICK }; this.values = new String[] { "#",
 * "javascript:changeTab('MainTreeSpan', 'tree" + i + "');" }; //$NON-NLS-1$
 * //$NON-NLS-2$ //$NON-NLS-3$ this.writer.startTag(A, this.attrs, this.values);
 * this.writer.writeText(((TreeModel) models.get(i)).getRoot().toString() + '
 * '); this.writer.endTag(A); } this.writer.endTag(DIV); for (int i = 0; i <
 * numTrees; i++) {
 *
 * attrs = new String[] { POSITION, LEFT, TOP, WIDTH, HEIGHT, OVERFLOW }; values
 * = new String[] { DEF_POSITION, 0 + PX, 0 + PX, 0 + PX, 0 + PX, DEF_OVERFLOW
 * };
 *
 * this.attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW };
 * this.values = new String[] { POSITION_RELATIVE, FLOAT_LEFT, 0 + PERCT, 0 +
 * PX, DISPLAY_HIDDEN }; style = this.getStyleString(this.attrs, this.values);
 * this.attrs = new String[] { ID, STYLE }; this.values = new String[] { "tree"
 * + i, style }; //$NON-NLS-1$ this.writer.startTag(DIV, this.attrs,
 * this.values); this.writer.endTag(DIV); }
 * 
 * this.attrs = new String[] { LANGUAGE }; this.values = new String[] {
 * LANGUAGE_JAVASCRIPT }; this.writer.startTag(SCRIPT, this.attrs, this.values);
 * for (int i = 0; i < numTrees; i++) { this.writeTree((TreeModel)
 * models.get(i), i); this.writer
 * .writePlainTextLine("document.getElementById('tree" + i +
 * "').innerHTML=rootNode" + i + ".toHtmlString();"); //$NON-NLS-1$
 * //$NON-NLS-2$ //$NON-NLS-3$ }
 * this.writer.writePlainTextLine("changeTab('MainTreeSpan', 'tree0');");
 * //$NON-NLS-1$ this.writer.endTag(SCRIPT);
 *
 * attrs = new String[] { POSITION, LEFT, TOP, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; values = new String[] { DEF_POSITION, WIDTH_LEFT * iWidth / 100 + PX, 0 +
 * PX, WIDTH_RIGHT * iWidth / 100 + PX, iHeight + PX, AUTO, DEF_BORDER };
 *
 * this.attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; this.values = new String[] { POSITION_RELATIVE, FLOAT_LEFT, WIDTH_RIGHT +
 * PERCT, this.iHeight + PX, DISPLAY_INLINE, DEF_BORDER }; style =
 * this.getStyleString(this.attrs, this.values); this.attrs = new String[] { ID,
 * STYLE }; this.values = new String[] { "RightView", style }; //$NON-NLS-1$
 * this.writer.startTag(DIV, this.attrs, this.values); this.writer.endTag(DIV);
 * 
 * this.iWidth = WIDTH_RIGHT * this.iWidth / 100; for (int i = 0; i < numTrees;
 * i++) { this.writeAllDivs((TreeModel) models.get(i)); } } else {
 *
 * attrs = new String[] { POSITION, LEFT, TOP, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; values = new String[] { DEF_POSITION, 0 + PX, 0 + PX, WIDTH_LEFT * iWidth
 * / 100 + PX, iHeight + PX, AUTO, DEF_BORDER };
 *
 * this.attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; this.values = new String[] { POSITION_RELATIVE, FLOAT_LEFT, WIDTH_RIGHT +
 * PERCT, this.iHeight + PX, DISPLAY_INLINE, DEF_BORDER }; String style =
 * this.getStyleString(this.attrs, this.values); this.attrs = new String[] { ID,
 * STYLE }; this.values = new String[] { "MainTreeSpan", style }; //$NON-NLS-1$
 * this.writer.startTag(DIV, this.attrs, this.values); this.writer.endTag(DIV);
 * this.attrs = new String[] { LANGUAGE }; this.values = new String[] {
 * LANGUAGE_JAVASCRIPT }; this.writer.startTag(SCRIPT, this.attrs, this.values);
 * this.writeTree((TreeModel) models.get(0), 0); this.writer.
 * writePlainTextLine("var treeSpan = document.getElementById('MainTreeSpan');"
 * ); //$NON-NLS-1$ this.writer.
 * writePlainTextLine("treeSpan.innerHTML = rootNode0.toHtmlString();");
 * //$NON-NLS-1$ this.writer.endTag(SCRIPT);
 *
 * 
 * attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW, BORDER };
 * values = new String[] { DEF_POSITION, WIDTH_LEFT * iWidth / 100 + PX, 0 + PX,
 * WIDTH_RIGHT * iWidth / 100 + PX, iHeight + PX, AUTO, DEF_BORDER };
 *
 * this.attrs = new String[] { POSITION, FLOAT, WIDTH, HEIGHT, OVERFLOW, BORDER
 * }; this.values = new String[] { POSITION_RELATIVE, FLOAT_LEFT, WIDTH_RIGHT +
 * PERCT, this.iHeight + PX, DISPLAY_INLINE, DEF_BORDER }; style =
 ...
 * this.writer.startTag(DIV, this.attrs, this.values); this.writer.endTag(DIV);
 * 
 * this.iWidth = WIDTH_RIGHT * this.iWidth / 100; this.writeAllDivs((TreeModel)
 * models.get(0)); }
 * 
 * this.endBodyTag(); this.writer.endTag(HTML); this.writer.writeFile(location,
 * "Result_" + this.sCurrentProject + this.getFileExtension()); //$NON-NLS-1$ }
 */