...
public class ReportDesignHandler {
	...
	public static void buildReport(JSONObject properties, String designFilePath) throws Exception {
	...
	public void createReport(JSONObject properties, String designFilePath) throws Exception {
		...
		// AppLogger.getLogger().debug("JSON Properties");
		// if(AppLogger.getLogger().isDebugEnabled())
		...
		this.limitResultRowsValue = Integer
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Finished Designing.");
		// System.out.println("Finished Designing.");
	}

	// Scripted Data Set

	private void createDataSources(String dbName, String tableName, String nameNodeId, JSONObject properties)
			throws Exception {
		...
		try {
			...
			// in case of log4j or xml hive definition query. XML and log4j
			// files are processed by custom MR jobs instead of hive
			String fileType = HiveTableDAO.getFileType(queryIOConnection, tableName, nameNodeId);
			if (QueryIOConstants.ADHOC_TYPE_LOG.equalsIgnoreCase(fileType)
					|| QueryIOConstants.ADHOC_TYPE_XML.equalsIgnoreCase(fileType)) {
				isHive = false;
				connectionDetail = DBConfigDAO.getConnectionDetail("Hive");
				AppLogger.getLogger().debug("1createDataSources isHive: " + isHive + " connectionDetails: "
						+ connectionDetail.getPrimaryConnectionURL() + " fileType: " + fileType);
			}

			if (AppLogger.getLogger().isDebugEnabled()) {
				AppLogger.getLogger()
						.debug("2createDataSources isHive: " + isHive + " connectionDetails: "
								+ connectionDetail.getPrimaryConnectionURL() + " fileType: " + fileType + " tableName: "
								+ tableName);
			}
			...
			if (isHive) {
				...
				dsHandle = reportDesignHandle.getElementFactory().newOdaDataSource("Data Source",
				...
				dsHandle.setProperty("odaDriverClass",
						connectionDetail.getPrimaryDriverName()/* "org.hsqldb.jdbcDriver" */);
				dsHandle.setProperty("odaURL",
						connectionDetail.getPrimaryConnectionURL()/* "jdbc:hsqldb:hsql://127.0.0.1:5434/metastore" */);
	...
	private void createDataSets(String query, String dbName, JSONObject properties, String selectedTable,
		...
		if (isHive && skipInputfileQIO == null) {
			String startPart = query.substring(0,
					query.indexOf("FROM") > 0 ? query.indexOf("FROM") : query.indexOf("from"));
		...
		try {
			...
			props = CustomTagDBConfigManager.getDatabaseDataTypeMap(null,
					CustomTagDBConfigManager.getConfig(dbName).getCustomTagDBType());
			if (isHive) {
				HashMap configKeys = new HashMap();

				String pathFilter = configureHiveConnection(properties, selectedTable, nameNodeId, queryIOConnection,
						configKeys, false);
		...
		OdaDataSetHandle dsHandle = reportDesignHandle.getElementFactory().newOdaDataSet("Data Set",
		...
		// .toLowerCase() + File.separator // FIXME: Do we really need to
		// perform lower case operation on all of these? It requires lowering
		// the case in UI as well.
		// +( (String)
		// properties.get(BigQueryIdentifiers.NAMENODEID)).toLowerCase()
		// + File.separator
		// + ((String)
		// properties.get(BigQueryIdentifiers.QUERYID)).toLowerCase()
		// + ".json";
		// int recordFetch = -1;
		...
		// "rdh = new
		// Packages.com.queryio.common.cusrtomdatasource.ReportDataConnection();"+
		...
		// dataSet.getPropertyHandle(IDataSetModel.COLUMN_HINTS_PROP);
		// PropertyHandle resultSet =
		// dataSet.getPropertyHandle(IDataSetModel.RESULT_SET_PROP);
		// int i=0;
		// for (Object object : resultSetColumns.keySet()) {
		//
		...
		//
		// fetchScript += "dataSetRow[\""+columnName+"\"] =
		// reportDataHandler.getColumnValue(count,\""+columnName+"\");";
		//
		//// System.out.println("columnName: " + columnName + " dataType: " +
		// dataType + " props.getKeyFromValue(dataType): " +
		// props.getKeyFromValue(dataType));
		//
		// dataType = getDataType(props, dataType);
		//
		//
	...
	private String configureHiveConnection(JSONObject properties, String selectedTable, String nameNodeId,
			Connection queryIOConnection, HashMap<String, Object> configKeys, boolean isEncodePattern)
			throws Exception {
		...
		if (rmConf != null) {
			...
			if (adHocBean != null) {
				// encodes spaces to "+". Then replace all "+"(space) by %20.
				// "+" can not be decoded back to space by hive.
				configKeys.put(QueryIOConstants.HIVE_QUERYIO_FILEPATH_FILTER,
						isEncodePattern
								? URLEncoder.encode(adHocBean.getFilePathPattern(), "UTF-8").replaceAll("\\+", "%20")
		...
		if (isFilterQuery) {
			...
			if (filterQueryObj != null) {
				...
				//
				// if (filterQuery != null) {
				// JSONObject filterSelectedWhere = (JSONObject)
				// filterQueryObj.get(BigQueryIdentifiers.SELECTEDWHERE);
				//
				// if (filterSelectedWhere != null) {
				// JSONObject filterFilepath = (JSONObject)
				// filterSelectedWhere.get(BigQueryIdentifiers.FILTERQUERY_FILEPATH);
				// if (filterFilepath != null) {
				// String filePathValue = (String)
				// filterFilepath.get(BigQueryIdentifiers.FILTERQUERY_VALUE);
				// if (filePathValue != null) {
				// Configuration nnConf =
				// ConfigurationManager.getConfiguration(queryIOConnection,
				// nameNodeId);
				// String templateDir =
				// nnConf.get(QueryIOConstants.HIVE_QUERYIO_TEMPLATE_DIR,
				// AdHocHiveClient.templateDir);
				// if (!templateDir.endsWith("/")) {
				// templateDir = templateDir.concat("/");
				// }
				// templateDir =
				// templateDir.substring(templateDir.lastIndexOf(":") + 1);
				// templateDir =
				// templateDir.substring(templateDir.indexOf("/"));
				// filterQueryNew = filterQuery.replace(filePathValue,
				// templateDir.concat(pathFilter));
				// }
				// }
				// }
				// configKeys.put(QueryIOConstants.HIVE_QUERYIO_FILTER_QUERY,
				// filterQueryNew);
				// if(AppLogger.getLogger().isDebugEnabled())
				// AppLogger.getLogger().debug("filterQueryNew: " +
				// filterQueryNew);
				// }
				if (isEncodePattern) {
					filterQuery = URLEncoder.encode(filterQuery, "UTF-8"); // encodes
																			// spaces
																			// to
																			// "+"
					filterQuery = filterQuery.replaceAll("\\+", "%20"); // replace
																		// all
																		// "+"(space)
																		// by
																		// %20.
	...
	private String getDataType(DBTypeProperties props, String dataType) {
		if (props.getKeyFromValue(dataType) != null) {
			if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.STRING_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_STRING;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.TIMESTAMP_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_DATETIME;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.LONG_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_DECIMAL;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.SHORT_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_INTEGER;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.INTEGER_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_INTEGER;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.REAL_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_FLOAT;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.DOUBLE_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_FLOAT;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.DECIMAL_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_DECIMAL;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.BOOLEAN_WRAPPER_CLASS.toString()))
				dataType = DesignChoiceConstants.COLUMN_DATA_TYPE_BOOLEAN;
			else if (props.getKeyFromValue(dataType).toString()
					.equalsIgnoreCase(MetadataConstants.BLOB_WRAPPER_CLASS.toString()))
	...
	private void createMasterPages() throws SemanticException {
		...
		simpleMasterPage.setProperty("orientation", "landscape"); // TODO:
																	// Remove
																	// later
	...
	private void createBody() throws Exception {
		...
		SimpleMasterPageHandle masterPage = (SimpleMasterPageHandle) reportDesignHandle.getMasterPages().getContents()
				.get(0);
			...
			while (itQueryHeader.hasNext()) {
				...
				if (props != null) {
					...
					while (i.hasNext()) {
						...
						if (propName != null) {
							if (this.setHighFidelityOutput) {
								if ((propName.equals(SharedStyleHandle.BACKGROUND_COLOR_PROP))
										|| (propName.equals(SharedStyleHandle.COLOR_PROP))) {
									try {
		...
		// int colspan
		// =((Long)this.colSpanDetail.get("queryHeader")).intValue();
		...
		for (int i = 0; i < groupHeaders.size(); i++) {
			defaultComputedColumnBindings
					.add(computedCols.get(getIndex(computedCols, groupHeaders.get(i).getColumnName())));
		}
		...
		for (int i = 0; i < defaultComputedColumnBindings.size(); i++) {
			...
			// defaultComputedColumnBindings.get(i).getColumnName());
		}
		...
		for (int i = 0; i < groupHeaders.size(); i++) {
			...
			if (groupHeaderInfo.getFunction() != null && !groupHeaderInfo.getFunction().equals("")) {
				computedColumn.setExpression("Total." + groupHeaderInfo.getFunction() + "(dataSetRow[\""
						+ groupHeaderInfo.getColumnName() + "\"])");
			} else {
				computedColumn.setExpression("dataSetRow[\"" + groupHeaderInfo.getColumnName() + "\"]");
			}

			// System.out.println("GRP HEADER: " +
			// computedColumn.getColumnName());
		...
		for (int i = 0; i < groupFooters.size(); i++) {
			...
			if (groupFooterInfo.getFunction() != null && !groupFooterInfo.getFunction().equals("")) {
				computedColumn.setExpression("Total." + groupFooterInfo.getFunction() + "(dataSetRow[\""
						+ groupFooterInfo.getColumnName() + "\"])");
			} else {
				computedColumn.setExpression("dataSetRow[\"" + groupFooterInfo.getColumnName() + "\"]");
			}

			// System.out.println("GRP FOOTER: " +
			// computedColumn.getColumnName());
		...
		while (it.hasNext()) {
			...
			// colHandle.getWidth().setValue(getWidthFromType(resultSetColumns,
			// columnName));
			...
			if (o != null && o instanceof JSONObject) {
				...
				while (i.hasNext()) {
					...
					if (key.equals("title")) {
						...
						// try {
						// System.out.println("colHandle.getWidth().getAbsoluteValue().getMeasure()
						// after : " + colHandle.getWidth().toString());
						// System.out.println("colHandle.getWidth().getAbsoluteValue().getMeasure()
						// after : " +
						// colHandle.getWidth().getAbsoluteValue().getMeasure());
						// System.out.println("colHandle.getWidth().getAbsoluteValue().getUnit()
						// after : " +
						// colHandle.getWidth().getAbsoluteValue().getUnits());
						// } catch(Exception e) {
						// e.printStackTrace();
						// }
					}
		...
		for (int i = 0; i < groupHeaders.size(); i++) {
			...
			tcell = (CellHandle) groupHeader.getCells()
					.get(getIndex(defaultComputedColumnBindings, groupHeaderInfo.getColumnName()));
		...
		if (this.setCheckBookStyle) {
			...
			detail.setOnRender(
					"if (i == 0) { this.getStyle().backgroundColor = 'rgb(237, 243, 254)'; i=1; } else { this.getStyle().backgroundColor = 'lightgray'; i=0; }");
		}
		...
		for (int i = 0; i < defaultComputedColumnBindings.size(); i++) {
			...
			// tcell.setProperty("width", getWidthFromType(resultSetColumns,
			// columnName));
		...
		if (tableGroupHandles.size() > 0) {
			...
			for (int i = 0; i < groupFooters.size(); i++) {
				...
				tcell = (CellHandle) groupFooterRow.getCells()
						.get(getIndex(defaultComputedColumnBindings, groupFooterInfo.getColumnName()));
				...
				if (ySeriesJSON != null) {
					...
					} else // Page Header
					{
						if (headerGrid.getRows() != null) {
							...
							// int j=0;
							// AppLogger.getLogger().debug("1
							// "+headerGridRow.getCells());
							// if (headerGridRow.getCells() != null)
							// {
							//
							// AppLogger.getLogger().debug("2
							// "+row.getCells().get(j++));
							//
						...
						/*
						 * if (headerGrid.getRows() != null) { for (int i=0;
						 * i<=rowPosition; i++) { if
						 * (headerGrid.getRows().get(i)==null) {
						 * headerGrid.getRows().add(elementFactory.newTableRow()
						 * ); } } RowHandle headerGridRow = (RowHandle)
						 * headerGrid.getRows().get(rowPosition);
						 * chartComponent.setWidth(chartWidth); for (int j=1;
						 * j<resultSetColumns.size(); j++) { cell = null; try {
						 * cell = (CellHandle) headerGridRow.getCells().get(j);
						 * } catch(Exception e){ // Ignore //
						 * e.printStackTrace(); } if (cell == null) { cell =
						 * elementFactory.newCell(); if(j == colSpan){
						 * cell.getContent().add(chartComponent); //
						 * cell.getWidth().setValue(chartWidth); }
						 * headerGridRow.getCells().add(cell); continue; } if(j
						 * == colSpan){ cell = (CellHandle)
						 * headerGridRow.getCells().get(colSpan);
						 * 
						 * cell.getContent().add(chartComponent); //
						 * cell.getWidth().setValue(chartWidth);
						 * 
					...
					try {
						if (type.equalsIgnoreCase("PIE")) {
							...
							chartComponent.getReportItem().setProperty("chart.instance",
									getPieChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, this.isHive, yGrouping));
						} else if (type.equalsIgnoreCase("RADAR")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									RadarChartHandler.getRadarChart(title, dimension, chartWidth, chartHeight,
											xSeriesObj, ySeriesJSON, xSeriesTitle, ySeriesTitle, chartPreferences,
											this.isHive, yGrouping));
						} else if (type.equalsIgnoreCase("LINE")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									getLineChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, this.isHive, yGrouping,
											chartScale, yScaleMinVal));
						} else if (type.equalsIgnoreCase("BAR")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									getBarChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, "bar", this.isHive, yGrouping,
											chartScale, yScaleMinVal));
						} else if (type.equalsIgnoreCase("AREA")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									AreaChartHandler.getAreaChart(title, dimension, chartWidth, chartHeight, xSeriesObj,
											ySeriesJSON, xSeriesTitle, ySeriesTitle, chartPreferences, this.isHive,
											yGrouping));
							...
							chartComponent.getReportItem().setProperty("chart.instance",
									BubbleChartHandler.getBubbleChart(title, dimension, chartWidth, chartHeight,
											xSeriesObj, ySeriesJSON, ySeriesValues, ySeriesSizeObj, xSeriesTitle,
											ySeriesTitle, chartPreferences, resultSetColumns, this.dbName, this.isHive,
											yGrouping));
						} else if (type.equalsIgnoreCase("SCATTER")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									ScatterChartHandler.getChart(title, dimension, chartWidth, chartHeight, xSeriesObj,
											ySeriesJSON, xSeriesTitle, ySeriesTitle, chartPreferences, this.isHive,
											yGrouping));
						} else if (type.equalsIgnoreCase("PYRAMID")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									getBarChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, "pyramid", this.isHive,
											yGrouping, chartScale, yScaleMinVal));
						} else if (type.equalsIgnoreCase("TUBE")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									getBarChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, "tube", this.isHive,
											yGrouping, chartScale, yScaleMinVal));
						} else if (type.equalsIgnoreCase("CONE")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									getBarChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, "cone", this.isHive,
											yGrouping, chartScale, yScaleMinVal));
						} else if (type.equalsIgnoreCase("STACKED")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									getBarChart(title, dimension, chartWidth, chartHeight, xSeriesObj, ySeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, "stacked", this.isHive,
											yGrouping, chartScale, yScaleMinVal));
							...
							chartComponent.getReportItem().setProperty("chart.instance",
									GanttChartHandler.getChart(title, dimension, chartWidth, chartHeight, xSeries,
											ySeriesJSON, ySeriesStartDateJSON, ySeriesEndDateJSON, ySeriesLabelJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, resultSetColumns, this.dbName,
											this.isHive, yGrouping));
							...
							chartComponent.getReportItem().setProperty("chart.instance",
									DifferenceChartHandler.getChart(title, dimension, chartWidth, chartHeight, xSeries,
											ySeriesPostiveJSON, ySeriesNegativeJSON, xSeriesTitle, ySeriesTitle,
											chartPreferences, resultSetColumns, this.isHive, yGrouping));
							...
							chartComponent.getReportItem().setProperty("chart.instance",
									StockChartHandler.getChart(title, dimension, chartWidth, chartHeight, xSeries,
											ySeriesJSON, highSeriesJSON, lowSeriesJSON, openSeriesJSON, closeSeriesJSON,
											xSeriesTitle, ySeriesTitle, chartPreferences, resultSetColumns, this.dbName,
											this.isHive, yGrouping));
						} else if (type.equalsIgnoreCase("METER")) {
							chartComponent.getReportItem().setProperty("chart.instance",
									MeterChartHandler.getChart(title, dimension, chartWidth, chartHeight, xSeries,
											ySeriesJSON, xSeriesTitle, ySeriesTitle, chartPreferences, resultSetColumns,
											this.isHive, yGrouping));
						}
					} catch (Exception e) {
						e.printStackTrace();
	...
	private Chart getBarChart(String title, int dimension, int chartWidth, int chartHeight, JSONObject xSeriesObj,
			JSONObject ySeriesJSON, String xSeriesTitle, String ySeriesTitle, JSONObject chartPreferences,
			String riserType, boolean isHive, String yGrouping, String ySeriesScale, String yScaleMinVal)
			throws Exception {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("xAxisProperties JSON : " + xAxisProperties.toJSONString());
			...
			if (xAxisGridProperties != null && xAxisGridProperties.size() > 0) {
				...
				if (isXAxisGridLineVisible) {
					...
					xAxis.getMajorGrid().getLineAttributes()
							.setColor(ChartPreferenceHandler.hexToRGB(xAxisGridLineColor));
					xAxis.getMajorGrid().getLineAttributes().setStyle(
		...
		while (it.hasNext()) {
			...
			if (yAxisProperties != null && yAxisProperties.size() > 0) {
				...
				if (visible) {
					...
					if (yAxisLblOutLine != null && yAxisLblOutLine.size() > 0) {
						...
						if (isYAxisLblOutLineVisible) {
							...
							yAxisLbl.setOutline(LineAttributesImpl.create(
									ChartPreferenceHandler.hexToRGB(yAxisLblOutlineColor),
					...
					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger().debug("yAxisLblFontColor " + yAxisLblFontColor);
				...
				if (yAxisGridProperties != null && yAxisGridProperties.size() > 0) {
					...
					if (isYAxisGridLineVisible) {
						...
						yAxis.getMajorGrid().getLineAttributes().setStyle(
			...
			// yAxis.setOrigin(AxisOriginImpl.create(IntersectionType.VALUE_LITERAL,
	...
	private Chart getLineChart(String title, int dimension, int chartWidth, int chartHeight, JSONObject xSeriesObj,
			JSONObject ySeriesJSON, String xSeriesTitle, String ySeriesTitle, JSONObject chartPreferences,
			boolean isHive, String yGrouping, String ySeriesScale, String yScaleMinVal) throws Exception {
			...
			if (visible) {
				...
				if (xAxisProperties.containsKey("titleFontSize")) {
					...
					xAxis.getTitle().getCaption()
							.setFont(FontDefinitionImpl.create(sName, xAxisTitleFontSize, bBold, bItalic, bUnderline,
									bStrikethrough, bWordWrap, 0,
									ChartPreferenceHandler.getTextAlignment(xAxisLblTextAlign, null)));
					xAxis.getTitle().getCaption().getFont().setSize(xAxisTitleFontSize);
				}
				xAxis.getTitle().getCaption().getFont().setName(sName);
				xAxis.getTitle().getCaption().getFont().setBold(bBold);
		...
		while (it.hasNext()) {
			...
			JSONObject yAxisProperties = (JSONObject) chartPreferences.get("yAxisJson");
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("yAxisProperties JSON : " + yAxisProperties.toJSONString());
			if (yAxisProperties != null && yAxisProperties.size() > 0) {
				Boolean visible = (Boolean) yAxisProperties.get("visible");
				if (visible) {
					String position = (String) yAxisProperties.get("position");
	...
	private LeaderLineStyle getLeaderLineStyle(int style) {
		switch (style) {
		case LeaderLineStyle.FIXED_LENGTH:
			return LeaderLineStyle.FIXED_LENGTH_LITERAL;
		case LeaderLineStyle.STRETCH_TO_SIDE:
			return LeaderLineStyle.STRETCH_TO_SIDE_LITERAL;
		default:
			return LeaderLineStyle.FIXED_LENGTH_LITERAL;
		}
	}

	private Chart getPieChart(String title, int dimension, int chartWidth, int chartHeight, JSONObject xSeriesObj,
			JSONObject ySeriesJSON, String xSeriesTitle, String ySeriesTitle, JSONObject chartPreferences,
			boolean isHive, String yGrouping) throws Exception {
		...
		for (int i = 0; i < topColors.size(); i++) {
			xSeriesDef.getSeriesPalette().getEntries().add(i,
					ChartPreferenceHandler.hexToRGB((String) topColors.get(i)));
		}
		...
		while (it.hasNext()) {
			...
			if (leaderLinesProperties != null && leaderLinesProperties.size() > 0) {
				...
				if (visible) {
					...
					ls.setStyle(
							ChartPreferenceHandler.getLineStyleType((String) leaderLinesProperties.get("lineStyle")));
			...
			if (topColors != null) {
				...
				for (int j = 0; j < topColors.size(); j++) {
					ySeriesDef.getSeriesPalette().getEntries().add(j,
							ChartPreferenceHandler.hexToRGB((String) topColors.get(j)));
				}
			}
	...
	private String getNumberFormat(String category, JSONObject patternObject) throws Exception {
		...
		if (category.equals("Fixed")) {
			...
			// numberFormat = new DecimalFormat(realPattern, new
			...
			format = sb.toString();
		} else // (category.equals("General Number") ||
				// category.equals("Unformatted"))
		{
	...
	public static String hex2Rgb(String colorStr) {
		...
		return ColorDefinitionImpl.create(Integer.valueOf(colorStr.substring(0, 2), 16),
				Integer.valueOf(colorStr.substring(2, 4), 16), Integer.valueOf(colorStr.substring(4, 6), 16))
				.toString();
	}

	private static String getStyleFromName(HashMap allColumns, String columnName, String cssClass, String dbName)
			throws Exception {
...
class FormatPatternInfo {
	...
	/**
	 * @param useSymbolSpace
	 *            the useSymbolSpace to set
	 */
