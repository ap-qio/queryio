...
public class StockChartHandler {
	...
	public static Chart getChart(String title, int dimension, int chartWidth, int chartHeight, String xSeries,
			JSONObject ySeriesJSON, JSONObject highSeriesJSON, JSONObject lowSeriesJSON, JSONObject openSeriesJSON,
			JSONObject closeSeriesJSON, String xSeriesTitle, String ySeriesTitle, JSONObject chartPreferences,
		...
		while (itHigh.hasNext()) {
			...
			} else if (highSeriesColumn.contains(props.getTypeMap().get(MetadataConstants.INTEGER_WRAPPER_CLASS))
					|| highSeriesColumn.contains(props.getTypeMap().get(MetadataConstants.DECIMAL_WRAPPER_CLASS))) {
				highSeriesGroup.setGroupType(DataType.TEXT_LITERAL);
