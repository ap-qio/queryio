...
public class GenerateReportServlet extends HttpServlet{
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("GenerateReport request received");
		try{
			if(queryId!=null){
				JSONObject object = BigQueryManager.executeQueryInternal(namenode, queryId,username, false, true, false, format);
				if(object!=null){
...
//						filePath = designFilePath.substring(0, designFilePath.lastIndexOf(File.separator))  + "/" + filePath;
						...
						try{
							...
							if ("HTML".equalsIgnoreCase(format)) {
								...
								if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File Path: " + filePath + " destZipFile: " + destZipFile);
								...
								if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File Path: " + filePath);
								is = new FileInputStream(new File(filePath));
								
								filename = (filePath.indexOf(File.separator)>-1?filePath.substring(filePath.lastIndexOf(File.separator)+1):filePath);
							}
