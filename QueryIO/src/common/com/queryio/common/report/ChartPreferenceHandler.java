...
public class ChartPreferenceHandler {
	...
	public static void setXAxisProperties(Axis axis ,  JSONObject properties, String axisTitle ){
			...
			if (xAxisGridProperties != null && xAxisGridProperties.size() > 0) {
		        ...
		        if(isYAxisGridLineVisible){
			        ...
			        axis.getMajorGrid().getLineAttributes().setColor(ChartPreferenceHandler.hexToRGB(yAxisGridLineColor));
			        axis.getMajorGrid().getLineAttributes().setStyle(ChartPreferenceHandler.getLineStyleType((String) xAxisGridProperties.get("style")));
	...
	public static void setYAxisProperties(Axis axis ,  JSONObject properties, String axisTitle ){
			...
			if (yAxisGridProperties != null && yAxisGridProperties.size() > 0) {
		        ...
		        if(isYAxisGridLineVisible){
			        ...
			        axis.getMajorGrid().getLineAttributes().setColor(ChartPreferenceHandler.hexToRGB(yAxisGridLineColor));
			        axis.getMajorGrid().getLineAttributes().setStyle(ChartPreferenceHandler.getLineStyleType((String) yAxisGridProperties.get("style")));
			        axis.getMajorGrid().getLineAttributes().setThickness(yAxisGridLineWidthInt);
	...
	public static void setTitleProperties(JSONObject titleJSON,Chart chart, String title ){
		if(titleJSON!=null&&titleJSON.size()>0){
			...
			chart.getTitle().getLabel().getCaption().setFont(FontDefinitionImpl.create(titleFontFamily, titleFontSize, bBold, bItalic, bUnderline, bStrikethrough, bWordWrap, 0,getTextAlignment(titleTextAlign,null)));
	        ...
	        Insets inset = InsetsImpl.create(titleInsetTop, titleInsetLeft, titleInsetBottom, titleInsetRight); //setting Inset
	        ...
	        if(titleOutlineVisible){
		        ...
		        chart.getTitle( ).setOutline(LineAttributesImpl.create( hexToRGB(titleOutlineColor),getLineStyleType((String) titleOutline.get("style")), titleOutlineWidthInt));
	        }
	      
		}
	}
	public static void setLabelProperties (JSONObject labelProperties, Series series, boolean isChartWithAxis, int dimension){
		if (labelProperties != null && labelProperties.size() > 0) {
			...
			if(visible){
				...
				if (lblOutLine != null && lblOutLine.size() > 0) {
			        ...
			        if(isLblOutLineVisible){
				        ...
				        lbl.setOutline(LineAttributesImpl.create( hexToRGB(lblOutlineColor),getLineStyleType((String) lblOutLine.get("style")), lblOutlineWidthInt));
			        }
				}
				...
				lbl.getCaption().setFont(FontDefinitionImpl.create(sName, fSize, bBold, bItalic, bUnderline, bStrikethrough, bWordWrap, 0,getTextAlignment(lblTextAlign, null)));
			}
			...
			if(("percent").equalsIgnoreCase(values)){
				...
				series.getDataPoint().getComponents().add(DataPointComponentImpl.create(DataPointComponentType.PERCENTILE_ORTHOGONAL_VALUE_LITERAL, JavaNumberFormatSpecifierImpl.create( "##.##%" )));
			}
		...
		if (legendProperties != null && legendProperties.size() > 0) {
			...
			if(visible){
				...
				if (legendinsets != null && legendinsets.size() > 0) {
			        ...
			        legend.setInsets(InsetsImpl.create(legendInsetTop, legendInsetLeft, legendInsetBottom, legendInsetRight));
			    }
				...
				if (legendOutLine != null && legendOutLine.size() > 0) {
			        ...
			        if(islegendOutLineVisible){
				        ...
				        legend.setOutline(LineAttributesImpl.create( hexToRGB(legendOutlineColor),getLineStyleType((String) legendOutLine.get("style")), legendOutlineWidthInt));
			        }
				}
		        ...
		        if(visibleTitle){
					...
					if(titleLblFontStyle.equalsIgnoreCase("bold"))
						bBold = true;
		        	legendTitle.getCaption().setFont(FontDefinitionImpl.create((String) legendProperties.get("title-font-family"),fsize ,
		        			bBold, bItalic, bUnderline, bStrikethrough, bWordWrap, 0,getTextAlignment("center", null)));
