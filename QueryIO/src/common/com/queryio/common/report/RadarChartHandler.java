...
public class RadarChartHandler {
	...
	public static Chart getRadarChart(String title, int dimension, int chartWidth,
...
//		radarChart.setSubType("Bullseye Radar Chart");
		...
		if (chartOutLine != null && chartOutLine.size() > 0) {
	        ...
	        if(isChartOutLineVisible){
		        ...
		        radarChart.getBlock().setOutline(LineAttributesImpl.create( ChartPreferenceHandler.hexToRGB(chartOutlineColor),ChartPreferenceHandler.getLineStyleType((String) chartOutLine.get("style")), chartOutlineWidthInt));
	        }
		}
		...
		if (titleinsets != null && titleinsets.size() > 0) {
	        ...
	        radarChart.getTitle().setInsets(InsetsImpl.create(titleInsetTop, titleInsetLeft, titleInsetBottom, titleInsetRight));
	    }
