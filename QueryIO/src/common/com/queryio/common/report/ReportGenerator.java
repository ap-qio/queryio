...
public class ReportGenerator {
	...
	{
		AppLogger.initLoggerProperties("/Users/indravardhan/QueryIO/tomcat/webapps/queryio/logger.properties", "/Users/indravardhan/QueryIO/tomcat/webapps/queryio/logs");
		System.out.println(generateViewReport("/Users/indravardhan/QueryIO/tomcat/webapps/queryio/Reports/a/namenode1/new_query_5243.rptdesign", ReportConstants.TYPE_HTML));
	}
...
//			CusomTagDatabase dbProps = RemoteManager.getCustomTagDBProperties();
//			dataSource.setProperty("odaURL", dbProps.getUrl());
//			dataSource.setProperty("odaDriverClass", dbProps.getDriverName());
...
//			dataSource.setProperty("odaPassword", dbProps.getPassword());
			...
			if(format.equalsIgnoreCase(ReportConstants.TYPE_HTML)){
				...
				AppLogger.getLogger().debug("Index of last File.separator : " + outputFilePath.lastIndexOf(File.separator));
				...
				File resourceDirecory = new File(outputFilePath.substring(0, outputFilePath.lastIndexOf(File.separator)) + File.separator + "resources_" + temp.replace(".html", ""));
...
//				((EXCELRenderOption) options).setBaseURL("&__format=xls");
				...
				options.setOption(EXCELRenderOption.EMITTER_ID, "org.eclipse.birt.report.engine.emitter.prototype.excel");
...
//					String nameNodeId = NodeDAO.getNameNodeForAnalyticsDBNameMapping(queryIOConnection, dataSource.getName());
//					
//					if (nameNodeId == null)
//						nameNodeId = NodeDAO.getNameNodeForDBNameMapping(queryIOConnection, dataSource.getName());
//					
//					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("nameNodeId: "+nameNodeId);
//
//					connection = AdHocHiveClient.getHiveConnection(queryIOConnection, nameNodeId);
//				} catch(Exception e) {
...
//			}
//			else
//				connection = CoreDBManager.getCustomTagDBConnection(dataSource.getName());
//			  
//	        task.getAppContext().put("OdaJDBCDriverPassInConnection", connection);
	        
			task.run();
			if(format.equalsIgnoreCase(ReportConstants.TYPE_HTML)){
				//if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File path :"+outputFilePath);
				File file = new File(outputFilePath);
				if(file.exists()){
					//if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File has been found.");
					writer = new FileWriter(file,true);
					output = new BufferedWriter(writer);
					output.append("<script type=\"text/javascript\">"+
							"var tables = document.getElementsByTagName('table');"+
							"for(var i= 0;i<tables.length;i++){"+
							"var firstTableBefore = tables[i];"+
							"firstTableBefore.style.width=\"100%\";"+
							"firstTableBefore.style.tableLayout=\"\";"+
							"firstTableBefore.style.overflow = \"auto\";"+
							"}"+
							"var header = document.getElementsByClassName('style_4');"+
							"for(var i= 0;i<header.length;i++){"+
							"var firstTableBefore = header[i];"+
							...
							"{"+
							"var newSrc = src.substring(src.indexOf('resources_'));"+
							"images[i].src= newSrc;"+
							"}"+
							"}"+
							"var tds = document.getElementsByTagName('img');"+
					    			...
					    			"{"+
					        			"var divr = tds[n].parentNode;"+
					        			"if(divr==undefined || divr == null)"+
					        			"continue;"+
					        			"divr.style.display = \"inline-block\";"+
					        			"divr.parentNode.style.display = \"-web-box\";"+
							...
							"</script>");
					//if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File has been written.");
				}else{
					//if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File not found");
				}
			}else{
				//if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("File Format is not html");
			}
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Finished Report Generating");
