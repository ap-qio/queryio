...
public class MeterChartHandler {
	...
	public static  Chart getChart(String title, int dimension, int chartWidth,
			int chartHeight, String dxSeries, JSONObject ySeriesJSON,
			String xSeriesTitle, String ySeriesTitle, JSONObject chartPreferences, HashMap resultSetColumns, boolean isHive, String yGrouping )
			throws Exception {
		...
		if (chartOutLine != null && chartOutLine.size() > 0) {
	        ...
	        if(isChartOutLineVisible){
		        ...
		        dChart.getBlock().setOutline(LineAttributesImpl.create( ChartPreferenceHandler.hexToRGB(chartOutlineColor),ChartPreferenceHandler.getLineStyleType((String) chartOutLine.get("style")), chartOutlineWidthInt));
	        }
		}
		//Chart insets.
		...
		if (titleinsets != null && titleinsets.size() > 0) {
	        ...
	        dChart.getTitle().setInsets(InsetsImpl.create(titleInsetTop, titleInsetLeft, titleInsetBottom, titleInsetRight));
	    }
