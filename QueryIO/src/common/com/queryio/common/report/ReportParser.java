...
public class ReportParser {
	...
	public static JSONObject getJSONProperties(String queryId, String queryDesc, InputStream reportDesignDocStream) {
		...
		try {
			...
			ReportDesignHandle report = (ReportDesignHandle) designHandle.getDesignHandle();
	...
	private static void populateJSON(ReportDesignHandle designHandle, JSONObject jsonObj) {
		...
		String queryHeader = ((LabelHandle) ((SlotHandle) ((CellHandle) ((RowHandle) ((GridHandle) designHandle
				.getBody().get(0)).getRows().get(0)).getCells().get(0)).getContent()).get(0)).getText();
		jsonObj.put(BigQueryIdentifiers.QUERYHEADER, queryHeader);

		String queryFooter = ((TextItemHandle) ((CellHandle) ((RowHandle) ((GridHandle) ((SimpleMasterPageHandle) designHandle
				.getMasterPages().getContents().get(0)).getPageFooter().get(0)).getRows().get(0)).getCells().get(0))
						.getContent().get(0)).getContent();
		jsonObj.put(BigQueryIdentifiers.QUERYFOOTER, queryFooter);

		SlotHandle handle = ((RowHandle) ((TableHandle) designHandle.getBody().get(1)).getHeader().getContents().get(0))
				.getCells();
		...
		handle = ((RowHandle) ((TableHandle) designHandle.getBody().get(1)).getDetail().getContents().get(0))
				.getCells();
		arr = new JSONObject();
		for (int i = 0; i < handle.getContents().size(); i++) {
			CellHandle cellHandle = ((CellHandle) handle.get(i));
