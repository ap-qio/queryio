...
public abstract class AppChart {
	...
	protected static Color getColorForIndex(final int index, final ChartProperties properties) {
		...
		if (axisSeriesColor != null) {
			...
			final Color defaultColors[] = { new Color(0xff, 0x00, 0x00), // red
					...
					new Color(0xc0, 0x00, 0x00), new Color(0x00, 0x00, 0xC0), new Color(0x00, 0xC0, 0x00),
					new Color(0xC0, 0xC0, 0x00), new Color(0xC0, 0x00, 0xC0), new Color(0x00, 0xC0, 0xC0),
					new Color(0x40, 0x40, 0x40), // darkGray
					new Color(0xFF, 0x40, 0x40), new Color(0x40, 0x40, 0xFF), new Color(0x40, 0xFF, 0x40),
					new Color(0xFF, 0xFF, 0x40), new Color(0xFF, 0x40, 0xFF), new Color(0x40, 0xFF, 0xFF),
	...
	public final void drawLegend(final UserInterface graphics) {
		...
		if (lenLegend > 0) {
			...
			final Rectangle r = new Rectangle(x, y, Math.min(this.iTotalLegendWidth, this.legendRect.width - 1),
					Math.min(iTotalLegendHeight, this.legendRect.height - 1));
			...
			x += LEGENDSTARTINSET;
			MarkerObject mo;
			if (this.iTotalLegendWidth <= this.legendRect.width) // Rendering
																	// all
																	// legend
																	// items in
																	// a single
																	// row.
			{
				for (int i = 0; i < legends.length; i++) {
					if (legends[i].getText() != null) {
						final int individualWidth = legends[i].getMaxWidth(graphics)
								+ (graphics.showSeriesSelector() ? LEGENDITEMINSET_SELECT : LEGENDITEMINSET);
						legends[i].legendFont = this.chartProperties.getLegendFont();
						mo = legends[i].draw(graphics, new Rectangle(x, y, individualWidth, iMaxHeight),
								this.shouldDrawSeries(i), i);
				...
				for (int i = 0; i < legends.length; i++) {
					if (legends[i].getText() != null) {
						legends[i].legendFont = this.chartProperties.getLegendFont();
						mo = legends[i].draw(graphics, new Rectangle(x, y, iMaxWidth, iMaxHeight),
								this.shouldDrawSeries(i), i);
						this.addMarkerObject(mo);
						x += iMaxWidth;
	...
	public Rectangle getChartAreaBoundary() {
		return chartAreaRect != null
				? new Rectangle(chartAreaRect.x, chartAreaRect.y, chartAreaRect.width, chartAreaRect.height) : null;
	}
}
