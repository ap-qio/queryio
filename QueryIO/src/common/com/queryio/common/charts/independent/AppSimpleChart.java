...
public abstract class AppSimpleChart extends AppChart {
	...
	protected final void fillArea(final UserInterface graphics) {
		...
		if (areaColour != null) {
			...
			graphics.fillRectangle(this.plotRect.x + this.shiftYaxisToRHS, this.plotRect.y,
					this.plotRect.width - this.shiftYaxisToRHS,
					this.plotRect.height - this.shiftXAxisAbove - this.iShiftYAxisAbove);
	...
	protected final void calculateDimensions(final UserInterface graphics) {
		...
		if (this.horizontal) {
			...
			if ((axisTitle != null) && this.chartProperties.getShowYAxisTitle().booleanValue()) {
				this.yAxisTitleRect = new Rectangle(clippedRect.x,
						clippedRect.y + clippedRect.height - axisTitle.getMaxHeight(graphics), clippedRect.width,
						axisTitle.getMaxHeight(graphics));
				this.plotRect.height -= this.yAxisTitleRect.height;
			}
		} else {
			// assign values to each rectangle
			if ((axisTitle2 != null) && this.chartProperties.getShowXAxisTitle().booleanValue()) {
				this.xAxisTitleRect = new Rectangle(clippedRect.x,
						clippedRect.y + clippedRect.height - axisTitle2.getMaxHeight(graphics), clippedRect.width,
						axisTitle2.getMaxHeight(graphics));
	...
	public Legend[] getLegend() {
		...
		for (int i = 0; i < legends.length; i++) {
			legends[i] = new Legend(this.chartProperties.getYAxisSeriesText(i),
