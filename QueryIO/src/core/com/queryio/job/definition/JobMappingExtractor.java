	...
	private void extractTableName() throws IOException{
	    ...
	    while( ( ze = zip.getNextEntry() ) != null ) {
	        ...
	        if(entryName.endsWith("class")){
	        	...
	        	if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("List entry: " + entryName);
	        	list.add( entryName  );
	        }
	    }
	    zip.close();
	    ...
	    for(int i=0; i<list.size(); i++){
	    	try{
	    		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Loading class: " + list.get(i));
