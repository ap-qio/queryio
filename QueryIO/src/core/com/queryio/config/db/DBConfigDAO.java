...
public class DBConfigDAO {
	...
	public static boolean isCustomTagDB(String connectionName) {
		...
		try {
			...
			document = documentBuilder.parse(
					EnvironmentalConstants.getWebinfDirectory() + File.separator + QueryIOConstants.DBCONFIG_XML);
			...
			for (int i = 1; i <= connectionList.getLength() - 1; i += 2) {
				...
				if (QueryIOConstants.DEFAULT_CUSTOMTAG_DB.equals(node.getNodeName())) {
					if (node.getAttributes().getNamedItem(QueryIOConstants.DB_NAME_TYPE).getTextContent()
			...
			AppLogger.getLogger().debug("Exception at isCustomTagDB(String connectionName): ", e);
		}
		return false;

	}

	public static DBConfigBean getConnectionDetail(String connectionName) {
		...
		try {
				...
				if (QueryIOConstants.DEFAULT_CUSTOMTAG_DB.equals(node.getNodeName())) {
					if (node.getAttributes().getNamedItem(QueryIOConstants.DB_NAME_TYPE) != null) {
						if (connectionName.equals(
								node.getAttributes().getNamedItem(QueryIOConstants.DB_NAME_TYPE).getTextContent())) {
							dbBean = new DBConfigBean();
							dbBean.setConnectionName(connectionName);

							dbBean.setMigrated(true);
							dbBean.setCustomTagDB(true);
							if (QueryIOConstants.DB_ROLE_METADATA.equals(
									node.getAttributes().getNamedItem(QueryIOConstants.DB_ROLE).getTextContent()))
								dbBean.setIsPrimary(true);
							else
								dbBean.setIsPrimary(false);
							dbBean.setConnectionType(
									node.getAttributes().getNamedItem(QueryIOConstants.DB_TYPE).getTextContent());

							NodeList connectionDetailList = node.getChildNodes();
							for (int j = 0; j < connectionDetailList.getLength(); j++) {
								if (connectionDetailList.item(j).getNodeName().equals("driver")) {
									dbBean.setPrimaryDriverName(connectionDetailList.item(j).getTextContent());
								} else if (connectionDetailList.item(j).getNodeName().equals("userName")) {
									dbBean.setPrimaryUsername(connectionDetailList.item(j).getTextContent());
								} else if (connectionDetailList.item(j).getNodeName().equals("password")) {
								...
								} else if (connectionDetailList.item(j).getNodeName().equals("url")) {
									dbBean.setPrimaryConnectionURL(connectionDetailList.item(j).getTextContent());
								} else if (connectionDetailList.item(j).getNodeName().equals("driverJar")) {
									dbBean.setPrimaryJdbcJar(connectionDetailList.item(j).getTextContent());
								} else if (connectionDetailList.item(j).getNodeName().equals("maxconnections")) {
									dbBean.setMaxConnection(
											Long.parseLong(connectionDetailList.item(j).getTextContent()));

								} else if (connectionDetailList.item(j).getNodeName().equals("maxidleconnections")) {
									dbBean.setMaxIdleConnection(
											Long.parseLong(connectionDetailList.item(j).getTextContent()));

								} else if (connectionDetailList.item(j).getNodeName().equals("waittimeinmillis")) {
									dbBean.setWaitTimeinMillis(
											Long.parseLong(connectionDetailList.item(j).getTextContent()));
			...
			AppLogger.getLogger().debug("Exception at getConnectionDetail(String connectionName): ", e);
		} finally {
			try {
				if (connection != null)
	...
	public static ArrayList getAllDBNameForOperation(final Connection connection) {
		...
		try {
			...
			rs = CoreDBManager.getQueryResultsForStatement(statement,
					QueryConstants.QRY_GET_ALL_DBNAME_MODE_FOR_OPERATION);
			while (rs.next()) {
		...
		try {
			...
			for (int i = 1; i <= connectionList.getLength() - 1; i += 2) {
				...
				if (QueryIOConstants.DEFAULT_CUSTOMTAG_DB.equals(node.getNodeName())) {
					...
					if (QueryIOConstants.DB_ROLE_METADATA.equalsIgnoreCase(
							node.getAttributes().getNamedItem(QueryIOConstants.DB_ROLE).getTextContent())) {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Exception caught while fetching DB Connections", e);
		}
		return arr;
	}
		...
		try {
			...
			for (int i = 1; i <= connectionList.getLength() - 1; i += 2) {
				...
				if (QueryIOConstants.DEFAULT_CUSTOMTAG_DB.equals(node.getNodeName())) {
					...
					if (isPrimary) {
						if (QueryIOConstants.DB_ROLE_METADATA
								.equals(node.getAttributes().getNamedItem(QueryIOConstants.DB_ROLE).getTextContent()))
							arr.add(node.getAttributes().getNamedItem(QueryIOConstants.DB_NAME_TYPE).getTextContent());
					} else {
						if (QueryIOConstants.DB_ROLE_ANALYTICS
								.equals(node.getAttributes().getNamedItem(QueryIOConstants.DB_ROLE).getTextContent()))
							arr.add(node.getAttributes().getNamedItem(QueryIOConstants.DB_NAME_TYPE).getTextContent());
					}
				}
	...
	private static void createMonitoringXMLNode(Document document, Element newConnection, String connectionURL,
			String username, String passwordStr, String driverName, String jarFileName, long maxConnections2,
			long maxIdleConnections2, long waitTimeMilliSeconds, String connectionName) {
		...
		Element maxidleconnections = document.createElement("maxidleconnections");
	...
	public static void addDBConnection(String connectionName, String connectionType, String primaryConnectionURL,
		...
		try {
			...
			createMonitoringXMLNode(document, newConnection, primaryConnectionURL, primaryUsername, primaryPassword,
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Exception caught while saving DB Connection", e);
			throw e;
		}
	}

	private static void createDBConfigTypeTags(Document document, Element newConnection, String connectionName,
		...
		timestampTag.appendChild(
				document.createTextNode(dbProp.getTypeMap().get(MetadataConstants.TIMESTAMP_WRAPPER_CLASS)));
		...
		integerTag
				.appendChild(document.createTextNode(dbProp.getTypeMap().get(MetadataConstants.INTEGER_WRAPPER_CLASS)));
		...
		decimalTag
				.appendChild(document.createTextNode(dbProp.getTypeMap().get(MetadataConstants.DECIMAL_WRAPPER_CLASS)));
		...
		booleanTag
				.appendChild(document.createTextNode(dbProp.getTypeMap().get(MetadataConstants.BOOLEAN_WRAPPER_CLASS)));
	...
	public static void updateDBConnection(String connectionName, String primaryConnectionURL, String primaryUsername,
			String primaryPassword, String primaryDriverName, String jarFile, long maxConnections,
			long maxIdleConnections, long waitTimeMilliSeconds) {
		...
		try {
			...
			for (int i = 1; i <= connectionList.getLength() - 1; i += 2) {
				...
				if (QueryIOConstants.DEFAULT_CUSTOMTAG_DB.equals(node.getNodeName())) {
					if (connectionName.equals(
						...
						for (int j = 0; j < connectionDetailList.getLength(); j++) {
							if (connectionDetailList.item(j).getNodeName().equals("driver")) {
								node.getChildNodes().item(j).setTextContent(primaryDriverName);
							} else if (connectionDetailList.item(j).getNodeName().equals("userName")) {
								node.getChildNodes().item(j).setTextContent(primaryUsername);
							} else if (connectionDetailList.item(j).getNodeName().equals("password")) {
								node.getChildNodes().item(j)
										.setTextContent(CryptManager.createInstance().encryptData(primaryPassword));
							} else if (connectionDetailList.item(j).getNodeName().equals("url")) {
								node.getChildNodes().item(j).setTextContent(primaryConnectionURL);
							} else if (connectionDetailList.item(j).getNodeName().equals("driverJar")) {
								if (jarFile != null && !jarFile.equals("")) {
									node.getChildNodes().item(8).setTextContent(jarFile);
								}
							} else if (connectionDetailList.item(j).getNodeName().equals("maxconnections")) {
								node.getChildNodes().item(j).setTextContent(String.valueOf(maxConnections));
							} else if (connectionDetailList.item(j).getNodeName().equals("maxidleconnections")) {
								node.getChildNodes().item(j).setTextContent(String.valueOf(maxIdleConnections));
							} else if (connectionDetailList.item(j).getNodeName().equals("waittimeinmillis")) {
								node.getChildNodes().item(j).setTextContent(String.valueOf(waitTimeMilliSeconds));
							}
						}
						break;
					}
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Exception caught while updating DB ", e);
		}
	}

	public static DWRResponse startMigration(DBConfigBean source, DBConfigBean destination, boolean isCustomTagDB,
			com.queryio.core.bean.Node namenode, Host namenodeHost, boolean createSchemaFlag, String tablePrefix) {
	...
	public static DWRResponse startTableDDLExport(DBConfigBean source, com.queryio.core.bean.Node namenode,
			Host namenodeHost) throws Exception {
		DWRResponse dwrResponse = new DWRResponse();
	...
	public static void insertDBMigrationStatusEntry(Connection connection, String sourceDBName, String destDBName,
			String status, String progress, long startTime, long endTime) throws Exception {
		PreparedStatement ps = null;
		try {
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_INSERT_DBMIGRATIONSTATUS);
	...
	public static void updateDBMigrationStatusFinal(Connection connection, String sourceDBName, String destDBName,
			String oldStatus, String status, String progress, long endTime) throws Exception {
		PreparedStatement ps = null;
		try {
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_UPDATE_DBMIGRATIONSTATUS_COMPLETE);
	...
	public static void updateDBMigrationStatusErrorFinal(Connection connection, String sourceDBName, String destDBName,
			String oldStatus, String status, String error, long endTime) throws Exception {
		PreparedStatement ps = null;
		try {
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_UPDATE_DBMIGRATIONSTATUS_ERROR_COMPLETE);
			ps.setString(1, status);
	...
	public static void updateDBMigrationProgress(Connection connection, String sourceDBName, String destDBName,
			String status, String progress) throws Exception {
		PreparedStatement ps = null;
		try {
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_UPDATE_DBMIGRATIONSTATUS_PROGRESS);
	...
	public static ArrayList getAllDBMigrationStatus(Connection connection) throws Exception {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_DB_MIGRATION_STATUS + " ORDER BY "
					+ ColumnConstants.COL_DB_MIGRATION_STATUS_STARTTIME + " DESC");
			...
			while (rs.next()) {
				...
				if (endTime != null
						&& endTime.getTime() < rs.getTimestamp(ColumnConstants.COL_MIGRATIONINFO_STARTTIME).getTime()) {
					endTime = null;
				}
				if (endTime != null) {
	...
	public static DWRResponse removeConnection(String connectionName) {
		...
		if (dwrResponse == null) {
			dwrResponse = new DWRResponse();
			dwrResponse.setTaskSuccess(true);
			dwrResponse.setResponseMessage("DB Connection Removed Successfully.");
			dwrResponse.setResponseCode(200);
		}
		return dwrResponse;
	}
	...
	public static void replaceDBConfigFromHadoop() throws Exception {
		...
		try {
			dbConfigHadoopFile = new File(
					StartupParameters.getHadoopDirLocation() + File.separator + QueryIOConstants.QUERYIOAGENT_DIR_NAME
