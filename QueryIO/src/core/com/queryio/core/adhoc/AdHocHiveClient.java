...
//		Statement stmt = null;
//		String query = "load data inpath '" + path + "' overwrite into table " + tableName;
//		
//		try
//		{
...
//		catch (Exception e)
//		{
//			AppLogger.getLogger().fatal("Error loading data: " + e.getMessage(), e);
//		}
//		finally
//		{
//			try
...
//		}
	...
	public static String getQueryString(String type, String tableName, String[] colNames, String[] datatypes, String[] colIndex, String delimiter, String valueSeparator, String pattern, String location, boolean ifFirstHeader) throws Exception
	{
		...
		for (int i = 0; i < colNames.length; i ++)
		{
			...
			if (QueryIOConstants.ADHOC_TYPE_ACCESSLOG.equalsIgnoreCase(type) || QueryIOConstants.ADHOC_TYPE_REGEX.equalsIgnoreCase(type))
		...
		if (QueryIOConstants.ADHOC_TYPE_CSV.equalsIgnoreCase(type) || QueryIOConstants.ADHOC_TYPE_IISLOG.equalsIgnoreCase(type)
		...
		{
//			res = stmt.executeQuery("CREATE EXTERNAL TABLE " + tableName + " (RAM INT,IP STRING,CPU INT) " +
//					"ROW FORMAT SERDE 'org.apache.hadoop.hive.serde2.json.JsonSerDe' STORED AS TEXTFILE LOCATION '" + location + "'");
		...
		else if (QueryIOConstants.ADHOC_TYPE_ACCESSLOG.equalsIgnoreCase(type) || QueryIOConstants.ADHOC_TYPE_REGEX.equalsIgnoreCase(type))
		{
...
//			System.out.println(QueryIOConstants.ACCESS_LOG_PATTERN_TO_REGEX_MAP.get(patternSeq[i]).toString() + " ");
		...
		regex = regex.trim();
		if (AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("regex: " + regex);
	...
	private static String getOutputFormatString(String[] colIndex)
	{
		...
		format = format.trim();
		if (AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("format: " + format);
...
//	}
	...
	public static String getQueryStringPersist(String resultTableName, String query) throws Exception
	{
		...
		// CREATE TABLE NEW_" + tableName + " ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' STORED AS TEXTFILE AS SELECT * FROM " + tableName + " WHERE CPU > 98
	...
	{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("createHiveTable DDL: " + createQuery);
...
//	    }
	...
	{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("dropHiveTable: tableName: " + tableName + " nameNodeId: " + nameNodeId);
