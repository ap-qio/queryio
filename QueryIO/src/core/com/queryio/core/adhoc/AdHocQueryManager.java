...
public class AdHocQueryManager {
	...
	public static DWRResponse addAdHocQueryCSV(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_CSV,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}
	...
	public static DWRResponse addAdHocQueryLOG(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_LOG,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}
	...
	public static DWRResponse addAdHocQueryAccessLog(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive,
				QueryIOConstants.ADHOC_TYPE_ACCESSLOG, adHocTableName, fileName, filePathPattern, fields, encoding,
	...
	public static DWRResponse addAdHocQueryIISLOG(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_IISLOG,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}
	...
	public static DWRResponse addAdHocQueryJSON(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_JSON,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}
	...
	public static DWRResponse addAdHocQueryKVPairs(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_PAIRS,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}

	@SuppressWarnings("unchecked")
	public static DWRResponse addAdHocQueryMBOX(String adHocId, String nameNodeId, String rmId, String sourcePath,
			boolean parseRecursive, String adHocTableName, String fileName, String filePathPattern, String fields,
			String encoding) {
		JSONObject arguments = new JSONObject();
		arguments.put(QueryIOConstants.ADHOC_PARSER_ENCODING, encoding);

		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_MBOX,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}
	...
	public static DWRResponse addAdHocQueryRegex(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_REGEX,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}
	...
	public static DWRResponse addAdHocQueryXML(String adHocId, String nameNodeId, String rmId, String sourcePath,
		...
		return addAdHocQuery(adHocId, nameNodeId, rmId, sourcePath, parseRecursive, QueryIOConstants.ADHOC_TYPE_XML,
				adHocTableName, fileName, filePathPattern, fields, encoding, arguments.toJSONString());
	}

	public static DWRResponse addAdHocQuery(String adHocId, String nameNodeId, String rmId, String sourcePath,
			boolean parseRecursive, String type, String adHocTableName, String fileName, String filePathPattern,
			String fields, String encoding, String arguments) {
		...
		try {
			...
			for (int i = 0; i < array.size(); i++) {
				...
				if (dtType.indexOf(MetadataConstants.GENERIC_DATA_TYPE_STRING) != -1) {
					dataType = CustomTagDBConfigManager.getDatabaseDataTypeMap(connectionName, null).getTypeMap()
							.get(MetadataConstants.STATIC_DATATYPES_TO_WRAPPER_MAP
									.get(MetadataConstants.GENERIC_DATA_TYPE_STRING));
					size = getSizeFromString(dtType);
					colTypes.add(dataType + " (" + size + ")");
				} else {
					dataType = CustomTagDBConfigManager.getDatabaseDataTypeMap(connectionName, null).getTypeMap()
							.get(MetadataConstants.STATIC_DATATYPES_TO_WRAPPER_MAP.get(dtType));
			...
			if (!UserDefinedTagDAO.checkIfTableExists(customTagConnection, adHocTableName)) {
				JobDefinitionDAO.createDatabaseTable(customTagConnection, adHocTableName,
						colNames.toArray(new String[colNames.size()]), colTypes.toArray(new String[colTypes.size()]),
						config.get(QueryIOConstants.CUSTOM_TAG_DB_CREATESTMT));

				if (QueryIOConstants.ADHOC_TYPE_CSV.equals(type) || QueryIOConstants.ADHOC_TYPE_JSON.equals(type)
						|| QueryIOConstants.ADHOC_TYPE_IISLOG.equals(type)
						|| QueryIOConstants.ADHOC_TYPE_PAIRS.equals(type)
						|| QueryIOConstants.ADHOC_TYPE_ACCESSLOG.equals(type)
						|| QueryIOConstants.ADHOC_TYPE_REGEX.equals(type)) {
					...
					for (int i = 0; i < maxRetryCount; i++) {
						...
						try {
							if (retryCount > 0) {
								// table from PostgreSQL got deleted in
								// exception, so re-create this table also.
								JobDefinitionDAO.createDatabaseTable(customTagConnection, adHocTableName,
							...
							AdHocHiveClient.createHiveTable(hiveConnection,
									AdHocHiveClient.getQueryString(type, adHocTableName,
											colNames.toArray(new String[colNames.size()]),
											colTypesHive.toArray(new String[colTypesHive.size()]),
											colIndex.toArray(new String[colIndex.size()]), delimiter, valueSeparator,
											pattern, location, ifFirstHeader));
							break; // Break the loop if no error occurs. This is
									// to fix add Hive entry issue in case of UI
									// server restart.
						} catch (Exception e) {
			...
			AdHocQueryDAO.addAdHocQueryInfo(connection, adHocId, nameNodeId, rmId, sourcePath, parseRecursive, type,
					adHocTableName, filePathPattern, fields, encoding, arguments);
	...
	public static DWRResponse updateAdHocQuery(String adHocId, String nameNodeId, String rmId, String filePathPattern,
		...
		try {
			...
			AppLogger.getLogger().fatal(e.getMessage(), e);
			dwrResponse.setDwrResponse(false, QueryIOConstants.ADHOC_QUERY_UPDATE_FAILED + " Reason: " + e.getMessage(),
					500);
		} finally {
			try {
				CoreDBManager.closeConnection(connection);
		...
		try {
			...
			if ((adHocIdList != null) && (adHocIdList.size() > 0)) {
				...
				dwrResponse.setDwrResponse(false,
						QueryIOConstants.ADHOC_QUERY_DELETED_FAILED + " Reason: No adHoc Id specified for delete.",
						500);
			}
		} catch (Exception e) {
			AppLogger.getLogger().fatal("Exception: " + e.getLocalizedMessage(), e);
			dwrResponse.setDwrResponse(false,
					QueryIOConstants.ADHOC_QUERY_DELETED_FAILED + " Reason: " + e.getLocalizedMessage(), 500);
		} finally {
			try {
				CoreDBManager.closeConnection(connection);
			} catch (Exception e) {
	...
	public static JSONObject getDefaultColumns(String type) {
		...
		String[] colNames = new String[] { "FILEPATH", "CATEGORY", "CLASS", "DATE", "FILE", "LINE", "LOCATION", "MDC",
				"MESSAGE", "METHOD", "ELAPSED", "NDC", "PRIORITY", "SEQUENCE", "THREAD" };
		String[] sqlTypes = new String[] { MetadataConstants.GENERIC_DATA_TYPE_STRING + "(1280)",
	...
	// private static void loadDataHive(String nameNodeId, String
	// filePathPattern, String tableName)
	// {
	// Connection connection = null;
	// Connection hiveConnection = null;
	// FileSystem dfs = null;
	...
	// ArrayList result = QueryIOAgentManager.getConfig(host, list, node,
	// "core-site.xml");
	// if (result == null) {
	// return;
	// }
	...
	////
	//// conf = ConfigurationManager.getKerberosConfiguration(connection,
	// nameNodeId);
	////
	...
	//// }
	//// catch(Exception e){
	//// AppLogger.getLogger().fatal("Could not authenticate user with kerberos,
	...
	// !name.equalsIgnoreCase(AdHocHiveClient.warehouseDir);
	// }
	// };
	//
	...
	// }
	//
	// AppLogger.getLogger().debug("Total input paths to process : " +
	// files.size());
	//
	...
	// }
	//
	// AdHocHiveClient.loadFileEntry(connection, fsDefaultName +
	// tempPath.toString(), tableName);
	// }
	// catch (Exception e)
	// {
	// AppLogger.getLogger().fatal("Exception: " + e.getLocalizedMessage(), e);
