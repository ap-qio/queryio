...
public class ListSelectedFilesRequest extends RequestProcessorCore {
	...
	public ListSelectedFilesRequest(String user, String path, String nodeId, int rows, int pageNo, String query)
			throws Exception {
	...
	public void process() throws Exception {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Rcvd Query: " + query);
		...
		String selectContent = query.substring(query.indexOf("SELECT ") + "SELECT ".length(),
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Executing Query: " + query);
		...
		nonLimitedQuery = nonLimitedQuery.substring(0, nonLimitedQuery.indexOf("SELECT") + "SELECT".length() + 1)
				+ " COUNT(*) " + nonLimitedQuery.substring(nonLimitedQuery.indexOf("FROM"));

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Query for count: " + nonLimitedQuery);
	...
	public void executeBigQuery(String query, String tableName, String nonLimitedQuery) throws Exception {
		...
		try {
			...
			while (limitedRS.next()) {
				...
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug(colNames);
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug(columnValues);
