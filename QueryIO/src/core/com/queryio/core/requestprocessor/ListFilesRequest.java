...
public class ListFilesRequest extends RequestProcessorCore {
	...
	static String queryStart = "SELECT " + EncryptionHandler.COL_COMPRESSION_TYPE + ","
	...
	public void process() throws Exception {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("FilterlistedPaths length :" + listedPaths.length);
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Total  path :" + fosf.getCount());
			...
			for (int k = 0; k < listedPaths.length; k++) {
				...
				if (stat.isDirectory()) {
					...
					try {
						...
						AppLogger.getLogger().debug("C: " + metadata.get(EncryptionHandler.COL_COMPRESSION_TYPE));

						AppLogger.getLogger().debug("E: " + metadata.get(EncryptionHandler.COL_ENCRYPTION_TYPE));

						row.add(metadata.get(EncryptionHandler.COL_COMPRESSION_TYPE));
						row.add(metadata.get(EncryptionHandler.COL_ENCRYPTION_TYPE));
					} catch (Exception e) {
	...
	public static Map<String, String> getMetadata(Configuration conf, String filePath) throws Exception {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Fetching metadata for: " + filePath);
	...
	public static Map<String, String> getObjectMetadata(Configuration conf, String filePath, String tableName)
			throws Exception {
		Map map = null;
		Connection connection = null;
		...
		try {
			...
			if (rs.next()) {
				AppLogger.getLogger().debug(
						"file: " + filePath + ", tableName: " + tableName + ", found: " + found + ", entry found");
				map = new HashMap();
