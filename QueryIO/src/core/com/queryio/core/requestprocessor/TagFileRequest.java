		...
		try {
			...
			if (EnvironmentalConstants.isUseKerberos()) {
				...
				dfs = QIODFSUtils.getFileSystemAs(user, group, conf,
						new URI(conf.get(DFSConfigKeys.FS_DEFAULT_NAME_KEY)));
			}

			IDataTagParser tagParser = UserDefinedTagResourceFactory.getParserFromConstructor(conf,
					this.path.toString(), null, null);
			...
			try {
				...
				DBTypeProperties dbTypeProp = CustomTagDBConfigManager.getDatabaseDataTypeMap(null,
						CustomTagDBConfigManager.getConfig(RemoteManager.getDBNameForNameNodeMapping(nameNodeId))
								.getCustomTagDBType());

				UserDefinedTagDAO.insertTagValues(metaCon, dbTypeProp, tableName, this.path.toString(), defaultTags,
						extraTags, true,
						tagParser == null ? new TableMetadata("DEFAULT", null)
								: tagParser
										.getTableMetaData(UserDefinedTagUtils.getFileExtension(this.path.toString())));
			} finally {
				try {
