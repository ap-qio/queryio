...
public class PutFileRequest extends RequestProcessorCore {
	...
	public PutFileRequest(String user, String group, Path path, String nameNodeId, String fsDefaultName, InputStream is,
			long contentLength, Configuration conf, boolean deflate, String compressionType, String encryptionType,
			FileUploadListener fileUploadListener, JSONObject tagsJSON) {
	...
	public PutFileRequest(String user, String group, Path path, String nameNodeId, String fsDefaultName, InputStream is,
			long contentLength, Configuration conf, boolean deflate, String compressionType, String encryptionType,
			FileUploadListener fileUploadListener, List<UserDefinedTag> extraTags, JSONObject tagsJSON) {
		this(user, group, path, nameNodeId, fsDefaultName, is, contentLength, conf, deflate, compressionType,
				encryptionType, fileUploadListener, tagsJSON);
	...
	public void process() throws Exception {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("PutFileRequest, user: " + this.user);
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("FsDefaultName: " + this.fsDefaultName);
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Path: " + this.path);
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("tagsJSON: " + this.tagsJSON);
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Compression Type: " + this.compressionType);
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Encryption Type: " + this.encryptionType);
			...
			try {
				...
				try {
					qioOutputStream = new QIODFSOutputStream(dfs, dfsOutputStream, this.compressionType,
							this.encryptionType, tagsJSON, this.path.toUri().getPath());
					qioOutputStream.addTags(extraTags);
				} catch (Exception e) {
				...
				try {
					...
					AppLogger.getLogger()
							.fatal("Tag parser exception for file : " + this.path + ", reason: " + e.getMessage(), e);
			...
			this.successful = true;
			this.errorMessage = "Tag processing for " + this.path + " failed but file uploaded succesfully. Reason: "
					+ tpe.getMessage();
			throw tpe;
		} catch (Exception e) {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Deleting path: " + this.path + "	isDeleted: " + isDeleted);
	...
	public void writeToStream(final Connection connection, InputStream inputStream, OutputStream outputStream)
			throws Exception {

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("is deflate (to be unzipped): " + deflate);
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug(
					"Writing to file: " + this.path.toString() + ", size: " + getFormattedStorageSize(this.totalBytes));
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("File upload: " + this.path.toString() + " complete");
