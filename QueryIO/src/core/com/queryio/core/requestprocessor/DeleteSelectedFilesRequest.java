...
public class DeleteSelectedFilesRequest extends RequestProcessorCore{
	...
	public void process() throws Exception{
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("In DeleteSelectedFilesRequest, user: " + this.user);
...
//					UserGroupInformation.getLoginUser(us.getUserName(), SecurityHandler.decryptData(us.getPassword()));
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Path: " + dfs.getFileStatus(new Path("/")).getPath());
		} finally {
			try
			{
				if(dfs!=null)
