...
public class AlertDAO
{
	 ...
	 * @throws Exception
	 */
	public static ArrayList getAlertList(final Connection connection, final long fromTime)
			throws Exception
	{
			...
			while (rs.next())
			{
				...
				if(node!=null)
				{
					...
					if(list!=null)
					{
						...
						{
							if(list.get(i)!=null)
							sBuf.append(((String)list.get(i)).substring(((String)list.get(i)).indexOf("#") + 1, ((String)list.get(i)).length()));
							if(i!=list.size()-1)	sBuf.append(", ");
						}
					}
				...
				if(host!=null)	alert.setHostname(host.getHostIP());
				...
				alert.setStartTime(rs.getTimestamp(ColumnConstants.COL_ALERTS_STARTTIME)!=null ? sdf.format(rs.getTimestamp(ColumnConstants.COL_ALERTS_STARTTIME).getTime()) : null);
				alert.setEndTime(rs.getTimestamp(ColumnConstants.COL_ALERTS_ENDTIME)!=null ? sdf.format(rs.getTimestamp(ColumnConstants.COL_ALERTS_ENDTIME).getTime()) : null);
	...
	public static ArrayList getActiveAlertListForNodeBeforeTime(final Connection connection, long beforeTime, String targetNodeId)
			throws Exception
	{
		ArrayList alertsList = new ArrayList();
		final PreparedStatement ps = DatabaseFunctions.getPreparedStatement(connection,
	...
	public static int getAlertCountForNode(final Connection connection, String nodeId) throws Exception
	{
		...
		try
		{
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_ALERT_COUNT_FOR_NODE);
	...
	public static ArrayList getAlertListForNodeBetween(final Connection connection, final long fromTime, long toTime, String selectedNodeId)
			throws Exception
	{
		ArrayList alertsList = new ArrayList();
		final PreparedStatement ps = DatabaseFunctions.getPreparedStatement(connection,
			...
			final ArrayList alertEndTimes) throws Exception
	{
		...
		{
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_ALERT_ATTRIBUTE_TIMES);
	...
	public static void fillControllerAlertTimes(final Connection connection, final String controllerId,
		final long startTime, final long endTime, final ArrayList alertStartTimes,
		final ArrayList alertEndTimes) throws Exception
	{
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		{
			ps = DatabaseFunctions.getPreparedStatement(connection,
				QueryConstants.PREPARED_QRY_ALERT_CONTROLLER_TIMES);
			ps.setString(1, controllerId);
	...
	public static boolean hasValue(Connection connection,String controllerId,String attrName,long startTime,long  endTime) throws Exception
	{
			...
			final long endTime) throws Exception
	{
		...
		try
		{
			psUpdate = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_RESET_ALERT);
			...
			final long startTime, final String severity) throws Exception
	{
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("writeGeneratedAlert, nodeId: " + nodeId);
		
		PreparedStatement ps = null;
		try
		{
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_INSERT_ALERT);
			...
			final String nodeId, final long startTime, final ArrayList attributeNames) throws Exception
	{
		...
		{
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_INSERT_ALERTATTRIBUTES);
	...
	{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("deleteAlerts() called");
		...
		try
		{
			...
			{
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(alertIdentifiers[i]);
				...
				java.util.Date parsedDate = dateFormat.parse(alertIdentifier.substring(alertIdentifier.indexOf('$') + 1,alertIdentifier.lastIndexOf('$')));
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Alerts deleted");
		}
		finally
		{
			try
	...
	public static boolean hasAlert(Connection connection, String nodeId, long startTime, long endTime, String alertType) throws Exception 
	{
		...
		if(alertList != null)
		{
			...
			{
				if(((Alert) alertList.get(i)).getSeverity().equalsIgnoreCase(alertType))	return true;
			}
		}
