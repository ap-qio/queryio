...
public class MigrationInfoDAO {
	...
	public static MigrationInfo delete(Connection connection, int id) throws Exception {
		...
		PreparedStatement pst = null;
		try {
			pst = connection.prepareStatement("DELETE FROM " + TableConstants.TABLE_MIGRATIONINFO + " WHERE "
	...
	public static ArrayList getAll(Connection connection) throws Exception {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_MIGRATIONINFO + " ORDER BY "
					+ ColumnConstants.COL_MIGRATIONINFO_STARTTIME + " DESC");
