...
public class DataSourceDAO {
	...
	public static void addDataConnection(final Connection connection, final String id, final short connectionType) throws Exception {
		PreparedStatement ps = null;
		try
		{
	...
	public static void addDataBaseDataSource(final Connection connection , final DataBaseDataSource ds) throws Exception {
	...
	public static DataBaseDataSource getDBDataSource(final Connection connection , String id)throws Exception {
		...
		ResultSet rs = null;
		try{
			ps = DatabaseFunctions.getPreparedStatement(connection, QueryConstants.PREPARED_QRY_GET_DATABASE_DATASOURCE);
			ps.setString(1, id);
			rs = CoreDBManager.getQueryResultsForPreparedStatement(ps);
			if(rs.next()){
				...
				ds.setPassword(SecurityHandler.decryptData(rs.getString(ColumnConstants.COL_DATABASE_DATASOURCE_PASSWORD)));
				ds.setJarFileName(rs.getString(ColumnConstants.COL_DATABASE_DATASOURCE_DRIVER_JAR));
				ds.setTableNames(DBAcivator.getAllTableNames(ds.getDriver(), ds.getConnectionURL(), ds.getUserName(), ds.getPassword(), ds.getJarFileName()));
	...
	public static EmailDataSource getEmailDataSource(final Connection connection, String id) throws Exception
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				ds.setPassword(SecurityHandler.decryptData(rs.getString(ColumnConstants.COL_EMAIL_DATASOURCE_PASSWORD)));
	...
	public static S3DataSource getS3DataSource(final Connection connection, String id) throws Exception
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				ds.setSecretAccessKey(SecurityHandler.decryptData(rs.getString(ColumnConstants.COL_S3_DATASOURCE_SECERT_ACCESS_KEY)));
			}
		}
		finally
		{
	...
	public static void deleteEmailDataSource(final Connection connection, String id) throws Exception
	{
		...
		try
		{
			ps = DatabaseFunctions.getPreparedStatement(connection, QueryConstants.PREPARED_QRY_DELETE_EMAIL_DATASOURCE);
			ps.setString(1, id);
