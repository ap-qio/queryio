...
public class HadoopConfigDAO {
	...
	public static SummaryTable getHAHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_HA + "'"
					+ " ORDER BY " + ColumnConstants.COL_HADOOPCONFIG_TYPE + ","
					+ ColumnConstants.COL_HADOOPCONFIG_HADOOPKEY + " ASC");
	...
	public static HadoopConfig getHadoopConfig(Connection connection, String key) throws Exception {
		...
		try {
			st = connection.createStatement();
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_HADOOPKEY + " = '" + key + "'");
	...
	public static ArrayList getAllHadoopConfigs(Connection connection, String configType) throws Exception {
		...
		try {
			st = connection.createStatement();
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + configType + "'");
			HadoopConfig config = null;
			while (rs.next()) {
				config = new HadoopConfig();
				config.setType(rs.getString(ColumnConstants.COL_HADOOPCONFIG_TYPE));
	...
	public static HashMap getAllDataNodeHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			st = connection.createStatement();
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " IN ('" + QueryIOConstants.HADOOP_CONFIGTYPE_HA + "','"
					+ QueryIOConstants.HADOOP_CONFIGTYPE_HDFS + "','" + QueryIOConstants.HADOOP_CONFIGTYPE_DATANODE
					+ "')");
			HadoopConfig config = null;
			while (rs.next()) {
				config = new HadoopConfig();
	...
	public static HashMap getAllNameNodeHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			st = connection.createStatement();
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " IN ('" + QueryIOConstants.HADOOP_CONFIGTYPE_COMMON
					+ "','" + QueryIOConstants.HADOOP_CONFIGTYPE_HA + "','" + QueryIOConstants.HADOOP_CONFIGTYPE_HDFS
					+ "','" + QueryIOConstants.HADOOP_CONFIGTYPE_NAMENODE + "')");
			HadoopConfig config = null;
			while (rs.next()) {
				config = new HadoopConfig();
				config.setType(rs.getString(ColumnConstants.COL_HADOOPCONFIG_TYPE));
	...
	public static HashMap getAllResourceManagerHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			st = connection.createStatement();
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " IN ('" + QueryIOConstants.HADOOP_CONFIGTYPE_MAPREDUCE
					+ "','" + QueryIOConstants.HADOOP_CONFIGTYPE_RESOURCEMANAGER + "')");
			HadoopConfig config = null;
			while (rs.next()) {
				config = new HadoopConfig();
				config.setType(rs.getString(ColumnConstants.COL_HADOOPCONFIG_TYPE));
	...
	public static HashMap getAllNodeManagerHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " IN ('" + QueryIOConstants.HADOOP_CONFIGTYPE_MAPREDUCE
					+ "','" + QueryIOConstants.HADOOP_CONFIGTYPE_NODEMANAGER + "')");
			HadoopConfig config = null;
			while (rs.next()) {
				config = new HadoopConfig();
				config.setType(rs.getString(ColumnConstants.COL_HADOOPCONFIG_TYPE));
	...
	public static void updateHadoopConfigDefaultValue(Connection connection, ArrayList configKeys,
			ArrayList configValues) throws Exception {
	...
	public static void updateHadoopConfigDefaultValue(Connection connection, String configkey, String configValue)
			throws Exception {

		Statement stmt = null;
		try {
			String query = "UPDATE " + TableConstants.TABLE_HADOOPCONFIG + " SET "
					+ ColumnConstants.COL_HADOOPCONFIG_DEFAULT_VALUE + "='" + configValue + "' WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_HADOOPKEY + "='" + configkey + "'";
	...
	public static void addHadoopConfigValue(Connection connection, String type, String configkey, String configValue,
			String description) throws Exception {

		Statement stmt = null;
		try {
			String query = "INSERT INTO " + TableConstants.TABLE_HADOOPCONFIG + " VALUES ('" + type + "', '" + configkey
	...
	public static SummaryTable getAllHDFSHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			st = connection.createStatement();
			String query = "SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_HDFS + "'"
					+ " OR " + ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_HA
					+ "'" + " OR " + ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '"
					+ QueryIOConstants.HADOOP_CONFIGTYPE_DATANODE + "'" + " OR " + ColumnConstants.COL_HADOOPCONFIG_TYPE
					+ " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_NAMENODE + "'" + " OR "
	...
	public static SummaryTable getAllMapReduceHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			st = connection.createStatement();
			String query = "SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_MAPREDUCE
					+ "'" + " OR " + ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '"
					+ QueryIOConstants.HADOOP_CONFIGTYPE_RESOURCEMANAGER + "'" + " OR "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_NODEMANAGER
					+ "'";
	...
	public static HashMap getAllCheckpointHadoopConfigs(Connection connection) throws Exception {
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + TableConstants.TABLE_HADOOPCONFIG + " WHERE "
					+ ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '" + QueryIOConstants.HADOOP_CONFIGTYPE_CHECKPOINTNODE
					+ "'" + " OR " + ColumnConstants.COL_HADOOPCONFIG_TYPE + " = '"
					+ QueryIOConstants.HADOOP_CONFIGTYPE_HDFS + "'");
			while (rs.next()) {
				HadoopConfig config = new HadoopConfig();
				config.setType(rs.getString(ColumnConstants.COL_HADOOPCONFIG_TYPE));
				config.setKey(rs.getString(ColumnConstants.COL_HADOOPCONFIG_HADOOPKEY));
