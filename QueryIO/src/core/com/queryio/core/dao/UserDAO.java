...
public class UserDAO
{
	...
	public static void insertUser(Connection connection, User user, String userRole,DWRResponse dwrResponse) throws Exception
	{
		...
		try
		{
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Adding user: "+ user.getUserName());
	...
	public static void updatePassword(Connection connection, int id, String oldPassword, String newPassword, DWRResponse dwrResponse) throws Exception
	{
		PreparedStatement ps = null;
		try
		{
	...
	public static User getUserDetail(Connection connection, int userId) throws Exception
	{
		...
		try
		{
			...
			if (rs.next())
			{
				...
				user.setGroups(UserGroupDAO.getGroupsForUser(connection,rs.getString(ColumnConstants.COL_USER_USERNAME)));
			}
		}
		finally
		{
