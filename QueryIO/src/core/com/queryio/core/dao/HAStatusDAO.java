...
public class HAStatusDAO {
	...
	public static void swapEntry(Connection connection, String newActiveNodeId, String newStandbyNodeId) throws Exception{
		PreparedStatement pst = null;
		String query = "UPDATE " + TableConstants.TABLE_HASTATUS + " SET " + ColumnConstants.COL_HASTATUS_ACTIVENODEID + " = ?, "
				+ ColumnConstants.COL_HASTATUS_STANDBYNODEID+ " = ? WHERE " + ColumnConstants.COL_HASTATUS_ACTIVENODEID + " = ?";
		try{
			pst = connection.prepareStatement(query);
	...
	public static boolean isHANode(Connection connection, String nodeId) throws Exception{
		PreparedStatement pst = null;
		String query = "SELECT * FROM " + TableConstants.TABLE_HASTATUS + " WHERE " + ColumnConstants.COL_HASTATUS_ACTIVENODEID + " = ? OR "
	...
	public static String getStandbyNodeId(Connection connection, String nodeId) throws Exception{
		PreparedStatement pst = null;
		String query = "SELECT * FROM " + TableConstants.TABLE_HASTATUS + " WHERE " + ColumnConstants.COL_HASTATUS_ACTIVENODEID + " = ?";
		ResultSet rs = null;
		try{
			pst = connection.prepareStatement(query);
			pst.setString(1, nodeId);
	...
	public static String getActiveNodeId(Connection connection, String nodeId) throws Exception{
		PreparedStatement pst = null;
		String query = "SELECT * FROM " + TableConstants.TABLE_HASTATUS + " WHERE " + ColumnConstants.COL_HASTATUS_STANDBYNODEID + " = ?";
		ResultSet rs = null;
		try{
			pst = connection.prepareStatement(query);
			pst.setString(1, nodeId);
	...
	public static void removeEntry(Connection connection, String activeNodeId) throws Exception{
		PreparedStatement pst = null;
		String query = "DELETE FROM " + TableConstants.TABLE_HASTATUS + " WHERE " + ColumnConstants.COL_HASTATUS_ACTIVENODEID + " = ?";
		try{
			pst = connection.prepareStatement(query);
			pst.setString(1, activeNodeId);
			pst.execute();
