...
public class ReportManager {
	...
	public static String generateHDFSStatusReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
	...
	public static String generateMapReduceStatusReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
	...
	private static String generateNamenodeStatusReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateDatanodeStatusReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateResourceManagerStatusReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateNodeManagerStatusReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateNamenodeInfoReport(Timestamp startTimestamp, Timestamp endTimestamp, int exportType, Host host, Node node,String title) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
		String fileName = QueryIOConstants.NN_DETAIL_REPORT + host.getHostIP().replace(".", "_") +"_"+ currentTimestamp.getDate() + "_" + 
	...
	private static String generateDatanodeInfoReport(Timestamp startTimestamp, Timestamp endTimestamp, int exportType, Host host, Node node, String title) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateResourceManagerInfoReport(Timestamp startTimestamp, Timestamp endTimestamp, int exportType, Host host, Node node, String title) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateNodeManagerInfoReport(Timestamp startTimestamp, Timestamp endTimestamp, int exportType, Host host, Node node, String title) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
	...
	private static String generateStorageForecastReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
		String fileName = "StorageForecastReport_"+ currentTimestamp.getDate() + "_" + 
		currentTimestamp.getMonth() + "_" + (1900 + currentTimestamp.getYear() + "_" + 
		currentTimestamp.getHours() + "_" + currentTimestamp.getMinutes()) ;		
	...
	private static void logReport(ArrayList attachments, String message) throws Exception{
		...
		{
			NotificationHandler.generateLogNotification(notifMgr, message + " - " + attachments.toString(), notify,true);
		}
		else{
			throw new Exception("Error Generating Notification");
		}
	...
	public static DWRResponse mailNodeReport(String nodeId, ArrayList exportFormatList, ArrayList usersIdList, String title, String startTime, String endTime){
		...
		try{
			...
			if(node.getNodeType().equals(QueryIOConstants.NAMENODE)){
				for(int i = 0; i < exportFormatList.size(); i ++){
					...
					String reportName = generateNamenodeInfoReport(startTimestamp, endTimestamp, exportId, host, node, title);
					if (exportId == ExportConstants.EXPORT_TYPE_HTML)
					{
						File reportHTML = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName);
						File reportHTMLFiles = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName + QueryIOConstants.REPORTS_HTML_FILES_APPEND);
						ArrayList listFiles = new ArrayList();
						listFiles.add(reportHTML);
						listFiles.add(reportHTMLFiles);
						String destZipFile = EnvironmentalConstants.getReportsDirectory() + File.separator + reportName.replace(".html", ".zip");
				...
				for(int i = 0; i < exportFormatList.size(); i ++){
					int exportId = Integer.parseInt((String)exportFormatList.get(i));
					String reportName = generateDatanodeInfoReport(startTimestamp, endTimestamp, exportId, host, node, title);
					if (exportId == ExportConstants.EXPORT_TYPE_HTML)
					{
						File reportHTML = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName);
						File reportHTMLFiles = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName + QueryIOConstants.REPORTS_HTML_FILES_APPEND);
				...
				for(int i = 0; i < exportFormatList.size(); i ++){
					int exportId = Integer.parseInt((String)exportFormatList.get(i));
					String reportName = generateResourceManagerInfoReport(startTimestamp, endTimestamp, exportId, host, node, title);
					if (exportId == ExportConstants.EXPORT_TYPE_HTML)
					{
						File reportHTML = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName);
						File reportHTMLFiles = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName + QueryIOConstants.REPORTS_HTML_FILES_APPEND);
				...
				for(int i = 0; i < exportFormatList.size(); i ++){
					int exportId = Integer.parseInt((String)exportFormatList.get(i));
					String reportName = generateNodeManagerInfoReport(startTimestamp, endTimestamp, exportId, host, node, title);
					if (exportId == ExportConstants.EXPORT_TYPE_HTML)
					{
						File reportHTML = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName);
						File reportHTMLFiles = new File(EnvironmentalConstants.getReportsDirectory() + File.separator + reportName + QueryIOConstants.REPORTS_HTML_FILES_APPEND);
...
public static DWRResponse logNodeReport(String nodeId, ArrayList exportFormatList, String title, String startTime, String endTime){
	...
	public static String viewNodeReport(String nodeId, String exportFormat, String title, String startTime, String endTime){
		Timestamp startTimestamp = parseDateString(startTime);
		Timestamp endTimestamp = parseDateString(endTime);
		Connection connection = null;
		String reportPath = "";
		try{
			...
			if(node.getNodeType().equals(QueryIOConstants.NAMENODE))
			{
					reportPath = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateNamenodeInfoReport(startTimestamp, endTimestamp, Integer.parseInt(exportFormat), host, node, title);
			}			
			else if(node.getNodeType().equals(QueryIOConstants.DATANODE))
			{
				reportPath = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateDatanodeInfoReport(startTimestamp, endTimestamp, Integer.parseInt(exportFormat), host, node, title);
			}
			else if(node.getNodeType().equals(QueryIOConstants.RESOURCEMANAGER))
			{
				reportPath = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateResourceManagerInfoReport(startTimestamp, endTimestamp, Integer.parseInt(exportFormat), host, node, title);
			}
			else if(node.getNodeType().equals(QueryIOConstants.NODEMANAGER))
			{
				reportPath = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateNodeManagerInfoReport(startTimestamp, endTimestamp, Integer.parseInt(exportFormat), host, node, title);
			}
		}catch(Exception e)
		{
	...
	public static DWRResponse mailGeneralReport(ArrayList reportTypeList, ArrayList exportFormatList, ArrayList usersIdList, String title, String startTime, String endTime){
		// generate attachments
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(reportTypeList + " " +exportFormatList + " " + usersIdList + " " + title + " " + startTime + " " + endTime);
	...
	public static DWRResponse logGeneralReport(ArrayList reportTypeList, ArrayList exportFormatList, String title, String startTime, String endTime){
		// generate attachments
...
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(reportTypeList + " " +exportFormatList + " " + usersIdList + " " + title + " " + startTime + " " + endTime);
	...
	public static String viewGeneralReport(ArrayList reportTypeList, String exportFormat, String title, String startTime, String endTime){
		// generate attachments
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(reportTypeList + " " +exportFormat + " " + " " + title + " " + startTime + " " + endTime);
		...
		try{
			...
			for(int var = 0; var < reportTypeList.size(); var++){
				...
				case QueryIOConstants.REPORT_DATANODE_STATUS:				
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateDatanodeStatusReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				
				case QueryIOConstants.REPORT_HDFS_STATUS:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateHDFSStatusReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				
				case QueryIOConstants.REPORT_MAPREDUCE_STATUS:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateMapReduceStatusReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				
				case QueryIOConstants.REPORT_NAMENODE_STATUS:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateNamenodeStatusReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				
				case QueryIOConstants.REPORT_RM_STATUS:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateResourceManagerStatusReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				
				case QueryIOConstants.REPORT_NM_STATUS:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateNodeManagerStatusReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				...
				case QueryIOConstants.REPORT_TOP10:
					break;
				case QueryIOConstants.REPORT_ALERT_STATUS:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateAlertReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				case QueryIOConstants.REPORT_STORAGE_FORECAST:
					attachments = QueryIOConstants.REPORTS_QUERYIO + File.separator + generateStorageForecastReport(Integer.parseInt(exportFormat), title, start, end);
					break;
				case QueryIOConstants.REPORT_IO_STATUS:
					break;
				default: return "";
	...
	public static ArrayList getReportName(String reportIdList){
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("getReportName reportIdList: " + reportIdList);
		String reportType[] = reportIdList.substring(1,reportIdList.length()-1).split(",");
		
		for (String reportId:reportType)
		{
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("getReportName reportId: " + reportId);
			if (!(reportId.trim().equals(QueryIOConstants.EMPTY_STRING)))
				reportTypeList.add(reportId.trim());
		}
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("getReportName reportTypeList: " + reportTypeList);
	...
	private static String generateAlertReport(int exportType, String title, Timestamp startTimestamp, Timestamp endTimestamp) throws Exception{
	...
	public static String generateBigQueryResult(String nodeId, String title, int exportType,int pages, String query) throws Exception{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Exporting big query result pages :"+pages);
		
		Timestamp currentTimestamp = new Timestamp(System.currentTimeMillis());
		String fileName = "BigQueryResult_"+ currentTimestamp.getDate() + "_" + 
		currentTimestamp.getMonth() + "_" + (1900 + currentTimestamp.getYear() + "_" + 
		currentTimestamp.getHours() + "_" + currentTimestamp.getMinutes()) ;		
	...
	public static DWRResponse emailBigDataReport(ArrayList reportTypeList, ArrayList exportFormatList, ArrayList usersIdList, String namenodeId, String queryId){
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Email Details :"+reportTypeList + " " +exportFormatList + " " + usersIdList + " " + namenodeId + " " + queryId);
		...
		try{
			...
			String designFilePath = BigQueryManager.getDesignFilePath(namenodeId, queryId, RemoteManager.getLoggedInUser());
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Attachments:"+attachments);
		}catch(Exception e){
				AppLogger.getLogger().fatal("Error Exporting Report", e);
				dwrResponse.setDwrResponse(false, e.getMessage(), 500);
				return dwrResponse;
