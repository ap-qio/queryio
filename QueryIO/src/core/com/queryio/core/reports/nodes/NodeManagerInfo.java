...
public class NodeManagerInfo extends AbstractExportableNode {
	...
	public NodeManagerInfo(String exportedFileName, Timestamp startTimestamp, Timestamp endTimestamp, Host host,
			Node node, String title) {
		super("NodeManager Info Report Id: " + node.getId() + " Host: " + host.getHostIP(), 0,
				"NodeManager Info Report Id: " + node.getId() + " Host: " + host.getHostIP(), exportedFileName,
				"NodeManager Info Report Id: " + node.getId() + " Host: " + host.getHostIP());
	...
	public IExportableItem[] getItems() {
		ArrayList chartDataList = ChartManager.getNodeManagerChartDataBetweenTime(node.getId(), startTimestamp,
				endTimestamp);
	...
	private AppChart getPieChart() {
		...
		if (!(containersCompletedPer == 0 && containersFailedPer == 0 && containersKilledPer == 0
				&& containersRunningPer == 0 && containersInitingPer == 0)) {
