...
public final class NotificationHandler {
	...
	public static void sendNotifications(final NotifyBean notify, final HashMap hm, final String subject,
		...
		for (final Iterator iterNotifiers = hm.keySet().iterator(); iterNotifiers.hasNext();) {
			...
			if (notifierType.equals(INotifierConstants.NOTIF_EMAIL)) {
				...
				// }
				// }
				// generateSmsNotification(notifMgr, subject, htmlMsg, notify,
				// (ArrayList) hm.get(notifierType), false, implicitSMS);
	 ...
	 * @throws Exception
	 */
	public static void generateEmailNotification(final NotificationManager notifMgr, final String notificationSubject,
		...
		if ((usersToBeNotified == null) || (usersToBeNotified.size() == 0)) {
			// AppLogger.getLogger().fatal("Email notification is called without
			// any recepients, mail subject: " + notificationSubject);
			return;
		}
		...
		for (final Iterator iter = usersToBeNotified.iterator(); iter.hasNext();) {
			...
			if (email != null && !email.equals("") && email != null) {
				if (emailNotifier == null) {
					...
					emailPropSet.addProperty(INotifierConstants.SMTP_SECURE,
							String.valueOf(notify.isSecuredProtocol()));
					...
					// if
					// (controllerState.equals(ControllerManager.CONTROLLER_STATE_POLLING))
					// emailPropSet.addProperty(INotifierConstants.NOTIF_PRIORITY,
					// "1");
					// else
					// emailPropSet.addProperty(INotifierConstants.NOTIF_PRIORITY,
					// "3");
	 ...
	 * @throws Exception
	 */
	// static void generateSmsNotification(final NotificationManager notifMgr,
	// final String subject,
	// final String notificationMessage, final NotifyBean notify, final
	// ArrayList usersToBeNotified,
	...
	// // initialize the email notifier
	// smsNotifier =
	// notifMgr.getEventNotifier(INotifierConstants.ALERT_RAISED_NOTIFICATION,
	...
	// // creating the propertyset for the email notifier
	// smsPropSet = new
	// PropertySet(INotifierConstants.MC_SMS_NOTIFIER_PROPERTYSET);
	//
	// smsPropSet.addProperty(INotifierConstants.SENDERS_MOBILE_NO,
	// notify.getSmsNumber());
	// smsPropSet.addProperty(INotifierConstants.COM_PORT,
	// notify.getSmsSerialPort());
	// smsPropSet.addProperty(INotifierConstants.MOBILE_MFG,
	// notify.getSmsManufacturer());
	// smsPropSet.addProperty(INotifierConstants.MOBILE_MODEL,
	// notify.getSmsModel());
	// smsPropSet.addProperty(INotifierConstants.BAUD_RATE,
	// notify.getSmsBaudRate());
	//
	// smsNotifier.setPropertySet(smsPropSet);
	// }
	//
	// smsPropSet.addProperty(INotifierConstants.RECIPIENTS_MOBILE_NO,
	// mobileNo);
	// }
	// }
	// if (found)
	// {
	// notifEvent = new
	// NotificationEvent(INotifierConstants.ALERT_RAISED_NOTIFICATION);
	// notifEvent.addProperty(INotifierConstants.ALERT_MESSAGE,
	// notificationMessage);
	//
	// try
	// {
	...
	// {
	// generateEmailNotification(notifMgr, subject, notificationMessage, notify,
	// smsThruEmailList, reThrowException);
	// }
	// }

	public static void generateLogNotification(final NotificationManager notifMgr, final String notificationMessage,
			final NotifyBean notify, boolean reThrowException) throws Exception {
	...
	public static void validateNotification(final NotifyBean notifyBean, String sNotify) throws Exception {
		...
		if (INotifierConstants.NOTIF_EMAIL.equals(sNotify)) {
			...
			NotificationHandler.generateEmailNotification(notifMgr, "QueryIO: Validate Notification Settings",
					"This is a test mail from QueryIO to validate email notification settings.", notifyBean,
					usersToBeNotified, true);
	...
	public static boolean isEmailValid(String email) {
		...
		while (stk.hasMoreTokens()) {
			...
			if (!((indexOfAtTheRate > 0 /* @ is not the first char */)
					&& (indexOfAtTheRate < length
							- 1 /* @ is not the last char */)
					...
					&& (indexOfAtTheRate < lastIndexOfDot /* @ comes before . */) && (lastIndexOfDot
							- indexOfAtTheRate != 1 /*
													 * . and @ don't come
													 * together
													 */)/*
