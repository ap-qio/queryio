...
public class Consolidator {
	...
	public void consolidate(Timestamp currentTimestamp, Timestamp oldestEntryTimestamp, String id, String type,
			boolean consolidateNodeStatus) throws Exception {
	 ...
	 *             the exception
	 */
	public void consolidate(Timestamp currentTimestamp, String id, String type, boolean consolidateNodeStatusData)
			throws Exception {
	...
	private void consolidateNodeStatusData(Timestamp currentTimestamp, String nodeId) throws Exception {
		...
		// AppLogger.getLogger().debug(selectSb.toString());
		...
		// if(AppLogger.getLogger().isDebugEnabled())
		// AppLogger.getLogger().debug(insertSb.toString());
	...
	private void consolidateMonitorData(Timestamp currentTimestamp, String id, String type) throws Exception {
		...
		String consoldationTableName = QueryIOConstants.MONITORDATA_TABLE_PREFIX + id + "_" + type
				+ "_CONSOLIDATEDDATA";

		String[] nodeAttributes = ConsolidationUtility.getNodeConsolidationAttributes(this.connection,
				monitoringTableName);
	...
	/**
	 * Update last consolidate Timestamp in LAST_RANGEMONITORING_CONSOLDIATE
	 * table.
	 *
	 * @param currentTimestamp
	 *            current timestamp
