...
public class HOFManager {
	private static String confFile = EnvironmentalConstants.getAppHome() + "../../../"
			+ "hdfs-over-ftp/hdfs-over-ftp.conf";
	private static String hadoopPropFile = EnvironmentalConstants.getAppHome() + "../../../"
			+ "hdfs-over-ftp/hadoop.properties";
	...
	public static DWRResponse startServer() {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Performing start on hdfs-over-ftp server");
		...
		try {
			...
			final Process targetProcess = Runtime.getRuntime().exec(cmd, null,
					new File(EnvironmentalConstants.getAppHome() + "../../../" + "hdfs-over-ftp"));
			serverStarted = true;
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("targetProcess.exitValue()" + targetProcess.exitValue());
	...
	public static boolean stopServer(boolean checkIfAdmin) {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Performing stop on hdfs-over-ftp server");
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("targetProcess.exitValue()" + targetProcess.exitValue());
			if (targetProcess.exitValue() != 0) {
				String message = "Following error occured while stopping hdfs-over-ftp server: "
