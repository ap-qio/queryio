...
public class UserGroupDAO {
	...
	public static boolean isUserAddedToGroup(final Connection connection, final String userName, final String groupName)
			throws Exception {
		boolean added = false;
	...
	public static String getDefaultGroupForUser(final Connection connection, String userName) throws Exception {
		...
		try {
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_GET_DEFAULT_GROUP_FOR_USER);
			ps.setInt(1, userId);
	...
	public static void addUserToGroup(final Connection connection, String userName, String groupName, boolean isDefault)
			throws Exception {

		if (isUserAddedToGroup(connection, userName, groupName))
			return;
	...
	public static void deleteUserFromGroup(final Connection connection, int userId, String groupName) throws Exception {
		...
		if (groupId == -1)
			throw new Exception("Group with specified name does not exist");
	...
	public static void deleteUserFromAllGroups(final Connection connection, int userId) throws Exception {
		...
		try {
			ps = DatabaseFunctions.getPreparedStatement(connection,
					QueryConstants.PREPARED_QRY_DELETE_USER_FROM_ALL_GROUP);
			ps.setInt(1, userId);
			CoreDBManager.executeUpdateStatement(connection, ps);
		} finally {
			DatabaseFunctions.closePreparedStatement(ps);
