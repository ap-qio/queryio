...
public class PermissionsManager {
	...
	public static DWRResponse deleteGroup(String groupName) {
		...
		try {
			...
			if (getUserForGroup(groupName).size() > 0) {
				dwrResponse.setDwrResponse(false, "Please remove all users from this group before deleting the group",
						500);
				return dwrResponse;
			}
	...
	public static HashMap<String, ArrayList<User>> getAllUserForGroup() {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("getAllUserForGroup");
	...
	public static DWRResponse setOwnerAndPermissions(String nodeId, String path, String owner, String group,
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Setting file permissions");
		...
		SetPermissionsRequest request = new SetPermissionsRequest(nodeId, RemoteManager.getLoggedInUser(),
				new Path(path), owner, group, permissions, recursive);

		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Processing");
	...
	public static DWRResponse deleteFiles(String nodeId, String dirPath, ArrayList list) {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("Deleting files: " + path);
	...
	public static boolean setFileOwnerAndPermissions(String nodeId, String username, String path, String owner,
			String group, int permissions, boolean recursive) {
		SetPermissionsRequest request = new SetPermissionsRequest(nodeId, username, new Path(path), owner, group,
