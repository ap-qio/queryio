...
public class DiagnosisProcessExecutor implements Job {
	...
	private String startDiagnosisProcess(String jobName, String message, String diagnosisId, String namenodeId, long startIndex, long endIndex, String loggedInUser) 
	{
		String status  = null;
		try
		{
	...
	private String pollApplicationStatus(String applicationId)
	{
		if (applicationId != null)
		{
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("applicationStatus: " + applicationStatus);
	...
	private void mailStatus(String message, String status, ArrayList usersList) throws Exception
	{
		...
		if(notify.isEmailEnabled()){
			NotificationHandler.generateEmailNotification(notifMgr, "Scheduled MapReduce Job", message+" : "+ status, notify, usersList, true);
		}
		else{
	...
	private void logStatus(String message, String status) throws Exception
	{
		...
		if(notify.isLogEnabled()){
			NotificationHandler.generateLogNotification(notifMgr,"[Scheduled MapReduce Job] - " + message+" : "+ status, notify, true);
		}
		else{
			throw new Exception("Error Generating Notification"); 
		}
