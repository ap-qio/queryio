...
public class GetDiagnosisReport extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) {
		...
		for (int i = 0; i < conflicts.size(); i++) {
			...
			if (conflictObject.get("conflictTypeDescription")
					.equals(NamespaceConflict.CONFLICT_TYPE_METADATA_DIFFERNECE_DESC))
				cell.add(NamespaceConflict.CONFLICT_TYPE_METADATA_DIFFERNECE_DESC
						+ " <a href='javascript:BAR.showConflict(\"conflictDiv_" + (i + 1)
						+ "\");'>Details</a><div id='conflictDiv_" + (i + 1) + "' style = 'display : none;'>"
						+ conflictObject.get("conflictInfo") + "</div>");
			else
