...
public class DataBrowserManager extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Data Browser Servlet Intialized");
	}
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("Request received for dirPath(Encoded): " + dirPath + " For Node " + nodeId);
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("fetching summary table for nodeId: " + nodeId);
		SummaryTable summaryTable = null;
		if (query == null) {
			summaryTable = RemoteManager.listFiles(nodeId, request.getRemoteUser(), dirPath, crow, cpage);
		} else {
			summaryTable = RemoteManager.listSelectedFiles(nodeId, request.getRemoteUser(), dirPath, crow, cpage,
					query);
		}
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("got summary table");
		...
		if (summaryTable != null && summaryTable.getTotalRowCount() != 0) {
			...
			comparator.setSortColIndex(sidx == null || sidx.length() == 0 ? 0 : columnMap.get(sidx).intValue()); // default
																													// sort
																													// according
																													// to
																													// name
			...
			int totalRecord = crow != 0 && crow < summaryTable.getTotalRowCount()
					? (int) Math.ceil((float) summaryTable.getTotalRowCount() / crow) : 1;
			...
			for (int i = 0; i < summaryRow.size(); i++) {
				...
				for (int j = 0; j < summaryRowArray.size() - 1; j++) {
					if (j == 0) {
						...
						if (Collections.binarySearch(EnvironmentalConstants.getCommomFileTypeNames(),
								fileExtension) < 0) {
							fileExtension = "_blank";
						}
						String path = String.valueOf(summaryRowArray.get(0));
						String tempPath = path.replace("'", "\\'");
						tableRow.add("<input type=\"checkbox\" value=\"" + path
								+ "\" onclick=\"javascript:DataBrowser.clickCheckBox(this.id, this.checked); DataBrowser.setSelectedRowId(this);\" id=\"chkbox_"
								+ (i + 1) + "\">");
						if (query == null) {
							if (summaryRowArray.get(summaryRowArray.size() - 1) != null
									&& summaryRowArray.get(summaryRowArray.size() - 1).equals("dir")) {
								tableRow.add(
										"<img style=\"vertical-align:middle; width: 18px; height: 16px;\" src=\"images/folder_icon.png\"> &nbsp;<a style=\"text-decoration:none\" href=\"javascript:DataBrowser.listFiles(\'"
												+ URLEncoder.encode(dirPath + tempPath, "UTF-8")
												+ "\');\" ><span id=\"name\">" + path + "</span></a>");
								tableRow.add("<span id=\"kind\" style=\"display:none\">Directory</span>");
							} else {
								tableRow.add("<a target=\"_blank\" href=\"FileDownload?namenode=" + nodeId
										+ "&filePath=" + URLEncoder.encode(dirPath + path, "UTF-8")
										+ "\"><img style=\"vertical-align:middle; width: 18px; height: 16px;\"  src=\"images/extensions/"
										+ fileExtension + ".png\"></a> &nbsp;<a href=\"FileDownload?namenode=" + nodeId
										+ "&filePath=" + URLEncoder.encode(dirPath + path, "UTF-8")
										+ "\" target=\"_blank\"><span id=\"name\">" + path + "</span></a>");
							...
							if (summaryRowArray.get(summaryRowArray.size() - 1) != null
								...
								tableRow.add("<a target=\"_blank\" href=\"FileDownload?namenode=" + nodeId
										...
										+ "\"><img style=\"vertical-align:middle; width: 18px; height: 16px;\"  src=\"images/extensions/"
										+ fileExtension + ".png\"></a>&nbsp;<a href=\"FileDownload?namenode=" + nodeId
										+ "&filePath=" + URLEncoder.encode(path, "UTF-8") + "\"><span id=\"name\">"
										+ path + "</span></a>");
								tableRow.add("<span id=\"kind\" style=\"display:none\">File</span>");
							}
							continue;
			...
			for (int i = 0; i < 11; i++) {
				if (i == 6)
					cellArray.add(
							"<center><label style = 'font-size : 13px;'>No data available on cluster</label></center>");
				else
					cellArray.add("");
			}
		...
		// if(AppLogger.getLogger().isDebugEnabled())
		// AppLogger.getLogger().debug("RowData: " + rowData.toJSONString());
