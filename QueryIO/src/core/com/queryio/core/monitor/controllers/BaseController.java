...
public abstract class BaseController implements Runnable {
	...
	public final void setHistoricalValue(final ControllerAttribute ca, final Object value) {
		...
		if (ca.getColumnName().equals(ColumnConstants.COL_CPUUSAGE)) {
			if (!bFirstTimeHistoricalCPUUsage) {
				...
				if (curCPUTime != 0) {
					this.controllerHistoricalData.setValue(ca.getColumnName(),
							(float) ((curCPUTime - lastCPUTimeHistorical) * 100) / (float) curCPUTime);
				} else {
			...
			if (bNotFirstTime[indexPersistColNames]) {
				this.controllerHistoricalData.setValue(ca.getColumnName(),
						curValue - lastValHistorical[indexPersistColNames]);
	...
	public final void setSummaryValue(final ControllerAttribute ca, final Object value) {
		if (ca.getColumnName().equals(ColumnConstants.COL_CPUUSAGE)) {
			if (!bFirstTimeSummaryCPUUsage) {
				...
				if (curCPUTime != 0) {
					this.controllerSummaryData.setValue(ca.getColumnName(),
							(float) ((curCPUTime - lastCPUTimeSummary) * 100) / (float) curCPUTime);
				} else {
			...
			if (!bFirstTimeSummaryGCTime) {
				...
				if (curGCTime != 0) {
					this.controllerSummaryData.setValue(ca.getColumnName(),
							(float) ((curGCTime - lastGCTimeSummary) * 100) / (float) curGCTime);
				} else {
					this.controllerSummaryData.setValue(ca.getColumnName(), 0);
				}
	...
	void stopCollectingData() {
		...
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("stopCollectingData called for " + nodeType + " on " + this.host.getHostIP());
	...
	void startCollectingData() {
		if (AppLogger.getLogger().isDebugEnabled())
			AppLogger.getLogger().debug("startCollectingData called for " + nodeType + " on " + this.host.getHostIP());
		...
		while (this.bCollectData) {
			try {
				if (this.bCollectData) {
					...
					this.controllerHistoricalData.setValue(ColumnConstants.COL_MONITORDATA_MONITOR_TIME,
							new Timestamp(curTime));
					...
					try {
						...
						if (nodeType.equals(QueryIOConstants.NAMENODE)) {
							// if(AppLogger.getLogger().isDebugEnabled())
							// AppLogger.getLogger().debug("Calling:
							// insertNameNodeMonitorData/updateNameNodeSummaryData");
							MonitorDAO.insertNameNodeMonitorData(connection, this.nodeId,
									this.controllerHistoricalData.getColumnNames(),
									this.controllerHistoricalData.getValues());
							MonitorDAO.updateNameNodeSummaryData(connection, this.nodeId,
									this.controllerSummaryData.getColumnNames(),
									this.controllerSummaryData.getValues());
						} else if (nodeType.equals(QueryIOConstants.DATANODE)) {
							// if(AppLogger.getLogger().isDebugEnabled())
							// AppLogger.getLogger().debug("Calling:
							// insertDataNodeMonitorData/updateDataNodeSummaryData");
							MonitorDAO.insertDataNodeMonitorData(connection, this.nodeId,
									this.controllerHistoricalData.getColumnNames(),
									this.controllerHistoricalData.getValues());
							MonitorDAO.updateDataNodeSummaryData(connection, this.nodeId,
									this.controllerSummaryData.getColumnNames(),
									this.controllerSummaryData.getValues());
						} else if (nodeType.equals(QueryIOConstants.RESOURCEMANAGER)) {
							// if(AppLogger.getLogger().isDebugEnabled())
							// AppLogger.getLogger().debug("Calling:
							// insertResourceManagerMonitorData/updateResourceManagerSummaryData");
							MonitorDAO.insertResourceManagerMonitorData(connection, this.nodeId,
									this.controllerHistoricalData.getColumnNames(),
									this.controllerHistoricalData.getValues());
							MonitorDAO.updateResourceManagerSummaryData(connection, this.nodeId,
									this.controllerSummaryData.getColumnNames(),
									this.controllerSummaryData.getValues());
						} else if (nodeType.equals(QueryIOConstants.NODEMANAGER)) {
							// if(AppLogger.getLogger().isDebugEnabled())
							// AppLogger.getLogger().debug("Calling:
							// insertNodeManagerMonitorData/updateNodeManagerSummaryData");
							MonitorDAO.insertNodeManagerMonitorData(connection, this.nodeId,
									this.controllerHistoricalData.getColumnNames(),
									this.controllerHistoricalData.getValues());
							MonitorDAO.updateNodeManagerSummaryData(connection, this.nodeId,
									this.controllerSummaryData.getColumnNames(),
									this.controllerSummaryData.getValues());
						}
						...
						AppLogger
								.getLogger().fatal(
										"Saving monitored data for nodetype: " + this.nodeType + " on hostname: "
												+ this.host.getHostIP() + " failed with exception: " + e.getMessage(),
										e);
					} finally {
						try {
							CoreDBManager.closeConnection(connection);
						} catch (Exception e) {
							AppLogger.getLogger().fatal("Connection could not be closed, Exception: " + e.getMessage(),
									e);
						}
					}
					...
					if (!notifiedOk) {
						...
						// AppLogger.getLogger().info("Monitoring " +
						// this.nodeType + " on " + this.host.getHostIP() + "
						// succeeded.");
					...
					StatusManager.addStatus(this.nodeId, System.currentTimeMillis(),
							QueryIOConstants.NODE_STATUS_SUSPENDED);
				}
			} catch (final Exception e) {
				AppLogger.getLogger().fatal("Monitoring failed for node: " + nodeId + ", " + e.getMessage(), e); // TODO:
																													// Temp
																													// Windows
																													// handling.
																													// uncomment
				...
				if ((failedAttempts > EnvironmentalConstants.getMonitorRetryCount()) && (!notifiedError)) {
					String subject = "Monitor Failure: " + this.nodeType + "[Node Id: " + this.nodeId + "] on "
							+ this.host.getHostIP();

					String message = "Monitoring " + this.nodeType + "[Node Id: " + this.nodeId + "] on "
							+ this.host.getHostIP() + " failed with exception: " + e.getMessage();
					...
					try {
						...
						if (failedAttempts <= 1) {
							AppLogger.getLogger().fatal("Monitoring " + this.nodeType + " on " + this.host.getHostIP()
