	...
	public void run() {
		...
		while (flag) {
			try {
				...
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(
						"Error occured during HiveServices monitoring for nodeId: "
								+ nodeId, e);
				flag = false;
			} finally {
				try {
...
//					AppLogger.getLogger().fatal(e1.getMessage(), e1);
	...
	public void stopCollectingData(){
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Stopping HiveServicesController for node: " + nodeId);
		this.flag = false;
		try {
			connection = CoreDBManager.getQueryIODBConnection();
			QueryIOServiceDAO.updateStatus(connection, nodeId, 
					QueryIOConstants.SERVICE_HIVE,
					QueryIOConstants.STATUS_STOPPED);
		} catch (Exception e) {
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug(
					"Failed to stop HiveServicesController for nodeId: "
							+ nodeId, e);
		} finally {
			try {
				CoreDBManager.closeConnection(connection);
