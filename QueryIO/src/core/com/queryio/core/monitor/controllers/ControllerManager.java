...
public class ControllerManager 
{
	...
	{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("setAllControllersDataFetchInterval() : " + dataFetchInterval);
		...
		if(controllerMap.size() != 0)
		{
			...
			while (itr.hasNext())
			{
				...
				if(controller!=null)	controller.setDataFetchInterval(dataFetchInterval);
				
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Updating data fetch interval for controller: " + controllerId);
			}
		}
	}
	
	public static void startNameNodeController(final Connection connection, final Host host, Node node, int dataFetchInterval) throws Exception
	{
		AbstractJMXController controller = new JMXMonitorController();

		ArrayList alControllerHistoricalAttributes = MonitorDAO.getNameNodeControllerHistoricalAttributes(connection, node.getId());
		ArrayList alControllerSummaryAttributes = MonitorDAO.getNameNodeControllerSummaryAttributes(connection, node.getId());
	...
	public static void startDataNodeController(final Connection connection, final Host host, Node node, int dataFetchInterval) throws Exception
	{
		AbstractJMXController controller = new JMXMonitorController();

		ArrayList alControllerHistoricalAttributes = MonitorDAO.getDataNodeControllerHistoricalAttributes(connection, node.getId());
		ArrayList alControllerSummaryAttributes = MonitorDAO.getDataNodeControllerSummaryAttributes(connection, node.getId());
	...
	public static void startResourceManagerController(final Connection connection, final Host host, Node node, int dataFetchInterval) throws Exception
	{
		AbstractJMXController controller = new JMXMonitorController();

		ArrayList alControllerHistoricalAttributes = MonitorDAO.getResourceManagerControllerHistoricalAttributes(connection, node.getId());
		ArrayList alControllerSummaryAttributes = MonitorDAO.getResourceManagerControllerSummaryAttributes(connection, node.getId());
		
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("alControllerHistoricalAttributes: " + alControllerHistoricalAttributes);
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("alControllerSummaryAttributes: " + alControllerSummaryAttributes);
	...
	public static void startNodeManagerController(final Connection connection, final Host host, Node node, int dataFetchInterval) throws Exception
	{
		AbstractJMXController controller = new JMXMonitorController();

		ArrayList alControllerHistoricalAttributes = MonitorDAO.getNodeManagerControllerHistoricalAttributes(connection, node.getId());
		ArrayList alControllerSummaryAttributes = MonitorDAO.getNodeManagerControllerSummaryAttributes(connection, node.getId());
	...
	public static void startHostMonitorController(final Connection connection, final Host host, final String username, final String password) throws Exception
	{
		HostMonitorController dm = new HostMonitorController();
		dm.setHost(host);
	...
	public static void stopNameNodeController(final String hostname, Node node)
	{
		...
		if(controller!=null)	controller.stopCollectingData();
		
		NodeConsolidationThread consolidationThread = (NodeConsolidationThread) consolidationThreadMap.get(node.getId());
		if(consolidationThread!=null)	consolidationThread.interrupt();
	...
	{
		QueryIOServicesController controller = (QueryIOServicesController) queryIoServicesMonitorMap.get(nodeId + QueryIOServicesController.class);
		if(controller!=null)	
			controller.stopCollectingData();		
	...
	{
		HiveServicesController controller = (HiveServicesController) hiveServicesMonitorMap.get(nodeId + HiveServicesController.class);
		if(controller!=null)	
			controller.stopCollectingData();		
		hiveServicesMonitorMap.remove(nodeId + HiveServicesController.class);
	}
	
	public static ArrayList getLiveAttributes(Connection connection, ArrayList alControllerAttributes, Host host, int connectorPort) throws Exception
	{
	...
	public static ArrayList getNameNodeLiveAttributes(Connection connection, Host hostname, int connectorPort) throws Exception
	{
		...
		{
			AppLogger.getLogger().fatal("Monitoring live attributes for Name Node failed with exception: " + e.getMessage(), e);
		}
	...
	public static ArrayList getDataNodeLiveAttributes(Connection connection, Host host, int connectorPort)
	{
		...
		{
			AppLogger.getLogger().fatal("Monitoring live attributes for Data Node failed with exception: " + e.getMessage(), e);
		}
	...
	public static boolean testNode(Host host, int connectorPort)
	{
		...
		try
		{
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("hostName responding at " + connectorPort);
			return true;
		}
		catch(Exception e)
		{
	...
	public static void stopAllControllers() {
		...
		if(queryIoServicesMonitorMap.size() != 0){
			...
			{
				controllerId = itr.next();
				QueryIOServicesController thread = (QueryIOServicesController) queryIoServicesMonitorMap.get(controllerId);
				if(thread!=null)	
	...
	public static boolean sendEmailNotification(String subject, String message)
	{
		...
		try
		{
			...
			NotificationHandler.generateEmailNotification(notifMgr, subject, message, NotifyDAO.getNotificationSettings(connection), userList, true);
