...
public class DurationFunctionExpression extends FunctionExpression {
	...
	public double calculate() throws ExpressionException {
		...
		// System.out.println(ruleKey + " Function: " + agFunction + " time: " +
		// dataTimeStamp + " value: " + value + " Sub-Expression: " +
		// this.args[4].toString());
		...
		if (startTime == -1 || startTime > dataTimeStamp - durationInMilliSecs) {
			...
			// System.out.println(ruleKey + " does have enough data, expression:
			// " + expression + " time diff time: " +
			// (((TimeValue)cachedObject.data.get(cachedObject.data.size() -
			// 1)).time - startTime) + " removed: " + r);
			...
			if (agFunction.equals("none")) {
				...
				if (n > 0) {
					...
					this.expressionParser.calculate();
					// System.out.println("Avg: " + (total/n) + " rule value: "
					// + this.expressionParser.getValue());
