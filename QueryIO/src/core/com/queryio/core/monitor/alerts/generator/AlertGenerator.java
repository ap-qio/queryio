...
public class AlertGenerator implements QueueItem {
	...
	private void generate() throws Exception {
		...
		// AppLogger.getLogger().debug("AlertGenerator generate()");
		...
		try {
			...
			// if(AppLogger.getLogger().isDebugEnabled())
			// AppLogger.getLogger().debug("NotifyBean: " + notify);
			...
			// Added to handle SMS via email internally without user configuring
			// SMS notification
			...
			if (alEnabledNotifications.size() == 0) {
				if (AppLogger.getLogger().isDebugEnabled())
					AppLogger.getLogger().debug("No notifications enabled");
				return;
			}
			...
			for (final Iterator iter = this.rules.iterator(); iter.hasNext();) {
				...
				// 6. for each enabled notifier which has got users for
				// notification iterate through the Users
				...
				if (DONOT_ALERT.equals(subject)) {
					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger()
								.debug("Not performing notification for rule id: " + rule.getRuleId() + " alert state: " //$NON-NLS-1$
										+ (alertRaised ? "raised" : "reset") + " as subject of the notification is "
										+ DONOT_ALERT);
					continue;
				}
				...
				if (msg != null && msg.indexOf(ALERT_DETAILS) != -1) {
					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger().debug("Sending mail with attribute values");
					...
					if (details == null) {
						...
						// boolean bHtml =
						// Boolean.getBoolean("notification.email.html");
						pw.print("Alert ");
						...
						if (attributes != null && attributes.size() > 0 && cd != null) {
							...
							for (int i = 0; i < attributes.size(); i++) {
								...
								if (alertAttributes != null && !alertAttributes.contains(ca.getName()))
									continue;
					...
					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger().debug("Sending mail without attribute values");
				}
				...
				for (int i = 0; i < users.size(); i++) {
					...
					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger().debug("User name: " + user.getUserName());
					String roleName = "";
					try {
						roleName = UserDAO.getRole(connection, user.getUserName());
					} catch (Exception e) {
						AppLogger.getLogger().fatal("Role name could not be obtained for user: " + user.getUserName(),
								e);
					}

					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger().debug("User role: " + roleName);

					if (roleName.equalsIgnoreCase(QueryIOConstants.ROLES_ADMIN)) {
						userList.add(user);
						if (AppLogger.getLogger().isDebugEnabled())
							AppLogger.getLogger().debug("Alert Notify User: " + user.getUserName());
					}
				}

				if (rule.getNotificationType().equalsIgnoreCase(INotifierConstants.NOTIF_EMAIL)) {
					if (AppLogger.getLogger().isDebugEnabled())
						AppLogger.getLogger().debug("Notification Type set for rule is: " + rule.getNotificationType());
