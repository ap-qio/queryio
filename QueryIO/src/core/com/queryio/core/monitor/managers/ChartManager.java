...
public class ChartManager {
	public static ArrayList getNameNodeChartData(String nodeId, String interval) {
		...
		try {
			...
			chartDataList = (ArrayList) getChartDataList(nodeSystemAttributes, monitorData, hostSystemAttributes,
					hostMonitorData);
	...
	public static ArrayList getNameNodeChartDataBetweenTime(String nodeId, Timestamp startTimestamp,
		...
		try {
			...
			MonitorData monitorData = MonitorDAO.getNameNodeMonitorData(connection, nodeId, startTimestamp,
					endTimestamp);

			ArrayList hostSystemAttributes = MonitorDAO.getHostSystemAttributes(connection);
			MonitorData hostMonitorData = MonitorDAO.getHostMonitorData(connection, host.getId(), startTimestamp,
					endTimestamp);

			chartDataList = (ArrayList) getChartDataList(systemAttributes, monitorData, hostSystemAttributes,
					hostMonitorData);
	...
	public static ArrayList getDataNodeChartData(String nodeId, String interval) {
		...
		try {
			...
			chartDataList = (ArrayList) getChartDataList(monitorAttributes, monitorData, hostSystemAttributes,
					hostMonitorData);
		} catch (Exception e) {
	...
	public static ArrayList getDataNodeChartDataBetweenTime(String nodeId, Timestamp startTimestamp,
			Timestamp endTimestamp) {
		ArrayList chartDataList = new ArrayList();
		Connection connection = null;
		try {
			...
			ArrayList monitorAttributes = MonitorDAO.getDataNodeSystemAttributes(connection);
			MonitorData monitorData = MonitorDAO.getDataNodeMonitorData(connection, nodeId, startTimestamp,
					endTimestamp);
	...
	public static ArrayList getResourceManagerChartDataBetweenTime(String nodeId, Timestamp startTimestamp,
			Timestamp endTimestamp) {
		ArrayList chartDataList = new ArrayList();
		Connection connection = null;
		try {
			...
			ArrayList monitorAttributes = MonitorDAO.getResourceManagerSystemAttributes(connection);
			MonitorData monitorData = MonitorDAO.getResourceManagerMonitorData(connection, nodeId, startTimestamp,
					endTimestamp);
			...
			AppLogger.getLogger()
					.fatal("getResourceManagerChartDataBetweenTime failed with exception: " + e.getMessage(), e);
		} finally {
			try {
				CoreDBManager.closeConnection(connection);
			} catch (Exception e) {
	...
	public static ArrayList getNodeManagerChartDataBetweenTime(String nodeId, Timestamp startTimestamp,
			Timestamp endTimestamp) {
		ArrayList chartDataList = new ArrayList();
		Connection connection = null;
		try {
			...
			ArrayList monitorAttributes = MonitorDAO.getNodeManagerSystemAttributes(connection);
			MonitorData monitorData = MonitorDAO.getNodeManagerMonitorData(connection, nodeId, startTimestamp,
					endTimestamp);
			...
			AppLogger.getLogger().fatal("getNodeManagerChartDataBetweenTime failed with exception: " + e.getMessage(),
					e);
		} finally {
			try {
				CoreDBManager.closeConnection(connection);
	...
	public static List getChartDataList(List systemAttributesList, MonitorData monitorData,
			List hostSystemAttributesList, MonitorData hostMonitorData) {
	...
	public static ArrayList getChartDataList(List systemAttributesList, MonitorData monitorData) {
		...
		for (int i = 1; i < monitorData.getAttributeCount(); i++) // 0th
																	// attribute
																	// is
																	// timestamp.
		{
			for (int j = 0; j < systemAttributesList.size(); j++) {
				if (((SystemAttribute) systemAttributesList.get(j)).getAttributeName()
						.equals(monitorData.getAttributeData(i).getName().toUpperCase())) {
					...
					if (addGroupToList == true) {
						...
						attributeMap.put(monitorData.getAttributeData(i).getName().toUpperCase(),
								groupNames.size() - 1);
					}
					break;
				}
		...
		for (int i = 1; i < monitorData.getAttributeCount(); i++) {
			((ChartData) chartDataList
					.get((Integer) attributeMap.get(monitorData.getAttributeData(i).getName().toUpperCase())))
							.addSeries(new Series(monitorData.getAttributeData(i).getDescription().toUpperCase(),
									monitorData.getAttributeData(i).getValues()));
		}

		return chartDataList;
	}

	public static ChartData getStatusChartData(ArrayList systemAttributesList, MonitorData monitorData,
			String nodeType) {
		ArrayList chartDataList = new ArrayList();
		HashMap attributeMap = new HashMap();
																	...
																	// timestamp.
		{
			for (int j = 0; j < systemAttributesList.size(); j++) {
				if (((SystemAttribute) systemAttributesList.get(j)).getAttributeName()
						.equals(monitorData.getAttributeData(i).getName())) {
					boolean addGroupToList = true;
					for (int p = 0; p < groupNames.size(); p++) {
						if (groupNames.get(p).equals(((SystemAttribute) systemAttributesList.get(j)).getGroupName())) {
							addGroupToList = false;
		...
		if (nodeType.equals(QueryIOConstants.NAMENODE))
			statusGroupName = QueryIOConstants.NAMENODE_STATUS_GROUP_NAME;
		else
			statusGroupName = QueryIOConstants.JOBTRACKER_STATUS_GROUP_NAME;
		...
		for (int i = 1; i < monitorData.getAttributeCount(); i++) {
			...
			if (insertIndex == statusGroupIndex) {
				((ChartData) chartDataList.get(insertIndex)).addSeries(new Series(
						monitorData.getAttributeData(i).getDescription(), monitorData.getAttributeData(i).getValues()));
			}
		}
