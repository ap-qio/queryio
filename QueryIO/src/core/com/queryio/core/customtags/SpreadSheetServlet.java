	...
	public static void processRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (type.equalsIgnoreCase(BigQueryIdentifiers.TYPE_REMOTE_MODEL)) {
				...
				String isRequestContainsMultipleQueries = request
						.getParameter(BigQueryIdentifiers.ISCONTAINSMULTIPLEQUERIES);
						...
						for (int i = 0; i < queryIdArray.size(); i++) {
							...
							if ((String) queryIdArray.get(i) != null) {
								if (i > 0)
									writer.write(",");
								AppLogger.getLogger()
										.debug("Request send for qid from json" + (String) queryIdArray.get(i));
								BigQueryManager.getSpreadSheetResults((String) queryIdArray.get(i), nameNodeId,
										userName, writer, isRunQuery);
