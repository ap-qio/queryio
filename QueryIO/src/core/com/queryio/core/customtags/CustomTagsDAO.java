...
public class CustomTagsDAO {
	public static HashMap getAllAvailableTags(final Connection connection, String tableName, DBTypeProperties props)
			throws SQLException {
	...
	public static JSONObject getAllAvailableTags(final Connection connection, ArrayList tableNames,
			DBTypeProperties props) throws SQLException {
			...
			try {
				...
				for (int i = 1; i <= numCol; i++) {
					...
					if ("blocks".equalsIgnoreCase(columnName))
						continue;
					// if(!columnName.equals(ColumnConstants.COL_TAG_VALUES_BLOCKSIZE)
					// &&
					// !columnName.equals(ColumnConstants.COL_TAG_VALUES_REPLICATION))
					dataType = GenericDBConfigManager.getGenericDataType(meta.getColumnTypeName(i), props);
					tags.put(columnName, dataType);
	...
	public static Map<String, Boolean> getAllTagTableNames(final Connection connection, String nameNodeId)
			throws SQLException {
		...
		try {
			...
			for (String tableName : tableNameList) {
				if (tableName.equalsIgnoreCase(TableConstants.TABLE_LOOKUP)
						|| tableName.equalsIgnoreCase(TableConstants.TABLE_DIRECTORIES)
	...
	public static TableModel getBigQueryResultTableModel(final Connection connection, String query, String tableName,
			int records) {
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i = 1; i <= rsmd.getColumnCount(); i++) {
					if (rsmd.getColumnName(i).equals(ColumnConstants.COL_TAG_VALUES_ACCESSTIME)
							|| rsmd.getColumnName(i).equals(ColumnConstants.COL_TAG_VALUES_MODIFICATIONTIME)) {
	...
	public static HashMap getResultSetColumns(final Connection connection, DBTypeProperties props, String query)
			throws SQLException {
	...
	public static HashMap getResultSetColumnsHive(final Connection hiveConnection, DBTypeProperties props, String query,
