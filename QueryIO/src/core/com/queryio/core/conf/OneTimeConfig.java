...
{	
	/*
	 * Installation of hadoop on remote system.
	 * return true if installation is successful.
	 */
	static boolean installHadoop(Host host, String userName, String password, String sshPrivateKey, String javaHomePath, String port, boolean isLocal) throws Exception
	{
...
//		updateLoggerPropertiesFile(host.getInstallDirPath());
		...
		synchronized(OneTimeConfig.class) 
		{
			if(!zipFile.exists())
			{
					File directoryToZip1 = new File(StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME);
					File directoryToZip2 = new File(StartupParameters.getHadoopDirLocation() + QueryIOConstants.QUERYIOAGENT_DIR_NAME);
					File directoryToZip3 = new File(StartupParameters.getHadoopDirLocation() + QueryIOConstants.QUERYIOSERVERS_DIR_NAME);
					File directoryToZip4 = new File(StartupParameters.getHadoopDirLocation() + QueryIOConstants.MAPREDRESOURCE);
					File directoryToZip5 = new File(StartupParameters.getHadoopDirLocation() + QueryIOConstants.HIVE_DIR_NAME);
					File directoryToZip6 = new File(StartupParameters.getHadoopDirLocation() + QueryIOConstants.SCRIPTS_DIR_NAME);
					...
					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Compression performed.");	        
			}
		}
		...
		if (isLocal)
		{
			...
			try
			{
				...
				String remoteDir = host.getInstallDirPath() != null ? host.getInstallDirPath() : StartupParameters.getDefaultDirRemote();
        		...
        		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("copyFile: " + scpSuccessful);
        		...
        		if (scpSuccessful)
        		{
        			if(SystemUtils.IS_OS_WINDOWS) {
        				...
        				if(scpSuccessful) {        					
        					String cmd = "java -cp " + remoteDir + "UnzipUtil.jar com.queryio.common.UnzipUtil " + (remoteDir + zipFileName) + " " + remoteDir;
        				...
        				{
        					final StreamPumper spError = new StreamPumper(new BufferedReader(new InputStreamReader(process
        				...
        				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("errorWriter.toString()" + errorWriter.toString());
        				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("inputWriter.toString()" + inputWriter.toString());
        				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("targetProcess.exitValue()" + process.exitValue());
        				...
        				inputWriter = new StringWriter();
        				final StreamPumper spInput1 = new StreamPumper(new BufferedReader(new InputStreamReader(process
        						.getInputStream())), inputWriter);
        				spInput1.start();
        				errorWriter = new StringWriter();
        				if (process.getErrorStream() != null)
		...
		else
		{
	        ...
	        try
	        {
	        	...
	        	if(password != null){
	        		...
	        		session = remoteExec.createSessionWithPrivateKeyFile(host.getHostIP(), userName, sshPrivateKey, Integer.parseInt(port));
	        	} else {
	        	...
	        	if (session != null)
	        	{
	        		...
	        		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("sftpFile: " + scpSuccessful);
	        		...
	        		if (scpSuccessful)
	        		{
	        			...
	        			if(isUnzipUtilCopied) {	        				
	        				...
	        				String cmd = null;
	        				if(host.isWindows()) {
	        					cmd = "java -cp " + remoteDir + "UnzipUtil.jar com.queryio.common.UnzipUtil " + (remoteDir + zipFileName) + " " + remoteDir + " && cd " + remoteDir + " && del /F " + (remoteDir + zipFileName);
	        				} else {
	        					cmd = "java -cp " + remoteDir + "UnzipUtil.jar com.queryio.common.UnzipUtil " + (remoteDir + zipFileName) + " " + remoteDir + "; cd " + remoteDir + "; chmod -R 700 *; rm -f " + (remoteDir + zipFileName);
	        				}
	...
	public static boolean isWindows(SSHRemoteExecution remoteExec, Session session) {
		try {
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("ver command output for windows : " + uname);
	...
	private static void updateJavaHome(String javaHomePath) throws IOException {
		...
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("updateJavaHome()");
		String[] filePaths = {StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/bin/yarn",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/datanode-conf/hadoop-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/namenode-conf/hadoop-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/nodemanager-conf/hadoop-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/resourcemanager-conf/hadoop-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/secondarynamenode-conf/hadoop-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/nodemanager-conf/yarn-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/resourcemanager-conf/yarn-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/nodemanager-conf/mapred-env.sh",
				StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME + "/etc/resourcemanager-conf/mapred-env.sh",				
	...
	private static void updateHadoopHome() throws IOException {
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("updateHadoopHome()");
		String hadoopHome = StartupParameters.getHadoopDirLocation() + QueryIOConstants.HADOOP_DIR_NAME;
		String[] filePaths = {StartupParameters.getHadoopDirLocation() + QueryIOConstants.HIVE_DIR_NAME + "/conf/hive-env.sh"};
		for(String filePath : filePaths){
			File in = new File(filePath);
			File out = new File(filePath);
	...
	private static void updateQueryIOServicsSerrverConfigFile(String servicesPort) throws IOException {
		String filePath = StartupParameters.getHadoopDirLocation() + QueryIOConstants.QUERYIOSERVERS_DIR_NAME+ "/conf/server_service.xml";
		
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("updateQueryIOServicsSerrverConfigFile()");
...
//	private static void updateAgentQueryIOPropertiesFile(String queryIOAgentPort) throws IOException{
//		String propFilePath = StartupParameters.getHadoopDirLocation() + QueryIOConstants.QUERYIOAGENT_DIR_NAME + "/webapps/" + QueryIOConstants.AGENT_QUERYIO + "/conf/queryioagent.properties";
//		
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("updateAgentQueryIOPropertiesFile()");
//		
...
//	    
//	    String queryIOGroupInfoURL = protocol + "://" + StaticUtilities.getHostAddress() + ":" + request.getServerPort() + "/queryio/GetUserGroupInformation";
//		
//	    if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("queryIOGroupInfoURL: " + queryIOGroupInfoURL);
//	    
//	    try
//	    {
//	    	reader = new BufferedReader(new FileReader(in));
...
//	}
	...
	private static void updateLoggerPropertiesFile(String homeDir) throws Exception{
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("updateLoggerPropertiesFile()");
		String propFilePath = StartupParameters.getHadoopDirLocation() + QueryIOConstants.QUERYIOSERVERS_DIR_NAME + "/webapps/hdfs-over-ftp/WEB-INF/log4j.properties";
		updateLoggerPropertiesFile(propFilePath, "ftp", homeDir, "ftpserver");
		
		propFilePath = StartupParameters.getHadoopDirLocation() + QueryIOConstants.QUERYIOSERVERS_DIR_NAME + "/webapps/os3server/WEB-INF/os3log4j.properties";
		updateLoggerPropertiesFile(propFilePath, "os3", homeDir, "os3server");
	}
	
	private static void updateLoggerPropertiesFile(String propFilePath, String prefix, String homeDir, String logFileName) throws Exception{
		File in = new File(propFilePath);
		File out = new File(propFilePath);
	...
	static void uninstallHost(Host host, String userName, String password, String sshPrivateKey, String port, boolean isLocal) throws Exception{
        if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Uninstalling host");
        	...
        	try
        	{
        		...
        		if (session != null)
        		{
        			...
        			if(password!=null){
        				...
        				pid = getRemotePIDusingPrivateKey(host, userName, sshPrivateKey, QueryIOConstants.QUERYIOAGENT_DIR_NAME, port);
        			}
        			if(pid != null){
        				cmd += "kill -9 " + pid + "; ";
        			}
	...
	public static String getRemotePID(Host host, String userName, String password, String procName, String port) throws Exception{
        if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Fetching remote PID for QueryIOAgent");
        SSHRemoteExecution remoteExec = null;
        Session session = null;
        ...
        try
        {
            ...
            if (session != null)
            {
                ...
                if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("PID: " + procPID);
            }
        }
		finally
        {
	...
	public static String getRemotePIDusingPrivateKey(Host host, String userName, String sshPrivateKey, String procName, String port) throws Exception{
        if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Fetching remote PID for QueryIOAgent");
        SSHRemoteExecution remoteExec = null;
        Session session = null;
        String procPID = QueryIOConstants.EMPTY_STRING;
...
//                if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Remote command output: " + remoteExec.getOutput());
                if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("PID: " + procPID);
            }
        }
		finally
	...
	private static String getProcessIDUnixBased(String procName,
			SSHRemoteExecution remoteExec, Session session)
			throws Exception {
		String cmd = "ps -ef";
		String procPID = "";
		remoteExec.executeCommand(session, cmd);
	...
	private static String getProcessIDWindows(String procName,
			SSHRemoteExecution remoteExec, Session session)
			throws Exception {
	...
	public static String getProcessIDWindows(String procName) throws Exception {
		...
		String[] splits = IOUtils.toString(Runtime.getRuntime().exec(cmd).getInputStream()).split(QueryIOConstants.NEW_LINE);
		String line = null;
		for(int i = 0; i<splits.length; i++){
			line = splits[i];
			if(line.contains(procName))
	...
	public static String getRemoteUserHomeDirectory(Host host, String userName, String password, String sshPrivateKey, int port) throws Exception{
        if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Fetching remote user home directory");
        SSHRemoteExecution remoteExec = null;
        Session session = null;
        ...
        try
        {
            ...
            if (session != null)
            {
                ...
                if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("UserHome directory for " + host.getHostIP() + ", user: " + userName + " is " + userHome);
            }
        }
		finally
        {
	...
	public static boolean validateJavaHome(String hostName, String userName, String password, String sshPrivateKey, String javaHome, String port) throws Exception{
        if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("validating java home");
        SSHRemoteExecution remoteExec = null;
        Session session = null;
        ...
        try
        {
            ...
            if(password != null){
        		...
        		session = remoteExec.createSessionWithPrivateKeyFile(hostName, userName, sshPrivateKey, Integer.parseInt(port));
        	} else {
        		throw new Exception("Session could not be created");
        	}
            ...
            if (session != null)
            {
                ...
                if(output!=null){
                	 if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("output :"+output+" length:"+output.length()+" cmd :"+cmd);
               ...
               if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("JAVA_Version for " + hostName + ", user: " + userName + " for JAVA_HOME " + javaHome+"  validation"+isValidationSuccess+" index :"+output != null ? output.indexOf(javaHome+"/bin/java") : "");
            }
        }
		finally
        {
	...
	public static String getJavaHome(String hostName, String userName, String password, String sshPrivateKey, int port) throws Exception {
		// /System/Library/Frameworks/JavaVM.framework/Versions/CurrentJDK/Home
		
        if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Fetching remote user JAVA_HOME");
        SSHRemoteExecution remoteExec = null;
        Session session = null;
...
//                    remoteExec.executeCommand(session, cmd);
                ...
                if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("JAVA_HOME for " + hostName + ", user: " + userName + " is " + javaHome);
            }
        }
		finally
        {
...
//	    channel.setInputStream(System.in);
	...
	public static boolean checkVersion(String versionInfo) {
		...
		if (version.length > 2) {
			...
			if (parseFloat >= 1.7) {
				if (AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Java version is 1.7 or higher");
				return true;
			}
		} 
...
//        	remoteExec = new SSHRemoteExecution();
//
//       		session = remoteExec.createSession("192.168.0.25", "AppPerfect", "appperfect", 22);
////       		session = remoteExec.createSession("192.168.0.22", "admin", "App4ever#", 22);
//        	
//        	
//        	if (session != null)
//        	{
...
//				String cmd = "cd & cd";
////				cmd = "java -cp " + remoteDir + "UnzipUtil.jar com.queryio.common.UnzipUtil " + (remoteDir + zipFileName) + " " + remoteDir + " && cd " + remoteDir + " && del /F " + (remoteDir + zipFileName);
////				cmd = "cd C:\\Users\\AppPerfect\\QueryIOPackage\\QueryIOAgent\\bin & start /B startQIOAgent.bat C:\\Users\\AppPerfect\\QueryIOPackage\\QueryIOAgent 6680 > help.txt";
////				cmd = "java -cp " + remoteDir + "UnzipUtil.jar com.queryio.common.UnzipUtil " + (remoteDir + zipFileName) + " " + remoteDir;
