...
public class MigrationManager {
	...
	public static String startS3MigrationDS(String nodeId, String title, String dataStore, String s3DataSourceId,
		...
		if (ds != null) {
			return startS3Migration(nodeId, title, dataStore, ds.getAccessKey(), ds.getSecretAccessKey(), bucketName,
					innerKey, hdfsPath, importType, secure, unzip, compressionType, encryptionType);
		} else {
			return "Datasource is not available";
		}
	}

	public static String startS3Migration(String nodeId, String title, String dataStore, String accessKey,
			String secureAccessKey, String bucketName, String innerKey, String hdfsPath, boolean importType,
			boolean secure, boolean unzip, String compressionType, String encryptionType) {
		Connection connection = null;
		try {
			connection = CoreDBManager.getQueryIODBConnection();
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("NodeId: " + node.getId());
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Host: " + host.getHostIP());
			...
			if (coreSiteKeys == null || hdfsSiteKeys == null) {
				// return QueryIOConstants.RETURN_FAILURE + "_Namenode not
				// responding";
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger()
						.debug("String nodeId, String title, String dataStore, String accessKey, String secureAccessKey, String bucketName, String innerKey, String hdfsPath, boolean importType, boolean secure, boolean unzip: "
								+ nodeId + ", " + title + ", " + dataStore + ", " + accessKey + ", " + secureAccessKey
								+ ", " + bucketName + ", " + innerKey + ", " + hdfsPath + ", " + importType + ", "
								+ secure + ", " + unzip);
			...
			if (importType) {
				thread = new AmazonImportMigrationThread(loginUser, loginUserGroup, migrationInfo, accessKey,
						secureAccessKey, conf, secure);
			} else {
				thread = new AmazonExportMigrationThread(loginUser, loginUserGroup, migrationInfo, accessKey,
						secureAccessKey, conf, secure);
			}
	...
	public static boolean resumeS3Migration(int id, String accessKey, String secureAccessKey, boolean overwrite) {
		...
		try {
			...
			if (migrationInfo.isImportType()) {
				thread = new AmazonImportMigrationThread(loginUser, loginUserGroup, migrationInfo, accessKey,
						secureAccessKey, conf, overwrite);
			} else {
				thread = new AmazonExportMigrationThread(loginUser, loginUserGroup, migrationInfo, accessKey,
						secureAccessKey, conf, overwrite);
			}
	...
	public static String startFTPMigrationDS(String ndoeId, String title, String dataStore, String ftpDataSourceId,
		...
		if (ds != null) {
			return startFTPMigration(ndoeId, title, dataStore, ds.getHost() + ":" + ds.getPort(), ds.getUsername(),
					ds.getPassword(), hdfsPath, sourcePath, importType, unzip, compressionType, encryptionType);
		} else {
			return "Datasource is not available";
		}
	}
	...
	public static String startFTPMigration(String nodeId, String title, String dataStore, String hostName,
		...
		try {
			...
			ArrayList values2 = QueryIOAgentManager.getConfig(host, keys2, node, "hdfs-site.xml");
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("values : " + values + "values2 ::" + values2);
			if (values == null || values2 == null) {
				return QueryIOConstants.RETURN_FAILURE + "_Host " + host.getHostIP() + " not responding";
			}
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger()
						.debug("String nodeId, String title, String dataStore, String hostName, String userName, String password, String hdfsPath, String sourcePath, boolean importType, boolean unzip: "
								+ nodeId + ", " + title + ", " + dataStore + ", " + hostName + ", " + userName + ", "
								+ password + ", " + hdfsPath + ", " + sourcePath + ", " + importType + ", " + unzip);
			...
			if (importType) {
				thread = new FTPImportMigrationThread(loginUser, loginUserGroup, migrationInfo, userName, password,
						conf, true);
			} else {
				thread = new FTPExportMigrationThread(loginUser, loginUserGroup, migrationInfo, userName, password,
						conf, true);
			}
			if (migrationThreadMap == null) {
				migrationThreadMap = new HashMap();
	...
	public static String exportToDataBase(String nodeId, String title, String dataStore, String driverClass,
		...
		try {
			...
			AppLogger.getLogger()
					.debug("String nodeId, String title, String dataStore, String driver, String driverJar,  String userName, String password, String hdfsPath, String sourcePath, boolean importType, boolean unzip: "
							+ nodeId + ", " + title + ", " + dataStore + ", " + driverClass + ", " + username + ", "
							+ password + ", " + hdfsPath + ", " + connectionUrl + ", " + false + ", " + "false"
							+ dbdriverjar);

			// MigrationInfoDAO.insert(connection, migrationInfo);
			//
			// migrationInfo = MigrationInfoDAO.get(connection,
			// migrationInfo.getStartTime());
			...
			AppLogger.getLogger()
					.debug("String nodeId, String title, String dataStore, String driver, String driverJar,  String userName, String password, String hdfsPath, String sourcePath, boolean importType, boolean unzip: "
							+ nodeId + ", " + title + ", " + dataStore + ", " + driverClass + ", " + username + ", "
							+ password + ", " + hdfsPath + ", " + connectionUrl + ", " + ", " + "false" + dbdriverjar);

			AppLogger.getLogger().debug("File Path = " + EnvironmentalConstants.getAppHome() + File.separator
					+ QueryIOConstants.DATABASE_DRIVER_JAR_FILE_DIR + File.separator + dbconfig.getJarFileName());
			File file = new File(EnvironmentalConstants.getAppHome() + File.separator
					+ QueryIOConstants.DATABASE_DRIVER_JAR_FILE_DIR + File.separator + dbconfig.getJarFileName());
			...
			new Thread() {
				public void run() {
					final ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(CORE_THREAD_POOL_SIZE, // core
																												// thread
																												// pool
																												// size
							maxThreadConnection, // maximum thread pool size
							maxWaitTimeThread, // time to wait before resizing
												// pool
							TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(MAX_THREAD_POOL_SIZE, FAIR),
					...
					try {
						...
						pcf = new PoolableConnectionFactory(cf, connectionPool, null, null, false, true);
						...
						for (int i = 0; i < fileList.length; i++) {
							Runnable thread;
							try {
								thread = new ExportFromHDFSToDataBase(loginUser, loginUserGroup, mInfo, dataSource,
										respJson, config, fileList[i].getPath());
						...
						while (!complete) {
							...
							AppLogger.getLogger().debug("Id : " + mInfo.getId() + " Migrated " + successCount + " of "
									+ statusList.length + " objects.");
							Thread.sleep(1000);
							if (complete) {
								mInfo.setProgress(progress);
								mInfo.setStatus("Migrated " + successCount + " of " + statusList.length + " objects.");
	...
	public static String startDataBaseMigration(String nodeId, String title, String dataStore, String driverClass,
		...
		try {
			...
			AppLogger.getLogger()
					.debug("String nodeId, String title, String dataStore, String driver, String driverJar,  String userName, String password, String hdfsPath, String sourcePath, boolean importType, boolean unzip: "
							+ nodeId + ", " + title + ", " + dataStore + ", " + driverClass + ", " + username + ", "
							+ password + ", " + hdfsPath + ", " + connectionUrl + ", " + importType + ", " + "false"
							+ dbdriverjar);
			...
			new Thread() {
				public void run() {
					final ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(CORE_THREAD_POOL_SIZE, // core
							...
							maxWaitTimeThread, // time to wait before resizing
												// pool
							TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(maxThreadConnection, FAIR),
							new ThreadPoolExecutor.CallerRunsPolicy());
					GenericObjectPool connectionPool = null;
					try {
						ArrayList<Future<?>> futures = new ArrayList<Future<?>>();
						...
						PoolableConnectionFactory pcf = new PoolableConnectionFactory(cf, connectionPool, null, null,
								false, true);
						...
						for (int i = 0; i < dbconfig.getTableNames().size(); i++) {
							Runnable thread;
							try {
								thread = new ImportFromDataBase(loginUser, loginUserGroup, mInfo, dataSource,
										dbconfig.getTableNames().get(i), conf, uri, true);
								futures.add(threadPoolExecutor.submit(thread));
							} catch (Exception e) {
	...
	public static String startSFTPMigration(String nodeId, String title, String dataStore, String hostName,
		...
		try {
			...
			if (importType) {
				thread = new SFTPImportMigrationThread(loginUser, loginUserGroup, migrationInfo, userName, password,
						conf, true);
			} else {
				thread = new SFTPExportMigrationThread(loginUser, loginUserGroup, migrationInfo, userName, password,
						conf, true);
			}
			if (migrationThreadMap == null) {
				migrationThreadMap = new HashMap();
	...
	public static String startEmailMigration(String nodeId, String title, String dataStore, String dataConnection,
		...
		try {
			...
			// if(AppLogger.getLogger().isDebugEnabled())
			// AppLogger.getLogger().debug("String nodeId, String title, String
			// dataStore, String hostName, String userName, String password,
			// String hdfsPath, String sourcePath, boolean importType, boolean
			// unzip: "
			// + nodeId + ", " + title + ", " + dataStore + ", " + hostName + ",
			// " + userName + ", " + password + ", " + hdfsPath + ", " +
			// sourcePath + ", " + importType + ", " + unzip);
			...
			thread = new EmailImportMigrationThread(loginUser, loginUserGroup, migrationInfo, dataConnection, folder,
					startDate, endDate, prefix, suffix, conf, true);
	...
	public static JSONObject getAllAvailableEmailFolders(String dataConnection) {
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger().debug("Folder List: " + folderList);
	...
	public static String startHTTPMigration(String nodeId, String title, String dataStore, String baseURL,
		...
		try {
			...
			if (migrationInfo.isImportType()) {
				thread = new HTTPImportMigrationThread(loginUser, loginUserGroup, migrationInfo, baseURL, userName,
						password, file, encoding, conf, true);
			} else {
				thread = new HTTPExportMigrationThread(loginUser, loginUserGroup, migrationInfo, userName, password,
						conf, true);
			}
	...
	public static String startHDFSMigration(String nodeId, String title, String dataStore, String hostName, String file,
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger()
						.debug("String nodeId, String title, String dataStore, String hostName, String file, String hdfsPath, boolean importType, boolean unzip: "
								+ nodeId + ", " + title + ", " + dataStore + ", " + hostName + ", " + hdfsPath + ", "
								+ isImportType + ", " + compressionType + ", " + encryptionType);
			...
			if (migrationInfo.isImportType()) {
				thread = new HDFSImportMigrationThread(loginUser, loginUserGroup, migrationInfo, hostName, file, conf,
						true);
			} else {
				thread = new HDFSExportMigrationThread(loginUser, loginUserGroup, migrationInfo, hostName, file, user,
						group, conf, true);
			}
	...
	public static String startSSHMigration(String nodeId, String title, String dataStore, String hostName, String port,
		...
		try {
			...
			if (AppLogger.getLogger().isDebugEnabled())
				AppLogger.getLogger()
						.debug("String nodeId, String title, String dataStore, String hostName, String port, String hdfsPath, boolean importType, boolean unzip: "
								+ nodeId + ", " + title + ", " + dataStore + ", " + hostName + ", " + hdfsPath + ", "
								+ isImportType + ", " + compressionType + ", " + encryptionType);
			...
			if (migrationInfo.isImportType())
				thread = new SSHImportMigrationThread(loginUser, loginUserGroup, migrationInfo, hostName, portInt,
						userName, password, path, isSSH, conf, true);
			else
				thread = new SSHExportMigrationThread(loginUser, loginUserGroup, migrationInfo, hostName, portInt,
						userName, password, path, isSSH, conf, true);
