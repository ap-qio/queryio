	...
	public DataTableParams(String request, int colSize) throws Exception {
		...
		try {
			...
			if (parse != null) {
				...
				if (AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("DataTable params : " + paramsDT.toString());
...
//			
//			this.orderByCol = Integer.parseInt(request.getParameter("order[0][column]"));
//			this.orderByDir = request.getParameter("order[0][dir]");
//			for (int i = 0; i < colSize; i++) {
//				if (StringUtils.isNotEmpty(request.getParameter(searchParamPrefix + i + searchParamSuffix))) {
//					this.searchVal = request.getParameter(searchParamPrefix + i + searchParamSuffix);
...
//			}
			...
			AppLogger.getLogger().error(
					"Error in parsing DataTable pagination detials."
							+ e.getMessage(), e);
			throw e;
		}
	}
	...
	public String toString() {
		return "DataTableParams [draw=" + draw + ", offsetInt=" + offsetInt
				+ ", countInt=" + countInt + ", searchVal=" + searchVal
				+ ", searchColIndex=" + searchColIndex + ", orderByCol="
				+ orderByCol + ", orderByDir=" + orderByDir + "]";
	}
