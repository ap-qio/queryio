...
public class UGIUpdater {
	...
	public static void sendUserInformationUpdates(){
		...
		try
		{
			...
			for(int i=0; i<namenodes.size(); i++){
				...
				String hostIP = HostDAO.getHostDetail(connection, NodeDAO.getNode(connection, nameNodeId).getHostId()).getHostIP();
				...
				url = protocol + hostIP + ":" + ftpServerPort + QueryIOConstants.QUERYIO_FTPSERVER_CONTEXT + QueryIOConstants.USER_INFO_UPDATE_URL;  
				
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Updating userinfo for ftp server [namenode: " + nameNodeId + "], url: " + url);
				
				fireUpdateRequest(url, QueryIOConstants.USER_INFO_REQUEST_HEADER_KEY, SecurityHandler.encryptData(UGIProvider.getUserPasswords()));
				...
				url = protocol + hostIP + ":" + os3ServerPort + QueryIOConstants.QUERYIO_OS3SERVER_CONTEXT + QueryIOConstants.USER_INFO_UPDATE_URL;
				
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Updating userinfo for os3 server [namenode: " + nameNodeId + "], url: " + url);
	...
	public static void sendUserGroupInformationUpdates(){
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Sending user-group information to all namenodes...");
		Connection connection = null;
...
//				Configuration conf = RemoteManager.getNameNodeConfiguration(nameNodeId);
//				
//				String httpAddress = conf.get(DFSConfigKeys.DFS_NAMENODE_HTTP_ADDRESS_KEY + "." + nameNodeId);
//				if(httpAddress.endsWith("/")) httpAddress = httpAddress.substring(0, httpAddress.length()-1);
//				
...
//				}
//				
//				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("UGI Update URL for " + httpAddress + ": " + httpAddress);
//				
//				try{
//					fireUpdateRequest(httpAddress, QueryIOConstants.GROUP_INFO_REQUEST_HEADER_KEY, SecurityHandler.encryptData(UGIProvider.getUserGroupInformation()));
//				}catch(Exception e){
//					AppLogger.getLogger().fatal("updateUserInformation() failed with exception: " + e.getMessage(), e);
//				}
				
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Request UserGroup Mapping Request for Name Node Id " + nameNodeId);
				...
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("UGI updates sent to " + nameNodeId);
	...
	public static void fireUpdateRequest(String remoteURL, String headerKey, String headerValue) throws Exception
	{
		...
		try{
			...
			if(httpCon.getResponseCode() != HttpStatus.SC_OK){
				throw new Exception ("Improper response code received, code: " + httpCon.getResponseCode() + ", message: " + httpCon.getResponseMessage());
			}
		}finally{
			if(httpCon != null)
				httpCon.disconnect();
