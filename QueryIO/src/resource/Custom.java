...
public class Custom {
	...
	public static void main(String arg[]) {
		...
		try {
			...
			////
			//// config.set(DFSConfigKeys.DFS_BLOCK_ACCESS_TOKEN_ENABLE_KEY,
			// "true");
			//// config.set(DFSConfigKeys.HADOOP_SECURITY_AUTHENTICATION,
			// "kerberos");
			//// config.set(DFSConfigKeys.HADOOP_SECURITY_AUTHORIZATION,
			// "true");
			//// config.set(DFSConfigKeys.DFS_NAMENODE_KEYTAB_FILE_KEY,
			// "/AppPerfect/Keytabs/eshan12.keytab");
			//// config.set(DFSConfigKeys.HADOOP_SECURITY_SERVICE_USER_NAME_KEY,
			// "hdfs/192.168.0.12");
			//// config.set("dfs.namenode.kerberos.principal",
			// "hdfs/192.168.0.12@queryiorealm");
			...
			// dfs.getFileStatus(path).getOwner());
			// System.out.println("Group: " +
			// dfs.getFileStatus(path).getGroup());
			// System.out.println("Owner: " +
			// dfs.getFileStatus(path).getPermission().toString());
