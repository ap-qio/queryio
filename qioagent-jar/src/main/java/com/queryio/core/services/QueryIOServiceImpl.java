...
public class QueryIOServiceImpl implements QueryIOService {
	...
	public QueryIOResponse startNode(String installDir, String nodeType,
			String nodeId) {
		return NodeOperation.startNode(installDir, nodeType, nodeId);
	}

	public QueryIOResponse stopNode(String installDir, String nodeType,
			String nodeId) {
	...
	public QueryIOResponse transitionNodeToActive(String installDir,
			String nodeId, String activeNode) {
	...
	public QueryIOResponse clearConfiguration(String installDir,
			String nodeType, String nodeId) {
		return NodeOperation.clearConfiguration(installDir, nodeType, nodeId);
	}
	public QueryIOResponse updateConfiguration(String installDir, String nodeType,
			String nodeId, String configFileName, ArrayList property,
			ArrayList value, boolean refresh) {
		return NodeOperation.updateConfiguration(installDir, nodeType, nodeId,
				configFileName, property, value, refresh);
	}

	public ArrayList getConfiguration(String installDir, String nodeType,
			String nodeId, String configFileName, ArrayList property) {
		return NodeOperation.getConfiguration(installDir, nodeType, nodeId,
				configFileName, property);
	}

	public String[] getAllConfiguration(String installDir, String nodeType,
			String nodeId, String configFileName) {
		return NodeOperation.getAllConfiguration(installDir, nodeType, nodeId,
				configFileName);
	}
	...
	public QueryIOResponse updateHostsList(String installDir, String nodeId,
			String[] datanodeAdds) {
		return NodeOperation.updateHostsList(installDir, nodeId, datanodeAdds);
	}

	public QueryIOResponse updateHostsExcludeList(String installDir, String nodeId,
			String[] datanodeAdds) {
	...
	public QueryIOResponse setSafemode(String installDir, String nodeId,
			boolean safemode) {
	...
	public ArrayList validateVolumeDiskMapping(ArrayList disks,
			ArrayList volumes) {
	...
	public QueryIOResponse updateNetworkConfig(String installDir, String nodeId,
			String[] hostIps, String[] rackNames) {
		return NodeOperation.updateNetworkConfig(installDir, nodeId, hostIps,
				rackNames);
	}
	...
	public QueryIOResponse performFailover(String installDir, String nodeId,
			String failoverArg) {
		return NodeOperation.performFailover(installDir, nodeId, failoverArg);
	}

	public QueryIOResponse updateHadoopEnv(String installDir, String nodeType,
			String nodeId) {
		return NodeOperation.updateHadoopEnv(installDir, nodeType, nodeId);
	}

	public QueryIOResponse updateYarnEnv(String installDir, String nodeType,
			String nodeId) {
		return NodeOperation.updateYarnEnv(installDir, nodeType, nodeId);
	}

	public QueryIOResponse copySharedDirLogstoEditsLogs(String installDir,
			String nodeId) {
	...
	public QueryIOResponse runJob(String installDir, String nodeId,
			String jobName, String jarName, String libJars, String files,
			String className, String arguments, String hdfsUri,
			String dbSourceId, String analyticsDbName, String encryptionKey, boolean isRecursive, boolean isFilterApply, String filterQuery, boolean deleteJobFile) {
		return MapRedOperation.runJob(installDir, nodeId, jobName, jarName,
				libJars, files, className, arguments, hdfsUri, dbSourceId, analyticsDbName, encryptionKey, isRecursive, isFilterApply, filterQuery, deleteJobFile);
	}
	
	public QueryIOResponse runJobDataTagging(String installDir, String nodeId,
			String jobName, String jarName, String libJars, String files,
			String className, String arguments, String hdfsUri,
			String dbSourceId, String encryptionKey, String fileTypeParser) {
		return MapRedOperation.runJobDataTagging(installDir, nodeId, jobName, jarName,
				libJars, files, className, arguments, hdfsUri, dbSourceId, encryptionKey, fileTypeParser);
	}
	...
	public QueryIOResponse createJarCopyStream(String installDir,
			String jarFileName) {
		return MapRedOperation.createJarCopyStream(installDir, jarFileName);
	}

	public QueryIOResponse writeJarToStream(String jarName, byte[] b, int offset,
			int length) {
	...
	public QueryIOResponse closeStream(String installDir, String relativePath, String jobName,
			boolean unzip) {
		return MapRedOperation.closeStream(installDir, relativePath, jobName, unzip);
	}

	public QueryIOResponse writeToStream(String fileName, byte[] b, int offset,
			int length) {
		return MapRedOperation.writeToStream(fileName, b, offset, length);
	}

	public QueryIOResponse unsetConfiguration(String installDir, String nodeType,
			String nodeId, String configFileName, ArrayList property) {
		return NodeOperation.unsetConfiguration(installDir, nodeType, nodeId,
				configFileName, property);
	}
	...
	public QueryIOResponse createDBConfigCopyStream(String installDir,
			String fileName) {
		return MapRedOperation.createDBConfigCopyStream(installDir, fileName);
	}

	public QueryIOResponse writeDBCOnfigFile(String fileName, byte[] b,
			int offset, int length) {
		return MapRedOperation.writeDBCOnfigFile(fileName, b, offset, length);
	}

	public QueryIOResponse closeDBConfigCopyStream(String installDir,
			String fileName) {
		return MapRedOperation.closeDBConfigCopyStream(installDir, fileName);
	}

	public boolean hasMapping(String hostName, String hostAddress)
			throws IOException {
		File file = null;
		...
		try {
			...
			while ((strLine = br.readLine()) != null) {
				...
				if ((!strLine.startsWith("#")) && strLine.contains(hostName)
						&& strLine.contains(hostAddress))
					return true;
			}
		} catch(Exception e) {
	...
	public boolean isReachable(String hostname){
		try{
			...
			if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Host " + hostname + " is not reachable from this machine");
		}
	...
	public QueryIOResponse deleteFile(String installDir, String folderName,
			String fileName) {
	...
	@Override
	public QueryIOResponse checkPortAvailability(String installDir,
			String nodeType, String nodeId) {
	...
	public void updateJavaHome(String installDir, String javaHomePath) throws Exception {
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("update Java home QueryIOServiceImpl");
	...
	public void updateLoggerPropertiesFile(String installDir) throws Exception {
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("update logger properties QueryIOServiceImpl");
		NodeOperation.updateLoggerPropertiesFile(installDir);
		
	}
	
	public void updateHiveHadoopHome(String installDir) throws Exception {
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("update Hadoop home QueryIOServiceImpl");
