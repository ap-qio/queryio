...
public class DateColSubtractIntervalDayTimeScalar extends VectorExpression {
  ...
  public DateColSubtractIntervalDayTimeScalar(int colNum, long value, int outputColumn) {
    this.colNum = colNum;
    this.value = (value);
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      outputVector[0] = TimestampUtils.daysToNanoseconds(vector[0]) - value;
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector[i]) - value;
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector[i]) - value;
