...
public class TimestampScalarSubtractIntervalYearMonthColumn extends VectorExpression {
  ...
  public TimestampScalarSubtractIntervalYearMonthColumn(long value, int colNum, int outputColumn) {
    this.colNum = colNum;
    this.value = (value);
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector.isRepeating) {
      outputVector[0] = dtm.addMonthsToNanosUtc(value, - (int) vector[0]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = dtm.addMonthsToNanosUtc(value, - (int) vector[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = dtm.addMonthsToNanosUtc(value, - (int) vector[i]);
