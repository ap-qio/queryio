...
public class DateColSubtractDateColumn extends VectorExpression {
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) { 
      outputVector[0] = TimestampUtils.daysToNanoseconds(vector1[0]) - TimestampUtils.daysToNanoseconds(vector2[0]);
    } else if (inputColVector1.isRepeating) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector1[0]) - TimestampUtils.daysToNanoseconds(vector2[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector1[0]) - TimestampUtils.daysToNanoseconds(vector2[i]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector1[i]) - TimestampUtils.daysToNanoseconds(vector2[0]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector1[i]) - TimestampUtils.daysToNanoseconds(vector2[0]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector1[i]) - TimestampUtils.daysToNanoseconds(vector2[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = TimestampUtils.daysToNanoseconds(vector1[i]) -  TimestampUtils.daysToNanoseconds(vector2[i]);
