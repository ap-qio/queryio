...
public class IntervalYearMonthColAddTimestampColumn extends VectorExpression {
  ...
  public void evaluate(VectorizedRowBatch batch) {
    ...
    if (inputColVector1.isRepeating && inputColVector2.isRepeating) { 
      outputVector[0] = dtm.addMonthsToNanosUtc((vector2[0]), + (int) vector1[0]);
    } else if (inputColVector1.isRepeating) {
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = dtm.addMonthsToNanosUtc((vector2[0]), + (int) vector1[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = dtm.addMonthsToNanosUtc((vector2[0]), + (int) vector1[i]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = dtm.addMonthsToNanosUtc((vector2[i]), + (int) vector1[0]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = dtm.addMonthsToNanosUtc((vector2[i]), + (int) vector1[0]);
      ...
      if (batch.selectedInUse) {
        for(int j = 0; j != n; j++) {
          int i = sel[j];
          outputVector[i] = dtm.addMonthsToNanosUtc((vector2[i]), + (int) vector1[i]);
        }
      } else {
        for(int i = 0; i != n; i++) {
          outputVector[i] = dtm.addMonthsToNanosUtc((vector2[i]), + (int) vector1[i]);
