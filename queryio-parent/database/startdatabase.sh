...
if [ "$#" -eq 2 ]; then
	...
	$JAVA_HOME/bin/java -Xms256m -Xmx2048m org.hsqldb.Server -port $1 -database.0 $QUERYIO_DATABASE -dbname.0 $2  
elif [ "$#" -eq 3 ]; then
	export QUERYIO_DATABASE1=$DB_HOME/$2
	export QUERYIO_DATABASE2=$DB_HOME/$3
	$JAVA_HOME/bin/java -Xms256m -Xmx2048m org.hsqldb.Server -port $1 -database.0 $QUERYIO_DATABASE1 -dbname.0 $2 -database.1 $QUERYIO_DATABASE2 -dbname.1 $3
else
