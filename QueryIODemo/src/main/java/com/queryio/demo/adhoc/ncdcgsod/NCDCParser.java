		...
		public void parseLine(String line) throws IOException, InterruptedException, SQLException {
			...
			while (token.hasMoreTokens()) {
				...
				if (index == 2) {
					LOG.info("DATE: " + value);
					try {
						this.entry.addValue(i++, value.substring(0, 4));
					} catch (Exception e) {
						LOG.fatal("DATE: " + value + ", Error: " + e.getMessage(), e);
					} // Year
					try {
						this.entry.addValue(i++, value.substring(4, 6));
					} catch (Exception e) {
						LOG.fatal("DATE: " + value + ", Error: " + e.getMessage(), e);
					} // Month
					try {
						this.entry.addValue(i++, value.substring(6, value.length()));
					} catch (Exception e) {
						LOG.fatal("DATE: " + value + ", Error: " + e.getMessage(), e);
					} // Day
				} else if (index == 17) {
					this.entry.addValue(i++, value.replace("*", ""));
					this.entry.addValue(i++,
					...
					this.entry.addValue(i++, value.substring(0, 1).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // fog
					this.entry.addValue(i++, value.substring(1, 2).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // rain
					this.entry.addValue(i++, value.substring(2, 3).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // snow
					this.entry.addValue(i++, value.substring(3, 4).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // hail
					this.entry.addValue(i++, value.substring(4, 5).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // thunder
					this.entry.addValue(i++, value.substring(5, 6).equals("1") ? String.valueOf(Boolean.TRUE)
