...
public class CSVParser implements IAdHocParser {
	...
	// {
	// dataType =
	// props.getTypeMap().get(MetadataConstants.STATIC_DATATYPES_TO_WRAPPER_MAP.get(MetadataConstants.GENERIC_DATA_TYPE_STRING));
	// size = Integer.parseInt(type.substring(type.indexOf("(") + 1,
	// type.lastIndexOf(")")));
	...
	// {
	// dataType =
	// props.getTypeMap().get(MetadataConstants.STATIC_DATATYPES_TO_WRAPPER_MAP.get(type));
	// return dataType;
	// }
	// }
	...
	// ("SMALLINT".equalsIgnoreCase(colType)))
	...
	// ("DECIMAL".equalsIgnoreCase(colType)) ||
	// ("NUMERIC".equalsIgnoreCase(colType)))
