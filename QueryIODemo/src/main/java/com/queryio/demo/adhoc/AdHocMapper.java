...
public class AdHocMapper extends Mapper<IntWritable, List<FileStatus>, FileStatus, AdHocEntry> {
	...
	public void map(IntWritable noOfFiles, List<FileStatus> fileStatuses, final Context context)
		...
		try {
			...
			IAdHocParser parser = (IAdHocParser) ReflectionUtils.newInstance(parserClass, context.getConfiguration());
			...
			for (int i = 0; i < maxThreadCount; i++) {
				...
				ParserThread thread = new ParserThread(dbName, parser, fileStatusList, streamList, tableName,
						maxBatchSize);
				thread.start();
				threads.add(thread);
			}
	...
	class ParserThread extends Thread {
		...
		ParserThread(String dbName, IAdHocParser parser, ArrayList<FileStatus> fileStatusList,
