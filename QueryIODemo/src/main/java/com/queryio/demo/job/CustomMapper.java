...
public class CustomMapper extends
		Mapper<List<FileStatus>, List<InputStream>, FileStatus, DBEntry> {
	private static final Log LOG = LogFactory.getLog(CustomMapper.class);

	public void map(List<FileStatus> fileStatuses, List<InputStream> streams,
			Context context) throws IOException, InterruptedException {
		try {
			UserDefinedTagResourceFactory.initConnectionPool(context
		...
		public void run() {
			...
			try {
				connection = UserDefinedTagResourceFactory
						.getConnectionWithPoolInit(context.getConfiguration(), true);				
					...
					try
					{
						IDataTagParser parser = UserDefinedTagResourceFactory
								.getParserFromConstructor(
										context.getConfiguration(), filePath,
										null, null);
						
						List<UserDefinedTag> tags = UserDefinedTagUtils
								.generateDefaultTags(fileStatus);
						...
						tags.add(new UserDefinedTag(TableMetadata.DEFAULT_TAG_COMPRESSION_TYPE, customStream.getCompressionType()));
						if(customStream.getEncryptionType()!= null && !customStream.getEncryptionType().isEmpty())
						tags.add(new UserDefinedTag(TableMetadata.DEFAULT_TAG_ENCRYPTION_TYPE, customStream.getEncryptionType()));
						if (parser != null) {
							LOG.info("Using Parser: " + parser.getClass()
									+ " for file: " + filePath);
//							String expression = context.getConfiguration().get(QueryIOConstants.CUSTOM_TAG_PARSER_EXPRESSION);
//							if (expression != null) {
//								parser.setFilterExpression(expression);
//							}
							parser.parseStream(stream,
									UserDefinedTagUtils.getFileExtension(filePath));
							if (parser.getCustomTagList() != null
									&& parser.getCustomTagList().size() > 0) {
								tags.addAll(parser.getCustomTagList());
							}
						}
						String tableName = UserDefinedTagResourceFactory.getTableName(
								parser, filePath);
						DBTypeProperties props = CustomTagDBConfigManager.getDatabaseDataTypeMap(dbName, null);
						UserDefinedTagDAO.insertTagValues(
								connection,
								props,
								tableName,
								filePath,
								...
								parser == null ? new TableMetadata("DEFAULT", null)
										: parser.getTableMetaData(UserDefinedTagUtils
