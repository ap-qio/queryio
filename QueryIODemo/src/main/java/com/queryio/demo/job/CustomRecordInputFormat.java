...
public class CustomRecordInputFormat extends FileInputFormat<LongWritable, Text> {
	...
	private FileStatus checkIfCompressedEncrypted(FileSystem fs, LocatedFileStatus stat) {
		try {
			...
			metadata = getObjectMetadata(context.getConfiguration(), stat.getPath().toString(),
					TableConstants.TABLE_HDFS_METADATA);

			if (metadata == null) {
				metadata = getObjectMetadata(context.getConfiguration(), stat.getPath().toString(),
						("DATATAGS_" + UserDefinedTagUtils.getFileExtension(stat.getPath().toString())).toUpperCase());
			}
