...
public class CustomLineRecordReader extends RecordReader<LongWritable, Text> {
	 ...
	 * tasks assigned InputSplit and TaskAttemptContext, and prepares the record
	 * reader. For file-based input formats, this is a good place to seek to the
	 * byte position in the file to begin reading.
	 */
	@Override
	public void initialize(InputSplit genericSplit, TaskAttemptContext context) throws IOException {
		...
		/*
		 * if (skipFirstLine) { Text dummy = new Text(); // Reset "start" to
		 * "start + line offset" start += in.readLine(dummy, 0, (int) Math.min(
