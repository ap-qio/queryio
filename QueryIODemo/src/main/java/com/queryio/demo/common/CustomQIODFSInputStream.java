...
public class CustomQIODFSInputStream extends InputStream {
	...
	private static final String QUERY_START = "SELECT " + CustomEncryptionHandler.COL_COMPRESSION_TYPE + ","
			+ CustomEncryptionHandler.COL_ENCRYPTION_TYPE + " FROM ";
	...
	public CustomQIODFSInputStream(InputStream dfsIn, Configuration conf, String filePath) throws Exception {
		...
		if (conf.get(QueryIOConstants.CUSTOM_TAG_DB_DBSOURCEID) == null) {
			in = new CustomEncryptionHandler(Cipher.DECRYPT_MODE, false,
			...
			if (metadata == null) {
				in = new CustomEncryptionHandler(Cipher.DECRYPT_MODE, false,
						conf.get(QueryIOConstants.QUERYIO_DFS_DATA_ENCRYPTION_KEY))
								.getCompressedCipherInputStream(dfsIn, CustomEncryptionHandler.COMPRESSION_TYPE_NONE);
			} else {
				...
				in = new CustomEncryptionHandler(Cipher.DECRYPT_MODE, encryptionType,
						conf.get(QueryIOConstants.QUERYIO_DFS_DATA_ENCRYPTION_KEY))
