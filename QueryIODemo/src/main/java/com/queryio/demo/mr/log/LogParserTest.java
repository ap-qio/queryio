...
public class LogParserTest {
	public static void main(String[] args) throws SQLException, IOException, InterruptedException{
		...
		try{
			...
			LogParser parser = new LogParser(connection, "DATA_LOG", "%d{dd MMM,HH:mm:ss:SSS} [%t] [%c] [%x] [%X] [%p] [%l] [%r] %C{3} %F-%L [%M] - %m%n", searchString, startDate, endDate, status, 100);
			parser.parse(is);
		} finally {
			try{
				if(is!=null)	is.close();
...
//		BasicConfigurator.configure();
//		
//		FileInputStream stream = new FileInputStream(new File("/AppPerfect/MESSAGE"));
//		  try {
...
//		    System.out.println("MATCHES: " + content.matches("NullPointerException"));
//		  }
//		  finally {
//		    stream.close();
//		  }
