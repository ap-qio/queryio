...
public class GenericParserJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		LOG.info(QueryIOConstants.DATATAGGING_GENERIC_FILE_TYPES_PARSERS_KEY + ": "
				+ conf.get(QueryIOConstants.DATATAGGING_GENERIC_FILE_TYPES_PARSERS_KEY));
		...
		if (fpJSON != null) {
			...
			while (it.hasNext()) {
				...
				// if
				// (valueOf.equalsIgnoreCase(QueryIOConstants.ADHOC_TYPE_IISLOG)
				// ||
				// valueOf.equalsIgnoreCase(QueryIOConstants.ADHOC_TYPE_ACCESSLOG))
	...
	private static class MultiPathFilter extends Configured implements PathFilter {
		...
		public void setConf(Configuration conf) {
			try {
				...
				if (conf != null) {
					...
					// PathFilter jobFilter =
					// FileInputFormat.getInputPathFilter(job);
					// if (jobFilter != null) {
		...
		public boolean accept(Path path) {
			try {
				...
				LOG.info("File: " + path + ", Modify: " + fileStatus.getModificationTime() + ", startTime: " + startTime
						+ ", EndTime : " + endTime);
