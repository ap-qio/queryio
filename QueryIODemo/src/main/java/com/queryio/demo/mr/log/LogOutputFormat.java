...
public class LogOutputFormat extends OutputFormat<FileStatus, LogEntry> {
	...
	class LogRecordWriter extends RecordWriter<FileStatus, LogEntry> {
		...
		public LogRecordWriter(Configuration conf) throws Exception {
			...
			// this.conf = conf;
			// connection =
			// UserDefinedTagResourceFactory.getConnectionWithPoolInit(this.conf);
		...
		@Override
		public void write(FileStatus fileStatus, LogEntry logEntry) throws IOException, InterruptedException {
			...
			//
			// try {
			// UserDefinedTagDAO.insertTagValues(connection, this.tableName,
			// fileStatus.getPath().toUri().getPath(), tags, true, false);
			// } catch (SQLException e) {
			// LOG.fatal(e.getMessage(), e);
			// }
