...
public class LogParserJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length < 1) {
			System.err.printf(
					"Usage: %s [generic options] <input-folder> [<result-table-name>] [<search-string>] [<log-pattern>] [<start-time-ms>] [<end-time-ms>]\n\n",
		...
		if (args.length >= 4) {
			...
			conf.set("log-pattern",
					"%d{dd MMM,HH:mm:ss:SSS} [%t] [%c] [%x] [%X] [%p] [%l] [%r] %C{3} %F-%L [%M] - %m%n");
		}
