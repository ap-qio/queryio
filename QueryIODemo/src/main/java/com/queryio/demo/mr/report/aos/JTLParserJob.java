...
public class JTLParserJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length < 1) {
			System.err.printf(
					"Usage: %s [generic options] <input-folder> [<result-table-name>] [<sample-duration>] [start-timestamp-index] [label-index] [error-count-index] [bytes-index] [latency-index] [start-time] [end-time] [wait-time-for-order-status]\n\n",
