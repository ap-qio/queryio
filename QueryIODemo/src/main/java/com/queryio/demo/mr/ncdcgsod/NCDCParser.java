...
public class NCDCParser {
	...
	public NCDCParser(NCDCExpressions ncdcExpressions, Connection connection, String tableName, FileStatus fileStatus,
			int maxBatchSize) {
		...
		if (columns != null) {
			...
			while (token.hasMoreTokens()) {
				...
				if (index == 2) {
					LOG.info("DATE: " + value);
					try {
						this.entry.addValue(value.substring(0, 4));
					} catch (Exception e) {
						LOG.fatal("DATE: " + value + ", Error: " + e.getMessage(), e);
					} // Year
					try {
						this.entry.addValue(value.substring(4, 6));
					} catch (Exception e) {
						LOG.fatal("DATE: " + value + ", Error: " + e.getMessage(), e);
					} // Month
					try {
						this.entry.addValue(value.substring(6, value.length()));
					} catch (Exception e) {
						LOG.fatal("DATE: " + value + ", Error: " + e.getMessage(), e);
					} // Day
				} else if (index == 17) {
					...
					this.entry.addValue(
							value.contains("*") ? String.valueOf(Boolean.TRUE) : String.valueOf(Boolean.FALSE));
				} else if (index == 18) {
					this.entry.addValue(value.replace("*", ""));
					this.entry.addValue(
							value.contains("*") ? String.valueOf(Boolean.TRUE) : String.valueOf(Boolean.FALSE));
				} else if (index == 21) {
					this.entry.addValue(value.substring(0, 1).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // fog
					this.entry.addValue(value.substring(1, 2).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // rain
					this.entry.addValue(value.substring(2, 3).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // snow
					this.entry.addValue(value.substring(3, 4).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // hail
					this.entry.addValue(value.substring(4, 5).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // thunder
					this.entry.addValue(value.substring(5, 6).equals("1") ? String.valueOf(Boolean.TRUE)
							: String.valueOf(Boolean.FALSE)); // tornado
				} else {
					if (value.contains(".")) {
						if (value.endsWith("E"))
							value = value + "+0";
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (column.equalsIgnoreCase(COL_TAG_VALUES_FILEPATH))
					continue;
