...
public class SystemMonitor 
{
    ...
    public static String startMonitorWrapper()
    {
		...
		try
		{
			...
			{
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("OS not supported for Network and Disk I/O");
		...
		{
			AppLogger.getLogger().fatal("Unix Monitor for Network and Disk I/O failed with Exception: " + e.getMessage(), e);
			return 	QueryIOConstants.RETURN_FAILURE + "_Unix Monitor for Network and Disk I/O failed.";
		}
		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Unix Monitor for Network and Disk I/O started successfully.");
    ...
    public static void startMonitoring() throws Exception {
    	if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Monitoring started");
...
//    	windowMonitor.ping("localhost");
    ...
    public static void stopMonitoring() {
    	if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Monitoring stopped");
...
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Monitoring request " + reqId + " reveived");
		...
		try
		{
			...
			else if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("OS not supported"); 
			...
			{
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Network Info could not be obtained"); 
			}			
		}
		catch(Exception e)
		{
...
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("isMacOS(): " + isMacOS(OS_NAME));
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("recdPacketsPerSec: " + recdPacketsPerSec);
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("sentPacketsPerSec: " + sentPacketsPerSec);
...
//		if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Monitoring request " + reqId + " returned at " + System.currentTimeMillis());
...
//				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Time remaining for monitor thread " + Thread.currentThread().getName() + " is " + (5-diff+1) + " seconds.");
			...
			if(monitorThread.isAlive()) {
				AppLogger.getLogger().fatal("Monitor thread for request " + Thread.currentThread().getName() + " timed out...interrupting monitor thread at " + System.currentTimeMillis());
				
				interrupted = true;
				
				monitorThread.interrupt();
			} else {
				if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Monitor thread " + Thread.currentThread().getName() + " ok");
			}
		}
		catch(Exception e)
		{
		...
		{
			if(interrupted) {
				AppLogger.getLogger().fatal("Monitor thread " + Thread.currentThread().getName() + " timed out...Disconnecting system monitor");
...
//					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Detected CPU Usage: " + cpuUssage);
...
//					if(AppLogger.getLogger().isDebugEnabled()) AppLogger.getLogger().debug("Detected CPU Usage: " + cpuUssage);
		...
		{
			/*if(args.length != 1) {
				System.err.println("USAGE : java SystemMonitor /path/to/QueryIO_services");
				return;
			}	*/
